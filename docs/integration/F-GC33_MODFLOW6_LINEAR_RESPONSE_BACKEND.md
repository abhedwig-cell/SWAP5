> Authority reconciliation: historical F-GC33 was owner-qualified against the provisional MultiSWAP cell-response identity F-GC31. Canonical F-GC31 is now the active-drainage tangent and the MultiSWAP cell response is canonically admitted as F-GC40. The F-GC33 production transform below is unchanged; only its upstream capability identity is corrected.
...
F-GC33 converts one already-qualified F-GC40 MultiSWAP groundwater-cell affine response into the linear boundary coefficients used by MODFLOW6.
...
## 1. Inputs from F-GC40

For one groundwater cell, F-GC40 provides
...
That sign matches F-GC40's positive outward-from-SWAP exchange.

## 3. Exact coefficient transform

Convert the F-GC40 areal flux to MODFLOW volume flux in m3/day:
...
Compose the same underlying F-GC40 tile responses around two different valid `H_ref` values.
...
- change F-GC30/F-GC31/F-GC40 science;
