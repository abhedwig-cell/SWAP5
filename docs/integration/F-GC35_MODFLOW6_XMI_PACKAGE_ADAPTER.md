> Authority reconciliation: F-GC35 is rematerialized from current canonical after F-GC34 closeout. The Python production adapter is byte-identical to the historically owner-qualified implementation. F-GC34 is now canonical authority for typed package publication; F-GC35 remains limited to XMI address/pointer acquisition and publication-window timing.
