> Authority reconciliation: F-GC36 is rematerialized from current canonical after F-GC34 and F-GC35 closeout. Both production bridge blobs are byte-identical to the historical owner-qualified live bridge. The live MODFLOW6 qualification is replayed because its runtime evidence depends on the current canonical adapter stack.
...
F-GC36 consumes the now-canonical closed authorities unchanged:

- F-GC33 `modflow6_linear_boundary_term_t` (closed canonical);
- F-GC34 `publish_modflow6_api_terms` (closed canonical);
- F-GC35 `Modflow6XmiPackageAdapter` (closed canonical).
