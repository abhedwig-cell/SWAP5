> Authority reconciliation: this rematerialization is based on closed canonical F-GC38. F-GC37 remains authoritative for the ownership boundary: predictor/corrector strategy belongs to the internal SWAP5–MODFLOW coupling service below iMOD Coupler. F-GC39 supersedes only F-GC37's groundwater per-iteration rollback abstraction, replacing it with the closed canonical F-GC38 prepared-solve semantics.

# F-GC39 — Prepared-solve internal SWAP5–MODFLOW coupling service contract

## Status

**CURRENT-CANONICAL OWNER REQUALIFICATION — contract/harness only; no production delta**
...
This canonical state contains closed F-GC40, F-GC33, F-GC34, F-GC35, F-GC36 and F-GC38 authority.
