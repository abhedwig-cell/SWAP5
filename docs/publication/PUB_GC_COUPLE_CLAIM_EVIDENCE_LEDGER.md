...
5. **E5 NEXT — oracle/IQN information-value falsification.** Compare fixed point/relaxation, credible IQN/Anderson, current `u_A`-informed coupling and a zero-cost `J_R` oracle at equal physical/coupling tolerances using total equivalent SWAP work and convergence domain.
