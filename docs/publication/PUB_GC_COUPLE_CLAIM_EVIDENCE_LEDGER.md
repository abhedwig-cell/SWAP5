...
| GC-C04 | Coupled convergence requires both groundwater nonlinear convergence and SWAP-groundwater exchange consistency. | Generic coupled-residual criteria are established; hydrological specialization requires evidence. | F-GC39, F-GC44, PUB-GC E3a | E3a shows MODFLOW-only one-pass solutions can miss the coupled interface criterion by 11–747x while strong coupling closes it in 2–4 correctors; broader hydrological materiality still requires stress regimes. | SUPPORTED_RESTRICTED |
...
| GC-C09 | F-GC30/F-GC44 response information has a clear physical relation to storage response J_S and actual exchange response J_R. | Dynamic storage response is established in MetaSWAP and transient-specific-yield literature. | Response infrastructure plus adverse frozen-affine E3a result | Direct u_FD vs J_S vs J_R characterization; derivative plateau, linearity radius and balance closure. E3a shows the frozen affine predictor response is not a safe proxy for the actual corrector response. | HYPOTHESIS — PRIORITY |
...
| GC-C11 | A weak-coupling regime exists in which sophisticated acceleration is unnecessary. | Schüller et al. 2025 makes this a serious null hypothesis, not novelty. | F-GC44 plus PUB-GC E3a | E3a shows nanometre-scale head corrections despite interface residuals 11–389x above tolerance; a stronger hydrological regime is still required before generalizing. | SUPPORTED_RESTRICTED |
...
3. **E3a closed — SUPPORTED_RESTRICTED** for one-window coupling consistency versus groundwater buffering; current case is hydrologically weak.
4. **E3b NEXT** — expand the hydrological/window envelope until a material state/exchange correction appears or a bounded failure is reached.
5. **E4 PRIORITY** — response identity u_FD vs J_S vs J_R, motivated directly by the adverse frozen-affine E3a result.
6. E5 oracle/IQN information-value test only after E4.
7. E6 broader hydrological stress extension.
8. E7 realistic case.
9. E8 scaling only after the scientific core is secure.
...
`PUB_GC_E3_RESULT.csv`

E3a's principal adverse result is that a frozen initial affine predictor response increased the one-pass interface residual in every successful matrix case. This result is retained as evidence and is not to be reinterpreted as an acceleration benefit.
