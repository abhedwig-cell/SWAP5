...
The SWAP5 coupling implementation, experiment definitions, machine-readable publication evidence and reproduction material are maintained in the public `abhedwig-cell/SWAP5` development repository. The exact SWAP5 publication version, persistent archive identifier and applicable software licence are intentionally not stated here until the corresponding release, redistribution and archive authorities are formally closed. The final submitted manuscript must replace this development-repository wording with the governed exact-version archive citation and licence statement.

Publication-specific evidence for E1–E7 is retained under `docs/publication/` and `docs/publication/evidence/`. This includes the preregistered experiment definitions, result records, workflow provenance, figure-generation material, supplementary methods and the governed E7 standalone-selection/component-domain result. Each reported experiment is tied to a frozen repository state and, where applicable, a recorded GitHub Actions run. Diagnostic publication experiments do not alter production physics or numerical tolerances.

The live groundwater experiments use MODFLOW 6.8.0. The historical SWAP 4.3.1 distribution used as an external reference asset is not redistributed through the SWAP5 repository. Its exact identity is SHA-256 `2b48353db6cdf00246a1e5c0dcaafc2c61858729fad18446a1dc66359ec2a360`, size 8,959,314 bytes. Public hash-anchored evidence derived from that asset is retained in the publication package; editor and reviewer access to the external distribution must follow the controlling redistribution authority.

A journal-neutral supplementary package freezes notation, workflow provenance and machine-readable evidence bindings through the closed E7 component-domain result. The controlling repository assets are `PUB_GC_NOTATION_AND_UNITS.md`, `PUB_GC_SUPPLEMENTARY_METHODS_AND_EVIDENCE.md` and `PUB_GC_REPRODUCIBILITY_MANIFEST.json`. Final GMD submission is blocked only on the governed SWAP5 publication identifier, explicit software-licence/redistribution authority and a persistent exact-version archive with a unique identifier.
