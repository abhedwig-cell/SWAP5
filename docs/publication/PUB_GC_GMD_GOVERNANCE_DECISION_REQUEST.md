...
- must not conflict with an existing SWAP/SWAP5 release convention;
- should be stable after publication;
- must not be invented by the publication workstream.

Current evidence:

- repository releases: none;
- repository tags: none;
- no semantic/date versioning policy found;
- Status-A authority permits creation of a release-candidate/tag but does not define its name.
