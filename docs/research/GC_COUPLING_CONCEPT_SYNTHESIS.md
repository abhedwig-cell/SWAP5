...
The current F-GC mode-5 evidence is closest to this **boundary/interface family**, with one important discretization question still explicit: a MODFLOW cell/node head is used as the SWAP lower-face head. Whether that is an adequate representation of the actual interface head is a spatial-discretization assumption, not an identity that follows from equal units.
