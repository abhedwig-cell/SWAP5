...
That evidence establishes executable behaviour very strongly. It does **not**, however, expose a separate complete table mapping every original SWAP input keyword, user-facing parameter name and input unit onto `cofgen(1:24)`. The SWAP-012 patch exposes several legacy local variable names, but not enough surrounding assignment context to prove a complete row mapping.

For that reason this page does not fill the gap from textbook convention.
...
Those names are useful provenance clues. The patch hunk, however, does not include a complete `cofgen(index) -> local name -> original input field` assignment table. F-DOC27 therefore does not use the familiar form of those names to back-fill the unresolved rows.
