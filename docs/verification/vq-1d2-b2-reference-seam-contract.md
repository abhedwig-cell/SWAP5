...
This is the minimum executable boundary needed for rollback, same-state rerun and warm-start qualification.
...
```text
1. exact current B1.8 oracle and synchronized evidence
...
TX/HY/RT may choose its internal implementation freely, but VQ accepts it as B2 only when one exact integrated commit provides the callable reference entrypoint, semantic result contract, valid reference-seam declaration and candidate repin. Only then may VQ start the first B1.8 -> B2 numerical comparison.
