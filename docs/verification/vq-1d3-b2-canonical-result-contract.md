...
This is the comparison surface for B1.7 -> B2 and later reference -> normal/relaxed/fallback qualification. It is not a required production serialization format.
...
After VQ-1d1/1d2/1d3 integrate, TX/HY/RT can implement the real reference seam against these acceptance surfaces. VQ then pins the exact integrated B2 commit, validates seam and result contract, normalizes the first accepted B2 record, independently recomputes mass, executes the first B1.7 -> B2 control comparison, and proceeds to VQ-1e transaction/rerun/warm-start/generic-time qualification.
