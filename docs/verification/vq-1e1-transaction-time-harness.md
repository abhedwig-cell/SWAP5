...
**Workstream:** VQ  
**Slice:** VQ-1e1  
**Depends on:** VQ-1d1..1d3 / clean B1.7 integration PR #43  
...
This operationalizes the distinction in invariant 8: reuse of a numerical guess is allowed; reuse of the wrong physical initial state is not.
...
- state continuity between the two committed subintervals;
- equal final endpoint;
- equal integrated external amount;
- equal integrated fixture result amount.
...
- invariant 30: the new qualification surface is documented and tested.
...
After the clean VQ-1d integration and this verifier harness are accepted, TX/HY/RT can supply one real integrated full-accuracy B2 seam. VQ then replaces the synthetic adapter with a production normalization adapter and runs the same named cases against the exact pinned B2 commit.
