...
BUILD="${TMPDIR:-/tmp}/swap5-lmfp05-attribution-v2-$$"
OUTDIR="$ROOT/lmfp05-artifacts"
mkdir -p "$BUILD" "$OUTDIR"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

STUB_PATCHER="$ROOT/experiments/lmfp/lmfp05_patch_refined_stubs.py"
DT_PATCHER="$ROOT/experiments/lmfp/lmfp05_patch_probe_dt.py"
BASE_STUBS="$ROOT/tests/fsi/fsi04_real_headcalc_stubs.f90"
STUBS="$BUILD/lmfp05_refined_linear_stubs.f90"
BASE_DRIVER="$ROOT/experiments/lmfp/test_lmfp05_refined_fullrichards_reference.f90"
DRIVER="$BUILD/test_lmfp05_refined_fullrichards_reference_probe.f90"
ANALYZER="$ROOT/experiments/lmfp/run_lmfp05_attribution.py"

python3 "$STUB_PATCHER" "$BASE_STUBS" "$STUBS"
python3 "$DT_PATCHER" "$BASE_DRIVER" "$DRIVER"
