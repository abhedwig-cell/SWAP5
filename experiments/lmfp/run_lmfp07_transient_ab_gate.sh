...
BUILD="${TMPDIR:-/tmp}/swap5-lmfp07-ab-$$"
...
STUBS="$BUILD/lmfp07_real_linear_stubs.f90"
DRIVER="$ROOT/experiments/lmfp/test_lmfp04_fullrichards_reference.f90"
COMPARATOR="$ROOT/experiments/lmfp/run_lmfp07_transient_ab.py"

python3 "$PATCHER" "$BASE_STUBS" "$STUBS"
python3 -m py_compile \
  "$COMPARATOR" \
  "$ROOT/experiments/lmfp/run_lmfp06_lookup_surrogate.py" \
...
if grep -Fq 'solution(i) = 0.0d0' "$STUBS"; then
  echo 'F-LMFP07 fixture still contains zero-correction tridag stub' >&2
  exit 1
fi
...
"$BUILD/fullrichards" | tee "$OUTDIR/F-LMFP07_FULLRICHARDS_REFERENCE.txt"
grep -Fq 'F-LMFP04_FULLRICHARDS_REFERENCE_PASS' "$OUTDIR/F-LMFP07_FULLRICHARDS_REFERENCE.txt"

python3 "$COMPARATOR" "$OUTDIR/F-LMFP07_FULLRICHARDS_REFERENCE.txt" "$OUTDIR/F-LMFP07_TRANSIENT_AB_EVIDENCE.json" \
  > "$OUTDIR/F-LMFP07_TRANSIENT_AB_EVIDENCE.stdout.json"

python3 - "$OUTDIR/F-LMFP07_TRANSIENT_AB_EVIDENCE.json" <<'PY'
import json, pathlib, sys
p = pathlib.Path(sys.argv[1])
e = json.loads(p.read_text())
assert e['structural_pass'] is True
assert e['tests']['all_candidate_steps_accepted']['pass'] is True
assert e['tests']['mass_closure']['pass'] is True
assert e['tests']['steady_uniform_cross_model_identity']['pass'] is True
assert e['tests']['lookup_envelope_respected']['pass'] is True
print(json.dumps({
  'structural_pass': e['structural_pass'],
  'lookup': e['lookup'],
  'tests': e['tests'],
  'observed_error_reduction': e['observed_error_reduction'],
