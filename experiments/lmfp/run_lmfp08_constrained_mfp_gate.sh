...
OUTDIR="$ROOT/lmfp08-constrained-mfp-artifacts"
mkdir -p "$OUTDIR"
cd "$ROOT"
...
python3 -m py_compile \
  experiments/lmfp/run_lmfp08_constrained_mfp_ratio.py \
...
python3 experiments/lmfp/run_lmfp08_constrained_mfp_ratio.py \
  lmfp04-artifacts/F-LMFP04_FULLRICHARDS_REFERENCE.txt \
  "$OUTDIR/F-LMFP08_CONSTRAINED_MFP_RATIO_EVIDENCE.json" \
  | tee "$OUTDIR/F-LMFP08_CONSTRAINED_MFP_RATIO_EVIDENCE.stdout.json"

python3 - "$OUTDIR/F-LMFP08_CONSTRAINED_MFP_RATIO_EVIDENCE.json" <<'PY'
...
print('F-LMFP08_CONSTRAINED_CONTINUOUS_MFP_RATIO_GATE PASS')
PY
