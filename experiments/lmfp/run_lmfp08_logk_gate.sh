...
OUTDIR="$ROOT/lmfp08-logk-artifacts"
mkdir -p "$OUTDIR"
cd "$ROOT"
...
python3 experiments/lmfp/run_lmfp08_logk_candidate.py \
  lmfp04-artifacts/F-LMFP04_FULLRICHARDS_REFERENCE.txt \
  "$OUTDIR/F-LMFP08_LOGK_EVIDENCE.json" \
  | tee "$OUTDIR/F-LMFP08_LOGK_EVIDENCE.stdout.json"

python3 - "$OUTDIR/F-LMFP08_LOGK_EVIDENCE.json" <<'PY'
...
print('F-LMFP08_LOG_KDAR_GATE PASS')
PY
