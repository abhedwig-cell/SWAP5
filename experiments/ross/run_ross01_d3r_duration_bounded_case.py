...
def main() -> int:
    ...
    req = d2q.base_request(a.material, t0=0.0, steps=8, perturb=0.01, pre=False)
    ...
    diagnostics = result.get("solver_work_diagnostics") or {}
    checks = {
        ...
        "request_unchanged": req == before and result.get("request_object_unchanged") is True and result.get("committed_state_mutated") is False,
        "fresh_worker": result.get("worker_isolation") == "FRESH_PROCESS_PER_TRIAL",
        "d2_launcher_reused": diagnostics.get("d2_fresh_process_launcher_reused") is True,
        "duration_child_preflight": diagnostics.get("d3r_duration_worker_preflight") is True,
