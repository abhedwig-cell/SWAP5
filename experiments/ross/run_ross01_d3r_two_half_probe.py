...
def main():
    ...
    p.add_argument("--attempt-index", type=int)
    a = p.parse_args()
    indices = range(adapter.CANONICAL_MAX_RETRIES + 1) if a.attempt_index is None else (a.attempt_index,)
    if any(i < 0 or i > adapter.CANONICAL_MAX_RETRIES for i in indices):
    ...
    for i in indices:
        for material in adapter.MATERIAL_IDS:
            ...
            immutable = base["committed_state"] == committed_before and all(x.get("committed_state_mutated") is False for x in (full, h1, h2))
            ...
            if ready:
                ...
                dh = max(abs(float(x)-float(y)) for x,y in zip(fh,hh))
                dt = max(abs(float(x)-float(y)) for x,y in zip(ft,ht))
                max_h, max_t = max(max_h,dh), max(max_t,dt)
            for x in (full,h1,h2):
                r=x.get("unrounded_mass_residual_cm")
                if isinstance(r,(int,float)): max_mass=max(max_mass,abs(float(r)))
            rows.append({"material":material,"attempt_index":i,"pass":ready and mass and immutable and same_forcing,"ready":ready,"mass":mass,"immutable":immutable,"same_forcing":same_forcing,"full_class":full.get("failure_classification"),"half1_class":h1.get("failure_classification"),"half2_class":h2.get("failure_classification"),"head_delta_cm":dh,"theta_delta":dt})

    supported = all(x["pass"] for x in rows)
    out={"work_unit":"F-ROSS01 D3R","kind":"two_half_probe_shard" if a.attempt_index is not None else "two_half_probe","live_canonical_head":a.canonical_head,"attempt_index":a.attempt_index,"canonical_two_half_candidate_chain_supported":supported,"chain_count":len(rows),"max_abs_mass_residual_cm":max_mass,"max_abs_full_vs_two_half_head_delta_cm":max_h,"max_abs_full_vs_two_half_theta_delta":max_t,"external_temporal_error_semantics_qualified":False,"temporal_error_disposition":"DIAGNOSTIC_STATE_DELTAS_ONLY_NO_ACCEPTANCE_METRIC_OR_TOLERANCE_CLAIM","chains":rows,"production_source_delta":[]}
    a.output.write_text(json.dumps(out,indent=2,sort_keys=True)+"\n")
    print(json.dumps({"attempt_index":a.attempt_index,"chain_supported":supported,"chain_count":len(rows),"max_mass":max_mass,"max_head_delta_cm":max_h,"max_theta_delta":max_t},sort_keys=True))
    return 0
