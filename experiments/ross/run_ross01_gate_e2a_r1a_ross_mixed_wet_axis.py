...
CONTRACT = "F-ROSS01_GATE_E2A_R1A_ROSS_MIXED_WET_AXIS_WET_DRY_CHARACTERIZATION_CONTRACT.json"
CATALOG = Path("integration/f-ross/F-ROSS01_GATE_C1_MATERIAL_CATALOG.json")
MATERIALS = ("B12", "O13", "B01", "O14")
CANDIDATES = (("U17_SAT17", 17, 17), ("U33_SAT17", 33, 17), ("U33_SAT33", 33, 33))
N_DRY = 241
H_WET_MIN = -1.0
H_WET_MAX = 1.0
H_DRY_MIN = -10000.0
...
PARENT = {"B12": (-0.011933669447898865, -1.994237053678148), "O13": (-0.007651396095752716, -141.70309854948502)}


def seed_for(material: str) -> int:
    return int(hashlib.sha256(("F-ROSS01-E2A-R1A:" + material).encode()).hexdigest()[:8], 16)
...
def wet_bracket(h: float, n_unsat: int, n_sat: int):
    if h < 0.0:
        s0_all = float(c1.core.s_of_h(H_WET_MIN))
        s = float(c1.core.s_of_h(h))
        i, f = loc_uniform(s, s0_all, 1.0, n_unsat)
        sa = s0_all + (1.0 - s0_all) * i / (n_unsat - 1)
        sb = s0_all + (1.0 - s0_all) * (i + 1) / (n_unsat - 1)
        h0 = float(c1.core.h_of_s(sa))
        h1 = 0.0 if i + 1 == n_unsat - 1 else float(c1.core.h_of_s(sb))
        return "UNSAT_S", i, f, h0, h1
...
def build_probes(material: str):
    ...
    if material in PARENT:
        ha, hb = PARENT[material]
        probes.append({"id": "parent_e2a_structural_witness", "kind": "parent_worst_witness", "h_above": ha, "h_below": hb})
    return probes
...
def summarize(rows):
    ...
    return {
        ...
        "unsaturated_wet_branch_max_hybrid": max((r["hybrid_metric"] for r in rows if r["wet_branch"] == "UNSAT_S"), default=0.0),
...
def evaluate_material(row):
    ...
    for cid, nu, ns in CANDIDATES:
        ...
        s.update({
            "candidate": cid, "unsaturated_nodes_including_h0": nu, "saturated_nodes_including_h0": ns,
            "logical_unique_wet_nodes": unique_wet, "lookup_failures": lookup_failures, "nan_or_inf": nonfinite,
            "node_evaluation_failures": 0, "sparse_node_pair_count_shared_across_candidates": len(pairs),
            "conceptual_full_wet_dry_table_plus_axes_bytes": conceptual_memory, "per_column_table_state_bytes": 0,
        })
        tests = {
        ...
        candidates[cid] = s
    return {
        "material": material, "seed": seed_for(material), "fresh_probe_count": FRESH_COUNT,
        "total_probe_count": len(probes), "node_evaluation_failures": 0, "candidate_results": candidates,
        "runtime_pass": True, "elapsed_seconds_descriptive": time.time() - started,
    }


def main():
    if len(sys.argv) != 2:
        raise SystemExit("usage: run_ross01_gate_e2a_r1a_ross_mixed_wet_axis.py OUTPUT.json")
    out = Path(sys.argv[1])
    ...
    for i, material in enumerate(MATERIALS, 1):
        ...
        results.append(r)
        print(json.dumps({"progress": f"{i}/{len(MATERIALS)}", "material": material, "runtime_pass": r["runtime_pass"], "candidate_pass": {cid: cr["pass"] for cid, cr in r.get("candidate_results", {}).items()}}, sort_keys=True), flush=True)
    ...
    result = {
        ...
        "gate": "E2A_R1A_ROSS_MIXED_WET_AXIS_WET_DRY_CHARACTERIZATION", "contract": CONTRACT,
        "production_implementation": False, "qualification_use": False,
        "candidate_order": [c[0] for c in CANDIDATES], "selected_candidate": selected,
        "materials": list(MATERIALS), "runtime_ok": runtime_ok, "results": results,
        "elapsed_seconds_descriptive": time.time() - started, "pass": passed,
        "decision": "ROSS_MIXED_WET_AXIS_WET_DRY_CHARACTERIZED_READY_FOR_FULL_NEAR_SATURATION_REGION_R1B" if passed else "ROSS_MIXED_WET_AXIS_WET_DRY_REJECTED_RECONCILE_BEFORE_ANY_FULL_REGION_R1B",
        "hard_guard": "Characterization only; no replacement near-saturation representation is qualified or admitted by R1A."
    }
