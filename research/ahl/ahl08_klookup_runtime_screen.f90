program ahl08_klookup_runtime_screen
  ...
  real(real64) :: t_analytic(NREP), t_direct(NREP), t_logit(NREP), t_split(NREP), t_konly(NREP)
  real(real64) :: checksum_a, checksum_d, checksum_l, checksum_s, checksum_k
  integer :: i, rep
  ...
  do rep = 1, NREP
    ...
    call time_konly(heads, p, xl, yl3, t_konly(rep), checksum_k)
  end do
  ...
  if (.not. ieee_is_finite(checksum_a + checksum_d + checksum_l + checksum_s + checksum_k)) error stop 'non-finite benchmark checksum'
  ...
  write(*,'(A,1X,A,1X,A,1X,ES18.10,1X,ES24.16)') 'RESULT', trim(material_id), 'konly', &
       t_konly(2) / real(NHEAD*NCYCLE,real64), checksum_k
...
contains
  ...
  subroutine time_konly(h, params, x, ylogk, elapsed, checksum)
    real(real64), intent(in) :: h(:), params(6), x(:), ylogk(:)
    real(real64), intent(out) :: elapsed
    real(real64), intent(inout) :: checksum
    real(real64) :: t0,t1,xv,f,theta,cap,cond,tr,ts,alpha,n,m,span,ah,term1
    integer :: c,j,idx
    tr=params(1);ts=params(2);alpha=params(3);n=params(4)
    m=1.0_real64-1.0_real64/n;span=ts-tr
    call cpu_time(t0)
    do c=1,NCYCLE
      do j=1,size(h)
        ...
        cap=n*m*alpha*(span/(1.0_real64+term1*ah)**(m+1.0_real64))*term1
        if (h(j)>-1.0_real64 .and. cap<DT*1.0e-7_real64) cap=DT*1.0e-7_real64
        xv=log10(-h(j))
        call locate_interval(x,xv,idx,f)
        cond=exp(ylogk(idx)+f*(ylogk(idx+1)-ylogk(idx)))
        checksum=checksum+theta+1.0e-6_real64*cap+1.0e-9_real64*cond
...
end program ahl08_klookup_runtime_screen
