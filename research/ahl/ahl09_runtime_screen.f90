program ahl09_runtime_screen
  ...
  write(*,'(A,1X,A,1X,A,1X,ES18.10,1X,ES24.16)') 'RESULT',trim(mid),'dc_hybrid',tc(2)/real(NHEAD*NCYCLE,real64),cc
contains
  ...
  pure subroutine candidate(h,p,x,z,mz,lk,theta,c,k)
    ...
    k=exp(lk(idx)+f*(lk(idx+1)-lk(idx)))
  end subroutine
...
end program ahl09_runtime_screen
