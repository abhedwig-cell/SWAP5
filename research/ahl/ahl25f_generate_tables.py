...
if len(sys.argv)!=3: raise SystemExit("usage OUTDIR LOGK_TOL")
OUT=pathlib.Path(sys.argv[1]);OUT.mkdir(parents=True,exist_ok=True)
kon.LOGK_TOL=float(sys.argv[2])
for name,pfull in kon.SETS.items():
    ...
    with (OUT/f"{name}.dat").open("w") as f:
        ...
        for h in rknots:
            z,m=ret.node(h,pret);f.write(f"R {ret.xh(h):.17e} {z:.17e} {m:.17e}\n")
        for h in kknots:f.write(f"K {kon.xh(h):.17e} {math.log(kon.exact_k(h,pfull)):.17e}\n")
