module mod_ahl13_dc_exactk_provider
  ...
  type, extends(constitutive_hydraulics_provider_t), public :: ahl13_dc_exactk_provider_t
    private
  ...
  end type ahl13_dc_exactk_provider_t

  public :: bind_ahl13_dc_exactk_provider

contains

  subroutine bind_ahl13_dc_exactk_provider(provider,parameters,step_duration,table_path,valid)
    type(ahl13_dc_exactk_provider_t),intent(out)::provider
  ...
  pure real(real64) function exact_k(c,head,theta) result(k)
    ...
    real(real64),parameter::HCRIT=-1.0e-2_real64,KSMALL=1.0e-10_real64
...
end module mod_ahl13_dc_exactk_provider
