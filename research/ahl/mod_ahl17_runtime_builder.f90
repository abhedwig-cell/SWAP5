module mod_ahl17_runtime_builder
  ...
  type, public :: ahl17_table_t
    integer :: n=0
    real(real64), allocatable :: x(:), z(:), dzdx(:), logk(:)
  end type ahl17_table_t

  public :: build_ahl17_table, validate_ahl17_table

contains

  subroutine build_ahl17_table(provider,theta_r,theta_s,table,ok)
    ...
    ok=.true.
  end subroutine build_ahl17_table
  ...
  subroutine validate_ahl17_table(provider,tr,ts,table,max_theta,max_logc,max_logk,ok)
    type(b110_default_mvg_provider_t),intent(in)::provider
    real(real64),intent(in)::tr,ts
    type(ahl17_table_t),intent(in)::table
    ...
    ok=.true.
  end subroutine validate_ahl17_table
...
end module mod_ahl17_runtime_builder
