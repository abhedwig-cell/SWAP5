module mod_ahl18_cache
  use, intrinsic :: iso_fortran_env, only: int64, real64
  use mod_b110_default_mvg_provider, only: b110_default_mvg_parameters_t, b110_default_mvg_provider_t
  use mod_ahl17_runtime_builder, only: ahl17_table_t, build_ahl17_table
  implicit none
  private
  ...
  type :: ahl18_cache_entry_t
    logical :: occupied=.false.
    type(ahl18_cache_key_t) :: key
    type(ahl17_table_t) :: table
  end type ahl18_cache_entry_t

  type, public :: ahl18_cache_t
    private
    type(ahl18_cache_entry_t) :: entry(AHL18_MAX_CACHE)
    integer :: builds=0, hits=0, misses=0
  contains
    procedure :: get_or_build => ahl18_get_or_build
    procedure :: stats => ahl18_stats
  end type ahl18_cache_t

  public :: make_ahl18_key

contains

  function make_ahl18_key(parameters,model_id,policy_version,branch_policy_version) result(key)
  ...
  subroutine ahl18_get_or_build(self,key,parameters,provider,table,was_hit,ok)
    class(ahl18_cache_t),intent(inout)::self
    type(ahl18_cache_key_t),intent(in)::key
    type(b110_default_mvg_parameters_t),intent(in)::parameters
    type(b110_default_mvg_provider_t),intent(in)::provider
    type(ahl17_table_t),intent(out)::table
    logical,intent(out)::was_hit,ok
    integer::i,slot
    logical::build_ok

    was_hit=.false.;ok=.false.;slot=0
    do i=1,AHL18_MAX_CACHE
    ...
    if(slot==0)return
    call build_ahl17_table(provider,parameters%cofgen(1,1),parameters%cofgen(2,1),table,build_ok)
    ...
    ok=.true.
  end subroutine ahl18_get_or_build

  subroutine ahl18_stats(self,builds,hits,misses,entries)
    class(ahl18_cache_t),intent(in)::self
  ...
  end subroutine ahl18_stats

end module mod_ahl18_cache
