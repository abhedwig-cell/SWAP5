module mod_ahl20_self_built_provider
  ...
  type(ahl18_cache_t), save :: shared_cache

  type, extends(constitutive_hydraulics_provider_t), public :: ahl20_self_built_provider_t
    private
  ...
  contains
    procedure :: evaluate => ahl20_evaluate
  end type ahl20_self_built_provider_t

  public :: bind_ahl20_self_built_provider
  public :: ahl20_cache_stats

contains

  subroutine bind_ahl20_self_built_provider(provider,parameters,step_duration,ok,was_hit)
    type(ahl20_self_built_provider_t),intent(out)::provider
    ...
    key=make_ahl18_key(sampler_parameters,MODEL_ID,POLICY_VERSION,BRANCH_POLICY_VERSION)
    ...
    provider%ready=.true.
  end subroutine bind_ahl20_self_built_provider

  subroutine ahl20_evaluate(self,pressure_head,water_content,conductivity,capacity,dconductivity_dhead)
    class(ahl20_self_built_provider_t),intent(in)::self
    ...
    if(.not.self%ready .or. .not.associated(self%cofgen))error stop 'AHL20 provider not ready'
  ...
  end subroutine ahl20_evaluate

  subroutine ahl20_cache_stats(builds,hits,misses,entries)
    integer,intent(out)::builds,hits,misses,entries
    call shared_cache%stats(builds,hits,misses,entries)
  end subroutine ahl20_cache_stats
...
end module mod_ahl20_self_built_provider
