...
python3 research/ahl/ahl04b_generate_attribution_tables.py "$BUILD/tables" >/tmp/ahl06_table_summary.json
...
gfortran "${COMMON[@]}" -O3 -J "$OUT" -I "$OUT" -c research/ahl/test_ahl06_split_end_to_end.f90 -o "$OUT/test.o"
gfortran -O3 "${objects[@]}" "$OUT/test.o" -o "$OUT/test"
"$OUT/test" "$BUILD/tables/base.dat" split | tee /tmp/ahl06_result.txt
