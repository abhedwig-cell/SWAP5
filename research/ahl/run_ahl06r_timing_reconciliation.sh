...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl06r-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$OUT" -I "$OUT" -c research/ahl/test_ahl06r_timing_reconciliation.f90 -o "$OUT/test.o"
gfortran -O3 "${objects[@]}" "$OUT/test.o" -o "$OUT/test"

RESULT="${1:-/tmp/ahl06r_result.txt}"
"$OUT/test" "$BUILD/tables/base.dat" | tee "$RESULT"
grep -Fq 'AHL06R_FIDELITY=PASS' "$RESULT"
