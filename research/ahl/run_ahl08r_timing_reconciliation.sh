...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl08r-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$OUT" -I "$OUT" -c research/ahl/test_ahl08r_timing_reconciliation.f90 -o "$OUT/test.o"
gfortran -O3 "${objects[@]}" "$OUT/test.o" -o "$OUT/test"

RESULT="${1:-/tmp/ahl08r_result.txt}"
"$OUT/test" "$BUILD/tables/base.dat" | tee "$RESULT"
grep -Fq 'AHL08R_FIDELITY=PASS' "$RESULT"
