...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl11-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/tables"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

PYTHONPATH=research/ahl python3 research/ahl/ahl11_generate_variants.py "$BUILD/tables" | tee "$BUILD/table_summary.txt"
...
RESULT="${1:-/tmp/ahl11_result.txt}"
: > "$RESULT"
for variant in base theta3 k3 both3; do
  while read -r material regime h0 hbot; do
    ...
    echo "AHL11_CASE_STATUS ${variant} ${material}:${regime} ${verdict}" | tee -a "$RESULT"
  done <<'CASES'
B01 wet -10 -7.5
B01 dry -500 -400
B12 wet -10 -7.5
...
echo "AHL11_ATTRIBUTION_MATRIX=COMPLETE" | tee -a "$RESULT"
