...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl12a-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O2 -J "$OUT" -I "$OUT" -c research/ahl/test_ahl12a_qbot_exactk.f90 -o "$OUT/test.o"
gfortran -O2 "${objects[@]}" "$OUT/test.o" -o "$OUT/test"

"$OUT/test" "$BUILD/tables/B01_ret_dc.dat" "$BUILD/tables/B01_k03_k.dat" B01 wet -10 | tee /tmp/ahl12a_result.txt
grep -Fq 'AHL12A B01:wet PASS' /tmp/ahl12a_result.txt
