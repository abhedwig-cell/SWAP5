...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl12f-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$OUT" -I "$OUT" -c research/ahl/test_ahl12f_qbot_timing.f90 -o "$OUT/test.o"
gfortran -O3 "${objects[@]}" "$OUT/test.o" -o "$OUT/test"

"$OUT/test" "$BUILD/tables/B01_ret_dc.dat" "$BUILD/tables/B01_k03_k.dat" | tee /tmp/ahl12f_result.txt
