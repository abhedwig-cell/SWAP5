...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl13-${GITHUB_RUN_ID:-local}-$$"
...
for material in B01 B12 O05 O14; do
  for spec in "wet -10 -7.5" "mid -75 -50" "dry -500 -400"; do
    ...
    set +e
    "$OUT/test" "$BUILD/tables/${material}_dc.dat" "$material" "$regime" "$h0" "$hbot" exactK 2>&1 | tee -a "$RESULT"
    rc=${PIPESTATUS[0]}
    set -e
    if [[ "$rc" -ne 0 ]]; then
      echo "AHL13_CASE_STATUS ${material}:${regime} FAIL rc=${rc}" | tee -a "$RESULT"
      fail=1
    else
      echo "AHL13_CASE_STATUS ${material}:${regime} PASS" | tee -a "$RESULT"
    fi
  done
done
if [[ "$fail" -ne 0 ]]; then
  echo "AHL13_STAGE1=FAIL" | tee -a "$RESULT"
  exit 1
fi
echo "AHL13_STAGE1=PASS" | tee -a "$RESULT"
