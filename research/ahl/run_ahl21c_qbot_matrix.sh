...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl21c-${GITHUB_RUN_ID:-local}-$$"
...
RESULT="${1:-/tmp/ahl21c_result.txt}"; : > "$RESULT"
fail=0
for material in B01 B12 O05 O14; do
  for spec in "wet -10" "mid -75" "dry -500"; do
    read -r regime h0 <<< "$spec"
    set +e
    "$OUT/test" "$BUILD/tables/${material}_dc.dat" "$material" "$regime" "$h0" 0.0 0.99 0.0625 2>&1 | tee -a "$RESULT"
    rc=${PIPESTATUS[0]}
    set -e
    if [[ "$rc" -ne 0 ]]; then
      echo "AHL21C_CASE_STATUS ${material}:${regime} FAIL rc=$rc" | tee -a "$RESULT"
      fail=1
    else
      echo "AHL21C_CASE_STATUS ${material}:${regime} PASS" | tee -a "$RESULT"
    fi
  done
done
if [[ "$fail" -ne 0 ]]; then
  echo "AHL21C_MATRIX=FAIL" | tee -a "$RESULT"
  exit 1
fi
echo "AHL21C_MATRIX_12_OF_12=PASS" | tee -a "$RESULT"
