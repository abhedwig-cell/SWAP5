...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl28-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c research/ahl/test_ahl28_binding_overhead.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/ahl28_binding.txt
grep -Fq 'AHL28_BINDING_MICROBENCH=PASS' /tmp/ahl28_binding.txt
