...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl29-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O2 -J "$BUILD" -I "$BUILD" -c research/ahl/test_ahl29_cache_capacity.f90 -o "$BUILD/test.o"
gfortran -O2 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/ahl29_stage1.txt
