...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl31-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD"   -c research/ahl/test_ahl31_memory_build_characterization.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/ahl31_result.txt
