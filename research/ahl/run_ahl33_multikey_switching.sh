...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl33-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c research/ahl/test_ahl33_multikey_switching.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/ahl33_result.txt
grep -Fq 'AHL33_CHARACTERIZATION=PASS' /tmp/ahl33_result.txt
