...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl34-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c research/ahl/test_ahl34_bind_decomposition.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/ahl34_result.txt
grep -Fq 'AHL34_CHARACTERIZATION=PASS' /tmp/ahl34_result.txt
