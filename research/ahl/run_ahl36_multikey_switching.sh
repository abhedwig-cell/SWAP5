...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl36-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c research/ahl/test_ahl36_multikey_switching.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/ahl36_result.txt
grep -Fq 'AHL36_CHARACTERIZATION=PASS' /tmp/ahl36_result.txt
