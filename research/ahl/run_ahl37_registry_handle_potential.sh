...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl37-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c research/ahl/test_ahl37_registry_handle_potential.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/ahl37_result.txt
grep -Fq 'AHL37_CHARACTERIZATION=PASS' /tmp/ahl37_result.txt
