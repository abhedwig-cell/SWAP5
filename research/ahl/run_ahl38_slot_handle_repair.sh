...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl38-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c research/ahl/test_ahl38_slot_handle_repair.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/ahl38_result.txt
grep -Fq 'AHL38_SEMANTIC_IDENTITY=PASS' /tmp/ahl38_result.txt
grep -Fq 'AHL38_PERFORMANCE_GATES=PASS' /tmp/ahl38_result.txt
