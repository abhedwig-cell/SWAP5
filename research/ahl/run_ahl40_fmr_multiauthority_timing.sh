...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ahl40-fmr-multiauthority-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c research/ahl/test_ahl40_fmr_multiauthority_timing.f90 -o "$BUILD/test.o"
gfortran -fopenmp -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/ahl40_result.txt
grep -Fq 'AHL40_APPLICATION_TIMING=PASS' /tmp/ahl40_result.txt
