...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl47-matrix-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -J "$BUILD" -I "$BUILD" -c research/ahl/test_fahl47_matrix.f90 -o "$BUILD/test.o"
