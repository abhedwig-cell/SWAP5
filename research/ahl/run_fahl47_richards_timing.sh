...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl47-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -J "$BUILD" -I "$BUILD" -c research/ahl/test_fahl47_richards_timing.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee "$BUILD/result.txt"
grep -Fq 'FAHL47_RICHARDS_TIMING=PASS' "$BUILD/result.txt"
