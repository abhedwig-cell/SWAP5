...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl48-shared-matrix-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
src=src.replace("program test_fahl47_matrix_timing","program test_fahl48_shared_matrix_timing")
src=src.replace("end program test_fahl47_matrix_timing","end program test_fahl48_shared_matrix_timing")
src=src.replace(
...
src=src.replace("type(ahl47_provider_t), target :: lookup","type(ahl48_shared_provider_t), target :: lookup")
src=src.replace("logical :: valid","logical :: valid, was_hit")
src=src.replace(
    "call bind_ahl47_provider(lookup,hp,total_dt,valid)",
    "call ahl48_reset_shared_pool()\n  call bind_ahl48_shared_provider(lookup,hp,total_dt,valid,was_hit)\n  call ahl48_freeze_shared_pool()")
src=src.replace("FAHL47_","FAHL48_SHARED_")
Path(__import__("sys").argv[1]).write_text(src)
PY
...
for line in open(sys.argv[1]):
    m=re.search(r'FAHL48_SHARED_TIMING_MEDIAN\|CASE=([^|]+)\|RATIO=([0-9Ee+.-]+)',line)
    if m: rows[m.group(1)]=float(m.group(2))
if len(rows)!=12: raise SystemExit(f'expected 12 medians got {len(rows)}')
vals=sorted(rows.values())
print(f'FAHL48_SHARED_MATRIX_MEDIAN={(vals[5]+vals[6])/2:.9f}')
print(f'FAHL48_SHARED_MATRIX_MIN={min(vals):.9f}')
print(f'FAHL48_SHARED_MATRIX_MAX={max(vals):.9f}')
print('FAHL48_SHARED_MATRIX_POSITIVE='+str(sum(v<0.98 for v in vals)))
print('FAHL48_SHARED_MATRIX_NEGATIVE='+str(sum(v>1.02 for v in vals)))
print('FAHL48_SHARED_MATRIX_TIMING=PASS')
PY
