program test_ahl04c_split_provider_replay
  ...
  use mod_ahl04c_split_lookup_provider, only: ahl04c_split_lookup_provider_t, bind_ahl04c_split_lookup_provider, &
       AHL04C_LOOKUP_ALL, AHL04C_EXACT_THETA, AHL04C_EXACT_CAPACITY, AHL04C_EXACT_CONDUCTIVITY
  implicit none
  ...
  type(ahl04c_split_lookup_provider_t), target :: lookup
  ...
  if (command_argument_count() /= 2) error stop 'usage: test TABLE_PATH VARIANT'
  call get_command_argument(1,table_path)
  call get_command_argument(2,label)
  select case(trim(label))
  case('lookup_all'); mode=AHL04C_LOOKUP_ALL
  case('exact_theta'); mode=AHL04C_EXACT_THETA
  case('exact_C'); mode=AHL04C_EXACT_CAPACITY
  case('exact_K'); mode=AHL04C_EXACT_CONDUCTIVITY
  case default; error stop 'unknown AHL04C variant'
  end select
  ...
  call bind_ahl04c_split_lookup_provider(lookup,hp,total_dt,trim(table_path),mode,valid)
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'AHL04C_METRIC',trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'AHL04C_DIAG',trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'AHL04C',trim(name),'MEASURED'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL04C_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl04c_split_provider_replay
