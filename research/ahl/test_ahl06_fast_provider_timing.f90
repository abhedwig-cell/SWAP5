program test_ahl06_fast_provider_timing
  ...
  integer, parameter :: NREPLAY=512, NTIMING=3
  ...
  real(real64) :: k0, ref_times(NTIMING), split_times(NTIMING)
  ...
  do r=1,NTIMING
    call cpu_time(t0)
    do j=1,NREPLAY
      request%base_state=initial_state;request%evaluation%constitutive=>analytical
      call solver_ref%solve(request,workspace_ref,ref_result)
      if(ref_result%status/=SW_SOLVE_CONVERGED) error stop 'timed reference failure'
    end do
    call cpu_time(t1);ref_times(r)=(t1-t0)/real(NREPLAY,real64)

    call cpu_time(t0)
    do j=1,NREPLAY
      request%base_state=initial_state;request%evaluation%constitutive=>split
      call solver_split%solve(request,workspace_split,split_result)
      if(split_result%status/=SW_SOLVE_CONVERGED) error stop 'timed split failure'
    end do
    call cpu_time(t1);split_times(r)=(t1-t0)/real(NREPLAY,real64)
  end do
  call sort3(ref_times);call sort3(split_times)

  write(*,'(A,1X,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL06_FIDELITY MAX_DH=',max_dh,'MAX_DTHETA=',max_dw,'DTOP=',dtop,'DBOTTOM=',dbottom,'MASS=',mass
  write(*,'(A,1X,I0,1X,I0,1X,I0,1X,I0)') 'AHL06_DIAG',ref_result%diagnostics%nonlinear_iterations, &
       split_result%diagnostics%nonlinear_iterations,ref_result%diagnostics%backtracking_attempts, &
       split_result%diagnostics%backtracking_attempts
  write(*,'(A,1X,ES18.10)') 'AHL06_ANALYTICAL_SEC_PER_SOLVE',ref_times(2)
  write(*,'(A,1X,ES18.10)') 'AHL06_SPLIT_SEC_PER_SOLVE',split_times(2)
  write(*,'(A,1X,ES18.10)') 'AHL06_SPLIT_REDUCTION_FRACTION',1.0_real64-split_times(2)/ref_times(2)
  write(*,'(A)') 'AHL06_FAST_SPLIT_FIDELITY=PASS'
...
end program test_ahl06_fast_provider_timing
