program test_ahl06_split_end_to_end
  ...
  type(ahl06_split_provider_t), target :: lookup
  ...
  real(real64) :: k0, t0, t1, analytical_times(3), split_times(3)
  integer, parameter :: nreplay=512
  integer :: rep, ireplay
  character(len=512) :: table_path, label
  logical :: valid
  integer :: k

  if (command_argument_count() /= 2) error stop 'usage: test TABLE_PATH LABEL'
  ...
  call bind_ahl06_split_provider(lookup,hp,total_dt,trim(table_path),valid)
  ...
  do rep=1,3
    ...
    do ireplay=1,nreplay
      ...
      request%base_state=initial_state
      call solver_ref%solve(request,workspace_ref,reference_result)
      call require(reference_result%status==SW_SOLVE_CONVERGED,'timed analytical solve converged')
    end do
    call cpu_time(t1); analytical_times(rep)=t1-t0
    ...
    do ireplay=1,nreplay
      ...
      request%base_state=initial_state
      call solver_candidate%solve(request,workspace_candidate,candidate_result)
      call require(candidate_result%status==SW_SOLVE_CONVERGED,'timed split solve converged')
    end do
    call cpu_time(t1); split_times(rep)=t1-t0
  end do
  call sort3(analytical_times); call sort3(split_times)
  write(*,'(A,1X,ES16.8,1X,A,1X,ES16.8,1X,A,1X,F10.6)') &
       'AHL06_TIMING_ANALYTICAL_S',analytical_times(2),'SPLIT_S',split_times(2),'RATIO',split_times(2)/analytical_times(2)
...
contains
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL04A_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl06_split_end_to_end
