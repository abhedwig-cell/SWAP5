program test_ahl07_convergence_attribution
  ...
  logical :: valid, use_theta, use_capacity, use_conductivity
  integer :: k
  ...
  use_theta=.true.; use_capacity=.true.; use_conductivity=.true.
  use_theta=.false.
  select case(trim(label))
  case('split')
    use_capacity=.true.; use_conductivity=.true.
  case('exactC')
    use_capacity=.false.; use_conductivity=.true.
  case('exactK')
    use_capacity=.true.; use_conductivity=.false.
  case('exactCK')
    use_capacity=.false.; use_conductivity=.false.
  case default
    error stop 'unknown AHL07 variant'
  end select
  call bind_ahl04_hybrid_lookup_provider(lookup,hp,total_dt,trim(table_path),valid, &
       use_theta,use_capacity,use_conductivity)
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'AHL07_METRIC',trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'AHL07_DIAG',trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'AHL07',trim(name),'MEASURED'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL07_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl07_convergence_attribution
