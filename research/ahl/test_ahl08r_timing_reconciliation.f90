program test_ahl08r_timing_reconciliation
  ...
  type(ahl08_klookup_provider_t), target :: klookup
  type(b110_source_sink_provider_t), target :: source_sink
  type(fmr04_fixed_flux_top_provider_t), target :: top_provider
  type(soil_water_physical_state_t) :: initial_state
  type(soil_water_solve_request_t) :: request
  type(soil_water_solve_result_t) :: ref_result, k_result
  type(reference_richards_legacy_solver_t) :: solver_ref, solver_k
  type(reference_richards_legacy_workspace_t) :: workspace_ref, workspace_k
  real(real64), allocatable, target :: drainage(:,:), subsurface(:), root_sink(:)
  real(real64), allocatable :: cofgen(:,:)
  real(real64) :: heads(numnod), water(numnod), conductivity(numnod), capacity(numnod), dkdh(numnod)
  real(real64) :: k0, ref_times(NTIMING), k_times(NTIMING), ratios(NTIMING)
  ...
  call bind_ahl08_klookup_provider(klookup,hp,total_dt,trim(table_path),valid)
  call require(valid,'klookup provider bound')
  ...
  request%base_state=initial_state;request%evaluation%constitutive=>klookup
  call solver_k%solve(request,workspace_k,k_result)
  call require(k_result%status==SW_SOLVE_CONVERGED,'klookup converged')

  max_dh=maxval(abs(k_result%candidate_state%pressure_head-ref_result%candidate_state%pressure_head))
  max_dw=maxval(abs(k_result%candidate_state%water_content-ref_result%candidate_state%water_content))
  dtop=abs(k_result%top_flux-ref_result%top_flux)
  dbottom=abs(k_result%bottom_flux-ref_result%bottom_flux)
  if(k_result%integrated_mass_balance_residual_available)then
    mass=abs(k_result%integrated_mass_balance_residual_cm)
  else
    mass=abs(k_result%unrounded_mass_balance_residual)
  end if
  ...
  call require(abs(k_result%diagnostics%nonlinear_iterations-ref_result%diagnostics%nonlinear_iterations)<=2,'iteration gate')
  call require(abs(k_result%diagnostics%backtracking_attempts-ref_result%diagnostics%backtracking_attempts)<=2,'backtrack gate')

  do r=1,NTIMING
    if(mod(r,2)==1) then
      call time_reference_batch(request,initial_state,analytical,solver_ref,workspace_ref,ref_result,ref_times(r))
      call time_k_batch(request,initial_state,klookup,solver_k,workspace_k,k_result,k_times(r))
    else
      call time_k_batch(request,initial_state,klookup,solver_k,workspace_k,k_result,k_times(r))
      call time_reference_batch(request,initial_state,analytical,solver_ref,workspace_ref,ref_result,ref_times(r))
    end if
    ratios(r)=k_times(r)/ref_times(r)
    write(*,'(A,1X,I0,1X,ES18.10,1X,ES18.10,1X,F10.6)') 'AHL08R_PAIR',r,ref_times(r),k_times(r),ratios(r)
  end do
  call sort7(ratios)

  write(*,'(A,1X,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL08R_FIDELITY MAX_DH=',max_dh,'MAX_DTHETA=',max_dw,'DTOP=',dtop,'DBOTTOM=',dbottom,'MASS=',mass
  write(*,'(A,1X,F10.6)') 'AHL08R_MEDIAN_PAIRED_RATIO',ratios(4)
  write(*,'(A,1X,F10.6)') 'AHL08R_MEDIAN_REDUCTION_FRACTION',1.0_real64-ratios(4)
  write(*,'(A)') 'AHL08R_FIDELITY=PASS'
...
contains
  ...
  subroutine time_k_batch(req,state,provider,solver,workspace,result,elapsed)
    type(soil_water_solve_request_t),intent(inout)::req
    type(soil_water_physical_state_t),intent(in)::state
    type(ahl08_klookup_provider_t),target,intent(in)::provider
    type(reference_richards_legacy_solver_t),intent(inout)::solver
    type(reference_richards_legacy_workspace_t),intent(inout)::workspace
    type(soil_water_solve_result_t),intent(out)::result
    real(real64),intent(out)::elapsed
    ...
    do q=1,NREPLAY
      req%base_state=state;req%evaluation%constitutive=>provider
      call solver%solve(req,workspace,result)
      if(result%status/=SW_SOLVE_CONVERGED) error stop 'timed klookup failure'
    end do
    call cpu_time(b);elapsed=(b-a)/real(NREPLAY,real64)
  end subroutine time_k_batch
...
end program test_ahl08r_timing_reconciliation
