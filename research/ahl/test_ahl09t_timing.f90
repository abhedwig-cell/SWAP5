program test_ahl09t_timing
  ...
  type(ahl09_dc_provider_t), target :: split
  ...
  call bind_ahl09_dc_provider(split,hp,total_dt,trim(table_path),valid)
  ...
  do r=1,NTIMING
    ...
    write(*,'(A,1X,I0,1X,ES18.10,1X,ES18.10,1X,F10.6)') 'AHL09T_PAIR',r,ref_times(r),split_times(r),ratios(r)
  end do
  call sort7(ratios)

  write(*,'(A,1X,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL09T_FIDELITY MAX_DH=',max_dh,'MAX_DTHETA=',max_dw,'DTOP=',dtop,'DBOTTOM=',dbottom,'MASS=',mass
  write(*,'(A,1X,F10.6)') 'AHL09T_MEDIAN_PAIRED_RATIO',ratios(4)
  write(*,'(A,1X,F10.6)') 'AHL09T_MEDIAN_REDUCTION_FRACTION',1.0_real64-ratios(4)
  write(*,'(A)') 'AHL09T_FIDELITY=PASS'
...
contains
  ...
  subroutine time_split_batch(req,state,provider,solver,workspace,result,elapsed)
    type(soil_water_solve_request_t),intent(inout)::req
    type(soil_water_physical_state_t),intent(in)::state
    ...
    do q=1,NREPLAY
      req%base_state=state;req%evaluation%constitutive=>provider
      call solver%solve(req,workspace,result)
      if(result%status/=SW_SOLVE_CONVERGED) error stop 'timed split failure'
    end do
    call cpu_time(b);elapsed=(b-a)/real(NREPLAY,real64)
  end subroutine time_split_batch
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL09T_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl09t_timing
