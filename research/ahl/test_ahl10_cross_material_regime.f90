program test_ahl10_cross_material_regime
  ...
  real(real64) :: k0, h0, hbot, tr, ts, alpha, npar, ksat, lambda
  character(len=512) :: table_path, label, material, arg
  logical :: valid
  integer :: k

  if (command_argument_count() /= 5) error stop 'usage: test TABLE_PATH MATERIAL H0 HBOT LABEL'
  ...
  call get_command_argument(5,label)
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'AHL10_METRIC',trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'AHL10_DIAG',trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'AHL10',trim(name),'PASS'
...
end program test_ahl10_cross_material_regime
