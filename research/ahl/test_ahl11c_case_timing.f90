program test_ahl11c_case_timing
  ...
  integer, parameter :: NREPLAY=16384, NTIMING=7
  ...
  character(len=512) :: ret_path, k_path, material, regime, arg
  real(real64) :: h0, hbot, tr, ts, alpha, nvg, ksat, lambda
  logical :: valid
  integer :: k,r,j

  if(command_argument_count()/=6) error stop 'usage: test RET K MATERIAL REGIME H0 HBOT'
  call get_command_argument(1,ret_path)
  call get_command_argument(2,k_path)
  call get_command_argument(3,material)
  call get_command_argument(4,regime)
  call get_command_argument(5,arg); read(arg,*) h0
  call get_command_argument(6,arg); read(arg,*) hbot
  ...
  do r=1,NTIMING
    ...
    write(*,'(A,1X,I0,1X,ES18.10,1X,ES18.10,1X,F10.6)') 'AHL11C_PAIR',r,ref_times(r),candidate_times(r),ratios(r)
  end do
  call sort7(ratios)

  write(*,'(A,1X,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL11C_FIDELITY MAX_DH=',max_dh,'MAX_DTHETA=',max_dw,'DTOP=',dtop,'DBOTTOM=',dbottom,'MASS=',mass
  write(*,'(A,1X,A,1X,F10.6)') 'AHL11C_CASE_MEDIAN_RATIO',trim(material)//':'//trim(regime),ratios(4)
  write(*,'(A,1X,F10.6)') 'AHL11C_MEDIAN_REDUCTION_FRACTION',1.0_real64-ratios(4)
  write(*,'(A)') 'AHL11C_FIDELITY=PASS'
...
contains
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL11C_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl11c_case_timing
