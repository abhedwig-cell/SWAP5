program test_ahl11d_o09_attribution
  ...
  use mod_ahl10a_attribution_provider, only: ahl10a_provider_t, bind_ahl10a_provider, &
       AHL10A_BASE, AHL10A_DC_EXACTK, AHL10A_EXACTRC_LOOKUPK
  implicit none
  ...
  type(ahl10a_provider_t), target :: lookup
  ...
  character(len=512) :: table_path, label, material, arg, variant
  logical :: valid
  integer :: k, mode

  if (command_argument_count() /= 6) error stop 'usage: test TABLE_PATH MATERIAL REGIME H0 HBOT VARIANT'
  ...
  call get_command_argument(6,variant)
  select case(trim(variant))
  case('base'); mode=AHL10A_BASE
  case('exactK'); mode=AHL10A_DC_EXACTK
  case('exactRC'); mode=AHL10A_EXACTRC_LOOKUPK
  case default; error stop 'unknown attribution variant'
  end select
  ...
  case('O09')
    tr=0.0_real64; ts=0.458246_real64; alpha=0.009715_real64; nvg=1.375784_real64
    ksat=3.766627_real64; lambda=-1.013083_real64
  case('O14')
  ...
  call bind_ahl10a_provider(lookup,hp,total_dt,trim(table_path),mode,valid)
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'AHL11D_METRIC',trim(material)//':'//trim(name)//':'//trim(variant),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'AHL11D_DIAG',trim(material)//':'//trim(name)//':'//trim(variant),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'AHL11D',trim(material)//':'//trim(name)//':'//trim(variant),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL11D_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl11d_o09_attribution
