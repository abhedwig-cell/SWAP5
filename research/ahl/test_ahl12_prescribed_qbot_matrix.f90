program test_ahl12_prescribed_qbot_matrix
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'AHL12_METRIC',trim(material)//':'//trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'AHL12_DIAG',trim(material)//':'//trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A,ES14.6)') 'AHL12_BOTTOM_HEAD',trim(material)//':'//trim(name),'DELTA_CM=', &
         abs(cand%candidate_state%pressure_head(numnod)-ref%candidate_state%pressure_head(numnod))
    write(*,'(A,1X,A,1X,A)') 'AHL12',trim(material)//':'//trim(name),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL12_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl12_prescribed_qbot_matrix
