program test_ahl12a_qbot_exactk
  ...
  call bind_ahl11_decoupled_provider(lookup,hp,total_dt,trim(ret_path),trim(k_path),valid,exact_k=.true.)
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'AHL12A_METRIC',trim(material)//':'//trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'AHL12A_DIAG',trim(material)//':'//trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A,ES14.6)') 'AHL12A_BOTTOM_HEAD',trim(material)//':'//trim(name),'DELTA_CM=', &
         abs(cand%candidate_state%pressure_head(numnod)-ref%candidate_state%pressure_head(numnod))
    write(*,'(A,1X,A,1X,A)') 'AHL12A',trim(material)//':'//trim(name),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL12A_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl12a_qbot_exactk
