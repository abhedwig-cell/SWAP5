program test_ahl12b_qbot_exact_retention
  ...
  character(len=512) :: ktable_path, label, material, arg
  logical :: valid
  integer :: k

  if (command_argument_count() /= 4) error stop 'usage: test KTABLE MATERIAL REGIME H0'
  call get_command_argument(1,ktable_path)
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'AHL12B_METRIC',trim(material)//':'//trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'AHL12B_DIAG',trim(material)//':'//trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    call require(transfer(cand%bottom_flux,0_int64)==transfer(prescribed_qbot,0_int64),'candidate prescribed-qbot exact identity')
    call require(transfer(ref%bottom_flux,0_int64)==transfer(prescribed_qbot,0_int64),'reference prescribed-qbot exact identity')
    ...
    write(*,'(A,1X,A,1X,A,ES14.6)') 'AHL12B_BOTTOM_HEAD',trim(material)//':'//trim(name),'DELTA_CM=', &
         abs(cand%candidate_state%pressure_head(numnod)-ref%candidate_state%pressure_head(numnod))
    write(*,'(A,1X,A,1X,A)') 'AHL12B',trim(material)//':'//trim(name),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL12B_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl12b_qbot_exact_retention
