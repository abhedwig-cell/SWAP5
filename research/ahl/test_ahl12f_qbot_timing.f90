program test_ahl12f_qbot_timing
  ...
  type(ahl11_decoupled_provider_t), target :: candidate
  ...
  real(real64) :: ref_times(NTIMING), candidate_times(NTIMING), ratios(NTIMING)
  real(real64) :: t0,t1,max_dh,max_dw,dtop,dbottom,mass
  character(len=512) :: ret_path, k_path
  real(real64), parameter :: h0=-10.0_real64
  real(real64) :: tr, ts, alpha, nvg, ksat, lambda
  logical :: valid
  integer :: k,r,j

  if(command_argument_count()/=2) error stop 'usage: test RET K'
  call get_command_argument(1,ret_path)
  call get_command_argument(2,k_path)
  call material_parameters('B01',tr,ts,alpha,nvg,ksat,lambda)
  ...
  call bind_ahl11_decoupled_provider(candidate,hp,total_dt,trim(ret_path),trim(k_path),valid)
  ...
  call build_request(request,parameters,initial_state,source_sink,top_provider)
  ...
  call require(transfer(ref_result%bottom_flux,0_int64)==transfer(prescribed_qbot,0_int64),'reference qbot identity')
  call require(transfer(candidate_result%bottom_flux,0_int64)==transfer(prescribed_qbot,0_int64),'candidate qbot identity')
  ...
  do r=1,NTIMING
    ...
    write(*,'(A,1X,I0,1X,ES18.10,1X,ES18.10,1X,F10.6)') 'AHL12F_PAIR',r,ref_times(r),candidate_times(r),ratios(r)
  end do
  call sort7(ratios)

  write(*,'(A,1X,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL12F_FIDELITY MAX_DH=',max_dh,'MAX_DTHETA=',max_dw,'DTOP=',dtop,'DBOTTOM=',dbottom,'MASS=',mass
  write(*,'(A,1X,F10.6)') 'AHL12F_MEDIAN_PAIRED_RATIO',ratios(4)
  write(*,'(A,1X,F10.6)') 'AHL12F_MEDIAN_REDUCTION_FRACTION',1.0_real64-ratios(4)
  write(*,'(A)') 'AHL12F_FIDELITY=PASS'
...
contains
  subroutine material_parameters(id,tr,ts,alpha,nvg,ksat,lambda)
    ...
    case default
      error stop 'unknown F-AHL11C material'
    end select
  end subroutine material_parameters

  subroutine build_request(req,p,state,sp,tp)
    ...
    req%boundary%top_flux=prescribed_qbot;req%boundary%top_head=h0
    req%boundary%bottom_flux=prescribed_qbot;req%boundary%bottom_head=777777.0_real64
  ...
  subroutine time_candidate_batch(req,state,provider,solver,workspace,result,elapsed)
    type(soil_water_solve_request_t),intent(inout)::req
    type(soil_water_physical_state_t),intent(in)::state
    type(ahl11_decoupled_provider_t),target,intent(in)::provider
    type(reference_richards_legacy_solver_t),intent(inout)::solver
    type(reference_richards_legacy_workspace_t),intent(inout)::workspace
    type(soil_water_solve_result_t),intent(out)::result
    real(real64),intent(out)::elapsed
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL12F_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl12f_qbot_timing
