program test_ahl13r_timing_reconciliation
  ...
  use mod_ahl13_dc_exactk_provider, only: ahl13_dc_exactk_provider_t, bind_ahl13_dc_exactk_provider
  implicit none
  ...
  type(ahl13_dc_exactk_provider_t), target :: candidate
  ...
  call bind_ahl13_dc_exactk_provider(candidate,hp,total_dt,trim(table_path),valid)
  ...
  do r=1,NTIMING
    ...
    write(*,'(A,1X,I0,1X,ES18.10,1X,ES18.10,1X,F10.6)') 'AHL13R_PAIR',r,ref_times(r),candidate_times(r),ratios(r)
  end do
  call sort7(ratios)

  write(*,'(A,1X,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL13R_FIDELITY MAX_DH=',max_dh,'MAX_DTHETA=',max_dw,'DTOP=',dtop,'DBOTTOM=',dbottom,'MASS=',mass
  write(*,'(A,1X,F10.6)') 'AHL13R_MEDIAN_PAIRED_RATIO',ratios(4)
  write(*,'(A,1X,F10.6)') 'AHL13R_MEDIAN_REDUCTION_FRACTION',1.0_real64-ratios(4)
  write(*,'(A)') 'AHL13R_FIDELITY=PASS'
...
contains
  ...
  subroutine time_candidate_batch(req,state,provider,solver,workspace,result,elapsed)
    type(soil_water_solve_request_t),intent(inout)::req
    type(soil_water_physical_state_t),intent(in)::state
    type(ahl13_dc_exactk_provider_t),target,intent(in)::provider
    type(reference_richards_legacy_solver_t),intent(inout)::solver
    type(reference_richards_legacy_workspace_t),intent(inout)::workspace
    type(soil_water_solve_result_t),intent(out)::result
    real(real64),intent(out)::elapsed
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL13R_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl13r_timing_reconciliation
