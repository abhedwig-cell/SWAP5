program test_ahl15r_b01wet_timing
  ...
  real(real64), parameter :: h0=-10.0_real64, hbot=-7.5_real64
  ...
  do k=1,numnod
    cofgen(1,k)=0.02_real64;cofgen(2,k)=0.427494_real64;cofgen(3,k)=31.225016_real64
    cofgen(4,k)=0.021659_real64;cofgen(5,k)=0.98087_real64;cofgen(6,k)=1.734737_real64
  ...
  do r=1,NTIMING
    ...
    write(*,'(A,1X,I0,1X,ES18.10,1X,ES18.10,1X,F10.6)') 'AHL15R_PAIR',r,ref_times(r),candidate_times(r),ratios(r)
  end do
  call sort7(ratios)

  write(*,'(A,1X,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL15R_FIDELITY MAX_DH=',max_dh,'MAX_DTHETA=',max_dw,'DTOP=',dtop,'DBOTTOM=',dbottom,'MASS=',mass
  write(*,'(A,1X,F10.6)') 'AHL15R_MEDIAN_PAIRED_RATIO',ratios(4)
  write(*,'(A,1X,F10.6)') 'AHL15R_MEDIAN_REDUCTION_FRACTION',1.0_real64-ratios(4)
  write(*,'(A)') 'AHL15R_FIDELITY=PASS'
...
contains
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL15R_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl15r_b01wet_timing
