program test_ahl16_case_timing
  ...
  do r=1,NTIMING
    ...
    write(*,'(A,1X,A,1X,A,1X,I0,1X,ES18.10,1X,ES18.10,1X,F10.6)') 'AHL16_PAIR',trim(material),trim(regime),r,ref_times(r),candidate_times(r),ratios(r)
  end do
  call sort7(ratios)

  write(*,'(A,1X,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL16_FIDELITY MAX_DH=',max_dh,'MAX_DTHETA=',max_dw,'DTOP=',dtop,'DBOTTOM=',dbottom,'MASS=',mass
  write(*,'(A,1X,A,1X,A,1X,F10.6)') 'AHL16_MEDIAN_PAIRED_RATIO',trim(material),trim(regime),ratios(4)
  write(*,'(A,1X,A,1X,A,1X,F10.6)') 'AHL16_MEDIAN_REDUCTION_FRACTION',trim(material),trim(regime),1.0_real64-ratios(4)
  write(*,'(A,1X,A,1X,A)') 'AHL16_FIDELITY=PASS',trim(material),trim(regime)
...
contains
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL16_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl16_case_timing
