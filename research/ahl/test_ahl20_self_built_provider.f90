program test_ahl20_self_built_provider
  ...
  use mod_ahl20_self_built_provider, only: ahl20_self_built_provider_t, bind_ahl20_self_built_provider, ahl20_cache_stats
  implicit none
  ...
  type(ahl20_self_built_provider_t), target :: lookup, lookup2
  ...
  call bind_ahl20_self_built_provider(lookup,hp,total_dt,valid,hit1)
  call require(valid,'first self-built provider bound')
  call require(.not.hit1,'first bind builds')
  call bind_ahl20_self_built_provider(lookup2,hp,total_dt,valid,hit2)
  call require(valid,'second self-built provider bound')
  call require(hit2,'second bind hits cache')
  call ahl20_cache_stats(builds,hits,misses,entries)
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'AHL20_METRIC',trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'AHL20_DIAG',trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'AHL20',trim(name),'PASS'
    write(*,'(A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') 'AHL20_CACHE_STATS','BUILDS=',builds,'HITS=',hits,'MISSES=',misses,'ENTRIES=',entries
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL20_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl20_self_built_provider
