program test_ahl20_stage3_timing
  ...
  use mod_ahl20_self_built_provider, only: ahl20_self_built_provider_t, bind_ahl20_self_built_provider
  implicit none
  ...
  type(ahl20_self_built_provider_t), target :: candidate, candidate_hit
  ...
  real(real64) :: t0,t1,max_dh,max_dw,dtop,dbottom,mass,cold_bind_s,warm_bind_s
  ...
  call cpu_time(t0)
  call bind_ahl20_self_built_provider(candidate,hp,total_dt,valid,hit1)
  call cpu_time(t1);cold_bind_s=t1-t0
  call require(valid,'candidate provider bound')
  call require(.not.hit1,'cold bind builds')
  call cpu_time(t0)
  call bind_ahl20_self_built_provider(candidate_hit,hp,total_dt,valid,hit2)
  call cpu_time(t1);warm_bind_s=t1-t0
  call require(valid,'warm candidate provider bound')
  call require(hit2,'warm bind hits cache')
  ...
  write(*,'(A,1X,F10.6)') 'AHL20S3_MEDIAN_PAIRED_RATIO',ratios(4)
  write(*,'(A,1X,F10.6)') 'AHL20S3_MEDIAN_REDUCTION_FRACTION',1.0_real64-ratios(4)
  write(*,'(A,1X,ES18.10)') 'AHL20S3_COLD_BIND_SECONDS',cold_bind_s
  write(*,'(A,1X,ES18.10)') 'AHL20S3_WARM_BIND_SECONDS',warm_bind_s
  write(*,'(A,1X,ES18.10)') 'AHL20S3_COLD_BUILD_PLUS_MEDIAN_SOLVE_SECONDS',cold_bind_s+candidate_times(4)
  write(*,'(A)') 'AHL20S3_FIDELITY=PASS'
...
contains
  ...
  subroutine time_candidate_batch(req,state,provider,solver,workspace,result,elapsed)
    type(soil_water_solve_request_t),intent(inout)::req
    type(soil_water_physical_state_t),intent(in)::state
    type(ahl20_self_built_provider_t),target,intent(in)::provider
    type(reference_richards_legacy_solver_t),intent(inout)::solver
    type(reference_richards_legacy_workspace_t),intent(inout)::workspace
    type(soil_water_solve_result_t),intent(out)::result
    real(real64),intent(out)::elapsed
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL20S3_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl20_stage3_timing
