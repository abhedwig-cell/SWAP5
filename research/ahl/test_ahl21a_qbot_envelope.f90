program test_ahl21a_qbot_envelope
  ...
  if(command_argument_count()/=6) error stop 'usage: test TABLE_PATH MATERIAL REGIME H0 HBOT QFACTOR'
  ...
  write(*,'(A,1X,A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL21A_HEAD_RESPONSE',trim(material),trim(regime),'REF_DH_BOTTOM=', &
  ...
  write(*,'(A,1X,A,1X,A,1X,A)') 'AHL21A_CASE',trim(material),trim(regime),'PASS'
...
contains
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL21A_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl21a_qbot_envelope
