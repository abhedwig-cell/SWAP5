program test_ahl21d_exactk
  ...
  write(*,'(A,1X,A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL21D_EXACTK_HEAD_RESPONSE',trim(material),trim(regime),'REF_DH_BOTTOM=', &
  ...
  write(*,'(A,1X,A,1X,A,1X,A)') 'AHL21D_EXACTK_CASE',trim(material),trim(regime),'PASS'
...
contains
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL21D_EXACTK_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl21d_exactk
