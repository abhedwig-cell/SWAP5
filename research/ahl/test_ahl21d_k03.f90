program test_ahl21d_k03
  ...
  if(command_argument_count()/=8) error stop 'usage: test RET K MATERIAL REGIME H0 HBOT QFACTOR DT'
  ...
  call get_command_argument(7,arg); read(arg,*) qfactor
  call get_command_argument(8,arg); read(arg,*) total_dt
  ...
  write(*,'(A,1X,A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL21D_K03_HEAD_RESPONSE',trim(material),trim(regime),'REF_DH_BOTTOM=', &
  ...
  write(*,'(A,1X,A,1X,A,1X,A)') 'AHL21D_K03_CASE',trim(material),trim(regime),'PASS'
...
contains
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL21D_K03_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl21d_k03
