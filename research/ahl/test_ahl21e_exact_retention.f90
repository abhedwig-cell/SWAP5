program test_ahl21e_exact_retention
  ...
  write(*,'(A,1X,A,1X,A,1X,A)') 'AHL21E_EXACT_RET_CASE',trim(material),trim(regime),'PASS'
...
contains
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL21E_EXACT_RET_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl21e_exact_retention
