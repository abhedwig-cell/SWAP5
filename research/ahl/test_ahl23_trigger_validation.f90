program test_ahl23_trigger_validation
  ...
  write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') 'AHL23_PATH',trim(material)//':'//trim(regime), &
       'REF_ITER=',ref_result%diagnostics%nonlinear_iterations,'CAND_ITER=',candidate_result%diagnostics%nonlinear_iterations, &
       'REF_BACKTRACK=',ref_result%diagnostics%backtracking_attempts,'CAND_BACKTRACK=',candidate_result%diagnostics%backtracking_attempts
  if(candidate_result%diagnostics%nonlinear_iterations==ref_result%diagnostics%nonlinear_iterations .and. &
     candidate_result%diagnostics%backtracking_attempts==ref_result%diagnostics%backtracking_attempts) then
    write(*,'(A,1X,A,1X,A)') 'AHL23_PATH_IDENTITY',trim(material)//':'//trim(regime),'PASS'
  else
    write(*,'(A,1X,A,1X,A)') 'AHL23_PATH_IDENTITY',trim(material)//':'//trim(regime),'FAIL'
  end if
  ...
  write(*,'(A,1X,A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'AHL23_HEAD_RESPONSE',trim(material),trim(regime),'REF_DH_BOTTOM=', &
  ...
  write(*,'(A,1X,A,1X,A,1X,A)') 'AHL23_CASE',trim(material),trim(regime),'PASS'
...
contains
  ...
  subroutine require(ok,msg)
    ...
    if(.not.ok)then;write(*,'(A,1X,A)')'AHL23_FAIL',trim(msg);error stop 1;end if
  end subroutine
end program test_ahl23_trigger_validation
