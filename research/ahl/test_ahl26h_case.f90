program test_ahl26h_case
  ...
  parameters%parameter_set_id=426020_int64;parameters%active_nodes=numnod
  ...
  write(*,'(A,1X,A,1X,A,1X,A,F10.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
   'AHL26H_METRIC',trim(material),trim(regime),'DT=',dt,'MAX_DH=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass,'ITER_DELTA=',cand_result%diagnostics%nonlinear_iterations-ref_result%diagnostics%nonlinear_iterations,'BACKTRACK_DELTA=',cand_result%diagnostics%backtracking_attempts-ref_result%diagnostics%backtracking_attempts
  ...
  write(*,'(A,1X,A,1X,A,1X,A)')'AHL26H_CASE',trim(material),trim(regime),'PASS'
contains
  ...
  subroutine req(x,msg);logical,intent(in)::x;character(len=*),intent(in)::msg;if(.not.x)then;write(*,'(A,1X,A)')'AHL26H_FAIL',trim(msg);error stop 1;end if;end subroutine
end program
