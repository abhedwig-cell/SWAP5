program test_ahl28b_fmr_provider_retention
  ...
  integer, parameter :: NREPLAY=64, NPAIR=1
  ...
  integer :: builds0,hits0,misses0,entries0,builds1,hits1,misses1,entries1
  integer :: i
  ...
  call backend_off%run_reference_floor_sample(column,template,p_off,committed_off,forcing,0.0_real64,duration,mass_tol, &
       result_off,cand_off,diag_off)
  call backend_on%run_reference_floor_sample(column,template,p_on,committed_on,forcing,0.0_real64,duration,mass_tol, &
       result_on,cand_on,diag_on)

  obs_off=backend_off%observation()
  obs_on=backend_on%observation()
  write(*,'(A,I0,1X,A,L1,1X,A,I0,1X,A,L1,1X,A,I0)') &
       'FAHL27_FLOOR_OFF_STATUS ',result_off%status,'sample_valid=',result_off%sample_valid, &
       'admission_rejections=',diag_off%admission_rejections,'solver_executed=',obs_off%solver_executed, &
       'solver_status=',obs_off%solver_status
  write(*,'(A,I0,1X,A,L1,1X,A,I0,1X,A,L1,1X,A,I0)') &
       'FAHL27_FLOOR_ON_STATUS ',result_on%status,'sample_valid=',result_on%sample_valid, &
       'admission_rejections=',diag_on%admission_rejections,'solver_executed=',obs_on%solver_executed, &
       'solver_status=',obs_on%solver_status

  call require(result_off%status==KERNEL_REFERENCE_FLOOR_STATUS_OK .and. result_off%sample_valid,'off sample valid')
  call require(result_on%status==KERNEL_REFERENCE_FLOOR_STATUS_OK .and. result_on%sample_valid,'on sample valid')
  ...
  write(*,'(A)') 'FAHL27_FLOOR_OPT_IN_RUNTIME=PASS'
  write(*,'(A)') 'AHL28_FMR_WARMUP_FIDELITY=PASS'
  call b110_adaptive_hydraulic_cache_stats(builds0,hits0,misses0,entries0)
  do i=1,NREPLAY
    call backend_on%run_reference_floor_sample(column,template,p_on,committed_on,forcing,0.0_real64,duration,mass_tol, &
         result_on,cand_on,diag_on)
    call require(result_on%status==KERNEL_REFERENCE_FLOOR_STATUS_OK,'repeated adaptive floor sample')
  end do
  call b110_adaptive_hydraulic_cache_stats(builds1,hits1,misses1,entries1)
  write(*,'(A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') 'AHL28B_FMR_CACHE_BEFORE BUILDS=',builds0,'HITS=',hits0,'MISSES=',misses0,'ENTRIES=',entries0
  write(*,'(A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') 'AHL28B_FMR_CACHE_AFTER BUILDS=',builds1,'HITS=',hits1,'MISSES=',misses1,'ENTRIES=',entries1
  write(*,'(A,I0)') 'AHL28B_FMR_ADVANCES ',NREPLAY
  write(*,'(A,I0)') 'AHL28B_FMR_HIT_DELTA ',hits1-hits0
  call require(hits1-hits0==0,'retained same-key provider must bypass shared-cache acquisition')
  call require(builds1==builds0 .and. misses1==misses0 .and. entries1==entries0,'warm cache remains stable')
  write(*,'(A)') 'AHL28B_FMR_PROVIDER_RETENTION=PASS'
...
contains
  subroutine time_batch(adaptive,elapsed)
    ...
    if(adaptive)then
      do i=1,NREPLAY
        call backend_on%run_reference_floor_sample(column,template,p_on,committed_on,forcing,0.0_real64,duration,mass_tol, &
             result_on,cand_on,diag_on)
        if(result_on%status/=KERNEL_REFERENCE_FLOOR_STATUS_OK) error stop 'timed adaptive floor sample failed'
      end do
    else
      do i=1,NREPLAY
        call backend_off%run_reference_floor_sample(column,template,p_off,committed_off,forcing,0.0_real64,duration,mass_tol, &
             result_off,cand_off,diag_off)
        if(result_off%status/=KERNEL_REFERENCE_FLOOR_STATUS_OK) error stop 'timed analytical floor sample failed'
      end do
    end if
    call cpu_time(t1)
  ...
  subroutine require(cond,msg)
    ...
    if(.not.cond)then
      write(*,'(A,1X,A)')'AHL28B_FMR_FAIL',trim(msg);error stop 1
    end if
  end subroutine
end program test_ahl28b_fmr_provider_retention
