program test_ahl39_fmr_multiauthority_timing
  ...
  integer, parameter :: NKEY=16, NREPLAY=16384, NPAIR=7
  ...
  do i=1,NKEY
    ...
    write(*,'(A,1X,I0,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
         'AHL39_FIDELITY',i,'MAX_DH=',max_dh,'MAX_DTHETA=',max_dw,'DBOTTOM=',dbot
  end do
  write(*,'(A)') 'AHL39_FIDELITY_16_OF_16=PASS'
  ...
  do r=1,NPAIR
    ...
    write(*,'(A,1X,I0,2(1X,ES18.10),1X,F10.6)') &
         'AHL39_PAIR',r,times_off(r),times_on(r),ratios(r)
  end do
  call sort7(ratios)
  write(*,'(A,1X,F10.6)') 'AHL39_MEDIAN_RATIO',ratios(4)
  write(*,'(A,1X,F10.6)') 'AHL39_MEDIAN_REDUCTION',1.0_real64-ratios(4)
  call require(ratios(4)<0.98_real64,'application speed gate')
  write(*,'(A)') 'AHL39_APPLICATION_TIMING=PASS'
...
contains
  ...
  subroutine require(cond,msg)
    ...
    if(.not.cond)then
      write(*,'(A,1X,A)')'AHL39_FAIL',trim(msg)
      error stop 1
    end if
  end subroutine require

end program test_ahl39_fmr_multiauthority_timing
