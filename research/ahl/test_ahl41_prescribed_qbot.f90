program test_ahl41_prescribed_qbot
  ...
  real(real64) :: k0, h0, qfrac, qbot, tr, ts, alpha, npar, ksat, lambda, bottom_node_delta
  character(len=512) :: label, material, arg
  logical :: valid, cache_hit
  integer :: k

  if (command_argument_count() /= 4) error stop 'usage: test MATERIAL H0 QBOT_OVER_K LABEL'
  call get_command_argument(1,material)
  call get_command_argument(2,arg); read(arg,*) h0
  call get_command_argument(3,arg); read(arg,*) qfrac
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
         'AHL41_METRIC',trim(name),'K0=',k0,'QBOT=',qbot,'MAX_DH=',dh,'BOTTOM_DH=',bottom_node_delta, &
         'MAX_DTHETA=',dw,'DBOTTOM_FLUX=',dbf
    write(*,'(A,1X,A,1X,A,ES18.10,1X,A,ES18.10,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,ES14.6)') &
         'AHL41_DIAG',trim(name),'HREF_BOTTOM=',href,'HCAND_BOTTOM=',hcand, &
    ...
    write(*,'(A,1X,A,1X,A)') 'AHL41',trim(name),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL41_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl41_prescribed_qbot
