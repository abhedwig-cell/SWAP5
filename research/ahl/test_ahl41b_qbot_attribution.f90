program test_ahl41b_qbot_attribution
  ...
  use mod_ahl41b_attribution_provider, only: ahl41b_attribution_provider_t, bind_ahl41b_attribution_provider, &
       AHL41B_ADAPTIVE_ALL, AHL41B_EXACT_RC, AHL41B_EXACT_K, AHL41B_ANALYTICAL_ALL
  implicit none
  ...
  type(ahl41b_attribution_provider_t), target :: lookup
  ...
  if (command_argument_count() /= 4) error stop 'usage: test MATERIAL H0 QBOT MODE'
  ...
  case('adaptive_all'); mode=AHL41B_ADAPTIVE_ALL
  case('exact_rc'); mode=AHL41B_EXACT_RC
  case('exact_k'); mode=AHL41B_EXACT_K
  case('analytical_all'); mode=AHL41B_ANALYTICAL_ALL
  case default; error stop 'unknown AHL41B mode'
  end select
  ...
  call bind_ahl41b_attribution_provider(lookup,hp,total_dt,mode,valid)
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,ES14.6)') &
      'AHL41B_METRIC',trim(name),'MAX_DH=',dh,'BOTTOM_DH=',bottom_node_delta,'MAX_DTHETA=',dw, &
      ...
      'DBOTTOM_FLUX=',dbf
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES18.10,1X,A,ES18.10)') &
      'AHL41B_DIAG',trim(name),'MASS=',mass,'HREF_BOTTOM=',href,'HCAND_BOTTOM=',hcand
    ...
    write(*,'(A,1X,A,1X,A)') 'AHL41B',trim(name),'MEASURED'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'AHL41B_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl41b_qbot_attribution
