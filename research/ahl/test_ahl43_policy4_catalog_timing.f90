program test_ahl43_policy4_catalog_timing
  ...
  integer, parameter :: NPAIR=5, NREPLAY=4096
  real(real64) :: k0, h0, hbot, tr, ts, alpha, nvg, ksat, lambda, pfile(6)
  real(real64) :: times_ref(NPAIR), times_adapt(NPAIR), ratios(NPAIR), t0, t1
  integer :: r, ireplay
  ...
  call bind_b110_adaptive_hydraulic_provider(lookup,hp,total_dt,valid,was_hit)
  call require(valid,'production adaptive provider bound')
  ...
  do r=1,NPAIR
    if(mod(r,2)==1) then
      call time_reference(times_ref(r))
      call time_adaptive(times_adapt(r))
    else
      call time_adaptive(times_adapt(r))
      call time_reference(times_ref(r))
    end if
    ratios(r)=times_adapt(r)/times_ref(r)
    write(*,'(A,1X,A,1X,I0,2(1X,ES18.10),1X,F10.6)') 'FAHL43_PAIR',trim(material),r, &
         times_ref(r),times_adapt(r),ratios(r)
  end do
  call sort5(ratios)
  write(*,'(A,1X,A,1X,F10.6)') 'FAHL43_MEDIAN_RATIO',trim(material),ratios(3)
  write(*,'(A,1X,A,1X,F10.6)') 'FAHL43_MEDIAN_REDUCTION',trim(material),1.0_real64-ratios(3)

contains

  subroutine time_reference(elapsed)
    real(real64),intent(out)::elapsed
    call cpu_time(t0)
    do ireplay=1,NREPLAY
      request%evaluation%constitutive=>analytical
      request%base_state=initial_state
      call solver_ref%solve(request,workspace_ref,reference_result)
      if(reference_result%status/=SW_SOLVE_CONVERGED) error stop 'FAHL43 timed analytical failure'
    end do
    call cpu_time(t1)
    elapsed=(t1-t0)/real(NREPLAY,real64)
  end subroutine time_reference

  subroutine time_adaptive(elapsed)
    real(real64),intent(out)::elapsed
    call cpu_time(t0)
    do ireplay=1,NREPLAY
      request%evaluation%constitutive=>lookup
      request%base_state=initial_state
      call solver_candidate%solve(request,workspace_candidate,candidate_result)
      if(candidate_result%status/=SW_SOLVE_CONVERGED) error stop 'FAHL43 timed adaptive failure'
    end do
    call cpu_time(t1)
    elapsed=(t1-t0)/real(NREPLAY,real64)
  end subroutine time_adaptive
  ...
  subroutine read_material_from_table(path,p)
    character(len=*),intent(in)::path
    real(real64),intent(out)::p(6)
    integer::u,ios
    open(newunit=u,file=path,status='old',action='read',iostat=ios)
    if(ios/=0) error stop 'cannot open AHL11B table'
    read(u,*,iostat=ios)p
    close(u)
    if(ios/=0) error stop 'cannot read AHL11B parameters'
  end subroutine read_material_from_table
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'FAHL43_METRIC',trim(material)//':'//trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'FAHL43_DIAG',trim(material)//':'//trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'FAHL43',trim(material)//':'//trim(name),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'FAHL43_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ahl43_policy4_catalog_timing
