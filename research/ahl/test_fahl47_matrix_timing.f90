program test_fahl47_matrix_timing
  ...
  integer, parameter :: NPAIR=5, NREPLAY=5000
  ...
  real(real64) :: times_ref(NPAIR), times_fast(NPAIR), ratios(NPAIR), t0, t1
  integer :: r, ireplay
  ...
  call require(valid,'self-built provider bound')
  ...
  do r=1,NPAIR
    if(mod(r,2)==1)then
      ...
      call time_fast(times_fast(r))
    else
      call time_fast(times_fast(r))
      call time_reference(times_ref(r))
    end if
    ratios(r)=times_fast(r)/times_ref(r)
    write(*,'(*(g0))') 'FAHL47_TIMING_PAIR|CASE=',trim(material)//':'//trim(label),'|PAIR=',r, &
         '|REF=',times_ref(r),'|FAST=',times_fast(r),'|RATIO=',ratios(r)
  end do
  call sort5(ratios)
  write(*,'(*(g0))') 'FAHL47_TIMING_MEDIAN|CASE=',trim(material)//':'//trim(label),'|RATIO=',ratios(3)
...
contains
  ...
  subroutine time_reference(elapsed)
    ...
    do ireplay=1,NREPLAY
      ...
      request%base_state=initial_state
      call solver_ref%solve(request,workspace_ref,reference_result)
      if(reference_result%status/=SW_SOLVE_CONVERGED)error stop 'timed reference failed'
    end do
    call cpu_time(t1)
    elapsed=(t1-t0)/real(NREPLAY,real64)
  end subroutine

  subroutine time_fast(elapsed)
    real(real64),intent(out)::elapsed
    call cpu_time(t0)
    do ireplay=1,NREPLAY
      request%evaluation%constitutive=>lookup
      request%base_state=initial_state
      call solver_candidate%solve(request,workspace_candidate,candidate_result)
      if(candidate_result%status/=SW_SOLVE_CONVERGED)error stop 'timed candidate failed'
    end do
    call cpu_time(t1)
    elapsed=(t1-t0)/real(NREPLAY,real64)
  end subroutine
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'FAHL47_MATRIX_METRIC',trim(material)//':'//trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'FAHL47_MATRIX_DIAG',trim(material)//':'//trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'FAHL47_MATRIX',trim(material)//':'//trim(name),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'FAHL47_MATRIX_FAIL',trim(message)
      error stop 1
    end if
