program test_fahl48_resolution_matrix
  ...
  logical :: valid
  integer :: k, resolution

  if (command_argument_count() /= 5) error stop 'usage: test RESOLUTION MATERIAL REGIME H0 HBOT'
  call get_command_argument(1,arg); read(arg,*) resolution
  ...
  if (resolution /= 64 .and. resolution /= 128 .and. resolution /= 256) error stop 'unsupported resolution'
  ...
  call bind_ahl47_provider(lookup,hp,total_dt,valid,resolution)
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A)') 'FAHL48_MATRIX',trim(material)//':'//trim(name),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'FAHL48_MATRIX_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_fahl48_resolution_matrix
