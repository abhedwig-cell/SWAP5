program test_fahl48_shared_richards_timing
  ...
  type(ahl48_shared_provider_t),target::adaptive
  ...
  call bind_ahl48_shared_provider(adaptive,hp,dt,ok,hit);call require(ok,'adaptive bind')
  ...
  write(*,'(*(g0))') 'FAHL48_SHARED_RICHARDS_FIDELITY|DH=',dh,'|DTHETA=',dtheta,'|ITER=',ra%diagnostics%nonlinear_iterations,'|BACKTRACK=',ra%diagnostics%backtracking_attempts,'|REF_DEMAND=',ra%diagnostics%constitutive_candidate_demand_evaluations,'|ADAPT_DEMAND=',rb%diagnostics%constitutive_candidate_demand_evaluations
  ...
  do r=1,npair
    ...
    rat(r)=tb(r)/ta(r)
    write(*,'(*(g0))') 'FAHL48_SHARED_RICHARDS_PAIR|PAIR=',r,'|REF=',ta(r),'|ADAPT=',tb(r),'|RATIO=',rat(r)
  end do
  call sortv(rat)
  write(*,'(*(g0))') 'FAHL48_SHARED_RICHARDS_MEDIAN_RATIO=',rat((npair+1)/2)
  write(*,'(A)') 'FAHL48_SHARED_RICHARDS_TIMING=PASS'
contains
  ...
  subroutine require(cond,label)
    ...
    if(.not.cond)then;write(*,'(A,1X,A)')'FAHL48_SHARED_FAIL',trim(label);error stop 1;end if
  end subroutine
end program
