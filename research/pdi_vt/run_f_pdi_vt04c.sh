...
TMP="$ROOT/research/pdi_vt/.f_pdi_vt04c_runtime.sh"
...
sed -i 's/f_pdi_vt04_make_case.py/f_pdi_vt04c_case.py/g' "$TMP"
...
for name in ["control-vap","candidate-vap","control-novap","candidate-novap"]:
    ...
    p[name]={"sha256":hashlib.sha256(b.encode()).hexdigest(),"nonfinite_tokens":len(bad),"finite":len(bad)==0}
...
Path("/tmp/f_pdi_vt04c_finite.json").write_text(json.dumps(p,indent=2)+"\n")
