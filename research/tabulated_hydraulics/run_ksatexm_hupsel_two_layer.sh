...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/tabhyd-ksatexm-two-${GITHUB_RUN_ID:-local}-$$"
...
run_one(){
  ...
  gfortran "${COMMON[@]}" -O"$opt" -J "$out" -I "$out"     research/tabulated_hydraulics/test_ksatexm_hupsel_two_layer.f90     "$out/contract.o" "$out/mvg.o" "$out/tspack.o" "$out/state.o" "$out/provider.o" -o "$out/test"
...
echo 'TABHYD_KSATEXM_TWO_LAYER_O0_O2_IDENTITY=PASS'
