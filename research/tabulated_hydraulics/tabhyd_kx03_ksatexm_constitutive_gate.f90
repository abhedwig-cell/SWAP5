program tabhyd_kx03_ksatexm_constitutive_gate
  ...
  real(real64) :: frac,expo,relsa,relst,seauth,thetaauth,f,err,relerr
  ...
  real(real64) :: max_transition_abs, continuity_jump, below_kdiff, fmin_active, fmax_active, &
       max_auth_theta_diff, max_auth_se_diff
  ...
  f_nonfinite=0
  max_auth_theta_diff=0.0_real64
  max_auth_se_diff=0.0_real64
  ...
  do q=1,NSCAN
    ...
    do i=1,NODES
      ...
      thetaauth=authority_theta(cof(:,i),h(i))
      seauth=(thetaauth-cof(1,i))/(cof(2,i)-cof(1,i))
      max_auth_theta_diff=max(max_auth_theta_diff,abs(thetaauth-ta(i)))
      max_auth_se_diff=max(max_auth_se_diff,abs(seauth-relsa))
      if ((relsa>cof(11,i)) .neqv. (seauth>cof(11,i))) mismatch=mismatch+1
      if (seauth>cof(11,i)) then
        f=(seauth-cof(11,i))/(1.0_real64-cof(11,i))
  ...
  do q=1,10001
    ...
    do i=1,NODES
      ...
      max_auth_theta_diff=max(max_auth_theta_diff,abs(thetaauth-ta(i)))
      max_auth_se_diff=max(max_auth_se_diff,abs(seauth-relsa))
      if ((relsa>cof(11,i)) .neqv. (seauth>cof(11,i))) then
  ...
  write(*,'(a,2(1x,es24.16))') 'KX03_SAT_ANALYTIC',ka
  write(*,'(a,2(1x,es24.16))') 'KX03_SAT_CANDIDATE',kc
  ...
  write(*,'(a,2(1x,es24.16))') 'KX03_HM1_ANALYTIC',ka
  write(*,'(a,2(1x,es24.16))') 'KX03_HM1_CANDIDATE',kc

  h=-5.0_real64
  call analytic_on%evaluate(h,ta,ka,ca,da)
  call analytic_off%evaluate(h,td,kd,cd,dd)
  below_kdiff=maxval(abs(ka-kd))
  write(*,'(a,es24.16)') 'KX03_BELOW_THRESHOLD_ANALYTIC_ON_OFF_MAX=',below_kdiff

  ! Evaluate candidate immediately around exact h=-2 branch.
  h=HTHR-1.0e-10_real64
  call table%evaluate(h,tt,kt,ct,dt)
  relst=(tt(2)-cof(1,2))/(cof(2,2)-cof(1,2))
  thetaauth=authority_theta(cof(:,2),h(2)); seauth=(thetaauth-cof(1,2))/(cof(2,2)-cof(1,2))
  kleft=kt(2)
  if(seauth>cof(11,2)) then
    f=(seauth-cof(11,2))/(1.0_real64-cof(11,2)); kleft=f*cof(10,2)+(1.0_real64-f)*cof(12,2)
  end if
  ...
  keq=kt(2)
  if(seauth>cof(11,2)) then
    f=(seauth-cof(11,2))/(1.0_real64-cof(11,2)); keq=f*cof(10,2)+(1.0_real64-f)*cof(12,2)
  end if
  ...
  kright=kt(2)
  if(seauth>cof(11,2)) then
    f=(seauth-cof(11,2))/(1.0_real64-cof(11,2)); kright=f*cof(10,2)+(1.0_real64-f)*cof(12,2)
  end if
  continuity_jump=max(abs(keq-kleft),abs(kright-keq))

  write(*,'(a,es24.16)') 'KX03_THETA_MAX_ABS=',max_theta
  write(*,'(a,es24.16)') 'KX03_C_MAX_ABS=',max_c
  write(*,'(a,es24.16)') 'KX03_LOG10K_MAX_ABS=',max_logk
  write(*,'(a,es24.16)') 'KX03_BRANCH_K_MAX_ABS=',max_branch_abs
  write(*,'(a,es24.16)') 'KX03_BRANCH_K_MAX_REL=',max_branch_rel
  write(*,'(a,es24.16)') 'KX03_TRANSITION_K_MAX_ABS=',max_transition_abs
  write(*,'(a,i0)') 'KX03_BRANCH_CLASS_MISMATCH=',mismatch
  write(*,'(a,i0)') 'KX03_BRANCH_CLASS_MISMATCH_LOCAL=',mismatch_local
  write(*,'(a,2(1x,i0))') 'KX03_BRANCH_CLASS_MISMATCH_NODE',mismatch_node
  write(*,'(a,2(1x,es24.16))') 'KX03_BRANCH_MISMATCH_HMIN',mismatch_hmin
  write(*,'(a,2(1x,es24.16))') 'KX03_BRANCH_MISMATCH_HMAX',mismatch_hmax
  write(*,'(a,i0)') 'KX03_ACTIVE_SAMPLES=',active_count
  write(*,'(a,es24.16)') 'KX03_CONTINUITY_LOCAL_JUMP=',continuity_jump
  write(*,'(a,2(1x,es24.16))') 'KX03_THRESHOLDS',cof(11,1),cof(11,2)
  write(*,'(a,2(1x,es24.16))') 'KX03_KTHR',cof(12,1),cof(12,2)
  write(*,'(a,es24.16)') 'KX03_ACTIVE_F_MIN=',fmin_active
  write(*,'(a,es24.16)') 'KX03_ACTIVE_F_MAX=',fmax_active
  write(*,'(a,i0)') 'KX03_ACTIVE_F_NONFINITE=',f_nonfinite
  write(*,'(a,es24.16)') 'KX03_AUTH_THETA_MAX_ABS=',max_auth_theta_diff
  write(*,'(a,es24.16)') 'KX03_AUTH_SE_MAX_ABS=',max_auth_se_diff
  write(*,'(a)') 'KX03_CONSTITUTIVE_GATE_COMPLETED'
contains
  ...
  pure real(real64) function authority_theta(c,hv) result(theta)
    ...
    if(hv>=0.0_real64) then
      theta=c(2)
    else if(hv> -1.0e-2_real64) then
      help=abs(c(4)*(-1.0e-2_real64))**c(6)
      theta_crit=c(1)+delta/((1.0_real64+help)**m)
      wet_capacity=(c(2)-theta_crit)/1.0e-2_real64
      theta=theta_crit+wet_capacity*(hv+1.0e-2_real64)
      theta=min(theta,c(2))
    else
      help=abs(c(4)*hv)**c(6)
...
end program tabhyd_kx03_ksatexm_constitutive_gate
