program tabhyd_reference_solver_integration_benchmark
  ...
  real(real64) :: step_duration, top_flux, bottom_flux, max_h, max_theta, checksum_a, checksum_t
  real(real64) :: t0,t1, atime(NROUNDS), ttime(NROUNDS), med_a, med_t
  integer :: nodes, nt, i,j,r,rep,iu,ios
  character(len=512) :: path
  character(len=64) :: scenario, soil

  if(command_argument_count()<1) error stop 'usage: ref-solver-bench INPUT'
  call get_command_argument(1,path)
  open(newunit=iu,file=trim(path),status='old',action='read',iostat=ios)
  if(ios/=0) error stop 'cannot open input'
  read(iu,*,iostat=ios) scenario, nodes, nt, step_duration, top_flux, bottom_flux
  ...
  write(*,'(a,a)') 'INTEGRATED_SCENARIO=',trim(scenario)
  write(*,'(a,i0)') 'INTEGRATED_ANALYTIC_STATUS=',res_a%status
  write(*,'(a,i0)') 'INTEGRATED_TABLE_STATUS=',res_t%status
  write(*,'(a,i0)') 'INTEGRATED_ANALYTIC_ITERS=',res_a%diagnostics%nonlinear_iterations
  write(*,'(a,i0)') 'INTEGRATED_TABLE_ITERS=',res_t%diagnostics%nonlinear_iterations
  ...
  write(*,'(a,es24.16)') 'INTEGRATED_HEAD_MAX_ABS=',max_h
  write(*,'(a,es24.16)') 'INTEGRATED_THETA_MAX_ABS=',max_theta
  ...
  do r=1,NROUNDS
    ...
    write(*,'(a,i0,a,es16.8,a,es16.8)') 'INTEGRATED_BLOCK round=',r,' analytic_s=',atime(r),' table_s=',ttime(r)
  end do
  med_a=median_small(atime); med_t=median_small(ttime)
  write(*,'(a,es24.16)') 'INTEGRATED_ANALYTIC_MEDIAN_S=',med_a
  write(*,'(a,es24.16)') 'INTEGRATED_TABLE_MEDIAN_S=',med_t
  write(*,'(a,f14.8)') 'INTEGRATED_TABLE_DELTA_PCT=',100.0_real64*(med_t/med_a-1.0_real64)
  write(*,'(a,es24.16)') 'INTEGRATED_ANALYTIC_CHECKSUM=',checksum_a
  write(*,'(a,es24.16)') 'INTEGRATED_TABLE_CHECKSUM=',checksum_t
...
end program tabhyd_reference_solver_integration_benchmark
