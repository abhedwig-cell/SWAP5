program tabhyd_typed_reference_runtime_pair
  ...
  integer, parameter :: NREPEAT=1200
  ...
  type(tabhyd_raw_provider_t), target :: table_provider
  class(transaction_state_t), allocatable :: snapshot
  real(real64) :: k0, h0, t0, t1
  real(real64), allocatable :: headtab(:,:), thetatab(:,:), conductab(:,:)
  logical :: ok, available
  integer :: rep, i
  character(len=32) :: material, mode
  character(len=512) :: table_path

  material='loam'; mode='analytic'; table_path=''
  ...
  if (command_argument_count()>=3) call get_command_argument(3,table_path)
  ...
  if (trim(mode)=='table') then
    call read_table(trim(table_path),parameters,headtab,thetatab,conductab)
    call initialize_tabhyd_raw_provider(table_provider,headtab,thetatab,conductab,parameters%cofgen,duration)
    call backend%set_research_constitutive_override(table_provider)
  else if (trim(mode)/='analytic') then
...
contains
  ...
  subroutine read_table(path,p,heads,waters,conductivities)
    character(len=*), intent(in) :: path
    type(fmr_b110_physical_parameters_t), intent(in) :: p
    real(real64), allocatable, intent(out) :: heads(:,:),waters(:,:),conductivities(:,:)
    integer :: iu,ios,nt,i,j
    real(real64) :: h,t,k
    if (len_trim(path)==0) error stop 'table mode requires table input'
    open(newunit=iu,file=trim(path),status='old',action='read',iostat=ios)
    if (ios/=0) error stop 'cannot open table input'
    read(iu,*,iostat=ios) nt
    if (ios/=0 .or. nt/=TABHYD_RAW_TABLE_N) error stop 'invalid table row count'
    allocate(heads(nt,p%active_nodes),waters(nt,p%active_nodes),conductivities(nt,p%active_nodes))
    do j=1,nt
      read(iu,*,iostat=ios) h,t,k
      if (ios/=0) error stop 'invalid table row'
      heads(j,:)=h; waters(j,:)=t; conductivities(j,:)=k
    end do
    close(iu)
...
end program tabhyd_typed_reference_runtime_pair
