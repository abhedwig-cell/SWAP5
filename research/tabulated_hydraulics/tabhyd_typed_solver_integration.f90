program tabhyd_typed_solver_integration
  ...
  do r=1,NROUNDS
    ...
    write(*,'(a,i0,a,es16.8,a,es16.8)') 'TABHYD_SOLVER_BLOCK round=',r,' analytic_s=',atime(r),' table_s=',ttime(r)
  end do

  med_a=median_small(atime)
  med_t=median_small(ttime)
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_DH_MAX=',max_h
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_DH_RMS=',rms_h
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_DTHETA_MAX=',max_theta
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_DTHETA_RMS=',rms_theta
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_DQTOP=',dtop
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_DQBOT=',dbot
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_DMASS=',dmass
  write(*,'(a,i0)') 'TABHYD_SOLVER_ANALYTIC_NONLINEAR_ITERS=',iter_a
  write(*,'(a,i0)') 'TABHYD_SOLVER_TABLE_NONLINEAR_ITERS=',iter_t
  write(*,'(a,i0)') 'TABHYD_SOLVER_ANALYTIC_LINEAR_SOLVES=',lin_a
  write(*,'(a,i0)') 'TABHYD_SOLVER_TABLE_LINEAR_SOLVES=',lin_t
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_ANALYTIC_MEDIAN_S=',med_a
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_TABLE_MEDIAN_S=',med_t
  write(*,'(a,f14.8)') 'TABHYD_SOLVER_TABLE_DELTA_PCT=',100.0_real64*(med_t/med_a-1.0_real64)
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_ANALYTIC_CHECKSUM=',checksum_a
  write(*,'(a,es24.16)') 'TABHYD_SOLVER_TABLE_CHECKSUM=',checksum_t
  write(*,'(a)') 'TABHYD_TYPED_SOLVER_INTEGRATION=PASS'
...
contains
  ...
  subroutine require(ok,label)
    ...
    if(.not.ok) then
      write(*,'(a,1x,a)') 'TABHYD_SOLVER_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program tabhyd_typed_solver_integration
