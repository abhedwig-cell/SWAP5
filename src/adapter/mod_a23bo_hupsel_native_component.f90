module mod_a23bo_hupsel_native_component
  use, intrinsic :: iso_fortran_env, only: real64
  use mod_transaction_reference, only: transaction_state_t, transaction_model_t, trial_outcome_t
  use mod_a23bo_legacy_solver_diagnostics, only: a23bo_reset_solver_diagnostics, a23bo_internal_retry_count
  ...
  type, extends(transaction_model_t), public :: hupsel_native_component_t
  ...
  end type hupsel_native_component_t

  public :: initialize_hupsel_component, finalize_hupsel_component
...
contains
  ...
  subroutine initialize_hupsel_component(model, committed, swp_file, outfile, seed_start, seed_end, base_t1900)
    type(hupsel_native_component_t), intent(out) :: model
    ...
    if (swmacro /= 0 .or. swsnow /= 0 .or. swhea /= 1 .or. swsolu /= 1) then
      error stop 'A23BO component: Hupsel qualification configuration mismatch'
    end if

    t0 = seed_start
    t1 = seed_end
    call a23bo_reset_solver_diagnostics()
    ...
    class default
      error stop 'A23BO component: state allocation failure'
    end select
  end subroutine initialize_hupsel_component

  subroutine finalize_hupsel_component()
    real(real64) :: t0, t1
    t0 = 0.0_real64
    t1 = 0.0_real64
    call SWAP(0, 3, t0, t1)
  end subroutine finalize_hupsel_component
  ...
  subroutine hupsel_column_clone(self, copy)
    ...
    class default
      error stop 'A23BO component: clone allocation failure'
    end select
  end subroutine hupsel_column_clone

  subroutine hupsel_component_advance(self, state, t0, t1, outcome)
    class(hupsel_native_component_t), intent(inout) :: self
    ...
    type is (hupsel_column_state_t)
      ...
      self%last_diagnostics%nonlinear_iterations = max(0, iter_after - iter_before)
      self%last_diagnostics%internal_retries = a23bo_internal_retry_count()
  ...
  function hupsel_component_storage(self, state) result(value)
    class(hupsel_native_component_t), intent(in) :: self
  ...
  subroutine restore_hupsel_column_state(state)
    ...
    if (.not. allocated(state%physical%h) .or. .not. allocated(state%replay%cmsy)) then
      error stop 'A23BO component: incomplete column state'
    end if
...
end module mod_a23bo_hupsel_native_component
