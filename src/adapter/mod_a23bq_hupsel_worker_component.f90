module mod_a23bq_hupsel_worker_component
  ...
  use variables, only: h, theta, pond, dt, ldwet, spev, saev, gwl, volact
  ...
  use mod_a23bq_worker_execution_context, only: a23bq_worker_context_t, a23bq_initialize_worker, &
       a23bq_release_worker, a23bq_reset_attempt_diagnostics, a23bq_scratch_payload_bytes
  implicit none
  private
  ...
  type, extends(transaction_model_t), public :: hupsel_worker_component_t
    ...
    type(a23bq_worker_context_t) :: worker
  ...
  interface
    subroutine SWAP(iCaller, iTask, tstart_in, tend_in, swp_file, outfile, worker)
      import :: fillen, a23bq_worker_context_t
...
contains
  ...
  subroutine initialize_hupsel_worker_component(model, committed, swp_file, outfile, seed_start, seed_end, base_t1900)
    ...
    if (swmacro /= 0 .or. swsnow /= 0 .or. swhea /= 1 .or. swsolu /= 1) then
      error stop 'A23BQ component: Hupsel qualification configuration mismatch'
    end if

    t0 = seed_start
    t1 = seed_end
    call a23bq_initialize_worker(model%worker, numnod, 0)
    call a23bq_reset_attempt_diagnostics(model%worker)
    ...
    class default
      error stop 'A23BQ component: state allocation failure'
    end select
  ...
  subroutine finalize_hupsel_worker_component(model)
    ...
    t1 = 0.0_real64
    call SWAP(0, 3, t0, t1)
    call a23bq_release_worker(model%worker)
  ...
  subroutine hupsel_column_clone(self, copy)
    ...
    class default
      error stop 'A23BQ component: clone allocation failure'
    end select
  ...
  subroutine hupsel_component_advance(self, state, t0, t1, outcome)
    ...
    type is (hupsel_column_state_t)
      ...
      call a23bq_reset_attempt_diagnostics(self%worker)
  ...
  subroutine restore_hupsel_column_state(state)
    ...
    if (.not. allocated(state%physical%h) .or. .not. allocated(state%replay%cmsy)) then
      error stop 'A23BQ component: incomplete column state'
    end if
  ...
  integer function hupsel_worker_scratch_payload_bytes(model) result(bytes)
    ...
    bytes = a23bq_scratch_payload_bytes(model%worker)
...
end module mod_a23bq_hupsel_worker_component
