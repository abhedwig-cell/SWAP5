module mod_a23bt_hupsel_worker_component
  ...
  use mod_a23bt_worker_execution_context, only: a23bt_worker_context_t, a23bt_initialize_worker, &
       a23bt_release_worker, a23bt_reset_attempt_diagnostics, a23bt_seed_timestep_control, &
       a23bt_seed_execution_window, a23bt_seed_reporting_progress, a23bt_scratch_payload_bytes
  implicit none
  private
  ...
  type, extends(transaction_model_t), public :: hupsel_worker_component_t
    ...
    type(a23bt_worker_context_t) :: worker
  ...
  interface
    subroutine SWAP(iCaller, iTask, tstart_in, tend_in, swp_file, outfile, worker)
      import :: fillen, a23bt_worker_context_t
      ...
      type(a23bt_worker_context_t), intent(inout), optional :: worker
    end subroutine SWAP
  end interface
...
contains
  ...
  subroutine initialize_hupsel_worker_component(model, committed, swp_file, outfile, seed_start, seed_end, base_t1900)
    ...
    if (swmacro /= 0 .or. swsnow /= 0 .or. swhea /= 1 .or. swsolu /= 1) then
      error stop 'A23BT component: Hupsel qualification configuration mismatch'
    end if
    if (nprintday /= 1 .or. flprintdt) then
      error stop 'A23BT component: qualification requires NPRINTDAY=1 and FLPRINTDT=.false.'
    end if

    t0 = seed_start
    t1 = seed_end
    call a23bt_initialize_worker(model%worker, numnod, 0)
    call a23bt_reset_attempt_diagnostics(model%worker)
    ...
    class default
      error stop 'A23BT component: state allocation failure'
    end select
  ...
  subroutine finalize_hupsel_worker_component(model)
    ...
    t1 = 0.0_real64
    call SWAP(0, 3, t0, t1)
    call a23bt_release_worker(model%worker)
  ...
  subroutine hupsel_column_clone(self, copy)
    ...
    class default
      error stop 'A23BT component: clone allocation failure'
    end select
  ...
  subroutine hupsel_component_advance(self, state, t0, t1, outcome)
    ...
    type is (hupsel_column_state_t)
      ...
      call a23bt_seed_timestep_control(self%worker, dt, dtmin)
      legacy_start = self%base_t1900 + real(nint(t0), real64)
      legacy_end = self%base_t1900 + real(nint(t1), real64) - 1.0_real64
      call a23bt_seed_execution_window(self%worker, t0, t1, legacy_start, legacy_end)
      call a23bt_seed_reporting_progress(self%worker)
      in0 = current_mass_in_total()
      out0 = current_mass_out_total()
      call a23bt_reset_attempt_diagnostics(self%worker)
  ...
  subroutine restore_hupsel_column_state(state)
    ...
    if (.not. allocated(state%physical%h) .or. .not. allocated(state%replay%cmsy)) then
      error stop 'A23BT component: incomplete column state'
    end if
  ...
  integer function hupsel_worker_scratch_payload_bytes(model) result(bytes)
    ...
    bytes = a23bt_scratch_payload_bytes(model%worker)
...
end module mod_a23bt_hupsel_worker_component
