module mod_a23bu_hupsel_worker_component
  ...
  use variables, only: h, theta, pond, dt, dtmin, ldwet, spev, saev, gwl, volact, &
       t1900, t, tcum, timjan1, daynr, daycum, iyear, imonth, date, nprintday, flprintdt
  ...
  use mod_a23bu_worker_execution_context, only: a23bu_worker_context_t, a23bu_initialize_worker, &
       a23bu_release_worker, a23bu_reset_attempt_diagnostics, a23bu_seed_timestep_control, &
       a23bu_seed_execution_window, a23bu_seed_reporting_progress, a23bu_scratch_payload_bytes
  implicit none
  private
  ...
  type, public :: hupsel_irrigation_continuation_t
    integer :: dayfix = 0
    integer :: nirri = 0
  end type hupsel_irrigation_continuation_t

  type, public :: hupsel_process_cursor_t
    type(hupsel_irrigation_continuation_t) :: irrigation
  ...
  type, extends(transaction_model_t), public :: hupsel_worker_component_t
    ...
    logical :: crop_calendar_enabled = .false.
  ...
  interface
    subroutine SWAP(iCaller, iTask, tstart_in, tend_in, swp_file, outfile, worker)
      import :: fillen, a23bu_worker_context_t
...
contains
  ...
  subroutine initialize_hupsel_worker_component(model, committed, swp_file, outfile, seed_start, seed_end, base_t1900)
    ...
    if (swmacro /= 0 .or. swsnow /= 0 .or. swhea /= 1 .or. swsolu /= 1) then
      error stop 'A23BU component: Hupsel qualification configuration mismatch'
    end if
    if (nprintday /= 1 .or. flprintdt) then
      error stop 'A23BU component: qualification requires NPRINTDAY=1 and FLPRINTDT=.false.'
    end if

    t0 = seed_start
    t1 = seed_end
    call a23bu_initialize_worker(model%worker, numnod, 0)
    call a23bu_reset_attempt_diagnostics(model%worker)
    ...
    model%irrigation_enabled = flirrigate
    model%crop_calendar_enabled = fl_cropcalendar
    ...
    class default
      error stop 'A23BU component: state allocation failure'
    end select
  ...
  subroutine finalize_hupsel_worker_component(model)
    ...
    t1 = 0.0_real64
    call SWAP(0, 3, t0, t1)
    call a23bu_release_worker(model%worker)
  ...
  subroutine hupsel_column_clone(self, copy)
    ...
    class default
      error stop 'A23BU component: clone allocation failure'
    end select
  ...
  subroutine hupsel_component_advance(self, state, t0, t1, outcome)
    ...
    type is (hupsel_column_state_t)
      ...
      flirrigate = self%irrigation_enabled
      fl_cropcalendar = self%crop_calendar_enabled
      call reset_legacy_irrigation_event_workspace()
      call a23bu_seed_timestep_control(self%worker, dt, dtmin)
      legacy_start = self%base_t1900 + real(nint(t0), real64)
      legacy_end = self%base_t1900 + real(nint(t1), real64) - 1.0_real64
      call a23bu_seed_execution_window(self%worker, t0, t1, legacy_start, legacy_end)
      call a23bu_seed_reporting_progress(self%worker)
      in0 = current_mass_in_total()
      out0 = current_mass_out_total()
      call a23bu_reset_attempt_diagnostics(self%worker)
  ...
  subroutine capture_hupsel_column_state(state)
    ...
    state%process%irrigation%dayfix = dayfix
    state%process%irrigation%nirri = nirri
  ...
  subroutine restore_hupsel_column_state(state)
    ...
    if (.not. allocated(state%physical%h) .or. .not. allocated(state%replay%cmsy)) then
      error stop 'A23BU component: incomplete column state'
    end if
    ...
    dayfix = state%process%irrigation%dayfix
    nirri = state%process%irrigation%nirri
  ...
  subroutine reset_legacy_irrigation_event_workspace()
    irrigevent = 0
    gird = 0.0_real64
    ...
    cirr = 0.0_real64
    nird = 0.0_real64
    qssdi(1:numnod) = 0.0_real64
    qssdisum = 0.0_real64
  end subroutine reset_legacy_irrigation_event_workspace
  ...
  integer function hupsel_worker_scratch_payload_bytes(model) result(bytes)
    ...
    bytes = a23bu_scratch_payload_bytes(model%worker)
...
end module mod_a23bu_hupsel_worker_component
