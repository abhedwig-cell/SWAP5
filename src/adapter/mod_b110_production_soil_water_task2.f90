...
contains
  ...
  logical function m1_b111_legacy_application_profile_active() result(active)
    ...
    ! application-process envelope required to prove the legacy-file-to-typed
    ! seam. It does not parse files and does not alter pre-existing routes.
    ...
    ! F-SI39 is opt-in. Enable it only where the exact B1.11
    ! parameterization declares a KSATEXM extension.
