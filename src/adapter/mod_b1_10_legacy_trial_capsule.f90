module mod_b1_10_legacy_trial_capsule
  ...
  use MOD_Solute, only: imdectot, imrottot, imsqbot, imsqdra, imsqirrig, imsqprec, imqsol, &
       sbaldev, sdstor, sampro, rottot, sqdra, isqbot, dectot, sqprec, sqirrig, sqbot, samini, isqtop
  ...
  type, public :: b1_10_legacy_trial_capsule_t
    ...
    real(real64) :: t = 0.0_real64, t1900 = 0.0_real64, tcum = 0.0_real64, timjan1 = 0.0_real64
    real(real64) :: tstart = 0.0_real64, tend = 0.0_real64
    ...
    real(real64) :: cirr = 0.0_real64, dt_irr_event = 0.0_real64, gird = 0.0_real64, nird = 0.0_real64, qssdisum = 0.0_real64
    ...
    real(real64) :: iqrot = 0.0_real64, iqssdi = 0.0_real64, iqredwet = 0.0_real64, iqreddry = 0.0_real64
    real(real64) :: iqredsol = 0.0_real64, iqredfrs = 0.0_real64, iintc = 0.0_real64, isintc = 0.0_real64
    real(real64) :: iepd = 0.0_real64, ipeva = 0.0_real64, iptra = 0.0_real64, ievap = 0.0_real64
    real(real64) :: iruno = 0.0_real64, irunon = 0.0_real64, iqbot = 0.0_real64, iqbdo = 0.0_real64
    real(real64) :: iqbup = 0.0_real64, iqtdo = 0.0_real64, iqtup = 0.0_real64
    ...
    real(real64) :: cqdra = 0.0_real64, iqdra = 0.0_real64, cgsnow = 0.0_real64, cmelt = 0.0_real64
    real(real64) :: csnrai = 0.0_real64, csubl = 0.0_real64, isnrai = 0.0_real64, igsnow = 0.0_real64, isubl = 0.0_real64
    real(real64) :: imdectot = 0.0_real64, imrottot = 0.0_real64, imsqbot = 0.0_real64
    real(real64) :: imsqdra = 0.0_real64, imsqirrig = 0.0_real64, imsqprec = 0.0_real64
    real(real64), allocatable :: imqsol(:)
    real(real64) :: sbaldev = 0.0_real64, sdstor = 0.0_real64, sampro = 0.0_real64
    real(real64) :: rottot = 0.0_real64, sqdra = 0.0_real64, isqbot = 0.0_real64, dectot = 0.0_real64
    real(real64) :: sqprec = 0.0_real64, sqirrig = 0.0_real64, sqbot = 0.0_real64, samini = 0.0_real64, isqtop = 0.0_real64
...
contains
  subroutine capture_b1_10_legacy_trial_capsule(c)
    ...
    c%outper=outper; c%date=date; c%fldayend=fldayend; c%fldaystart=fldaystart; c%flrunend=flrunend
    ...
    c%ipos_qtop=ipos_qtop; c%ipos_tetop=ipos_tetop; c%ipos_tebot=ipos_tebot
    c%dayfix=dayfix; c%nirri=nirri; c%irrigevent=irrigevent; c%isua=isua; c%cirr=cirr
    c%dt_irr_event=dt_irr_event; c%gird=gird; c%nird=nird; c%qssdisum=qssdisum; c%flirrigate=flirrigate; c%qssdi=qssdi
    c%inqrot=inqrot; c%inq=inq; c%inqssdi=inqssdi; c%inqpotrot=inqpotrot; c%inqredrot=inqredrot; c%iqdo=iqdo; c%iqup=iqup
    ...
    c%imdectot=imdectot; c%imrottot=imrottot; c%imsqbot=imsqbot; c%imsqdra=imsqdra; c%imsqirrig=imsqirrig; c%imsqprec=imsqprec
    c%imqsol=imqsol; c%sbaldev=sbaldev; c%sdstor=sdstor; c%sampro=sampro; c%rottot=rottot; c%sqdra=sqdra
    c%isqbot=isqbot; c%dectot=dectot; c%sqprec=sqprec; c%sqirrig=sqirrig; c%sqbot=sqbot; c%samini=samini; c%isqtop=isqtop
  ...
  subroutine restore_b1_10_legacy_trial_capsule(c)
    ...
    if (.not. allocated(c%qssdi) .or. .not. allocated(c%inqrot) .or. .not. allocated(c%inq) .or. &
        ...
        .not. allocated(c%inqpotrot_day) .or. .not. allocated(c%inqredrot_day) .or. .not. allocated(c%imqsol)) then
    ...
    if (size(c%qssdi) /= macp .or. size(c%inqrot) /= macp .or. size(c%inq) /= macp+1 .or. &
        ...
        size(c%ithetabeg) /= macp .or. size(c%inqpotrot_day) /= macp .or. size(c%inqredrot_day) /= macp .or. &
        size(c%imqsol) /= macp+1) error stop 'B1.10 legacy trial capsule: shape mismatch'
    ...
    outper=c%outper; date=c%date; fldayend=c%fldayend; fldaystart=c%fldaystart; flrunend=c%flrunend
    ...
    ipos_qtop=c%ipos_qtop; ipos_tetop=c%ipos_tetop; ipos_tebot=c%ipos_tebot
    dayfix=c%dayfix; nirri=c%nirri; irrigevent=c%irrigevent; isua=c%isua; cirr=c%cirr
    dt_irr_event=c%dt_irr_event; gird=c%gird; nird=c%nird; qssdisum=c%qssdisum; flirrigate=c%flirrigate; qssdi=c%qssdi
    ...
    imdectot=c%imdectot; imrottot=c%imrottot; imsqbot=c%imsqbot; imsqdra=c%imsqdra; imsqirrig=c%imsqirrig; imsqprec=c%imsqprec
    imqsol=c%imqsol; sbaldev=c%sbaldev; sdstor=c%sdstor; sampro=c%sampro; rottot=c%rottot; sqdra=c%sqdra
    isqbot=c%isqbot; dectot=c%dectot; sqprec=c%sqprec; sqirrig=c%sqirrig; sqbot=c%sqbot; samini=c%samini; isqtop=c%isqtop
