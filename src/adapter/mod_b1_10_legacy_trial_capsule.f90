module mod_b1_10_legacy_trial_capsule
  ...
  use MOD_irrigation, only: schedule, dayfix, nirri, irrigevent, isua, cirr, dt_irr_event, gird, nird, &
  ...
  type, public :: b1_10_legacy_trial_capsule_t
    ...
    ! Solute balance/accounting rollback. Physical concentrations remain in process state.
...
contains
  ...
  subroutine capture_b1_10_legacy_trial_capsule(c)
    ...
    c%schedule=schedule; c%dayfix=dayfix; c%nirri=nirri; c%irrigevent=irrigevent; c%isua=isua; c%cirr=cirr
  ...
  subroutine restore_b1_10_legacy_trial_capsule(c)
    ...
    schedule=c%schedule; dayfix=c%dayfix; nirri=c%nirri; irrigevent=c%irrigevent; isua=c%isua; cirr=c%cirr
