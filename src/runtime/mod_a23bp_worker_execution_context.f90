module mod_a23bp_worker_execution_context
  use, intrinsic :: iso_fortran_env, only: real64
  implicit none
  private

  type, public :: a23bp_headcalc_scratch_t
  ...
  end type a23bp_headcalc_scratch_t

  type, public :: a23bp_solver_history_t
    logical :: flwarn = .true.
    integer :: iwarn = 0
    integer :: nstep = 0
  end type a23bp_solver_history_t

  type, public :: a23bp_solver_diagnostics_t
  ...
  end type a23bp_solver_diagnostics_t

  type, public :: a23bp_worker_context_t
    integer :: worker_id = -1
    integer :: active_nodes = 0
    type(a23bp_headcalc_scratch_t) :: headcalc
    type(a23bp_solver_history_t) :: history
    type(a23bp_solver_diagnostics_t) :: diagnostics
  end type a23bp_worker_context_t

  public :: a23bp_initialize_worker, a23bp_release_worker
  public :: a23bp_reset_attempt_diagnostics, a23bp_record_internal_retry
  public :: a23bp_scratch_payload_bytes

contains

  subroutine a23bp_initialize_worker(worker, active_nodes, worker_id)
    ...
    call a23bp_release_worker(worker)
    ...
    worker%history = a23bp_solver_history_t()
    worker%diagnostics = a23bp_solver_diagnostics_t()
  end subroutine a23bp_initialize_worker

  subroutine a23bp_release_worker(worker)
    type(a23bp_worker_context_t), intent(inout) :: worker
    ...
    worker%history = a23bp_solver_history_t()
    worker%diagnostics = a23bp_solver_diagnostics_t()
  end subroutine a23bp_release_worker

  subroutine a23bp_reset_attempt_diagnostics(worker)
    type(a23bp_worker_context_t), intent(inout) :: worker
    worker%diagnostics = a23bp_solver_diagnostics_t()
  end subroutine a23bp_reset_attempt_diagnostics

  subroutine a23bp_record_internal_retry(worker)
    type(a23bp_worker_context_t), intent(inout) :: worker
    worker%diagnostics%internal_retries = worker%diagnostics%internal_retries + 1
  end subroutine a23bp_record_internal_retry

  integer function a23bp_scratch_payload_bytes(worker) result(bytes)
    type(a23bp_worker_context_t), intent(in) :: worker
    integer :: rb, lb
  ...
  end function a23bp_scratch_payload_bytes

end module mod_a23bp_worker_execution_context
