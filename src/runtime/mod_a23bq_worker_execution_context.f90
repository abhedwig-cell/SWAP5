module mod_a23bq_worker_execution_context
  use, intrinsic :: iso_fortran_env, only: real64
  implicit none
  private

  type, public :: a23bq_headcalc_scratch_t
  ...
  end type a23bq_headcalc_scratch_t

  type, public :: a23bq_solver_history_t
    logical :: flwarn = .true.
    integer :: iwarn = 0
    integer :: nstep = 0
  end type a23bq_solver_history_t

  type, public :: a23bq_solver_diagnostics_t
  ...
  end type a23bq_solver_diagnostics_t

  type, public :: a23bq_numerical_control_t
    integer :: last_numbit = 0
    logical :: request_dt_reduction = .false.
  end type a23bq_numerical_control_t

  type, public :: a23bq_worker_context_t
    integer :: worker_id = -1
    integer :: active_nodes = 0
    type(a23bq_headcalc_scratch_t) :: headcalc
    type(a23bq_solver_history_t) :: history
    type(a23bq_solver_diagnostics_t) :: diagnostics
    type(a23bq_numerical_control_t) :: control
  end type a23bq_worker_context_t

  public :: a23bq_initialize_worker, a23bq_release_worker
  public :: a23bq_reset_attempt_diagnostics, a23bq_record_internal_retry
  public :: a23bq_reset_numerical_control, a23bq_request_dt_reduction
  public :: a23bq_scratch_payload_bytes

contains

  subroutine a23bq_initialize_worker(worker, active_nodes, worker_id)
    type(a23bq_worker_context_t), intent(inout) :: worker
    integer, intent(in) :: active_nodes
    integer, intent(in), optional :: worker_id

    if (active_nodes <= 0) error stop 'A23BP worker: active_nodes must be positive'
    call a23bq_release_worker(worker)
    ...
    worker%history = a23bq_solver_history_t()
    worker%diagnostics = a23bq_solver_diagnostics_t()
    worker%control = a23bq_numerical_control_t()
  end subroutine a23bq_initialize_worker

  subroutine a23bq_release_worker(worker)
    type(a23bq_worker_context_t), intent(inout) :: worker
    ...
    worker%diagnostics = a23bq_solver_diagnostics_t()
    worker%control = a23bq_numerical_control_t()
  end subroutine a23bq_release_worker

  subroutine a23bq_reset_attempt_diagnostics(worker)
    type(a23bq_worker_context_t), intent(inout) :: worker
    worker%diagnostics = a23bq_solver_diagnostics_t()
  end subroutine a23bq_reset_attempt_diagnostics


  subroutine a23bq_reset_numerical_control(worker)
    type(a23bq_worker_context_t), intent(inout) :: worker
    worker%control = a23bq_numerical_control_t()
  end subroutine a23bq_reset_numerical_control

  subroutine a23bq_request_dt_reduction(worker)
    type(a23bq_worker_context_t), intent(inout) :: worker
    worker%control%request_dt_reduction = .true.
  end subroutine a23bq_request_dt_reduction
  subroutine a23bq_record_internal_retry(worker)
    type(a23bq_worker_context_t), intent(inout) :: worker
    worker%diagnostics%internal_retries = worker%diagnostics%internal_retries + 1
  end subroutine a23bq_record_internal_retry

  integer function a23bq_scratch_payload_bytes(worker) result(bytes)
    type(a23bq_worker_context_t), intent(in) :: worker
    integer :: rb, lb
  ...
  end function a23bq_scratch_payload_bytes

end module mod_a23bq_worker_execution_context
