module mod_a23bt_worker_execution_context
  use, intrinsic :: iso_fortran_env, only: real64
  implicit none
  private

  type, public :: a23bt_headcalc_scratch_t
  ...
  end type a23bt_headcalc_scratch_t

  type, public :: a23bt_solver_history_t
    logical :: flwarn = .true.
    integer :: iwarn = 0
    integer :: nstep = 0
  end type a23bt_solver_history_t

  type, public :: a23bt_solver_diagnostics_t
  ...
  end type a23bt_solver_diagnostics_t

  type, public :: a23bt_numerical_control_t
    integer :: last_numbit = 0
    logical :: request_dt_reduction = .false.
    logical :: at_min_dt = .false.
  end type a23bt_numerical_control_t

  type, public :: a23bt_execution_time_t
  ...
  end type a23bt_execution_time_t

  type, public :: a23bt_reporting_progress_t
    ...
    logical :: output_short = .false.
    logical :: balance_output = .false.
    logical :: header = .false.
    logical :: output = .false.
    logical :: reset_intermediate = .true.
    logical :: reset_cumulative = .false.
  end type a23bt_reporting_progress_t

  type, public :: a23bt_worker_context_t
    integer :: worker_id = -1
    integer :: active_nodes = 0
    type(a23bt_headcalc_scratch_t) :: headcalc
    type(a23bt_solver_history_t) :: history
    type(a23bt_solver_diagnostics_t) :: diagnostics
    type(a23bt_numerical_control_t) :: control
    type(a23bt_execution_time_t) :: time
    type(a23bt_reporting_progress_t) :: reporting
  end type a23bt_worker_context_t

  public :: a23bt_initialize_worker, a23bt_release_worker
  public :: a23bt_reset_attempt_diagnostics, a23bt_record_internal_retry
  public :: a23bt_reset_attempt_control, a23bt_reset_all_numerical_control
  public :: a23bt_seed_timestep_control, a23bt_request_dt_reduction
  public :: a23bt_seed_execution_window, a23bt_reset_calendar_events
  public :: a23bt_seed_reporting_progress
  public :: a23bt_scratch_payload_bytes, a23bt_reporting_shim_payload_bytes

contains

  subroutine a23bt_initialize_worker(worker, active_nodes, worker_id)
    type(a23bt_worker_context_t), intent(inout) :: worker
    integer, intent(in) :: active_nodes
    integer, intent(in), optional :: worker_id

    if (active_nodes <= 0) error stop 'A23BT worker: active_nodes must be positive'
    call a23bt_release_worker(worker)
    ...
    worker%history = a23bt_solver_history_t()
    worker%diagnostics = a23bt_solver_diagnostics_t()
    worker%control = a23bt_numerical_control_t()
    worker%time = a23bt_execution_time_t()
    worker%reporting = a23bt_reporting_progress_t()
  end subroutine a23bt_initialize_worker

  subroutine a23bt_release_worker(worker)
    type(a23bt_worker_context_t), intent(inout) :: worker
    ...
    worker%time = a23bt_execution_time_t()
    worker%reporting = a23bt_reporting_progress_t()
  end subroutine a23bt_release_worker

  subroutine a23bt_reset_attempt_diagnostics(worker)
    type(a23bt_worker_context_t), intent(inout) :: worker
    worker%diagnostics = a23bt_solver_diagnostics_t()
  end subroutine a23bt_reset_attempt_diagnostics

  subroutine a23bt_reset_attempt_control(worker)
    type(a23bt_worker_context_t), intent(inout) :: worker
    worker%control%last_numbit = 0
    worker%control%request_dt_reduction = .false.
  end subroutine a23bt_reset_attempt_control

  subroutine a23bt_reset_all_numerical_control(worker)
    type(a23bt_worker_context_t), intent(inout) :: worker
    worker%control = a23bt_numerical_control_t()
  end subroutine a23bt_reset_all_numerical_control

  subroutine a23bt_seed_timestep_control(worker, dt, dtmin)
    type(a23bt_worker_context_t), intent(inout) :: worker
    real(real64), intent(in) :: dt, dtmin
    real(real64), parameter :: dtcrit = 1.0e-8_real64
    call a23bt_reset_attempt_control(worker)
    worker%control%at_min_dt = dt <= (1.0_real64 + dtcrit) * dtmin
  end subroutine a23bt_seed_timestep_control

  subroutine a23bt_request_dt_reduction(worker)
    type(a23bt_worker_context_t), intent(inout) :: worker
    worker%control%request_dt_reduction = .true.
  end subroutine a23bt_request_dt_reduction

  subroutine a23bt_seed_execution_window(worker, t0, t1, legacy_start, legacy_end)
    type(a23bt_worker_context_t), intent(inout) :: worker
    ...
    call a23bt_reset_calendar_events(worker)
  end subroutine a23bt_seed_execution_window

  subroutine a23bt_reset_calendar_events(worker)
    type(a23bt_worker_context_t), intent(inout) :: worker
    worker%time%day_start_event = .false.
    worker%time%day_end_event = .false.
  end subroutine a23bt_reset_calendar_events

  subroutine a23bt_seed_reporting_progress(worker)
    type(a23bt_worker_context_t), intent(inout) :: worker
    worker%reporting = a23bt_reporting_progress_t()
  end subroutine a23bt_seed_reporting_progress

  subroutine a23bt_record_internal_retry(worker)
    type(a23bt_worker_context_t), intent(inout) :: worker
    worker%diagnostics%internal_retries = worker%diagnostics%internal_retries + 1
  end subroutine a23bt_record_internal_retry

  integer function a23bt_reporting_shim_payload_bytes() result(bytes)
    integer :: rb, ib, lb
    rb = storage_size(0.0_real64)/8
    ib = storage_size(0)/8
    lb = storage_size(.false.)/8
    bytes = 4*ib + 2*rb + 6*lb
  end function a23bt_reporting_shim_payload_bytes

  integer function a23bt_scratch_payload_bytes(worker) result(bytes)
    type(a23bt_worker_context_t), intent(in) :: worker
    integer :: rb, lb
    rb = storage_size(0.0_real64)/8
    lb = storage_size(.false.)/8
    bytes = 0
  ...
  end function a23bt_scratch_payload_bytes

end module mod_a23bt_worker_execution_context
