module mod_fmr_groundwater_swap_participant
  ...
  type, public :: fmr_groundwater_swap_participant_t
    ...
    integer(int64) :: forcing_materializer_generation = 0_int64
  contains
...
contains
  ...
  subroutine fmr_swap_trial_from_origin(self, backend, column, template, parameters, committed, materializer, &
    ...
    integer(int64) :: materializer_generation
    ...
    class is (fmr_groundwater_head_forcing_materializer_t)
      materializer_generation = typed_materializer%current_generation()
      if (materializer_generation <= 0_int64) then
        status = GW_SWAP_PARTICIPANT_FORCING_FAILED
        return
      end if
      if (self%forcing_materializer_generation /= materializer_generation) then
        ...
        self%forcing_materializer_generation = materializer_generation
      end if
