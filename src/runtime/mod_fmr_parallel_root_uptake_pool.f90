module mod_fmr_parallel_root_uptake_pool
  ...
  use mod_fmr_serialized_multiswap_runtime, only: fmr_serialized_column_result_t, &
       fmr_serialized_batch_diagnostics_t, fmr_execute_serialized_physical_column, FMR_SERIAL_DISPATCH_OK
  ...
  integer, parameter, public :: FMR_PARALLEL_ROOT_POOL_OK = 0
  integer, parameter, public :: FMR_PARALLEL_ROOT_POOL_INVALID_WORKER_COUNT = 1
  integer, parameter, public :: FMR_PARALLEL_ROOT_POOL_PROFILE_NOT_ADMITTED = 2

  public :: fmr_run_parallel_root_uptake_multiswap

contains

  subroutine fmr_run_parallel_root_uptake_multiswap(columns, templates, parameter_registry, forcing_registry, &
    ...
    pool_status = FMR_PARALLEL_ROOT_POOL_INVALID_WORKER_COUNT
    ...
    ! This is an explicitly separate root-active physical capability.  It has
    ! no worker_count=1 delegation: the qualified serialized runtime remains
    ! the reference oracle, and this production entry point is admitted only
    ! for the restricted 2- and 4-worker parallel profile.
    if ((worker_count /= 2 .and. worker_count /= 4) .or. batch_size <= 0 .or. t1 <= t0 .or. &
        worker_count > omp_get_thread_limit() .or. &
        .not. parallel_root_uptake_profile_admitted(columns, templates, parameter_registry, forcing_registry, &
                                                     state_registry, top_boundary)) then
      pool_status = FMR_PARALLEL_ROOT_POOL_PROFILE_NOT_ADMITTED
      call initialize_rejected_outputs(columns, t0, t1, 'ROOT_UPTAKE_PROFILE_NOT_ADMITTED', &
           results, diagnostics, aggregate, local_runtime)
      call canonicalize_publication(columns, results, diagnostics)
      if (present(runtime_diagnostics)) runtime_diagnostics = local_runtime
      return
    ...
    pool_status = FMR_PARALLEL_ROOT_POOL_OK
    if (present(runtime_diagnostics)) runtime_diagnostics = local_runtime
  end subroutine fmr_run_parallel_root_uptake_multiswap

  logical function parallel_root_uptake_profile_admitted(columns, templates, parameter_registry, forcing_registry, &
    ...
    do i = 1, size(columns)
      ...
      if (parameter_registry(parameter_index)%bottom_mode /= 7 .or. &
          ...
          .not. parameter_registry(parameter_index)%root_extraction_active .or. &
    ...
    admitted = .true.
  end function parallel_root_uptake_profile_admitted
...
end module mod_fmr_parallel_root_uptake_pool
