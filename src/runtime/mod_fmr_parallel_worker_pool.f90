...
contains
  ...
  subroutine fmr_run_parallel_physical_multiswap(columns, templates, parameter_registry, forcing_registry, &
    ...
    if (schedule_status /= FMR_PARALLEL_SCHEDULE_OK) then
      call initialize_rejected_outputs(columns, t0, t1, worker_count, 'INVALID_WORKER_COUNT', &
    ...
    if (worker_count /= 1) then
      ...
      call initialize_rejected_outputs(columns, t0, t1, worker_count, 'MULTIWORKER_NOT_ADMITTED', &
           results, diagnostics, aggregate, local_runtime)
      if (present(runtime_diagnostics)) runtime_diagnostics = local_runtime
      return
    end if
  ...
  subroutine initialize_rejected_outputs(columns, t0, t1, requested_workers, failure_classification, &
                                         results, diagnostics, aggregate, runtime)
    type(fmr_logical_column_t), intent(in) :: columns(:)
    real(real64), intent(in) :: t0, t1
    integer, intent(in) :: requested_workers
    ...
    if (requested_workers < 0) aggregate%workers = 0
