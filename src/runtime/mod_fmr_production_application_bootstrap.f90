...
contains
  ...
  subroutine production_application_initialize(self, config, status)
    ...
    allocate(self%columns(n), self%templates(n), self%participant_handles(n))
    allocate(self%parameters(n), self%base_forcing(n), self%committed(n), self%materializers(n), self%ledgers(n))
    allocate(self%registry, self%backend, self%top_boundary)
    ...
    do i = 1, n
      ...
      call self%materializers(i)%initialize(self%base_forcing(i))
  ...
  logical function production_application_ready(self) result(ready)
    ...
    if (.not. ready) return
    ready = allocated(self%columns) .and. allocated(self%templates) .and. allocated(self%participant_handles)
    if (.not. ready) return
    ready = associated(self%parameters) .and. associated(self%base_forcing) .and. associated(self%committed) .and. &
         associated(self%materializers) .and. associated(self%ledgers) .and. associated(self%registry) .and. &
    ...
    ready = size(self%columns) > 0 .and. size(self%templates) == size(self%columns) .and. &
         size(self%participant_handles) == size(self%columns) .and. size(self%parameters) == size(self%columns) .and. &
         size(self%base_forcing) == size(self%columns) .and. size(self%committed) == size(self%columns) .and. &
         size(self%materializers) == size(self%columns) .and. size(self%ledgers) == size(self%columns)
    if (.not. ready) return
  ...
  logical function tile_config_valid(tile, ntiles, slot) result(valid)
    ...
    valid = .false.
    if (tile%tile_id <= 0_int64 .or. tile%ledger_id <= 0_int64) return
    ...
    if (.not. tile%groundwater_datum%valid()) return
