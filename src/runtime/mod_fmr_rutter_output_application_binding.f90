module mod_fmr_rutter_output_application_binding
  ...
  integer, parameter, public :: FMR_RUTTER_BIND_INVALID_SURFACE_FLUX = 2
  integer, parameter, public :: FMR_RUTTER_BIND_INVALID_PTRA = 3
  integer, parameter, public :: FMR_RUTTER_BIND_INVALID_ROOT_GEOMETRY = 4
  integer, parameter, public :: FMR_RUTTER_BIND_ASSEMBLED_ROOT_REJECTED = 5

  type, public :: fmr_rutter_output_binding_diagnostics_t
    integer :: status = FMR_RUTTER_BIND_OK
    integer :: upstream_status = RUTTER_OK
    ...
    logical :: surface_fluxes_bound = .false.
  ...
  end type fmr_rutter_output_binding_diagnostics_t
...
contains
  ...
  subroutine fmr_bind_rutter_surface_fluxes_to_dynamic_top(base_request, rutter_result, upstream, &
    ...
    net_rain = rutter_result%net_rain_cm_per_day
    net_irrigation = rutter_result%net_surface_irrigation_cm_per_day
    if (.not. ieee_is_finite(net_rain) .or. .not. ieee_is_finite(net_irrigation) .or. &
        net_rain < 0.0_real64 .or. net_irrigation < 0.0_real64) then
      diagnostics%status = FMR_RUTTER_BIND_INVALID_SURFACE_FLUX
      return
    end if
    ...
    diagnostics%surface_fluxes_bound = .true.
    diagnostics%result_produced = .true.
  end subroutine fmr_bind_rutter_surface_fluxes_to_dynamic_top

  subroutine fmr_bind_rutter_ptra_to_root_input(base_input, active_nodes, rutter_result, upstream, &
    ...
    ptra = rutter_result%potential_transpiration_cm_per_day
...
end module mod_fmr_rutter_output_application_binding
