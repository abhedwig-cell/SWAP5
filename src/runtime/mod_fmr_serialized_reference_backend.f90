module mod_fmr_serialized_reference_backend
  ...
  type, extends(kernel_parameters_t), public :: fmr_b110_physical_parameters_t
    ...
    ! F-AHL42 explicit opt-in. Default false preserves the authoritative analytical route.
...
contains
  ...
  logical function fmr_serialized_execution_admitted(self, parameters, numerical_config)
    ...
    type is (fmr_b110_physical_parameters_t)
      ...
      if (parameters%adaptive_hydraulics_active) then
        ! Bounded admission candidate: only Reference execution is eligible.
  ...
  subroutine fmr_serialized_configure_parameters(self, parameters)
    ...
    type is (fmr_b110_physical_parameters_t)
      ...
      ! Retain the adaptive provider across identical reconfiguration so its
      ! exact-key immutable representation can be reused. Disablement releases it.
  ...
  subroutine fmr_serialized_advance(self, state, t0, t1, outcome)
    ...
    ! Adaptive hydraulics is admitted only for prescribed-head mode 5.
    ! Prescribed-qbot and other modes stay fully analytical and avoid adaptive
    ! cache/build side effects.
