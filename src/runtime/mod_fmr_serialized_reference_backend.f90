module mod_fmr_serialized_reference_backend
  ...
  type, extends(kernel_parameters_t), public :: fmr_b110_physical_parameters_t
    ...
    ! F-AHL canonical opt-in. Default false preserves the authoritative analytical route.
...
contains
  ...
  logical function fmr_serialized_execution_admitted(self, parameters, numerical_config)
    ...
    type is (fmr_b110_physical_parameters_t)
      ...
      if (parameters%adaptive_hydraulics_active) then
        ! Adaptive hydraulics is admitted only on the Reference route. Actual
        ! use remains lower-boundary dependent: prescribed-head mode 5 may use
        ! the adaptive provider; qbot and other modes remain analytical.
  ...
  subroutine fmr_serialized_configure_parameters(self, parameters)
    ...
    type is (fmr_b110_physical_parameters_t)
      ...
      ! an exact same-key immutable representation can be reused. Disablement
      ! releases the provider. Analytical fallback is rebound every advance.
  ...
  subroutine fmr_serialized_advance(self, state, t0, t1, outcome)
    ...
    ! Adaptive hydraulics is qualified only for prescribed-head mode 5.
    ! Prescribed-qbot and other lower-boundary modes remain fully analytical
    ! and do not construct or query the adaptive representation.
