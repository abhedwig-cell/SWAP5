module mod_fmr_serialized_reference_backend
  ...
  type, extends(kernel_parameters_t), public :: fmr_b110_physical_parameters_t
    ...
    ! F-AHL opt-in. Default false preserves the authoritative analytical route.
...
contains
  ...
  logical function fmr_serialized_execution_admitted(self, parameters, numerical_config)
    ...
    type is (fmr_b110_physical_parameters_t)
      ...
      if (parameters%adaptive_hydraulics_active) then
        ...
        ! Mode 5 may use AHL; prescribed-qbot and other lower-boundary modes
        ! remain authoritative analytical fallback.
  ...
  subroutine fmr_serialized_configure_parameters(self, parameters)
    ...
    type is (fmr_b110_physical_parameters_t)
      ...
      ! Retain only the adaptive provider object across repeated adaptive
      ! configurations so immutable same-key representations can be reused.
      ! Its analytical fallback remains rebound for the current step duration.
  ...
  subroutine fmr_serialized_advance(self, state, t0, t1, outcome)
    ...
    ! Qualified ownership boundary: only prescribed-head mode 5 uses the
    ! adaptive provider. Other lower-boundary modes stay fully analytical.
