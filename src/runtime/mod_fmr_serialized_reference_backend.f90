module mod_fmr_serialized_reference_backend
  ...
  type, extends(kernel_parameters_t), public :: fmr_b110_physical_parameters_t
    ...
    ! F-AHL43: explicit opt-in. Default false preserves authoritative analytical hydraulics.
...
contains
  ...
  logical function fmr_serialized_execution_admitted(self, parameters, numerical_config)
    ...
    type is (fmr_b110_physical_parameters_t)
      ...
      if (parameters%adaptive_hydraulics_active) then
        ! F-AHL43 production boundary: opt-in, Reference-only. Prescribed-head
        ! mode 5 may use adaptive hydraulics; qbot and other lower boundaries
        ! fall back to authoritative analytical hydraulics at execution.
  ...
  subroutine fmr_serialized_configure_parameters(self, parameters)
    ...
    type is (fmr_b110_physical_parameters_t)
      ...
      ! Keep the adaptive provider across repeated opt-in configurations so an
      ! immutable exact-key representation can be reused. Disablement releases it.
  ...
  subroutine fmr_serialized_advance(self, state, t0, t1, outcome)
    ...
    ! F-AHL43: acquire adaptive hydraulics only inside the qualified lower-boundary
    ! envelope. Prescribed-qbot and every other mode remain fully analytical and
    ! incur no adaptive cache/build side effects.
