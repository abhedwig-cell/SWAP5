module mod_fmr_wofost81_crop_event_lifecycle
  ...
  use mod_fmr_wofost81_crop_transaction, only: fmr_wofost81_crop_transaction_state_t
  implicit none
  private

  integer, parameter, public :: FMR_WOF81_LIFECYCLE_OK = 0
  integer, parameter, public :: FMR_WOF81_LIFECYCLE_INVALID_WINDOW = 1
  integer, parameter, public :: FMR_WOF81_LIFECYCLE_INVALID_COMMITTED_STATE = 2
  integer, parameter, public :: FMR_WOF81_LIFECYCLE_NO_MATCHING_COMMITTED_RECEIPT = 3
  integer, parameter, public :: FMR_WOF81_LIFECYCLE_RETIREMENT_REJECTED = 4
  integer, parameter, public :: FMR_WOF81_LIFECYCLE_ALREADY_RETIRED = 5

  public :: reconcile_committed_wofost81_crop_event_receipt

contains

  ! WOFOST81 analogue of the admitted F-WOF39 lifecycle seam. Physics is
  ! already committed atomically by the WOFOST81 transaction. This routine can
  ! only retire the matching accepted-window delivery token; it cannot publish
  ! crop state or alter the committed kernel carrier.
  subroutine reconcile_committed_wofost81_crop_event_receipt(window, committed_crop, retired, status)
    ...
    retired = .false.
    status = FMR_WOF81_LIFECYCLE_INVALID_WINDOW
    if (.not. window%ready()) return

    if (window%delivery_committed()) then
      status = FMR_WOF81_LIFECYCLE_ALREADY_RETIRED
      return
    end if
    if (.not. window%complete()) return

    status = FMR_WOF81_LIFECYCLE_INVALID_COMMITTED_STATE
    ...
    if (lineage_status /= FMR_WOFOST_LINEAGE_OK .or. .not. event_available .or. .not. token%ready()) then
      status = FMR_WOF81_LIFECYCLE_RETIREMENT_REJECTED
      return
    end if
    ...
    status = FMR_WOF81_LIFECYCLE_NO_MATCHING_COMMITTED_RECEIPT
    ...
    retired = .true.
    status = FMR_WOF81_LIFECYCLE_OK
  end subroutine reconcile_committed_wofost81_crop_event_receipt

end module mod_fmr_wofost81_crop_event_lifecycle
