module mod_b110_adaptive_hydraulic_provider
  ...
  contains
    ...
    procedure :: evaluate_demand => b110_adaptive_hydraulic_evaluate_demand
...
contains
  ...
  subroutine b110_adaptive_hydraulic_evaluate_demand(self,pressure_head,demand_mask,water_content,conductivity,capacity,dconductivity_dhead)
    class(b110_adaptive_hydraulic_provider_t),intent(in)::self
    real(real64),intent(in)::pressure_head(:)
    ...
    real(real64) :: x0,x1,z0,z1,m0,m1,k0,k1
    integer :: i,idx
    logical :: inside,need_fallback,need_capacity
    ...
    need_capacity=demand_mask==CONSTITUTIVE_DEMAND_CAPACITY
    ...
    if(need_fallback)call self%analytical%evaluate_demand(pressure_head,demand_mask,wa,ka,ca,da)
    ...
    do i=1,size(pressure_head)
      ...
      if(self%registry_handle_active)then
        if(pressure_head(i)<=LOOKUP_H_MAX .and. xv>=self%registry_xmin .and. xv<=self%registry_xmax)then
          ...
          h00=2*t**3-3*t**2+1;h10=t**3-2*t**2+t;h01=-2*t**3+3*t**2;h11=t**3-t**2
          zz=h00*z0+h10*dx*m0+h01*z1+h11*dx*m1
          if(zz>=0.0_real64)then;se=1.0_real64/(1.0_real64+exp(-zz));else;se=exp(zz)/(1.0_real64+exp(zz));end if
          span=self%cofgen(2,i)-self%cofgen(1,i)
          ...
          if(need_capacity)then;capacity(i)=ca(i);else;water_content(i)=wa(i);end if
        end if
      else
        if(pressure_head(i)<=LOOKUP_H_MAX .and. xv>=self%table%x(1) .and. xv<=self%table%x(self%table%n))then
          call locate(self%table%x,xv,idx,f)
          dx=self%table%x(idx+1)-self%table%x(idx);t=f
          h00=2*t**3-3*t**2+1;h10=t**3-2*t**2+t;h01=-2*t**3+3*t**2;h11=t**3-t**2
          zz=h00*self%table%z(idx)+h10*dx*self%table%dzdx(idx)+h01*self%table%z(idx+1)+h11*dx*self%table%dzdx(idx+1)
          if(zz>=0.0_real64)then;se=1.0_real64/(1.0_real64+exp(-zz));else;se=exp(zz)/(1.0_real64+exp(zz));end if
          span=self%cofgen(2,i)-self%cofgen(1,i)
          if(need_capacity)then
            dh00=6*t*t-6*t;h10d=3*t*t-4*t+1;dh01=-6*t*t+6*t;dh11=3*t*t-2*t
            dz_x=(dh00*self%table%z(idx)+h10d*dx*self%table%dzdx(idx)+dh01*self%table%z(idx+1)+dh11*dx*self%table%dzdx(idx+1))/dx
            capacity(i)=(span*se*(1.0_real64-se)*dz_x)/(pressure_head(i)*LN10)
          else
            water_content(i)=self%cofgen(1,i)+span*se
          end if
  ...
  end subroutine b110_adaptive_hydraulic_evaluate_demand
