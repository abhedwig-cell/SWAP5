module mod_b110_default_mvg_directional_provider
  ...
  public :: evaluate_b110_default_mvg_conductivity_direction

contains

  subroutine evaluate_b110_default_mvg_conductivity_direction(provider, pressure_head, pressure_head_direction, &
                                                               conductivity_direction, available, route)
    type(b110_default_mvg_provider_t), intent(in) :: provider
    real(real64), intent(in) :: pressure_head(:), pressure_head_direction(:)
    real(real64), intent(out) :: conductivity_direction(:)
    ...
    if (size(pressure_head) /= n .or. size(pressure_head_direction) /= n .or. &
        size(conductivity_direction) /= n) then
    ...
    do i = 1, n
       call b110_smooth_dkdh(provider%parameters%cofgen(:,i), pressure_head(i), dkdh, node_ok)
       if (.not. node_ok) then
          route = 'b110-mvg-nonsmooth-conductivity-branch'
          conductivity_direction = 0.0_real64
          return
       end if
       conductivity_direction(i) = dkdh * pressure_head_direction(i)
    end do
    if (any(.not. ieee_is_finite(conductivity_direction))) then
       route = 'b110-mvg-conductivity-direction-nonfinite'
       conductivity_direction = 0.0_real64
       return
    end if
  ...
  end subroutine evaluate_b110_default_mvg_conductivity_direction

  subroutine b110_smooth_dkdh(c, head, dkdh, ok)
    real(real64), intent(in) :: c(:), head
    real(real64), intent(out) :: dkdh
    logical, intent(out) :: ok

    real(real64) :: theta, dtheta, relsat, invm, one_minus_term, term1
    ...
    ! Exact derivative of the B1.10 water-retention branch used by hconduc.
    if (head > 0.0_real64) then
       ...
       if (head > B110_H_CRIT) then
          ...
          dtheta = c(6)*c(7)*alpha*c(25)*u**(c(6)-1.0_real64) / &
  ...
  end subroutine b110_smooth_dkdh
