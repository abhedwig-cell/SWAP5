...
contains
  ...
  subroutine rossfast_d3r_table_kernel_solve(self, request, result)
    ...
    integer :: fast_solves, fallback_solves, total_solves
    logical :: ok
    ...
    ! F-ROSS20 production rule promoted from F-ROSS19: K2 is authoritative only
    ! when its own model certificate is acceptable. Otherwise recompute
    ! independently from the original base state with K4. No K2 state is reused.
    if (indicator > 1.0_real64) then
      call solve_certificate(self, request, FALLBACK_INTERNAL_SUBSTEPS, candidate, indicator, fallback_solves, ok)
      if (.not. ok) return
  ...
  subroutine solve_certificate(self, request, internal_substeps, candidate, indicator, linear_solves, ok)
    ...
    if (internal_substeps /= FAST_INTERNAL_SUBSTEPS .and. &
        internal_substeps /= FALLBACK_INTERNAL_SUBSTEPS) return
  ...
  logical function request_contract_is_admitted(request)
    ...
    ! F-ROSS20 production ABI advertises the fast K2 attempt. K4 is an internal
    ! fallback recomputed from the same base state only after K2 rejects.
