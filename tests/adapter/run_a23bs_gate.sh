...
: "${A23BS_CASE:?Set A23BS_CASE to Hupsel qualification case}"
: "${A23BS_B16_CANONICAL:?Set A23BS_B16_CANONICAL to exact B1.6 source tree}"
: "${A23BS_TTUTIL_ROOT:?Set A23BS_TTUTIL_ROOT to TTUTIL source root}"
BUILD="${A23BS_BUILD:-$ROOT/.a23bs-build}"
rm -rf "$BUILD"; mkdir -p "$BUILD"

B16_WORKER="$BUILD/B1.6-worker"
python "$ROOT/tools/a23bs_prepare_execution_time_source.py" \
  --source "$A23BS_B16_CANONICAL" --output "$B16_WORKER" \
  --ttutil-prefs "$A23BS_TTUTIL_ROOT/ttutilprefs.f90" | tee "$BUILD/postimage.json"
...
print('A23BS_SOURCE_CONTROL_AUDIT PASS')
PY_AUDIT

TT="$A23BS_TTUTIL_ROOT"; TTB="$BUILD/ttbuild"; mkdir -p "$TTB"
...
for opt in 0 2; do
  ...
  gfortran "${COMMON[@]}" -c "$ROOT/src/runtime/mod_a23bs_worker_execution_context.f90" -o "$O/mod_a23bs_worker_execution_context.o"
  ...
  gfortran "${COMMON[@]}" -c "$ROOT/src/adapter/mod_a23bs_hupsel_worker_component.f90" -o "$O/mod_a23bs_hupsel_worker_component.o"
  gfortran "${COMMON[@]}" -c "$ROOT/tests/adapter/test_a23bs_hupsel_worker_component.f90" -o "$O/test_a23bs.o"
  objs=$(find "$O" -maxdepth 1 -name '*.o' ! -name 'test_a23bs.o' ! -name 'mod_transaction_reference.o' ! -name 'mod_a23bs_hupsel_worker_component.o' ! -name 'mod_a23bs_worker_execution_context.o' -printf '%p ')
  gfortran -O$opt -o "$O/test_a23bs" "$O/mod_a23bs_worker_execution_context.o" "$O/mod_transaction_reference.o" "$O/mod_a23bs_hupsel_worker_component.o" "$O/test_a23bs.o" $objs "$TTLIB"
  cp -a "$A23BS_CASE" "$O/case"
  python - "$O/case/swap.swp" <<'PY'
from pathlib import Path
import sys,re
p=Path(sys.argv[1]); s=p.read_text(encoding='latin1')
s,n1=re.subn(r'(?m)^\s*SWEND\s*=\s*0\b.*$', '  SWEND = 1                  ! A23BS VQ: binary endpoint at closure', s)
s,n2=re.subn(r'(?m)^\s*SWENDTYPE\s*=\s*1\b.*$', '  SWENDTYPE = 2              ! A23BS VQ: unformatted continuation state', s)
s,n3=re.subn(r'(?m)^\s*SWCSV\s*=\s*1\b.*$', '  SWCSV = 0                  ! A23BS VQ: CSV observer disabled', s)
if n1!=1 or n2!=1 or n3!=1: raise SystemExit(f'swap.swp VQ anchors changed: {n1},{n2},{n3}')
p.write_text(s,encoding='latin1')
PY
  (cd "$O/case" && ../test_a23bs) | tee "$O/gate.log"
  sha256sum "$O/case/a23bs_worker.end" > "$O/end.sha256"
  grep -q 'A23BS_WORKER_COMPONENT_GATE PASS' "$O/gate.log"
...
for opt in 0 2; do
  ...
  gfortran -O$opt -std=f2008 -Wall -Wextra -Werror -fcheck=all -fbacktrace -fopenmp -ffree-line-length-none \
    -J "$O" -I "$O" "$ROOT/src/runtime/mod_a23bs_worker_execution_context.f90" \
    "$ROOT/tests/runtime/test_a23bs_worker_context.f90" -o "$O/test_worker"
  OMP_NUM_THREADS=8 "$O/test_worker" > "$O/gate.log"
done
cmp "$BUILD/worker_o0/gate.log" "$BUILD/worker_o2/gate.log"

echo A23BS_GATE_PASS
