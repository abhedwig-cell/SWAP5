program test_a23bp_hupsel_worker_component
  use, intrinsic :: iso_fortran_env, only: real64
  use mod_transaction_reference
  use mod_a23bp_hupsel_worker_component
  use mod_a23bp_worker_execution_context, only: a23bp_initialize_worker, a23bp_release_worker
  implicit none
  ...
  call initialize_hupsel_worker_component(model, seed, 'swap.swp', 'a23bp_worker', &
  ...
  call a23bp_initialize_worker(model_b%worker, size(model_b%dz), 1)
  ...
  class default
    error stop 'A23BP test: committed type mismatch'
  end select
  print '(A)', 'A23BP_WORKER_COMPONENT_GATE PASS'
  ...
  call a23bp_release_worker(model_b%worker)
...
contains
  ...
  subroutine print_layout(state)
    ...
    integer :: rb, ib, lb
    integer :: physical_bytes, forcing_bytes, process_bytes, numerical_bytes, replay_bytes, accounting_bytes
    ...
    class default
      error stop 'A23BP test: state layout type mismatch'
    end select
    ...
    type is (hupsel_column_state_t)
      ...
      type is (hupsel_column_state_t)
        column_states_equal = exact_real_array(x%physical%h,y%physical%h) .and. &
          ...
          abs(x%numerical%dt-y%numerical%dt) <= 0.0_real64 .and. &
...
end program test_a23bp_hupsel_worker_component
