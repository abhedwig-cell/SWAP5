program test_a23br_hupsel_worker_component
  use, intrinsic :: iso_fortran_env, only: real64
  use mod_transaction_reference
  use mod_a23br_hupsel_worker_component
  use mod_a23br_worker_execution_context, only: a23br_initialize_worker, a23br_release_worker
  use variables, only: legacy_global_dt => dt
  implicit none
  ...
  call initialize_hupsel_worker_component(model, seed, 'swap.swp', 'a23br_worker', &
  ...
  call a23br_initialize_worker(model_b%worker, size(model_b%dz), 1)
  ...
  print '(A)', 'A23BR_TIMESTEP_OWNERSHIP_POISON PASS'
  ...
  class default
    error stop 'A23BR test: committed type mismatch'
  end select
  print '(A)', 'A23BR_WORKER_COMPONENT_GATE PASS'
  ...
  call a23br_release_worker(model_b%worker)
...
contains
  ...
  subroutine print_layout(state)
    ...
    class default
      error stop 'A23BR test: state layout type mismatch'
    end select
...
end program test_a23br_hupsel_worker_component
