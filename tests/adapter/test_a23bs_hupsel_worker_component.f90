program test_a23bs_hupsel_worker_component
  use, intrinsic :: iso_fortran_env, only: real64
  use mod_transaction_reference
  use mod_a23bs_hupsel_worker_component
  use mod_a23bs_worker_execution_context, only: a23bs_initialize_worker, a23bs_release_worker
  ...
  call initialize_hupsel_worker_component(model, seed, 'swap.swp', 'a23bs_worker', &
  ...
  call a23bs_initialize_worker(model_b%worker, size(model_b%dz), 1)
  ...
  print '(A)', 'A23BS_TIME_PROJECTION_POISON PASS'
  ...
  class default
    error stop 'A23BS test: committed type mismatch'
  end select
  print '(A)', 'A23BS_WORKER_COMPONENT_GATE PASS'
  ...
  call a23bs_release_worker(model_b%worker)
...
contains
  ...
  subroutine print_layout(state)
    ...
    class default
      error stop 'A23BS test: state layout type mismatch'
    end select
...
end program test_a23bs_hupsel_worker_component
