program test_a23bu_hupsel_worker_component
  use, intrinsic :: iso_fortran_env, only: real64
  use mod_transaction_reference
  use mod_a23bu_hupsel_worker_component
  use mod_a23bu_worker_execution_context, only: a23bu_initialize_worker, a23bu_release_worker
  ...
  use MOD_irrigation, only: legacy_flirrigate => flirrigate, legacy_dayfix => dayfix, legacy_nirri => nirri, &
       legacy_irrigevent => irrigevent, legacy_gird => gird, legacy_dt_irr_event => dt_irr_event, legacy_isua => isua, &
       legacy_cirr => cirr, legacy_nird => nird, legacy_qssdi => qssdi, legacy_qssdisum => qssdisum
  use MOD_cropdevelopment, only: legacy_fl_cropcalendar => fl_cropcalendar
  implicit none

  type(hupsel_worker_component_t) :: model, model_b
  class(transaction_state_t), allocatable :: seed, committed, endpoint
  class(transaction_state_t), allocatable :: a_ref, b_seed, b_ref, a_test, b_test, cursor_bad
  ...
  call initialize_hupsel_worker_component(model, seed, 'swap.swp', 'a23bu_worker', &
                                   37256.0_real64, 37258.0_real64, 37259.0_real64)
  inquire(file='a23bu_worker.bal', size=bal_size_before)
  inquire(file='a23bu_worker.blc', size=blc_size_before)
  ...
  model_b%irrigation_enabled = model%irrigation_enabled
  model_b%crop_calendar_enabled = model%crop_calendar_enabled
  allocate(model_b%dz(size(model%dz)))
  model_b%dz = model%dz
  model_b%initialized = .true.
  call a23bu_initialize_worker(model_b%worker, size(model_b%dz), 1)
  ...
  inquire(file='a23bu_worker.bal', size=bal_size_after)
  inquire(file='a23bu_worker.blc', size=blc_size_after)
  ...
  print '(A)', 'A23BU_TRIAL_OUTPUT_SUPPRESSION PASS'

  ! Negative control: the fixed-irrigation event cursor is true physical continuation state.
  call seed%clone(cursor_bad)
  select type (bad => cursor_bad)
  type is (hupsel_column_state_t)
    call require(bad%process%irrigation%nirri == 1, 'unexpected seed irrigation cursor')
    bad%process%irrigation%nirri = 2
  class default
    error stop 'A23BU test: cursor_bad type mismatch'
  end select
  call model%advance(cursor_bad, 0.0_real64, 2.0_real64, a_ref_out)
  call require(a_ref_out%solver_ok, 'cursor negative-control advance failed')
  print '(A,ES24.16)', 'bad_cursor_storage=', model%storage(cursor_bad)
  call require(abs(model%storage(cursor_bad)-accepted_storage) > 1.0e-3_real64, &
       'nirri corruption did not change physical endpoint')
  print '(A)', 'A23BU_IRRIGATION_CURSOR_NEGATIVE_CONTROL PASS'

  ! Build independent serial references for two logical columns at different progress points.
  ...
  ! Interleave in the opposite progression order through the same legacy singleton backend.
  call seed%clone(a_test)
  call b_seed%clone(b_test)
  call poison_worker(model_b)
  call poison_legacy_irrigation_workspace()
  ...
  print '(A)', 'A23BU_PROCESS_EVENT_WORKSPACE_POISON PASS'
  ...
  class default
    error stop 'A23BU test: committed type mismatch'
  end select
  print '(A)', 'A23BU_WORKER_COMPONENT_GATE PASS'
  ...
  call a23bu_release_worker(model_b%worker)
...
contains
  ...
  subroutine print_layout(state)
    ...
    class default
      error stop 'A23BU test: state layout type mismatch'
    end select
  ...
  subroutine poison_legacy_irrigation_workspace()
    legacy_flirrigate = .false.
    legacy_fl_cropcalendar = .not. model%crop_calendar_enabled
    legacy_dayfix = -999
    legacy_nirri = 2
    legacy_irrigevent = 2
    legacy_gird = 9999.0_real64
    legacy_dt_irr_event = 0.000123_real64
    legacy_isua = 2
    legacy_cirr = 777.0_real64
    legacy_nird = 888.0_real64
    legacy_qssdi = 666.0_real64
    legacy_qssdisum = 555.0_real64
  end subroutine poison_legacy_irrigation_workspace
    ...
    type is (hupsel_column_state_t)
      ...
      type is (hupsel_column_state_t)
        column_states_equal = exact_real_array(x%physical%h,y%physical%h) .and. &
          ...
          x%process%irrigation%dayfix == y%process%irrigation%dayfix .and. &
          x%process%irrigation%nirri == y%process%irrigation%nirri .and. &
...
end program test_a23bu_hupsel_worker_component
