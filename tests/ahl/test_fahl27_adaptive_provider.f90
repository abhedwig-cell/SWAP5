program test_fahl27_adaptive_provider
  ...
  use mod_b110_adaptive_mvg_provider, only: b110_adaptive_mvg_provider_t, bind_b110_adaptive_mvg_provider, b110_adaptive_mvg_cache_stats
  implicit none
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'FAHL27_METRIC',trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'FAHL27_DIAG',trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'FAHL27',trim(name),'PASS'
    write(*,'(A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') 'FAHL27_CACHE_STATS','BUILDS=',builds,'HITS=',hits,'MISSES=',misses,'ENTRIES=',entries
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'FAHL27_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_fahl27_adaptive_provider
