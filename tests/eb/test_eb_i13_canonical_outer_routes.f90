...
contains
  ...
  subroutine outer_prepare_interval(self, forcing, interval, config)
    ...
    if (.not. same_type_as(self, self) .or. .not. same_type_as(forcing, forcing)) error stop 'unreachable EB-I13 outer prepare types'
  ...
  subroutine outer_storage_status(self, state, complete, missing_mask)
    ...
    if (.not. same_type_as(self, self) .or. .not. same_type_as(state, state)) error stop 'unreachable EB-I13 outer storage types'
    complete = .true.
...
contains
  ...
  subroutine verify_multi_substep_candidate()
    ...
    call require(result%status == CANONICAL_STATUS_COMPLETED .and. result%completed, 'multi-substep outer interval completed')
    ...
    call require(ok .and. candidate%ready() .and. candidate%sample_count() == 4, 'multi-substep candidate materializes')
    do i = 1, 4
  ...
  subroutine verify_partial_outer_failure_is_not_materializable()
    ...
    ! Even if a caller incorrectly attempted materialization after this failed
    ! outer interval, the carrier cannot claim the requested [0,1] interval.
    ! The production backend additionally gates this call on result%completed.
    call model%carrier%materialize_candidate(0.0_real64, 1.0_real64, candidate, ok)
    call require(.not. ok .and. .not. candidate%ready(), 'partial outer carrier cannot materialize as full candidate')
