...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl27-s2-matrix-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -O2 -J "$BUILD" -I "$BUILD" -c tests/fahl/test_fahl27_stage2_matrix.f90 -o "$BUILD/test.o"
gfortran -O2 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"

RESULT="${1:-/tmp/fahl27_stage2_matrix.txt}"
: > "$RESULT"
pass=0
...
[[ "$pass" -eq 12 ]]
grep -c 'FAHL27_MATRIX .* PASS' "$RESULT" | grep -qx '12'
echo "FAHL27_STAGE2_MATRIX_12_OF_12=PASS" | tee -a "$RESULT"
