...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl27-paired-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c tests/fahl/test_fahl27_stage2_paired_timing.f90 -o "$BUILD/test.o"
gfortran -fopenmp -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/fahl27_stage2_paired_timing.txt
grep -Fq 'FAHL27_STAGE2_FMR_TIMING=PASS' /tmp/fahl27_stage2_paired_timing.txt
