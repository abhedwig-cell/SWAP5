...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl27-single-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c tests/fahl/test_fahl27_stage2_single_trial_timing.f90 -o "$BUILD/test.o"
gfortran -fopenmp -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/fahl27_stage2_single_trial_timing.txt
grep -Fq 'FAHL27_STAGE2_SINGLE_TIMING=PASS' /tmp/fahl27_stage2_single_trial_timing.txt
