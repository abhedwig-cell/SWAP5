...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl27-trajectory-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail(){ echo "FAHL27_TRAJECTORY_GATE_FAIL $*" >&2; exit 1; }
...
for opt in 0 2; do
  ...
  gfortran "${COMMON[@]}" -O"$opt" -J "$OUT" -I "$OUT" \
    -c tests/fahl/test_fahl27_stage2_trajectory_fallback.f90 -o "$OUT/test.o" || fail "compile O$opt runtime oracle"
  ...
  for marker in \
    ...
    'FKT22_FMR_SERIALIZED_RUNTIME_GATE=PASS' \
    'FAHL27_TRAJECTORY_CACHE BUILDS=0 HITS=0 MISSES=0 ENTRIES=0' \
    'FAHL27_STAGE2_TRAJECTORY_FALLBACK=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing O$opt marker $marker"; }
  done
  cat "$OUT/output.txt"
  echo "FAHL27_TRAJECTORY_O${opt}=PASS"
done
...
git diff --check -- tests/fahl/test_fahl27_stage2_trajectory_fallback.f90 \
  tests/fkt/run_fkt22_fmr_runtime_gate.sh

echo 'FAHL27_TRAJECTORY_O0_O2_IDENTITY=PASS'
echo 'FAHL27_STAGE2_TRAJECTORY_GATE=PASS'
