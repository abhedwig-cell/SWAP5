...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl28b-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c tests/fahl/test_fahl28b_same_key_memoization.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/fahl28b_provider.txt
grep -Fq 'AHL28B_PROVIDER_MEMOIZATION=PASS' /tmp/fahl28b_provider.txt
