...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl28c-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c tests/fahl/test_fahl28c_paired_memoization_timing.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/fahl28c_result.txt
grep -Fq 'AHL28C_PAIRED_TIMING=PASS' /tmp/fahl28c_result.txt
