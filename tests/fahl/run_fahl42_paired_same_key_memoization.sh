...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl42-memo-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O3 -J "$BUILD" -I "$BUILD" -c tests/fahl/test_fahl42_paired_same_key_memoization.f90 -o "$BUILD/test.o"
gfortran -O3 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/fahl42_paired_memoization.txt
grep -Fq 'FAHL42_PAIRED_MEMOIZATION=PASS' /tmp/fahl42_paired_memoization.txt
