...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fahl-layer-${GITHUB_RUN_ID:-local}-$$"
...
gfortran "${COMMON[@]}" -O2 -J "$BUILD" -I "$BUILD" -c tests/fahl/test_fahl_admission_layered_authority.f90 -o "$BUILD/test.o"
gfortran -O2 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"
"$BUILD/test" | tee /tmp/fahl_layered_authority.txt
grep -Fq 'FAHL_LAYERED_AUTHORITY=' /tmp/fahl_layered_authority.txt
