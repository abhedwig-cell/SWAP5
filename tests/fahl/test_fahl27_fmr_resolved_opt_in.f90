program test_fahl27_fmr_resolved_opt_in
  ...
  write(*,'(A)') 'FAHL27_FMR_RESOLVED_OPT_IN=PASS'
...
contains
  ...
  subroutine initialize_committed(committed,p,ok)
    ...
    call initialize_b110_default_mvg_parameters(hp,p%cofgen)
    call bind_b110_default_mvg_provider(provider,hp,dt)
    hh=h0;call provider%evaluate(hh,ww,kk,cc,dd)
  ...
  subroutine initialize_forcing(f,qtop)
...
end program test_fahl27_fmr_resolved_opt_in
