program test_fahl27_reference_floor_timing
  ...
  integer(int64), parameter :: column_id=527101_int64
  ...
  ! Untimed warm-up. The adaptive call builds the immutable representation;
  ! subsequent timed calls include ordinary configuration/bind/cache-hit work.
  call sample_off()
  call sample_on()
  ...
  write(*,'(A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6)') &
       'FAHL27_RF_FIDELITY MAX_DH=',max_dh,'MAX_DTHETA=',max_dw, &
       'DBOTTOM=',abs(obs_on%bottom_flux-obs_off%bottom_flux), &
       'MASS=',max(abs(result_off%mass%residual),abs(result_on%mass%residual))
  write(*,'(A,I0,1X,A,I0)') 'FAHL27_RF_PATH ITER=',obs_on%solver_diagnostics%nonlinear_iterations, &
       'BACKTRACK=',obs_on%solver_diagnostics%backtracking_attempts
  write(*,'(A)') 'FAHL27_REFERENCE_FLOOR_FIDELITY=PASS'
  ...
  do r=1,NPAIR
    ...
    write(*,'(A,I0,2(1X,ES18.10),1X,F10.6)') 'FAHL27_RF_PAIR ',r,times_off(r),times_on(r),ratios(r)
  end do
  call sort7(ratios)
  write(*,'(A,F10.6)') 'FAHL27_RF_MEDIAN_RATIO ',ratios(4)
  write(*,'(A,F10.6)') 'FAHL27_RF_MEDIAN_REDUCTION ',1.0_real64-ratios(4)
  if(ratios(4)<0.98_real64)then
    write(*,'(A)') 'FAHL27_REFERENCE_FLOOR_TIMING=PASS'
  else if(ratios(4)<=1.02_real64)then
    write(*,'(A)') 'FAHL27_REFERENCE_FLOOR_TIMING=EQUIVOCAL'
  else
    write(*,'(A)') 'FAHL27_REFERENCE_FLOOR_TIMING=FAIL'
  end if

contains

  subroutine sample_off()
    call backend_off%run_reference_floor_sample(column,template,p_off,committed_off,forcing, &
         0.0_real64,duration,mass_tol,result_off,cand_off,diag_off)
  end subroutine sample_off

  subroutine sample_on()
    call backend_on%run_reference_floor_sample(column,template,p_on,committed_on,forcing, &
         0.0_real64,duration,mass_tol,result_on,cand_on,diag_on)
  end subroutine sample_on
  ...
  subroutine time_batch(adaptive,elapsed)
    ...
    if(adaptive)then
      do i=1,NREPLAY
        call sample_on()
        if(result_on%status/=KERNEL_REFERENCE_FLOOR_STATUS_OK) error stop 'timed adaptive sample failed'
      end do
    else
      do i=1,NREPLAY
        call sample_off()
        if(result_off%status/=KERNEL_REFERENCE_FLOOR_STATUS_OK) error stop 'timed analytical sample failed'
      end do
    end if
    call cpu_time(t1)
  ...
  subroutine init_parameters(p)
    ...
    integer::i
    p%parameter_set_id=527101_int64; p%active_nodes=numnod
  ...
  subroutine init_column(c,t)
    ...
    t%template_id=527111_int64;t%physics_topology_id=527112_int64;t%vertical_layout_id=527113_int64
    t%state_layout_id=527114_int64;t%solver_interface_id=527115_int64
  ...
  subroutine init_committed(committed,p,initialized)
    ...
    call fmr_new_b110_committed_state(committed,column_id,st,0.0_real64,initialized)
  ...
  subroutine require(cond,msg)
    ...
    if(.not.cond)then
      write(*,'(A,1X,A)')'FAHL27_RF_FAIL',trim(msg)
      error stop 1
    end if
  end subroutine require
end program test_fahl27_reference_floor_timing
