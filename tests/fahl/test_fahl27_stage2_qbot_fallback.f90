program test_fahl27_stage2_qbot_fallback
...
contains
  ...
  subroutine verify_no_adaptive_cache_use()
    ...
    call require(builds==0 .and. hits==0 .and. misses==0 .and. entries==0, &
         'qbot opt-in must not touch adaptive cache')
    write(*,'(A,I0,A,I0,A,I0,A,I0)') 'FAHL27_QBOT_CACHE BUILDS=',builds,' HITS=',hits, &
         ' MISSES=',misses,' ENTRIES=',entries
  end subroutine verify_no_adaptive_cache_use
  ...
  subroutine execute_case(bottom_mode, top_flux, bottom_flux, bottom_head, duration, use_certificate, hydrostatic, output, observation, &
    ...
    if (use_certificate) then
      ...
      call initialize_committed(committed, parameters, hydrostatic, ok)
    end if
  ...
  subroutine initialize_physical_state(parameters, hydrostatic, state)
    type(fmr_b110_physical_parameters_t), intent(in) :: parameters
    logical, intent(in) :: hydrostatic
  ...
  subroutine require(condition, label)
    ...
    if (.not. condition) then
      write(*,'(A,1X,A)') 'FAHL27_QBOT_FAIL', trim(label)
      error stop 1
    end if
  end subroutine require

end program test_fahl27_stage2_qbot_fallback
