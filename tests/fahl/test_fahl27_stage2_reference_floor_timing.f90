program test_fahl27_stage2_reference_floor_timing
  ...
  integer, parameter :: NREPLAY=16384, NPAIR=7
  ...
  do r=1,NPAIR
    ...
    write(*,'(A,I0,2(1X,ES18.10),1X,F10.6)') 'FAHL27_FLOOR_PAIR ',r,times_off(r),times_on(r),ratios(r)
  end do
  call sort7(ratios)
  write(*,'(A,F10.6)') 'FAHL27_FLOOR_MEDIAN_RATIO ',ratios(4)
  write(*,'(A,F10.6)') 'FAHL27_FLOOR_MEDIAN_REDUCTION ',1.0_real64-ratios(4)
  call require(ratios(4)<0.98_real64,'application speed gate')
  write(*,'(A)') 'FAHL27_STAGE2_FLOOR_TIMING=PASS'
...
contains
  ...
  subroutine require(cond,msg)
    ...
    if(.not.cond)then
      write(*,'(A,1X,A)')'FAHL27_FLOOR_TIMING_FAIL',trim(msg);error stop 1
    end if
  end subroutine
end program test_fahl27_stage2_reference_floor_timing
