program test_fahl27_stage2_single_trial_timing
  ...
  write(*,'(A,I0,1X,A,L1,1X,A,I0,1X,A,I0,1X,A,L1,1X,A,I0)') &
       'FAHL27_SINGLE_OFF_STATUS ',result_off%status,'completed=',result_off%completed, &
  ...
  write(*,'(A,I0,1X,A,L1,1X,A,I0,1X,A,I0,1X,A,L1,1X,A,I0)') &
       'FAHL27_SINGLE_ON_STATUS ',result_on%status,'completed=',result_on%completed, &
  ...
  write(*,'(A)') 'FAHL27_SINGLE_OPT_IN_RUNTIME=PASS'
  do r=1,NPAIR
    if(mod(r,2)==1)then
    ...
    write(*,'(A,I0,2(1X,ES18.10),1X,F10.6)') 'FAHL27_SINGLE_PAIR ',r,times_off(r),times_on(r),ratios(r)
  end do
  call sort7(ratios)
  write(*,'(A,F10.6)') 'FAHL27_SINGLE_MEDIAN_RATIO ',ratios(4)
  write(*,'(A,F10.6)') 'FAHL27_SINGLE_MEDIAN_REDUCTION ',1.0_real64-ratios(4)
  call require(ratios(4)<0.98_real64,'application speed gate')
  write(*,'(A)') 'FAHL27_STAGE2_SINGLE_TIMING=PASS'
...
contains
  ...
  subroutine init_config(c)
    ...
    c%transaction%temporal_mode=TX_TEMPORAL_NONE;c%transaction%temporal_tolerance=1.0e-6_real64
  ...
  subroutine require(cond,msg)
    ...
    if(.not.cond)then
      write(*,'(A,1X,A)')'FAHL27_SINGLE_TIMING_FAIL',trim(msg);error stop 1
    end if
  end subroutine
end program test_fahl27_stage2_single_trial_timing
