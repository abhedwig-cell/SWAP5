program test_fahl27_stage2_trajectory_fallback
  ...
  block
    ...
    call require(builds==0 .and. hits==0 .and. misses==0 .and. entries==0, &
         'qbot trajectory route leaves AHL cache untouched')
    write(*,'(A,I0,A,I0,A,I0,A,I0)') 'FAHL27_TRAJECTORY_CACHE BUILDS=',builds,' HITS=',hits, &
         ' MISSES=',misses,' ENTRIES=',entries
  end block
  write(*,'(A)') 'FAHL27_STAGE2_TRAJECTORY_FALLBACK=PASS'
...
contains
  ...
  subroutine require(condition, label)
    ...
    if (.not. condition) then
      write(*,'(A,1X,A)') 'FAHL27_TRAJECTORY_FAIL', trim(label)
      error stop 1
    end if
  end subroutine require

end program test_fahl27_stage2_trajectory_fallback
