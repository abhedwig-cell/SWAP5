program test_fahl27b_fixture_qualification
  ...
  real(real64) :: k0,max_dh,max_dw,hbot
  character(len=64) :: hbot_arg
  real(real64) :: times_off(NPAIR),times_on(NPAIR),ratios(NPAIR)
  integer :: r

  if(command_argument_count()/=1) error stop 'usage: test HBOT'
  call get_command_argument(1,hbot_arg)
  read(hbot_arg,*) hbot
  ...
  write(*,'(A,ES16.8,A,I0,A,L1,A,I0,A,I0,A,I0,A,I0)') 'FAHL27B_ANALYTICAL HBOT=',hbot, &
       ' STATUS=',result_off%status,' COMPLETED=',result_off%completed,' ACCEPTED=',diag_off%accepted_substeps, &
       ' RETRIES=',diag_off%retries,' SOLVER_REJ=',diag_off%solver_rejections,' MASS_REJ=',diag_off%mass_rejections
  stop 0
...
end program test_fahl27b_fixture_qualification
