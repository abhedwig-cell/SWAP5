program test_fahl42_paired_same_key_memoization
  ...
  logical :: ok,hit
  integer :: r,b0,h0,m0,e0,b1,h1,m1,e1
  ...
  do r=1,NPAIR
    ...
    write(*,'(A,I0,2(1X,ES18.10),1X,F12.8)') 'FAHL42_MEMO_PAIR ',r,baseline_t(r),memo_t(r),ratio(r)
  end do
  call sort9(ratio)
  write(*,'(A,F12.8)') 'FAHL42_MEMO_MEDIAN_RATIO ',ratio(5)
  write(*,'(A,F12.8)') 'FAHL42_MEMO_MEDIAN_REDUCTION ',1.0_real64-ratio(5)
  call require(ratio(5)<=0.25_real64,'paired memoization missed frozen 75 percent reduction gate')
  write(*,'(A)') 'FAHL42_PAIRED_MEMOIZATION=PASS'
contains
  ...
  subroutine time_memoized(elapsed)
    ...
    call require(cb1==cb0 .and. ch1==ch0 .and. cm1==cm0 .and. ce1==ce0, &
         'memoized timed path touched shared cache')
    elapsed=(b-a)/real(NOP,real64)
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)')'FAHL42_MEMO_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require
end program test_fahl42_paired_same_key_memoization
