program test_fahl49_matrix
  ...
  use mod_b110_direct_retention_core, only: acquire_b110_direct_retention_slot, freeze_b110_direct_retention_pool, &
       reset_b110_direct_retention_pool
  ...
  type(b110_direct_retention_provider_t), target :: lookup
  ...
  call require(valid .and. .not.was_hit .and. direct_slot==1,'production direct-retention acquire')
  call freeze_b110_direct_retention_pool()
  call bind_b110_direct_retention_provider(lookup,hp,total_dt,direct_slot,valid)
  call require(valid,'production direct-retention provider bound')
...
contains
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'FAHL49_MATRIX_METRIC',trim(material)//':'//trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'FAHL49_MATRIX_DIAG',trim(material)//':'//trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'FAHL49_MATRIX',trim(material)//':'//trim(name),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'FAHL49_MATRIX_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_fahl49_matrix
