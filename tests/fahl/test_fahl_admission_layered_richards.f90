program test_fahl_admission_layered_richards
  ...
  do k=1,numnod
    if(k<=split)then
      call set_mvg(cofgen(:,k),0.02_real64,0.427494_real64,31.225016_real64,0.021659_real64,0.98087_real64,1.734737_real64)
    else
      call set_mvg(cofgen(:,k),0.01_real64,0.393878_real64,2.495984_real64,0.003288_real64,0.514012_real64,1.616573_real64)
    end if
  end do
  ...
  call require(valid,'layered provider bound')
...
contains
  ...
  subroutine set_mvg(v,tr,ts,ksat,alpha,lambda,n)
    real(real64),intent(inout)::v(:)
    real(real64),intent(in)::tr,ts,ksat,alpha,lambda,n
    v=0.0_real64
    v(1)=tr;v(2)=ts;v(3)=ksat;v(4)=alpha;v(5)=lambda;v(6)=n
    v(7)=1.0_real64-1.0_real64/n;v(8)=alpha
    v(9)=0.0_real64;v(10)=ksat;v(11)=0.999_real64;v(12)=0.99_real64*ksat
    v(22)=-1.0e6_real64;v(23)=1.0e-12_real64
  ...
  subroutine report_and_gate(name,ref,cand)
    ...
    write(*,'(A,1X,A,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,ES14.6,1X,A,I0,1X,A,I0)') &
         'FAHL_LAYER_RICHARDS_METRIC',trim(name),'MAX_DH_CM=',dh,'MAX_DTHETA=',dw,'DTOP=',dtf,'DBOTTOM=',dbf,'MASS=',mass, &
         'ITER_DELTA=',diter,'BACKTRACK_DELTA=',dbt
    write(*,'(A,1X,A,1X,A,I0,1X,A,I0,1X,A,I0,1X,A,I0)') &
         'FAHL_LAYER_RICHARDS_DIAG',trim(name),'REF_ITER=',ref%diagnostics%nonlinear_iterations, &
    ...
    write(*,'(A,1X,A,1X,A)') 'FAHL_LAYER_RICHARDS',trim(name),'PASS'
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'FAHL_LAYER_RICHARDS_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_fahl_admission_layered_richards
