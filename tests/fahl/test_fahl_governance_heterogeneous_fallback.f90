program test_fahl_governance_heterogeneous_fallback
  ...
  block
    ...
    call require(builds==0 .and. hits==0 .and. misses==0 .and. entries==0, &
         'heterogeneous opt-in must not touch adaptive cache')
    write(*,'(A,I0,A,I0,A,I0,A,I0)') 'FAHL_HETERO_CACHE BUILDS=',builds,' HITS=',hits, &
         ' MISSES=',misses,' ENTRIES=',entries
  end block
  write(*,'(A)') 'FAHL_HETEROGENEOUS_ANALYTICAL_FALLBACK=PASS'
...
contains
  ...
  subroutine require(cond,msg)
    ...
    if(.not.cond)then
      write(*,'(A,1X,A)')'FAHL27_RESOLVED_FAIL',trim(msg);error stop 1
    end if
  end subroutine
end program test_fahl_governance_heterogeneous_fallback
