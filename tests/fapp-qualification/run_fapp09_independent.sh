...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fapp09-vq-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"
fail(){ echo "FAPP09_VQ_GATE_FAIL $*" >&2; exit 91; }

CANDIDATE=ba3699f970d48b6b15db24fac1d1ec619f7fcb32
git merge-base --is-ancestor "$CANDIDATE" HEAD || fail "qualification head not descended from frozen candidate"
test -z "$(git diff --name-only "$CANDIDATE..HEAD" -- src reference)" || fail "qualification branch changes src/reference"
echo 'FAPP09_VQ_SOURCE_FREEZE=PASS'

python3 - "$BUILD/compile-order.txt" <<'PY'
from pathlib import Path
import re,sys
test=Path("tests/fapp-qualification/test_fapp09_independent_window_semantics.f90")
...
print(f"FAPP09_VQ_COMPILE_CLOSURE_FILES={len(order)}")
PY
...
for opt in 0 2; do
  ...
  for marker in FAPP09_VQ_POSITIVE_WINDOW_LEDGER=PASS FAPP09_VQ_MISMATCH_DISCARD_REPLAY=PASS FAPP09_VQ_NEGATIVE_WINDOW_LEDGER=PASS FAPP09_VQ_INDEPENDENT_WINDOW_TRANSACTION=PASS; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing $marker O$opt"; }
  done
  grep '^FAPP09_VQ_' "$OUT/output.txt" > "$OUT/stable.txt"
done
diff -u "$BUILD/o0/stable.txt" "$BUILD/o2/stable.txt"
cat "$BUILD/o0/stable.txt"
echo 'FAPP09_VQ_O0_O2_EXACT_IDENTITY=PASS'
echo 'F-APP09 INDEPENDENT QUALIFICATION GATE PASS'
