...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ppa-root-hyd01-d1-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "PPA_ROOT_HYD01_D1_FAIL $*" >&2; exit 61; }
...
run_one(){
  ...
  gfortran "${COMMON[@]}" -O"$opt" -J "$out" -I "$out" -c tests/fapp/test_ppa_root_hyd01_sink_equivalence.f90 -o "$out/test.o" || fail "compile PPA-ROOT-HYD01 D1 test O$opt"
  gfortran -fopenmp -O"$opt" "${objects[@]}" "$out/test.o" -o "$out/test" || fail "link PPA-ROOT-HYD01 D1 test O$opt"
  "$out/test" > "$out/output.txt" 2>&1 || { cat "$out/output.txt" >&2; fail "run PPA-ROOT-HYD01 D1 test O$opt"; }
}
...
[[ "$(grep -c '^PPA_ROOT_HYD01_GENERIC duration=' "$BUILD/o0/output.txt")" -eq 5 ]] || fail "expected five GENERIC duration results"
[[ "$(grep -c '^PPA_ROOT_HYD01_PAIR duration=' "$BUILD/o0/output.txt")" -eq 5 ]] || fail "expected five paired duration results"
