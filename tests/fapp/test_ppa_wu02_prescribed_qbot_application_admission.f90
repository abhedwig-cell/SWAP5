program test_ppa_wu02_prescribed_qbot_application_admission
  ...
  do i = 1, NTILE
    ...
    call require(abs(config%tiles(i)%base_forcing%bottom_flux) > 0.0_real64, 'nonzero prescribed qbot forcing')
  end do
  ...
  if (status /= FMR_APP_BOOT_OK) then
    write(*,'(a,1x,i0)') 'PPA_WU02_DEBUG_STANDALONE_STATUS', status
    if (allocated(results)) then
      do i = 1, size(results)
        write(*,'(a,1x,i0,1x,a,1x,i0,1x,l1,1x,l1,1x,l1,1x,i0)') 'PPA_WU02_DEBUG_RESULT', i, &
  ...
  call require(all(results%admitted) .and. all(results%solver_executed), 'prescribed qbot reaches admitted solver')
  call require(all(results%completed) .and. all(results%committed), 'standalone accepted commits')
  call require(all(results%final_committed_time_bound), 'generic-time commit bound')
  call require(maxval(abs(results%final_committed_time - T1)) == 0.0_real64, 'generic-time exact endpoint')
  ...
  print '(a)', 'PPA_WU02_TYPED_CONFIG_TO_FMR_OWNER=PASS'
  print '(a)', 'PPA_WU02_STANDALONE_REFERENCE_RICHARDS_RUNTIME=PASS'
  print '(a)', 'PPA_WU02_NONZERO_PRESCRIBED_QBOT_REACHED=PASS'
  print '(a)', 'PPA_WU02_GENERIC_TIME_ENDPOINT=PASS'
  print '(a)', 'PPA_WU02_STANDALONE_HARD_MASS=PASS'
  print '(a)', 'PPA_WU02_COMMITTED_STATE_FORTRAN_OWNED=PASS'
  print '(a)', 'PPA_WU02_FGC49B_REGISTRY_FORTRAN_OWNED=PASS'
  print '(a)', 'PPA_WU02_MASS_LEDGERS_FORTRAN_OWNED=PASS'
  print '(a)', 'PPA_WU02_FGC49D_CONTEXT_FROM_PRODUCTION_OWNER=PASS'
  print '(a)', 'PPA_WU02_NO_QUALIFICATION_FIXTURE_BOOTSTRAP=PASS'
  print '(a)', 'PPA_WU02_UNADMITTED_PROFILE_FAILS_CLOSED=PASS'
  print '(a)', 'PPA-WU02 PRODUCTION APPLICATION BOOTSTRAP GATE PASS'
...
contains
  ...
  subroutine make_predictor(input, tile, cell, href, slot)
    ...
    call compose_modflow6_swap_predictor_response(window, lineage, 0.001_real64, href, &
         ...
         coverage, 'ppa-wu02', 'typed-production-bootstrap', input%response, local_status)
  ...
  subroutine require(condition, message)
    ...
    if (.not. condition) then
      write(*,'(a,1x,a)') 'PPA_WU02_FAIL', trim(message)
      error stop 1
    end if
  end subroutine require

end program test_ppa_wu02_prescribed_qbot_application_admission
