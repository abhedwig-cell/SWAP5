...
module MOD_swap_base
  implicit none
  integer :: swhea=1,swsolu=1,swcrop=1,swirfix=1
...
module MOD_irrigation
  ...
  real(real64) :: cirr=0,dt_irr_event=0,gird=0,nird=0,qssdisum=0,qssdi(macp)=0
...
module MOD_integral
  ...
  real(real64) :: iqdo(macp+1)=0,iqup(macp+1)=0,cqdrain(madr)=0,cqdrainin(madr)=0,cqdrainout(madr)=0
...
module MOD_wofost
  ...
  end type wofost_states
  type :: agro_management
    real(real64) :: marker=0
  end type agro_management
