...
end module fci12_backend_globals
...
subroutine SWAP(iCaller, iTask, tstart_in, tend_in, swp_file, outfile, toswap, fromswap, worker, trial_mass, interval)
  ...
  if (iCaller /= 0) error stop 'FCI12 stub: caller'
  if (present(swp_file) .or. present(outfile) .or. present(toswap) .or. present(fromswap)) error stop 'FCI12 stub: unexpected IO args'
  if (.not. present(worker) .or. .not. present(trial_mass) .or. .not. present(interval)) error stop 'FCI12 stub: missing args'
  if (iTask == 22) then
    interval%prepared=.true.
    interval%complete=.false.
    return
  end if
  if (iTask /= 2) error stop 'FCI12 stub: task'
