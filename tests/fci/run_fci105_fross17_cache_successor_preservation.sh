...
SOURCE=f135f45088aae2d710809957c8792944ab1cceee
...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fci105-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT

fail() { echo "FCI105_ROSSFAST_CACHE_ADMISSION_FAIL $*" >&2; exit 1; }
...
test "$source_delta" = "$KERNEL" || {
  printf 'FCI105_UNEXPECTED_SOURCE_DELTA=\n%s\n' "$source_delta" >&2
  fail 'production/reference delta is not exact cache-only kernel mutation'
}
echo 'FCI105_EXACT_ONE_FILE_PRODUCTION_DELTA=PASS'
...
print("FCI105_FROSS17_IMMUTABLE_NEUTRALITY_EVIDENCE=PASS")
PY
...
echo "FCI105_ROSSFAST_CACHE_SUCCESSOR_SHA256=$(sha256sum "$BUILD/combined.txt" | awk '{print $1}')"
echo 'FCI105_FROSS17_CACHE_SCIENTIFIC_NEUTRALITY=PASS'
echo 'FCI105_FROSS13_PROVIDER_SUCCESSOR=PASS'
echo 'FCI105_FROSS12_ROUTE_SEMANTICS=PASS'
echo 'FCI105_P2E01_SOLVER_SEAM_PRESERVATION=PASS'
echo 'FCI105_ROSSFAST_TRANSFORM_CACHE_ADMISSION_GATE=PASS'
