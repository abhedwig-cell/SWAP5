...
BUILD="${TMPDIR:-/tmp}/swap5-fci28-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail() { echo "FCI28_GATE_FAIL $*" >&2; exit 1; }
CANONICAL_BASE=0aa4f7ca88a1cd2f3cf7333a35946c4415d9258d
...
# Independent source authority and immutability.
git merge-base --is-ancestor "$CANONICAL_BASE" "$CANDIDATE" || fail 'candidate not descended from F-CI27 canonical base'
git merge-base --is-ancestor "$CANDIDATE" HEAD || fail 'qualification head not descended from exact candidate'
mapfile -t candidate_delta < <(git diff --name-only "$CANONICAL_BASE".."$CANDIDATE" -- src)
printf '%s\n' "${candidate_delta[@]}" | sort > "$BUILD/candidate-src.txt"
...
cmp -s "$BUILD/candidate-src.txt" "$BUILD/expected-src.txt" || {
  ...
  fail 'candidate production delta is not exactly two restart modules'
}
if ! git diff --quiet "$CANDIDATE"..HEAD -- src; then
...
echo 'FCI28_SOURCE_AND_ATTACK_BLOB_LOCKS=PASS'
echo 'FCI28_FCI27_ET_RUNTIME_BLOB_PRESERVATION=PASS'
...
assert restart.count('fmr_restart_state_matches_template') >= 3
...
assert 'mod_fmr_reference_et_demand_binding' not in restart
assert 'mod_fmr_committed_restart' not in etbind
...
for forbidden in ('headcalc','newton','jacobian','root_water_uptake','mass_accounting','calendar'):
    assert forbidden not in etbind, forbidden
print('FCI28_EXPLICIT_STATE_FAMILY_DISCRIMINATOR=PASS')
print('FCI28_ATOMIC_PUBLICATION_SEAM=PASS')
print('FCI28_COMPACT_RESTART_NO_SOLVER_SCRATCH=PASS')
print('FCI28_KERNEL_IO_SEPARATION_STATIC=PASS')
print('FCI28_ET_RESTART_SCOPE_SEPARATION_STATIC=PASS')
PY

# Frozen real-process attack, varied only in a temporary copy to add irregular
# sizes and a fresh non-calendar qualification window. The repository attack
# blob itself remains exact and source-locked above.
...
replacements={
 'real(real64), parameter :: t0 = 1000.125_real64':'real(real64), parameter :: t0 = 6200.0625_real64',
 'real(real64), parameter :: tm = 1000.375_real64':'real(real64), parameter :: tm = 6200.3125_real64',
 'real(real64), parameter :: t1 = 1000.625_real64':'real(real64), parameter :: t1 = 6200.6875_real64',
...
for old,new in replacements.items():
    if old not in s: raise SystemExit(f'missing frozen token: {old}')
    s=s.replace(old,new,1)
p.write_text(s)
PY
echo 'FCI28_REAL_PROCESS_MATRIX_1_2_7_8_17_31_32=PASS'
echo 'FCI28_GENERIC_TIME_WINDOW_6200_0625_6200_3125_6200_6875=PASS'

RFLAGS=(-std=f2008 -ffree-line-length-none -Wall -Wextra -fcheck=all -fbacktrace -ffpe-trap=invalid,zero,overflow)
EFLAGS=(-std=f2008 -ffree-line-length-none -Wall -Wextra -Werror -fcheck=all -fbacktrace -ffpe-trap=invalid,zero,overflow)
MODULE_SRC=(
...
for opt in 0 2; do
  ...
  for src in "${MODULE_SRC[@]}"; do
    ...
    gfortran "${RFLAGS[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c "$src" -o "$obj"
    objects+=("$obj")
  done
  gfortran "${RFLAGS[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c "$BUILD/test_process.f90" -o "$OUT/process.o"
  gfortran -O"$opt" "${objects[@]}" "$OUT/process.o" -o "$OUT/process"
  "$OUT/process" > "$OUT/process.txt" 2>&1 || { cat "$OUT/process.txt" >&2; fail "process O$opt"; }
  for n in 1 2 7 8 17 31 32; do grep -Fq "FMR19_BATCH_SIZE_${n}=PASS" "$OUT/process.txt" || fail "size $n O$opt"; done
  for marker in \
    ...
    'FMR19_REAL_HEADCALC_PROCESS_RESTART_TEST PASS'; do grep -Fq "$marker" "$OUT/process.txt" || fail "process marker $marker O$opt"; done

  gfortran "${RFLAGS[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c tests/fmq/test_fmq27_restart_contract_requalification.f90 -o "$OUT/negative.o"
  gfortran -O"$opt" "${objects[@]}" "$OUT/negative.o" -o "$OUT/negative"
  "$OUT/negative" > "$OUT/negative.txt" 2>&1 || { cat "$OUT/negative.txt" >&2; fail "negative O$opt"; }
  for marker in \
    ...
    'FMQ27_RESTART_CONTRACT_REQUALIFICATION_TEST PASS'; do grep -Fq "$marker" "$OUT/negative.txt" || fail "negative marker $marker O$opt"; done

  # Current-canonical ET runtime preservation, without historical single-delta assumptions.
  gfortran "${EFLAGS[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c src/process/mod_reference_et_demand_process.f90 -o "$OUT/et_process.o"
  gfortran "${EFLAGS[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c src/runtime/mod_fmr_reference_et_demand_binding.f90 -o "$OUT/et_binding.o"
  gfortran "${EFLAGS[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c tests/fmr/test_fmr23_reference_et_runtime_binding.f90 -o "$OUT/et_owner_test.o"
  ...
  "$OUT/et_owner" > "$OUT/et_owner.txt" 2>&1 || { cat "$OUT/et_owner.txt" >&2; fail "ET owner O$opt"; }
  grep -Fq 'FMR23_REFERENCE_ET_RUNTIME_BINDING_TEST PASS' "$OUT/et_owner.txt" || fail "ET owner marker O$opt"

  gfortran "${EFLAGS[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c tests/fvq/test_fvq36_fmr23_reference_et_runtime_oracle.f90 -o "$OUT/et_oracle_test.o"
  ...
  "$OUT/et_oracle" > "$OUT/et_oracle.txt" 2>&1 || { cat "$OUT/et_oracle.txt" >&2; fail "ET oracle O$opt"; }
  for marker in \
    ...
    'FVQ36_FMR23_REFERENCE_ET_RUNTIME_ORACLE PASS'; do grep -Fq "$marker" "$OUT/et_oracle.txt" || fail "ET oracle marker $marker O$opt"; done
  echo "FCI28_O${opt}=PASS"
done

for f in process negative et_owner et_oracle; do
  cmp -s "$BUILD/o0/$f.txt" "$BUILD/o2/$f.txt" || fail "$f O0/O2 output identity"
done
echo 'FCI28_O0_O2_OUTPUT_IDENTITY=PASS'
echo 'FCI28_RESTART_MASS_AND_CONTINUATION=PASS'
echo 'FCI28_FAIL_CLOSED_STATE_FAMILY_AND_ATOMICITY=PASS'
echo 'FCI28_FCI27_ET_RUNTIME_PRESERVATION=PASS'
cat "$BUILD/o0/process.txt"
cat "$BUILD/o0/negative.txt"
cat "$BUILD/o0/et_owner.txt"
cat "$BUILD/o0/et_oracle.txt"
echo 'FCI28_RESTART_CURRENT_CANONICAL_ADMISSION_GATE=PASS'
