...
TMP="$ROOT/tests/fci/.fci31-admission-v3-$TAG.sh"
...
replacement = r'''if old not in s: raise SystemExit('FCI30 F-CI28 source-delta anchor missing')
s=s.replace(old,new,1)
...
p.write_text(s,encoding='utf-8')'''
if s.count(anchor) != 1:
    raise SystemExit(f'FCI31 V3 F-CI30 nested F-CI28 patch anchor count={s.count(anchor)}')
...
if s.count(old_path) != 1:
    raise SystemExit(f'FCI31 V3 replay-path anchor count={s.count(old_path)}')
...
if s.count(old_run) != 1:
    raise SystemExit(f'FCI31 V3 F-CI30 execution anchor count={s.count(old_run)}')
...
echo 'FCI31_NESTED_REPLAY_PATH_HARDENED_V3=PASS'
bash "$TMP"
