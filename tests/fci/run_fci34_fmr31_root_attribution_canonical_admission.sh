...
# Reconstruct independent F-VQ50 scientific sources by exact blob from its
# closed qualification branch. The owner F-MR31 oracle is deliberately absent.
...
for opt in 0 2; do
  ...
  gfortran "${COMMON[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c \
    "$BUILD/test_fvq50_independent_root_attribution_requalification.f90" -o "$OUT/test.o"
...
# Real B1.10/HeadCalc preservation from independently qualified F-VQ21 oracle.
