...
cd "$ROOT"
base="774c01d69c619f463e0d471eb797f8f0418fee07"
allowed=(
  ...
  "integration/f-gc/F-GC18R_PRE_REGISTRATION.json"
  "integration/f-gc/F-GC18R_ARCHITECTURE_AUDIT.json"
  "integration/f-gc/F-GC18R_STATUS.json"
)
mapfile -t changed < <(git diff --name-only "$base..HEAD")
for path in "${changed[@]}"; do
  ok=0
  for candidate in "${allowed[@]}"; do [[ "$path" == "$candidate" ]] && ok=1 && break; done
  [[ "$ok" -eq 1 ]] || { echo "FGC18R_SCOPE_FAIL unexpected path: $path" >&2; exit 20; }
done
...
assert 'subroutine groundwater_commit_prepared(service, checkpoint, prepared, status)' in s
...
if [[ -f integration/f-gc/F-GC18R_ARCHITECTURE_AUDIT.json ]]; then
python3 - <<'PY'
import json
d=json.load(open('integration/f-gc/F-GC18R_ARCHITECTURE_AUDIT.json'))
...
echo 'FGC18R_SCOPE_ALLOWLIST=PASS'
echo 'FGC18R_FGC17_CONTRACT_LOCK=PASS'
echo 'FGC18R_O0_O2_IDENTITY=PASS'
