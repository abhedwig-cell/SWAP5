...
mkdir -p "$B"
trap 'rm -rf "$B"' EXIT
python3 - "$OWNER" "$B/fixture.f90" "$B/fgc24_module.f90" <<'PY'
...
assert text.count(marker)==1
Path(sys.argv[3]).write_text(text.split(marker,1)[0]+'\n')
PY
S=(
...
gfortran -std=f2008 -ffree-line-length-none -O0 -J "$B" -I "$B" "${S[@]}" "$B/fixture.f90" "$B/fgc24_module.f90" tests/fgc/fgc24_export_probe.f90 -o "$B/probe"
"$B/probe"
