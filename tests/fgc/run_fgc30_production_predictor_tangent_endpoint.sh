...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fgc30-production-tangent-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail(){ echo "FGC30_PRODUCTION_TANGENT_FAIL $*" >&2; exit 1; }
...
for opt in 0 2; do
  ...
  gfortran "${COMMON[@]}" -O"$opt" -J "$OUT" -I "$OUT" \
    -c tests/fgc/test_fgc30_production_predictor_tangent_endpoint.f90 -o "$OUT/test.o" || fail "compile O$opt oracle"
  gfortran -fopenmp -O"$opt" "${objects[@]}" "$OUT/test.o" -o "$OUT/test" || fail "link O$opt oracle"
  "$OUT/test" > "$OUT/output.txt" 2>&1 || { cat "$OUT/output.txt" >&2; fail "runtime O$opt"; }
  for marker in \
    'FGC30_PRODUCTION_ACCEPTED_TRAJECTORY_BINDING=PASS' \
  ...
  cat "$OUT/output.txt"
  echo "FGC30_PRODUCTION_TANGENT_O${opt}=PASS"
done
...
echo 'FGC30_PRODUCTION_TANGENT_O0_O2_IDENTITY=PASS'
echo 'FGC30_PRODUCTION_PREDICTOR_TANGENT_QUALIFICATION=PASS'
