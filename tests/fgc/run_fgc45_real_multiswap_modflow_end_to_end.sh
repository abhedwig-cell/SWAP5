...
gfortran -shared -fopenmp -O2 "${objects[@]}" -o "$BUILD/bridge/libfgc45_multiswap.so" || fail "link shared library"
nm -D "$BUILD/bridge/libfgc45_multiswap.so" | grep -q 'fgc45_initialize_c' || fail "missing F-GC45 C ABI"
nm -D "$BUILD/bridge/libfgc45_multiswap.so" | grep -q 'fgc34_publish_c' || fail "missing F-GC34 publisher"

LIBMF6="$BUILD/modflow-bin/libmf6.so" FGC45_MULTISWAP_LIB="$BUILD/bridge/libfgc45_multiswap.so" python3 tests/fgc/test_fgc45_real_multiswap_modflow_end_to_end.py | tee "$BUILD/e2e.txt"

for marker in   'FGC45_TWO_REAL_SWAP_LINEAGES=PASS'   'FGC45_FGC40_N1_AFFINE_RESPONSE=PASS'   'FGC45_AREA_WEIGHTED_REAL_CORRECTORS=PASS'   'FGC45_LIVE_MODFLOW680_PREPARED_SOLVE=PASS'   'FGC45_CONJUNCTIVE_N1_CONVERGENCE=PASS'   'FGC45_ALL_TILE_PREFLIGHTS_BEFORE_PUBLICATION=PASS'   'FGC45_MODFLOW_THEN_ALL_SWAP_THEN_ALL_LEDGER_PUBLICATION=PASS'   'FGC45_REAL_MULTISWAP_MODFLOW_END_TO_END=PASS'; do
  grep -Fq "$marker" "$BUILD/e2e.txt" || fail "missing marker $marker"
done

git diff --check -- tests/fgc/support/mod_fgc45_real_multiswap_c_bridge.f90   tests/fgc/support/fgc45_real_multiswap_ctypes.py   tests/fgc/test_fgc45_real_multiswap_modflow_end_to_end.py
echo 'F-GC45 REAL MULTISWAP + MODFLOW6 END-TO-END GATE PASS'
