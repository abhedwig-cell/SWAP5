...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fgc46-multicell-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/modflow-bin" "$BUILD/downloads" "$BUILD/bridge"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "FGC46_MULTICELL_FAIL $*" >&2; exit 1; }
...
MODULE_SRC=(
  ...
  tests/fgc/support/mod_fgc46_real_multicell_c_bridge.f90
)
objects=()
...
gfortran -shared -fopenmp -O2 "${objects[@]}" -o "$BUILD/bridge/libfgc46_multicell.so" || fail "link shared library"
nm -D "$BUILD/bridge/libfgc46_multicell.so" | grep -q 'fgc46_initialize_c' || fail "missing F-GC46 C ABI"
nm -D "$BUILD/bridge/libfgc46_multicell.so" | grep -q 'fgc34_publish_c' || fail "missing F-GC34 publisher"

LIBMF6="$BUILD/modflow-bin/libmf6.so" FGC46_MULTICELL_LIB="$BUILD/bridge/libfgc46_multicell.so" python3 tests/fgc/test_fgc46_real_multicell_live_modflow.py | tee "$BUILD/e2e.txt"

for marker in   'FGC46_TWO_DISTINCT_LIVE_MODFLOW_CELLS=PASS'   'FGC46_FGC34_TWO_SLOT_NODE_BINDING=PASS'   'FGC46_TWO_REAL_SWAP_SAME_ORIGIN_CORRECTORS=PASS'   'FGC46_ONE_PREPARED_SOLVE_TWO_INTERFACES=PASS'   'FGC46_CONJUNCTIVE_TWO_CELL_CONVERGENCE=PASS'   'FGC46_ALL_INTERFACE_PREFLIGHTS_BEFORE_PUBLICATION=PASS'   'FGC46_SINGLE_MODFLOW_THEN_ALL_SWAP_THEN_ALL_LEDGER_PUBLICATION=PASS'   'FGC46_PER_CELL_LEDGER_CLOSURE=PASS'   'FGC46_REAL_MULTICELL_END_TO_END=PASS'; do
