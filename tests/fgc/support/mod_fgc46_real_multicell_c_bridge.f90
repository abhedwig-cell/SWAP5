module mod_fgc46_real_multicell_c_bridge
  ...
  integer, parameter :: NIF=2
  integer(int64), parameter :: COLUMN_ID(NIF)=[550045_int64,550046_int64]
  integer(int64), parameter :: LEDGER_ID(NIF)=[750045_int64,750046_int64]
  ...
  integer(int64), parameter :: COUPLING_ID(NIF)=[460046_int64,460047_int64]
  integer(int64), parameter :: GW_CELL_ID(NIF)=[7001_int64,7002_int64]
  integer(int64), parameter :: GW_SERVICE_ID=660046_int64
  integer(int64), parameter :: GW_LINEAGE_ID(NIF)=[660047_int64,660048_int64]
  real(real64), parameter :: AREA_M2=1.0_real64

  type(fmr_b110_physical_parameters_t), save :: predictor_parameters(NIF), corrector_parameters(NIF)
  type(fmr_b110_physical_forcing_t), save :: base_forcing(NIF)
  type(fmr_logical_column_t), save :: column(NIF)
  type(fmr_template_t), save :: template(NIF)
  type(canonical_numerical_config_t), save :: predictor_config, corrector_config
  type(kernel_committed_state_t), save :: committed(NIF)
  type(fmr_serialized_reference_backend_t), save :: predictor_backend(NIF), corrector_backend(NIF)
  type(fmr_groundwater_head_forcing_materializer_t), save :: materializer(NIF)
  type(fmr_groundwater_swap_participant_t), save :: participant(NIF)
  type(groundwater_swap_trial_t), save :: last_trial(NIF)
  type(groundwater_head_datum_t), save :: datum
  type(groundwater_coupling_window_t), save :: window
  type(groundwater_interface_mass_ledger_t), save :: ledger(NIF)
  type(groundwater_interface_mass_prepared_t), save :: prepared_ledger(NIF)
  type(fixed_flux_top_boundary_provider_t), target, save :: top(NIF)
  logical, save :: initialized=.false.
  logical, save :: ledger_prepared(NIF)=.false.

  public :: fgc46_initialize_c, fgc46_trial_c, fgc46_discard_c
  public :: fgc46_swap_preflight_c, fgc46_ledgers_prepare_c, fgc46_ledgers_preflight_c
  public :: fgc46_swap_commit_c, fgc46_ledgers_commit_c, fgc46_abort_prepublication_c
  public :: fgc46_state_c

contains

  integer(c_int) function fgc46_initialize_c(hcof1,rhs1,href1,hcof2,rhs2,href2) bind(C,name="fgc46_initialize_c")
    real(c_double),intent(out)::hcof1,rhs1,href1,hcof2,rhs2,href2
    type(modflow6_swap_predictor_response_t) :: response(NIF), one_response(1)
    type(groundwater_direct_tile_binding_t) :: binding(1)
    type(modflow6_multiswap_cell_response_t) :: cell
    type(modflow6_linear_boundary_term_t) :: term(NIF)
    integer :: i,status
    logical :: ok

    fgc46_initialize_c=1_c_int
    ...
    datum%available=.true.; datum%datum_id=560046_int64; datum%bottom_boundary_elevation_m=0.0_real64
    window%t0=0.0_real64; window%t1=DURATION_DAY
    call initialize_configs(predictor_config,corrector_config)

    do i=1,NIF
      call initialize_parameters(predictor_parameters(i),SW_STEP_CONTROL_BOTTOM_FLUX,3000_int64+i)
      call initialize_parameters(corrector_parameters(i),5,4000_int64+i)
      ...
      binding(1)%groundwater_cell_id=GW_CELL_ID(i)
      binding(1)%tile_id=COLUMN_ID(i)
      binding(1)%area_fraction=1.0_real64
      one_response(1)=response(i)
      call compose_modflow6_multiswap_cell_response(binding,one_response,response(i)%h_bot_end_m,cell,status)
      if(status/=MODFLOW6_MULTI_CELL_OK .or. .not.cell%valid)return
      call compose_modflow6_linear_boundary_term(cell,AREA_M2,term(i),status)
      if(status/=MODFLOW6_LINEAR_BACKEND_OK .or. .not.term(i)%valid)return
    ...
    hcof1=term(1)%hcof_m2_per_day; rhs1=term(1)%rhs_m3_per_day; href1=term(1)%reference_head_m
    hcof2=term(2)%hcof_m2_per_day; rhs2=term(2)%rhs_m3_per_day; href2=term(2)%reference_head_m
    initialized=.true.; fgc46_initialize_c=0_c_int
  end function fgc46_initialize_c

  integer(c_int) function fgc46_trial_c(head1_m,head2_m,q1,q2) bind(C,name="fgc46_trial_c")
    real(c_double),value,intent(in)::head1_m,head2_m
    real(c_double),intent(out)::q1,q2
    real(real64) :: head(NIF),q(NIF)
    integer :: i,j,status

    fgc46_trial_c=1_c_int; q1=0.0_c_double; q2=0.0_c_double
    if(.not.initialized)return
    head(1)=real(head1_m,real64); head(2)=real(head2_m,real64)
    do i=1,NIF
      ...
      if(status/=GW_SWAP_PARTICIPANT_OK .or. .not.last_trial(i)%valid)then
        ...
        fgc46_trial_c=int(100*i+max(1,status),c_int)
        return
      end if
      q(i)=last_trial(i)%q_swap_m_per_s
    end do
    q1=q(1); q2=q(2)
    fgc46_trial_c=0_c_int
  end function fgc46_trial_c

  integer(c_int) function fgc46_discard_c() bind(C,name="fgc46_discard_c")
    integer :: i
    fgc46_discard_c=1_c_int
    if(.not.initialized)return
    do i=1,NIF
      if(participant(i)%has_live_candidate())call participant(i)%discard_candidate(corrector_backend(i))
    end do
    fgc46_discard_c=0_c_int
  end function fgc46_discard_c

  integer(c_int) function fgc46_swap_preflight_c() bind(C,name="fgc46_swap_preflight_c")
    integer :: i
    fgc46_swap_preflight_c=1_c_int
    if(.not.initialized)return
    do i=1,NIF
      if(.not.participant(i)%publication_ready(committed(i),window))return
    end do
    fgc46_swap_preflight_c=0_c_int
  end function fgc46_swap_preflight_c

  integer(c_int) function fgc46_ledgers_prepare_c() bind(C,name="fgc46_ledgers_prepare_c")
    type(groundwater_interface_lineage_t) :: lineage
    integer :: i,j,status
    fgc46_ledgers_prepare_c=1_c_int
    if(.not.initialized)return
    if(any(ledger_prepared))return
    do i=1,NIF
      if(.not.last_trial(i)%valid)return
      lineage=groundwater_interface_lineage_t()
      lineage%coupling_id=COUPLING_ID(i); lineage%swap_lineage_id=COLUMN_ID(i); lineage%swap_origin_revision=0_int64
      lineage%groundwater_lineage_id=GW_LINEAGE_ID(i); lineage%groundwater_origin_revision=0_int64
      lineage%candidate_revision=1_int64
      call ledger(i)%stage_exchange(window,lineage,last_trial(i)%bottom_outward_exchange_cm*0.01_real64,status)
    ...
    fgc46_ledgers_prepare_c=0_c_int
  end function fgc46_ledgers_prepare_c

  integer(c_int) function fgc46_ledgers_preflight_c() bind(C,name="fgc46_ledgers_preflight_c")
    integer :: i
    fgc46_ledgers_preflight_c=1_c_int
    if(.not.initialized .or. .not.all(ledger_prepared))return
    do i=1,NIF
      if(.not.ledger(i)%prepared_ready_for_commit(prepared_ledger(i)))return
    end do
    fgc46_ledgers_preflight_c=0_c_int
  end function fgc46_ledgers_preflight_c

  integer(c_int) function fgc46_swap_commit_c() bind(C,name="fgc46_swap_commit_c")
    integer :: i,status,committed_count
    logical :: did_commit
    fgc46_swap_commit_c=1_c_int; committed_count=0
    if(.not.initialized)return
    do i=1,NIF
      ...
      if(.not.did_commit .or. status/=GW_SWAP_PARTICIPANT_OK)then
        if(committed_count>0)error stop 'F-GC46 late SWAP commit failure after publication point'
        return
      end if
      committed_count=committed_count+1
    end do
    fgc46_swap_commit_c=0_c_int
  end function fgc46_swap_commit_c

  integer(c_int) function fgc46_ledgers_commit_c() bind(C,name="fgc46_ledgers_commit_c")
    integer :: i
    fgc46_ledgers_commit_c=1_c_int
    if(.not.initialized .or. .not.all(ledger_prepared))return
    do i=1,NIF
      if(.not.ledger(i)%prepared_ready_for_commit(prepared_ledger(i)))error stop 'F-GC46 late ledger invariant'
    end do
    do i=1,NIF
      call ledger(i)%commit_prepared(prepared_ledger(i)); ledger_prepared(i)=.false.
    end do
    fgc46_ledgers_commit_c=0_c_int
  end function fgc46_ledgers_commit_c

  integer(c_int) function fgc46_abort_prepublication_c() bind(C,name="fgc46_abort_prepublication_c")
    integer :: i
    fgc46_abort_prepublication_c=1_c_int
    if(.not.initialized)return
    do i=1,NIF
      if(participant(i)%has_live_candidate())call participant(i)%discard_candidate(corrector_backend(i))
    ...
    fgc46_abort_prepublication_c=0_c_int
  end function fgc46_abort_prepublication_c

  integer(c_int) function fgc46_state_c(rev1,rev2,t1,t2,count1,count2,exchange1,exchange2) bind(C,name="fgc46_state_c")
    ...
    logical::a1,a2
    fgc46_state_c=1_c_int; rev1=-1; rev2=-1; count1=-1; count2=-1
    ...
    fgc46_state_c=0_c_int
  end function fgc46_state_c
  ...
  subroutine build_tile_predictor(i,response,status)
    ...
    predictor_lineage%coupling_id=COUPLING_ID(i); predictor_lineage%swap_lineage_id=COLUMN_ID(i)
    predictor_lineage%swap_origin_revision=0_int64; predictor_lineage%groundwater_service_id=GW_SERVICE_ID
    predictor_lineage%groundwater_lineage_id=GW_LINEAGE_ID(i); predictor_lineage%groundwater_origin_revision=0_int64
...
end module mod_fgc46_real_multicell_c_bridge
