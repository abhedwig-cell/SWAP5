module mod_fgc47_mixed_topology_c_bridge
  ...
  integer(int64), parameter :: COLUMN_ID(NPART)=[570047_int64,570048_int64,570049_int64]
  integer(int64), parameter :: LEDGER_ID(NPART)=[770047_int64,770048_int64,770049_int64]
  integer(int64), parameter :: CELL_ID(NPART)=[7001_int64,7001_int64,7002_int64]
  integer(int64), parameter :: COUPLING_ID(NPART)=[470047_int64,470047_int64,470048_int64]
  integer(int64), parameter :: GW_LINEAGE_ID(NPART)=[670047_int64,670047_int64,670048_int64]
  ...
  integer(int64), parameter :: GW_SERVICE_ID=670046_int64
  real(real64), parameter :: AREA_M2=1.0_real64

  type(fmr_b110_physical_parameters_t), save :: predictor_parameters(NPART), corrector_parameters(NPART)
  type(fmr_b110_physical_forcing_t), save :: base_forcing(NPART)
  type(fmr_logical_column_t), save :: column(NPART)
  type(fmr_template_t), save :: template(NPART)
  type(canonical_numerical_config_t), save :: predictor_config, corrector_config
  type(kernel_committed_state_t), save :: committed(NPART)
  type(fmr_serialized_reference_backend_t), save :: predictor_backend(NPART), corrector_backend(NPART)
  type(fmr_groundwater_head_forcing_materializer_t), save :: materializer(NPART)
  type(fmr_groundwater_swap_participant_t), save :: participant(NPART)
  type(groundwater_swap_trial_t), save :: last_trial(NPART)
  type(groundwater_head_datum_t), save :: datum
  type(groundwater_coupling_window_t), save :: window
  type(groundwater_interface_mass_ledger_t), save :: ledger(NPART)
  type(groundwater_interface_mass_prepared_t), save :: prepared_ledger(NPART)
  type(fixed_flux_top_boundary_provider_t), target, save :: top(NPART)
  logical, save :: initialized=.false.
  logical, save :: ledger_prepared(NPART)=.false.

  public :: fgc47_initialize_c, fgc47_trial_c, fgc47_discard_c
  public :: fgc47_swap_preflight_c, fgc47_ledgers_prepare_c, fgc47_ledgers_preflight_c
  public :: fgc47_swap_commit_c, fgc47_ledgers_commit_c, fgc47_abort_prepublication_c
  public :: fgc47_state_c

contains

  integer(c_int) function fgc47_initialize_c(hcof1,rhs1,href1,hcof2,rhs2,href2) bind(C,name="fgc47_initialize_c")
    real(c_double),intent(out)::hcof1,rhs1,href1,hcof2,rhs2,href2
    type(modflow6_swap_predictor_response_t) :: response(NPART)
    type(groundwater_direct_tile_binding_t) :: binding1(2),binding2(1)
    type(modflow6_multiswap_cell_response_t) :: cell1,cell2
    type(modflow6_linear_boundary_term_t) :: term1,term2
    type(modflow6_swap_predictor_response_t) :: response2(1)
    real(real64) :: href_cell1
    integer :: i,status
    logical :: ok

    fgc47_initialize_c=1_c_int
    hcof1=0.0_c_double; rhs1=0.0_c_double; href1=0.0_c_double
    hcof2=0.0_c_double; rhs2=0.0_c_double; href2=0.0_c_double
    initialized=.false.; ledger_prepared=.false.
    datum%available=.true.; datum%datum_id=570047_int64; datum%bottom_boundary_elevation_m=0.0_real64
    window%t0=0.0_real64; window%t1=DURATION_DAY
    call initialize_configs(predictor_config,corrector_config)

    do i=1,NPART
      call initialize_parameters(predictor_parameters(i),SW_STEP_CONTROL_BOTTOM_FLUX,5000_int64+i)
      call initialize_parameters(corrector_parameters(i),5,6000_int64+i)
      ...
      call build_predictor(i,response(i),status)
      if(status/=0)return
    end do

    binding1(1)%groundwater_cell_id=7001_int64
    binding1(1)%tile_id=COLUMN_ID(1)
    binding1(1)%area_fraction=FRACTION(1)
    binding1(2)%groundwater_cell_id=7001_int64
    binding1(2)%tile_id=COLUMN_ID(2)
    binding1(2)%area_fraction=FRACTION(2)
    href_cell1=FRACTION(1)*response(1)%h_bot_end_m+FRACTION(2)*response(2)%h_bot_end_m
    call compose_modflow6_multiswap_cell_response(binding1,response(1:2),href_cell1,cell1,status)
    if(status/=MODFLOW6_MULTI_CELL_OK .or. .not.cell1%valid)return
    call compose_modflow6_linear_boundary_term(cell1,AREA_M2,term1,status)
    if(status/=MODFLOW6_LINEAR_BACKEND_OK .or. .not.term1%valid)return

    binding2(1)%groundwater_cell_id=7002_int64
    binding2(1)%tile_id=COLUMN_ID(3)
    binding2(1)%area_fraction=1.0_real64
    response2(1)=response(3)
    call compose_modflow6_multiswap_cell_response(binding2,response2,response(3)%h_bot_end_m,cell2,status)
    if(status/=MODFLOW6_MULTI_CELL_OK .or. .not.cell2%valid)return
    call compose_modflow6_linear_boundary_term(cell2,AREA_M2,term2,status)
    if(status/=MODFLOW6_LINEAR_BACKEND_OK .or. .not.term2%valid)return

    hcof1=term1%hcof_m2_per_day; rhs1=term1%rhs_m3_per_day; href1=term1%reference_head_m
    hcof2=term2%hcof_m2_per_day; rhs2=term2%rhs_m3_per_day; href2=term2%reference_head_m
    ...
    initialized=.true.; fgc47_initialize_c=0_c_int
  end function fgc47_initialize_c

  integer(c_int) function fgc47_trial_c(head1_m,head2_m,qcell1,qcell2,q1,q2,q3) bind(C,name="fgc47_trial_c")
    real(c_double),value,intent(in)::head1_m,head2_m
    real(c_double),intent(out)::qcell1,qcell2,q1,q2,q3
    real(real64)::head(NPART),q(NPART)
    integer::i,j,status

    fgc47_trial_c=1_c_int
    qcell1=0.0_c_double; qcell2=0.0_c_double; q1=0.0_c_double; q2=0.0_c_double; q3=0.0_c_double
    if(.not.initialized)return
    head=[real(head1_m,real64),real(head1_m,real64),real(head2_m,real64)]
    do i=1,NPART
      call participant(i)%trial_from_origin(corrector_backend(i),column(i),template(i),corrector_parameters(i), &
           committed(i),materializer(i),corrector_config,datum,window,head(i),last_trial(i),status)
      if(status/=GW_SWAP_PARTICIPANT_OK .or. .not.last_trial(i)%valid)then
        ...
        fgc47_trial_c=int(100*i+max(1,status),c_int)
        return
      end if
      q(i)=last_trial(i)%q_swap_m_per_s
    end do
    qcell1=FRACTION(1)*q(1)+FRACTION(2)*q(2)
    qcell2=q(3); q1=q(1); q2=q(2); q3=q(3)
    fgc47_trial_c=0_c_int
  end function fgc47_trial_c

  integer(c_int) function fgc47_discard_c() bind(C,name="fgc47_discard_c")
    integer::i
    fgc47_discard_c=1_c_int
    if(.not.initialized)return
    do i=1,NPART
      if(participant(i)%has_live_candidate())call participant(i)%discard_candidate(corrector_backend(i))
    end do
    fgc47_discard_c=0_c_int
  end function fgc47_discard_c

  integer(c_int) function fgc47_swap_preflight_c() bind(C,name="fgc47_swap_preflight_c")
    integer::i
    fgc47_swap_preflight_c=1_c_int
    if(.not.initialized)return
    do i=1,NPART
      if(.not.participant(i)%publication_ready(committed(i),window))return
    end do
    fgc47_swap_preflight_c=0_c_int
  end function fgc47_swap_preflight_c

  integer(c_int) function fgc47_ledgers_prepare_c() bind(C,name="fgc47_ledgers_prepare_c")
    type(groundwater_interface_lineage_t)::lineage
    integer::i,j,status
    real(real64)::weight
    fgc47_ledgers_prepare_c=1_c_int
    if(.not.initialized .or. any(ledger_prepared))return
    do i=1,NPART
      if(.not.last_trial(i)%valid)return
      lineage=groundwater_interface_lineage_t()
      lineage%coupling_id=COUPLING_ID(i)
      lineage%swap_lineage_id=COLUMN_ID(i)
      lineage%swap_origin_revision=0_int64
      lineage%groundwater_lineage_id=GW_LINEAGE_ID(i)
      lineage%groundwater_origin_revision=0_int64
      lineage%candidate_revision=1_int64
      weight=FRACTION(i)
      call ledger(i)%stage_exchange(window,lineage,weight*last_trial(i)%bottom_outward_exchange_cm*0.01_real64,status)
    ...
    fgc47_ledgers_prepare_c=0_c_int
  end function fgc47_ledgers_prepare_c

  integer(c_int) function fgc47_ledgers_preflight_c() bind(C,name="fgc47_ledgers_preflight_c")
    integer::i
    fgc47_ledgers_preflight_c=1_c_int
    if(.not.initialized .or. .not.all(ledger_prepared))return
    do i=1,NPART
      if(.not.ledger(i)%prepared_ready_for_commit(prepared_ledger(i)))return
    end do
    fgc47_ledgers_preflight_c=0_c_int
  end function fgc47_ledgers_preflight_c

  integer(c_int) function fgc47_swap_commit_c() bind(C,name="fgc47_swap_commit_c")
    integer::i,status,committed_count
    logical::did_commit
    fgc47_swap_commit_c=1_c_int; committed_count=0
    if(.not.initialized)return
    do i=1,NPART
      ...
      if(.not.did_commit .or. status/=GW_SWAP_PARTICIPANT_OK)then
        if(committed_count>0)error stop 'F-GC47 late SWAP commit failure after publication point'
        return
      end if
      committed_count=committed_count+1
    end do
    fgc47_swap_commit_c=0_c_int
  end function fgc47_swap_commit_c

  integer(c_int) function fgc47_ledgers_commit_c() bind(C,name="fgc47_ledgers_commit_c")
    integer::i
    fgc47_ledgers_commit_c=1_c_int
    if(.not.initialized .or. .not.all(ledger_prepared))return
    do i=1,NPART
      if(.not.ledger(i)%prepared_ready_for_commit(prepared_ledger(i)))error stop 'F-GC47 late ledger invariant'
    end do
    do i=1,NPART
      call ledger(i)%commit_prepared(prepared_ledger(i)); ledger_prepared(i)=.false.
    end do
    fgc47_ledgers_commit_c=0_c_int
  end function fgc47_ledgers_commit_c

  integer(c_int) function fgc47_abort_prepublication_c() bind(C,name="fgc47_abort_prepublication_c")
    integer::i
    fgc47_abort_prepublication_c=1_c_int
    if(.not.initialized)return
    do i=1,NPART
      if(participant(i)%has_live_candidate())call participant(i)%discard_candidate(corrector_backend(i))
    ...
    fgc47_abort_prepublication_c=0_c_int
  end function fgc47_abort_prepublication_c

  integer(c_int) function fgc47_state_c(r1,r2,r3,t1,t2,t3,c1,c2,c3,e1,e2,e3) bind(C,name="fgc47_state_c")
    integer(c_int),intent(out)::r1,r2,r3,c1,c2,c3
    real(c_double),intent(out)::t1,t2,t3,e1,e2,e3
    type(groundwater_interface_mass_snapshot_t)::snap(NPART)
    real(real64)::tv(NPART)
    logical::available
    integer::i
    fgc47_state_c=1_c_int
    r1=-1; r2=-1; r3=-1; c1=-1; c2=-1; c3=-1
    t1=0.0_c_double; t2=0.0_c_double; t3=0.0_c_double
    e1=0.0_c_double; e2=0.0_c_double; e3=0.0_c_double
    if(.not.initialized)return
    do i=1,NPART
      call committed(i)%current_time(tv(i),available); if(.not.available)return
      call ledger(i)%snapshot(snap(i)); if(.not.snap(i)%available)return
    end do
    r1=int(committed(1)%current_revision(),c_int); r2=int(committed(2)%current_revision(),c_int)
    r3=int(committed(3)%current_revision(),c_int)
    t1=tv(1); t2=tv(2); t3=tv(3)
    c1=int(snap(1)%committed_exchange_count,c_int); c2=int(snap(2)%committed_exchange_count,c_int)
    c3=int(snap(3)%committed_exchange_count,c_int)
    e1=snap(1)%committed_swap_outward_exchange_m; e2=snap(2)%committed_swap_outward_exchange_m
    e3=snap(3)%committed_swap_outward_exchange_m
    fgc47_state_c=0_c_int
  end function fgc47_state_c

  subroutine build_predictor(i,response,status)
    integer,intent(in)::i
    ...
    predictor_lineage%coupling_id=COUPLING_ID(i)
    predictor_lineage%swap_lineage_id=COLUMN_ID(i)
    predictor_lineage%swap_origin_revision=0_int64
    predictor_lineage%groundwater_service_id=GW_SERVICE_ID
    predictor_lineage%groundwater_lineage_id=GW_LINEAGE_ID(i)
    ...
    status=0
  end subroutine build_predictor
  ...
  subroutine initialize_column_template(c,t,i)
    ...
    integer,intent(in)::i
    t%template_id=570000_int64+i; t%physics_topology_id=570010_int64; t%vertical_layout_id=570020_int64
    t%state_layout_id=570030_int64; t%solver_interface_id=570040_int64; t%optional_state_layout_id=0_int64
    ...
    c%column_id=COLUMN_ID(i); c%template_id=t%template_id; c%parameter_ref=i
    c%state_handle=i; c%forcing_handle=i; c%backend_id=FMR_BACKEND_SERIALIZED_REFERENCE
...
end module mod_fgc47_mixed_topology_c_bridge
