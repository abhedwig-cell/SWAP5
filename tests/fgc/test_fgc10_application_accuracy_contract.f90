program test_fgc10_application_accuracy_contract
  ...
  call check(available .and. close_real(budget, 3.0_real64), 'HEAD budget composes H_app*A_temporal', failures)
  call contract%materialize_model_temporal_budget(config, materialized)
  call check(materialized .and. config%model_temporal_indicator_budget_available, 'HEAD budget materializes', failures)
  call check(close_real(config%model_temporal_indicator_budget, 3.0_real64), 'carrier receives native HEAD budget in cm', failures)
  ...
  call check(.not. contract%temporal_budget_ready(), 'NaN H_app fails closed under traps', failures)
  ...
  call check(.not. contract%temporal_budget_ready(), 'NaN A_temporal fails closed under traps', failures)
  ...
  if (failures /= 0) then
    write(*,'(A,I0)') 'FGC10_FAILURES=', failures
    error stop 1
  end if

  write(*,'(A)') 'FGC10_CONTRACT_MATRIX=PASS'
  write(*,'(A)') 'FGC10_NO_DEFAULT_NUMERIC_POLICY=PASS'
  write(*,'(A)') 'FGC10_STALE_BUDGET_CLEARING=PASS'
  write(*,'(A)') 'FGC10_APPLICATION_ACCURACY_CONTRACT PASS'
...
contains
  ...
  subroutine check(condition, label, failures)
    ...
    if (.not. condition) then
      failures = failures + 1
      write(*,'(A,A)') 'FGC10_CHECK_FAIL: ', trim(label)
    end if
  end subroutine check

end program test_fgc10_application_accuracy_contract
