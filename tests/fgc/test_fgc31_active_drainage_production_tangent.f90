program test_fgc31_active_drainage_production_tangent
  ...
  real(real64), parameter :: temporal_head_budget = 2.0e-5_real64
  ...
  real(real64) :: fd_face, scale_face, scale_head, max_head_error
  integer :: status
  ...
  call require(tangent_result%accepted_trajectory_direction%accepted_steps==tangent_diag%accepted_substeps, &
       'trajectory accepted-step count matches runtime')
  ...
  call require(tangent_result%accepted_trajectory_direction%additional_full_nonlinear_solves==0, &
       'active drainage tangent adds no full nonlinear solve')
  ...
  call require(endpoint%coverage%drainage_active .and. endpoint%coverage%drainage_covered, &
       'endpoint carries active drainage coverage')
  call require(endpoint%coverage%tangent_complete(),'active drainage endpoint coverage complete')
  call require(endpoint%accepted_steps>=2,'endpoint provenance includes multiple accepted steps')
  ...
  call require(plain_result%status==CANONICAL_STATUS_COMPLETED .and. plain_result%completed, &
       'plain physical production interval completed')
  call require(plain_candidate%ready(),'plain physical candidate ready')
  call require(plain_diag%accepted_substeps==tangent_diag%accepted_substeps .and. &
       plain_diag%retries==tangent_diag%retries,'tangent request changed physical acceptance topology')
  ...
  call require(abs(fd_face-endpoint%bottom_face%dpressure_head_cm_per_qbot_cm_per_day) <= fd_rel_gate*scale_face, &
       'active drainage terminal bottom-face tangent disagrees with production FD')

  max_head_error=maxval(abs((plus_head-minus_head)/(2.0_real64*fd_eps)- &
  ...
  call require(max_head_error <= fd_rel_gate*scale_head, &
       'active drainage final pressure-head direction disagrees with production FD')

  call require(ieee_is_finite(endpoint%bottom_face%dpressure_head_cm_per_qbot_cm_per_day), &
       'active drainage endpoint derivative finite')
  write(*,'(A,I0)') 'FGC31_ACTIVE_DRAINAGE_ACCEPTED_SUBSTEPS=',tangent_diag%accepted_substeps
  write(*,'(A,I0)') 'FGC31_ACTIVE_DRAINAGE_RETRIES=',tangent_diag%retries
  write(*,'(A,1X,ES14.6)') 'FGC31_ACTIVE_DRAINAGE_FACE_FD_ERROR=', &
       abs(fd_face-endpoint%bottom_face%dpressure_head_cm_per_qbot_cm_per_day)
  write(*,'(A,1X,ES14.6)') 'FGC31_ACTIVE_DRAINAGE_HEAD_FD_MAX_ERROR=',max_head_error
  write(*,'(A)') 'FGC31_ACTIVE_DRAINAGE_MULTI_SUBSTEP=PASS'
  write(*,'(A)') 'FGC31_ACTIVE_DRAINAGE_COVERAGE_PROVENANCE=PASS'
  write(*,'(A)') 'FGC31_ACTIVE_DRAINAGE_TANGENT_PHYSICAL_IDENTITY=PASS'
  write(*,'(A)') 'FGC31_ACTIVE_DRAINAGE_PRODUCTION_FD=PASS'
...
contains
  ...
  subroutine initialize_parameters(value)
    ...
    value%max_iterations=16; value%max_backtracking=8; value%min_step_duration=1.0e-10_real64
    ...
    allocate(value%drainage_response_levels(2)%tabulated%groundwater_depth(2), &
  ...
  subroutine initialize_column_template(c,t)
    ...
    t%template_id=531101_int64; t%physics_topology_id=531102_int64; t%vertical_layout_id=531103_int64
    t%state_layout_id=531104_int64; t%solver_interface_id=531105_int64
  ...
  subroutine initialize_committed(committed,initialized)
    ...
    state%ponding_depth=0.0_real64; state%groundwater_level=-0.3_real64
  ...
  subroutine run_candidate(bottom_flux,request_trajectory,result,candidate,diagnostics)
    ...
    if(.not.result%completed) then
      write(*,'(A,I0,A,F12.8,A,I0,A,I0,A,I0)') 'FGC31_E2E_DIAG status=',result%status, &
           ' completed_t=',result%completed_t,' retries=',diagnostics%retries, &
           ' temporal_rejections=',diagnostics%temporal_rejections,' accepted=',diagnostics%accepted_substeps
    end if
  ...
  subroutine materialize_solver_view(candidate,state,parameter_set)
    ...
    parameter_set%z=parameters%z; parameter_set%dz=parameters%dz
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'FGC31_ACTIVE_DRAINAGE_E2E_FAIL',trim(label)
      error stop 31
    end if
  end subroutine require
end program test_fgc31_active_drainage_production_tangent
