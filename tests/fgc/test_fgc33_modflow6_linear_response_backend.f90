...
contains
  ...
  subroutine test_exact_hcof_rhs_transform(failures)
    ...
    call require(status == MODFLOW6_LINEAR_BACKEND_OK .and. term%valid, &
         'valid F-GC31 cell rejected', failures)
    ...
    call require(status == MODFLOW6_MULTI_CELL_OK, 'F-GC31 second-head evaluation rejected', failures)
    ...
    call require_close(q_volume, expected_volume, 1.0e-10_real64, &
         'second-head MODFLOW/F-GC31 closure mismatch', failures)
