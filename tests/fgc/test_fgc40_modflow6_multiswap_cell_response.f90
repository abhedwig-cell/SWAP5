program test_fgc40_modflow6_multiswap_cell_response
  ...
  if (failures /= 0) then
    write(*,'(A,I0)') 'F-GC40 FAILURES=', failures
    error stop 1
  end if

  print '(A)', 'FGC40_SINGLE_TILE_EQUIVALENCE=PASS'
  print '(A)', 'FGC40_HETEROGENEOUS_AFFINE_CLOSURE=PASS'
  print '(A)', 'FGC40_REFERENCE_HEAD_RECONCILIATION=PASS'
  print '(A)', 'FGC40_AREA_WEIGHTED_U=PASS'
  print '(A)', 'FGC40_PERMUTATION_DETERMINISM=PASS'
  print '(A)', 'FGC40_CANONICAL_TILE_PROVENANCE=PASS'
  print '(A)', 'FGC40_PROVENANCE_FAIL_CLOSED=PASS'
  print '(A)', 'FGC40_CELL_RESPONSE_CONTRACT_GATE=PASS'
...
contains
  ...
  subroutine make_response(swap_lineage_id, response, second)
    ...
    call compose_modflow6_swap_predictor_response(window, lineage, qbot, hstart, hend, dhdq, &
         MODFLOW6_DERIVATIVE_TRAJECTORY_TANGENT, coverage, 'fgc40-fixture', &
         'drainage-free-smooth-route', response, status)
    if (status /= MODFLOW6_PREDICTOR_OK .or. .not. response%valid) then
      write(*,'(A)') 'FGC40 fixture construction failed'
      error stop 2
    end if
  ...
  subroutine require(condition, message, failures)
    ...
    if (.not. condition) then
      failures = failures + 1
      write(*,'(A)') 'FGC40_FAIL: '//trim(message)
    end if
...
end program test_fgc40_modflow6_multiswap_cell_response
