...
run_one(){
  ...
  gfortran "${COMMON[@]}" "$opt" -J "$out" -I "$out" -c tests/fkt/test_fkt22_canonical_directional_response.f90 -o "$out/test.o"
  gfortran "$opt" "$out/transaction.o" "$out/contracts.o" "$out/runtime.o" "$out/test.o" -o "$out/test_fkt22"
  "$out/test_fkt22" | tee "$out/output.txt"

  grep -Fq 'FKT22_CANONICAL_DIRECTIONAL_RESPONSE PASS' "$out/output.txt" || fail "canonical response PASS missing $tag"
  grep -Fq 'FKT22_MULTI_TRANSACTION_WHOLE_WINDOW=PASS' "$out/output.txt" || fail "multi-transaction marker missing $tag"
  grep -Fq 'FKT22_FAILURE_NO_PUBLICATION=PASS' "$out/output.txt" || fail "failure publication marker missing $tag"
  grep -Fq 'FKT22_OUTER_LIFECYCLE_ONCE=PASS' "$out/output.txt" || fail "outer lifecycle marker missing $tag"
...
grep '^FKT22_' "$BUILD/o0/output.txt" > "$BUILD/o0/stable.txt"
grep '^FKT22_' "$BUILD/o2/output.txt" > "$BUILD/o2/stable.txt"
