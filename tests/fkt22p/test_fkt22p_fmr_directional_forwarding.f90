module fkt22p_test_support
  ...
  type, extends(kernel_model_t) :: probe_model_t
...
contains
  ...
  subroutine probe_configure_parameters(self, parameters)
    ...
    if (.not. same_type_as(self, self) .or. .not. same_type_as(parameters, parameters)) &
      error stop 'F-KT22P probe configure unreachable'
  ...
  logical function probe_execution_admitted(self, parameters, numerical_config) result(admitted)
    ...
    if (.not. same_type_as(self, self) .or. .not. same_type_as(parameters, parameters)) &
      error stop 'F-KT22P probe admission unreachable'
  ...
  subroutine probe_advance(self, state, t0, t1, outcome)
    ...
    class default
      error stop 'F-KT22P probe unexpected state'
    end select
    ...
    if (.not. same_type_as(self, self)) error stop 'F-KT22P probe advance unreachable'
  ...
  real(real64) function probe_storage(self, state) result(value)
    ...
    type is (probe_state_t)
      value = typed%value
    class default
      error stop 'F-KT22P probe unexpected storage state'
    end select
    if (.not. same_type_as(self, self)) error stop 'F-KT22P probe storage unreachable'
  ...
  real(real64) function probe_temporal_error(self, full_state, half_state) result(value)
    ...
    if (.not. same_type_as(self, self) .or. .not. same_type_as(full_state, full_state) .or. &
        .not. same_type_as(half_state, half_state)) error stop 'F-KT22P temporal unreachable'
  ...
  subroutine probe_storage_status(self, state, complete, missing_mask)
    ...
    if (.not. same_type_as(self, self) .or. .not. same_type_as(state, state)) &
      error stop 'F-KT22P storage status unreachable'
  ...
  subroutine probe_prepare_interval(self, forcing, interval, config)
    ...
    if (.not. same_type_as(self, self) .or. .not. same_type_as(forcing, forcing) .or. &
      ...
      error stop 'F-KT22P prepare invalid'
...
end module fkt22p_test_support

program test_fkt22p_fmr_directional_forwarding
  use, intrinsic :: iso_fortran_env, only: int64, real64
  ...
  use mod_kernel_transactions, only: kernel_executor_t, kernel_committed_state_t, kernel_checkpoint_t, &
       kernel_candidate_state_t, kernel_result_t, kernel_diagnostics_t
  use mod_fmr_checkpoint_orchestrator, only: fmr_capture_checkpoint, fmr_trial_from_checkpoint, fmr_discard_candidate
  use fkt22p_test_support, only: probe_state_t, probe_forcing_t, probe_parameters_t, probe_model_t
  implicit none
  ...
  logical :: initialized, checkpoint_ok

  call seed%clone(initial)
  call committed%initialize(191_int64, initial, initialized, initial_time=0.0_real64)
  call assert_true(initialized, 'committed initialized')
  call executor%bind_model(model)
  call fmr_capture_checkpoint(committed, checkpoint, checkpoint_ok)
  call assert_true(checkpoint_ok .and. checkpoint%ready(), 'checkpoint captured')
  ...
  call fmr_trial_from_checkpoint(executor, parameters, committed, forcing, config, 0.0_real64, 1.0_real64, &
       checkpoint, result, candidate, diagnostics, request)
  call assert_true(result%completed .and. candidate%ready(), 'requested FMR trial completed')
  call assert_true(result%directional_response%requested, 'request forwarded')
  call assert_true(result%directional_response%available, 'response available')
  call assert_true(result%directional_response%control_coordinate == 2, 'control coordinate preserved')
  call assert_true(result%directional_response%origin_t0 == 0.0_real64 .and. &
       result%directional_response%accepted_t1 == 1.0_real64, 'whole-window provenance preserved')
  call assert_true(abs(result%directional_response%accepted_bottom_exchange_derivative-3.0_real64) < 1.0e-14_real64, &
       'whole-window derivative preserved')
  call assert_true(model%begin_calls == 1 .and. model%finish_calls == 1, 'directional lifecycle once')
  call fmr_discard_candidate(executor, candidate, diagnostics)

  call fmr_trial_from_checkpoint(executor, parameters, committed, forcing, config, 0.0_real64, 1.0_real64, &
       checkpoint, result, candidate, diagnostics)
  call assert_true(result%completed .and. candidate%ready(), 'legacy FMR trial completed')
  call assert_true(.not. result%directional_response%requested, 'legacy path remains non-directional')
  call assert_true(model%begin_calls == 1 .and. model%finish_calls == 1, 'legacy path does not invoke lifecycle')
  call fmr_discard_candidate(executor, candidate, diagnostics)

  print '(A)', 'FKT22P_FMR_DIRECTIONAL_FORWARDING PASS'
  print '(A)', 'FKT22P_REQUEST_FORWARDING=PASS'
  print '(A)', 'FKT22P_WHOLE_WINDOW_RESULT=PASS'
  print '(A)', 'FKT22P_LEGACY_PATH_IDENTITY=PASS'
  print '(A)', 'FKT22P_TRIAL_ONLY_NO_PUBLICATION=PASS'
...
end program test_fkt22p_fmr_directional_forwarding
