...
[[ "$changed_src" == "$EXPECTED_SRC" ]] || { echo 'FMR28_UNEXPECTED_PRODUCTION_DELTA' >&2; printf '%s\n' "$changed_src" >&2; exit 1; }
echo 'FMR28_PRODUCTION_DELTA_SINGLE_COMPOSITION=PASS'

check_blob() { local path="$1" expected="$2" actual; actual="$(git hash-object "$path")"; [[ "$actual" == "$expected" ]] || { echo "FMR28_PROTECTED_BLOB_MISMATCH $path expected=$expected actual=$actual" >&2; exit 1; }; }
...
for s in ['fmr_bind_reference_et_ptra_to_root_input','fmr_evaluate_shared_crop_root_uptake','root_water_uptake_flux_result_t','kernel_committed_state_t']: assert s in p,s
for s in ['evaluate_macro_feddes_drought_uptake','reference_et_mm_per_day','vegetation_cover_fraction','crop_factor','co2_transpiration_factor','pressure_head','headcalc','newton','jacobian','mass_accounting','total_in','total_out','open(','read(','write(','save']: assert s not in p,s
assert '0.1_real64' not in p and '0.1d0' not in p
...
for opt in 0 2; do
  ...
  for src in "${DEPENDENCY_SRC[@]}"; do obj="$OUT/$(basename "${src%.*}").o"; gfortran "${COMMON[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c "$src" -o "$obj"; objects+=("$obj"); done
  src=src/runtime/mod_fmr_reference_et_root_uptake_composition.f90; obj="$OUT/mod_fmr_reference_et_root_uptake_composition.o"
  gfortran "${STRICT[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c "$src" -o "$obj"; objects+=("$obj"); echo "FMR28_NEW_SOURCE_STRICT_WARNINGS_O${opt}=PASS"
  ...
  for marker in 'FMR28_EXPLICIT_FCI29_FMR12_FMR10_CHAIN_BITWISE_IDENTITY=PASS' 'FMR28_STALE_INCOMING_PTRA_CANNOT_AFFECT_ROOT_UPTAKE=PASS' 'FMR28_UPSTREAM_ET_REJECTION_BLOCKS_ROOT_EXECUTION=PASS' 'FMR28_INVALID_ROOT_GEOMETRY_BLOCKS_ROOT_EXECUTION=PASS' 'FMR28_INACTIVE_CROP_DEPENDENCY_FREE_ZERO_ROUTE=PASS' 'FMR28_COMMITTED_STATE_READ_ONLY=PASS' 'FMR28_STATELESS_A_B_A_IDENTITY=PASS' 'FMR28_REFERENCE_ET_ROOT_UPTAKE_EXECUTION_COMPOSITION_TEST PASS'; do grep -Fq "$marker" "$OUT/output.txt"; done
