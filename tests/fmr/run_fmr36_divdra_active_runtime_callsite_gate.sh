...
BUILD="${TMPDIR:-/tmp}/swap5-fmr36-$$"
EVIDENCE_DIR="${FMR36_EVIDENCE_DIR:-}"
...
fail() { echo "FMR36_GATE_FAIL $*" >&2; exit 1; }
check_blob() {
...
git merge-base --is-ancestor "$BASE" HEAD || fail 'HEAD not descended from F-CI35 canonical base'
...
import re
helper=Path('src/runtime/mod_fmr_divdra_serialized_composition.f90').read_text().lower()
runtime=Path('src/runtime/mod_fmr_divdra_serialized_runtime.f90').read_text().lower()
combined=helper+'\n'+runtime
...
assert not re.search(r'%mass%[a-z0-9_]+\s*=',combined)
assert 'forcing_registry =' not in runtime
assert 'forcing_use_count(forcing_index) /= 1' in helper
...
print('FMR36_STATIC_SCOPE_AND_ORDERING=PASS')
PY

echo 'FMR36_PRODUCTION_CANDIDATE_BYTE_LOCK=PASS'
echo 'FMR36_GENERIC_RUNTIME_BYTE_IDENTICAL=PASS'
echo 'FMR36_REFERENCE_SOURCE_UNCHANGED=PASS'
...
for opt in 0 2; do
  ...
  objects+=("$obj")
  gfortran "${COMMON[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c tests/fmr/test_fmr36_divdra_active_runtime_callsite.f90 -o "$OUT/test.o"
...
echo "FMR36_O0_O2_OUTPUT_SHA256=$HASH"
echo 'FMR36_O0_O2_OUTPUT_IDENTITY=PASS'
