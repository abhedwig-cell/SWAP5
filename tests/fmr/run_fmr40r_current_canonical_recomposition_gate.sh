...
BUILD="${RUNNER_TEMP:-/tmp}/fmr40r-${GITHUB_RUN_ID:-local}"
...
trap cleanup EXIT
fail(){ echo "FMR40R_GATE_FAIL $*" >&2; exit 40; }
...
echo "FMR40R_CURRENT_CANONICAL_RACE_GUARD=PASS:$CURRENT"
...
[[ "$(git rev-parse HEAD:src/runtime/mod_fmr_restart_state_contract.f90)" == "$FMR39_RESTART" ]] || fail 'F-MR39 restart blob drift'
[[ "$(git rev-parse ${FMR39}:src/runtime/mod_fmr_serialized_reference_backend.f90)" == "$FMR39_BACKEND" ]] || fail 'F-MR39 authority backend drift'
[[ "$(git rev-parse ${FMR39}:src/runtime/mod_fmr_restart_state_contract.f90)" == "$FMR39_RESTART" ]] || fail 'F-MR39 authority restart drift'
echo 'FMR40R_EXACT_FMR39_SOURCE_RECOMPOSITION=PASS'
echo 'FMR40R_BOUNDED_PRODUCTION_DELTA=PASS'

[[ "$(git rev-parse ${BASE}:$FCI44_MODULE)" == "$FCI44_MODULE_BLOB" ]] || fail 'F-CI44 module drift in base'
[[ "$(git rev-parse HEAD:$FCI44_MODULE)" == "$FCI44_MODULE_BLOB" ]] || fail 'F-CI44 module changed by recomposition'
[[ "$(git rev-parse ${BASE}:$FCI44P_STATUS)" == "$FCI44P_STATUS_BLOB" ]] || fail 'F-CI44P status drift in base'
[[ "$(git rev-parse HEAD:$FCI44P_STATUS)" == "$FCI44P_STATUS_BLOB" ]] || fail 'F-CI44P status changed by recomposition'
echo 'FMR40R_FCI44P_POSTIMAGE_AUTHORITY_PRESERVED=PASS'

# Rebind only the historical F-MR39 canonical pin. The test and all scientific/runtime acceptance markers remain exact.
...
git show "${FMR39}:tests/fmr/run_fmr39_soil_temperature_runtime_gate.sh" | sed "s/^CANONICAL=.*/CANONICAL=$BASE/" > "$TEMP_GATE"
...
grep -Fq 'FMR39_RUNTIME_OUTPUT_SHA256=cb08b8dc528f9a1dfc11db9ffffad5598fa9c4584b12feada1d129e47a236942' "$BUILD/fmr39-rebound.txt" || fail 'F-MR39 runtime fingerprint drift'
echo 'FMR40R_FMR39_FULL_REBOUND_REPLAY=PASS'
echo 'FMR40R_FMR39_RUNTIME_FINGERPRINT_IDENTITY=PASS'

# Focused replay of the F-CI44 application-accuracy contract on the recomposed backend.
...
echo 'FMR40R_REFERENCE_AND_SCIENCE_PRESERVATION=PASS'
echo 'FMR40R_CURRENT_CANONICAL_RECOMPOSITION_GATE=PASS'
