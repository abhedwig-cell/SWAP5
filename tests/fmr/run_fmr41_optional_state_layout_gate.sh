...
BUILD="${RUNNER_TEMP:-/tmp}/fmr41-${GITHUB_RUN_ID:-local}"
rm -rf "$BUILD"; mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT

fail(){ echo "FMR41_GATE_FAIL $*" >&2; exit 41; }
need_commit(){ git cat-file -e "$1^{commit}" 2>/dev/null || git fetch --no-tags origin "$1" >/dev/null 2>&1 || fail "cannot fetch $1"; }
need_commit "$BASE"; need_commit "$FMR39"; need_commit "$FMR06"

# The workunit is based on one immutable current-canonical image.  A moving
# canonical is a recomposition event, not something this qualification guesses through.
...
echo 'FMR41_BOUNDED_THREE_FILE_PRODUCTION_DELTA=PASS'

git diff --check "$BASE" -- src/runtime tests/fmr integration/f-mr .github/workflows || fail 'diff check failed'
echo 'FMR41_DIFF_CHECK=PASS'

# Pin the two historical identities to the exact qualification oracles that used them.
...
echo 'FMR41_HISTORICAL_LAYOUT_IDENTITIES_PINNED=PASS'
...
print('FMR41_STATIC_TYPED_LAYOUT_CONTRACT=PASS')
PY
...
if src.count('390501_int64') != 1:
    raise SystemExit(f'FMR41 mutation anchor count={src.count("390501_int64")}')
Path(sys.argv[2]).write_text(src.replace('390501_int64','390502_int64',1))
print('FMR41_UNKNOWN_THERMAL_LAYOUT_ORACLE_MATERIALIZED=PASS')
PY
...
for opt in 0 2; do
  ...
  gfortran "${RFLAGS[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c tests/fmr/test_fmr41_optional_state_layout_contract.f90 -o "$OUT/fmr41.o"
  gfortran -O"$opt" "${objects[@]}" "$OUT/fmr41.o" -o "$OUT/fmr41"
  "$OUT/fmr41" > "$OUT/fmr41.txt" 2>&1 || { cat "$OUT/fmr41.txt" >&2; fail "contract matrix O$opt"; }
  grep -Fq 'FMR41_OPTIONAL_STATE_LAYOUT_CONTRACT_TEST PASS' "$OUT/fmr41.txt" || fail "contract marker O$opt"
  grep -Fq 'FMR41_UNKNOWN_POSITIVE_LAYOUT_FAIL_CLOSED=PASS' "$OUT/fmr41.txt" || fail "unknown restart marker O$opt"
  ...
  echo "FMR41_UNKNOWN_POSITIVE_THERMAL_LAYOUT_REJECTED_O${opt}=PASS"
  ...
  echo "FMR41_BASE_LAYOUT_FMR19_PRESERVATION_O${opt}=PASS"
done

cmp "$BUILD/o0/fmr41.txt" "$BUILD/o2/fmr41.txt" || fail 'F-MR41 contract O0/O2 drift'
...
echo "FMR41_FMR39_VALID_RUNTIME_OUTPUT_SHA256=$ACTUAL_SHA"
echo 'FMR41_VALID_THERMAL_AUTHORITY_PRESERVED=PASS'
echo 'FMR41_O0_O2_DETERMINISM=PASS'
echo 'FMR41_GATE=PASS'
