...
CANON=699021ab95df35d44e9711612d9ce97939f0025e
CANON_TREE=8b3afdf8c7c13a6ef198b8a41d34a36c714c5f12
PREVIOUS_CANON=df51575e18777856a47a5d0d1e2e1c7456be4601
FCI56_SECOND_PARENT=bbaa4f2b826b6da7bdca0dfb670b49813616a950
...
parents=( $(git rev-list --parents -n 1 "$CANON") )
[[ ${#parents[@]} -eq 3 ]] || fail "F-CI56 canonical admission is not a two-parent merge"
[[ "${parents[1]}" == "$PREVIOUS_CANON" ]] || fail "unexpected F-CI56 first parent"
[[ "${parents[2]}" == "$FCI56_SECOND_PARENT" ]] || fail "unexpected F-CI56 second parent"
echo 'FMR42_CURRENT_CANONICAL_TRUE_TWO_PARENT_ADMISSION=PASS'
...
assert auth['current_canonical_binding']['sha']=='699021ab95df35d44e9711612d9ce97939f0025e'
assert auth['current_canonical_binding']['tree']=='8b3afdf8c7c13a6ef198b8a41d34a36c714c5f12'
assert auth['current_canonical_binding']['broad_current_canonical_run']==34745820847
assert auth['current_canonical_binding']['current_restricted_canonical_preservation_job']==103693633972
...
assert f56['qualification_green'] is True
...
# Frozen Serialized-MultiSWAP dependency surface must remain byte-identical to
# the last qualified production composition that owns those blobs.
d10_surface=(
...
# F-CI56 must not have altered any pre-existing D10 dependency. It may add its
# bounded groundwater-only files, which are outside the frozen D10 denominator.
for path in "${d10_surface[@]}"; do
  git diff --quiet "$PREVIOUS_CANON" "$CANON" -- "$path" || fail "F-CI56 modified D10 dependency: $path"
done
echo 'FMR42_FCI56_NO_D10_DEPENDENCY_DELTA=PASS'

FCI="$(git show "$CANON:.github/workflows/fci-canonical.yml")"
grep -Fq "AUTH=$PRESERVE" <<<"$FCI" || fail "canonical moving-preservation authority mismatch"
for token in \
...
assert x['superseded_attempt']['result']=='FAIL_CLOSED_AS_DESIGNED'
assert x['superseded_attempt']['capability_failure'] is False
