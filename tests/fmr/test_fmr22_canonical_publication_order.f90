program test_fmr22_canonical_publication_order
  ...
  do i = 1, ncol
    call require(results(i)%column_id == base_columns(i)%column_id, 'canonical result id')
    call require(diagnostics(i)%column_id == base_columns(i)%column_id, 'canonical diagnostic id')
    call require(results(i)%dispatch_ordinal == i, 'canonical dispatch ordinal')
  end do

  write(*,'(A)') 'FMR22_CANONICAL_PUBLICATION_ORDER=PASS'
  write(*,'(A,I0)') 'FMR22_FIRST_PUBLISHED_COLUMN_ID=', results(1)%column_id
  write(*,'(A,I0)') 'FMR22_LAST_PUBLISHED_COLUMN_ID=', results(ncol)%column_id
...
end program test_fmr22_canonical_publication_order
