program test_fmr41_optional_state_layout_contract
  ...
  template%template_id = 410001_int64
  ...
  ! A typed layout is a template capability identity.  An inactive column may
  ! therefore carry no optional physical state while still using that template.
  ...
  print '(a)', 'FMR41_KNOWN_LAYOUT_IDENTITY_MATRIX=PASS'
  print '(a)', 'FMR41_INACTIVE_TYPED_CAPABILITY_SEMANTICS=PASS'
  print '(a)', 'FMR41_SNOW_LAYOUT_RESTART_IDENTITY=PASS'
  print '(a)', 'FMR41_UNKNOWN_POSITIVE_LAYOUT_FAIL_CLOSED=PASS'
  print '(a)', 'FMR41_OPTIONAL_STATE_LAYOUT_CONTRACT_TEST PASS'
...
contains
  ...
  subroutine expect(condition, label)
    ...
    if (.not. condition) then
      write(*,'(a,1x,a)') 'FMR41_ASSERT_FAIL', trim(label)
      error stop 41
    end if
  end subroutine expect

end program test_fmr41_optional_state_layout_contract
