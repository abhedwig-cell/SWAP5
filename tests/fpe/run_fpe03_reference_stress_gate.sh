...
check_blob() {
  ...
  [[ "$actual" == "$expected" ]] || {
    echo "FPE03_BLOB_MISMATCH $path expected=$expected actual=$actual" >&2
    exit 1
  }
}

# Exact current F-PE02 production postimage and its qualified physical dependencies.
...
echo 'FPE03_EXACT_SOURCE_PREIMAGE=PASS'
...
for opt in 0 2; do
  ...
  gfortran "${COMMON[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c tests/fpe/test_fpe03_reference_stress.f90 -o "$OUT/test_fpe03.o"
  gfortran -O"$opt" "${objects[@]}" "$OUT/test_fpe03.o" -o "$OUT/fpe03_reference_stress"
  "$OUT/fpe03_reference_stress" > "$OUT/output.txt" 2>&1 || { cat "$OUT/output.txt" >&2; exit 1; }
  ...
  echo "FPE03_O${opt}_EXECUTABLE_SHA256=$(sha256sum "$OUT/fpe03_reference_stress" | cut -d' ' -f1)"
  echo "FPE03_O${opt}_OUTPUT_SHA256=$(sha256sum "$OUT/output.txt" | cut -d' ' -f1)"
  echo "FPE03_O${opt}=PASS"
done

cat "$BUILD/o0/output.txt"
if cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt"; then
  echo 'FPE03_O0_O2_OUTPUT_IDENTITY=PASS'
else
  echo 'FPE03_O0_O2_OUTPUT_IDENTITY=FAIL_CHARACTERIZATION'
  diff -u "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" | head -120 || true
fi

echo 'FPE03_REFERENCE_STRESS_GATE=PASS_CHARACTERIZATION_ONLY'
