...
for mode in exact a2; do
  LIBMF6="$BUILD/modflow-bin/libmf6.so" FGC44_SWAP_LIB="$BUILD/$mode/libfgc44_swap.so"     python3 "$BUILD/py/test_a2_e2e.py" > "$BUILD/$mode.txt"
...
print(f"APPROX02_A2_E2E|EXACT_HEAD={e['head']:.17e}|A2_HEAD={a['head']:.17e}|HEAD_DIFF={a['head']-e['head']:.17e}")
print(f"APPROX02_A2_E2E|EXACT_QSWAP={e['qswap']:.17e}|A2_QSWAP={a['qswap']:.17e}|QSWAP_DIFF={a['qswap']-e['qswap']:.17e}")
print(f"APPROX02_A2_E2E|EXACT_LEDGER={e['ledger']:.17e}|A2_LEDGER={a['ledger']:.17e}|LEDGER_DIFF={a['ledger']-e['ledger']:.17e}")
print(f"APPROX02_A2_E2E|EXACT_ITERS={e['iters']}|A2_ITERS={a['iters']}")
print(f"APPROX02_A2_E2E|EXACT_SECONDS={e['seconds']:.9f}|A2_SECONDS={a['seconds']:.9f}|RATIO={a['seconds']/e['seconds']:.9f}|SPEEDUP_PERCENT={(1-a['seconds']/e['seconds'])*100:.6f}")
print(f"APPROX02_A2_E2E|HEAD_REL={abs(a['head']-e['head'])/max(abs(e['head']),1e-30):.17e}|QSWAP_REL={abs(a['qswap']-e['qswap'])/max(abs(e['qswap']),1e-30):.17e}|LEDGER_REL={abs(a['ledger']-e['ledger'])/max(abs(e['ledger']),1e-30):.17e}")
if abs(e['residual'])>1e-15 or abs(a['residual'])>1e-15: raise SystemExit('coupled residual gate drift')
print('FPE_APPROX02_A2_MODFLOW_E2E=PASS')
PY
