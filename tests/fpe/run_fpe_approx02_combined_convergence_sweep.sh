...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-approx02-combined-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "APPROX02_COMBINED_SWEEP_FAIL $*" >&2; exit 1; }
...
echo 'mode,balance_mult,rep,ns,status,nonlinear,jacobian,linear,backtrack,bottom_flux,mass_residual,h1,h2,h3,h4,th1,th2,th3,th4' > "$CSV"

run_one(){
  local mode="$1" head_mult="$2" balance_mult="$3" rep="$4" raw line
  raw="$("$BUILD/test" O14 -10 -10 0 5e-2 "$head_mult" 400 "$balance_mult")"
  line="$(printf '%s\n' "$raw" | grep '^APPROX02_SOLVE|')"
  python3 - "$mode" "$balance_mult" "$rep" "$line" "$CSV" <<'PY'
import csv,sys
mode,balance,rep,line,path=sys.argv[1:]
d={}
...
with open(path,'a',newline='') as f:
    csv.writer(f).writerow([mode,balance,rep]+[d[k] for k in keys])
print(line)
PY
}
...
for rep in $(seq 1 "$REPS"); do
  run_one exact 1 1 "$rep"
  if (( rep % 2 == 1 )); then order=("${levels[@]}"); else order=(10000000000 100000000 1000000 10000 1000 100 10 1); fi
  for mult in "${order[@]}"; do run_one relaxed 10000000000 "$mult" "$rep"; done
done

python3 - "$CSV" <<'PY'
import csv,statistics,sys
rows=list(csv.DictReader(open(sys.argv[1])))
exact=[r for r in rows if r['mode']=='exact']
levels=(1,10,100,1000,10000,1000000,100000000,10000000000)
by={t:[r for r in rows if r['mode']=='relaxed' and int(float(r['balance_mult']))==t] for t in levels}
if len(exact)!=12 or any(len(v)!=12 for v in by.values()): raise SystemExit('missing combined sweep rows')
if any(int(r['status'])!=1 for r in exact): raise SystemExit('exact reference nonconverged')
ref=exact[0]
ref_heads=[float(ref[f'h{i}']) for i in range(1,5)]
ref_theta=[float(ref[f'th{i}']) for i in range(1,5)]
ref_flux=float(ref['bottom_flux']); ref_mass=float(ref['mass_residual'])
ref_ns=statistics.median(float(r['ns']) for r in exact)
for t in levels:
    rr=by[t]
    statuses=sorted({int(r['status']) for r in rr})
    if statuses != [1]:
      print(f"APPROX02_COMBINED_RESULT|BALANCE_MULT={t}|STATUS={statuses}|NONCONVERGED=1")
      continue
    r0=rr[0]
    ...
    med=statistics.median(float(r['ns']) for r in rr); ratio=med/ref_ns
    print(
      f"APPROX02_COMBINED_RESULT|HEAD_MULT=10000000000|BALANCE_MULT={t}|MEDIAN_NS={med:.17e}"
      f"|RUNTIME_RATIO={ratio:.9f}|SPEEDUP_PERCENT={(1-ratio)*100:.6f}"
      f"|NONLINEAR={sorted({int(r['nonlinear']) for r in rr})}|BACKTRACK={sorted({int(r['backtrack']) for r in rr})}"
...
print('FPE_APPROX02_COMBINED_SWEEP=PASS')
PY
