...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-approx02-tol-sweep-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "APPROX02_TOLERANCE_SWEEP_FAIL $*" >&2; exit 1; }
...
for t in levels:
    ...
    flux=float(r0['bottom_flux'])
    mass=float(r0['mass_residual'])
...
print('FPE_APPROX02_TOLERANCE_SWEEP=PASS')
PY
