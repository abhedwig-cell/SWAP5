...
PY
        ...
        printf 'MATERIAL=%s|REGIME=%s|ORIENTATION=%s|ARM=%s|MEDIAN_SECONDS=%s|%s\n'           "$material" "$regime" "$orientation" "$arm" "$median" "${last#APPROX03_T2A_MATRIX_RAW|}" >> "$summary"
      done
    done
  done
done
...
for material in ("B01","B12","O05","O14"):
  for regime in ("wet","mid","dry"):
    for orientation in ("plus","minus"):
      ...
      cand=rows[(material,regime,orientation,"T2A")]
      ...
      print(
        f"APPROX03_T2A_CASE|MATERIAL={material}|REGIME={regime}|ORIENTATION={orientation}"
        f"|SPEEDUP_PERCENT={speed:.6f}|REF_SUBSTEPS={ii(ref,'SUBSTEPS')}|T2A_SUBSTEPS={ii(cand,'SUBSTEPS')}"
        f"|REF_NONLINEAR={ii(ref,'NONLINEAR')}|T2A_NONLINEAR={ii(cand,'NONLINEAR')}"
        f"|REF_HEADCALC={ii(ref,'HEADCALC')}|T2A_HEADCALC={ii(cand,'HEADCALC')}"
...
print(
  f"APPROX03_T2A_SUMMARY|CASES={len(speedups)}|WORK_REDUCED={work_reduced}"
...
print("FPE_APPROX03_T2A_MATRIX=PASS")
PY
