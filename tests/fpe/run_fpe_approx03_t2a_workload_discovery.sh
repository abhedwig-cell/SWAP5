...
for factor in "${factors[@]}"; do
  for dt in "${dts[@]}"; do
    set +e
    raw="$("$BUILD/test" "$material" "$h0" "$factor" "$factor" "$dt" "$budget" 2>&1)"
    rc=$?
    set -e
    printf '%s\n' "$raw"
    if [[ $rc -eq 0 ]]; then
...
echo "APPROX03_T2A_WORKLOAD_DISCOVERY_SUMMARY|ROWS=$rows|REFINED=$refined"
[[ $refined -ge 1 ]] || fail "B01 wet has no stable multi-substep reference workload"
echo "FPE_APPROX03_T2A_WORKLOAD_DISCOVERY=PASS"
