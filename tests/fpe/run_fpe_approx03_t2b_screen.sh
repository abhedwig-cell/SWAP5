...
insert="""\n  subroutine material_parameters(name,tr,ts,alpha,nvg,ksat,lambda)
  ...
  subroutine run_t2_point()
    ...
    write(*,'(*(g0))') 'APPROX03_T2A_MATRIX_RAW|MATERIAL=',trim(t2_material),'|H0=',h0, &
...
qbot_factor=5e-5
dt=1e-4
budgets=(1e-5 1.25e-5 1.5e-5 1.75e-5 2e-5)
mults=(1.00 1.25 1.50 1.75 2.00)
reps=9
summary="$BUILD/t2b.txt"
: > "$summary"

for i in "${!budgets[@]}"; do
  budget="${budgets[$i]}"
  mult="${mults[$i]}"
  times="$BUILD/times-${mult}.txt"
  : > "$times"
  committed_count=0
  last=""
  for ((rep=1; rep<=reps; rep++)); do
    ...
    rc=$?
    set -e
    line="$(printf '%s\n' "$raw" | grep '^APPROX03_T2A_MATRIX_RAW|' | tail -1 || true)"
    if [[ $rc -ne 0 || -z "$line" ]]; then
      printf 'APPROX03_T2B_RAW|MULT=%s|REP=%s|EXECUTION_FAILED=T\n' "$mult" "$rep"
      continue
    fi
    committed="$(printf '%s\n' "$line" | sed -n 's/.*|COMMITTED=\([^|]*\).*/\1/p')"
    mass_complete="$(printf '%s\n' "$line" | sed -n 's/.*|MASS_COMPLETE=\([^|]*\).*/\1/p')"
    printf 'APPROX03_T2B_RAW|MULT=%s|REP=%s|%s\n' "$mult" "$rep" "${line#APPROX03_T2A_MATRIX_RAW|}"
...
PY
    ...
    printf 'MULT=%s|BUDGET=%s|COMMITTED_COUNT=%s|MEDIAN_SECONDS=%s|%s\n'       "$mult" "$budget" "$committed_count" "$median" "${last#APPROX03_T2A_MATRIX_RAW|}" >> "$summary"
  else
    printf 'MULT=%s|BUDGET=%s|COMMITTED_COUNT=0\n' "$mult" "$budget" >> "$summary"
  fi
done
...
for line in open(sys.argv[1]):
    ...
    rows[float(d["MULT"])]=d
ref=rows[1.0]
if int(ref["COMMITTED_COUNT"]) != 9:
    raise SystemExit("reference did not commit in all replicas")
...
for mult in (1.0,1.25,1.5,1.75,2.0):
    r=rows[mult]
    n=int(r["COMMITTED_COUNT"])
    if n==0:
        print(f"APPROX03_T2B_POINT|MULT={mult:.2f}|COMMITTED_COUNT=0|ROBUST=F")
        continue
    rh=[f(ref,f"H{i}") for i in range(1,5)]
    ch=[f(r,f"H{i}") for i in range(1,5)]
    rt=[f(ref,f"TH{i}") for i in range(1,5)]
    ct=[f(r,f"TH{i}") for i in range(1,5)]
    speed=100.0*(1.0-f(r,"MEDIAN_SECONDS")/f(ref,"MEDIAN_SECONDS"))
    print(
      f"APPROX03_T2B_POINT|MULT={mult:.2f}|COMMITTED_COUNT={n}|ROBUST={'T' if n==9 else 'F'}"
      f"|SPEEDUP_PERCENT={speed:.6f}|SUBSTEPS={ii(r,'SUBSTEPS')}|RETRIES={ii(r,'RETRIES')}"
      f"|NONLINEAR={ii(r,'NONLINEAR')}|HEADCALC={ii(r,'HEADCALC')}"
      f"|HEAD_REL={max(rel(a,b) for a,b in zip(rh,ch)):.17e}"
      f"|THETA_REL={max(rel(a,b) for a,b in zip(rt,ct)):.17e}"
...
print("FPE_APPROX03_T2B_SCREEN=PASS")
PY
