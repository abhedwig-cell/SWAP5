...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-baltol01-p0-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "BALTOL01_P0_FAIL $*" >&2; exit 1; }
...
print("BALTOL01_P0_RAW|"+json.dumps(d,separators=(",",":")))
PY
...
tols=(1e-12 2e-12 5e-12 1e-11 1e-10)
...
for line in open(sys.argv[1]):
    if not line.startswith("BALTOL01_P0_RAW|"): continue
    payload,reg=line.strip().split("|REGIME=")
    d=json.loads(payload.split("|",1)[1]); d["regime"]=reg
    groups[(d["material"],reg,d["imbalance"],d["offset_cm"],d["dt"])].append(d)
if len(groups)!=240: raise SystemExit(f"expected 240 physical groups, got {len(groups)}")
if sum(len(v) for v in groups.values())!=1200: raise SystemExit("expected 1200 tolerance points")

tol_summary=collections.defaultdict(lambda:{"pass":0,"total":0,"lost_strict":0,"max_dh":0.0,"max_dt":0.0,"max_df":0.0})
recoveries=collections.defaultdict(int)
strict_pass_set=set()
for key,rs in sorted(groups.items()):
    rs=sorted(rs,key=lambda r:r["tol"])
    bytol={r["tol"]:r for r in rs}
    strict=bytol[1e-12]
    if strict["floor_status"]==0 and strict["sample_valid"]==1: strict_pass_set.add(key)
    successes=[r for r in rs if r["floor_status"]==0 and r["sample_valid"]==1]
    if not successes:
        for r in rs:
            x=tol_summary[r["tol"]]; x["total"]+=1
        print(f"BALTOL01_P0_GROUP|MATERIAL={key[0]}|REGIME={key[1]}|IMBALANCE={key[2]:.3f}|OFFSET_CM={key[3]:.6f}|DT={key[4]:.8e}|STRICT_SUCCESS=FALSE|ANY_SUCCESS=FALSE")
        continue
    ref=successes[0]
    for r in rs:
        tol=r["tol"]; x=tol_summary[tol]; x["total"]+=1
        ok=r["floor_status"]==0 and r["sample_valid"]==1
        x["pass"]+=int(ok)
        if key in strict_pass_set and not ok: x["lost_strict"]+=1
        if ok:
            if r["mass_complete"]!=1: raise SystemExit(f"incomplete mass on success {key} tol={tol}")
        ...
        if tol>1e-12 and strict["floor_status"]!=0 and ok: recoveries[tol]+=1
    print(f"BALTOL01_P0_GROUP|MATERIAL={key[0]}|REGIME={key[1]}|IMBALANCE={key[2]:.3f}|OFFSET_CM={key[3]:.6f}|DT={key[4]:.8e}"
          f"|STRICT_SUCCESS={str(strict['floor_status']==0 and strict['sample_valid']==1).upper()}|STRICTEST_SUCCESS_TOL={ref['tol']:.1e}")

for tol in sorted(tol_summary):
    x=tol_summary[tol]
    print(f"BALTOL01_P0_TOLERANCE|TOL={tol:.1e}|PASS={x['pass']}|TOTAL={x['total']}|RECOVERED_STRICT_FAILURES={recoveries[tol]}"
          f"|LOST_STRICT_SUCCESSES={x['lost_strict']}|MAX_DH_VS_STRICTEST_SUCCESS={x['max_dh']:.17e}"
          f"|MAX_DTHETA_VS_STRICTEST_SUCCESS={x['max_dt']:.17e}|MAX_DFLUX_VS_STRICTEST_SUCCESS={x['max_df']:.17e}")
print("FPE_BALTOL01_P0=PASS")
PY
