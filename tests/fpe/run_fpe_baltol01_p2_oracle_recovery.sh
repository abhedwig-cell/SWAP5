...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-baltol01-p2-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "BALTOL01_P2_FAIL $*" >&2; exit 1; }
...
insert="""contains
  ...
  integer(c_int) function fgc44_temporal03_oracle_c(prescribed_head_m,nsub,heads,theta,bottom_exchange, &
    ...
    if(status/=GW_SWAP_FORCING_OK .or. .not.allocated(generic_forcing))then
      write(*,'(*(g0))') 'BALTOL01_P2_FAILPOINT|STAGE=MATERIALIZE|STATUS=',status
      return
    end if
    ...
    do i=1,int(nsub)
      ...
      if(.not.r%sample_valid .or. .not.r%mass%complete .or. .not.c%ready())then
        write(*,'(*(g0))') 'BALTOL01_P2_FAILPOINT|STAGE=FLOOR|I=',i,'|STATUS=',r%status, &
             '|VALID=',r%sample_valid,'|MASS=',r%mass%complete,'|READY=',c%ready()
        return
      end if
      if(.not.r%bottom_interface_exchange_available)then
        write(*,'(*(g0))') 'BALTOL01_P2_FAILPOINT|STAGE=EXCHANGE|I=',i
        return
      end if
      ...
      if(.not.did_commit .or. status/=0)then
        write(*,'(*(g0))') 'BALTOL01_P2_FAILPOINT|STAGE=COMMIT|I=',i,'|DID=',did_commit,'|STATUS=',status
        return
      end if
    end do
...
print("BALTOL01_P2_RAW|"+json.dumps({
  ...
  for imbalance in "${imbalances[@]}"; do
    for offset in "${offsets[@]}"; do
      for nsub in "${levels[@]}"; do
        ...
        printf '%s\n' "$raw" | grep '^BALTOL01_P2_FAILPOINT|' || true
        line="$(printf '%s\n' "$raw" | grep '^BALTOL01_P2_RAW|' | tail -1)"
...
for line in open(sys.argv[1]):
    if not line.startswith("BALTOL01_P2_RAW|"): continue
...
for key,lev in sorted(groups.items()):
    ...
    if any(lev[n]["status"]!=0 for n in (8,16,32)):
        print(f"BALTOL01_P2_POINT|MATERIAL={key[0]}|REGIME={key[1]}|IMBALANCE={key[2]:.3f}|OFFSET_CM={key[3]:.6f}|CONVERGED=FALSE")
        continue
    passed+=1
    def diff(a,b):
    ...
    monotone+=int(mono)
    print(f"BALTOL01_P2_POINT|MATERIAL={key[0]}|REGIME={key[1]}|IMBALANCE={key[2]:.3f}|OFFSET_CM={key[3]:.6f}"
...
print(f"BALTOL01_P2_SUMMARY|POINTS=32|ALL_LEVELS_CONVERGED={passed}|MONOTONE_ALL_METRICS={monotone}")
if passed!=32: raise SystemExit(f"oracle recovery incomplete: {passed}/32")
print("FPE_BALTOL01_P2=PASS")
PY
