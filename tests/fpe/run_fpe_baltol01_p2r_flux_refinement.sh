...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-baltol01-p2r-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "BALTOL01_P2R_FAIL $*" >&2; exit 1; }
...
insert="""contains
  ...
  integer(c_int) function fgc44_temporal03_oracle_c(prescribed_head_m,nsub,heads,theta,bottom_exchange, &
    ...
    if(status/=GW_SWAP_FORCING_OK .or. .not.allocated(generic_forcing))then
      write(*,'(*(g0))') 'BALTOL01_P2R_FAILPOINT|STAGE=MATERIALIZE|STATUS=',status
      return
    end if
    ...
    do i=1,int(nsub)
      ...
      if(.not.r%sample_valid .or. .not.r%mass%complete .or. .not.c%ready())then
        write(*,'(*(g0))') 'BALTOL01_P2R_FAILPOINT|STAGE=FLOOR|I=',i,'|STATUS=',r%status, &
             '|VALID=',r%sample_valid,'|MASS=',r%mass%complete,'|READY=',c%ready()
        return
      end if
      if(.not.r%bottom_interface_exchange_available)then
        write(*,'(*(g0))') 'BALTOL01_P2R_FAILPOINT|STAGE=EXCHANGE|I=',i
        return
      end if
      ...
      if(.not.did_commit .or. status/=0)then
        write(*,'(*(g0))') 'BALTOL01_P2R_FAILPOINT|STAGE=COMMIT|I=',i,'|DID=',did_commit,'|STATUS=',status
        return
      end if
    end do
...
print("BALTOL01_P2R_RAW|"+json.dumps({
...
material="B01"; regime="wet"; h0="-10"; imbalance="0.10"; offset="-0.01"
levels=(16 32 64)
for nsub in "${levels[@]}"; do
  ...
  line="$(printf '%s\n' "$raw" | grep '^BALTOL01_P2R_RAW|' | tail -1)"
  [[ -n "$line" ]] || fail "missing P2R record"
...
for line in open(sys.argv[1]):
    if not line.startswith("BALTOL01_P2R_RAW|"): continue
    payload,reg=line.strip().split("|REGIME=")
    d=json.loads(payload.split("|",1)[1]); d["regime"]=reg; lev[d["nsub"]]=d
if set(lev)!={16,32,64}: raise SystemExit(f"missing levels {set(lev)}")
if any(lev[n]["status"]!=0 for n in (16,32,64)): raise SystemExit("P2R level failed")
def diff(a,b):
    return (
      max(abs(x-y) for x,y in zip(a["heads"],b["heads"])),
      max(abs(x-y) for x,y in zip(a["theta"],b["theta"])),
      abs(a["terminal_flux_cm_per_day"]-b["terminal_flux_cm_per_day"]),
      abs(a["bottom_exchange_cm"]-b["bottom_exchange_cm"])
    )
d1632=diff(lev[16],lev[32]); d3264=diff(lev[32],lev[64])
mono=all(b<=a for a,b in zip(d1632,d3264))
print(f"BALTOL01_P2R_RESULT|DH_16_32={d1632[0]:.17e}|DH_32_64={d3264[0]:.17e}"
      f"|DTHETA_16_32={d1632[1]:.17e}|DTHETA_32_64={d3264[1]:.17e}"
      f"|DFLUX_16_32={d1632[2]:.17e}|DFLUX_32_64={d3264[2]:.17e}"
      f"|DEXCHANGE_16_32={d1632[3]:.17e}|DEXCHANGE_32_64={d3264[3]:.17e}"
      f"|MAX_STEP_MASS_RESIDUAL_64={lev[64]['max_abs_step_mass_residual']:.17e}|MONOTONE={str(mono).upper()}")
if not mono: raise SystemExit("P2R refinement still nonmonotone")
print("FPE_BALTOL01_P2R=PASS")
PY
