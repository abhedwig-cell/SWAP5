...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-dir01-repair02-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/base" "$BUILD/candidate" "$BUILD/src"
trap 'rm -rf "$BUILD"' EXIT

cp src/transaction/mod_transaction_reference.f90 "$BUILD/src/transaction_base.f90"

python3 - "$BUILD/src/transaction_candidate.f90" <<'PY'
from pathlib import Path
import sys
src=Path("src/transaction/mod_transaction_reference.f90").read_text()
old="    class(transaction_attempt_context_t), allocatable :: checkpoint_context\n    class(transaction_attempt_context_t), allocatable :: half_context\n"
new="    class(transaction_attempt_context_t), allocatable :: checkpoint_context\n"
if old not in src:
    raise SystemExit("REPAIR02 local-context seam missing")
src=src.replace(old,new,1)
old="        if (context_required) call model%capture_attempt_context(half_context)\n"
if old not in src:
    raise SystemExit("REPAIR02 half-context capture seam missing")
src=src.replace(old,"",1)
old="      if (context_required) call model%restore_attempt_context(half_context)\n      call move_alloc(half_state, committed)\n"
new="      call move_alloc(half_state, committed)\n"
if old not in src:
    raise SystemExit("REPAIR02 accepted restore seam missing")
src=src.replace(old,new,1)
...
compile_variant() {
  ...
  for source in "${SRC[@]}"; do
    if [[ "$source" == TRANSACTION_PLACEHOLDER ]]; then source="$transaction"; fi
...
compile_variant base "$BUILD/src/transaction_base.f90"
compile_variant candidate "$BUILD/src/transaction_candidate.f90"
...
print(f'DIR01_REPAIR02_MEAN_RATIO={statistics.mean(ratios):.9f}')
print(f'DIR01_REPAIR02_MEDIAN_RATIO={statistics.median(ratios):.9f}')
print(f'DIR01_REPAIR02_MEAN_SPEEDUP_PERCENT={(1-statistics.mean(ratios))*100:.6f}')
print(f'DIR01_REPAIR02_MEDIAN_SPEEDUP_PERCENT={(1-statistics.median(ratios))*100:.6f}')
print(f'DIR01_REPAIR02_MEAN_DELTA_NS={statistics.mean(deltas):.6f}')
print(f'DIR01_REPAIR02_MIN_RATIO={min(ratios):.9f}')
print(f'DIR01_REPAIR02_MAX_RATIO={max(ratios):.9f}')
print(f'DIR01_REPAIR02_PAIRS={len(ratios)}')
print('FPE_DIR01_REPAIR02_EXPERIMENT=PASS')
PY
