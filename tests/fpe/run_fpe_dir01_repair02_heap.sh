...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-dir01-repair02-heap-${GITHUB_RUN_ID:-local}-$$"
...
src=src.replace("    class(transaction_attempt_context_t), allocatable :: checkpoint_context\n    class(transaction_attempt_context_t), allocatable :: half_context\n",
                "    class(transaction_attempt_context_t), allocatable :: checkpoint_context\n",1)
src=src.replace("        if (context_required) call model%capture_attempt_context(half_context)\n","",1)
src=src.replace("      if (context_required) call model%restore_attempt_context(half_context)\n      call move_alloc(half_state, committed)\n",
                "      call move_alloc(half_state, committed)\n",1)
...
void dir01_heap_report(void){
  printf("DIR01_R2_HEAP|MALLOC=%llu|CALLOC=%llu|REALLOC=%llu|FREE=%llu|MALLOC_BYTES=%llu|CALLOC_BYTES=%llu|REALLOC_BYTES=%llu\n",nm,nc,nr,nf,bm,bc,br);
}
C
...
for variant in base candidate; do
  ...
  grep '^DIR01_R2_HEAP|' "$BUILD/$variant.out" | sed "s/^/DIR01_R2_${variant^^}_/"
done
...
def read(path):
    txt=open(path).read()
    m=re.search(r'^DIR01_R2_HEAP\|(.+)$',txt,re.M)
...
for k in ('MALLOC','CALLOC','REALLOC','FREE','MALLOC_BYTES','CALLOC_BYTES','REALLOC_BYTES'):
    delta=c[k]-b[k]
    print(f'DIR01_REPAIR02_HEAP_DELTA|FIELD={k}|TOTAL={delta}|PER_INTERVAL={delta/calls:.6f}')
print('FPE_DIR01_REPAIR02_HEAP=PASS')
PY
