...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-dir01-repair03-${GITHUB_RUN_ID:-local}-$$"
...
new=old+"    type(soil_water_accepted_step_direction_request_t) :: trajectory_request_workspace\n"
...
print(f'DIR01_REPAIR03_MEAN_RATIO={statistics.mean(ratios):.9f}')
print(f'DIR01_REPAIR03_MEDIAN_RATIO={statistics.median(ratios):.9f}')
print(f'DIR01_REPAIR03_MEAN_SPEEDUP_PERCENT={(1-statistics.mean(ratios))*100:.6f}')
print(f'DIR01_REPAIR03_MEDIAN_SPEEDUP_PERCENT={(1-statistics.median(ratios))*100:.6f}')
print(f'DIR01_REPAIR03_MEAN_DELTA_NS={statistics.mean(deltas):.6f}')
print(f'DIR01_REPAIR03_MIN_RATIO={min(ratios):.9f}')
print(f'DIR01_REPAIR03_MAX_RATIO={max(ratios):.9f}')
print('FPE_DIR01_REPAIR03_EXPERIMENT=PASS')
PY
