...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-dir01-repair04-heap-${GITHUB_RUN_ID:-local}-$$"
...
void dir01_heap_report(void){
 printf("DIR01_R4_HEAP|MALLOC=%llu|CALLOC=%llu|REALLOC=%llu|FREE=%llu|MALLOC_BYTES=%llu|CALLOC_BYTES=%llu|REALLOC_BYTES=%llu\n",nm,nc,nr,nf,bm,bc,br);
}
C
...
for variant in base candidate; do
  ...
  grep '^DIR01_R4_HEAP|' "$BUILD/$variant.out" | sed "s/^/DIR01_R3_${variant^^}_/"
...
def read(path):
    txt=open(path).read()
    m=re.search(r'^DIR01_R4_HEAP\|(.+)$',txt,re.M)
...
for k in ('MALLOC','CALLOC','REALLOC','FREE','MALLOC_BYTES','CALLOC_BYTES','REALLOC_BYTES'):
    delta=c[k]-b[k]
    print(f'DIR01_REPAIR04_HEAP_DELTA|FIELD={k}|TOTAL={delta}|PER_INTERVAL={delta/calls:.6f}')
if c['MALLOC'] >= b['MALLOC']:
    raise SystemExit('REPAIR04 did not reduce malloc count')
print('FPE_DIR01_REPAIR04_HEAP=PASS')
PY
