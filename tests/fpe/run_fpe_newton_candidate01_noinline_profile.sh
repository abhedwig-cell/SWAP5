...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-newton-candidate01-noinline-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
COMMON=(-std=f2008 -ffree-line-length-none -O2 -pg -fno-inline -fno-inline-functions -fno-inline-small-functions)
MODULE_SRC=(
...
gfortran -O2 -pg -fno-inline -fno-inline-functions -fno-inline-small-functions "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"

(
  cd "$BUILD"
  rm -f gmon.out
  ./test easy 30000 | tee easy.txt
  mv gmon.out easy.gmon
  gprof -b ./test easy.gmon > easy.gprof.txt
  rm -f gmon.out
  ./test hard 3000 | tee hard.txt
...
for label,path in [('easy',sys.argv[1]),('hard',sys.argv[2])]:
    ...
    print(f'NEWTON_CANDIDATE01_NOINLINE_GPROF_BEGIN|MODE={label}')
    for line in text.splitlines():
        low=line.lower()
        if any(n in low for n in names):
            print(line)
    print(f'NEWTON_CANDIDATE01_NOINLINE_GPROF_END|MODE={label}')
print('NEWTON_CANDIDATE01_NOINLINE_PROFILE=PASS')
PY
