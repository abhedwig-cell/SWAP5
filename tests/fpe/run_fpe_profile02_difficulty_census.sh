...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-profile02-difficulty-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"
fail() { echo "PUB_P2E04_CENSUS_GATE_FAIL $*" >&2; exit 1; }
TEST=tests/fpe/test_fpe_profile02_difficulty_census.f90
...
for opt in 2; do
  ...
  grep -Fq 'PROFILE02_DIFFICULTY_CENSUS=PASS' "$OUT/output.txt" || fail "O$opt PROFILE02 marker"
  cat "$OUT/output.txt"
  echo "PUB_P2E04_CENSUS_O${opt}=PASS"
done
echo 'PROFILE02_DIFFICULTY_CENSUS_GATE=PASS'
