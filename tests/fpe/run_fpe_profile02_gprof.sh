...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-profile02-gprof-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -J "$BUILD" -I "$BUILD" -c tests/fpe/test_fpe_profile02_scale_baseline.f90 -o "$BUILD/test.o"
...
(
  ...
  ./test 10000 1
  test -f gmon.out
  gprof ./test gmon.out > profile02-gprof.txt
  cat profile02-gprof.txt
)
echo 'PROFILE02_GPROF=PASS'
