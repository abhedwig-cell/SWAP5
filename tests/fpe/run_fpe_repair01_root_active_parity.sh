...
mkdir -p "$BUILD/repaired" "$BUILD/old" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPAIR01_ROOT_ACTIVE_FAIL $*" >&2; exit 1; }

cp tests/fgc/support/mod_fgc44_real_swap_c_bridge.f90 "$BUILD/repaired/mod_fgc44_real_swap_c_bridge.f90"
cp tests/fgc/support/mod_fgc44_real_swap_c_bridge.f90 "$BUILD/old/mod_fgc44_real_swap_c_bridge.f90"

for variant in repaired old; do
...
src=src.replace("f%drainage_flux_by_level=0.0_real64; f%subsurface_irrigation_source=0.0_real64; f%root_extraction_sink=0.0_real64",
                "f%drainage_flux_by_level=0.0_real64; f%subsurface_irrigation_source=0.0_real64; f%root_extraction_sink=2.5e-7_real64",1)
p.write_text(src)

PY
done

cp src/adapter/mod_reference_richards_legacy_binding.f90 "$BUILD/repaired/mod_reference_richards_legacy_binding.f90"
cp src/adapter/mod_reference_richards_legacy_binding.f90 "$BUILD/old/mod_reference_richards_legacy_binding.f90"
python3 - "$BUILD/old/mod_reference_richards_legacy_binding.f90" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]); src=p.read_text()
...
old="""    qrosum = sum(richards%provider_root_sink(1:n))
    qdrtot = sum(richards%sink(1:n))
"""
if new not in src: raise SystemExit("repaired qrosum seam missing")
p.write_text(src.replace(new,old,1))
PY

python3 - "$BUILD/py/fgc44_repair01_root_active_probe.py" <<'PY'
from pathlib import Path
import sys
...
if __name__=="__main__":
    ...
    print(f"REPAIR01_ROOT_ACTIVE|HCOF={hcof:.17e}|RHS={rhs:.17e}|HREF={href:.17e}|TRIAL_STATUS={status}|Q={q.value:.17e}|{fields}")
    raise SystemExit(0)
'''
...
compile_variant repaired
compile_variant old

export PYTHONPATH="$BUILD/py:$ROOT/src/adapter:$ROOT/tests/fgc/support"
cycles=20
for mode in repaired old; do
  : > "$BUILD/$mode.txt"
  pass=0
  for cycle in $(seq 1 "$cycles"); do
    line="$(FGC44_SWAP_LIB="$BUILD/$mode/libfgc44_swap.so" python3 "$BUILD/py/fgc44_repair01_root_active_probe.py" | grep '^REPAIR01_ROOT_ACTIVE|' | tail -1)"
    [[ -n "$line" ]] || fail "missing root-active record $mode $cycle"
    status="$(printf '%s\n' "$line" | sed -n 's/.*|TRIAL_STATUS=\([^|]*\).*/\1/p')"
    [[ "$status" == "0" ]] || fail "$mode root-active trial failed cycle $cycle"
    printf '%s\n' "$line" >> "$BUILD/$mode.txt"
    pass=$((pass+1))
  done
  echo "REPAIR01_ROOT_ACTIVE_SUMMARY|MODE=$mode|PASS=$pass|FAIL=0"
done

python3 - "$BUILD/repaired.txt" "$BUILD/old.txt" <<'PY'
import sys
def sig(path):
    rows=[x.strip() for x in open(path) if x.strip()]
    # Drop mode-independent HCOF/RHS/HREF? Keep everything because builds should be physically identical.
    return rows
r=sig(sys.argv[1]); o=sig(sys.argv[2])
if len(r)!=len(o): raise SystemExit("root-active count mismatch")
for i,(a,b) in enumerate(zip(r,o),1):
    if a!=b:
        raise SystemExit(f"root-active repaired/old mismatch at cycle {i}\n{a}\n{b}")
print("FPE_REPAIR01_ROOT_ACTIVE_PARITY=PASS")
PY
