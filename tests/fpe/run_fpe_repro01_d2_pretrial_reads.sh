...
fail(){ echo "REPRO01_D2_FAIL $*" >&2; exit 1; }
...
python3 - "$BUILD/py/fgc44_repro01_d2_probe.py" <<'PY'
from pathlib import Path
import sys
...
if __name__=="__main__":
    ...
    e1txt="NA"; statetxt="NA"
    if "E" in mode:
        e1=swap.e1_diagnostics()
        e1txt=",".join(f"{k}:{e1[k]}" for k in sorted(e1))
    if "S" in mode:
        st=swap.state()
        statetxt=",".join(str(v) for v in st)
    import ctypes
    q=ctypes.c_double()
    status=swap.lib.fgc44_swap_trial_c(float(href),ctypes.byref(q))
    d=swap.repro01_diag()
    fields="|".join(f"{k}={v}" for k,v in d.items())
    print(f"REPRO01_D2|MODE={mode}|HCOF={hcof:.17e}|RHS={rhs:.17e}|HREF={href:.17e}|E1={e1txt}|STATE={statetxt}|TRIAL_STATUS={status}|Q={q.value:.17e}|{fields}")
    raise SystemExit(0)
'''
...
cycles=40
for mode in N E S ES; do
  pass=0
  failcount=0
  for cycle in $(seq 1 "$cycles"); do
    line="$(FGC44_SWAP_LIB="$BUILD/exact/libfgc44_swap.so" python3 "$BUILD/py/fgc44_repro01_d2_probe.py" "$mode" | grep '^REPRO01_D2|' | tail -1)"
    [[ -n "$line" ]] || fail "missing D2 record mode $mode cycle $cycle"
    status="$(printf '%s\n' "$line" | sed -n 's/.*|TRIAL_STATUS=\([^|]*\).*/\1/p')"
    echo "REPRO01_D2_CYCLE|MODE=$mode|CYCLE=$cycle|${line#REPRO01_D2|}"
    if [[ "$status" == "0" ]]; then pass=$((pass+1)); else failcount=$((failcount+1)); fi
  done
  echo "REPRO01_D2_SUMMARY|MODE=$mode|CYCLES=$cycles|PASS=$pass|FAIL=$failcount"
done
echo "FPE_REPRO01_D2=OBSERVED"
