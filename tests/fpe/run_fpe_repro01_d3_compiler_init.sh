...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro01-d3-${GITHUB_RUN_ID:-local}-$"
CANDIDATE_TOL="${APPROX02_CANDIDATE_TOL:-1e-4}"
mkdir -p "$BUILD/default" "$BUILD/zero_init" "$BUILD/snan_check" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO01_D3_FAIL $*" >&2; exit 1; }

cp tests/fgc/support/mod_fgc44_real_swap_c_bridge.f90 "$BUILD/default/mod_fgc44_real_swap_c_bridge.f90"

python3 - "$BUILD/default/mod_fgc44_real_swap_c_bridge.f90" <<'PY'
...
PY
cp "$BUILD/default/mod_fgc44_real_swap_c_bridge.f90" "$BUILD/zero_init/mod_fgc44_real_swap_c_bridge.f90"
cp "$BUILD/default/mod_fgc44_real_swap_c_bridge.f90" "$BUILD/snan_check/mod_fgc44_real_swap_c_bridge.f90"

python3 - "$BUILD/py/fgc44_repro01_d3_probe.py" <<'PY'
from pathlib import Path
import sys
...
if __name__=="__main__":
    ...
    print(f"REPRO01_D3|HCOF={hcof:.17e}|RHS={rhs:.17e}|HREF={href:.17e}|TRIAL_STATUS={status}|Q={q.value:.17e}|{fields}")
    raise SystemExit(0)
'''
...
compile_variant(){
  ...
  for source in "${MODULE_SRC[@]}"; do
    ...
    gfortran "${COMMON[@]}" -O2 "${extra[@]}" -J "$out" -I "$out" -c "$source" -o "$obj" || fail "compile $name $source"
    objects+=("$obj")
  done
  gfortran -shared -fopenmp -O2 "${extra[@]}" "${objects[@]}" -o "$out/libfgc44_swap.so" || fail "link $name"
}
compile_variant default
compile_variant zero_init -finit-local-zero
compile_variant snan_check -finit-real=snan -finit-integer=-999999 -fcheck=all -ffpe-trap=invalid,zero,overflow

export PYTHONPATH="$BUILD/py:$ROOT/src/adapter:$ROOT/tests/fgc/support"
cycles=100
for mode in default zero_init snan_check; do
  pass=0
  failcount=0
  process_fail=0
  for cycle in $(seq 1 "$cycles"); do
    set +e
    raw="$(FGC44_SWAP_LIB="$BUILD/$mode/libfgc44_swap.so" python3 "$BUILD/py/fgc44_repro01_d3_probe.py" 2>&1)"
    rc=$?
    set -e
    line="$(printf '%s\n' "$raw" | grep '^REPRO01_D3|' | tail -1 || true)"
    if [[ $rc -ne 0 || -z "$line" ]]; then
      process_fail=$((process_fail+1))
      echo "REPRO01_D3_PROCESS_FAIL|MODE=$mode|CYCLE=$cycle|RC=$rc"
      printf '%s\n' "$raw" | tail -8
      continue
    fi
  ...
  echo "REPRO01_D3_SUMMARY|MODE=$mode|CYCLES=$cycles|PASS=$pass|TRIAL_FAIL=$failcount|PROCESS_FAIL=$process_fail"
done
echo "FPE_REPRO01_D3=OBSERVED"
