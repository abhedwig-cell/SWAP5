...
python3 - "$BUILD/py/fgc44_repro01_d4_probe.py" <<'PY'
from pathlib import Path
import sys
...
if __name__=="__main__":
    ...
    print(f"REPRO01_D4|HCOF={hcof:.17e}|RHS={rhs:.17e}|HREF={href:.17e}|TRIAL_STATUS={status}|Q={q.value:.17e}|{fields}")
    raise SystemExit(0)
'''
...
cycles=500
for mode in DEFAULT OMP1 OMP1_STATIC OMP1_BIND; do
  pass=0
  trial_fail=0
  process_fail=0
  first_failure=""
  for cycle in $(seq 1 "$cycles"); do
    case "$mode" in
      DEFAULT) env_args=() ;;
      OMP1) env_args=(OMP_NUM_THREADS=1) ;;
      OMP1_STATIC) env_args=(OMP_NUM_THREADS=1 OMP_DYNAMIC=FALSE) ;;
      OMP1_BIND) env_args=(OMP_NUM_THREADS=1 OMP_DYNAMIC=FALSE OMP_PROC_BIND=TRUE) ;;
    esac
    set +e
    raw="$(env "${env_args[@]}" FGC44_SWAP_LIB="$BUILD/exact/libfgc44_swap.so" python3 "$BUILD/py/fgc44_repro01_d4_probe.py" 2>&1)"
    rc=$?
    set -e
    line="$(printf '%s\n' "$raw" | grep '^REPRO01_D4|' | tail -1 || true)"
    if [[ $rc -ne 0 || -z "$line" ]]; then
      process_fail=$((process_fail+1))
      if [[ -z "$first_failure" ]]; then first_failure="PROCESS_RC_$rc"; fi
      continue
    fi
    ...
    if [[ "$status" == "0" ]]; then
      ...
      if [[ -z "$first_failure" ]]; then first_failure="$line"; fi
    fi
  done
  echo "REPRO01_D4_SUMMARY|MODE=$mode|CYCLES=$cycles|PASS=$pass|TRIAL_FAIL=$trial_fail|PROCESS_FAIL=$process_fail"
  if [[ -n "$first_failure" ]]; then echo "REPRO01_D4_FIRST_FAILURE|MODE=$mode|$first_failure"; fi
done
echo "FPE_REPRO01_D4=OBSERVED"
