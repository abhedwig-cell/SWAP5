...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro01-d6-${GITHUB_RUN_ID:-local}-$"
CANDIDATE_TOL="${APPROX02_CANDIDATE_TOL:-1e-4}"
mkdir -p "$BUILD/clean" "$BUILD/poison" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO01_D6_FAIL $*" >&2; exit 1; }

cp tests/fgc/support/mod_fgc44_real_swap_c_bridge.f90 "$BUILD/clean/mod_fgc44_real_swap_c_bridge.f90"

python3 - "$BUILD/clean/mod_fgc44_real_swap_c_bridge.f90" <<'PY'
...
PY
cp "$BUILD/clean/mod_fgc44_real_swap_c_bridge.f90" "$BUILD/poison/mod_fgc44_real_swap_c_bridge.f90"
cp src/runtime/mod_fmr_serialized_reference_backend.f90 "$BUILD/clean/mod_fmr_serialized_reference_backend.f90"
cp src/runtime/mod_fmr_serialized_reference_backend.f90 "$BUILD/poison/mod_fmr_serialized_reference_backend.f90"

python3 - "$BUILD/poison/mod_fmr_serialized_reference_backend.f90" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]); src=p.read_text()
needle="  use mod_reference_richards_legacy_binding, only: reference_richards_legacy_solver_t, &\n       reference_richards_legacy_workspace_t\n"
rep=needle+"  use mod_reference_richards_workspace, only: ensure_reference_workspace_shape, poison_reference_workspace\n"
...
rep="""        if (request%boundary%bottom_mode == 5) then
          ...
          call poison_reference_workspace(self%workspace%richards)
        end if
...
python3 - "$BUILD/py/fgc44_repro01_d6_probe.py" <<'PY'
from pathlib import Path
import sys
...
if __name__=="__main__":
    ...
    print(f"REPRO01_D6|HCOF={hcof:.17e}|RHS={rhs:.17e}|HREF={href:.17e}|TRIAL_STATUS={status}|Q={q.value:.17e}|{fields}")
    raise SystemExit(0)
'''
...
for mode in clean poison; do
  ...
  for cycle in $(seq 1 "$cycles"); do
    line="$(FGC44_SWAP_LIB="$BUILD/$mode/libfgc44_swap.so" python3 "$BUILD/py/fgc44_repro01_d6_probe.py" | grep '^REPRO01_D6|' | tail -1)"
    [[ -n "$line" ]] || fail "missing D6 record mode $mode cycle $cycle"
    status="$(printf '%s\n' "$line" | sed -n 's/.*|TRIAL_STATUS=\([^|]*\).*/\1/p')"
    if [[ "$status" == "0" ]]; then
      pass=$((pass+1))
      current="$(printf '%s\n' "$line" | sed 's/|HCOF=.*|TRIAL_STATUS=/|TRIAL_STATUS=/')"
      if [[ -z "$sig" ]]; then sig="$current"; elif [[ "$current" != "$sig" ]]; then mismatch=$((mismatch+1)); fi
    else
      failcount=$((failcount+1))
      if [[ -z "$first" ]]; then first="$line"; fi
    fi
  done
  echo "REPRO01_D6_SUMMARY|MODE=$mode|CYCLES=$cycles|PASS=$pass|FAIL=$failcount|SUCCESS_SIGNATURE_MISMATCH=$mismatch"
  if [[ -n "$first" ]]; then echo "REPRO01_D6_FIRST_FAILURE|MODE=$mode|$first"; fi
done
echo "FPE_REPRO01_D6=OBSERVED"
