...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro01-d7-${GITHUB_RUN_ID:-local}-$"
CANDIDATE_TOL="${APPROX02_CANDIDATE_TOL:-1e-4}"
mkdir -p "$BUILD/test" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO01_D7_FAIL $*" >&2; exit 1; }

cp tests/fgc/support/mod_fgc44_real_swap_c_bridge.f90 "$BUILD/test/mod_fgc44_real_swap_c_bridge.f90"

python3 - "$BUILD/test/mod_fgc44_real_swap_c_bridge.f90" <<'PY'
...
PY
cp src/runtime/mod_fmr_serialized_reference_backend.f90 "$BUILD/test/mod_fmr_serialized_reference_backend.f90"
python3 - "$BUILD/test/mod_fmr_serialized_reference_backend.f90" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]); src=p.read_text()
src=src.replace(
...
rep="""        if (request%boundary%bottom_mode == 5) then
          repro01_group = 'ZERO'
          call get_environment_variable('REPRO01_D7_GROUP', repro01_group, status=repro01_env_status)
          ...
          if (repro01_env_status == 0) then
            ...
            case ('DFDH')
              self%workspace%richards%dfdh_lower = repro01_qnan
              self%workspace%richards%dfdh_main = repro01_qnan
              self%workspace%richards%dfdh_upper = repro01_qnan
            case ('RESIDUAL_DELTA')
              self%workspace%richards%residual = repro01_qnan
              self%workspace%richards%delta_head = repro01_qnan
            case ('SOURCE_SINK')
              self%workspace%richards%source = repro01_qnan
              self%workspace%richards%sink = repro01_qnan
            case ('PROVIDER')
            ...
            case ('DCON_OLD')
              self%workspace%richards%dconductivity_dhead = repro01_qnan
              self%workspace%richards%old_head = repro01_qnan
            case ('FLUX_GRAD')
              self%workspace%richards%vertical_flux = repro01_qnan
              self%workspace%richards%head_gradient = repro01_qnan
            case ('BAND')
              self%workspace%richards%band_matrix = repro01_qnan
              self%workspace%richards%band_aux = repro01_qnan
              self%workspace%richards%band_rhs = repro01_qnan
            case default
              continue
            end select
          end if
...
python3 - "$BUILD/py/fgc44_repro01_d7_probe.py" <<'PY'
from pathlib import Path
import sys
...
if __name__=="__main__":
    ...
    print(f"REPRO01_D7|HCOF={hcof:.17e}|RHS={rhs:.17e}|HREF={href:.17e}|TRIAL_STATUS={status}|Q={q.value:.17e}|{fields}")
    raise SystemExit(0)
'''
...
compile_variant test

export PYTHONPATH="$BUILD/py:$ROOT/src/adapter:$ROOT/tests/fgc/support"
cycles=40
groups=(ZERO DFDH RESIDUAL_DELTA SOURCE_SINK PROVIDER DCON_OLD FLUX_GRAD BAND)
for group in "${groups[@]}"; do
  pass=0
  failcount=0
  first=""
  for cycle in $(seq 1 "$cycles"); do
    line="$(REPRO01_D7_GROUP="$group" FGC44_SWAP_LIB="$BUILD/test/libfgc44_swap.so" python3 "$BUILD/py/fgc44_repro01_d7_probe.py" | grep '^REPRO01_D7|' | tail -1)"
    [[ -n "$line" ]] || fail "missing D7 record group $group cycle $cycle"
  ...
  echo "REPRO01_D7_SUMMARY|GROUP=$group|CYCLES=$cycles|PASS=$pass|FAIL=$failcount"
  if [[ -n "$first" ]]; then echo "REPRO01_D7_FIRST_FAILURE|GROUP=$group|$first"; fi
done
echo "FPE_REPRO01_D7=OBSERVED"
