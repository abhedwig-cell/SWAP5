...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro01-d8-${GITHUB_RUN_ID:-local}-$"
CANDIDATE_TOL="${APPROX02_CANDIDATE_TOL:-1e-4}"
mkdir -p "$BUILD/test" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO01_D8_FAIL $*" >&2; exit 1; }
...
rep="""        if (request%boundary%bottom_mode == 5) then
          repro01_group = 'ZERO'
          call get_environment_variable('REPRO01_D8_GROUP', repro01_group, status=repro01_env_status)
...
python3 - "$BUILD/py/fgc44_repro01_d8_probe.py" <<'PY'
from pathlib import Path
import sys
...
if __name__=="__main__":
    ...
    print(f"REPRO01_D8|HCOF={hcof:.17e}|RHS={rhs:.17e}|HREF={href:.17e}|TRIAL_STATUS={status}|Q={q.value:.17e}|{fields}")
    raise SystemExit(0)
'''
...
cycles=40
groups=(ZERO THETA K CAPACITY DKDH ROOT_SINK)
for group in "${groups[@]}"; do
  pass=0
  failcount=0
  first=""
  for cycle in $(seq 1 "$cycles"); do
    line="$(REPRO01_D8_GROUP="$group" FGC44_SWAP_LIB="$BUILD/test/libfgc44_swap.so" python3 "$BUILD/py/fgc44_repro01_d8_probe.py" | grep '^REPRO01_D8|' | tail -1)"
  ...
  echo "REPRO01_D8_SUMMARY|GROUP=$group|CYCLES=$cycles|PASS=$pass|FAIL=$failcount"
  if [[ -n "$first" ]]; then echo "REPRO01_D8_FIRST_FAILURE|GROUP=$group|$first"; fi
done
echo "FPE_REPRO01_D8=OBSERVED"
