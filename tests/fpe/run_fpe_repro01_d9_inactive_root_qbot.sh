...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro01-d9-${GITHUB_RUN_ID:-local}-$"
CANDIDATE_TOL="${APPROX02_CANDIDATE_TOL:-1e-4}"
mkdir -p "$BUILD/clean" "$BUILD/original" "$BUILD/fix" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO01_D9_FAIL $*" >&2; exit 1; }

cp tests/fgc/support/mod_fgc44_real_swap_c_bridge.f90 "$BUILD/clean/mod_fgc44_real_swap_c_bridge.f90"
cp tests/fgc/support/mod_fgc44_real_swap_c_bridge.f90 "$BUILD/original/mod_fgc44_real_swap_c_bridge.f90"
cp tests/fgc/support/mod_fgc44_real_swap_c_bridge.f90 "$BUILD/fix/mod_fgc44_real_swap_c_bridge.f90"

for variant in clean original fix; do
python3 - "$BUILD/$variant/mod_fgc44_real_swap_c_bridge.f90" <<'PY'
...
for variant in clean original fix; do
  cp src/runtime/mod_fmr_serialized_reference_backend.f90 "$BUILD/$variant/mod_fmr_serialized_reference_backend.f90"
done
for variant in original fix; do
python3 - "$BUILD/$variant/mod_fmr_serialized_reference_backend.f90" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]); src=p.read_text()
src=src.replace(
...
needle="  use mod_reference_richards_legacy_binding, only: reference_richards_legacy_solver_t, &\n       reference_richards_legacy_workspace_t\n"
rep=needle+"  use mod_reference_richards_workspace, only: ensure_reference_workspace_shape, reset_reference_workspace\n"
if needle not in src: raise SystemExit("workspace import seam missing")
src=src.replace(needle,rep,1)
needle="    character(len=64) :: drainage_direction_route\n"
rep=needle+"    character(len=32) :: repro01_group\n    integer :: repro01_env_status\n    real(real64) :: repro01_qnan\n"
if needle not in src: raise SystemExit("advance declaration seam missing")
src=src.replace(needle,rep,1)
needle="""        call solve_with_accepted_step_direction(self%solver, request, self%workspace, self%trajectory_request_workspace, &
             solve_result, direction_result)
"""
rep="""        if (request%boundary%bottom_mode == 5) then
          repro01_group = 'ZERO'
          call get_environment_variable('REPRO01_D9_GROUP', repro01_group, status=repro01_env_status)
          call ensure_reference_workspace_shape(self%workspace%richards, self%soil_parameters%active_nodes)
          call reset_reference_workspace(self%workspace%richards)
          repro01_qnan = ieee_value(0.0_real64, ieee_quiet_nan)
          if (repro01_env_status == 0) then
            select case (trim(repro01_group))
            case ('THETA')
              self%workspace%richards%provider_theta = repro01_qnan
            case ('K')
              self%workspace%richards%provider_k = repro01_qnan
            case ('CAPACITY')
              self%workspace%richards%provider_capacity = repro01_qnan
            case ('DKDH')
              self%workspace%richards%provider_dkdh = repro01_qnan
            case ('ROOT_SINK')
              self%workspace%richards%provider_root_sink = repro01_qnan
            case default
              continue
            end select
          end if
        ...
        call solve_with_accepted_step_direction(self%solver, request, self%workspace, self%trajectory_request_workspace, &
             solve_result, direction_result)
"""
if needle not in src: raise SystemExit("directional solve seam missing")
p.write_text(src.replace(needle,rep,1))
PY
done

for variant in clean original fix; do
  cp src/adapter/mod_reference_richards_legacy_binding.f90 "$BUILD/$variant/mod_reference_richards_legacy_binding.f90"
done

python3 - "$BUILD/fix/mod_reference_richards_legacy_binding.f90" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]); src=p.read_text()
old="""    qrosum = sum(richards%provider_root_sink(1:n))
    qdrtot = sum(richards%sink(1:n))
"""
new="""    if (associated(request%evaluation%root_sink)) then
    ...
    qdrtot = sum(richards%sink(1:n))
"""
if old not in src: raise SystemExit("qrosum seam missing")
p.write_text(src.replace(old,new,1))
PY

python3 - "$BUILD/py/fgc44_repro01_d9_probe.py" <<'PY'
from pathlib import Path
import sys
...
if __name__=="__main__":
    ...
    print(f"REPRO01_D9|HCOF={hcof:.17e}|RHS={rhs:.17e}|HREF={href:.17e}|TRIAL_STATUS={status}|Q={q.value:.17e}|{fields}")
    raise SystemExit(0)
'''
...
compile_variant(){
  ...
  for source in "${MODULE_SRC[@]}"; do
    if [[ "$source" == BACKEND_PLACEHOLDER ]]; then source="$BUILD/$name/mod_fmr_serialized_reference_backend.f90"; fi
    if [[ "$source" == LEGACY_PLACEHOLDER ]]; then source="$BUILD/$name/mod_reference_richards_legacy_binding.f90"; fi
...
compile_variant clean
compile_variant original
compile_variant fix

export PYTHONPATH="$BUILD/py:$ROOT/src/adapter:$ROOT/tests/fgc/support"
cycles=100
for mode in clean original fix; do
  pass=0
  failcount=0
  first=""
  if [[ "$mode" == "clean" ]]; then group=ZERO; else group=ROOT_SINK; fi
  for cycle in $(seq 1 "$cycles"); do
    line="$(REPRO01_D9_GROUP="$group" FGC44_SWAP_LIB="$BUILD/$mode/libfgc44_swap.so" python3 "$BUILD/py/fgc44_repro01_d9_probe.py" | grep '^REPRO01_D9|' | tail -1)"
    [[ -n "$line" ]] || fail "missing D9 record mode $mode cycle $cycle"
  ...
  echo "REPRO01_D9_SUMMARY|MODE=$mode|GROUP=$group|CYCLES=$cycles|PASS=$pass|FAIL=$failcount"
  if [[ -n "$first" ]]; then echo "REPRO01_D9_FIRST_FAILURE|MODE=$mode|$first"; fi
done
echo "FPE_REPRO01_D9=OBSERVED"
