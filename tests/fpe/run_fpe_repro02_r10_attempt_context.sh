...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro02-r10-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO02_R10_FAIL $*" >&2; exit 1; }
...
print("REPRO02_R10_RAW|"+json.dumps(d,separators=(",",":")))
PY
...
OUT="$BUILD/r10.txt"; : > "$OUT"
cases=("B01|wet|-10" "B01|mid|-75" "B12|wet|-10" "O05|wet|-10" "O14|wet|-10" "O14|mid|-75")
offsets=(-0.001 0 0.001)
modes=(BASE NO_CONTEXT)
reps=3
for case_spec in "${cases[@]}"; do
  IFS='|' read -r material regime h0 <<< "$case_spec"
  for offset in "${offsets[@]}"; do
    for mode in "${modes[@]}"; do
      if [[ "$mode" == "NO_CONTEXT" ]]; then noctx=1; else noctx=0; fi
      for rep in $(seq 1 "$reps"); do
        raw="$(REPRO02_NO_CONTEXT="$noctx" FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$offset" "$mode")" || { printf '%s\n' "$raw" >&2; fail "$material $regime $offset $mode"; }
        line="$(printf '%s\n' "$raw" | grep '^REPRO02_R10_RAW|' | tail -1)"
        [[ -n "$line" ]] || fail "missing R10 record"
...
for line in open(sys.argv[1]):
    if not line.startswith("REPRO02_R10_RAW|"): continue
...
for key,rs in sorted(groups.items()):
    ...
    if len(sig)!=1: raise SystemExit(f"nondeterministic R10 signature {key}: {sig}")
    r=rs[0]
    print(f"REPRO02_R10_POINT|MATERIAL={key[0]}|REGIME={key[1]}|OFFSET_CM={key[2]:.6f}|ARM={key[3]}"
          f"|PARTICIPANT_STATUS={r['participant_status']}|SOLVER_STATUS={r['solver_status']}"
          f"|NONLINEAR={r['nonlinear']}|BACKTRACK={r['backtrack']}")
for mode in ("BASE","NO_CONTEXT"):
    keys=[k for k in groups if k[3]==mode]
    ok=sum(1 for k in keys if groups[k][0]["participant_status"]==0)
    nonzero=[k for k in keys if abs(k[2])>0]
    nzok=sum(1 for k in nonzero if groups[k][0]["participant_status"]==0)
    print(f"REPRO02_R10_SUMMARY|ARM={mode}|PASS={ok}|NONZERO_PASS={nzok}|TOTAL=18|NONZERO_TOTAL=12")
print("FPE_REPRO02_R10=PASS")
PY
