...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro02-r11-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO02_R11_FAIL $*" >&2; exit 1; }
...
old="""  logical function fmr_serialized_attempt_context_required(self) result(required)
...
"""
new="""  logical function fmr_serialized_attempt_context_required(self) result(required)
    class(fmr_serialized_reference_model_t), intent(in) :: self
    character(len=16) :: repro02_no_context
    integer :: repro02_env_status

    repro02_no_context=''
    call get_environment_variable('REPRO02_NO_CONTEXT',repro02_no_context,status=repro02_env_status)
    if(repro02_env_status==0 .and. trim(repro02_no_context)=='1')then
      required=.false.
      return
    end if
    required = self%trajectory_direction_requested .or. self%drainage_response_active .or. &
...
"""
if old not in src: raise SystemExit("R10 attempt-context seam missing")
...
"  type(fmr_serialized_reference_backend_t), save :: predictor_backend, corrector_backend, repro11_fresh_backend\n"
"  logical, save :: repro11_use_fresh=.false.\n",1)
src=src.replace(
"  public :: fgc44_predictor_run_diagnostics_c\n",
"  public :: fgc44_predictor_run_diagnostics_c\n"
"  public :: fgc44_repro11_set_fresh_backend_c\n",1)

r11_contains="contains\n\n"
r11_insert="""contains

  integer(c_int) function fgc44_repro11_set_fresh_backend_c(enabled) bind(C,name="fgc44_repro11_set_fresh_backend_c")
    integer(c_int), value, intent(in) :: enabled
    fgc44_repro11_set_fresh_backend_c=1_c_int
    if(initialized)return
    repro11_use_fresh=enabled/=0_c_int
    fgc44_repro11_set_fresh_backend_c=0_c_int
  end function fgc44_repro11_set_fresh_backend_c

"""
if r11_contains not in src: raise SystemExit("R11 contains seam missing")
src=src.replace(r11_contains,r11_insert,1)

old_trial="""    call participant%trial_from_origin(corrector_backend,column,template,corrector_parameters,committed,materializer, &
         corrector_config,datum,window,real(head_m,real64),last_trial,status)
"""
new_trial="""    if(repro11_use_fresh)then
      call repro11_fresh_backend%initialize(top)
      call participant%trial_from_origin(repro11_fresh_backend,column,template,corrector_parameters,committed,materializer, &
           corrector_config,datum,window,real(head_m,real64),last_trial,status)
    else
...
"""
if old_trial not in src: raise SystemExit("R11 trial seam missing")
src=src.replace(old_trial,new_trial,1)

old_obs="    obs=corrector_backend%observation()\n"
new_obs="""    if(repro11_use_fresh)then
      obs=repro11_fresh_backend%observation()
    else
      obs=corrector_backend%observation()
    end if
"""
if old_obs not in src: raise SystemExit("R11 observation seam missing")
src=src.replace(old_obs,new_obs,1)
...
armfn=swap.lib.fgc44_repro11_set_fresh_backend_c
armfn.restype=ctypes.c_int; armfn.argtypes=[ctypes.c_int]
if armfn(1 if mode=="FRESH_BACKEND" else 0): raise RuntimeError("R11 arm config failed")
...
d.update(material=material,h0=h0,offset_cm=offset_cm,href=href,predictor_q=predictor_q.value,participant_status=status,q=q.value,mode=mode)
print("REPRO02_R11_RAW|"+json.dumps(d,separators=(",",":")))
PY
  ...
  for offset in "${offsets[@]}"; do
    for mode in "${modes[@]}"; do
      for rep in $(seq 1 "$reps"); do
        ...
        line="$(printf '%s\n' "$raw" | grep '^REPRO02_R11_RAW|' | tail -1)"
        [[ -n "$line" ]] || fail "missing R11 record"
...
for line in open(sys.argv[1]):
    if not line.startswith("REPRO02_R11_RAW|"): continue
...
for key,rs in sorted(groups.items()):
    sig=collections.Counter((r["participant_status"],r["solver_status"],r["nonlinear"],r["backtrack"],r["certificate_available"]) for r in rs)
    if len(sig)!=1: raise SystemExit(f"nondeterministic R11 signature {key}: {sig}")
    r=rs[0]
    print(f"REPRO02_R11_POINT|MATERIAL={key[0]}|REGIME={key[1]}|OFFSET_CM={key[2]:.6f}|ARM={key[3]}"
          f"|PARTICIPANT_STATUS={r['participant_status']}|SOLVER_STATUS={r['solver_status']}"
          f"|NONLINEAR={r['nonlinear']}|BACKTRACK={r['backtrack']}|CERT_AVAILABLE={r['certificate_available']}")
for mode in ("BASE","FRESH_BACKEND"):
    keys=[k for k in groups if k[3]==mode]
    ok=sum(1 for k in keys if groups[k][0]["participant_status"]==0)
    nz=[k for k in keys if abs(k[2])>0]
    nzok=sum(1 for k in nz if groups[k][0]["participant_status"]==0)
    print(f"REPRO02_R11_SUMMARY|ARM={mode}|PASS={ok}|NONZERO_PASS={nzok}|TOTAL=18|NONZERO_TOTAL=12")
print("FPE_REPRO02_R11=PASS")
PY
