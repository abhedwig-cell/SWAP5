...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro02-r12-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO02_R12_FAIL $*" >&2; exit 1; }
...
old="""    type is (fmr_b110_temporal_indicator_state_t)
...
"""
new="""    type is (fmr_b110_temporal_indicator_state_t)
      block
        character(len=16) :: repro12_allow
        integer :: repro12_status
        repro12_allow=''
        call get_environment_variable('REPRO12_ALLOW_TEMPORAL_FLOOR',repro12_allow,status=repro12_status)
        matches = temporal_history_enabled .and. .not. fixed_weir_surface_water_active .and. &
             .not. black_evaporation_active .and. .not. boesten_evaporation_active
        if(repro12_status==0 .and. trim(repro12_allow)=='1') then
          matches = .not. fixed_weir_surface_water_active .and. .not. black_evaporation_active .and. &
               .not. boesten_evaporation_active
        end if
      end block
"""
if old not in src: raise SystemExit("R12 temporal carrier seam missing")
...
"  real(real64), save :: REPRO_ALPHA=0.0135_real64, REPRO_LAMBDA=0.365_real64, REPRO_NVG=1.455_real64\n",
"  real(real64), save :: REPRO_ALPHA=0.0135_real64, REPRO_LAMBDA=0.365_real64, REPRO_NVG=1.455_real64\n"
"  logical, save :: REPRO12_TEMPORAL_FLOOR=.false.\n",1)
src=src.replace(
"  public :: fgc44_repro02_observation_c, fgc44_repro02_floor_c\n",
"  public :: fgc44_repro02_observation_c, fgc44_repro02_floor_c\n"
"  public :: fgc44_repro12_set_temporal_floor_c\n",1)
r12_contains="contains\n\n"
r12_insert="""contains

  integer(c_int) function fgc44_repro12_set_temporal_floor_c(enabled) bind(C,name="fgc44_repro12_set_temporal_floor_c")
    integer(c_int), value, intent(in) :: enabled
    fgc44_repro12_set_temporal_floor_c=1_c_int
    if(initialized)return
    REPRO12_TEMPORAL_FLOOR=enabled/=0_c_int
    fgc44_repro12_set_temporal_floor_c=0_c_int
  end function fgc44_repro12_set_temporal_floor_c

"""
if r12_contains not in src: raise SystemExit("R12 contains seam missing")
src=src.replace(r12_contains,r12_insert,1)

old_ctor="""    call fmr_new_b110_committed_state(floor_committed,COLUMN_ID,base,window%t0,ok)
    if(.not.ok)return
"""
new_ctor="""    if(REPRO12_TEMPORAL_FLOOR)then
      block
        real(real64) :: zero_derivative(numnod)
        zero_derivative=0.0_real64
        call fmr_new_b110_temporal_indicator_committed_state(floor_committed,COLUMN_ID,base,window%t0,ok,zero_derivative)
      end block
    else
      call fmr_new_b110_committed_state(floor_committed,COLUMN_ID,base,window%t0,ok)
    end if
    if(.not.ok)return
"""
if old_ctor not in src: raise SystemExit("R12 floor constructor seam missing")
...
armfn=swap.lib.fgc44_repro12_set_temporal_floor_c
armfn.restype=ctypes.c_int; armfn.argtypes=[ctypes.c_int]
if armfn(1 if mode=="TEMPORAL_FLOOR" else 0): raise RuntimeError("R12 arm configuration failed")
...
print("REPRO02_R12_RAW|"+json.dumps(d,separators=(",",":")))
PY
...
OUT="$BUILD/r12.txt"; : > "$OUT"
cases=("B01|wet|-10" "B01|mid|-75" "B12|wet|-10" "O05|wet|-10" "O14|wet|-10" "O14|mid|-75")
offsets=(-0.001 0 0.001)
modes=(PLAIN_FLOOR TEMPORAL_FLOOR)
reps=3
for case_spec in "${cases[@]}"; do
  IFS='|' read -r material regime h0 <<< "$case_spec"
  for offset in "${offsets[@]}"; do
    for mode in "${modes[@]}"; do
      if [[ "$mode" == "TEMPORAL_FLOOR" ]]; then allow=1; else allow=0; fi
      for rep in $(seq 1 "$reps"); do
        raw="$(REPRO12_ALLOW_TEMPORAL_FLOOR="$allow" FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$offset" "$mode")" || { printf '%s\n' "$raw" >&2; fail "$material $regime $offset $mode"; }
        line="$(printf '%s\n' "$raw" | grep '^REPRO02_R12_RAW|' | tail -1)"
        [[ -n "$line" ]] || fail "missing R12 record"
...
for line in open(sys.argv[1]):
    if not line.startswith("REPRO02_R12_RAW|"): continue
...
for key,rs in sorted(groups.items()):
    sig=collections.Counter((r["floor"]["floor_status"],r["floor"]["sample_valid"],r["floor"]["nonlinear"],r["floor"]["backtrack"],
                             r["floor"]["mass_complete"],r["floor"]["mass_residual"]) for r in rs)
    if len(sig)!=1: raise SystemExit(f"nondeterministic R12 signature {key}: {sig}")
    f=rs[0]["floor"]
    print(f"REPRO02_R12_POINT|MATERIAL={key[0]}|REGIME={key[1]}|OFFSET_CM={key[2]:.6f}|ARM={key[3]}"
          f"|STATUS={f['floor_status']}|VALID={f['sample_valid']}|NONLINEAR={f['nonlinear']}|BACKTRACK={f['backtrack']}"
          f"|MASS_COMPLETE={f['mass_complete']}|MASS_RESIDUAL={f['mass_residual']:.17e}"
          f"|BOTTOM_EXCHANGE={f['bottom_exchange']:.17e}")
for mode in ("PLAIN_FLOOR","TEMPORAL_FLOOR"):
    keys=[k for k in groups if k[3]==mode]
    ok=sum(1 for k in keys if groups[k][0]["floor"]["floor_status"]==0 and groups[k][0]["floor"]["sample_valid"]==1)
    print(f"REPRO02_R12_SUMMARY|ARM={mode}|PASS={ok}|TOTAL=18")
print("FPE_REPRO02_R12=PASS")
PY
