...
initrep="""    result%temporal_acceptance_source = TX_TEMPORAL_MODEL_CERTIFICATE
    repro13_trace=''
    call get_environment_variable('REPRO13_TRACE',repro13_trace,status=repro13_env_status)
...
solverrep="""      if (.not. outcome%solver_ok) then
        if(repro13_enabled) write(*,'(*(g0))') 'REPRO02_R13_ATTEMPT|RETRY_INDEX=',retry_index, &
...
"""
acceptrep="""      if(repro13_enabled) write(*,'(*(g0))') 'REPRO02_R13_ATTEMPT|RETRY_INDEX=',retry_index, &
...
print("REPRO02_R13_RAW|"+json.dumps(d,separators=(",",":")))
PY
...
for case_spec in "${cases[@]}"; do
  ...
  for offset in "${offsets[@]}"; do
    for rep in $(seq 1 "$reps"); do
      raw="$(FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$offset" 2>&1)" || { printf '%s\n' "$raw" >&2; fail "$material $regime $offset"; }
      trace="$(printf '%s\n' "$raw" | grep '^REPRO02_R13_ATTEMPT|' || true)"
      [[ -n "$trace" ]] || fail "missing R13 attempt trace"
      while IFS= read -r line; do
        printf '%s|MATERIAL=%s|REGIME=%s|OFFSET_CM=%s|REP=%s\n' "$line" "$material" "$regime" "$offset" "$rep" | tee -a "$OUT"
      done <<< "$trace"
      final="$(printf '%s\n' "$raw" | grep '^REPRO02_R13_RAW|' | tail -1)"
      [[ -n "$final" ]] || fail "missing R13 final record"
      printf '%s|MATERIAL2=%s|REGIME=%s|OFFSET_CM2=%s|REP=%s\n' "$final" "$material" "$regime" "$offset" "$rep" >> "$OUT"
    done
  done
done
...
for line in open(sys.argv[1]):
    if not line.startswith("REPRO02_R13_ATTEMPT|"): continue
    d={}
...
for d in attempts:
    groups[(d["MATERIAL"],d["REGIME"],d["OFFSET_CM"],d["REP"])].append(d)
...
for key,rows in sorted(groups.items()):
    ...
    for r in rows:
        print(f"REPRO02_R13_POINT|MATERIAL={key[0]}|REGIME={key[1]}|OFFSET_CM={float(key[2]):.6f}|REP={key[3]}"
              f"|RETRY_INDEX={r['RETRY_INDEX']}|DT={float(r['DT']):.17e}|REASON={r['REASON']}"
              f"|SOLVER_OK={r['SOLVER_OK']}|NONLINEAR={r['NONLINEAR']}|BACKTRACK={r['BACKTRACK']}"
