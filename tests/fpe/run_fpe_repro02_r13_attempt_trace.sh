...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro02-r13-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO02_R13_FAIL $*" >&2; exit 1; }
...
solver_block="""      if (.not. outcome%solver_ok) then
        result%solver_rejections = result%solver_rejections + 1
"""
solver_new="""      if (.not. outcome%solver_ok) then
        write(*,'(*(g0))') 'REPRO02_R13_ATTEMPT|RETRY_INDEX=',retry_index,'|DURATION=',attempt_dt, &
             '|SOLVER_OK=FALSE|NONLINEAR=',outcome%nonlinear_iterations,'|BACKTRACK=',outcome%backtracking_attempts, &
             '|MASS_OK=NA|CERT_AVAILABLE=',outcome%temporal_certificate_available, &
             '|TEMPORAL_INDICATOR=',outcome%temporal_indicator,'|TEMPORAL_OK=NA|REASON=SOLVER'
        result%solver_rejections = result%solver_rejections + 1
"""
if solver_block not in src: raise SystemExit("R13 solver rejection seam missing")
src=src.replace(solver_block,solver_new,1)

mass_block="""      if (.not. mass_ok) then
        result%mass_rejections = result%mass_rejections + 1
"""
mass_new="""      if (.not. mass_ok) then
        write(*,'(*(g0))') 'REPRO02_R13_ATTEMPT|RETRY_INDEX=',retry_index,'|DURATION=',attempt_dt, &
             '|SOLVER_OK=TRUE|NONLINEAR=',outcome%nonlinear_iterations,'|BACKTRACK=',outcome%backtracking_attempts, &
             '|MASS_OK=FALSE|CERT_AVAILABLE=',outcome%temporal_certificate_available, &
             '|TEMPORAL_INDICATOR=',outcome%temporal_indicator,'|TEMPORAL_OK=NA|REASON=MASS'
        result%mass_rejections = result%mass_rejections + 1
"""
if mass_block not in src: raise SystemExit("R13 mass rejection seam missing")
src=src.replace(mass_block,mass_new,1)

temporal_block="""      if (.not. temporal_ok) then
        result%temporal_rejections = result%temporal_rejections + 1
"""
temporal_new="""      if (.not. temporal_ok) then
        write(*,'(*(g0))') 'REPRO02_R13_ATTEMPT|RETRY_INDEX=',retry_index,'|DURATION=',attempt_dt, &
             '|SOLVER_OK=TRUE|NONLINEAR=',outcome%nonlinear_iterations,'|BACKTRACK=',outcome%backtracking_attempts, &
             '|MASS_OK=TRUE|CERT_AVAILABLE=',outcome%temporal_certificate_available, &
             '|TEMPORAL_INDICATOR=',outcome%temporal_indicator,'|TEMPORAL_OK=FALSE|REASON=TEMPORAL'
        result%temporal_rejections = result%temporal_rejections + 1
"""
if temporal_block not in src: raise SystemExit("R13 temporal rejection seam missing")
src=src.replace(temporal_block,temporal_new,1)

accept_block="""      result%accepted_storage_start = storage0
"""
accept_new="""      write(*,'(*(g0))') 'REPRO02_R13_ATTEMPT|RETRY_INDEX=',retry_index,'|DURATION=',attempt_dt, &
           '|SOLVER_OK=TRUE|NONLINEAR=',outcome%nonlinear_iterations,'|BACKTRACK=',outcome%backtracking_attempts, &
           '|MASS_OK=TRUE|CERT_AVAILABLE=',outcome%temporal_certificate_available, &
           '|TEMPORAL_INDICATOR=',outcome%temporal_indicator,'|TEMPORAL_OK=TRUE|REASON=ACCEPTED'
      result%accepted_storage_start = storage0
"""
if accept_block not in src: raise SystemExit("R13 accepted seam missing")
src=src.replace(accept_block,accept_new,1)
...
for case_spec in "${cases[@]}"; do
  ...
  for offset in "${offsets[@]}"; do
    for rep in $(seq 1 "$reps"); do
      raw="$(FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$offset" "1e-10" 2>&1)" || { printf '%s\n' "$raw" >&2; fail "$material $regime $offset"; }
      attempts="$(printf '%s\n' "$raw" | grep '^REPRO02_R13_ATTEMPT|' || true)"
      [[ -n "$attempts" ]] || fail "missing R13 attempt trace"
      while IFS= read -r line; do
        printf '%s|MATERIAL=%s|REGIME=%s|OFFSET_CM=%s|REP=%s\n' "$line" "$material" "$regime" "$offset" "$rep" | tee -a "$OUT"
      done <<< "$attempts"
      final="$(printf '%s\n' "$raw" | grep '^REPRO02_R1_RAW|' | tail -1)"
      [[ -n "$final" ]] || fail "missing R13 final record"
      printf '%s|MATERIAL=%s|REGIME=%s|OFFSET_CM=%s|REP=%s\n' "${final/REPRO02_R1_RAW/REPRO02_R13_FINAL}" "$material" "$regime" "$offset" "$rep" | tee -a "$OUT"
    done
  done
done
...
for line in open(sys.argv[1]):
    line=line.strip()
    if line.startswith("REPRO02_R13_ATTEMPT|"):
        d={}
        for p in line.split("|")[1:]:
            k,v=p.split("=",1); d[k]=v
        key=(d["MATERIAL"],d["REGIME"],float(d["OFFSET_CM"]),int(d["REP"]))
        attempts[key].append(d)
    elif line.startswith("REPRO02_R13_FINAL|"):
        payload,tail=line.split("|MATERIAL=",1)
        mat,tail=tail.split("|REGIME=",1); reg,tail=tail.split("|OFFSET_CM=",1); off,rep=tail.split("|REP=",1)
        d=json.loads(payload.split("|",1)[1])
        finals[(mat,reg,float(off),int(rep))]=d
if len(finals)!=54: raise SystemExit(f"expected 54 final records, got {len(finals)}")
if len(attempts)!=54: raise SystemExit(f"expected traces for 54 runs, got {len(attempts)}")

# Every repetition of one point must have the same ordered reason/duration/solver signature.
point_signatures=collections.defaultdict(list)
for key,seq in sorted(attempts.items()):
    sig=tuple((x["RETRY_INDEX"],x["DURATION"],x["SOLVER_OK"],x["NONLINEAR"],x["BACKTRACK"],
               x["MASS_OK"],x["CERT_AVAILABLE"],x["TEMPORAL_INDICATOR"],x["TEMPORAL_OK"],x["REASON"]) for x in seq)
    point_signatures[key[:3]].append(sig)
    for x in seq:
        print(f"REPRO02_R13_TRACE|MATERIAL={key[0]}|REGIME={key[1]}|OFFSET_CM={key[2]:.6f}|REP={key[3]}"
              f"|RETRY_INDEX={x['RETRY_INDEX']}|DURATION={float(x['DURATION']):.17e}|SOLVER_OK={x['SOLVER_OK']}"
              f"|NONLINEAR={x['NONLINEAR']}|BACKTRACK={x['BACKTRACK']}|MASS_OK={x['MASS_OK']}"
              f"|CERT_AVAILABLE={x['CERT_AVAILABLE']}|TEMPORAL_INDICATOR={float(x['TEMPORAL_INDICATOR']):.17e}"
              f"|TEMPORAL_OK={x['TEMPORAL_OK']}|REASON={x['REASON']}")
for point,sigs in point_signatures.items():
    if len(sigs)!=3 or len(set(sigs))!=1: raise SystemExit(f"nondeterministic R13 sequence {point}: {sigs}")

first_reason_counts=collections.Counter()
pattern_counts=collections.Counter()
for point,sigs in sorted(point_signatures.items()):
    sig=sigs[0]
    reasons=tuple(x[-1] for x in sig)
    first_reason_counts[reasons[0]]+=1
    pattern_counts[reasons]+=1
    final=finals[(point[0],point[1],point[2],1)]
    print(f"REPRO02_R13_POINT|MATERIAL={point[0]}|REGIME={point[1]}|OFFSET_CM={point[2]:.6f}"
          f"|FIRST_REASON={reasons[0]}|SEQUENCE={','.join(reasons)}|PARTICIPANT_STATUS={final['participant_status']}")
print("REPRO02_R13_SUMMARY|"+ "|".join(f"FIRST_{k}={v}" for k,v in sorted(first_reason_counts.items())) +
      f"|UNIQUE_PATTERNS={len(pattern_counts)}|POINTS={len(point_signatures)}")
