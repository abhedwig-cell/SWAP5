...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro02-r1-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO02_R1_FAIL $*" >&2; exit 1; }
...
d.update(material=material,h0=h0,offset_cm=offset_cm,href=href,predictor_q=predictor_q.value,participant_status=status,q=q.value)
...
for line in open(sys.argv[1]):
    if not line.startswith("REPRO02_R1_RAW|"): continue
...
for d in rows: groups[(d["material"],d["regime"],d["offset_cm"])].append(d)
for key,rs in sorted(groups.items()):
    sig=collections.Counter((r["participant_status"],r["solver_status"],r["temporal_status"],r["temporal_available"],r["certificate_available"]) for r in rs)
    if len(sig)!=1: raise SystemExit(f"nondeterministic signature {key}: {sig}")
    r=rs[0]
    print(
      f"REPRO02_R1_POINT|MATERIAL={key[0]}|REGIME={key[1]}|OFFSET_CM={key[2]:.6f}|REPS=5"
      f"|PARTICIPANT_STATUS={r['participant_status']}|SOLVER_EXECUTED={r['solver_executed']}|SOLVER_STATUS={r['solver_status']}"
      f"|NONLINEAR={r['nonlinear']}|JACOBIAN={r['jacobian']}|LINEAR={r['linear']}|BACKTRACK={r['backtrack']}"
      f"|RETRIES={r['retries']}|CONSTITUTIVE={r['constitutive']}|EQ_AVAILABLE={r['eq_available']}|EQ_RESIDUAL={r['eq_residual']:.17e}"
      f"|TEMPORAL_ENABLED={r['temporal_enabled']}|TEMPORAL_STATUS={r['temporal_status']}|TEMPORAL_AVAILABLE={r['temporal_available']}"
      f"|CERT_AVAILABLE={r['certificate_available']}|HEAD_BOUND={r['head_bound']:.17e}|BUDGET={r['budget']:.17e}"
      f"|NORMALIZED={r['normalized']:.17e}|TOP_FLUX={r['top_flux']:.17e}|BOTTOM_FLUX={r['bottom_flux']:.17e}"
    )
print("FPE_REPRO02_R1=PASS")
PY
