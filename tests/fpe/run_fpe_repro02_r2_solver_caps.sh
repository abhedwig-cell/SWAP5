...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro02-r2-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO02_R2_FAIL $*" >&2; exit 1; }
...
"    p%max_iterations=REPRO_MAX_ITER; p%max_backtracking=REPRO_MAX_BACKTRACK; p%min_step_duration=1.0e-8_real64"
)
...
"  public :: fgc44_repro02_observation_c, fgc44_repro02_solver_limits_c\n",1)
needle="contains\n\n"
insert="""contains

  integer(c_int) function fgc44_repro02_solver_limits_c(max_iter,max_backtrack) bind(C,name="fgc44_repro02_solver_limits_c")
    integer(c_int), value, intent(in) :: max_iter,max_backtrack
    fgc44_repro02_solver_limits_c=1_c_int
    if(initialized)return
    if(max_iter<=0_c_int .or. max_backtrack<=0_c_int)return
    REPRO_MAX_ITER=int(max_iter)
    REPRO_MAX_BACKTRACK=int(max_backtrack)
    fgc44_repro02_solver_limits_c=0_c_int
  end function fgc44_repro02_solver_limits_c
...
material,h0_s,offset_s,maxit_s,maxbt_s=sys.argv[1:]
h0=float(h0_s); offset_cm=float(offset_s); maxit=int(maxit_s); maxbt=int(maxbt_s)
...
limitfn=swap.lib.fgc44_repro02_solver_limits_c
limitfn.restype=ctypes.c_int; limitfn.argtypes=[ctypes.c_int,ctypes.c_int]
if limitfn(maxit,maxbt): raise RuntimeError("solver limit configuration failed")
...
d.update(material=material,h0=h0,offset_cm=offset_cm,maxit=maxit,maxbt=maxbt,href=href,predictor_q=predictor_q.value,participant_status=status,q=q.value)
...
OUT="$BUILD/r2.txt"; : > "$OUT"
cases=("B01|wet|-10" "B01|mid|-75" "B12|wet|-10" "O05|wet|-10" "O14|wet|-10" "O14|mid|-75")
offsets=(-0.001 0.001)
limits=("16|8" "32|8" "16|16" "32|16" "48|16")
reps=3
for case_spec in "${cases[@]}"; do
  IFS='|' read -r material regime h0 <<< "$case_spec"
  for limit_spec in "${limits[@]}"; do
    IFS='|' read -r maxit maxbt <<< "$limit_spec"
    for offset in "${offsets[@]}"; do
      for rep in $(seq 1 "$reps"); do
        raw="$(FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$offset" "$maxit" "$maxbt")" || { printf '%s\n' "$raw" >&2; fail "$material $regime $maxit/$maxbt $offset"; }
        line="$(printf '%s\n' "$raw" | grep '^REPRO02_R1_RAW|' | tail -1)"
        [[ -n "$line" ]] || fail "missing R2 record"
        printf '%s|REGIME=%s|REP=%s\n' "${line/REPRO02_R1_RAW/REPRO02_R2_RAW}" "$regime" "$rep" | tee -a "$OUT"
      done
    done
  done
done
...
for line in open(sys.argv[1]):
    if not line.startswith("REPRO02_R2_RAW|"): continue
...
if len(rows)!=180: raise SystemExit(f"expected 180 rows, got {len(rows)}")
groups=collections.defaultdict(list)
for d in rows: groups[(d["material"],d["regime"],d["maxit"],d["maxbt"],d["offset_cm"])].append(d)
for key,rs in sorted(groups.items()):
    sig=collections.Counter((r["participant_status"],r["solver_status"],r["nonlinear"],r["backtrack"],r["retries"],r["certificate_available"]) for r in rs)
    if len(sig)!=1: raise SystemExit(f"nondeterministic signature {key}: {sig}")
    r=rs[0]
    print(
      f"REPRO02_R2_POINT|MATERIAL={key[0]}|REGIME={key[1]}|MAX_ITER={key[2]}|MAX_BACKTRACK={key[3]}|OFFSET_CM={key[4]:.6f}"
      f"|PARTICIPANT_STATUS={r['participant_status']}|SOLVER_STATUS={r['solver_status']}|NONLINEAR={r['nonlinear']}"
      f"|BACKTRACK={r['backtrack']}|RETRIES={r['retries']}|CERT_AVAILABLE={r['certificate_available']}"
    )
for lim in [(16,8),(32,8),(16,16),(32,16),(48,16)]:
    keys=[k for k in groups if k[2:4]==lim]
    pass_points=sum(1 for k in keys if groups[k][0]["participant_status"]==0)
    print(f"REPRO02_R2_SUMMARY|MAX_ITER={lim[0]}|MAX_BACKTRACK={lim[1]}|PASS_POINTS={pass_points}|TOTAL_POINTS={len(keys)}")
print("FPE_REPRO02_R2=PASS")
PY
