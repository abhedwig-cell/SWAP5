...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro02-r3-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO02_R3_FAIL $*" >&2; exit 1; }
...
OUT="$BUILD/r3.txt"; : > "$OUT"
cases=("B01|wet|-10" "B01|mid|-75" "B12|wet|-10" "O05|wet|-10" "O14|wet|-10" "O14|mid|-75")
offsets=(-0.001 0.001)
minsteps=(1e-8 1e-9 1e-10 1e-11 1e-12)
reps=3
for case_spec in "${cases[@]}"; do
  IFS='|' read -r material regime h0 <<< "$case_spec"
  for minstep in "${minsteps[@]}"; do
    for offset in "${offsets[@]}"; do
      for rep in $(seq 1 "$reps"); do
        raw="$(FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$offset" "$minstep")" || { printf '%s\n' "$raw" >&2; fail "$material $regime $minstep $offset"; }
        line="$(printf '%s\n' "$raw" | grep '^REPRO02_R1_RAW|' | tail -1)"
        [[ -n "$line" ]] || fail "missing R3 record"
        printf '%s|REGIME=%s|REP=%s\n' "${line/REPRO02_R1_RAW/REPRO02_R3_RAW}" "$regime" "$rep" | tee -a "$OUT"
      done
    done
  done
done
...
for line in open(sys.argv[1]):
    if not line.startswith("REPRO02_R3_RAW|"): continue
...
for d in rows: groups[(d["material"],d["regime"],d["minstep"],d["offset_cm"])].append(d)
for key,rs in sorted(groups.items()):
    ...
    r=rs[0]
    print(
      f"REPRO02_R3_POINT|MATERIAL={key[0]}|REGIME={key[1]}|MIN_STEP={key[2]:.1e}|OFFSET_CM={key[3]:.6f}"
...
for ms in [1e-8,1e-9,1e-10,1e-11,1e-12]:
    keys=[k for k in groups if abs(k[2]-ms)<=ms*1e-12]
    passed=sum(1 for k in keys if groups[k][0]["participant_status"]==0)
    print(f"REPRO02_R3_SUMMARY|MIN_STEP={ms:.1e}|PASS_POINTS={passed}|TOTAL_POINTS={len(keys)}")
print("FPE_REPRO02_R3=PASS")
PY
