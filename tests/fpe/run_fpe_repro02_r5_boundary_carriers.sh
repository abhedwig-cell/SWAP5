...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-repro02-r5-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "REPRO02_R5_FAIL $*" >&2; exit 1; }
...
rep="""  dreq%incoming_ponding_depth=0.0_real64; dreq%direct_control_derivative=1.0_real64
  ...
  case('BOTH')
    ...
    request%boundary%top_head=h0
  case default
    error stop 'unknown R5 carrier mode'
  end select
  context_ok=.true.
...
PY
    ...
    for mode in "${modes[@]}"; do
      for rep in $(seq 1 "$reps"); do
        ...
        [[ -n "$line" ]] || fail "missing R5 record"
        printf '%s|REGIME=%s|OFFSET_CM=%s|REP=%s\n' "${line/APPROX01_MATRIX_POINT/REPRO02_R5_RAW}" "$regime" "$offset" "$rep" | tee -a "$OUT"
      done
    done
  done
done
...
for line in open(sys.argv[1]):
    if not line.startswith("REPRO02_R5_RAW|"): continue
    d={}
...
for key,rs in sorted(groups.items()):
    ...
    if len(sig)!=1: raise SystemExit(f"nondeterministic R5 signature {key}: {sig}")
    r=rs[0]
    print(
      f"REPRO02_R5_POINT|MATERIAL={key[0]}|REGIME={key[1]}|OFFSET_CM={float(key[2]):.6f}|MODE={key[3]}"
      f"|SOLVE_STATUS={r['SOLVE_STATUS']}|DIRECTION_STATUS={r['DIRECTION_STATUS']}|NONLINEAR={r['NONLINEAR']}|BACKTRACK={r['BACKTRACK']}"
    )
for mode in ("BASE","QBOT","TOPHEAD","BOTH"):
    keys=[k for k in groups if k[3]==mode]
    passed=sum(1 for k in keys if groups[k][0]["SOLVE_STATUS"]=="1")
    print(f"REPRO02_R5_SUMMARY|MODE={mode}|PASS={passed}|TOTAL={len(keys)}")
print("FPE_REPRO02_R5=PASS")
PY
