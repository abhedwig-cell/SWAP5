...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-shortstep01-p2-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "SHORTSTEP01_P2_FAIL $*" >&2; exit 1; }
...
"   sumold = 0.5d0 * dot_product(fsi_ws%residual(1:NN), fsi_ws%residual(1:NN))\n"
"   if(shortstep_trace_enabled) write(*,'(*(g0))') 'SHORTSTEP01_P2_INIT|DT=',dt,'|SUMOLD=',sumold, &\n"
"        '|FMAX=',maxval(dabs(fsi_ws%residual(1:NN)))\n",1)
src=src.replace(
"      factor = 1.0d0\n      do itry = 1, MaxBackTr\n",
"      if(shortstep_trace_enabled) write(*,'(*(g0))') 'SHORTSTEP01_P2_NEWTON|ITER=',solver_numbit,'|DT=',dt, &\n"
"           '|SUMOLD=',sumold,'|MAX_DELTA=',maxval(dabs(fsi_ws%delta_head(1:NN)))\n"
"      factor = 1.0d0\n      do itry = 1, MaxBackTr\n",1)
src=src.replace(
"         Fmax = maxval(dabs(fsi_ws%residual(1:NN)))\n\n!        test for iteration progress",
"         Fmax = maxval(dabs(fsi_ws%residual(1:NN)))\n"
"         if(shortstep_trace_enabled) write(*,'(*(g0))') 'SHORTSTEP01_P2_TRIAL|ITER=',solver_numbit, &\n"
...
"!  Convergence could not been reached\n"
"   if(shortstep_trace_enabled) write(*,'(*(g0))') 'SHORTSTEP01_P2_END|DT=',dt,'|STATUS=EXHAUSTED', &\n"
...
print("SHORTSTEP01_P2_RAW|"+json.dumps(d,separators=(",",":")))
PY
...
for line in open(sys.argv[1]):
    line=line.strip()
    if line.startswith("SHORTSTEP01_P2_RAW|"):
    ...
    elif line.startswith("SHORTSTEP01_P2_"):
        tag=line.split("|",1)[0]
        d={"tag":tag}
        ...
        trace[key].append(d)
if len(finals)!=8 or len(trace)!=8: raise SystemExit(f"incomplete P2 data finals={len(finals)} traces={len(trace)}")
for key,d in sorted(finals.items()):
    gates=[r for r in trace[key] if r["tag"]=="SHORTSTEP01_P2_GATE"]
    if not gates: raise SystemExit(f"missing gate trace {key}")
    last=gates[-1]
    print(f"SHORTSTEP01_P2_SUMMARY|MATERIAL={key[0]}|REGIME={key[1]}|IMBALANCE={key[2]:.3f}|OFFSET_CM={key[3]:.6f}|DT={key[4]:.8e}"
          f"|FLOOR_STATUS={d['floor_status']}|NONLINEAR={d['nonlinear']}|FINAL_BAL_FAIL={last['BAL_FAIL']}"
          f"|FINAL_HEAD_FAIL={last['HEAD_FAIL']}|FINAL_MAX_HEAD_METRIC={float(last['MAX_HEAD_METRIC']):.17e}"
          f"|FINAL_SUM1={float(last['SUM1']):.17e}|FINAL_TOTAL_FAIL={last['TOTAL_FAIL']}|FINAL_FLNONCONV={last['FLNONCONV']}")
print("FPE_SHORTSTEP01_P2=PASS")
PY
