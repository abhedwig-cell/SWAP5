...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-shortstep01-p3-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "SHORTSTEP01_P3_FAIL $*" >&2; exit 1; }
...
print("SHORTSTEP01_P3_RAW|"+json.dumps(d,separators=(",",":")))
PY
...
OUT="$BUILD/p3.txt"; : > "$OUT"
cases=(
  "B01|wet|-10|0.10|0.001|7.5e-5|PASS_CONTROL"
  "B01|wet|-10|0.10|0.001|5e-5|FAIL_TARGET"
  "O05|wet|-10|-0.10|0.001|1.875e-5|PASS_CONTROL"
  "O05|wet|-10|-0.10|0.001|1.25e-5|FAIL_TARGET"
  "O14|wet|-10|0.10|-0.001|5e-5|PASS_CONTROL"
  "O14|wet|-10|0.10|-0.001|3.75e-5|FAIL_TARGET"
)
tols=(1e-12 2e-12 5e-12 1e-11 1e-10)
for spec in "${cases[@]}"; do
  IFS='|' read -r material regime h0 imbalance offset dt role <<< "$spec"
  for tol in "${tols[@]}"; do
    raw="$(FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$imbalance" "$offset" "$dt" "$tol")" || { printf '%s\n' "$raw" >&2; fail "$spec tol=$tol"; }
    line="$(printf '%s\n' "$raw" | grep '^SHORTSTEP01_P3_RAW|' | tail -1)"
    [[ -n "$line" ]] || fail "missing P3 record"
    printf '%s|REGIME=%s|ROLE=%s\n' "$line" "$regime" "$role" | tee -a "$OUT"
  done
done
...
for line in open(sys.argv[1]):
    if not line.startswith("SHORTSTEP01_P3_RAW|"): continue
    payload,tail=line.strip().split("|REGIME=",1)
    reg,role=tail.split("|ROLE=",1)
    d=json.loads(payload.split("|",1)[1]); d["regime"]=reg; d["role"]=role
    groups[(d["material"],reg,d["imbalance"],d["offset_cm"],d["dt"])].append(d)
if len(groups)!=6: raise SystemExit(f"expected 6 target groups, got {len(groups)}")
for key,rs in sorted(groups.items()):
    rs=sorted(rs,key=lambda r:r["tol"])
    successes=[r for r in rs if r["floor_status"]==0 and r["sample_valid"]==1]
    if not successes: raise SystemExit(f"no successful tolerance arm {key}")
    ref=successes[0]
    for r in rs:
        ok=r["floor_status"]==0 and r["sample_valid"]==1
        if ok:
            dh=max(abs(a-b) for a,b in zip(r["heads"],ref["heads"]))
            dtheta=max(abs(a-b) for a,b in zip(r["theta"],ref["theta"]))
            dflux=abs(r["terminal_flux_cm_per_day"]-ref["terminal_flux_cm_per_day"])
        else:
            dh=dtheta=dflux=float("nan")
        print(f"SHORTSTEP01_P3_POINT|MATERIAL={key[0]}|REGIME={key[1]}|IMBALANCE={key[2]:.3f}|OFFSET_CM={key[3]:.6f}|DT={key[4]:.8e}"
              f"|ROLE={r['role']}|TOL={r['tol']:.1e}|PASS={str(ok).upper()}|NONLINEAR={r['nonlinear']}|BACKTRACK={r['backtrack']}"
              f"|MASS_RESIDUAL={r['mass_residual']:.17e}|MAX_DH_VS_STRICT_SUCCESS={dh:.17e}"
              f"|MAX_DTHETA_VS_STRICT_SUCCESS={dtheta:.17e}|DFLUX_VS_STRICT_SUCCESS={dflux:.17e}")
print("FPE_SHORTSTEP01_P3=PASS")
PY
