...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-temporal02-p0-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "TEMPORAL02_P0_FAIL $*" >&2; exit 1; }
...
solverrep="""      if (.not. outcome%solver_ok) then
        if(repro13_enabled) write(*,'(*(g0))') 'TEMPORAL02_P0_ATTEMPT|RETRY_INDEX=',retry_index, &
...
"""
acceptrep="""      if(repro13_enabled) write(*,'(*(g0))') 'TEMPORAL02_P0_ATTEMPT|RETRY_INDEX=',retry_index, &
...
print("TEMPORAL02_P0_RAW|"+json.dumps(d,separators=(",",":")))
PY
...
offsets=(-0.001 0.001)
budgets=(1e-5 2e-5 5e-5 1e-4 2e-4 5e-4 1e-3)
reps=3
  ...
  for budget in "${budgets[@]}"; do
    for offset in "${offsets[@]}"; do
      for rep in $(seq 1 "$reps"); do
        ...
        trace="$(printf '%s\n' "$raw" | grep '^TEMPORAL02_P0_ATTEMPT|' || true)"
        [[ -n "$trace" ]] || fail "missing P0 attempt trace"
        while IFS= read -r line; do
          printf '%s|MATERIAL=%s|REGIME=%s|OFFSET_CM=%s|BUDGET=%s|REP=%s\n' "$line" "$material" "$regime" "$offset" "$budget" "$rep" >> "$OUT"
        done <<< "$trace"
        final="$(printf '%s\n' "$raw" | grep '^TEMPORAL02_P0_RAW|' | tail -1)"
        [[ -n "$final" ]] || fail "missing P0 final record"
...
if len(finals)!=84: raise SystemExit(f"expected 84 final groups, got {len(finals)}")
if len(attempts)!=252: raise SystemExit(f"expected 252 trace groups, got {len(attempts)}")

for key,rs in finals.items():
    sig={(r["participant_status"],r["solver_status"],r["nonlinear"],r["backtrack"],r["certificate_available"]) for r in rs}
    if len(rs)!=3 or len(sig)!=1: raise SystemExit(f"nondeterministic final signature {key}: {sig}")

budget_summary=collections.defaultdict(lambda:{"pass":0,"points":0,"events":0,"solver":0,"temporal":0})
for (mat,reg,off,budget),rs in sorted(finals.items()):
    seqs=[]
    for rep in (1,2,3):
        seq=attempts[(mat,reg,off,budget,rep)]
        seqs.append(tuple((x["RETRY_INDEX"],x["REASON"],round(float(x["DT"]),16)) for x in seq))
    if len(set(seqs))!=1: raise SystemExit(f"nondeterministic ordered trace {(mat,reg,off,budget)}")
    reasons=[x[1] for x in seqs[0]]
    success=rs[0]["participant_status"]==0
    x=budget_summary[budget]
    x["points"]+=1; x["pass"]+=int(success); x["events"]+=len(reasons)
    x["solver"]+=sum(r=="SOLVER" for r in reasons); x["temporal"]+=sum(r=="TEMPORAL" for r in reasons)
    print(f"TEMPORAL02_P0_POINT|MATERIAL={mat}|REGIME={reg}|OFFSET_CM={off:.6f}|BUDGET_CM={budget:.1e}"
          f"|PASS={str(success).upper()}|PARTICIPANT_STATUS={rs[0]['participant_status']}|FIRST_REASON={reasons[0]}"
          f"|ATTEMPT_EVENTS={len(reasons)}|TEMPORAL_REJECTIONS={sum(r=='TEMPORAL' for r in reasons)}"
          f"|SOLVER_REJECTIONS={sum(r=='SOLVER' for r in reasons)}|SEQUENCE={','.join(reasons)}")
for budget in sorted(budget_summary):
    x=budget_summary[budget]
    print(f"TEMPORAL02_P0_BUDGET|BUDGET_CM={budget:.1e}|PASS={x['pass']}|TOTAL={x['points']}"
          f"|ATTEMPT_EVENTS={x['events']}|TEMPORAL_REJECTIONS={x['temporal']}|SOLVER_REJECTIONS={x['solver']}")
print("FPE_TEMPORAL02_P0=PASS")
PY
