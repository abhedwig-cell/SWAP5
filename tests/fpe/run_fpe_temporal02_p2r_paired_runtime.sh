...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-temporal02-p2r-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "TEMPORAL02_P2R_FAIL $*" >&2; exit 1; }
...
bfn=swap.lib.fgc44_temporal02_head_budget_c
...
for _ in range(5):
    ...
    if status: raise RuntimeError(f"warmup failed {status}")
    swap.discard()
times=[]; qs=[]
for _ in range(30):
    ...
    if status: raise RuntimeError(f"measured trial failed {status}")
    times.append(t1-t0); qs.append(q.value)
    swap.discard()
if max(qs)!=min(qs): raise RuntimeError("q nondeterministic")
times_sorted=sorted(times)
def pct(p):
    i=round((len(times_sorted)-1)*p)
    return times_sorted[i]
print("TEMPORAL02_P2R_RAW|"+json.dumps({
 "material":material,"h0":h0,"offset_cm":offset_cm,"budget":budget,"q":qs[0],
 "median_ns":statistics.median(times),"p25_ns":pct(0.25),"p75_ns":pct(0.75),
...
for case_spec in "${cases[@]}"; do
  ...
  for offset in "${offsets[@]}"; do
    for batch in $(seq 1 "$batches"); do
      if (( batch % 2 == 1 )); then budgets=(5e-4 1e-3); else budgets=(1e-3 5e-4); fi
      for budget in "${budgets[@]}"; do
        raw="$(FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$offset" "$budget")" || { printf '%s\n' "$raw" >&2; fail "$material $regime $offset batch=$batch $budget"; }
        line="$(printf '%s\n' "$raw" | grep '^TEMPORAL02_P2R_RAW|' | tail -1)"
        printf '%s|REGIME=%s|BATCH=%s\n' "$line" "$regime" "$batch" | tee -a "$OUT"
      done
    done
  done
done
...
for line in open(sys.argv[1]):
    if not line.startswith("TEMPORAL02_P2R_RAW|"): continue
    payload,tail=line.strip().split("|REGIME=")
    reg,batch=tail.split("|BATCH=")
    d=json.loads(payload.split("|",1)[1]); d["regime"]=reg; batch=int(batch)
    groups[(d["material"],reg,d["offset_cm"],batch)][d["budget"]]=d
if len(groups)!=84: raise SystemExit(f"expected 84 paired batches, got {len(groups)}")
point_ratios=collections.defaultdict(list)
for (mat,reg,off,batch),arms in sorted(groups.items()):
    if set(round(x,7) for x in arms)!={0.0005,0.001}: raise SystemExit(f"missing arm {(mat,reg,off,batch)}")
    a=next(v for k,v in arms.items() if abs(k-5e-4)<1e-15)
    b=next(v for k,v in arms.items() if abs(k-1e-3)<1e-15)
    ratio=a["median_ns"]/b["median_ns"]
    point_ratios[(mat,reg,off)].append(ratio)
for key,ratios in sorted(point_ratios.items()):
    if len(ratios)!=7: raise SystemExit(f"expected 7 ratios {key}")
    med=statistics.median(ratios)
    path="RETRY_DIRECT" if key[0]=="O14" and key[1]=="wet" else "DIRECT_DIRECT"
    print(f"TEMPORAL02_P2R_POINT|MATERIAL={key[0]}|REGIME={key[1]}|OFFSET_CM={key[2]:.6f}|PATH={path}"
          f"|PAIRED_BATCHES=7|MEDIAN_RATIO_5E4_OVER_1E3={med:.9f}|MIN_RATIO={min(ratios):.9f}|MAX_RATIO={max(ratios):.9f}")
direct=[statistics.median(v) for k,v in point_ratios.items() if not (k[0]=="O14" and k[1]=="wet")]
retry=[statistics.median(v) for k,v in point_ratios.items() if k[0]=="O14" and k[1]=="wet"]
print(f"TEMPORAL02_P2R_SUMMARY|DIRECT_DIRECT_POINTS={len(direct)}|DIRECT_DIRECT_MEDIAN_RATIO={statistics.median(direct):.9f}"
      f"|RETRY_DIRECT_POINTS={len(retry)}|RETRY_DIRECT_MEDIAN_RATIO={statistics.median(retry):.9f}")
print("FPE_TEMPORAL02_P2R=PASS")
PY
