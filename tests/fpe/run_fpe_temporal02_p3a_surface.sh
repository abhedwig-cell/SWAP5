...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-temporal02-p3a-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "TEMPORAL02_P3A_FAIL $*" >&2; exit 1; }
...
solverrep="""      if (.not. outcome%solver_ok) then
        if(repro13_enabled) write(*,'(*(g0))') 'TEMPORAL02_P3A_ATTEMPT|RETRY_INDEX=',retry_index, &
...
"""
acceptrep="""      if(repro13_enabled) write(*,'(*(g0))') 'TEMPORAL02_P3A_ATTEMPT|RETRY_INDEX=',retry_index, &
...
print("TEMPORAL02_P3A_RAW|"+json.dumps(d,separators=(",",":")))
PY
...
OUT="$BUILD/p3a.txt"; : > "$OUT"
cases=("B01|wet|-10" "B01|mid|-75" "B12|wet|-10" "O05|wet|-10" "O14|wet|-10" "O14|mid|-75")
offsets=(-0.001 0.001 -0.01 0.01 -0.05 0.05 -0.10 0.10 -0.25 0.25)
budgets=(5e-4 1e-3 2e-3 5e-3 1e-2 2e-2 5e-2)
for case_spec in "${cases[@]}"; do
  ...
  for budget in "${budgets[@]}"; do
    for offset in "${offsets[@]}"; do
      ...
      trace="$(printf '%s\n' "$raw" | grep '^TEMPORAL02_P3A_ATTEMPT|' || true)"
      [[ -n "$trace" ]] || fail "missing P3A attempt trace"
      while IFS= read -r line; do
        printf '%s|MATERIAL=%s|REGIME=%s|OFFSET_CM=%s|BUDGET=%s\n' "$line" "$material" "$regime" "$offset" "$budget" >> "$OUT"
      done <<< "$trace"
      final="$(printf '%s\n' "$raw" | grep '^TEMPORAL02_P3A_RAW|' | tail -1)"
      [[ -n "$final" ]] || fail "missing P3A final record"
      printf '%s|REGIME=%s\n' "$final" "$regime" >> "$OUT"
    done
  done
done
...
for line in open(sys.argv[1]):
    line=line.strip()
    if line.startswith("TEMPORAL02_P3A_ATTEMPT|"):
        d={}
        for p in line.split("|")[1:]:
            k,v=p.split("=",1); d[k]=v
        key=(d["MATERIAL"],d["REGIME"],float(d["OFFSET_CM"]),float(d["BUDGET"]))
        attempts[key].append(d)
    elif line.startswith("TEMPORAL02_P3A_RAW|"):
        payload,reg=line.split("|REGIME=",1)
        d=json.loads(payload.split("|",1)[1]); d["regime"]=reg
        finals[(d["material"],reg,d["offset_cm"],d["budget"])]=d
if len(finals)!=420: raise SystemExit(f"expected 420 final points, got {len(finals)}")
if len(attempts)!=420: raise SystemExit(f"expected 420 trace groups, got {len(attempts)}")
...
mags=sorted({abs(k[2]) for k in finals})
for mag in mags:
    vals=[]
    ...
    common=max(vals)
    if common==float("inf"):
        print(f"TEMPORAL02_P3A_MAGNITUDE|ABS_OFFSET_CM={mag:.6f}|COMMON_COMPLETING_BUDGET=NONE")
    else:
        print(f"TEMPORAL02_P3A_MAGNITUDE|ABS_OFFSET_CM={mag:.6f}|COMMON_COMPLETING_BUDGET_CM={common:.1e}")
print("FPE_TEMPORAL02_P3A=PASS")
PY
