...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-temporal02-p3b-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "TEMPORAL02_P3B_FAIL $*" >&2; exit 1; }
...
solverrep="""      if (.not. outcome%solver_ok) then
        if(repro13_enabled) write(*,'(*(g0))') 'TEMPORAL02_P3B_ATTEMPT|RETRY_INDEX=',retry_index, &
...
"""
acceptrep="""      if(repro13_enabled) write(*,'(*(g0))') 'TEMPORAL02_P3B_ATTEMPT|RETRY_INDEX=',retry_index, &
...
d.update(material=material,h0=h0,offset_cm=offset_cm,budget=budget,href=href,predictor_q=predictor_q.value,participant_status=status,q=q.value)
print("TEMPORAL02_P3B_RAW|"+json.dumps(d,separators=(",",":")))
PY
...
PY
      ...
      for budget in "${budgets[@]}"; do
        for rep in $(seq 1 "$reps"); do
          raw="$(FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$offset" "$budget" 2>&1)" || { printf '%s\n' "$raw" >&2; fail "$material $regime $offset $budget rep=$rep"; }
          trace="$(printf '%s\n' "$raw" | grep '^TEMPORAL02_P3B_ATTEMPT|' || true)"
          [[ -n "$trace" ]] || fail "missing P3B attempt trace"
          while IFS= read -r line; do
            printf '%s|MATERIAL=%s|REGIME=%s|OFFSET_CM=%s|BUDGET=%s|REP=%s\n' "$line" "$material" "$regime" "$offset" "$budget" "$rep" >> "$OUT"
          done <<< "$trace"
          final="$(printf '%s\n' "$raw" | grep '^TEMPORAL02_P3B_RAW|' | tail -1)"
          [[ -n "$final" ]] || fail "missing P3B final record"
...
for line in open(sys.argv[1]):
    line=line.strip()
    if line.startswith("TEMPORAL02_P3B_ATTEMPT|"):
        d={}
        for p in line.split("|")[1:]:
            k,v=p.split("=",1); d[k]=v
        key=(d["MATERIAL"],d["REGIME"],float(d["OFFSET_CM"]),float(d["BUDGET"]),int(d["REP"]))
        attempts[key].append(d)
    elif line.startswith("TEMPORAL02_P3B_RAW|"):
        payload,tail=line.split("|REGIME=",1)
        reg,rep=tail.split("|REP=",1)
        d=json.loads(payload.split("|",1)[1]); d["regime"]=reg; d["rep"]=int(rep)
        finals[(d["material"],reg,d["offset_cm"],d["budget"])].append(d)

expected_groups=6*2*(2+2+2+2+1)
if len(finals)!=expected_groups:
    raise SystemExit(f"expected {expected_groups} final groups, got {len(finals)}")
for key,rs in sorted(finals.items()):
    if len(rs)!=3: raise SystemExit(f"expected 3 reps {key}")
    sig={(r["participant_status"],r["solver_status"],r["nonlinear"],r["backtrack"],r["q"]) for r in rs}
    if len(sig)!=1: raise SystemExit(f"nondeterministic final signature {key}: {sig}")
    seqs=[]
    mat,reg,off,budget=key
    for rep in (1,2,3):
        seq=attempts[(mat,reg,off,budget,rep)]
        seqs.append(tuple((x["RETRY_INDEX"],x["REASON"],round(float(x["DT"]),16)) for x in seq))
    if len(set(seqs))!=1: raise SystemExit(f"nondeterministic ordered trace {key}")
    reasons=[x[1] for x in seqs[0]]
    ok=rs[0]["participant_status"]==0
    print(f"TEMPORAL02_P3B_POINT|MATERIAL={mat}|REGIME={reg}|OFFSET_CM={off:.6f}|BUDGET_CM={budget:.1e}"
          f"|PASS={str(ok).upper()}|TEMPORAL_REJECTIONS={sum(r=='TEMPORAL' for r in reasons)}"
          f"|SOLVER_REJECTIONS={sum(r=='SOLVER' for r in reasons)}|EVENTS={len(reasons)}")

cases=[("B01","wet"),("B01","mid"),("B12","wet"),("O05","wet"),("O14","wet"),("O14","mid")]
checks=[(0.001,2e-4,5e-4),(0.01,2e-3,5e-3),(0.05,2e-2,5e-2),(0.10,2e-2,5e-2)]
for mag,strict,front in checks:
    front_ok=True
    strict_all=True
    for mat,reg in cases:
        for sign in (-1,1):
            fk=(mat,reg,sign*mag,front)
            sk=(mat,reg,sign*mag,strict)
            front_ok &= finals[fk][0]["participant_status"]==0
            strict_all &= finals[sk][0]["participant_status"]==0
    print(f"TEMPORAL02_P3B_MAGNITUDE|ABS_OFFSET_CM={mag:.6f}|STRICTER_BUDGET_CM={strict:.1e}|STRICTER_ALL_PASS={str(strict_all).upper()}"
          f"|FRONTIER_BUDGET_CM={front:.1e}|FRONTIER_ALL_PASS={str(front_ok).upper()}")
# 0.25 cm upper-bound check
upper_all=True; upper_pass=0
for mat,reg in cases:
    for sign in (-1,1):
        k=(mat,reg,sign*0.25,5e-2)
        ok=finals[k][0]["participant_status"]==0
        upper_pass+=int(ok); upper_all &= ok
print(f"TEMPORAL02_P3B_MAGNITUDE|ABS_OFFSET_CM=0.250000|TESTED_CEILING_BUDGET_CM=5.0e-2|PASS={upper_pass}|TOTAL=12|COMMON_FRONTIER_WITHIN_CEILING={str(upper_all).upper()}")
print("FPE_TEMPORAL02_P3B=PASS")
PY
