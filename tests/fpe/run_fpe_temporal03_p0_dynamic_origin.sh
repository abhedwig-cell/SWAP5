...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-temporal03-p0-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "TEMPORAL03_P0_FAIL $*" >&2; exit 1; }
...
"  public :: fgc44_temporal03_dynamic_origin_c\n",1)
needle="contains\n\n"
insert="""contains
...
h0=float(h0_s); imbalance=float(imb_s)
...
fn=swap.lib.fgc44_temporal03_dynamic_origin_c
fn.restype=ctypes.c_int
fn.argtypes=[ctypes.c_double,*([ctypes.POINTER(ctypes.c_double)]*3)]
rate=ctypes.c_double(); mass=ctypes.c_double(); head=ctypes.c_double()
status=fn(imbalance,ctypes.byref(rate),ctypes.byref(mass),ctypes.byref(head))
print("TEMPORAL03_P0_RAW|"+json.dumps({
 "material":material,"h0":h0,"imbalance":imbalance,"status":status,
 "max_abs_derivative_cm_per_day":rate.value,"mass_residual":mass.value,
 "origin_head_m":head.value,"predictor_q":predictor_q.value
},separators=(",",":")))
PY
...
for case_spec in "${cases[@]}"; do
  ...
  for imbalance in "${imbalances[@]}"; do
    for rep in $(seq 1 "$reps"); do
      raw="$(FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$imbalance")" || { printf '%s\n' "$raw" >&2; fail "$material $regime $imbalance"; }
      line="$(printf '%s\n' "$raw" | grep '^TEMPORAL03_P0_RAW|' | tail -1)"
...
for line in open(sys.argv[1]):
    if not line.startswith("TEMPORAL03_P0_RAW|"): continue
    payload,tail=line.strip().split("|REGIME=",1)
    reg,rep=tail.split("|REP=",1)
    d=json.loads(payload.split("|",1)[1]); d["regime"]=reg; d["rep"]=int(rep)
    groups[(d["material"],reg,d["imbalance"])].append(d)
if len(groups)!=12: raise SystemExit(f"expected 12 groups, got {len(groups)}")
for key,rs in sorted(groups.items()):
    sig={(r["status"],r["max_abs_derivative_cm_per_day"],r["mass_residual"],r["origin_head_m"]) for r in rs}
    if len(rs)!=3 or len(sig)!=1: raise SystemExit(f"nondeterministic dynamic origin {key}: {sig}")
    r=rs[0]
    if r["status"]!=0: raise SystemExit(f"dynamic origin failed {key}: {r}")
    if not math.isfinite(r["max_abs_derivative_cm_per_day"]) or r["max_abs_derivative_cm_per_day"]<=0:
        raise SystemExit(f"non-dynamic origin {key}")
    if abs(r["mass_residual"])>1e-12: raise SystemExit(f"mass residual {key}: {r['mass_residual']}")
    print(f"TEMPORAL03_P0_POINT|MATERIAL={key[0]}|REGIME={key[1]}|IMBALANCE={key[2]:.3f}"
          f"|MAX_ABS_DERIVATIVE_CM_PER_DAY={r['max_abs_derivative_cm_per_day']:.17e}"
          f"|MASS_RESIDUAL={r['mass_residual']:.17e}|ORIGIN_HEAD_M={r['origin_head_m']:.17e}")
print("FPE_TEMPORAL03_P0=PASS")
PY
