...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-temporal03-p1a-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "TEMPORAL03_P1A_FAIL $*" >&2; exit 1; }
...
print("TEMPORAL03_P1A_RAW|"+json.dumps({
...
PY
        ...
        for budget in "${parts[@]:1}"; do
          ...
          line="$(printf '%s\n' "$raw" | grep '^TEMPORAL03_P1A_RAW|' | tail -1)"
          [[ -n "$line" ]] || fail "missing P1A record"
...
for line in open(sys.argv[1]):
    if not line.startswith("TEMPORAL03_P1A_RAW|"): continue
    payload,reg=line.strip().split("|REGIME=")
    d=json.loads(payload.split("|",1)[1]); d["regime"]=reg; rows.append(d)
if len(rows)!=216: raise SystemExit(f"expected 216 rows, got {len(rows)}")
groups=collections.defaultdict(list)
for r in rows: groups[(abs(r["offset_cm"]),r["budget"])].append(r)
for key,rs in sorted(groups.items()):
    ok=sum(r["participant_status"]==0 for r in rs)
    print(f"TEMPORAL03_P1A_BUDGET|ABS_OFFSET_CM={key[0]:.6f}|BUDGET_CM={key[1]:.1e}|PASS={ok}|TOTAL={len(rs)}")
# Report common frontier over all 24 dynamic signed-history points at each magnitude.
for mag in (0.001,0.01,0.05):
    ...
    for b in budgets:
        ...
        if len(rs)==24 and all(r["participant_status"]==0 for r in rs):
            frontier=b; break
    print(f"TEMPORAL03_P1A_MAGNITUDE|ABS_OFFSET_CM={mag:.6f}|COMMON_BUDGET_CM={frontier if frontier is not None else 'NONE'}")
print("FPE_TEMPORAL03_P1A=PASS")
PY
