...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-temporal03-p1c-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/lib" "$BUILD/py"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "TEMPORAL03_P1C_FAIL $*" >&2; exit 1; }
...
print("TEMPORAL03_P1C_RAW|"+json.dumps({
...
PY
        ...
        for budget in "${parts[@]:1}"; do
          for rep in $(seq 1 "$reps"); do
            raw="$(FGC44_SWAP_LIB="$BUILD/lib/libswap.so" python3 "$BUILD/py/bench.py" "$material" "$h0" "$imbalance" "$offset" "$budget")" || { printf '%s\n' "$raw" >&2; fail "$material $regime $imbalance $offset $budget rep=$rep"; }
            line="$(printf '%s\n' "$raw" | grep '^TEMPORAL03_P1C_RAW|' | tail -1)"
            [[ -n "$line" ]] || fail "missing P1C record"
...
for line in open(sys.argv[1]):
    if not line.startswith("TEMPORAL03_P1C_RAW|"): continue
    payload,tail=line.strip().split("|REGIME=",1)
    reg,rep=tail.split("|REP=",1)
    d=json.loads(payload.split("|",1)[1]); d["regime"]=reg; d["rep"]=int(rep)
    groups[(d["material"],reg,d["imbalance"],d["offset_cm"],d["budget"])].append(d)
if len(groups)!=96: raise SystemExit(f"expected 96 groups, got {len(groups)}")
for key,rs in groups.items():
    if len(rs)!=3: raise SystemExit(f"expected 3 reps {key}")
    sig={(r["participant_status"],r["q"],r["max_abs_derivative_cm_per_day"],r["origin_mass_residual"]) for r in rs}
    if len(sig)!=1: raise SystemExit(f"nondeterministic P1C signature {key}: {sig}")

for mag,strict,front in ((0.001,0.1,0.2),(0.01,0.05,0.1)):
    for budget,label in ((strict,"STRICTER"),(front,"FRONTIER")):
        rs=[r for vals in groups.values() for r in vals[:1]
            if abs(abs(r["offset_cm"])-mag)<1e-12 and abs(r["budget"]-budget)<1e-15]
        ok=sum(r["participant_status"]==0 for r in rs)
        print(f"TEMPORAL03_P1C_MAGNITUDE|ABS_OFFSET_CM={mag:.6f}|ARM={label}|BUDGET_CM={budget:.1e}|PASS={ok}|TOTAL={len(rs)}")
    frontier_rows=[r for vals in groups.values() for r in vals[:1]
        if abs(abs(r["offset_cm"])-mag)<1e-12 and abs(r["budget"]-front)<1e-15]
    strict_rows=[r for vals in groups.values() for r in vals[:1]
        if abs(abs(r["offset_cm"])-mag)<1e-12 and abs(r["budget"]-strict)<1e-15]
    if len(frontier_rows)!=24 or not all(r["participant_status"]==0 for r in frontier_rows):
        raise SystemExit(f"frontier did not replicate at {mag}")
    if len(strict_rows)!=24 or all(r["participant_status"]==0 for r in strict_rows):
        raise SystemExit(f"stricter arm unexpectedly all-pass at {mag}")
print("FPE_TEMPORAL03_P1C=PASS")
PY
