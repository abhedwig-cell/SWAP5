...
SANITIZER_MODE="${FGC49D_SANITIZER_MODE:-combined}"
case "$SANITIZER_MODE" in
  address) SAN_FLAGS="address" ;;
  undefined) SAN_FLAGS="undefined" ;;
  combined) SAN_FLAGS="address,undefined" ;;
  *) echo "FGC49D_SANITIZER_MODE_FAIL $SANITIZER_MODE" >&2; exit 2 ;;
esac
export TMP_RUNNER TMP_TEST SANITIZER_MODE SAN_FLAGS
...
san_flags = os.environ["SAN_FLAGS"]
replacement = f'COMMON=(-std=f2008 -ffree-line-length-none -Wall -Wextra -fPIC -fopenmp -fcheck=all -fbacktrace -g -O0 -fsanitize={san_flags} -fno-omit-frame-pointer)'
...
runner_src = runner_src.replace(
    ...
    f'gfortran -shared -fopenmp -g -O0 -fsanitize={san_flags} -fno-omit-frame-pointer "${{objects[@]}}" -o "$OUT/libfgc49d_application.so"',
)
...
if [[ "$SAN_FLAGS" == *undefined* ]]; then
  export UBSAN_OPTIONS="print_stacktrace=1:halt_on_error=1"
fi
echo "FGC49D_SANITIZER_MODE=$SANITIZER_MODE"
bash "$TMP_RUNNER"
