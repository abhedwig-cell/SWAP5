...
root_line="  src/solver/mod_b110_root_sink_provider.f90\n"
temporal_line="  src/solver/mod_reference_richards_temporal_indicator.f90\n"
if root_line not in s or temporal_line not in s:
    raise SystemExit("HDIR02 harness repair: root/temporal compile anchor missing")
if s.index(root_line) > s.index(temporal_line):
    s=s.replace(root_line,"",1)
    s=s.replace(temporal_line,root_line+temporal_line,1)

p.write_text(s,encoding="utf-8")
print("FPE_ZERO_WASTE01_HDIR02_HISTORICAL_DEPENDENCY_REPAIRED=PASS")
print("FPE_ZERO_WASTE01_HDIR02_ROOT_SINK_ORDER_REPAIRED=PASS")
PY
