program test_fpe_profile02_backend_interval_timing
  ...
  write(*,'(A,I0,A,ES24.16,A,ES24.16,A,I0,A,I0,A,ES24.16)') &
       'PROFILE02_BACKEND_TIMING,calls=',calls,',seconds=',elapsed_seconds,',ns_per_interval=', &
  ...
  if (timing_directional) then
    write(*,'(A)') 'PROFILE02_BACKEND_TIMING_MODE=directional'
  else
    write(*,'(A)') 'PROFILE02_BACKEND_TIMING_MODE=reference'
  end if
...
end program test_fpe_profile02_backend_interval_timing
