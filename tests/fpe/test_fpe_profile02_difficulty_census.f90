program test_fpe_profile02_difficulty_census
  ...
  write(*,'(A)') 'PROFILE02_DIFFICULTY_CENSUS=PASS'
...
contains
  ...
  subroutine classify_case(material_id,se,forcing_id,top_factor,bottom_factor,coarse_dt,class_id,failed_stage)
    ...
    call emit_profile(material_id,se,forcing_id,coarse_dt,'COARSE',result,clock_start,clock_end,clock_rate)
    ...
    call solver%solve(request,workspace,result)
    call system_clock(clock_end)
    call emit_profile(material_id,se,forcing_id,coarse_dt,'HALF1',result,clock_start,clock_end,clock_rate)
    call classify_result(result,workspace,material,class_id)
    ...
    call solver%solve(request,workspace,result)
    call system_clock(clock_end)
    call emit_profile(material_id,se,forcing_id,coarse_dt,'HALF2',result,clock_start,clock_end,clock_rate)
    call classify_result(result,workspace,material,class_id)
    if (class_id/=C_ACCEPTED) then; failed_stage=S_HALF2; return; end if
    failed_stage=S_NONE
  end subroutine classify_case

  subroutine emit_profile(material_id,se,forcing_id,coarse_dt,stage,result,c0,c1,rate)
    ...
    integer(int64),intent(in) :: c0,c1,rate
    real(real64) :: seconds
    seconds=real(c1-c0,real64)/real(rate,real64)
    write(*,'(*(g0))') 'PROFILE02_DIFFICULTY|M=',trim(material_id),'|SE=',se,'|F=',trim(forcing_id), &
         '|DT=',coarse_dt,'|STAGE=',trim(stage),'|SECONDS=',seconds,'|STATUS=',result%status, &
         ...
         '|CONST=',result%diagnostics%constitutive_evaluations,'|CONST_INIT=', &
         result%diagnostics%constitutive_initial_full_evaluations,'|CONST_CAND_FULL=', &
         result%diagnostics%constitutive_candidate_full_evaluations,'|CONST_DEMAND=', &
         result%diagnostics%constitutive_candidate_demand_evaluations,'|CONST_CAP=', &
         result%diagnostics%constitutive_capacity_only_evaluations,'|CONST_TERM=', &
         result%diagnostics%constitutive_candidate_terminal_evaluations
  end subroutine emit_profile
...
end program test_fpe_profile02_difficulty_census
