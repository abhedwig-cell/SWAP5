program test_fpe_zero_waste01_groundwater_forcing_materialization
  ...
  call materializer%materialize_reused(a_head,datum,reusable,.true.,status)
  if (status /= GW_SWAP_FORCING_OK) error stop 'H-GWFORCE01 production reusable initialization failed'
  ...
  do r=1,reps
    ...
    call materializer%materialize_reused(h,datum,reusable,.false.,status)
  ...
  call require_base_fields_preserved(base,reusable)
  call materializer%materialize_reused(a_head,datum,reusable,.false.,status)
  if (status /= GW_SWAP_FORCING_OK) error stop 'H-GWFORCE01 A1 failed'
  a_bottom_first = reusable%bottom_head
  call materializer%materialize_reused(b_head,datum,reusable,.false.,status)
