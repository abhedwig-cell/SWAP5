program test_pub_p2e01_solver_seam_paired_pilot
  ...
  call require(reference_result%diagnostics%constitutive_initial_full_evaluations == 1, &
       'H04 multi-iteration initial full count')
  call require(reference_result%diagnostics%constitutive_candidate_full_evaluations == 0, &
       'H04 multi-iteration no candidate full tuple')
  call require(reference_result%diagnostics%constitutive_candidate_demand_evaluations == &
       reference_result%diagnostics%backtracking_attempts, &
       'H04 multi-iteration one candidate demand per backtracking attempt')
  call require(reference_result%diagnostics%constitutive_capacity_only_evaluations == &
       reference_result%diagnostics%nonlinear_iterations - 1, &
       'H04 multi-iteration capacity materialized only for subsequent Newton iterations')
  call require(reference_result%diagnostics%constitutive_candidate_terminal_evaluations == 1, &
       'H04 multi-iteration exactly one terminal candidate')
  call require(reference_result%diagnostics%constitutive_candidate_capacity_reuses == 0, &
       'H04 multi-iteration water-content-only candidate does not claim capacity reuse')
  write(*,'(A,I0)') 'FPE_H04_MULTI_INITIAL_FULL=', &
       reference_result%diagnostics%constitutive_initial_full_evaluations
  write(*,'(A,I0)') 'FPE_H04_MULTI_CANDIDATE_FULL=', &
       reference_result%diagnostics%constitutive_candidate_full_evaluations
  write(*,'(A,I0)') 'FPE_H04_MULTI_CANDIDATE_DEMAND=', &
       reference_result%diagnostics%constitutive_candidate_demand_evaluations
  write(*,'(A,I0)') 'FPE_H04_MULTI_CAPACITY_ONLY=', &
       reference_result%diagnostics%constitutive_capacity_only_evaluations
  write(*,'(A,I0)') 'FPE_H04_MULTI_TERMINAL=', &
       reference_result%diagnostics%constitutive_candidate_terminal_evaluations
  write(*,'(A,I0)') 'FPE_H04_MULTI_CAPACITY_REUSE=', &
       reference_result%diagnostics%constitutive_candidate_capacity_reuses
  write(*,'(A)') 'FPE_ZERO_WASTE01_H04_MULTI_DEMAND=PASS'
