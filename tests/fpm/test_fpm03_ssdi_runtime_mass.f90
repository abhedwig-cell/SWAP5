program test_fpm03_ssdi_runtime_mass
  ...
  real(real64), parameter :: t1 = 1600.875_real64
  ...
  type(irrigation_parameters_t) :: irrigation_parameters
  type(irrigation_state_t) :: irrigation_committed, irrigation_candidate
  type(irrigation_management_request_t) :: irrigation_request
  ...
  irrigation_parameters%fixed_irrigation_enabled = .true.
  irrigation_parameters%active_nodes = numnod
  irrigation_parameters%ssdi_first_node = 2
  irrigation_parameters%ssdi_last_node = 3
  allocate(irrigation_parameters%fixed_events(1))
  irrigation_parameters%fixed_events(1) = fixed_irrigation_event_t(t0, IRRIGATION_APPLICATION_SSDI, &
       0.0625_real64, 0.125_real64, 99.0_real64)
  irrigation_request%t0 = t0
  irrigation_request%t1 = t1
  call evaluate_fixed_irrigation_interval(irrigation_parameters, irrigation_committed, irrigation_request, &
       irrigation_candidate, irrigation_flux, irrigation_diag)
  call require(irrigation_diag%status == IRRIGATION_OK .and. irrigation_flux%applied, 'irrigation process event')
  call require(allocated(irrigation_flux%subsurface_source), 'irrigation source allocated')
  call require(irrigation_candidate%next_fixed_event_index == 2 .and. .not. irrigation_candidate%active_event, &
       'irrigation process candidate completion')
  ...
  tolerance = 128.0_real64 * epsilon(1.0_real64) * max(1.0_real64, abs(irrigation_flux%external_inflow_amount))
  call require(abs(total_in_delta-irrigation_flux%external_inflow_amount) <= tolerance, &
       'authoritative total_in equals process reconciliation evidence exactly once')
  call require(abs(total_out_delta-irrigation_flux%external_inflow_amount) <= tolerance, &
       'balancing drain isolates identical physics')
  ...
  write(*,'(A,1X,ES24.16)') 'FPM03_SSDI_PROCESS_EVIDENCE=', irrigation_flux%external_inflow_amount
  write(*,'(A,1X,ES24.16)') 'FPM03_SSDI_AUTHORITATIVE_TOTAL_IN_DELTA=', total_in_delta
  write(*,'(A)') 'FPM03_SSDI_AUTHORITATIVE_MASS_EXACTLY_ONCE=PASS'
  write(*,'(A)') 'FPM03_SSDI_REAL_PHYSICS_STATE_IDENTITY=PASS'
  write(*,'(A)') 'FPM03_SSDI_RUNTIME_MASS_TEST PASS'
...
contains
  ...
  subroutine configure_column(column, template)
    ...
    template%template_id = 703_int64
    template%physics_topology_id = 70301_int64
    template%vertical_layout_id = 70302_int64
    template%state_layout_id = 70303_int64
    template%solver_interface_id = 70304_int64
    template%optional_state_layout_id = 70305_int64
    template%compatible_backend_id = FMR_BACKEND_SERIALIZED_REFERENCE
    column%column_id = 703001_int64
  ...
  subroutine configure_physical_parameters(p,state,conductivity0)
    ...
    p%parameter_set_id = 70301_int64
  ...
  subroutine require(condition,label)
    ...
    if (.not. condition) then
      write(*,'(A,1X,A)') 'FPM03_SSDI_RUNTIME_FAIL', trim(label)
      error stop 1
    end if
  end subroutine require
end program test_fpm03_ssdi_runtime_mass
