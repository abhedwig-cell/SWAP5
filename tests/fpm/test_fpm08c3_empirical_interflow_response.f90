program test_fpm08c3_empirical_interflow_response
  ...
  call require(close(fd, analytic), 'analytic tangent finite difference')
  ...
  call require(close(r%signed_soil_to_drain_rate, 0.0_real64), 'activation flux zero')
  call require(.not. r%derivative_defined, 'activation full derivative unavailable')
  call require(d%singular_activation_tangent, 'sublinear activation singular')
  call require(.not. d%drainage_side_activation_tangent_defined, 'singular drainage-side tangent unavailable')
  ...
  view%groundwater_level = -100.0_real64
  call evaluate_empirical_interflow_response(p, control, view, r, d)
  call require(d%at_activation .and. .not. d%singular_activation_tangent, 'linear activation non-singular')
  call require(.not. r%derivative_defined, 'linear activation full derivative still unavailable')
  call require(d%drainage_side_activation_tangent_defined, 'linear drainage-side tangent available')
  call require(close(d%drainage_side_activation_tangent, 3.0_real64), 'linear drainage-side tangent value')
  ...
  call require(d%active .and. r%derivative_defined, 'tiny positive difference active')
  call require(close(r%dq_dgroundwater_level, expected), 'tiny positive exact tangent')
  ...
  call require(d%active .and. close(r%signed_soil_to_drain_rate, expected), 'coefficient/exponent bound flux')
  write(*,'(A)') 'FPM08C3_PARAMETER_BOUND_RESPONSE=PASS'

  p_bad = p
  p_bad%coefficient = 0.009_real64
  call evaluate_empirical_interflow_response(p_bad, control, view, r, d)
  call require(d%status == INTERFLOW_INVALID_PARAMETERS .and. .not. d%evaluated, 'coefficient lower bound')
  p_bad = p
  p_bad%coefficient = 10.001_real64
  call evaluate_empirical_interflow_response(p_bad, control, view, r, d)
  call require(d%status == INTERFLOW_INVALID_PARAMETERS, 'coefficient upper bound')
  p_bad = p
  p_bad%exponent = 0.099_real64
  call evaluate_empirical_interflow_response(p_bad, control, view, r, d)
  call require(d%status == INTERFLOW_INVALID_PARAMETERS, 'exponent lower bound')
  p_bad = p
  p_bad%exponent = 1.001_real64
  call evaluate_empirical_interflow_response(p_bad, control, view, r, d)
  call require(d%status == INTERFLOW_INVALID_PARAMETERS, 'exponent upper bound')
  ...
  call require(d%status == INTERFLOW_INVALID_HYDRAULIC_VIEW .and. .not. d%evaluated, 'nonfinite hydraulic view')
  ...
  call require(.not. same_result_bits(a1, b), 'A differs from B')
  view%groundwater_level = -10.0_real64
  ...
  call require(da1%mass_is_authoritative_external_transfer_contribution, 'mass contribution ownership')
  call require(da1%contribution_is_interflow_only, 'interflow-only scope')
...
contains
  ...
  logical function same_diag_bits(a, b) result(equal)
    ...
    equal = a%status == b%status .and. (a%evaluated .eqv. b%evaluated) .and. &
         ...
         (a%inactive_below_activation .eqv. b%inactive_below_activation) .and. &
         (a%at_activation .eqv. b%at_activation) .and. &
         ...
         same_bits(a%drain_head, b%drain_head) .and. same_bits(a%activation_difference, b%activation_difference) .and. &
