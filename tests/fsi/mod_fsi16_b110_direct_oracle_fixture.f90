...
contains
  ...
  subroutine fsi16_oracle_constitutive_evaluate(self, pressure_head, water_content, conductivity, capacity, dconductivity_dhead)
    ...
    if (self%unused_marker() /= 0) error stop 'invalid F-SI16 oracle constitutive marker'
  ...
  subroutine fsi16_oracle_source_sink_evaluate(self, pressure_head, water_content, source, sink)
    ...
    if (size(pressure_head) /= size(water_content) .or. self%unused_marker() /= 0) error stop 'invalid F-SI16 source/sink fixture'
    source = 0.0_real64
    sink = 0.0_real64
  ...
  subroutine fsi16_oracle_root_sink_evaluate(self, pressure_head, water_content, root_sink)
    ...
    if (size(pressure_head) /= size(water_content) .or. self%unused_marker() /= 0) error stop 'invalid F-SI16 root fixture'
  ...
  subroutine fsi16_oracle_top_evaluate(self, pressure_head_top, water_content_top, requested, actual_top_flux, surface_head, runoff_flux)
    ...
    if (self%unused_marker() /= 0 .or. pressure_head_top > huge(pressure_head_top) .or. &
  ...
  integer function unused_constitutive_marker(self) result(value)
    class(fsi16_oracle_constitutive_t), intent(in) :: self
    value = 0
  end function unused_constitutive_marker

  integer function unused_source_sink_marker(self) result(value)
    class(fsi16_oracle_source_sink_t), intent(in) :: self
    value = 0
  end function unused_source_sink_marker

  integer function unused_root_marker(self) result(value)
    class(fsi16_oracle_root_sink_t), intent(in) :: self
    value = 0
  end function unused_root_marker

  integer function unused_top_marker(self) result(value)
    class(fsi16_oracle_top_t), intent(in) :: self
    value = 0
  end function unused_top_marker
