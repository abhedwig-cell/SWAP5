...
BUILD="${TMPDIR:-/tmp}/swap5-fsi23-c1b-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

BASE=394d064a0dad0a7f7852b129bae99713b9aeb4c0
PLAN=integration/f-si/F-SI23_GATE_C1B_HYDRAULIC_EIGENMODE_PLAN.json
PLAN_BLOB=d5b03f1bbea416448d4b0aa84d408b51f01be79b
DRIVER=tests/fsi/test_fsi23_gate_c1b_hydraulic_eigenmode.f90
...
fail() { echo "FSI23_C1B_GATE_FAIL $*" >&2; exit 1; }
...
[[ "$(git rev-parse HEAD:$PLAN)" == "$PLAN_BLOB" ]] || fail 'C1B plan drift after freeze'
echo 'FSI23_C1B_SOURCE_AND_PLAN_LOCK=PASS'
...
echo 'FSI23_C1B_REFERENCE_TRIDAG=PASS'
...
build_and_run() {
  ...
  timeout 300s "$out/test" > "$out/output.txt" 2>&1 || {
    ...
    fail "C1B execution failed opt=$tag"
  }
  grep -Fq 'FSI23_C1B_HYDRAULIC_EIGENMODE PASS' "$out/output.txt" || {
    cat "$out/output.txt" >&2
    fail "C1B PASS marker missing opt=$tag"
  }
  [[ "$(grep -c '^FSI23_C1B_POINT:' "$out/output.txt")" -eq 5 ]] || fail "unexpected C1B point count opt=$tag"
  [[ "$(grep -c '^FSI23_C1B_EIGENPAIR:' "$out/output.txt")" -eq 1 ]] || fail "unexpected eigenpair marker count opt=$tag"
  echo "FSI23_C1B_RUN=PASS:OPT=$tag"
}
...
echo 'FSI23_C1B_O0_O2_IDENTITY=PASS'
...
git diff --quiet "$BASE" -- \
  ...
  src/adapter/mod_reference_richards_legacy_binding.f90 \
  src/runtime/mod_a23bu_worker_execution_context.f90 \
  || fail 'production source changed during C1B'
echo 'FSI23_C1B_PRODUCTION_SOURCE_UNCHANGED=PASS'
echo 'FSI23_C1B_GATE PASS'
