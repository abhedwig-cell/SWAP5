...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ftab03-serial-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
required=[
 ...
 "(parameters%bottom_mode == 2 .or. parameters%bottom_mode == 7)",
 ".not. parameters%root_extraction_active",
 ".not. parameters%snow_active",
 ".not. parameters%soil_temperature_active",
 ".not. parameters%drainage_response_active",
 ".not. parameters%black_evaporation_active",
 ".not. parameters%boesten_evaporation_active",
 ".not. self%fixed_weir_surface_water_active",
 "self%soil_water_selection%uses_reference()",
 "all(parameters%cofgen(9,1:parameters%active_nodes) == 0.0_real64)",
]
for token in required:
    assert token in admit, token
assert ".not. parameters%ksatexm_extension_active" not in admit
assert "initialize_b110_generated_mvg_table_state" not in s[s.index("subroutine fmr_serialized_advance"):s.index("end subroutine fmr_serialized_advance")]
print("F_TAB03_SERIAL_ONLY_KSATEXM_COMPOSITION_GUARD_WIDENED=PASS")
print("F_TAB03_SERIAL_ALL_OTHER_FTAB02_GUARDS_PRESERVED=PASS")
print("F_TAB03_SERIAL_GENERATION_REMAINS_OUTSIDE_TRIAL_HOTLOOP=PASS")
PY
