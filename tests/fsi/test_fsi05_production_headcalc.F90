program test_fsi05_production_headcalc
  ...
  if (failures /= 0) then
    write(*,'(A,I0)') 'F-SI05_PRODUCTION_HEADCALC FAIL failures=', failures
    error stop 1
  end if
  write(*,'(A)') 'F-SI05_PRODUCTION_HEADCALC PASS'
...
contains
  ...
  subroutine run_case(head_value, fallback, label, fingerprint, fails)
    ...
    if (fallback) then
       do i = 1, numnod
          if (workspace%band_pivots(i) /= i) fails = fails + 1
       end do
    else
       if (any(workspace%band_pivots /= 0)) fails = fails + 1
    end if
#endif
#else
    call headcalc(worker)
...
end program test_fsi05_production_headcalc
