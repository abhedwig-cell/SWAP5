program test_fsi07_real_concurrency_provider
  ...
  if (.not. any(nthreads == [1,2,4,8])) error stop 'F-SI07 provider concurrency gate requires 1/2/4/8 threads'
  ...
  evaluation%top_boundary => top_provider
  boundary%top_mode = FSI_TOP_MODE_EXPLICIT_FLUX
  boundary%bottom_mode = 7
  boundary%top_flux = -1.0_real64

  do i = 1, nthreads
    call initialize_column(serial_state(i), serial_ws(i), serial_worker(i), serial_history(i), i)
    call headcalc(serial_worker(i), serial_ws(i), serial_history(i), serial_state(i), evaluation, boundary)
...
!$omp parallel default(shared) private(i)
  ...
  if (i <= nthreads) then
    call headcalc(parallel_worker(i), parallel_ws(i), parallel_history(i), parallel_state(i), evaluation, boundary)
...
end program test_fsi07_real_concurrency_provider
