program test_fsi08_provider_context_parallel
  ...
  use mod_fsi08_provider_fixture, only: fsi08_constitutive_provider_t, fsi08_source_sink_provider_t
  implicit none

  type(soil_water_parameter_set_t), target :: params
  type(fsi08_constitutive_provider_t), target :: constitutive(8)
  ...
  if (.not. any(nthreads == [1,2,4,8])) error stop 'F-SI08 requires 1/2/4/8 workers'
  failures = 0
  call seed_shared_fixture()
  call configure_parameters()
  call configure_providers()
  ...
  if (failures /= 0) then
    write(*,'(A,I0)') 'F-SI08_PROVIDER_CONTEXT FAIL failures=', failures
    error stop 1
  end if
  write(*,'(A,I0,A)') 'F-SI08_PROVIDER_CONTEXT_', nthreads, '_PASS'

contains

  subroutine configure_parameters()
    params%parameter_set_id = 4808_int64
  ...
  subroutine configure_providers()
    integer :: j
    do j=1,8
      constitutive(j)%water_content_value = 0.30_real64
      constitutive(j)%conductivity_value = 0.50_real64 + 0.10_real64*real(j,real64)
      constitutive(j)%capacity_value = 0.0_real64
      constitutive(j)%dconductivity_dhead_value = 0.0_real64
      source_sink(j)%source_value = 0.001_real64*real(j,real64)
      source_sink(j)%sink_value = source_sink(j)%source_value
      top_provider(j)%fixed_flux = -constitutive(j)%conductivity_value
      top_provider(j)%surface_tracks_head = .true.
    end do
  ...
  subroutine seed_shared_fixture()
    ...
    ! These legacy process globals are deliberately incompatible with the admitted fixture.
    ! Any common-path read will destroy the zero residual and fail the gate.
  ...
  subroutine make_request(request, column)
    ...
    request%boundary%top_flux = top_provider(column)%fixed_flux
    request%boundary%top_head = head_value
    request%boundary%bottom_flux = -constitutive(column)%conductivity_value
    ...
    request%base_state%water_content = constitutive(column)%water_content_value
  ...
  subroutine run_serial_baseline(fails)
    ...
    integer :: j
    real(real64) :: expected_k, residual
    do j=1,nthreads
      ...
      expected_k=constitutive(j)%conductivity_value
      ...
      if(.not.same_real(serial_results(j)%top_flux,-expected_k)) fails=fails+1
      if(.not.same_real(serial_results(j)%bottom_flux,-expected_k)) fails=fails+1
      if(.not.all(abs(serial_results(j)%candidate_state%water_content-0.30_real64)<=16.0_real64*epsilon(1.0_real64))) fails=fails+1
...
!$omp end parallel
  ...
  subroutine run_aba(fails, first_fp, second_fp)
    ...
    call solver%solve(requests(min(2,nthreads)),ws,b)
    call prepare_workspace(ws,1)
...
end program test_fsi08_provider_context_parallel
