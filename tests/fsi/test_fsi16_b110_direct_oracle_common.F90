program test_fsi16_b110_direct_oracle_common
  ...
  use mod_fsi16_b110_direct_oracle_fixture, only: fsi16_oracle_constitutive_t, &
       fsi16_oracle_source_sink_t, fsi16_oracle_root_sink_t, fsi16_oracle_top_t
  implicit none
  ...
  type(fsi16_oracle_constitutive_t), target :: constitutive
  type(fsi16_oracle_source_sink_t), target :: source_sink
  type(fsi16_oracle_root_sink_t), target :: root_sink
  type(fsi16_oracle_top_t), target :: top_boundary
  ...
  request%boundary%bottom_flux = 123.0_real64
  request%boundary%bottom_head = -74.0_real64
  ...
  if (result%status /= SW_SOLVE_CONVERGED) error stop 'F-SI16 direct B1.10 oracle common route did not converge'
...
end program test_fsi16_b110_direct_oracle_common
