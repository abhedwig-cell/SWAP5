program test_fsi20_fixed_horizon_tolerance_axis
  ...
  integer, parameter :: nlevels = 11, ntol = 3
  integer, parameter :: nsteps(nlevels) = [1,2,4,8,16,32,64,128,256,512,1024]
  real(real64), parameter :: tolerances(ntol) = [1.0e-12_real64, 1.0e-13_real64, 1.0e-14_real64]
  ...
  type(soil_water_physical_state_t) :: endpoints(nlevels,ntol)
  real(real64), allocatable, target :: drainage(:,:), subsurface(:), root_sink(:)
  real(real64), allocatable :: cofgen(:,:)
  real(real64) :: max_mass_residual(nlevels,ntol), max_solver_residual(nlevels,ntol)
  real(real64) :: conductivity0, dhead, dtheta, cross_head, cross_theta
  integer :: ilevel, itol
  ...
  write(*,'(A,ES26.17E3,A,ES26.17E3,A,ES26.17E3)') &
       'FSI20_TOLAXIS_BEGIN:TOTAL_DT=', total_dt, ':H0=', initial_head_cm, ':HBOT=', predictor_bottom_head_cm
  write(*,'(A,I0,A,I0,A,ES26.17E3,A,ES26.17E3)') &
       'FSI20_TOLAXIS_NUMERICS:MAXIT=', 16, ':MAXBACK=', 8, ':DTMIN=', 1.0e-6_real64, &
       ':MASS_TOL=', hard_mass_gate

  do itol = 1, ntol
    write(*,'(A,I0,A,ES26.17E3)') 'FSI20_TOLAXIS_CASE:INDEX=', itol, ':HEAD_TOL=', tolerances(itol)
    do ilevel = 1, nlevels
      call run_trajectory(nsteps(ilevel), tolerances(itol), parameters, hydraulic_parameters, constitutive, &
           source_sink, top_provider, solver, workspace, initial_state, conductivity0, endpoints(ilevel,itol), &
           max_mass_residual(ilevel,itol), max_solver_residual(ilevel,itol))
      write(*,'(A,I0,A,ES26.17E3,A,I0,A,ES26.17E3,A,ES26.17E3)') &
           'FSI20_TOLAXIS_ENDPOINT:TOL_INDEX=', itol, ':HEAD_TOL=', tolerances(itol), &
           ':N=', nsteps(ilevel), ':MAX_MASS_RESIDUAL=', max_mass_residual(ilevel,itol), &
           ':MAX_SOLVER_RESIDUAL=', max_solver_residual(ilevel,itol)
    end do
    do ilevel = 1, nlevels-1
      dhead = maxval(abs(endpoints(ilevel,itol)%pressure_head - endpoints(ilevel+1,itol)%pressure_head))
      dtheta = maxval(abs(endpoints(ilevel,itol)%water_content - endpoints(ilevel+1,itol)%water_content))
      write(*,'(A,I0,A,ES26.17E3,A,I0,A,I0,A,ES26.17E3,A,ES26.17E3)') &
           'FSI20_TOLAXIS_COMPARE:TOL_INDEX=', itol, ':HEAD_TOL=', tolerances(itol), &
           ':N=', nsteps(ilevel), ':N2=', nsteps(ilevel+1), ':DHEAD=', dhead, ':DTHETA=', dtheta
    end do
  end do

  do itol = 2, ntol
    do ilevel = 1, nlevels
      cross_head = maxval(abs(endpoints(ilevel,1)%pressure_head - endpoints(ilevel,itol)%pressure_head))
      cross_theta = maxval(abs(endpoints(ilevel,1)%water_content - endpoints(ilevel,itol)%water_content))
      write(*,'(A,I0,A,ES26.17E3,A,I0,A,ES26.17E3,A,ES26.17E3)') &
           'FSI20_TOLAXIS_CROSS:TOL_INDEX=', itol, ':HEAD_TOL=', tolerances(itol), ':N=', nsteps(ilevel), &
           ':DHEAD_BASE_TO_STRICT=', cross_head, ':DTHETA_BASE_TO_STRICT=', cross_theta
    end do
  end do

  write(*,'(A)') 'FSI20_FIXED_HORIZON_TOLERANCE_AXIS_DRIVER PASS'
...
contains
  ...
  subroutine configure_problem(p, hp, cp, sp, tp, state, qdra, qssdi, qrot, c, k0)
    ...
    p%parameter_set_id = 202021_int64
  ...
  subroutine run_trajectory(ns, head_tol, p, hp, cp, sp, tp, s, ws, initial, k0, endpoint, max_mass, max_solver)
    integer, intent(in) :: ns
    real(real64), intent(in) :: head_tol
    ...
    call require(subdt >= 1.0e-6_real64, 'trajectory substep above F-GC02 dtmin')
    state = initial; max_mass = 0.0_real64; max_solver = 0.0_real64
    do istep = 1, ns
      ...
      request%numerical%head_abs_tolerance = head_tol
      request%numerical%head_rel_tolerance = head_tol
      request%numerical%ponding_tolerance = head_tol
      ...
      call require(result%status == SW_SOLVE_CONVERGED, 'tolerance-axis direct Richards solve converged')
  ...
  subroutine require(condition,label)
    ...
    if (.not. condition) then
      write(*,'(A,1X,A)') 'FSI20_TOLAXIS_FAIL', trim(label)
      error stop 1
    end if
  end subroutine require
end program test_fsi20_fixed_horizon_tolerance_axis
