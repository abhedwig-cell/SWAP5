program test_fsi20_hydraulic_state_refinement
  ...
  real(real64), parameter :: head_jump = 0.01_real64
  real(real64), parameter :: hard_mass_gate = 1.0e-12_real64
  integer, parameter :: nh = 3, nlevels = 10
  ...
  write(*,'(A,ES26.17E3,A,ES26.17E3,A,I0)') &
       'FSI20_STATE_REFINE_BEGIN:TOTAL_DT=', total_dt, ':HEAD_JUMP=', head_jump, ':NREF=', nsteps(nlevels)
  write(*,'(A,I0,A,I0,A,ES26.17E3,A,ES26.17E3)') &
       'FSI20_STATE_REFINE_NUMERICS:MAXIT=', 8, ':MAXBACK=', 4, ':DTMIN=', 1.0e-6_real64, &
       ':MASS_TOL=', hard_mass_gate

  do ih = 1, nh
    call characterize_state(ih, initial_heads(ih), parameters, hydraulic_parameters, constitutive, &
         source_sink, top_provider, solver, workspace)
  end do

  write(*,'(A)') 'FSI20_HYDRAULIC_STATE_REFINEMENT_DRIVER PASS'
...
contains
  ...
  subroutine configure_shared(p, hp, cp, sp, tp, qdra, qssdi, qrot, c)
    ...
    p%parameter_set_id = 202023_int64
  ...
  subroutine characterize_state(state_id, h0, p, hp, cp, sp, tp, s, ws)
    ...
    do k = 2, numnod
      call require(transfer(conductivity(k),0_int64) == transfer(conductivity(1),0_int64), &
           'uniform state-refinement initial conductivity')
    end do
    k0 = conductivity(1)
    hbot = h0 + head_jump
    ...
    write(*,'(A,I0,A,ES26.17E3,A,ES26.17E3,A,ES26.17E3)') &
         'FSI20_STATE_REFINE_STATE=', state_id, ':H0=', h0, ':HBOT=', hbot, ':K0=', k0

    do ilevel = 1, nlevels
      call run_trajectory(nsteps(ilevel), h0, hbot, k0, p, hp, cp, sp, tp, s, ws, initial, &
           endpoints(ilevel), max_mass(ilevel), max_solver(ilevel))
      write(*,'(A,I0,A,I0,A,ES26.17E3,A,ES26.17E3)') &
           'FSI20_STATE_REFINE_ENDPOINT:STATE=', state_id, ':N=', nsteps(ilevel), &
           ':MAX_MASS=', max_mass(ilevel), ':MAX_SOLVER_RES=', max_solver(ilevel)
    end do
  ...
  subroutine run_trajectory(ns, h0, hbot, k0, p, hp, cp, sp, tp, s, ws, initial, endpoint, max_mass, max_solver)
    ...
    call require(subdt >= 1.0e-6_real64, 'state-refinement trajectory above exact F-GC02 dtmin')
    state = initial
    max_mass = 0.0_real64
    ...
    do istep = 1, ns
      ...
      call require(result%status == SW_SOLVE_CONVERGED, 'state-refinement direct Richards solve converged')
      ...
      call require(abs(residual) <= hard_mass_gate, 'hard state-refinement mass gate')
  ...
  subroutine require(condition,label)
    ...
    if (.not. condition) then
      write(*,'(A,1X,A)') 'FSI20_STATE_REFINE_FAIL', trim(label)
      error stop 1
    end if
  end subroutine require
end program test_fsi20_hydraulic_state_refinement
