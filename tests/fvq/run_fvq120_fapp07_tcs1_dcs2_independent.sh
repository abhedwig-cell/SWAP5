...
SUBJECT=f05e73b9631e48d31025715bc1da9a46fdbb030d
...
COMP_TEST=tests/fvq/test_fvq120_fapp07_composition_independent.f90
...
for opt in 0 2; do
  ...
  grep -Fq 'F_VQ120_COMPOSITION_RECORDS=110' "$OUT/composition-output.txt" || fail "composition count O$opt"
  grep -Fq 'F_VQ120_COMPOSITION_SWINTER0=18' "$OUT/composition-output.txt" || fail "SWINTER0 count O$opt"
  grep -Fq 'F_VQ120_COMPOSITION_SWINTER3=92' "$OUT/composition-output.txt" || fail "SWINTER3 count O$opt"
  grep -Fq 'F_VQ120_EXACT_110_BINDING_COMPOSITION=PASS' "$OUT/composition-output.txt" || fail "composition marker O$opt"
done
...
echo "F_VQ120_PROCESS_SHA256=$(sha256sum "$BUILD/o0/process-output.txt" | awk '{print $1}')"
echo "F_VQ120_COMPOSITION_SHA256=$(sha256sum "$BUILD/o0/composition-output.txt" | awk '{print $1}')"
