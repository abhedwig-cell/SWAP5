...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-fvq123-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "F_VQ123_FAIL $*" >&2; exit 1; }

SUBJECT=ebcefbbf3d6476fead6aab08db239a7043f47ac7
SRC=src/process/mod_pmdirect_swetr0_process.f90
BIND=src/runtime/mod_fmr_pmdirect_swinter0_dynamic_top_binding.f90
TEST=tests/fvq/test_fvq123_fapp06_swinter0_independent.f90
EXACT=tests/fvq/test_fvq123_fapp06_exact_b111.f90
...
for opt in 0 2; do
  ...
  for m in F_VQ123_GENERIC_IDENTITY_SWEEP=PASS F_VQ123_DYNAMIC_TOP_PRESERVATION=PASS F_VQ123_FAIL_CLOSED=PASS; do
  ...
  grep -Fq 'F_VQ123_EXACT_B111_RECORDS=3505' "$OUT/exact-output.txt" || fail "exact record count O$opt"
  grep -Fq 'F_VQ123_EXACT_3505_PROCESS_AND_BINDING=PASS' "$OUT/exact-output.txt" || fail "exact marker O$opt"
done
...
echo "F_VQ123_GENERIC_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo "F_VQ123_EXACT_SHA256=$(sha256sum "$BUILD/o0/exact-output.txt" | awk '{print $1}')"
echo 'F_VQ123_FAPP06_INDEPENDENT=PASS'
