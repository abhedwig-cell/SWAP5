...
BUILD="${TMPDIR:-/tmp}/swap5-fvq21-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

CANDIDATE=17a70b7e21b18ba847e5882037577b27aa8afd87
CANDIDATE_TREE=59f420b898fda6fcce2bfee7c5c93ae22aa23932
[[ "$(git rev-parse "$CANDIDATE^{tree}")" == "$CANDIDATE_TREE" ]]
git diff --quiet "$CANDIDATE" -- src

echo 'FVQ21_CANDIDATE_PRODUCTION_IMMUTABILITY=PASS'

check_blob() {
  local path="$1" expected="$2" actual
  actual="$(git hash-object "$path")"
  [[ "$actual" == "$expected" ]] || {
    echo "FVQ21_BLOB_MISMATCH $path expected=$expected actual=$actual" >&2
    exit 1
  }
}
...
for opt in 0 2; do
  ...
  echo "FVQ21_ROOT_SINK_ORACLE_O${opt}=PASS"
done

cmp "$BUILD/o0/output.txt" "$BUILD/o2/output.txt"
echo 'FVQ21_ROOT_SINK_ORACLE_O0_O2_IDENTITY=PASS'
cat "$BUILD/o0/output.txt"
echo "FVQ21_OUTPUT_SHA256=$(sha256sum "$BUILD/o0/output.txt" | cut -d' ' -f1)"
echo 'FVQ21_ROOT_SINK_RUNTIME_GATE PASS'
