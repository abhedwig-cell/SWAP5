...
BUILD="${TMPDIR:-/tmp}/swap5-fvq26-v2-$$"
FSI_WORKTREE="${TMPDIR:-/tmp}/swap5-fvq26-v2-fsi16-$$"
...
echo 'FVQ26V2_SOURCE_LOCKS PASS'
...
owner = Path('tests/fmr/test_fmr11_prescribed_bottom_head_runtime.f90').read_text()
...
# Preserve the discovered audit fact: the old owner positive-sign row did not
# require trial_b completion and used a positive bottom_flux seed. V2 must not
# inherit that evidence pattern.
assert 'forcing_b%bottom_flux = 314.159_real64' in owner
assert "call require(obs_b%bottom_flux > 0.0_real64" in owner
assert "call require(trial_b%completed" not in owner
print('FVQ26V2_OWNER_POSITIVE_SIGN_EVIDENCE_DEFECT_REPRODUCED PASS')
print('FVQ26V2_STATIC_INDEPENDENT_AUDIT PASS')
PY
...
TESTS=(
  tests/fvq/test_fvq26_prescribed_bottom_head_runtime_v2.f90
...
for opt in 0 2; do
  ...
  VQ="$OUT/test_fvq26_prescribed_bottom_head_runtime_v2.txt"
  for marker in \
  ...
  echo "FVQ26V2_O${opt}=PASS"
done

cmp "$BUILD/o0/test_fvq26_prescribed_bottom_head_runtime_v2.txt" "$BUILD/o2/test_fvq26_prescribed_bottom_head_runtime_v2.txt"
cmp "$BUILD/o0/output.txt" "$BUILD/o2/output.txt"
echo 'FVQ26V2_O0_O2_OUTPUT_IDENTITY PASS'
echo 'FVQ26V2_FMR09_REGRESSION PASS'
echo 'FVQ26V2_FMR10_REGRESSION PASS'
echo 'FVQ26V2_FMR07_REGRESSION PASS'
echo 'FVQ26V2_FMR06_REGRESSION PASS'
echo 'FVQ26V2_FVQ22_REGRESSION PASS'
cat "$BUILD/o0/test_fvq26_prescribed_bottom_head_runtime_v2.txt"
echo "FVQ26V2_RUNTIME_OUTPUT_SHA256=$(sha256sum "$BUILD/o0/output.txt" | cut -d' ' -f1)"
...
echo 'FVQ26V2_FSI16_CANONICAL_B110_REPLAY PASS'
echo "FVQ26V2_FSI16_ORACLE_SHA256=$(sha256sum "$BUILD/fsi16-oracle.txt" | cut -d' ' -f1)"
echo 'FVQ26V2_INDEPENDENT_PRESCRIBED_BOTTOM_HEAD_RUNTIME_GATE PASS'
