...
BUILD="${TMPDIR:-/tmp}/swap5-fvq29-calibration-floor-aware-$$"
mkdir -p "$BUILD"
...
make_driver() {
  local target="$1" h0="$2" hb="$3" horizon="$4" depth="$5"
  cp "$DRIVER" "$target"
  python3 - "$target" "$h0" "$hb" "$horizon" "$depth" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]); h0=float(sys.argv[2]); hb=float(sys.argv[3]); horizon=float(sys.argv[4]); depth=int(sys.argv[5]); s=p.read_text()
repls={
...
if depth == 512:
    ...
    raise SystemExit(f'unsupported reference depth {depth}')
...
classify_common_reference() {
  local output="$1" h0="$2" hb="$3"
  python3 - "$output" "$h0" "$hb" <<'PY'
from pathlib import Path
import re,sys
p=Path(sys.argv[1]); h0=float(sys.argv[2]); hb=float(sys.argv[3]); eps=sys.float_info.epsilon
vals=[]
for line in p.read_text().splitlines():
    if line.startswith('FSI20_FIXED_COMPARE:'):
        m=re.search(r'DHEAD_N_N2=\s*([^:]+)', line)
        if not m: raise SystemExit('missing DHEAD_N_N2')
        vals.append(float(m.group(1)))
if len(vals)!=9:
    raise SystemExit(f'common ladder expected 9 head defects, got {len(vals)}')
floor=128.0*eps*max(1.0,abs(h0),abs(hb))
d128=vals[-2]
d256=vals[-1]
kind='FLOOR' if d128 <= floor and d256 <= floor else 'EXTENDED'
print(f'{kind}|{d128:.17e}|{d256:.17e}|{floor:.17e}')
PY
}
  ...
  for horizon in "${HORIZONS[@]}"; do
    for h0 in "${HEADS[@]}"; do
      for jump in "${JUMPS[@]}"; do
        cid=$((cid+1))
        local hb common_driver common_output decision kind d128 d256 floor final_driver
        hb="$(python3 - <<PY
print(f'{float("$h0")+float("$jump"):.17e}')
PY
)"
        common_driver="$out/case_${cid}_common.f90"
        common_output="$out/case_${cid}_common.txt"
        make_driver "$common_driver" "$h0" "$hb" "$horizon" 512
        gfortran "${COMMON[@]}" "$opt" -J "$out" -I "$out" -c "$common_driver" -o "$out/case_${cid}_common.o"
        gfortran "$opt" "${objects[@]}" "$out/case_${cid}_common.o" -o "$out/case_${cid}_common"
        timeout 300s "$out/case_${cid}_common" > "$common_output" 2>&1 || {
          cat "$common_output" >&2
          fail "common reference case $cid execution opt=$tag h0=$h0 jump=$jump horizon=$horizon"
        }
        grep -Fq 'FSI20_FIXED_HORIZON_REFERENCE_DRIVER PASS' "$common_output" || fail "case $cid common ladder missing PASS"
        decision="$(classify_common_reference "$common_output" "$h0" "$hb")"
        IFS='|' read -r kind d128 d256 floor <<< "$decision"
        if [[ "$kind" == "FLOOR" ]]; then
          cp "$common_output" "$out/case_${cid}.txt"
          printf '%s %s %s %s %s\n' "$cid" 512 "$d128" "$d256" "$floor" >> "$out/reference.tsv"
          echo "FVQ29_REFERENCE_DECISION=FLOOR_RESOLVED:OPT=$tag:CASE=$cid:D128=$d128:D256=$d256:FLOOR=$floor:REFERENCE_N=512"
        elif [[ "$kind" == "EXTENDED" ]]; then
          final_driver="$out/case_${cid}_extended.f90"
          make_driver "$final_driver" "$h0" "$hb" "$horizon" 2048
          gfortran "${COMMON[@]}" "$opt" -J "$out" -I "$out" -c "$final_driver" -o "$out/case_${cid}_extended.o"
          gfortran "$opt" "${objects[@]}" "$out/case_${cid}_extended.o" -o "$out/case_${cid}_extended"
          timeout 300s "$out/case_${cid}_extended" > "$out/case_${cid}.txt" 2>&1 || {
            cat "$out/case_${cid}.txt" >&2
            fail "extended reference case $cid execution opt=$tag h0=$h0 jump=$jump horizon=$horizon"
          }
          grep -Fq 'FSI20_FIXED_HORIZON_REFERENCE_DRIVER PASS' "$out/case_${cid}.txt" || fail "case $cid extended ladder missing PASS"
          printf '%s %s %s %s %s\n' "$cid" 2048 "$d128" "$d256" "$floor" >> "$out/reference.tsv"
          echo "FVQ29_REFERENCE_DECISION=EXTENDED_REQUIRED:OPT=$tag:CASE=$cid:D128=$d128:D256=$d256:FLOOR=$floor:REFERENCE_N=2048"
        else
          fail "unexpected reference classification case $cid: $kind"
        fi
        printf '%s %s %s %s\n' "$cid" "$h0" "$jump" "$horizon" >> "$out/cases.tsv"
        echo "FVQ29_CALIBRATION_CASE_RUN=PASS:OPT=$tag:CASE=$cid:H0=$h0:JUMP=$jump:HORIZON=$horizon:REFERENCE_KIND=$kind"
      done
    done
  done
...
cmp "$BUILD/o0/reference.tsv" "$BUILD/o2/reference.tsv" || fail 'O0/O2 reference-depth decision drift'
...
refs={}
for line in (root/'reference.tsv').read_text().splitlines():
    cid,nref,d128,d256,floor=line.split()
    refs[int(cid)]=(int(nref),float(d128),float(d256),float(floor))
...
for cid,h0,jump,horizon in meta:
    nref,d128_common,d256_common,floor=refs[cid]
    ...
    expected_endpoints=10 if nref==512 else 12
    expected_comps=9 if nref==512 else 11
    if len(endpoints)!=expected_endpoints or len(comps)!=expected_comps:
        raise SystemExit(f'case {cid}: reference {nref}, expected {expected_endpoints} endpoints/{expected_comps} compares got {len(endpoints)}/{len(comps)}')
    if endpoints[-1][0] != nref or comps[-1][1] != nref:
        raise SystemExit(f'case {cid}: final reference level mismatch {nref}')
    floor_expected=128.0*sys.float_info.epsilon*max(1.0,abs(h0),abs(h0+jump))
    if floor != floor_expected:
        raise SystemExit(f'case {cid}: frozen floor drift {floor} != {floor_expected}')
    floor_pair=(d128_common<=floor and d256_common<=floor)
    if nref==512 and not floor_pair:
        raise SystemExit(f'case {cid}: N512 reference without frozen two-defect floor condition')
    if nref==2048 and floor_pair:
        raise SystemExit(f'case {cid}: N2048 reference despite frozen floor resolution')
    if nref==512: ref512+=1
    else: ref2048+=1
    ...
    steps=[x[0] for x in comps]
    if not all(math.isfinite(x) and x>=0 for x in d+e2): raise SystemExit(f'case {cid}: invalid head metric')
    cases.append(dict(cid=cid,h0=h0,jump=jump,horizon=horizon,d=d,e2=e2,steps=steps,floor=floor,nref=nref))

selected=None
for nmin in candidates:
    ratios=[]
    for c in cases:
        idx=c['steps'].index(nmin)
    ...
    for c in cases:
        ...
        if dN<=c['floor']:
            floor_cases+=1
            if err>c['floor']:
                failures.append((c['cid'],'floor',dN,err,c['floor'],c['nref']))
            continue
        resolved_cases+=1
        bound=mult*dN
        min_margin=min(min_margin,bound-err)
        if bound < err:
            failures.append((c['cid'],'bound',bound,err,c['floor'],c['nref']))
    status='PASS' if not failures else 'FAIL'
    margin_text=f'{min_margin:.17e}' if math.isfinite(min_margin) else 'NA'
    print(f'FVQ29_CALIBRATION_CANDIDATE:NMIN={nmin}:QCAP={qcap:.17e}:MULT={mult:.17e}:RESOLVED_CASES={resolved_cases}:FLOOR_CASES={floor_cases}:FAILURES={len(failures)}:MIN_BOUND_MINUS_ERROR={margin_text}:STATUS={status}')
    for f in failures[:8]:
...
for horizon in sorted({c['horizon'] for c in cases}):
    ...
    qtext=f'{max(rs):.17e}' if rs else 'NO_RESOLVED_RATIO'
    print(f'FVQ29_CALIBRATION_HORIZON_QMAX:HORIZON={horizon:.17e}:QMAX={qtext}')
for h0 in sorted({c['h0'] for c in cases}):
    ...
    for c in cases:
        ...
        for j in range(idx,len(c['d'])-1):
            ...
            if a>c['floor'] and b>c['floor']: rs.append(b/a)
    qtext=f'{max(rs):.17e}' if rs else 'NO_RESOLVED_RATIO'
    print(f'FVQ29_CALIBRATION_STATE_QMAX:H0={h0:.17e}:QMAX={qtext}')
floor_tails=[c['d'][-1] for c in cases if c['nref']==512]
extended_tails=[c['d'][-1] for c in cases if c['nref']==2048]
print(f'FVQ29_CALIBRATION_REFERENCE_N512_CASES={ref512}')
print(f'FVQ29_CALIBRATION_REFERENCE_N2048_CASES={ref2048}')
print(f'FVQ29_CALIBRATION_MAX_FLOOR_X256_X512_DHEAD={(max(floor_tails) if floor_tails else 0.0):.17e}')
print(f'FVQ29_CALIBRATION_MAX_EXTENDED_X1024_X2048_DHEAD={(max(extended_tails) if extended_tails else 0.0):.17e}')
print(f'FVQ29_CALIBRATION_MAX_MASS_RESIDUAL={max_mass:.17e}')
margin_text=f'{min_margin:.17e}' if math.isfinite(min_margin) else 'NA'
print(f'FVQ29_CALIBRATION_SELECTED:NMIN={nmin}:QCAP={qcap:.17e}:MULT={mult:.17e}:RESOLVED_CASES={resolved_cases}:FLOOR_CASES={floor_cases}:MIN_BOUND_MINUS_ERROR={margin_text}')
print('FVQ29_CALIBRATION_REFERENCE_PROTOCOL=FLOOR_AWARE_COMMON_N512_EXTEND_ONLY_IF_REQUIRED')
