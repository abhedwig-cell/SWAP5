...
BUILD="${TMPDIR:-/tmp}/swap5-fvq31-disjoint-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

PLAN=integration/f-vq/F-VQ31_QUALIFICATION_PLAN.json
PLAN_BLOB=6695f9be69c4485e24ebcba802927ce90af8beac
...
fail() { echo "FVQ31_DISJOINT_FAIL $*" >&2; exit 1; }

[[ "$(git rev-parse HEAD:$PLAN)" == "$PLAN_BLOB" ]] || fail 'frozen F-VQ31 plan drift'
...
with open(sys.argv[2],'w') as f:
    for c in m['cases']:
        ...
        assert t not in fsi24 and t not in fvq30 and t not in fkt10 and t not in seen
        seen.add(t)
        f.write(f"{c['case']}\t{t[0]:.17e}\t{t[1]:.17e}\t{t[2]:.17e}\n")
print('FVQ31_G01_PLAN_AND_MATRIX_LOCK=PASS')
print('FVQ31_G06_DISJOINT_CASES=12')
print('FVQ31_G06_EXACT_TRIPLET_OVERLAP_PRIOR=0')
PY
...
assert '(solve_result%candidate_state%pressure_head-request%base_state%pressure_head)/dt' in indicator
...
assert 'call physical%temporal_history%replace(indicator_result%current_right_derivative' in backend
assert 'outcome%temporal_certificate_available = .true.' not in backend
print('FVQ31_G04_SAME_PRODUCTION_INDICATOR_SERVICE=PASS')
print('FVQ31_G06_CURRENT_DERIVATIVE_STRUCTURAL_EQUIVALENCE=PASS')
print('FVQ31_G10_NO_FKT09_CERTIFICATE_PROMOTION=PASS')
PY
...
echo 'FVQ31_G04_REFERENCE_TRIDAG_SOURCE=PASS'
...
if s.count(old)!=1: raise SystemExit('FVQ31 direct-driver dt token drift')
...
row=[x for x in lines if x.startswith('FSI25_PROD_ROW:')]
...
PY
    ...
    echo "FVQ31_G06_CASE_RUN=PASS:OPT=$tag:CASE=$cid:H0=$h0:JUMP=$jump:DT=$dt:DIRECT_BINF=$direct_binf"
...
for tag in ('o0','o2'):
  ...
  for cid in range(1,13):
    ...
    if not math.isfinite(direct) or not math.isfinite(tx) or diff>65536.0*sys.float_info.epsilon*scale:
    ...
    rows.append((cid,direct,tx,diff,dmass,tmass,dr[0].group(8)))
  summary[tag]=rows
if summary['o0']!=summary['o2']:
  raise SystemExit('O0/O2 normalized disjoint summary drift')
maxdiff=max(r[3] for r in summary['o0']); maxmass=max(max(r[4],r[5]) for r in summary['o0'])
for r in summary['o0']:
  print(f'FVQ31_G06_ROW:CASE={r[0]}:DIRECT_BINF={r[1]:.17e}:TX_BINF={r[2]:.17e}:ABS_DIFF={r[3]:.17e}:DIRECT_MASS={r[4]:.17e}:TX_MASS={r[5]:.17e}:ROUTE={r[6]}')
print('FVQ31_G06_DIRECT_TX_CASES=12')
print(f'FVQ31_G06_MAX_BINF_ABS_DIFF={maxdiff:.17e}')
print(f'FVQ31_G08_MAX_MASS={maxmass:.17e}')
print('FVQ31_G07_BOUNDED_COST=PASS')
print('FVQ31_G08_HARD_MASS_NONINTERFERENCE=PASS')
print('FVQ31_G09_O0_O2_IDENTITY=PASS')
print('FVQ31_G06_DISJOINT_COMPOSITION_EQUIVALENCE=PASS')
PY

echo 'FVQ31_DISJOINT_TRANSACTION_COMPOSITION PASS'
