...
fail() { echo "FVQ66_PRESERVATION_FAIL:$*" >&2; exit 1; }
COMMON=(-std=f2008 -ffree-line-length-none -Wall -Wextra -fcheck=all -fbacktrace -ffpe-trap=invalid,zero,overflow)

# F-KT15 production solver-service/reference-model transport, rebuilt against
# the immutable F-KT18 candidate. The historical fixture is input only.
...
echo 'FVQ66_FKT15_SOLVER_SERVICE_REFERENCE_MODEL_PRESERVATION=PASS'
echo 'FVQ66_FKT15R_CURRENT_RECOMPOSED_TRANSPORT_SURFACE=PASS'

# F-KT16 state/persistence/restart plus Serialized MultiSWAP continuation.
RESTART_SRC=(
...
echo 'FVQ66_FKT16_STATE_PERSISTENCE_RESTART_PRESERVATION=PASS'
echo 'FVQ66_SERIALIZED_MULTISWAP_PRESERVATION=PASS'
echo 'FVQ66_GENERIC_TIME_ROLLBACK_RETRY_RESTART_MASS_TRANSPORT=PASS'

# Production parallel runtime against immutable independently-qualified FMQ fixtures.
...
echo 'FVQ66_PRODUCTION_PARALLEL_RUNTIME_PRESERVATION=PASS'

# F-VQ64 independently qualified restricted predictor/corrector replay.
...
echo 'FVQ66_FVQ64_PREDICTOR_CORRECTOR_PRESERVATION=PASS'

echo 'FVQ66_CURRENT_PRESERVATION_GATE=PASS'
