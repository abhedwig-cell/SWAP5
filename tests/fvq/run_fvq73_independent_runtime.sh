...
BUILD="${TMPDIR:-/tmp}/swap5-fvq73-runtime-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail(){ echo "FVQ73_INDEPENDENT_RUNTIME_GATE_FAIL $*" >&2; exit 73; }
...
for opt in 0 2; do
  ...
  gfortran "${COMMON[@]}" -O"$opt" -J "$OUT" -I "$OUT" -c \
    tests/fvq/test_fvq73_fpm14_drainage_response_independent.f90 -o "$OUT/test.o"
  gfortran -O"$opt" "${objects[@]}" "$OUT/test.o" -o "$OUT/test"
  "$OUT/test" > "$OUT/output.txt" 2>&1 || {
    cat "$OUT/output.txt" >&2
    fail "independent test execution O$opt"
  }
  for marker in \
  ...
  echo "FVQ73_INDEPENDENT_RUNTIME_O${opt}=PASS"
done
...
echo 'FVQ73_INDEPENDENT_RUNTIME_O0_O2_EXACT_IDENTITY=PASS'
echo "FVQ73_INDEPENDENT_RUNTIME_OUTPUT_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
cat "$BUILD/o0/output.txt"
echo 'FVQ73_INDEPENDENT_RUNTIME_GATE=PASS'
