program test_fvq123_fapp06_swinter0_independent
  ...
  print '(A)','F_VQ123_GENERIC_IDENTITY_SWEEP=PASS'
  print '(A)','F_VQ123_DYNAMIC_TOP_PRESERVATION=PASS'
  print '(A)','F_VQ123_FAIL_CLOSED=PASS'
contains
  ...
  subroutine require(ok,n)
    ...
    if(.not.ok) then
      write(*,'(A,I0)') 'F_VQ123_FAIL=',n
      error stop 1
    end if
  end subroutine
end program test_fvq123_fapp06_swinter0_independent
