...
contains
  ...
  subroutine test_outer_selection_and_nonmidnight_time()
    ...
    call evaluate_scheduled_irrigation_interval(p, committed, req, view, candidate, flux, diag)
    call require(diag%status == IRRIGATION_OK .and. .not. diag%selection_evaluated, 'fixed precedence blocks scheduled selection')
    ...
    call evaluate_scheduled_irrigation_interval(p, committed, req, view, candidate, flux, diag)
    call require(diag%status == IRRIGATION_OK .and. diag%selection_evaluated, 'explicit opportunity evaluated')
    ...
    call require(.not. candidate%active_event .and. candidate%active_event_origin == IRRIGATION_EVENT_NONE, &
         'full event completes without persistent state')
  ...
  subroutine test_tcs7_dcs2_source_oracle()
    ...
    call evaluate_scheduled_irrigation_interval(p, committed, req, view, candidate, flux, diag)
    call require(diag%status == IRRIGATION_OK .and. diag%selection_evaluated .and. diag%triggered, &
         'TCS7 equality must trigger')
    call require(same_bits(diag%interpolated_threshold,expected_threshold), 'TCS7 linear interpolation')
    call require(same_bits(diag%interpolated_depth,expected_depth), 'DCS2 translated-cm interpolation')
    call require(same_bits(flux%event_duration,expected_duration), 'legacy duration depth/rate')
    call require(same_bits(flux%active_duration,expected_duration), 'full event active duration')
    call require(same_bits(flux%external_inflow_amount,expected_depth), 'single-node full-event mass equals depth')
    call require(allocated(flux%subsurface_source), 'SSDI source allocated')
    call require(size(flux%subsurface_source) == p%active_nodes, 'SSDI source shape')
    call require(same_bits(flux%subsurface_source(p%single_ssdi_node),expected_rate), 'qssdi node equals rate')
    call require(sum_nonselected_abs(flux%subsurface_source,p%single_ssdi_node) == 0.0_real64, 'only one SSDI node active')

    call configure_view(view, p%active_nodes, p%sensor_node, expected_threshold + 1.0_real64)
    call evaluate_scheduled_irrigation_interval(p, committed, req, view, candidate, flux, diag)
    call require(diag%status == IRRIGATION_OK .and. diag%selection_evaluated .and. .not. diag%triggered, &
         'head above threshold must not trigger')
  ...
  subroutine test_afgen_boundary_oracle()
    ...
    call evaluate_scheduled_irrigation_interval(p, committed, req, view, candidate, flux, diag)
    call require(diag%status == IRRIGATION_OK .and. diag%triggered, 'below-first clamp triggers at first value')
    call require(same_bits(diag%interpolated_threshold,-100.0_real64), 'AFGEN below-first threshold clamp')
    call require(same_bits(diag%interpolated_depth,0.20_real64), 'AFGEN below-first depth clamp')
    ...
    call evaluate_scheduled_irrigation_interval(p, committed, req, view, candidate, flux, diag)
    call require(diag%status == IRRIGATION_INVALID_PARAMETERS, 'partial table above last must fail closed')
    call require(.not. flux%applied .and. state_equal(committed,candidate), 'partial above-last no water/state')

    p%tcs7_knot_count = 7
    p%dcs2_knot_count = 7
    p%tcs7_dvs = 0.0_real64
    p%tcs7_pressure_head = 0.0_real64
    p%dcs2_dvs = 0.0_real64
    p%dcs2_depth_cm = 0.0_real64
    do i = 1, 7
    ...
    call require(same_bits(diag%interpolated_threshold,-50.0_real64), 'full table final threshold clamp')
    call require(same_bits(diag%interpolated_depth,0.70_real64), 'full table final depth clamp')
  ...
  subroutine test_continuation_and_no_implicit_retrigger()
    ...
    type(irrigation_flux_result_t) :: flux
    type(irrigation_diagnostics_t) :: diag
    real(real64), parameter :: t0 = 1700.375_real64
    ...
    call require(same_bits(active%active_event_end,t0+duration), 'persist event end')

    req = scheduled_irrigation_request_t()
    req%t0 = t0 + 0.10_real64
    req%t1 = t0 + duration
    req%dvs = 999.0_real64
    req%selection_opportunity = .false.
    call evaluate_scheduled_irrigation_interval(p, active, req, unused_view, finished, flux, diag)
    call require(diag%status == IRRIGATION_OK .and. flux%event_finished, 'continuation completes without selection/view')
    call require(.not. finished%active_event, 'completion clears active event')
    call require(same_bits(flux%active_duration,0.15_real64), 'continuation duration from persisted event')

    req = scheduled_irrigation_request_t()
    req%t0 = t0 + duration
    req%t1 = t0 + duration + 0.10_real64
    req%dvs = 0.5_real64
    ...
    call require(diag%status == IRRIGATION_OK .and. diag%selection_evaluated .and. diag%triggered, &
         'later explicit opportunity may retrigger')
  ...
  subroutine test_split_rollback_replay()
    ...
    call require(same_bits(da%split_time,t0+0.25_real64), 'split at source-derived event end')
    call require(state_equal(committed,ca) .and. state_equal(committed,cb), 'split leaves committed candidate unchanged')
    call require(.not. fa%applied .and. .not. allocated(fa%subsurface_source), 'split adds no water')
  ...
  subroutine test_a_b_a()
    ...
    pb = pa
    pb%tcs7_pressure_head(1:pb%tcs7_knot_count) = -120.0_real64
    ...
    call require(fa1%applied .and. .not. fb%applied .and. fa2%applied, 'A/B/A distinguishes threshold sets')
    call require(state_equal(ca1,ca2) .and. flux_equal(fa1,fa2) .and. diag_equal(da1,da2), 'A replay exact after B')
  ...
  subroutine configure_three_knot_parameters(p)
    ...
    ! 1.0 mm/h * 0.1 cm/mm * 24 h/d = 2.4 cm/d.
    ...
    ! Legacy FID values [2,4,6] mm are translated before the process to [0.2,0.4,0.6] cm.
  ...
  subroutine configure_view(view,n,sensor,head_sensor)
    ...
    view%pressure_head = -75.0_real64
    view%pressure_head(sensor) = head_sensor
  ...
  logical function state_equal(a,b)
    ...
    state_equal = a%next_fixed_event_index == b%next_fixed_event_index .and. &
                  a%active_event .eqv. b%active_event .and. &
                  a%active_event_origin == b%active_event_origin .and. &
                  a%active_event_index == b%active_event_index .and. &
                  same_bits(a%active_event_start,b%active_event_start) .and. &
  ...
  logical function flux_equal(a,b)
    ...
    flux_equal = a%applied .eqv. b%applied
    flux_equal = flux_equal .and. (a%event_started .eqv. b%event_started)
    flux_equal = flux_equal .and. (a%event_finished .eqv. b%event_finished)
    flux_equal = flux_equal .and. (a%event_remains_active .eqv. b%event_remains_active)
    flux_equal = flux_equal .and. a%event_origin == b%event_origin .and. a%event_index == b%event_index
    flux_equal = flux_equal .and. a%application_type == b%application_type
    flux_equal = flux_equal .and. same_bits(a%event_duration,b%event_duration)
    flux_equal = flux_equal .and. same_bits(a%active_duration,b%active_duration)
    flux_equal = flux_equal .and. same_bits(a%surface_gross_rate,b%surface_gross_rate)
    flux_equal = flux_equal .and. same_bits(a%external_inflow_amount,b%external_inflow_amount)
  ...
  logical function diag_equal(a,b)
    ...
    diag_equal = a%status == b%status .and. (a%event_match .eqv. b%event_match) .and. &
                 ...
                 (a%selection_evaluated .eqv. b%selection_evaluated) .and. &
                 (a%triggered .eqv. b%triggered) .and. &
                 same_bits(a%interpolated_threshold,b%interpolated_threshold) .and. &
                 same_bits(a%interpolated_depth,b%interpolated_depth)
