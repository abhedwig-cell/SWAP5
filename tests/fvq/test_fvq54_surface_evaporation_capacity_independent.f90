program test_fvq54_surface_evaporation_capacity_independent
  ...
  do family = 1, nfamily
    ...
    ! Independent constitutive oracle. Expected values never call candidate code.
    do ih = 1, size(heads)
      call evaluate_b110_default_mvg_conductivity(candidate_hydraulics, 1, heads(ih), scalar_k, ok)
      call require(ok, 541)
      call require_close(scalar_k, oracle_hconduc(oracle_c(:,1), heads(ih)), 542)
      conductivity_cases = conductivity_cases + 1
    end do

    k_atm = oracle_hconduc(oracle_c(:,1), hatm)
    call require(k_atm >= 0.0_real64, 543)
    ...
    do method = 1, 6
      ...
      do ih = 1, size(heads)
        ...
        call require(result%status == SURFACE_EVAP_CAPACITY_AVAILABLE, 545)
        ...
        call require_close(result%evaporation_capacity, expected, 546)

        call require(all_same_real_bits(state%pressure_head, head_before), 547)
        call require(all_same_real_bits(state%water_content, theta_before), 548)
        call require(same_real_bits(state%ponding_depth, pond_before), 549)
        call require(same_real_bits(state%groundwater_level, gw_before), 550)
      ...
      call provider%evaluate(state, result)
      call require(result%status == SURFACE_EVAP_CAPACITY_AVAILABLE, 551)
      call require(abs(result%evaporation_capacity) <= &
           8192.0_real64*epsilon(1.0_real64)*max(1.0_real64,k_atm), 552)

      state%pressure_head(1) = -300000.0_real64
      call provider%evaluate(state, result)
      call require(result%status == SURFACE_EVAP_CAPACITY_AVAILABLE, 553)
      call require(result%evaporation_capacity < 0.0_real64, 554)
    end do
  end do

  call require(oracle_cases == nfamily*6*size(heads), 555)
  call require(conductivity_cases == nfamily*size(heads), 556)
  ...
  call require(result%status == SURFACE_EVAP_CAPACITY_INVALID_INPUT, 557)
  call require(same_real_bits(result%evaporation_capacity, 0.0_real64), 558)

  call bind_b110_surface_evaporation_capacity_provider(provider, geometry, candidate_hydraulics, &
       7, .false., .false., ok)
  call require(ok, 559)
  call provider%evaluate(state, result)
  call require(result%status == SURFACE_EVAP_CAPACITY_UNSUPPORTED_CONFIGURATION, 560)
  call require(same_real_bits(result%evaporation_capacity, 0.0_real64), 561)

  call bind_b110_surface_evaporation_capacity_provider(provider, geometry, candidate_hydraulics, &
       0, .false., .false., ok)
  call provider%evaluate(state, result)
  call require(result%status == SURFACE_EVAP_CAPACITY_UNSUPPORTED_CONFIGURATION, 562)

  call bind_b110_surface_evaporation_capacity_provider(provider, geometry, candidate_hydraulics, &
       1, .true., .false., ok)
  call provider%evaluate(state, result)
  call require(result%status == SURFACE_EVAP_CAPACITY_UNSUPPORTED_CONFIGURATION, 563)

  call bind_b110_surface_evaporation_capacity_provider(provider, geometry, candidate_hydraulics, &
       1, .false., .true., ok)
  call provider%evaluate(state, result)
  call require(result%status == SURFACE_EVAP_CAPACITY_UNSUPPORTED_CONFIGURATION, 564)

  call bind_b110_surface_evaporation_capacity_provider(provider, geometry, candidate_hydraulics, &
       1, .false., .false., ok)
  state%pressure_head(1) = nanv
  call provider%evaluate(state, result)
  call require(result%status == SURFACE_EVAP_CAPACITY_INVALID_INPUT, 565)
  state%pressure_head(1) = -100.0_real64

  geometry%node_distance(1) = 0.0_real64
  call provider%evaluate(state, result)
  call require(result%status == SURFACE_EVAP_CAPACITY_INVALID_INPUT, 566)
  geometry%node_distance(1) = 2.75_real64

  ! Ponding classification remains downstream. Hydraulic capacity must not depend on finite ponding depth.
  state%ponding_depth = 0.0_real64
  call provider%evaluate(state, a1)
  state%ponding_depth = 12.5_real64
  call provider%evaluate(state, a2)
  call require(a1%status == SURFACE_EVAP_CAPACITY_AVAILABLE, 567)
  call require(a2%status == SURFACE_EVAP_CAPACITY_AVAILABLE, 568)
  call require_close(a1%evaporation_capacity, a2%evaporation_capacity, 569)

  ! A-B-A replay checks absence of hidden evolving provider state.
  state%ponding_depth = 0.0_real64
  ...
  state%pressure_head(1) = -37.0_real64
  call provider%evaluate(state, a2)
  call require(a1%status == SURFACE_EVAP_CAPACITY_AVAILABLE, 570)
  call require(b%status == SURFACE_EVAP_CAPACITY_AVAILABLE, 571)
  call require(a2%status == a1%status, 572)
  call require_close(a2%evaporation_capacity, a1%evaporation_capacity, 573)

  write(*,'(A,I0)') 'FVQ54_ORACLE_CASES=', oracle_cases
  write(*,'(A,I0)') 'FVQ54_CONDUCTIVITY_CASES=', conductivity_cases
  write(*,'(A)') 'FVQ54_B110_CONSTITUTIVE_ORACLE=PASS'
  write(*,'(A)') 'FVQ54_HCOMEAN_1_TO_6_ORACLE=PASS'
  write(*,'(A)') 'FVQ54_SIGNED_EMAX_ORACLE=PASS'
  write(*,'(A)') 'FVQ54_HELD_SCOPE_FAIL_CLOSED=PASS'
  write(*,'(A)') 'FVQ54_STATELESS_READ_ONLY_ABA=PASS'
  write(*,'(A)') 'FVQ54_INDEPENDENT_ORACLE=PASS'
...
contains
  ...
  subroutine require_close(actual, expected_value, code)
    ...
    if (abs(actual-expected_value) > 8192.0_real64*epsilon(1.0_real64)*scale) then
      write(*,'(A,I0,2(1X,ES24.16))') 'FVQ54_NUMERIC_MISMATCH ', code, actual, expected_value
      error stop 54
    end if
  ...
  subroutine require(condition, code)
    ...
    if (.not. condition) then
      write(*,'(A,I0)') 'FVQ54_FAIL ', code
      error stop 54
    end if
  end subroutine require

end program test_fvq54_surface_evaporation_capacity_independent
