program test_fvq66_mass_completeness_attack
  ...
  type, extends(transaction_state_t) :: vq66_state_t
    real(real64) :: water = 0.0_real64
    integer(int64) :: sentinel = 0_int64
  contains
    procedure :: clone => vq66_clone
  end type vq66_state_t

  type, extends(transaction_model_t) :: vq66_model_t
    integer :: scenario = SC_GOOD
    integer :: advance_calls = 0
  contains
    procedure :: advance => vq66_advance
    procedure :: storage => vq66_storage
    procedure :: temporal_error => vq66_temporal_error
    procedure :: storage_accounting_status => vq66_storage_accounting_status
  end type vq66_model_t
  ...
  print '(a)', 'FVQ66_ZERO_RESIDUAL_INCOMPLETE_FAIL_CLOSED=PASS'
  print '(a)', 'FVQ66_NONZERO_MISSING_MASK_FAIL_CLOSED=PASS'
  print '(a)', 'FVQ66_COMPLETE_IN_TOLERANCE_ACCEPTS=PASS'
  print '(a)', 'FVQ66_COMPLETE_OUTSIDE_TOLERANCE_REJECTS=PASS'
  print '(a)', 'FVQ66_NONFINITE_MASS_FAIL_CLOSED=PASS'
  print '(a)', 'FVQ66_REJECTED_COMMITTED_STATE_BITWISE_IMMUTABLE=PASS'
  print '(a)', 'FVQ66_RETRY_NO_PHYSICAL_ACCUMULATION=PASS'
  print '(a)', 'FVQ66_RETRY_EXHAUSTION_FAIL_CLOSED=PASS'
  print '(a)', 'FVQ66_INCOMPLETE_CANNOT_MATERIALIZE_ACCEPTED_CANDIDATE=PASS'
  print '(a)', 'FVQ66_NORMAL_COMPLETE_LEDGER_PRESERVATION=PASS'
  print '(a)', 'FVQ66_ALTERNATIVE_SOLVER_INCOMPLETE_FAIL_CLOSED=PASS'
  print '(a)', 'FVQ66_SECOND_HALF_INCOMPLETE_FAIL_CLOSED=PASS'
  print '(a)', 'FVQ66_MASS_RESULT_TRANSPORT=PASS'
  print '(a)', 'FVQ66_GENERIC_TIME_TRANSACTION=PASS'
  print '(a)', 'FVQ66_ATTACK_MATRIX=PASS'
...
contains
  ...
  subroutine assert_true(condition, label)
    ...
    if (.not. condition) then
      write(*,'(a)') 'FVQ66_ASSERT_FAIL=' // trim(label)
      error stop 66
    end if
  ...
  subroutine allocate_state(state, water, sentinel)
    ...
    allocate(vq66_state_t :: state)
    select type (state)
    type is (vq66_state_t)
      state%water = water
  ...
  subroutine reject_case(scenario, temporal_mode, label, expect_alternative)
    ...
    call assert_true(result%accepted_dt == 0.0_real64, trim(label)//':accepted_dt')
    ...
    write(*,'(a)') 'FVQ66_CASE_' // trim(label) // '=PASS'
  ...
  subroutine vq66_clone(self, copy)
    class(vq66_state_t), intent(in) :: self
    class(transaction_state_t), allocatable, intent(out) :: copy
    allocate(vq66_state_t :: copy)
    select type (copy)
    type is (vq66_state_t)
  ...
  end subroutine vq66_clone

  subroutine vq66_advance(self, state, t0, t1, outcome)
    class(vq66_model_t), intent(inout) :: self
  ...
  end subroutine vq66_advance

  function vq66_storage(self, state) result(value)
    class(vq66_model_t), intent(in) :: self
    class(transaction_state_t), intent(in) :: state
    real(real64) :: value
    if (self%scenario < SC_GOOD) error stop 66
    select type (state)
    type is (vq66_state_t)
      value = state%water
    class default
  ...
  end function vq66_storage

  subroutine vq66_storage_accounting_status(self, state, complete, missing_mask)
    class(vq66_model_t), intent(in) :: self
    class(transaction_state_t), intent(in) :: state
    logical, intent(out) :: complete
    integer(int64), intent(out) :: missing_mask
    if (self%scenario < SC_GOOD .or. .not. same_type_as(state, state)) error stop 66
    complete = .true.
    missing_mask = TX_MASS_MISSING_NONE
  end subroutine vq66_storage_accounting_status

  function vq66_temporal_error(self, full_state, half_state) result(value)
    class(vq66_model_t), intent(in) :: self
    class(transaction_state_t), intent(in) :: full_state, half_state
    real(real64) :: value
    if (self%scenario < SC_GOOD .or. .not. same_type_as(full_state, full_state) .or. &
        .not. same_type_as(half_state, half_state)) error stop 66
    value = 0.0_real64
  end function vq66_temporal_error

end program test_fvq66_mass_completeness_attack
