...
BUILD="${TMPDIR:-/tmp}/swap5-fwof41-production-owner-persistence-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

# Preserve the complete F-WOF39 lifecycle/atomic publication behavior against
# the production owner modules that now contain the persistence API.
bash tests/fwof/run_fwof39_crop_event_lifecycle_gate.sh

# Prove that the production owner source is exactly the candidate source that
# F-WOF40 qualified. The candidate is regenerated from the definitive F-WOF40
# tested head, never from the already-materialized production source.
FWO40=2394431312932df27e52d77b25475f72f11fa38d
git show "$FWO40:src/runtime/mod_fmr_wofost_accepted_window_lineage.f90" > "$BUILD/fwof40_lineage.f90"
git show "$FWO40:src/runtime/mod_fmr_wofost_crop_transaction.f90" > "$BUILD/fwof40_transaction.f90"
python3 tools/fwof40_materialize_persistence_candidate.py \
  "$BUILD/fwof40_lineage.f90" \
  "$BUILD/fwof40_transaction.f90" \
  "$BUILD/expected_lineage.f90" \
  "$BUILD/expected_transaction.f90" > "$BUILD/materialize.txt"
grep -Fq 'FWOF40_OWNER_PERSISTENCE_CANDIDATE_MATERIALIZED=PASS' "$BUILD/materialize.txt"
cmp "$BUILD/expected_lineage.f90" src/runtime/mod_fmr_wofost_accepted_window_lineage.f90
cmp "$BUILD/expected_transaction.f90" src/runtime/mod_fmr_wofost_crop_transaction.f90
echo 'FWOF41_PRODUCTION_SOURCE_EXACT_FWO40_CANDIDATE=PASS'

# The external text codec remains qualification-only. Its impure validation
# calls are ordered explicitly in a temporary test copy; no owner source is
# generated or replaced in this positive path.
...
if src.count(marker) != 1:
    raise SystemExit(f'F-WOF41 F-WOF34 module boundary count={src.count(marker)}')
...
for OPT in 0 2; do
  ...
  if grep -Eqi 'accepted_window|event_delivered|delivery_committed|solver|scratch|forcing|parameter' restart.artifact; then
    echo 'F-WOF41 retired/non-state data leaked into restart artifact' >&2
...
PY
  ...
  echo "FWOF41_PRODUCTION_OWNER_RESTART_O${OPT}=PASS"
  popd >/dev/null
done
...
echo 'FWOF41_PRODUCTION_OWNER_API_DIRECT_COMPILE=PASS'
echo 'FWOF41_TWO_SEPARATE_OS_PROCESSES_PRODUCER_THEN_CONSUMER=PASS'
echo 'FWOF41_CONTINUOUS_VERSUS_PROCESS_RESTART_ENDPOINT_EXACT=PASS'
echo 'FWOF41_RETIRED_ACCEPTED_WINDOW_NOT_PERSISTED=PASS'
echo 'FWOF41_EXTERNAL_NEGATIVE_CASES_FAIL_CLOSED=PASS'
echo 'FWOF41_EXTERNAL_REPRESENTATION_O0_O2_IDENTITY=PASS'
echo 'FWOF41_PRODUCTION_OWNER_PERSISTENCE_QUALIFICATION_GATE PASS'
