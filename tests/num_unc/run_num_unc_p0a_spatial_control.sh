...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-num-unc-p0a-spatial-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"; trap 'rm -rf "$BUILD"' EXIT; cd "$ROOT"
fail(){ echo "NUM_UNC_P0A_SPATIAL_GATE_FAIL $*" >&2; exit 1; }
TEST=tests/num_unc/test_num_unc_p0a_spatial_control.f90
...
for opt in 0 2; do
  ...
  grep -Fq 'NUM_UNC_P0A_SPATIAL_GATE=PASS' "$OUT/output.txt" || fail "missing gate O$opt"
  cat "$OUT/output.txt"; echo "NUM_UNC_P0A_SPATIAL_O${opt}=PASS"
done
cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" || { diff -u "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" >&2 || true; fail 'O0/O2 drift'; }
echo "NUM_UNC_P0A_SPATIAL_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'NUM_UNC_P0A_SPATIAL_HARNESS_GATE=PASS'
