...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-num-unc-p0b0-${GITHUB_RUN_ID:-local}-$$"
...
grep -Fq '"N1_execution_allowed_in_B0": false' "$MANIFEST" || fail 'B0 N1 firewall missing'
if grep -Fq '0.0032' "$TEST"; then fail 'N1 timestep entered B0'; fi
...
for opt in 0 2; do
  ...
  grep -Fq 'NUM_UNC_P0B0_GATE=PASS' "$OUT/output.txt" || fail "missing gate O$opt"
  cat "$OUT/output.txt"; echo "NUM_UNC_P0B0_O${opt}=PASS"
done
cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" || { diff -u "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" >&2 || true; fail 'O0/O2 drift'; }
echo "NUM_UNC_P0B0_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'NUM_UNC_P0B0_HARNESS_GATE=PASS'
