...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-num-unc-p0c-c1-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"; trap 'rm -rf "$BUILD"' EXIT; cd "$ROOT"
fail(){ echo "NUM_UNC_P0C_C1_GATE_FAIL $*" >&2; exit 1; }
TEST=tests/num_unc/test_num_unc_p0c_c1.f90
...
grep -Fq '"authoritative_freeze": true' "$C0" || fail 'corrected C0 freeze missing'
grep -Fq '"status": "PASS_OUTER_BRACKET_STABLE"' "$SPATIAL" || fail 'spatial gate not passed'
grep -Fq '"posthoc_case_relocation_allowed": false' "$MANIFEST" || fail 'case relocation firewall missing'
grep -Fq '0.0032_real64' "$TEST" || fail 'N1 treatment missing from C1 source'
...
for opt in 0 2; do
  ...
  grep -Fq 'NUM_UNC_P0C_C1_GATE=PASS' "$OUT/output.txt" || fail "missing gate O$opt"
  cat "$OUT/output.txt"; echo "NUM_UNC_P0C_C1_O${opt}=PASS"
done
cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" || { diff -u "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" >&2 || true; fail 'O0/O2 drift'; }
echo "NUM_UNC_P0C_C1_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'NUM_UNC_P0C_C1_HARNESS_GATE=PASS'
