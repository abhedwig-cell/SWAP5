program test_num_unc_p0a0_stress_locator
  ...
  integer, parameter :: n=16, rooted_nodes=6, blocks=192, nsearch=5
  integer, parameter :: STRESS_BEFORE_RESCUE=1, NOT_BEFORE_RESCUE=2, INADMISSIBLE=9
  integer, parameter :: rescue_start_block=65, rescue_end_block=80, bisect_n=12
  real(real64), parameter :: dz_cm=10.0_real64, dt=0.0064_real64
  ...
  real(real64), parameter :: search_delta(nsearch)=[0.0_real64,0.0025_real64,0.005_real64, &
       0.010_real64,0.020_real64]
  real(real64), parameter :: freeze_offset=0.0025_real64

  integer :: classes(nsearch),i,lo_i,iter,mid_class,minus_class,zero_class,plus_class
  logical :: valid
  real(real64) :: hcrit,secrit,se0,lo,hi,mid,se_star,se_minus,se_plus
  real(real64) :: min_ratio,max_mass,first_stress_time
  integer :: first_stress_block
  character(len=64) :: failure

  call derive_transition_authority(hcrit,secrit,valid)
  if (.not.valid) then
    write(*,'(A)') 'NUM_UNC_P0A0_STATUS=BLOCKED_TRANSITION_AUTHORITY'
    write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
    stop
  end if

  write(*,'(A)') 'NUM_UNC_P0A0_STAGE=N0_ONLY_STRESS_LOCATOR'
  write(*,'(A)') 'NUM_UNC_P0A0_N1_EXECUTED=FALSE'
  write(*,'(*(g0))') 'NUM_UNC_P0A0_DT_DAY=',dt
  write(*,'(*(g0))') 'NUM_UNC_P0A0_PTRA_CM_PER_DAY=',ptra
  write(*,'(*(g0))') 'NUM_UNC_P0A0_HCRIT_CM=',hcrit
  write(*,'(*(g0))') 'NUM_UNC_P0A0_SECRIT=',secrit
  write(*,'(A,I0)') 'NUM_UNC_P0A0_ROOTED_NODES=',rooted_nodes
  write(*,'(*(g0))') 'NUM_UNC_P0A0_RESCUE_TIME_DAY=',real(rescue_start_block-1,real64)*dt

  classes=INADMISSIBLE
  do i=1,nsearch
    se0=secrit+search_delta(i)
    call run_case(se0,classes(i),valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
    call report_run('SEARCH',se0,classes(i),valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
    if (.not.valid) then
      write(*,'(A)') 'NUM_UNC_P0A0_STATUS=BLOCKED_INADMISSIBLE_SEARCH_POINT'
      write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
      stop
    end if
  end do
  ...
  lo_i=0
  do i=1,nsearch-1
    if (classes(i)==STRESS_BEFORE_RESCUE .and. classes(i+1)==NOT_BEFORE_RESCUE) then
      lo_i=i
      exit
    end if
  end do
  if (lo_i==0) then
    write(*,'(A)') 'NUM_UNC_P0A0_STATUS=NO_TRANSITION_IN_DECLARED_ENVELOPE'
    write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
    stop
  end if

  lo=secrit+search_delta(lo_i)
  hi=secrit+search_delta(lo_i+1)
  write(*,'(*(g0))') 'NUM_UNC_P0A0_INITIAL_BRACKET|LO_SE=',lo,'|HI_SE=',hi

  do iter=1,bisect_n
    mid=0.5_real64*(lo+hi)
    call run_case(mid,mid_class,valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
    call report_run('BISECT',mid,mid_class,valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
    if (.not.valid) then
      write(*,'(A)') 'NUM_UNC_P0A0_STATUS=BLOCKED_INADMISSIBLE_BISECTION'
      write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
      stop
    end if
    if (mid_class==STRESS_BEFORE_RESCUE) then
      lo=mid
    else if (mid_class==NOT_BEFORE_RESCUE) then
      hi=mid
    else
      write(*,'(A)') 'NUM_UNC_P0A0_STATUS=BLOCKED_INVALID_CLASSIFICATION'
      write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
      stop
    end if
  end do

  se_star=0.5_real64*(lo+hi)
  se_minus=se_star-freeze_offset
  se_plus=se_star+freeze_offset
  if (se_minus<=0.0_real64 .or. se_plus>=1.0_real64) then
    write(*,'(A)') 'NUM_UNC_P0A0_STATUS=BLOCKED_FREEZE_OUTSIDE_SE_DOMAIN'
    write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
    stop
  end if

  call run_case(se_minus,minus_class,valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
  call report_run('FREEZE_MINUS',se_minus,minus_class,valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
  if (.not.valid) then
    write(*,'(A)') 'NUM_UNC_P0A0_STATUS=BLOCKED_INADMISSIBLE_FREEZE_MINUS'
    write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
    stop
  end if

  call run_case(se_star,zero_class,valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
  call report_run('FREEZE_ZERO',se_star,zero_class,valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
  if (.not.valid) then
    write(*,'(A)') 'NUM_UNC_P0A0_STATUS=BLOCKED_INADMISSIBLE_FREEZE_ZERO'
    write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
    stop
  end if

  call run_case(se_plus,plus_class,valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
  call report_run('FREEZE_PLUS',se_plus,plus_class,valid,min_ratio,first_stress_block,first_stress_time,max_mass,failure)
  if (.not.valid) then
    write(*,'(A)') 'NUM_UNC_P0A0_STATUS=BLOCKED_INADMISSIBLE_FREEZE_PLUS'
    write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
    stop
  end if

  if (minus_class/=STRESS_BEFORE_RESCUE .or. plus_class/=NOT_BEFORE_RESCUE) then
    write(*,'(A)') 'NUM_UNC_P0A0_STATUS=BLOCKED_PREREGISTERED_OFFSETS_DO_NOT_STRADDLE'
    write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'
    stop
  end if

  write(*,'(*(g0))') 'NUM_UNC_P0A0_FREEZE|BRACKET_LO_SE=',lo,'|BRACKET_HI_SE=',hi, &
       '|SE_STAR=',se_star,'|A_MINUS_SE=',se_minus,'|A_ZERO_SE=',se_star,'|A_PLUS_SE=',se_plus, &
       '|A_ZERO_CLASS=',trim(class_name(zero_class))
  write(*,'(A)') 'NUM_UNC_P0A0_STATUS=FREEZE_READY'
  write(*,'(A)') 'NUM_UNC_P0A0_GATE=PASS'

contains

  subroutine derive_transition_authority(hcritical,secritical,ok)
    real(real64),intent(out) :: hcritical,secritical
    ...
    if (ptra<adcrl) then
      ...
      hcritical=hlim3h+((adcrh-ptra)/(adcrh-adcrl))*(hlim3l-hlim3h)
    else
      hcritical=hlim3h
    end if
    m=1.0_real64-1.0_real64/material%n
    secritical=(1.0_real64+(material%alpha_per_cm*abs(hcritical))**material%n)**(-m)
    ok=ieee_is_finite(hcritical) .and. ieee_is_finite(secritical) .and. secritical>0.0_real64 .and. secritical<1.0_real64
  end subroutine derive_transition_authority

  subroutine run_case(initial_se,class_id,is_valid,min_actual_ratio,first_block,first_time,max_mass_residual,failure_stage)
    real(real64),intent(in) :: initial_se
    integer,intent(out) :: class_id,first_block
    logical,intent(out) :: is_valid
    real(real64),intent(out) :: min_actual_ratio,first_time,max_mass_residual
    ...
    class_id=INADMISSIBLE
    is_valid=.false.
    min_actual_ratio=huge(1.0_real64)
    first_block=0
    first_time=-1.0_real64
    max_mass_residual=0.0_real64
    failure_stage='INITIALIZATION'

    if (.not.ieee_is_finite(initial_se) .or. initial_se<=0.0_real64 .or. initial_se>=1.0_real64) then
      failure_stage='INITIAL_SE_DOMAIN'
      return
    end if
    ...
    source_root_zero=0.0_real64
    root_sink_vec=0.0_real64
    call bind_b110_source_sink_provider(source_sink,drainage,irrigation,source_root_zero)

    root_parameters%active_nodes=n
    root_parameters%hlim3l=hlim3l
    root_parameters%hlim3h=hlim3h
    root_parameters%hlim4=hlim4
    root_parameters%adcrl=adcrl
    root_parameters%adcrh=adcrh
    ...
    if (first_block>0 .and. first_time<real(rescue_start_block-1,real64)*dt) then
  ...
  logical function result_valid(result,material) result(ok)
    ...
    if (result%diagnostics%internal_retries/=0) return
    if (result%diagnostics%alternative_solver_calls/=0) return
  ...
  subroutine initialize_request(req,p,hyd,source,root,top,base,qtop)
    ...
    req%boundary%top_head=base%pressure_head(1)
    ...
    req%evaluation%constitutive=>hyd
    req%evaluation%source_sink=>source
    req%evaluation%root_sink=>root
  ...
  subroutine initialize_parameters(p,c,mat)
    ...
    p%dz=dz_cm
    p%node_distance=dz_cm
    p%node_distance(1)=0.5_real64*dz_cm
    c=0.0_real64
    do j=1,n
      c(1,j)=mat%theta_r
      c(2,j)=mat%theta_s
      c(3,j)=mat%ksatfit_cm_per_day
      c(4,j)=mat%alpha_per_cm
      c(5,j)=mat%lambda
      c(6,j)=mat%n
      c(7,j)=m
      c(8,j)=mat%alpha_per_cm
      c(9,j)=mat%h_enpr_cm
      c(10,j)=mat%ksatfit_cm_per_day
      c(11,j)=0.999_real64
      c(12,j)=0.99_real64*mat%ksatfit_cm_per_day
      c(22,j)=-1.0e6_real64
  ...
  subroutine report_run(stage,se0,class_id,is_valid,min_ratio,first_block,first_time,max_mass,failure_stage)
    ...
    write(*,'(*(g0))') 'NUM_UNC_P0A0_RUN|STAGE=',trim(stage),'|SE0=',se0, &
         '|CLASS=',trim(class_name(class_id)),'|VALID=',is_valid,'|MIN_TA_TP=',min_ratio, &
         '|FIRST_STRESS_BLOCK=',first_block,'|FIRST_STRESS_TIME_DAY=',first_time, &
         '|MAX_MASS_CM=',max_mass,'|FAILURE=',trim(failure_stage)
  ...
  pure function class_name(class_id) result(name)
    ...
    case(STRESS_BEFORE_RESCUE)
      name='STRESS_BEFORE_RESCUE'
    case(NOT_BEFORE_RESCUE)
      name='NOT_BEFORE_RESCUE'
    case default
      name='INADMISSIBLE'
    end select
  end function class_name

end program test_num_unc_p0a0_stress_locator
