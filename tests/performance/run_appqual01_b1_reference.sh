...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-appqual01-b1-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -J "$BUILD" -I "$BUILD" -c tests/performance/test_appqual01_b1_reference.f90 -o "$BUILD/test.o"
