...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-appqual01-b1-rossfast-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
gfortran "${COMMON[@]}" -J "$BUILD" -I "$BUILD" -c tests/performance/test_appqual01_b1_rossfast_paired.f90 -o "$BUILD/test.o"
gfortran -O2 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/test"

for n in 100 1000 10000; do
  "$BUILD/test" "$n" REFERENCE
  "$BUILD/test" "$n" ROSSFAST
done
