program test_appqual01_b1_reference
  ...
  read(arg,*) n
  if(n<=0) error stop 'APPQUAL01 B1 requires N>0'

  call build_config(config,n)
  call system_clock(c0,rate)
  call app%initialize(config,status)
  call system_clock(c1)
  if(status/=FMR_APP_BOOT_OK .or. .not.app%ready()) error stop 'APPQUAL01 B1 bootstrap failed'
  init_s=real(c1-c0,real64)/real(rate,real64)

  call system_clock(c0)
  call app%run_standalone(T0,T1,results,status)
  call system_clock(c1)
  if(status/=FMR_APP_BOOT_OK) error stop 'APPQUAL01 B1 run failed'
  run_s=real(c1-c0,real64)/real(rate,real64)
  if(.not.allocated(results) .or. size(results)/=n) error stop 'APPQUAL01 B1 result shape'
  if(.not.all(results%completed) .or. .not.all(results%committed)) error stop 'APPQUAL01 B1 incomplete result'
  max_residual=maxval(abs(results%mass%residual))
  if(max_residual>TOL) error stop 'APPQUAL01 B1 hard mass gate'

  write(*,'(A,I0,A,ES24.16,A,ES24.16,A,ES24.16,A,ES24.16)') &
       'APPQUAL01_B1_REFERENCE,n=',n,',init_seconds=',init_s,',run_seconds=',run_s, &
       ',ns_per_column=',1.0e9_real64*run_s/real(n,real64),',max_mass_residual=',max_residual
  write(*,'(A)') 'APPQUAL01_B1_REFERENCE=PASS'

  call app%close(status)
  if(status/=FMR_APP_BOOT_OK) error stop 'APPQUAL01 B1 close failed'
...
end program test_appqual01_b1_reference
