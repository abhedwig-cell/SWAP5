...
MODULE_SRC=(
  ...
  tests/publication/mod_pub_gc_e6_active_drainage_bridge.f90
)
objects=()
...
gfortran -shared -fopenmp -O2 "${objects[@]}" -o "$BUILD/bridge/libpub_gc_e6_swap.so" || fail "link F-GC44 shared library"
nm -D "$BUILD/bridge/libpub_gc_e6_swap.so" | grep -q 'pub_gc_e6_swap_initialize_c' || fail "missing SWAP C ABI"
nm -D "$BUILD/bridge/libpub_gc_e6_swap.so" | grep -q 'fgc34_publish_c' || fail "missing F-GC34 publisher C ABI"

LIBMF6="$BUILD/modflow-bin/libmf6.so" PUB_GC_E6_SWAP_LIB="$BUILD/bridge/libpub_gc_e6_swap.so" \
  python3 tests/publication/test_pub_gc_e6_active_drainage_init.py | tee "$BUILD/e6-init.txt"
...
echo 'PUB-GC E6 ACTIVE-DRAINAGE INITIALIZATION GATE PASS'
