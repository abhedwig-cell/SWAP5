program pub_gc_e1_screen_case
  ...
  real(real64) :: t1, window_dt, candidate_head_b
  ...
  type(kernel_diagnostics_t) :: diag, diag_a1, diag_b, diag_a2, diag_history, discard_diag
  ...
  real(real64) :: conductivity_reference, synthetic_time, endpoint_head_delta, endpoint_water_delta
  character(len=64) :: arg
  real(real64) :: accepted_predecessor_right_derivative(numnod)
  logical :: ok, available

  call require(command_argument_count() == 2, 'screen requires <window_dt_days> <candidate_b_head_cm>')
  call get_command_argument(1, arg)
  read(arg,*,iostat=diag%attempts) window_dt
  call require(diag%attempts == 0 .and. ieee_is_finite(window_dt) .and. window_dt > 0.0_real64, 'valid screen window')
  call get_command_argument(2, arg)
  read(arg,*,iostat=diag%attempts) candidate_head_b
  call require(diag%attempts == 0 .and. ieee_is_finite(candidate_head_b), 'valid screen B head')
  t1 = t0 + window_dt
  ...
  call run_valid_trial(origin, origin_checkpoint, forcing_a, result_a1, candidate, diag_a1, snap_a1, 'same-origin A1')
  ...
  call run_valid_trial(origin, origin_checkpoint, forcing_b, result_b, candidate, diag_b, snap_b, 'same-origin B')
  ...
  call run_valid_trial(origin, origin_checkpoint, forcing_a, result_a2, candidate, diag_a2, snap_a2, 'same-origin A2')
  ...
  call run_valid_trial(synthetic, synthetic_checkpoint, forcing_a, result_history, candidate, diag_history, snap_history, &
  ...
  call endpoint_deltas(snap_a1, snap_history, endpoint_head_delta, endpoint_water_delta, ok)
  call require(ok, 'screen endpoint delta extraction')
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_WINDOW_DAYS=', window_dt
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_B_HEAD_CM=', candidate_head_b
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_Q_A_SAME=', result_a1%bottom_outward_exchange_native
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_Q_B_SAME=', result_b%bottom_outward_exchange_native
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_Q_A_HISTORY=', result_history%bottom_outward_exchange_native
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_ABS_DELTA_Q=', &
       abs(result_history%bottom_outward_exchange_native-result_a1%bottom_outward_exchange_native)
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_ENDPOINT_HEAD_DELTA=', endpoint_head_delta
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_ENDPOINT_WATER_DELTA=', endpoint_water_delta
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_MASS_A_SAME=', result_a1%mass%residual
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_MASS_B_SAME=', result_b%mass%residual
  write(*,'(a,es26.17e3)') 'PUB_GC_E1_SCREEN_MASS_A_HISTORY=', result_history%mass%residual
  write(*,'(a,i0)') 'PUB_GC_E1_SCREEN_RETRIES_A_SAME=', diag_a1%retries
  write(*,'(a,i0)') 'PUB_GC_E1_SCREEN_RETRIES_B_SAME=', diag_b%retries
  write(*,'(a,i0)') 'PUB_GC_E1_SCREEN_RETRIES_A_HISTORY=', diag_history%retries
  ...
  print '(a)', 'PUB_GC_E1_SCREEN_CASE=PASS'
...
end program pub_gc_e1_screen_case
