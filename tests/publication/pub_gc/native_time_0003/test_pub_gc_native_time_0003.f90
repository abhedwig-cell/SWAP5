program test_pub_gc_native_time
  ...
  integer, parameter :: ncase=5, nlevel=6
  ...
  integer, parameter :: interval_count(nlevel)=[4,8,16,32,64,128]
  real(real64), parameter :: native_dt(nlevel)=[0.01_real64,0.005_real64,0.0025_real64,0.00125_real64,0.000625_real64,0.0003125_real64]
  ...
  if(fine_guard) then
    do ic=1,ncase
      call compare_levels(r(ic,5),r(ic,6),level_ok,'FINE',ic)
  ...
  if(.not.all_valid) then
    ...
    do il=1,5
      level_ok=.true.
      do ic=1,ncase
        call compare_levels(r(ic,il),r(ic,6),ok,'SELECT',ic)
