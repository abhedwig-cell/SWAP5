#!/usr/bin/env python3
"""Emit the current-canonical Fortran module closure needed by PUB-ME D2.
...
TEST = ROOT / "tests/publication/test_pub_me_d2_retry_accounting.f90"
...
if unresolved_dups:
    for m,ps in sorted(unresolved_dups.items()):
        print("PUB_ME_D2_BUILD_DUPLICATE_MODULE",m,*(str(p.relative_to(ROOT)) for p in ps),file=sys.stderr)
    raise SystemExit(31)
...
if REAL_DIRECTION in visited:
    raise RuntimeError("real optional directional service entered D2 closure")
if DIRECTION_STUB not in visited:
    raise RuntimeError("D2 directional stub not selected by backend closure")
