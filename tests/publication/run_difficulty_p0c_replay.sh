...
for opt in 0 2; do
  ...
  for marker in \
    'DIFFICULTY_P0C_SOURCE_IMMUTABILITY=PASS' \
    'DIFFICULTY_P0C_ORDER_INDEPENDENCE=PASS' \
    'DIFFICULTY_P0C_DETERMINISTIC_REPLAY=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing O$opt marker $marker"; }
  done
  cat "$OUT/output.txt"
  echo "DIFFICULTY_P0C_O${opt}=PASS"
done
...
echo 'DIFFICULTY_P0C_COUNTERFACTUAL_REPLAY=PASS'
