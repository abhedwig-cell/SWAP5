...
OUT="${PUB_GC_EVIDENCE_DIR:-$ROOT/build/pub-gc-e3r}"
mkdir -p "$OUT"
: > "$OUT/cases.jsonl"
: > "$OUT/case-output.txt"
...
for item in "${CASES[@]}"; do
  ...
  for kval in "${KS[@]}"; do
    echo "PUB_GC_E3R_CASE window=$window qbot=$qbot K=$kval" | tee -a "$OUT/case-output.txt"
    CASE_OUT="$(
    ...
    test -n "$JSON_LINE" || fail "missing E3R structured case window=$window qbot=$qbot K=$kval"
...
if len(records)!=24:
    raise SystemExit(f"expected 24 E3R cases, got {len(records)}")

keys={(float(r["window_day"]),float(r["predictor_qbot_cm_per_day"]),float(r["k_m_per_day"])) for r in records}
if len(keys)!=24:
    raise SystemExit("duplicate or missing E3R matrix keys")
...
if bad:
    raise SystemExit(f"unexpected E3R statuses: {bad[:2]}")

# All predictor points were demonstrated ready by E3-D. Zero-gradient changes
# only the later MODFLOW fixture, so predictor unavailability is a regression.
pred_fail=[r for r in records if r.get("status")=="SWAP_PREDICTOR_UNAVAILABLE"]
if pred_fail:
    raise SystemExit(f"E3R predictor regression at preregistered ready points: {pred_fail[:2]}")

conv=[r for r in records if r["status"]=="CONVERGED"]
for r in conv:
    it=r["iterative"]; lo=r["loose"]
    vals=[
        it["head_m"],it["q_gw_m_per_s"],it["q_swap_m_per_s"],it["residual_m_per_s"],
        lo["head_m"],lo["q_gw_m_per_s"],lo["q_swap_m_per_s"],lo["residual_m_per_s"],
        r["delta_h_iter_minus_loose_m"],r["delta_qswap_iter_minus_loose_m_per_s"],
        r["loose_relative_flux_mismatch"],r["predictor_reference_head_m"],
    ]
    if not all(math.isfinite(float(v)) for v in vals):
        raise SystemExit("nonfinite converged E3R measurement")
    if abs(float(it["residual_m_per_s"]))>1e-15:
        raise SystemExit("E3R case labeled converged above flux tolerance")

def displacement(r,which):
    return float(r[which]["head_m"])-float(r["predictor_reference_head_m"])

summary_rows=[]
for r in records:
    row={
        "window_day":r["window_day"],
        "predictor_qbot_cm_per_day":r["predictor_qbot_cm_per_day"],
        "k_m_per_day":r["k_m_per_day"],
        "status":r["status"],
        "failure_stage":r.get("failure_stage",""),
        "reference_head_m":r.get("predictor_reference_head_m"),
    }
    if r["status"]=="CONVERGED":
        row.update({
            "loose_head_m":r["loose"]["head_m"],
            "iterative_head_m":r["iterative"]["head_m"],
            "loose_head_displacement_m":displacement(r,"loose"),
            "iterative_head_displacement_m":displacement(r,"iterative"),
            "loose_residual_m_per_s":r["loose"]["residual_m_per_s"],
            "loose_relative_flux_mismatch":r["loose_relative_flux_mismatch"],
            "delta_h_iter_minus_loose_m":r["delta_h_iter_minus_loose_m"],
            "delta_qswap_iter_minus_loose_m_per_s":r["delta_qswap_iter_minus_loose_m_per_s"],
            "coupling_outer_iterations":r["iterative"]["coupling_outer_iterations"],
            "iterative_residual_m_per_s":r["iterative"]["residual_m_per_s"],
        })
    summary_rows.append(row)

status_counts=Counter(r["status"] for r in records)
payload={
    "schema":"pub-gc-e3r-result-v1",
    "case_count":len(records),
    "converged_count":len(conv),
    "status_counts":dict(sorted(status_counts.items())),
    "max_abs_loose_head_displacement_m":max((abs(displacement(r,"loose")) for r in conv),default=None),
    "max_abs_iterative_head_displacement_m":max((abs(displacement(r,"iterative")) for r in conv),default=None),
    "max_abs_iterative_correction_m":max((abs(float(r["delta_h_iter_minus_loose_m"])) for r in conv),default=None),
    "max_abs_iterative_q_correction_m_per_s":max((abs(float(r["delta_qswap_iter_minus_loose_m_per_s"])) for r in conv),default=None),
    "max_loose_relative_flux_mismatch":max((float(r["loose_relative_flux_mismatch"]) for r in conv),default=None),
    "records":records,
}
(out/"PUB_GC_E3R_RESULT.json").write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n")

fields=sorted({k for row in summary_rows for k in row.keys()})
with (out/"PUB_GC_E3R_CASES.csv").open("w",newline="") as fh:
...
print("PUB_GC_E3R_MATRIX_COMPLETE=PASS")
PY
