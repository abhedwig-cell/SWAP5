...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-pub-me-d6-fci98-${GITHUB_RUN_ID:-local}-$"
...
EXECUTION_BASE=7b864853ca22baa73141b2dec9ed2f3915ef520d
...
CHECKPOINT=docs/publications/PUB-ME_D6_FCI98_SUCCESSOR_REPLAY.md
TEST=tests/publication/test_pub_me_d6_rejected_publication.f90
RUNNER=tests/publication/run_pub_me_d6_fci98_successor_replay.sh
RESULT=docs/publications/PUB-ME_D6_RESULT.md
TEST_BLOB=76f3394cdb11367c609d5f6c8028be80b82f45e7
RESULT_BLOB=4bd7a3aa752fda37c29517c93d64d91d0da4ef95
EXPECTED_OUTPUT_SHA=9563ed157258f1993a826a9ef498498cafa45254266818d877db73bb28c88643
...
[[ "$(git rev-parse "HEAD:$TEST")" == "$TEST_BLOB" ]] || fail 'D6 test blob drift'
[[ "$(git rev-parse "HEAD:$RESULT")" == "$RESULT_BLOB" ]] || fail 'D6 result blob drift'
echo 'PUB_ME_D6_FCI98_IMMUTABLE_D6_EVIDENCE=PASS'
...
actual_sha="$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
[[ "$actual_sha" == "$EXPECTED_OUTPUT_SHA" ]] || fail "D6 semantic output drift $actual_sha"
grep -Fq 'PUB_ME_D6_CLASSIFICATION=EARLIER_DETECTION' "$BUILD/o0/output.txt" || fail 'D6 classification drift'
echo "PUB_ME_D6_FCI98_OUTPUT_SHA256=$actual_sha"
echo 'PUB_ME_D6_FCI98_CLASSIFICATION_PRESERVED=PASS:EARLIER_DETECTION'
echo 'PUB_ME_D6_FCI98_SUCCESSOR_REPLAY=PASS'
