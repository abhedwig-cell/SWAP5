...
TEST=tests/publication/test_pub_p2e01_e0_paired_pilot.f90
...
for opt in 0 2; do
  ...
  if ! "$OUT/test" > "$OUT/output.txt" 2>&1; then
    ...
    fail "paired pilot runtime O$opt"
  fi
  for marker in \
    'PUB_P2E01_PAIRED_EXTRACTION_READY=PASS' \
    'PUB_P2E01_SCIENTIFIC_ADMISSIBILITY_NOT_EVALUATED=TRUE' \
    'PUB_P2E01_PREDICTED_FLUX_EQUIVALENCE_NOT_EVALUATED=TRUE'; do
...
echo 'PUB_P2E01_E0_PAIRED_PILOT=PASS'
