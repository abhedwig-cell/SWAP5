...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-pub-p2e10-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail() { echo "PUB_P2E10_GATE_FAIL $*" >&2; exit 1; }

PREREG=docs/publication/P2E10_E0_BROAD_PAIRED_MATRIX_PREREGISTRATION.json
TEST=tests/publication/test_pub_p2e10_e0_broad_paired_matrix.f90

P2E09_RESULT_BLOB=1a82511cb13a4c39235292de83dc626478bfb6a6
P2E01_TYPED_PILOT_BLOB=751f033be07fb688cfbe38dd994eca90c63999e4
...
PREREG_BLOB=f593879074e0981ba62aa02ca1f48171c29ac074

[[ -f "$PREREG" ]] || fail 'missing P2E10 preregistration'
[[ -f "$TEST" ]] || fail 'missing P2E10 paired matrix test'

grep -Fq '"phase": "PREREGISTERED_BEFORE_BROAD_ROSSFAST_DISCREPANCY_EXECUTION"' "$PREREG" || fail 'preregistration phase drift'
grep -Fq '"case_count": 54' "$PREREG" || fail '54-case matrix not frozen'
grep -Fq '"step_duration_day": 0.0016' "$PREREG" || fail 'paired duration drift'
grep -Fq '"threshold_retuning_after_execution_allowed": false' "$PREREG" || fail 'threshold-retuning firewall missing'
grep -Fq '"transaction_level_pair": "EXCLUDED"' "$PREREG" || fail 'transaction-layer firewall missing'
...
git diff --quiet "$CURRENT_BASE" HEAD -- src reference || fail 'P2E10 mutated src or reference'
echo "PUB_P2E10_RECONCILED_BASE=$CURRENT_BASE"

test "$(git rev-parse HEAD:$PREREG)" = "$PREREG_BLOB" || fail 'P2E10 preregistration drift'
...
for opt in 0 2; do
  ...
  if ! "$OUT/test" > "$OUT/output.txt" 2>&1; then
    ...
    fail "broad paired matrix runtime O$opt"
  fi

  for marker in     'PUB_P2E10_CASE_COUNT=54'     'PUB_P2E10_LAYER=A_FIXED_INTERVAL_SOLVER_SEAM'     'PUB_P2E10_STEP_DURATION_DAY=0.0016'     'PUB_P2E10_THRESHOLDS_RETUNED=FALSE'     'PUB_P2E10_TRANSACTION_LEVEL_PAIR_EXECUTED=FALSE'     'PUB_P2E10_PERFORMANCE_CLAIM_EVALUATED=FALSE'     'PUB_P2E10_BROAD_MATRIX_EXECUTION_GATE=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing O$opt marker $marker"; }
  done

  cases="$(grep -Fc 'PUB_P2E10_CASE|' "$OUT/output.txt")"
  thresholds="$(grep -Fc 'PUB_P2E10_THRESHOLD|' "$OUT/output.txt")"
  [[ "$cases" = "54" ]] || fail "expected 54 case records, got $cases"
  [[ "$thresholds" = "3" ]] || fail "expected three threshold strata, got $thresholds"

  cat "$OUT/output.txt"
  echo "PUB_P2E10_O${opt}=PASS"
done

cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" || {
  diff -u "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" >&2 || true
  fail 'O0/O2 broad paired scientific output drift'
}

echo "PUB_P2E10_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'PUB_P2E10_BROAD_E0_PAIRED_MATRIX_GATE=PASS'
