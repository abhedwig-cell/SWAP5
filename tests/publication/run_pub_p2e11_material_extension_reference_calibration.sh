...
TEST=tests/publication/test_pub_p2e11_material_extension_reference_calibration.f90
PREREG=docs/publication/P2E11_MATERIAL_EXTENSION_REFERENCE_CALIBRATION_PREREGISTRATION.json
...
git diff --quiet "$CANONICAL_BASE" HEAD -- src reference || fail 'P2E11 mutated src or reference relative to canonical base'
...
for opt in 0 2; do
  ...
  [[ "$feasibility_lines" = "1" ]] || { cat "$OUT/output.txt" >&2; fail "expected one temporal feasibility summary"; }

  if grep -Fq 'PUB_P2E11_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E11_SELECTED_VALID_CASE_COUNT=180' "$OUT/output.txt" || fail "qualified domain did not retain 180/180 cases"
    selected_lines="$(grep -Fc 'PUB_P2E11_SELECTED_CASE|' "$OUT/output.txt")"
    [[ "$selected_lines" = "180" ]] || fail "qualified domain did not emit 180 selected cases"
  elif grep -Fq 'PUB_P2E11_COMMON_DOMAIN_STATUS=BLOCKED_REFERENCE_COMMON_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E11_SCIENTIFIC_OUTCOME=NEGATIVE_COMMON_DOMAIN_NOT_FOUND' "$OUT/output.txt" || fail "blocked domain missing negative-outcome marker"
  else
    cat "$OUT/output.txt" >&2
  ...
  if grep -Fq 'PUB_P2E11_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    se_max_lines="$(grep -Fc 'PUB_P2E11_SE_MAX|' "$OUT/output.txt")"
    [[ "$se_max_lines" = "15" ]] || { cat "$OUT/output.txt" >&2; fail "expected fifteen Se-stratified metric maxima"; }
  fi
  cat "$OUT/output.txt"
...
echo 'PUB_P2E11_REFERENCE_VALID_CALIBRATION_DOMAIN_GATE=PASS'
