...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-pub-p2e11-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail() { echo "PUB_P2E11_GATE_FAIL $*" >&2; exit 1; }

TEST=tests/publication/test_pub_p2e11_reference_material_extension_calibration.f90
PREREG=docs/publication/P2E11_REFERENCE_MATERIAL_EXTENSION_CALIBRATION_PREREGISTRATION.json
...
PREREG_BLOB=3f928d470a11282b71f6f05d7d5d44ae55fcada4
TEST_BLOB=6eee6911b57f797d7f8a0ffb75adf7dcaa326d18
...
grep -Fq '"phase": "PREREGISTERED_BEFORE_EXTENSION_REFERENCE_EXECUTION"' "$PREREG" || fail 'preregistration phase missing'
...
grep -Fq '"temporal_schedule_search_after_results_allowed": false' "$PREREG" || fail 'fixed temporal schedule firewall missing'
grep -Fq '"threshold_freezing_in_this_workunit_allowed": false' "$PREREG" || fail 'threshold-freeze firewall missing'

git merge-base --is-ancestor "$CANONICAL_BASE" HEAD || fail 'canonical base is not an ancestor'
git diff --quiet "$CANONICAL_BASE" HEAD -- src reference || fail 'P2E11 mutated src or reference'
...
test "$(git rev-parse HEAD:$TEST)" = "$TEST_BLOB" || fail 'test oracle drift'
...
for opt in 0 2; do
  ...
  if ! "$OUT/test" > "$OUT/output.txt" 2>&1; then
    ...
    fail "Reference material-extension calibration O$opt"
  fi

  for marker in     'PUB_P2E11_PHYSICAL_CASE_COUNT=180'     'PUB_P2E11_TEMPORAL_LEVEL_COUNT=1'     'PUB_P2E11_TRIAL_PAIR_COUNT=180'     'PUB_P2E11_ROSSFAST_SOLVER_EXECUTED=FALSE'     'PUB_P2E11_THRESHOLD_FROZEN=FALSE'     'PUB_P2E11_REFERENCE_TOLERANCE_CHANGED=FALSE'     'PUB_P2E11_P2E03_REPAIRED_OR_RECLASSIFIED=FALSE'     'PUB_P2E11_REFERENCE_MATERIAL_EXTENSION_CALIBRATION_GATE=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing O$opt marker $marker"; }
  done

  feasibility_lines="$(grep -Fc 'PUB_P2E11_DT_FEASIBILITY|' "$OUT/output.txt")"
  [[ "$feasibility_lines" = "1" ]] || { cat "$OUT/output.txt" >&2; fail "expected one frozen temporal feasibility summary"; }

  if grep -Fq 'PUB_P2E11_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_EXTENSION_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E11_SELECTED_VALID_CASE_COUNT=180' "$OUT/output.txt" || fail "qualified extension domain did not retain 180/180 cases"
    selected_lines="$(grep -Fc 'PUB_P2E11_SELECTED_CASE|' "$OUT/output.txt")"
    [[ "$selected_lines" = "180" ]] || fail "qualified extension domain did not emit 180 selected cases"
  elif grep -Fq 'PUB_P2E11_COMMON_DOMAIN_STATUS=BLOCKED_EXTENSION_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E11_SCIENTIFIC_OUTCOME=NEGATIVE_EXTENSION_REFERENCE_DOMAIN_INCOMPLETE' "$OUT/output.txt" || fail "blocked domain missing negative-outcome marker"
  else
    cat "$OUT/output.txt" >&2
  ...
  cat "$OUT/output.txt"
  echo "PUB_P2E11_O${opt}=PASS"
done

cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" || {
  diff -u "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" >&2 || true
  fail 'O0/O2 Reference material-extension output drift'
}

echo "PUB_P2E11_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'PUB_P2E11_REFERENCE_MATERIAL_EXTENSION_CALIBRATION_GATE=PASS'
