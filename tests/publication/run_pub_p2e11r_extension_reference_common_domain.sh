...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-pub-p2e11r-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail() { echo "PUB_P2E11R_GATE_FAIL $*" >&2; exit 1; }

TEST=tests/publication/test_pub_p2e11r_extension_reference_common_domain.f90
PREREG=docs/publication/P2E11R_EXTENSION_REFERENCE_COMMON_DOMAIN_PREREGISTRATION.json
...
grep -Fq '"phase": "PREREGISTERED_REFERENCE_ONLY_COMMON_DOMAIN_RECOVERY"' "$PREREG" || fail 'preregistration phase missing'
...
git diff --quiet "$CANONICAL_BASE" HEAD -- src reference || fail 'P2E11R mutated src or reference relative to canonical base'
...
for opt in 0 2; do
  ...
  for marker in     'PUB_P2E11R_PHYSICAL_CASE_COUNT=180'     'PUB_P2E11R_TEMPORAL_LEVEL_COUNT=5'     'PUB_P2E11R_TRIAL_PAIR_COUNT=900'     'PUB_P2E11R_ROSSFAST_SOLVER_EXECUTED=FALSE'     'PUB_P2E11R_THRESHOLD_FILE_FROZEN=FALSE'     'PUB_P2E11R_REFERENCE_TOLERANCE_CHANGED=FALSE'     'PUB_P2E11R_P2E03_REPAIRED_OR_RECLASSIFIED=FALSE'     'PUB_P2E11R_REFERENCE_VALID_CALIBRATION_DOMAIN_GATE=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing O$opt marker $marker"; }
  done

  feasibility_lines="$(grep -Fc 'PUB_P2E11R_DT_FEASIBILITY|' "$OUT/output.txt")"
  [[ "$feasibility_lines" = "5" ]] || { cat "$OUT/output.txt" >&2; fail "expected five temporal feasibility summaries"; }

  if grep -Fq 'PUB_P2E11R_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E11R_SELECTED_VALID_CASE_COUNT=180' "$OUT/output.txt" || fail "qualified domain did not retain 180/180 cases"
    selected_lines="$(grep -Fc 'PUB_P2E11R_SELECTED_CASE|' "$OUT/output.txt")"
    [[ "$selected_lines" = "180" ]] || fail "qualified domain did not emit 180 selected cases"
  elif grep -Fq 'PUB_P2E11R_COMMON_DOMAIN_STATUS=BLOCKED_REFERENCE_COMMON_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E11R_SCIENTIFIC_OUTCOME=NEGATIVE_COMMON_DOMAIN_NOT_FOUND' "$OUT/output.txt" || fail "blocked domain missing negative-outcome marker"
  else
    cat "$OUT/output.txt" >&2
...
echo "PUB_P2E11R_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'PUB_P2E11R_REFERENCE_VALID_CALIBRATION_DOMAIN_GATE=PASS'
