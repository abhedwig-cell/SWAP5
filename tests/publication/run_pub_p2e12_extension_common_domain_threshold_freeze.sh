...
TEST=tests/publication/test_pub_p2e12_extension_common_domain_threshold_freeze.f90
PREREG=docs/publication/P2E12_EXTENSION_COMMON_DOMAIN_THRESHOLD_FREEZE_PREREGISTRATION.json
...
P2E11D2_RESULT_BLOB=25a5723cd680936821ccbf2e1adf0db31ef1b960
...
git diff --quiet "$CANONICAL_BASE" HEAD -- src reference || fail 'P2E12 mutated src or reference relative to canonical base'
...
test "$(git rev-parse HEAD:docs/publication/P2E11D2_REFERENCE_FAILED_ENDPOINT_SPLIT_RESULT.json)" = "$P2E11D2_RESULT_BLOB" || fail 'P2E11D2 diagnosis drift'
...
for opt in 0 2; do
  ...
  for marker in     'PUB_P2E12_PHYSICAL_CASE_COUNT=180'     'PUB_P2E12_TEMPORAL_LEVEL_COUNT=1'     'PUB_P2E12_TRIAL_PAIR_COUNT=180'     'PUB_P2E12_ROSSFAST_SOLVER_EXECUTED=FALSE'     'PUB_P2E12_THRESHOLD_FREEZE_STATUS=QUALIFIED_REFERENCE_ONLY_COMMON_DOMAIN'     'PUB_P2E12_REFERENCE_TOLERANCE_CHANGED=FALSE'     'PUB_P2E12_P2E03_REPAIRED_OR_RECLASSIFIED=FALSE'     'PUB_P2E12_EXTENSION_THRESHOLD_FREEZE_GATE=PASS'; do
  ...
  if grep -Fq 'PUB_P2E12_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E12_IN_COMMON_DOMAIN_COUNT=176' "$OUT/output.txt" || fail "qualified domain did not retain 180/180 cases"
    selected_lines="$(grep -Fc 'PUB_P2E12_IN_DOMAIN_CASE|' "$OUT/output.txt")"
    [[ "$selected_lines" = "176" ]] || fail "threshold freeze did not emit 176 in-domain cases"
  ...
  if grep -Fq 'PUB_P2E12_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    threshold_lines="$(grep -Fc 'PUB_P2E12_THRESHOLD|' "$OUT/output.txt")"
    [[ "$threshold_lines" = "15" ]] || { cat "$OUT/output.txt" >&2; fail "expected fifteen frozen Reference thresholds"; }
  fi
  cat "$OUT/output.txt"
