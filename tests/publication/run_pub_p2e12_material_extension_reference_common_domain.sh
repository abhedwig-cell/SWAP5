...
TEST=tests/publication/test_pub_p2e12_material_extension_reference_common_domain.f90
PREREG=docs/publication/P2E12_MATERIAL_EXTENSION_REFERENCE_COMMON_DOMAIN_PREREGISTRATION.json
P2E11_BASE=7fc3d6c64f44fd07d6271d71b7d270fbd708977d
...
P2E11_RESULT_BLOB=68f6770b3e57929dadbf8460416b58db9b27fbb4

[[ -f "$PREREG" ]] || fail 'missing preregistration'
grep -Fq '"phase": "PREREGISTERED_BEFORE_EXTENSION_COMMON_DOMAIN_EXECUTION"' "$PREREG" || fail 'preregistration phase missing'
...
git merge-base --is-ancestor "$P2E11_BASE" HEAD || fail 'P2E11 blocked-result base is not an ancestor'
git diff --quiet "$P2E11_BASE" HEAD -- src reference || fail 'P2E12 mutated src or reference relative to P2E11 base'
...
for opt in 0 2; do
  ...
  for marker in     'PUB_P2E12_PHYSICAL_CASE_COUNT=180'     'PUB_P2E12_TEMPORAL_LEVEL_COUNT=5'     'PUB_P2E12_TRIAL_PAIR_COUNT=900'     'PUB_P2E12_ROSSFAST_SOLVER_EXECUTED=FALSE'     'PUB_P2E12_THRESHOLD_FROZEN=FALSE'     'PUB_P2E12_REFERENCE_TOLERANCE_CHANGED=FALSE'     'PUB_P2E12_P2E03_REPAIRED_OR_RECLASSIFIED=FALSE'     'PUB_P2E12_REFERENCE_VALID_CALIBRATION_DOMAIN_GATE=PASS'; do
  ...
  if grep -Fq 'PUB_P2E12_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E12_SELECTED_VALID_CASE_COUNT=180' "$OUT/output.txt" || fail "qualified domain did not retain 180/180 cases"
    selected_lines="$(grep -Fc 'PUB_P2E12_SELECTED_CASE|' "$OUT/output.txt")"
    [[ "$selected_lines" = "180" ]] || fail "qualified domain did not emit 180 selected cases"
  elif grep -Fq 'PUB_P2E12_COMMON_DOMAIN_STATUS=BLOCKED_REFERENCE_COMMON_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E12_SCIENTIFIC_OUTCOME=NEGATIVE_COMMON_DOMAIN_NOT_FOUND' "$OUT/output.txt" || fail "blocked domain missing negative-outcome marker"
  else
    cat "$OUT/output.txt" >&2
  ...
  if grep -Fq 'PUB_P2E12_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    se_max_lines="$(grep -Fc 'PUB_P2E12_SE_MAX|' "$OUT/output.txt")"
...
echo 'PUB_P2E12_REFERENCE_VALID_CALIBRATION_DOMAIN_GATE=PASS'
