...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-pub-p2e13-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail() { echo "PUB_P2E13_GATE_FAIL $*" >&2; exit 1; }

TEST=tests/publication/test_pub_p2e13_reference_common_material_state_domain.f90
PREREG=docs/publication/P2E13_REFERENCE_COMMON_MATERIAL_STATE_DOMAIN_PREREGISTRATION.json
PARENT_BASE=f66d496799b52aee4c19fe0da5e4005160c60824
P2E11D1_RESULT_BLOB=1be7d63e82eac9d1f846c9715fabf80a5700bdf8
REFERENCE_TREE=684f1e2889b6992e5aedc88f52bb45f4558bb3e4
SOLVER_CONTRACT_BLOB=40a1ddc05fb8e2c1822763de645fd07a094568a3
MODEL_BINDING_BLOB=5442fd7e7a2f39235292de83dc626478bfb6a6
...
grep -Fq '"phase": "PREREGISTERED_BEFORE_COMMON_MATERIAL_STATE_DOMAIN_EXECUTION"' "$PREREG" || fail 'preregistration phase missing'
grep -Fq '"rossfast_numerical_solver_execution_allowed": false' "$PREREG" || fail 'RossFast execution firewall missing'
grep -Fq '"material_removal_allowed": false' "$PREREG" || fail 'material retention firewall missing'
grep -Fq '"post_execution_Se_grid_change_allowed": false' "$PREREG" || fail 'state-grid freeze missing'
grep -Fq '"threshold_creation_allowed": false' "$PREREG" || fail 'threshold firewall missing'

git merge-base --is-ancestor "$PARENT_BASE" HEAD || fail 'P2E11D1 parent is not an ancestor'
git diff --quiet "$PARENT_BASE" HEAD -- src reference || fail 'P2E13 mutated src or reference'
...
test "$(git rev-parse HEAD:docs/publication/P2E11D1_REFERENCE_EXTENSION_FAILURE_ADJUDICATION_RESULT.json)" = "$P2E11D1_RESULT_BLOB" || fail 'P2E11D1 authority drift'
...
for opt in 0 2; do
  ...
  if ! "$OUT/test" > "$OUT/output.txt" 2>&1; then
    ...
    fail "Reference common-material state-domain diagnostic O$opt"
  fi

  for marker in \
    'PUB_P2E13_PHYSICAL_CASE_COUNT=936' \
    'PUB_P2E13_MATERIAL_COUNT=36' \
    'PUB_P2E13_SE_LEVEL_COUNT=13' \
    'PUB_P2E13_FORCING_COUNT=2' \
    'PUB_P2E13_ROSSFAST_SOLVER_EXECUTED=FALSE' \
    'PUB_P2E13_THRESHOLD_FROZEN=FALSE' \
    'PUB_P2E13_REFERENCE_COMMON_MATERIAL_STATE_DOMAIN_GATE=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing O$opt marker $marker"; }
  done

  domain_lines="$(grep -Fc 'PUB_P2E13_SE_DOMAIN|' "$OUT/output.txt")"
  [[ "$domain_lines" = "13" ]] || { cat "$OUT/output.txt" >&2; fail "expected thirteen state-domain summaries"; }

  cat "$OUT/output.txt"
  echo "PUB_P2E13_O${opt}=PASS"
done

cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" || {
  diff -u "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" >&2 || true
  fail 'O0/O2 common-material state-domain output drift'
}

echo "PUB_P2E13_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'PUB_P2E13_REFERENCE_COMMON_MATERIAL_STATE_DOMAIN_GATE=PASS'
