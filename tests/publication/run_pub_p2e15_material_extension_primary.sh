...
PREREG=docs/publication/P2E15_MATERIAL_EXTENSION_PRIMARY_PREREGISTRATION.json
TEST=tests/publication/test_pub_p2e15_material_extension_primary.f90
...
PREREG_BLOB=930e2dd0af83c28544718aed08e7a88598cd3fe5

[[ -f "$PREREG" ]] || fail 'missing P2E15 preregistration'
[[ -f "$TEST" ]] || fail 'missing P2E15 paired matrix test'

grep -Fq '"phase": "PREREGISTERED_BEFORE_EXTENSION_ROSSFAST_DISCREPANCY_EXECUTION"' "$PREREG" || fail 'preregistration phase drift'
grep -Fq '"case_count": 180' "$PREREG" || fail '180-case matrix not frozen'
...
for opt in 0 2; do
  ...
  if ! "$OUT/test" > "$OUT/output.txt" 2>&1; then
    ...
    fail "material extension primary runtime O$opt"
  fi

  for marker in     'PUB_P2E15_CASE_COUNT=180'     'PUB_P2E15_LAYER=A_FIXED_INTERVAL_SOLVER_SEAM'     'PUB_P2E15_STEP_DURATION_DAY=0.0016'     'PUB_P2E15_THRESHOLDS_RETUNED=FALSE'     'PUB_P2E15_TRANSACTION_LEVEL_PAIR_EXECUTED=FALSE'     'PUB_P2E15_PERFORMANCE_CLAIM_EVALUATED=FALSE'     'PUB_P2E15_MATERIAL_EXTENSION_PRIMARY_GATE=PASS'; do
  ...
  [[ "$cases" = "180" ]] || fail "expected 180 case records, got $cases"
...
cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" || {
  ...
  fail 'O0/O2 material-extension scientific output drift'
}

echo "PUB_P2E15_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'PUB_P2E15_MATERIAL_EXTENSION_PRIMARY_GATE=PASS'
