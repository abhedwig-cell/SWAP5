...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-pub-p2e16-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail() { echo "PUB_P2E16_GATE_FAIL $*" >&2; exit 1; }

TEST=tests/publication/test_pub_p2e16_top_boundary_reference_calibration.f90
PREREG=docs/publication/P2E16_TOP_BOUNDARY_REFERENCE_CALIBRATION_PREREGISTRATION.json
...
grep -Fq '"phase": "PREREGISTERED_BEFORE_TOP_BOUNDARY_REFERENCE_CALIBRATION"' "$PREREG" || fail 'preregistration phase missing'
...
grep -Fq '"forcing_specific_thresholds": false' "$PREREG" || fail 'forcing-threshold firewall missing'
grep -Fq '"threshold_change_after_execution_allowed": false' "$PREREG" || fail 'threshold freeze missing'

git merge-base --is-ancestor "$PARENT_BASE" HEAD || fail 'P2E15 parent is not an ancestor'
git diff --quiet "$PARENT_BASE" HEAD -- src reference || fail 'P2E16 mutated src or reference'
...
test "$(git rev-parse HEAD:docs/publication/P2E15_MATERIAL_EXTENSION_PRIMARY_RESULT.json)" = "$P2E15_RESULT_BLOB" || fail 'P2E15 authority drift'
...
for opt in 0 2; do
  ...
  if ! "$OUT/test" > "$OUT/output.txt" 2>&1; then
    ...
    fail "top-boundary Reference calibration diagnostic O$opt"
  fi

  for marker in \
    'PUB_P2E16_PHYSICAL_CASE_COUNT=324' \
    'PUB_P2E16_MATERIAL_COUNT=36' \
    'PUB_P2E16_SE_LEVEL_COUNT=3' \
    'PUB_P2E16_FORCING_COUNT=3' \
    'PUB_P2E16_ROSSFAST_SOLVER_EXECUTED=FALSE' \
    'PUB_P2E16_BOUNDARY_PROBE_RULE_PREREGISTERED=TRUE' \
    'PUB_P2E16_TOP_BOUNDARY_REFERENCE_CALIBRATION_GATE=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing O$opt marker $marker"; }
  done

  domain_lines="$(grep -Fc 'PUB_P2E16_SE_REFERENCE_DOMAIN|' "$OUT/output.txt")"
  [[ "$domain_lines" = "3" ]] || { cat "$OUT/output.txt" >&2; fail "expected three Reference-domain summaries"; }

  if grep -Fq 'PUB_P2E16_SCIENTIFIC_OUTCOME=QUALIFIED_TOP_BOUNDARY_REFERENCE_SURFACE' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E16_THRESHOLD_SURFACE_FROZEN=TRUE' "$OUT/output.txt" || fail 'qualified threshold surface missing frozen marker'
    threshold_lines="$(grep -Fc 'PUB_P2E16_THRESHOLD|' "$OUT/output.txt")"
    [[ "$threshold_lines" = "15" ]] || { cat "$OUT/output.txt" >&2; fail "expected exactly fifteen frozen threshold values"; }
  elif grep -Fq 'PUB_P2E16_SCIENTIFIC_OUTCOME=BLOCKED_TOP_BOUNDARY_REFERENCE_DOMAIN' "$OUT/output.txt"; then
    grep -Fq 'PUB_P2E16_THRESHOLD_SURFACE_FROZEN=FALSE' "$OUT/output.txt" || fail 'blocked threshold surface missing non-frozen marker'
  else
    cat "$OUT/output.txt" >&2
  ...
  cat "$OUT/output.txt"
  echo "PUB_P2E16_O${opt}=PASS"
done

cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" || {
  diff -u "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" >&2 || true
  fail 'O0/O2 top-boundary Reference calibration output drift'
}

echo "PUB_P2E16_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'PUB_P2E16_TOP_BOUNDARY_REFERENCE_CALIBRATION_GATE=PASS'
