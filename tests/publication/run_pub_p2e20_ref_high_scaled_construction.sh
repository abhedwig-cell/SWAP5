...
TEST_BLOB=c88b7e68785271762561831935c3018ed35920e2
...
for opt in 0 2; do
  ...
  for marker in     'PUB_P2E20_CASE_COUNT=36'     'PUB_P2E20_FINE_LEVEL_COUNT=3'     'PUB_P2E20_REF_HIGH_ENDPOINT=32_SUBSTEPS'     'PUB_P2E20_INTEGRATED_ALLOWANCE_CM=1.6E-15'     'PUB_P2E20_ROSSFAST_EXECUTED=FALSE'     'PUB_P2E20_TIMING_EXECUTED=FALSE'     'PUB_P2E20_PRODUCTION_TOLERANCE_CHANGED=FALSE'     'PUB_P2E20_SCIENTIFIC_RESULT_IS_CI_FAILURE=FALSE'     'PUB_P2E20_CASEWISE_REF_HIGH_CONSTRUCTION_GATE=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing marker $marker"; }
  done

  qualified="$(grep 'PUB_P2E20_QUALIFIED_COUNT=' "$OUT/output.txt" | tail -1 | cut -d= -f2)"
  route_unresolved="$(grep 'PUB_P2E20_ROUTE_UNRESOLVED_COUNT=' "$OUT/output.txt" | tail -1 | cut -d= -f2)"
  stability_unresolved="$(grep 'PUB_P2E20_STABILITY_UNRESOLVED_COUNT=' "$OUT/output.txt" | tail -1 | cut -d= -f2)"
  [[ $((qualified + route_unresolved + stability_unresolved)) -eq 36 ]] || fail 'case classifications do not sum to 36'
