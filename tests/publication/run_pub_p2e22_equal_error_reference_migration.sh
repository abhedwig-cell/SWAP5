...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-pub-p2e22-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail() { echo "PUB_P2E22_GATE_FAIL $*" >&2; exit 1; }

PREREG=docs/publication/P2E22_EQUAL_ERROR_REFERENCE_MIGRATION_PREREGISTRATION.json
PARENT_RESULT=docs/publication/P2E21A_EQUAL_ERROR_CONTROL_SELECTION_RESULT.json
TEST=tests/publication/test_pub_p2e22_equal_error_reference_migration.f90
PARENT_BASE=44a569a55679e793a3657989b9dd895de84ab9ce

PREREG_BLOB=401989d851b036863a636b8df01be9ef0f804268
TEST_BLOB=7928745e0a174cb7d7c6e0d10283fb5a482e142a
P2E21A_RESULT_BLOB=f274b2609263b619118b9313d61eb82bd0eba876
...
[[ -f "$PREREG" && -f "$PARENT_RESULT" && -f "$TEST" ]] || fail 'missing P2E22 authority'
test "$(git rev-parse HEAD:$PREREG)" = "$PREREG_BLOB" || fail 'P2E22 preregistration drift'
test "$(git rev-parse HEAD:$TEST)" = "$TEST_BLOB" || fail 'P2E22 test drift'
test "$(git rev-parse HEAD:$PARENT_RESULT)" = "$P2E21A_RESULT_BLOB" || fail 'P2E21A result drift'
test "$(git rev-parse "$P2E21_RESULT_COMMIT:$P2E21_RESULT_PATH")" = "$P2E21_RESULT_BLOB" || fail 'P2E21 representation-bounded REF-HIGH result drift'

grep -Fq '"phase": "PREREGISTERED_BEFORE_36_CASE_UNTIMED_CONTROL_MIGRATION"' "$PREREG" || fail 'phase drift'
grep -Fq '"cases": 36' "$PREREG" || fail '36-case population drift'
grep -Fq '"segments": [' "$PREREG" || fail 'common control ladder missing'
grep -Fq '"head_theta_target_fractions_of_P2E14": [' "$PREREG" || fail 'accuracy target list missing'
grep -Fq '"timing_calls_allowed": false' "$PREREG" || fail 'timing firewall missing'
grep -Fq '"production_configuration_count": 432' "$PREREG" || fail 'configuration cardinality drift'
...
if grep -Eiq '\b(cpu_time|system_clock)\b' "$TEST"; then
  fail 'timing call detected in untimed P2E22 migration harness'
fi

git merge-base --is-ancestor "$PARENT_BASE" HEAD || fail 'P2E21A result parent is not an ancestor'
git diff --quiet "$PARENT_BASE" HEAD -- src reference || fail 'P2E22 mutated src or reference'
...
for opt in 0 2; do
  ...
  if ! "$OUT/test" > "$OUT/output.txt" 2>&1; then
    ...
    fail "P2E22 runtime O$opt"
  fi

  for marker in \
    'PUB_P2E22_PERFORMANCE_CASE_COUNT=36' \
    'PUB_P2E22_PRODUCTION_CONFIG_COUNT=432' \
    'PUB_P2E22_REF_HIGH_RECONSTRUCTION_COUNT=36' \
    'PUB_P2E22_TIMING_EXECUTED=FALSE' \
    'PUB_P2E22_STORAGE_TARGET_SCALED=FALSE' \
    'PUB_P2E22_ACCURACY_TARGETS_CHANGED=FALSE' \
    'PUB_P2E22_CASE_POPULATION_CHANGED=FALSE' \
    'PUB_P2E22_SCIENTIFIC_RESULT_IS_CI_FAILURE=FALSE' \
    'PUB_P2E22_EQUAL_ERROR_REFERENCE_MIGRATION_GATE=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || { cat "$OUT/output.txt" >&2; fail "missing marker $marker"; }
  done

  [[ "$(grep -Fc 'PUB_P2E22_CONFIG|' "$OUT/output.txt")" = "432" ]] || fail 'expected 432 production configuration records'
  [[ "$(grep -Fc 'PUB_P2E22_REF_HIGH|' "$OUT/output.txt")" = "36" ]] || fail 'expected 36 REF-HIGH records'
  [[ "$(grep -Fc 'PUB_P2E22_SELECTION|' "$OUT/output.txt")" = "216" ]] || fail 'expected 216 solver-target selection records'
  [[ "$(grep -Fc 'PUB_P2E22_MIGRATION|' "$OUT/output.txt")" = "108" ]] || fail 'expected 108 migration records'
  overlap_changed="$(grep 'PUB_P2E22_OVERLAP_SELECTION_CHANGED_COUNT=' "$OUT/output.txt" | tail -1 | cut -d= -f2)"
  overlap_unchanged="$(grep 'PUB_P2E22_OVERLAP_SELECTION_UNCHANGED_COUNT=' "$OUT/output.txt" | tail -1 | cut -d= -f2)"
  new_records="$(grep 'PUB_P2E22_NEW_CASE_SELECTION_RECORD_COUNT=' "$OUT/output.txt" | tail -1 | cut -d= -f2)"
  [[ $((overlap_changed + overlap_unchanged)) -eq 99 ]] || fail 'expected 99 overlapping case-target migration classifications'
  [[ "$new_records" = "9" ]] || fail 'expected 9 new case-target migration records'
  [[ "$(grep -Fc 'PUB_P2E22_THRESHOLD|' "$OUT/output.txt")" = "3" ]] || fail 'expected 3 threshold records'
  [[ "$(grep -Fc 'PUB_P2E22_TARGET|' "$OUT/output.txt")" = "3" ]] || fail 'expected 3 target records'

  cat "$OUT/output.txt"
  echo "PUB_P2E22_O${opt}=PASS"
done

cmp -s "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" || {
  diff -u "$BUILD/o0/output.txt" "$BUILD/o2/output.txt" >&2 || true
  fail 'O0/O2 P2E22 scientific output drift'
}

echo "PUB_P2E22_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'PUB_P2E22_EQUAL_ERROR_REFERENCE_MIGRATION_GATE=PASS'
