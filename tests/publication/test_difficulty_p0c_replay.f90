program test_difficulty_p0c_replay
  ...
  call require(all(r1%candidate_state%pressure_head==r2%candidate_state%pressure_head),'reference order independent')
  call require(all(a1%candidate_state%pressure_head==a2%candidate_state%pressure_head),'alternative order independent')
...
contains
  ...
  subroutine init_request(q,ps,ch,sp,tp,th,k0)
    ...
    q%boundary%top_mode=FSI_TOP_MODE_EXPLICIT_FLUX; q%boundary%top_flux=0.0_real64; q%boundary%bottom_flux=0.0_real64
