program test_pub_me_d2_retry_accounting
  ...
  real(real64), parameter :: historical_rejected_binf = 3.70914014642538750e-6_real64
  real(real64), parameter :: historical_accepted_binf = 1.23637538907952566e-7_real64
  integer(int64), parameter :: column_id = 920002_int64
  ...
  write(*,'(A,ES26.17E3)') 'PUB_ME_D2_REJECTED_BINF_CM=', mutant%rejected_binf
  write(*,'(A,ES26.17E3)') 'PUB_ME_D2_ACCEPTED_BINF_CM=', mutant%accepted_binf
  write(*,'(A,ES26.17E3)') 'PUB_ME_D2_ACCEPTED_TOTAL_IN_CM=', mutant%accepted_total_in
  write(*,'(A,ES26.17E3)') 'PUB_ME_D2_ACCEPTED_TOTAL_OUT_CM=', mutant%accepted_total_out
  write(*,'(A,ES26.17E3)') 'PUB_ME_D2_LEDGER_IN_AT_RETRY_CM=', mutant%ledger_in_at_retry
  write(*,'(A,ES26.17E3)') 'PUB_ME_D2_FINAL_FAULTY_LEDGER_IN_CM=', mutant%final_ledger_in
  write(*,'(A)') 'PUB_ME_D2_B1_FINAL_ACCEPTED_TOTAL_REGRESSION=DETECTED'
  write(*,'(A)') 'PUB_ME_D2_B2_RETRY_ENTRY_AUTHORITY=DETECTED'
  write(*,'(A)') 'PUB_ME_D2_CLASSIFICATION=EARLIER_DETECTION'
  write(*,'(A)') 'PUB_ME_D2_PRIMARY_STAGE_B=PASS'
...
contains
  ...
  subroutine run_case(inject_fault, record)
    ...
    if (inject_fault) then
      ! Qualification-only D2 defect: publish the prescribed rejected-trial
      ! throughflow into an accepted ledger before acceptance. q is positive
      ! outward at the top and inward at the prescribed-qbot lower boundary.
      faulty_in = q*rejected_dt
      faulty_out = q*rejected_dt
    end if
