program test_pub_p2e08_reference_valid_calibration_domain
  ...
  integer, parameter :: nmat=6, nse=3, nforcing=3, ndt=5, nphys=nmat*nse*nforcing
  ...
  do idt=1,ndt
    ...
    call require(ip==nphys,'exact 54 physical cases at each temporal level')
  end do
  ...
  write(*,'(A,I0)') 'PUB_P2E08_PHYSICAL_CASE_COUNT=',nphys
  write(*,'(A,I0)') 'PUB_P2E08_TEMPORAL_LEVEL_COUNT=',ndt
  write(*,'(A,I0)') 'PUB_P2E08_TRIAL_PAIR_COUNT=',nphys*ndt
  do idt=1,ndt
    write(*,'(*(g0))') 'PUB_P2E08_DT_FEASIBILITY|INDEX=',idt,'|DT=',dt_levels(idt), &
         '|VALID=',valid_by_dt(idt),'|INVALID=',invalid_by_dt(idt)
  end do
  write(*,'(A)') 'PUB_P2E08_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E08_THRESHOLD_FROZEN=FALSE'
  write(*,'(A)') 'PUB_P2E08_REFERENCE_TOLERANCE_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E08_P2E03_REPAIRED_OR_RECLASSIFIED=FALSE'
  ...
  write(*,'(*(g0))') 'PUB_P2E08_SELECTED_COARSE_DT_DAY=',dt_levels(selected_dt_index)
  write(*,'(*(g0))') 'PUB_P2E08_SELECTED_HALF_DT_DAY=',0.5_real64*dt_levels(selected_dt_index)
  write(*,'(A,I0)') 'PUB_P2E08_SELECTED_VALID_CASE_COUNT=',valid_by_dt(selected_dt_index)
  ...
  do imat=1,nmat
    do ise=1,nse
      do iforce=1,nforcing
        ...
        write(*,'(*(g0))') 'PUB_P2E08_SELECTED_CASE|CASE=',ip,'|M=',trim(material_ids(imat)), &
...
contains
  ...
  subroutine run_case(case_id,dt_index,material_id,se,forcing_id,top_factor,bottom_factor,coarse_dt,valid, &
    ...
    call report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,failure_stage)
    write(*,'(*(g0))') 'PUB_P2E08_TYPED_MASS|DT_INDEX=',dt_index,'|CASE=',case_id, &
  ...
  subroutine report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,stage)
    ...
    write(*,'(*(g0))') 'PUB_P2E08_TRIAL|DT_INDEX=',dt_index,'|CASE=',case_id,'|M=',trim(material_id), &
  ...
  subroutine report_metric_summary(label,values)
    ...
    write(*,'(*(g0))') 'PUB_P2E08_METRIC_SUMMARY|METRIC=',trim(label),'|MIN=',minval(values), &
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E08_FAIL',trim(label)
      error stop 1
    end if
