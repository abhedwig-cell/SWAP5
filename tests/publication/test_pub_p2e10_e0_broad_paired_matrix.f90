program test_pub_p2e10_e0_broad_paired_matrix
  ...
  integer, parameter :: nmat=6, nse=3, nforcing=3, expected_cases=nmat*nse*nforcing
  ...
  character(len=7), parameter :: forcing_ids(nforcing) = [character(len=7) :: 'DRYING ','NOMINAL','WETTING']
  real(real64), parameter :: se_levels(nse) = [0.65_real64,0.85_real64,0.98_real64]
  real(real64), parameter :: qtop_factor(nforcing) = [-0.005_real64,0.010_real64,0.025_real64]
  real(real64), parameter :: qbot_factor(nforcing) = [-0.019_real64,-0.004_real64,0.011_real64]
  ...
  call require(case_id==expected_cases,'exact 54-case broad matrix attempted')
  ...
  write(*,'(A,I0)') 'PUB_P2E10_CASE_COUNT=',case_id
  write(*,'(A,I0)') 'PUB_P2E10_ADMISSIBLE_COUNT=',count_admissible
  write(*,'(A,I0)') 'PUB_P2E10_DISCREPANCY_FAIL_COUNT=',count_discrepancy_fail
  write(*,'(A,I0)') 'PUB_P2E10_REFERENCE_INVALID_COUNT=',count_reference_invalid
  write(*,'(A,I0)') 'PUB_P2E10_ROSSFAST_INVALID_COUNT=',count_rossfast_invalid
  write(*,'(A,I0)') 'PUB_P2E10_BOTH_INVALID_COUNT=',count_both_invalid
  write(*,'(A,I0)') 'PUB_P2E10_FAIL_D_H_INF_COUNT=',fail_h_inf
  write(*,'(A,I0)') 'PUB_P2E10_FAIL_D_H_RMS_COUNT=',fail_h_rms
  write(*,'(A,I0)') 'PUB_P2E10_FAIL_D_THETA_INF_COUNT=',fail_theta_inf
  write(*,'(A,I0)') 'PUB_P2E10_FAIL_D_THETA_RMS_COUNT=',fail_theta_rms
  write(*,'(A,I0)') 'PUB_P2E10_FAIL_D_STORAGE_COUNT=',fail_storage
  write(*,'(A)') 'PUB_P2E10_LAYER=A_FIXED_INTERVAL_SOLVER_SEAM'
  write(*,'(A)') 'PUB_P2E10_STEP_DURATION_DAY=0.0016'
  write(*,'(A)') 'PUB_P2E10_THRESHOLDS_RETUNED=FALSE'
  write(*,'(A)') 'PUB_P2E10_TRANSACTION_LEVEL_PAIR_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E10_PERFORMANCE_CLAIM_EVALUATED=FALSE'
  write(*,'(A)') 'PUB_P2E10_BROAD_MATRIX_EXECUTION_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,ise,material_id,se,forcing_id,top_factor,bottom_factor,classification,metrics_available, &
    ...
    write(*,'(*(g0))') 'PUB_P2E10_CASE|CASE=',case_id,'|M=',trim(material_id),'|SE=',se, &
    ...
    if (reference_result%integrated_mass_balance_residual_available) then
      write(*,'(*(g0))') 'PUB_P2E10_REFERENCE_MASS|CASE=',case_id,'|CM=',reference_result%integrated_mass_balance_residual_cm, &
      ...
      write(*,'(*(g0))') 'PUB_P2E10_REFERENCE_MASS|CASE=',case_id,'|AVAILABLE=FALSE'
    end if

    if (initialized .and. alternative_result%integrated_mass_balance_residual_available) then
      write(*,'(*(g0))') 'PUB_P2E10_ROSSFAST_MASS|CASE=',case_id,'|CM=',alternative_result%integrated_mass_balance_residual_cm, &
           '|RATE_AVAILABLE=',alternative_result%native_balance_rate_residual_available
    else
      write(*,'(*(g0))') 'PUB_P2E10_ROSSFAST_MASS|CASE=',case_id,'|AVAILABLE=FALSE'
    end if
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E10_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_pub_p2e10_e0_broad_paired_matrix
