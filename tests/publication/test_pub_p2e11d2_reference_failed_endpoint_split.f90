program test_pub_p2e11d2_reference_failed_endpoint_split
  use, intrinsic :: iso_fortran_env, only: real64, real128
  use, intrinsic :: ieee_arithmetic, only: ieee_is_finite
  use mod_soil_water_solver_contract, only: soil_water_parameter_set_t, soil_water_solve_request_t, &
       soil_water_solve_result_t, SW_SOLVE_CONVERGED, SW_SOLVE_RETRY_ADVISED, SW_SOLVE_FAILED
  ...
  integer, parameter :: nclass = 7, ndt = 5
  ...
  character(len=3), parameter :: material_ids(30) = [character(len=3) :: &
       'B02','B03','B04','B05','B06','B07','B08','B09','B10','B11', &
       'B13','B14','B15','B16','B17','B18','O02','O03','O04','O06', &
       'O07','O08','O09','O10','O11','O12','O13','O15','O16','O17']
  ...
  real(real64), parameter :: dt_levels(ndt) = [0.0016_real64,0.0032_real64,0.0064_real64,0.0128_real64,0.0256_real64]
      ...
      do iforce=1,size(forcing_ids)
        do idt=1,ndt
          ...
          if (class_id/=C_ACCEPTED) then
            write(*,'(*(g0))') 'PUB_P2E11D2_FAILURE|PHYS_CASE=', &
                 (imat-1)*size(se_levels)*size(forcing_ids)+(ise-1)*size(forcing_ids)+iforce, &
                 '|DT_INDEX=',idt,'|M=',trim(material_ids(imat)),'|SE=',se_levels(ise), &
                 '|F=',trim(forcing_ids(iforce)),'|DT=',dt_levels(idt), &
                 '|CLASS=',trim(class_name(class_id)),'|STAGE=',trim(stage_label(failed_stage))
          end if
  ...
  call require(total_cases==900,'exact extension diagnostic matrix replayed')

  write(*,'(A,I0)') 'PUB_P2E11D2_CASE_COUNT=',total_cases
  write(*,'(A,I0)') 'PUB_P2E11D2_ACCEPTED_COUNT=',counts(C_ACCEPTED)
  write(*,'(A,I0)') 'PUB_P2E11D2_TOTAL_ONLY_COUNT=',counts(C_TOTAL_ONLY)
  write(*,'(A,I0)') 'PUB_P2E11D2_LOCAL_BAL_COUNT=',counts(C_LOCAL_BAL)
  write(*,'(A,I0)') 'PUB_P2E11D2_HEAD_COUNT=',counts(C_HEAD)
  write(*,'(A,I0)') 'PUB_P2E11D2_MIXED_COUNT=',counts(C_MIXED)
  write(*,'(A,I0)') 'PUB_P2E11D2_RETRY_OTHER_COUNT=',counts(C_RETRY_OTHER)
  write(*,'(A,I0)') 'PUB_P2E11D2_FAILED_COUNT=',counts(C_FAILED)
  write(*,'(A,I0)') 'PUB_P2E11D2_LOCAL_STAGE_COARSE=',local_stage_counts(S_COARSE)
  write(*,'(A,I0)') 'PUB_P2E11D2_LOCAL_STAGE_HALF1=',local_stage_counts(S_HALF1)
  write(*,'(A,I0)') 'PUB_P2E11D2_LOCAL_STAGE_HALF2=',local_stage_counts(S_HALF2)

  do idt=1,ndt
    write(*,'(*(g0))') 'PUB_P2E11D2_CLASS_BY_DT|DT=',dt_levels(idt), &
         '|ACCEPTED=',counts_by_dt(C_ACCEPTED,idt), &
         '|TOTAL_ONLY=',counts_by_dt(C_TOTAL_ONLY,idt), &
         '|LOCAL_BAL=',counts_by_dt(C_LOCAL_BAL,idt), &
         '|HEAD=',counts_by_dt(C_HEAD,idt), &
         '|MIXED=',counts_by_dt(C_MIXED,idt), &
         '|RETRY_OTHER=',counts_by_dt(C_RETRY_OTHER,idt), &
         '|FAILED=',counts_by_dt(C_FAILED,idt)
    if (counts_by_dt(C_LOCAL_BAL,idt)>0) then
      write(*,'(*(g0))') 'PUB_P2E11D2_DT_SUMMARY|DT=',dt_levels(idt), &
  ...
  write(*,'(*(g0))') 'PUB_P2E11D2_MAX_RECONSTRUCTION_CLOSURE=',max_closure_abs
  write(*,'(A,I0)') 'PUB_P2E11D2_RECONSTRUCTION_SUPPORTED_TOTAL=',sum(reconstruction_supported_by_dt)
  write(*,'(A,I0)') 'PUB_P2E11D2_FLOOR_GE_TOL_TOTAL=',sum(floor_ge_tol_by_dt)
  write(*,'(A,I0)') 'PUB_P2E11D2_INTERNAL_STORAGE_NONZERO_TOTAL=',sum(internal_storage_nonzero_by_dt)
  write(*,'(A,I0)') 'PUB_P2E11D2_PUBLIC_STORAGE_ZERO_TOTAL=',sum(public_storage_zero_by_dt)
  write(*,'(A)') 'PUB_P2E11D2_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E11D2_PRODUCTION_OR_REFERENCE_SOURCE_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E11D2_TOLERANCE_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E11D2_REFERENCE_FAILED_ENDPOINT_SPLIT=PASS'
...
contains
  ...
  subroutine diagnose_case(case_id,material_id,se,forcing_id,top_factor,bottom_factor,coarse_dt,class_id,failed_stage, &
    ...
    if (class_id/=C_ACCEPTED) then
      ...
      if (class_id==C_FAILED) call emit_failed_detail(case_id,material_id,se,forcing_id,coarse_dt,failed_stage, &
           request,result,material)
  ...
  subroutine emit_failed_detail(case_id,material_id,se,forcing_id,coarse_dt,stage,request,result,material)
    ...
    real(real64) :: hminv,hmaxv,tminv,tmaxv,native_rate,integrated
    logical :: head_shape_ok,theta_shape_ok

    reason=failed_reason(result,material)
    hminv=huge(0.0_real64); hmaxv=-huge(0.0_real64)
    tminv=huge(0.0_real64); tmaxv=-huge(0.0_real64)
    head_shape_ok=allocated(result%candidate_state%pressure_head)
    if (head_shape_ok) head_shape_ok=size(result%candidate_state%pressure_head)==n
    theta_shape_ok=allocated(result%candidate_state%water_content)
    if (theta_shape_ok) theta_shape_ok=size(result%candidate_state%water_content)==n
    if (head_shape_ok) then
      hminv=minval(result%candidate_state%pressure_head)
      hmaxv=maxval(result%candidate_state%pressure_head)
    end if
    if (theta_shape_ok) then
      tminv=minval(result%candidate_state%water_content)
      tmaxv=maxval(result%candidate_state%water_content)
    end if
    native_rate=huge(0.0_real64)
    if (result%native_balance_rate_residual_available) native_rate=result%native_balance_rate_residual_cm_per_day
    integrated=huge(0.0_real64)
    if (result%integrated_mass_balance_residual_available) integrated=result%integrated_mass_balance_residual_cm

    write(*,'(*(g0))') 'PUB_P2E11D2_FAILED_DETAIL|CASE=',case_id,'|M=',trim(material_id),'|SE=',se, &
         '|F=',trim(forcing_id),'|COARSE_DT=',coarse_dt,'|STAGE=',trim(stage_label(stage)), &
         '|ACTUAL_DT=',request%step_duration,'|STATUS=',result%status,'|RETRY=',result%retry_advised, &
         '|ROUTE=',trim(result%diagnostics%route),'|REASON=',trim(reason), &
         '|NATIVE_AVAILABLE=',result%native_balance_rate_residual_available,'|NATIVE_RATE=',native_rate, &
         '|INTEGRATED_AVAILABLE=',result%integrated_mass_balance_residual_available,'|INTEGRATED_CM=',integrated, &
         '|HMIN=',hminv,'|HMAX=',hmaxv,'|H_LOWER=',ROSSFAST_D3R_H_MIN_CM,'|H_UPPER=',ROSSFAST_D3R_H_MAX_CM, &
         '|THETA_MIN=',tminv,'|THETA_MAX=',tmaxv,'|THETA_R=',material%theta_r,'|THETA_S=',material%theta_s
  end subroutine emit_failed_detail

  function failed_reason(result,material) result(reason)
    type(soil_water_solve_result_t),intent(in) :: result
    type(rossfast_d3r_material_t),intent(in) :: material
    character(len=64) :: reason

    reason='UNKNOWN_FAILED_CLASS'
    if (result%status==SW_SOLVE_FAILED) then
      reason='SOLVER_STATUS_FAILED'
      return
    end if
    if (result%status/=SW_SOLVE_CONVERGED) then
      reason='NONCONVERGED_STATUS_OTHER'
      return
    end if
    if (trim(result%diagnostics%route)/='legacy-reference-bound') then
      reason='CONVERGED_ROUTE_MISMATCH'
      return
    end if
    if (.not.result%native_balance_rate_residual_available) then
      reason='NATIVE_RATE_UNAVAILABLE'
      return
    end if
    if (.not.ieee_is_finite(result%native_balance_rate_residual_cm_per_day)) then
      reason='NATIVE_RATE_NONFINITE'
      return
    end if
    if (abs(result%native_balance_rate_residual_cm_per_day)>reference_balance_rate_tol) then
      reason='CONVERGED_NATIVE_RATE_GT_TOL'
      return
    end if
    if (result%candidate_state%active_nodes/=n) then
      reason='ENDPOINT_ACTIVE_NODE_MISMATCH'
      return
    end if
    if (.not.allocated(result%candidate_state%pressure_head)) then
      reason='ENDPOINT_HEAD_UNALLOCATED'
      return
    end if
    if (.not.allocated(result%candidate_state%water_content)) then
      reason='ENDPOINT_THETA_UNALLOCATED'
      return
    end if
    if (size(result%candidate_state%pressure_head)/=n) then
      reason='ENDPOINT_HEAD_SHAPE'
      return
    end if
    if (size(result%candidate_state%water_content)/=n) then
      reason='ENDPOINT_THETA_SHAPE'
      return
    end if
    if (any(.not.ieee_is_finite(result%candidate_state%pressure_head))) then
      reason='ENDPOINT_HEAD_NONFINITE'
      return
    end if
    if (any(.not.ieee_is_finite(result%candidate_state%water_content))) then
      reason='ENDPOINT_THETA_NONFINITE'
      return
    end if
    if (any(result%candidate_state%pressure_head<=ROSSFAST_D3R_H_MIN_CM)) then
      reason='ENDPOINT_HEAD_AT_OR_BELOW_LOWER'
      return
    end if
    if (any(result%candidate_state%pressure_head>=ROSSFAST_D3R_H_MAX_CM)) then
      reason='ENDPOINT_HEAD_AT_OR_ABOVE_UPPER'
      return
    end if
    if (any(result%candidate_state%water_content<=material%theta_r)) then
      reason='ENDPOINT_THETA_AT_OR_BELOW_RESIDUAL'
      return
    end if
    if (any(result%candidate_state%water_content>=material%theta_s)) then
      reason='ENDPOINT_THETA_AT_OR_ABOVE_SATURATION'
      return
    end if
  end function failed_reason
  ...
  subroutine diagnose_local_failure(case_id,material_id,se,forcing_id,coarse_dt,stage,request,result,workspace, &
    ...
    write(*,'(*(g0))') 'PUB_P2E11D2_LOCAL|CASE=',case_id,'|M=',trim(material_id),'|SE=',se,'|F=',trim(forcing_id), &
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then; write(*,'(A,1X,A)') 'PUB_P2E11D2_FAIL',trim(label); error stop 1; end if
  end subroutine require
end program test_pub_p2e11d2_reference_failed_endpoint_split
