program test_pub_p2e11d_extension_reference_failure_diagnostics
      ...
      do iforce=1,size(forcing_ids)
        do idt=1,ndt
          ...
          if (class_id/=C_ACCEPTED) then
            write(*,'(*(g0))') 'PUB_P2E11D_FAILURE|PHYS_CASE=', &
  ...
  write(*,'(A,I0)') 'PUB_P2E11D_CASE_COUNT=',total_cases
  write(*,'(A,I0)') 'PUB_P2E11D_ACCEPTED_COUNT=',counts(C_ACCEPTED)
  write(*,'(A,I0)') 'PUB_P2E11D_TOTAL_ONLY_COUNT=',counts(C_TOTAL_ONLY)
  write(*,'(A,I0)') 'PUB_P2E11D_LOCAL_BAL_COUNT=',counts(C_LOCAL_BAL)
  write(*,'(A,I0)') 'PUB_P2E11D_HEAD_COUNT=',counts(C_HEAD)
  write(*,'(A,I0)') 'PUB_P2E11D_MIXED_COUNT=',counts(C_MIXED)
  write(*,'(A,I0)') 'PUB_P2E11D_RETRY_OTHER_COUNT=',counts(C_RETRY_OTHER)
  write(*,'(A,I0)') 'PUB_P2E11D_FAILED_COUNT=',counts(C_FAILED)
  write(*,'(A,I0)') 'PUB_P2E11D_LOCAL_STAGE_COARSE=',local_stage_counts(S_COARSE)
  write(*,'(A,I0)') 'PUB_P2E11D_LOCAL_STAGE_HALF1=',local_stage_counts(S_HALF1)
  write(*,'(A,I0)') 'PUB_P2E11D_LOCAL_STAGE_HALF2=',local_stage_counts(S_HALF2)

  do idt=1,ndt
    write(*,'(*(g0))') 'PUB_P2E11D_CLASS_BY_DT|DT=',dt_levels(idt), &
    ...
    if (counts_by_dt(C_LOCAL_BAL,idt)>0) then
      write(*,'(*(g0))') 'PUB_P2E11D_DT_SUMMARY|DT=',dt_levels(idt), &
  ...
  write(*,'(*(g0))') 'PUB_P2E11D_MAX_RECONSTRUCTION_CLOSURE=',max_closure_abs
  write(*,'(A,I0)') 'PUB_P2E11D_RECONSTRUCTION_SUPPORTED_TOTAL=',sum(reconstruction_supported_by_dt)
  write(*,'(A,I0)') 'PUB_P2E11D_FLOOR_GE_TOL_TOTAL=',sum(floor_ge_tol_by_dt)
  write(*,'(A,I0)') 'PUB_P2E11D_INTERNAL_STORAGE_NONZERO_TOTAL=',sum(internal_storage_nonzero_by_dt)
  write(*,'(A,I0)') 'PUB_P2E11D_PUBLIC_STORAGE_ZERO_TOTAL=',sum(public_storage_zero_by_dt)
  write(*,'(A)') 'PUB_P2E11D_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E11D_PRODUCTION_OR_REFERENCE_SOURCE_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E11D_TOLERANCE_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E11D_REFERENCE_FAILURE_DIAGNOSTICS=PASS'
...
contains
  ...
  subroutine diagnose_local_failure(case_id,material_id,se,forcing_id,coarse_dt,stage,request,result,workspace, &
    ...
    write(*,'(*(g0))') 'PUB_P2E11D_LOCAL|CASE=',case_id,'|M=',trim(material_id),'|SE=',se,'|F=',trim(forcing_id), &
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then; write(*,'(A,1X,A)') 'PUB_P2E11D_FAIL',trim(label); error stop 1; end if
  end subroutine require
end program test_pub_p2e11d_extension_reference_failure_diagnostics
