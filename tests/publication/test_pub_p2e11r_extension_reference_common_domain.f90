program test_pub_p2e11r_extension_reference_common_domain
  ...
  integer, parameter :: nmat=30, nse=3, nforcing=2, ndt=5, nphys=nmat*nse*nforcing
  ...
  write(*,'(A,I0)') 'PUB_P2E11R_PHYSICAL_CASE_COUNT=',nphys
  write(*,'(A,I0)') 'PUB_P2E11R_TEMPORAL_LEVEL_COUNT=',ndt
  write(*,'(A,I0)') 'PUB_P2E11R_TRIAL_PAIR_COUNT=',nphys*ndt
  do idt=1,ndt
    write(*,'(*(g0))') 'PUB_P2E11R_DT_FEASIBILITY|INDEX=',idt,'|DT=',dt_levels(idt), &
         '|VALID=',valid_by_dt(idt),'|INVALID=',invalid_by_dt(idt)
  end do
  write(*,'(A)') 'PUB_P2E11R_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E11R_THRESHOLD_FILE_FROZEN=FALSE'
  write(*,'(A)') 'PUB_P2E11R_REFERENCE_TOLERANCE_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E11R_P2E03_REPAIRED_OR_RECLASSIFIED=FALSE'

  if (selected_dt_index==0) then
    write(*,'(A)') 'PUB_P2E11R_COMMON_DOMAIN_STATUS=BLOCKED_REFERENCE_COMMON_DOMAIN'
    write(*,'(A)') 'PUB_P2E11R_SCIENTIFIC_OUTCOME=NEGATIVE_COMMON_DOMAIN_NOT_FOUND'
    write(*,'(A)') 'PUB_P2E11R_REFERENCE_VALID_CALIBRATION_DOMAIN_GATE=PASS'
    stop
  end if

  write(*,'(A,I0)') 'PUB_P2E11R_SELECTED_DT_INDEX=',selected_dt_index
  write(*,'(*(g0))') 'PUB_P2E11R_SELECTED_COARSE_DT_DAY=',dt_levels(selected_dt_index)
  write(*,'(*(g0))') 'PUB_P2E11R_SELECTED_HALF_DT_DAY=',0.5_real64*dt_levels(selected_dt_index)
  write(*,'(A,I0)') 'PUB_P2E11R_SELECTED_VALID_CASE_COUNT=',valid_by_dt(selected_dt_index)
  ...
  do imat=1,nmat
    do ise=1,nse
      do iforce=1,nforcing
        ...
        write(*,'(*(g0))') 'PUB_P2E11R_SELECTED_CASE|CASE=',ip,'|M=',trim(material_ids(imat)), &
  ...
  write(*,'(A)') 'PUB_P2E11R_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_REFERENCE_DOMAIN'
  write(*,'(A)') 'PUB_P2E11R_SCIENTIFIC_OUTCOME=QUALIFIED_COMPLETE_DOMAIN'
  write(*,'(A)') 'PUB_P2E11R_REFERENCE_VALID_CALIBRATION_DOMAIN_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,dt_index,material_id,se,forcing_id,top_factor,bottom_factor,coarse_dt,valid, &
    ...
    call report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,failure_stage)
    write(*,'(*(g0))') 'PUB_P2E11R_TYPED_MASS|DT_INDEX=',dt_index,'|CASE=',case_id, &
  ...
  subroutine report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,stage)
    ...
    write(*,'(*(g0))') 'PUB_P2E11R_TRIAL|DT_INDEX=',dt_index,'|CASE=',case_id,'|M=',trim(material_id), &
  ...
  subroutine report_metric_summary(label,values)
    ...
    write(*,'(*(g0))') 'PUB_P2E11R_METRIC_SUMMARY|METRIC=',trim(label),'|MIN=',minval(values), &
  ...
  subroutine report_metric_by_se(label,values)
    ...
    do ise=1,nse
      ...
      write(*,'(*(g0))') 'PUB_P2E11R_SE_MAX|SE=',se_levels(ise),'|METRIC=',trim(label),'|MAX=',vmax
    end do
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E11R_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_pub_p2e11r_extension_reference_common_domain
