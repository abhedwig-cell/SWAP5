program test_pub_p2e12_extension_common_domain_threshold_freeze
  ...
  integer, parameter :: nmat=30, nse=3, nforcing=2, ndt=1, nphys=nmat*nse*nforcing
  ...
  do idt=1,ndt
    ...
    call require(ip==nphys,'exact 180 physical cases at each temporal level')
  end do

  call require(sum(valid_by_dt+invalid_by_dt)==nphys*ndt,'exact 180 preregistered Reference pairs executed')

  selected_dt_index=1
  call require(valid_by_dt(selected_dt_index)==176,'exact 176 in-common-domain extension cases at frozen schedule')
  call require(invalid_by_dt(selected_dt_index)==4,'exact four explicit common-domain exclusions')

  write(*,'(A,I0)') 'PUB_P2E12_PHYSICAL_CASE_COUNT=',nphys
  write(*,'(A,I0)') 'PUB_P2E12_TEMPORAL_LEVEL_COUNT=',ndt
  write(*,'(A,I0)') 'PUB_P2E12_TRIAL_PAIR_COUNT=',nphys*ndt
  write(*,'(*(g0))') 'PUB_P2E12_FROZEN_COARSE_DT_DAY=',dt_levels(selected_dt_index)
  write(*,'(*(g0))') 'PUB_P2E12_FROZEN_HALF_DT_DAY=',0.5_real64*dt_levels(selected_dt_index)
  write(*,'(A,I0)') 'PUB_P2E12_IN_COMMON_DOMAIN_COUNT=',valid_by_dt(selected_dt_index)
  write(*,'(A,I0)') 'PUB_P2E12_COMMON_DOMAIN_EXCLUSION_COUNT=',invalid_by_dt(selected_dt_index)
  write(*,'(A)') 'PUB_P2E12_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E12_REFERENCE_TOLERANCE_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E12_COMMON_DOMAIN_PREDICATE_CHANGED=FALSE'
  ...
  do imat=1,nmat
    do ise=1,nse
      do iforce=1,nforcing
        ip=ip+1
        if (valid_matrix(ip,selected_dt_index)) then
          write(*,'(*(g0))') 'PUB_P2E12_IN_DOMAIN_CASE|CASE=',ip,'|M=',trim(material_ids(imat)), &
          ...
          write(*,'(*(g0))') 'PUB_P2E12_DOMAIN_EXCLUSION|CASE=',ip,'|M=',trim(material_ids(imat)), &
               '|SE=',se_levels(ise),'|F=',trim(forcing_ids(iforce)),'|DT=',dt_levels(selected_dt_index)
        end if
      end do
    end do
  end do

  call report_threshold_by_se('U_H_INF',uh_inf(:,selected_dt_index),valid_matrix(:,selected_dt_index))
  call report_threshold_by_se('U_H_RMS',uh_rms(:,selected_dt_index),valid_matrix(:,selected_dt_index))
  call report_threshold_by_se('U_THETA_INF',utheta_inf(:,selected_dt_index),valid_matrix(:,selected_dt_index))
  call report_threshold_by_se('U_THETA_RMS',utheta_rms(:,selected_dt_index),valid_matrix(:,selected_dt_index))
  call report_threshold_by_se('U_STORAGE',ustorage(:,selected_dt_index),valid_matrix(:,selected_dt_index))

  write(*,'(A)') 'PUB_P2E12_THRESHOLD_RULE=SE_STRATIFIED_REFERENCE_MAX_FACTOR_1_FLOOR_0'
  write(*,'(A)') 'PUB_P2E12_THRESHOLD_FREEZE_STATUS=QUALIFIED_REFERENCE_ONLY_COMMON_DOMAIN'
  write(*,'(A)') 'PUB_P2E12_EXTENSION_THRESHOLD_FREEZE_GATE=PASS'
    stop
  end if

  write(*,'(A,I0)') 'PUB_P2E12_SELECTED_DT_INDEX=',selected_dt_index
  write(*,'(*(g0))') 'PUB_P2E12_SELECTED_COARSE_DT_DAY=',dt_levels(selected_dt_index)
  write(*,'(*(g0))') 'PUB_P2E12_SELECTED_HALF_DT_DAY=',0.5_real64*dt_levels(selected_dt_index)
  write(*,'(A,I0)') 'PUB_P2E12_SELECTED_VALID_CASE_COUNT=',valid_by_dt(selected_dt_index)
  ...
  do imat=1,nmat
    do ise=1,nse
      do iforce=1,nforcing
        ip=ip+1
        call require(valid_matrix(ip,selected_dt_index),'selected common dt retains every physical case')
        write(*,'(*(g0))') 'PUB_P2E12_SELECTED_CASE|CASE=',ip,'|M=',trim(material_ids(imat)), &
             '|SE=',se_levels(ise),'|F=',trim(forcing_ids(iforce)), &
             '|DT=',dt_levels(selected_dt_index),'|U_H_INF=',uh_inf(ip,selected_dt_index), &
             '|U_H_RMS=',uh_rms(ip,selected_dt_index),'|U_THETA_INF=',utheta_inf(ip,selected_dt_index), &
             '|U_THETA_RMS=',utheta_rms(ip,selected_dt_index),'|U_STORAGE=',ustorage(ip,selected_dt_index)
  ...
  call report_metric_by_se('U_H_INF',uh_inf(:,selected_dt_index))
  call report_metric_by_se('U_H_RMS',uh_rms(:,selected_dt_index))
  call report_metric_by_se('U_THETA_INF',utheta_inf(:,selected_dt_index))
  call report_metric_by_se('U_THETA_RMS',utheta_rms(:,selected_dt_index))
  call report_metric_by_se('U_STORAGE',ustorage(:,selected_dt_index))

  write(*,'(A)') 'PUB_P2E12_COMMON_DOMAIN_STATUS=QUALIFIED_COMPLETE_REFERENCE_DOMAIN'
  write(*,'(A)') 'PUB_P2E12_SCIENTIFIC_OUTCOME=QUALIFIED_COMPLETE_DOMAIN'
  write(*,'(A)') 'PUB_P2E12_REFERENCE_VALID_CALIBRATION_DOMAIN_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,dt_index,material_id,se,forcing_id,top_factor,bottom_factor,coarse_dt,valid, &
    ...
    call report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,failure_stage)
    write(*,'(*(g0))') 'PUB_P2E12_TYPED_MASS|DT_INDEX=',dt_index,'|CASE=',case_id, &
  ...
  subroutine report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,stage)
    ...
    write(*,'(*(g0))') 'PUB_P2E12_TRIAL|DT_INDEX=',dt_index,'|CASE=',case_id,'|M=',trim(material_id), &
         '|SE=',se,'|F=',trim(forcing_id),'|DT=',coarse_dt,'|VALID=',valid,'|STAGE=',trim(stage)
  end subroutine report_trial

  subroutine report_threshold_by_se(label,values,valid_mask)
    character(len=*),intent(in) :: label
    real(real64),intent(in) :: values(:)
    logical,intent(in) :: valid_mask(:)
    real(real64) :: vmax
    integer :: ise,imat,iforce,ip,nvalid
    call require(size(values)==nphys .and. size(valid_mask)==nphys,'threshold vectors cover all physical cases')
    do ise=1,nse
      vmax=-huge(0.0_real64); nvalid=0
      do imat=1,nmat
        do iforce=1,nforcing
          ip=(imat-1)*nse*nforcing+(ise-1)*nforcing+iforce
          if (.not.valid_mask(ip)) cycle
          call require(ieee_is_finite(values(ip)),'finite in-domain Reference metric')
          vmax=max(vmax,values(ip)); nvalid=nvalid+1
        end do
      end do
      if (ise==1 .or. ise==2) call require(nvalid==60,'complete 60-case lower-Se stratum')
      if (ise==3) call require(nvalid==56,'56-case wet-state common domain after explicit exclusions')
      write(*,'(*(g0))') 'PUB_P2E12_THRESHOLD|SE=',se_levels(ise),'|METRIC=',trim(label), &
           '|N=',nvalid,'|VALUE=',vmax
    end do
  ...
  subroutine report_metric_summary(label,values)
    character(len=*),intent(in) :: label
    real(real64),intent(in) :: values(:)
    call require(size(values)==nphys,'metric summary contains all 54 cases')
    call require(all(ieee_is_finite(values)),'finite selected metric summary')
    write(*,'(*(g0))') 'PUB_P2E12_METRIC_SUMMARY|METRIC=',trim(label),'|MIN=',minval(values), &
         '|MEAN=',sum(values)/real(size(values),real64),'|MAX=',maxval(values)
  end subroutine report_metric_summary

  subroutine report_metric_by_se(label,values)
    character(len=*),intent(in) :: label
    real(real64),intent(in) :: values(:)
    real(real64) :: vmax
    integer :: ise,imat,iforce,ip
    call require(size(values)==nphys,'Se metric summary contains all 180 cases')
    do ise=1,nse
      vmax=-huge(0.0_real64)
      do imat=1,nmat
        do iforce=1,nforcing
          ip=(imat-1)*nse*nforcing+(ise-1)*nforcing+iforce
          call require(ieee_is_finite(values(ip)),'finite Se metric value')
          vmax=max(vmax,values(ip))
        end do
      end do
      write(*,'(*(g0))') 'PUB_P2E12_SE_MAX|SE=',se_levels(ise),'|METRIC=',trim(label),'|MAX=',vmax
    end do
  end subroutine report_metric_by_se
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E12_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_pub_p2e12_extension_common_domain_threshold_freeze
