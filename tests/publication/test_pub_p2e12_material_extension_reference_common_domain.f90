program test_pub_p2e12_material_extension_reference_common_domain
  ...
  do idt=1,ndt
    write(*,'(*(g0))') 'PUB_P2E12_DT_FEASIBILITY|INDEX=',idt,'|DT=',dt_levels(idt), &
         '|VALID=',valid_by_dt(idt),'|INVALID=',invalid_by_dt(idt)
  end do
  write(*,'(A)') 'PUB_P2E12_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E12_THRESHOLD_FROZEN=FALSE'
  write(*,'(A)') 'PUB_P2E12_REFERENCE_TOLERANCE_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E12_P2E03_REPAIRED_OR_RECLASSIFIED=FALSE'

  if (selected_dt_index==0) then
    write(*,'(A)') 'PUB_P2E12_COMMON_DOMAIN_STATUS=BLOCKED_REFERENCE_COMMON_DOMAIN'
    write(*,'(A)') 'PUB_P2E12_SCIENTIFIC_OUTCOME=NEGATIVE_COMMON_DOMAIN_NOT_FOUND'
...
contains
  ...
  subroutine report_metric_summary(label,values)
    ...
    call require(size(values)==nphys,'metric summary contains all 180 cases')
...
end program test_pub_p2e12_material_extension_reference_common_domain
