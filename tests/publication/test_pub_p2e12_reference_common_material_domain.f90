program test_pub_p2e12_reference_common_material_domain
  ...
  integer, parameter :: nmat=30, nse=5, nforcing=2, ndt=1, nphys=nmat*nse*nforcing
  ...
  character(len=3), parameter :: material_ids(nmat) = [character(len=3) :: 'B02','B03','B04','B05','B06','B07','B08','B09','B10','B11','B13','B14','B15','B16','B17','B18','O02','O03','O04','O06','O07','O08','O09','O10','O11','O12','O13','O15','O16','O17']
  character(len=7), parameter :: forcing_ids(nforcing) = [character(len=7) :: 'DRYING ','NOMINAL']
  real(real64), parameter :: se_levels(nse) = [0.90_real64,0.92_real64,0.94_real64,0.96_real64,0.98_real64]
  ...
  integer :: valid_by_dt(ndt), invalid_by_dt(ndt), valid_by_se(nse), invalid_by_se(nse)
  integer :: idt,imat,ise,iforce,ip,selected_se_index
  logical :: valid
  ...
  valid_by_dt=0; invalid_by_dt=0; valid_by_se=0; invalid_by_se=0
  ...
  do idt=1,ndt
    ...
    call require(ip==nphys,'exact 300 domain-map physical cases at the frozen temporal level')
  end do
  ...
  selected_se_index=0
  do ise=1,nse
    write(*,'(*(g0))') 'PUB_P2E12_SE_FEASIBILITY|INDEX=',ise,'|SE=',se_levels(ise), &
         '|VALID=',valid_by_se(ise),'|INVALID=',invalid_by_se(ise)
    if (valid_by_se(ise)==nmat*nforcing) selected_se_index=ise
  end do

  if (selected_se_index>0) then
    write(*,'(A,I0)') 'PUB_P2E12_HIGHEST_GLOBAL_COMMON_SE_INDEX=',selected_se_index
    write(*,'(*(g0))') 'PUB_P2E12_HIGHEST_GLOBAL_COMMON_SE=',se_levels(selected_se_index)
  else
    write(*,'(A)') 'PUB_P2E12_HIGHEST_GLOBAL_COMMON_SE=NONE_IN_PREREGISTERED_SCAN'
  end if

  write(*,'(A)') 'PUB_P2E12_DOMAIN_MAP_STATUS=COMPLETE'
  write(*,'(A)') 'PUB_P2E12_REFERENCE_COMMON_MATERIAL_DOMAIN_GATE=PASS'
...
contains
  ...
  subroutine report_metric_summary(label,values)
    ...
    call require(size(values)==nphys,'metric summary contains all 300 domain-map cases')
...
end program test_pub_p2e12_reference_common_material_domain
