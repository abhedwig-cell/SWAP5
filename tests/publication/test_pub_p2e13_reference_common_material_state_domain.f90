program test_pub_p2e13_reference_common_material_state_domain
  ...
  integer, parameter :: nmat=36, nse=13, nforcing=2, nphys=nmat*nse*nforcing
  ...
  real(real64), parameter :: se_levels(nse) = [0.50_real64,0.55_real64,0.60_real64,0.65_real64,0.70_real64, &
       0.75_real64,0.80_real64,0.85_real64,0.90_real64,0.92_real64,0.94_real64,0.96_real64,0.98_real64]
  ...
  integer :: imat,ise,iforce,ip,common_count,dry_idx,mid_idx,wet_idx
  ...
  call require(ip==nphys,'exact 936 Reference-only physical cases executed')
  write(*,'(A,I0)') 'PUB_P2E13_PHYSICAL_CASE_COUNT=',nphys
  write(*,'(A,I0)') 'PUB_P2E13_MATERIAL_COUNT=',nmat
  write(*,'(A,I0)') 'PUB_P2E13_SE_LEVEL_COUNT=',nse
  write(*,'(A,I0)') 'PUB_P2E13_FORCING_COUNT=',nforcing
  write(*,'(A)') 'PUB_P2E13_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E13_THRESHOLD_FROZEN=FALSE'

  do ise=1,nse
    write(*,'(*(g0))') 'PUB_P2E13_SE_DOMAIN|SE=',se_levels(ise),'|VALID=',valid_by_se(ise), &
         '|INVALID=',invalid_by_se(ise),'|COMPLETE=',valid_by_se(ise)==nmat*nforcing
  end do

  common_count=count(valid_by_se==nmat*nforcing)
  write(*,'(A,I0)') 'PUB_P2E13_COMMON_SE_COUNT=',common_count

  if (common_count < 3) then
    write(*,'(A)') 'PUB_P2E13_SCIENTIFIC_OUTCOME=BLOCKED_FEWER_THAN_THREE_COMMON_STATE_LEVELS'
    write(*,'(A)') 'PUB_P2E13_REFERENCE_COMMON_MATERIAL_STATE_DOMAIN_GATE=PASS'
    stop
  end if

  dry_idx=nearest_common_index(0.65_real64,0)
  mid_idx=nearest_common_index(0.85_real64,dry_idx)
  wet_idx=highest_common_index(dry_idx,mid_idx)
  call require(dry_idx>0 .and. mid_idx>0 .and. wet_idx>0,'three distinct selected common anchors')

  do ise=1,nse
    if (valid_by_se(ise)==nmat*nforcing) then
      write(*,'(*(g0))') 'PUB_P2E13_COMMON_SE|SE=',se_levels(ise)
    end if
  end do
  write(*,'(*(g0))') 'PUB_P2E13_SELECTED_DRY_SE=',se_levels(dry_idx)
  write(*,'(*(g0))') 'PUB_P2E13_SELECTED_INTERMEDIATE_SE=',se_levels(mid_idx)
  write(*,'(*(g0))') 'PUB_P2E13_SELECTED_WET_SE=',se_levels(wet_idx)
  write(*,'(A)') 'PUB_P2E13_SCIENTIFIC_OUTCOME=QUALIFIED_REFERENCE_COMMON_MATERIAL_STATE_DOMAIN'
  write(*,'(A)') 'PUB_P2E13_REFERENCE_COMMON_MATERIAL_STATE_DOMAIN_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,dt_index,material_id,se,forcing_id,top_factor,bottom_factor,coarse_dt,valid, &
    ...
    call report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,failure_stage)
    write(*,'(*(g0))') 'PUB_P2E13_TYPED_MASS|DT_INDEX=',dt_index,'|CASE=',case_id, &
  ...
  subroutine report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,stage)
    ...
    write(*,'(*(g0))') 'PUB_P2E13_TRIAL|DT_INDEX=',dt_index,'|CASE=',case_id,'|M=',trim(material_id), &
  ...
  subroutine report_metric_summary(label,values)
    ...
    write(*,'(*(g0))') 'PUB_P2E13_METRIC_SUMMARY|METRIC=',trim(label),'|MIN=',minval(values), &
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E13_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_pub_p2e13_reference_common_material_state_domain
