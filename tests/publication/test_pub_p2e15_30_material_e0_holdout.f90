program test_pub_p2e15_30_material_e0_holdout
  ...
  integer, parameter :: nmat=30, nse=3, nforcing=2, expected_cases=nmat*nse*nforcing
  ...
  real(real64), parameter :: th_storage(nse) = [ &
       2.1316282072803006e-14_real64, 2.842170943040401e-14_real64, 2.842170943040401e-14_real64 ]
  ...
  do ise=1,nse
    write(*,'(*(g0))') 'PUB_P2E15_THRESHOLD|SE=',se_levels(ise), &
  ...
  call require(case_id==expected_cases,'exact 180-case extension holdout attempted')
  ...
  write(*,'(A,I0)') 'PUB_P2E15_CASE_COUNT=',case_id
  write(*,'(A,I0)') 'PUB_P2E15_ADMISSIBLE_COUNT=',count_admissible
  write(*,'(A,I0)') 'PUB_P2E15_DISCREPANCY_FAIL_COUNT=',count_discrepancy_fail
  write(*,'(A,I0)') 'PUB_P2E15_REFERENCE_INVALID_COUNT=',count_reference_invalid
  write(*,'(A,I0)') 'PUB_P2E15_ROSSFAST_INVALID_COUNT=',count_rossfast_invalid
  write(*,'(A,I0)') 'PUB_P2E15_BOTH_INVALID_COUNT=',count_both_invalid
  write(*,'(A,I0)') 'PUB_P2E15_FAIL_D_H_INF_COUNT=',fail_h_inf
  write(*,'(A,I0)') 'PUB_P2E15_FAIL_D_H_RMS_COUNT=',fail_h_rms
  write(*,'(A,I0)') 'PUB_P2E15_FAIL_D_THETA_INF_COUNT=',fail_theta_inf
  write(*,'(A,I0)') 'PUB_P2E15_FAIL_D_THETA_RMS_COUNT=',fail_theta_rms
  write(*,'(A,I0)') 'PUB_P2E15_FAIL_D_STORAGE_COUNT=',fail_storage
  write(*,'(A)') 'PUB_P2E15_LAYER=A_FIXED_INTERVAL_SOLVER_SEAM'
  write(*,'(A)') 'PUB_P2E15_STEP_DURATION_DAY=0.0016'
  write(*,'(A)') 'PUB_P2E15_THRESHOLDS_RETUNED=FALSE'
  write(*,'(A)') 'PUB_P2E15_TRANSACTION_LEVEL_PAIR_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E15_PERFORMANCE_CLAIM_EVALUATED=FALSE'
  write(*,'(A)') 'PUB_P2E15_HOLDOUT_EXECUTION_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,ise,material_id,se,forcing_id,top_factor,bottom_factor,classification,metrics_available, &
    ...
    write(*,'(*(g0))') 'PUB_P2E15_CASE|CASE=',case_id,'|M=',trim(material_id),'|SE=',se, &
    ...
    if (reference_result%integrated_mass_balance_residual_available) then
      write(*,'(*(g0))') 'PUB_P2E15_REFERENCE_MASS|CASE=',case_id,'|CM=',reference_result%integrated_mass_balance_residual_cm, &
      ...
      write(*,'(*(g0))') 'PUB_P2E15_REFERENCE_MASS|CASE=',case_id,'|AVAILABLE=FALSE'
    end if

    if (initialized .and. alternative_result%integrated_mass_balance_residual_available) then
      write(*,'(*(g0))') 'PUB_P2E15_ROSSFAST_MASS|CASE=',case_id,'|CM=',alternative_result%integrated_mass_balance_residual_cm, &
           '|RATE_AVAILABLE=',alternative_result%native_balance_rate_residual_available
    else
      write(*,'(*(g0))') 'PUB_P2E15_ROSSFAST_MASS|CASE=',case_id,'|AVAILABLE=FALSE'
    end if
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E15_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_pub_p2e15_30_material_e0_holdout
