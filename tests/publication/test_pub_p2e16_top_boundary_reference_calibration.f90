program test_pub_p2e16_top_boundary_reference_calibration
  ...
  integer, parameter :: nmat=36, nse=3, nforcing=3, nphys=nmat*nse*nforcing
  ...
  call require(ip==nphys,'exact 324 Reference-only physical cases executed')
  write(*,'(A,I0)') 'PUB_P2E16_PHYSICAL_CASE_COUNT=',nphys
  write(*,'(A,I0)') 'PUB_P2E16_MATERIAL_COUNT=',nmat
  write(*,'(A,I0)') 'PUB_P2E16_SE_LEVEL_COUNT=',nse
  write(*,'(A,I0)') 'PUB_P2E16_FORCING_COUNT=',nforcing
  write(*,'(A)') 'PUB_P2E16_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E16_BOUNDARY_PROBE_RULE_PREREGISTERED=TRUE'

  do ise=1,nse
    write(*,'(*(g0))') 'PUB_P2E16_SE_REFERENCE_DOMAIN|SE=',se_levels(ise),'|VALID=',valid_by_se(ise), &
         '|INVALID=',invalid_by_se(ise),'|COMPLETE=',valid_by_se(ise)==nmat*nforcing
  end do

  if (any(valid_by_se/=nmat*nforcing)) then
    write(*,'(A)') 'PUB_P2E16_THRESHOLD_SURFACE_FROZEN=FALSE'
    write(*,'(A)') 'PUB_P2E16_SCIENTIFIC_OUTCOME=BLOCKED_TOP_BOUNDARY_REFERENCE_DOMAIN'
    write(*,'(A)') 'PUB_P2E16_TOP_BOUNDARY_REFERENCE_CALIBRATION_GATE=PASS'
    stop
  end if
  ...
  write(*,'(A)') 'PUB_P2E16_THRESHOLD_SURFACE_FROZEN=TRUE'
  write(*,'(A)') 'PUB_P2E16_SCIENTIFIC_OUTCOME=QUALIFIED_TOP_BOUNDARY_REFERENCE_SURFACE'
  write(*,'(A)') 'PUB_P2E16_REFERENCE_THRESHOLD_FREEZE_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,dt_index,material_id,se,forcing_id,top_factor,bottom_factor,coarse_dt,valid, &
    ...
    call report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,failure_stage)
    write(*,'(*(g0))') 'PUB_P2E16_TYPED_MASS|DT_INDEX=',dt_index,'|CASE=',case_id, &
  ...
  subroutine report_trial(case_id,dt_index,material_id,se,forcing_id,coarse_dt,valid,stage)
    ...
    write(*,'(*(g0))') 'PUB_P2E16_TRIAL|DT_INDEX=',dt_index,'|CASE=',case_id,'|M=',trim(material_id), &
  ...
  subroutine report_metric_summary(label,values)
    ...
    write(*,'(*(g0))') 'PUB_P2E16_METRIC_SUMMARY|METRIC=',trim(label),'|MIN=',minval(values), &
  ...
  subroutine report_threshold_by_se(label,values)
    ...
    call require(size(values)==nphys,'threshold metric contains all 324 cases')
    do ise=1,nse
      ...
      write(*,'(*(g0))') 'PUB_P2E16_THRESHOLD|SE=',se_levels(ise),'|METRIC=',trim(label),'|VALUE=',vmax
    end do
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E16_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_pub_p2e16_top_boundary_reference_calibration
