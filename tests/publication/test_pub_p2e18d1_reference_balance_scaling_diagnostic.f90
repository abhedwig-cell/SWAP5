program test_pub_p2e18d1_reference_balance_scaling_diagnostic
  ...
  do il=1,nlevels
    write(*,'(*(g0))') 'PUB_P2E18D1_POLICY|NSUB=',nsub_levels(il), &
      ...
      do iforce=1,nforcing
        ...
        do il=1,nlevels
          call run_level(case_id,nsub_levels(il),material_ids(imat),se_levels(ise),forcing_ids(iforce), &
               qtop_factor(iforce),qbot_factor(iforce),valid,stage)
          if (valid) then
  ...
  call require(case_id==ncases,'exact 36 diagnostic cases attempted')
  do il=1,nlevels
    call require(valid_by_level(il)+invalid_by_level(il)==ncases,'every level classified')
    write(*,'(*(g0))') 'PUB_P2E18D1_LEVEL_SUMMARY|NSUB=',nsub_levels(il), &
         '|VALID=',valid_by_level(il),'|INVALID=',invalid_by_level(il)
  end do

  write(*,'(A,I0)') 'PUB_P2E18D1_CASE_COUNT=',ncases
  write(*,'(A,I0)') 'PUB_P2E18D1_LEVEL_RECORD_COUNT=',ncases*nlevels
  write(*,'(A)') 'PUB_P2E18D1_POLICY=INVARIANT_INTEGRATED_BALANCE_SCALE'
  write(*,'(A)') 'PUB_P2E18D1_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E18D1_TIMING_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E18D1_REF_HIGH_STABILITY_QUALIFIED=FALSE'
  if (valid_by_level(4)==ncases .and. valid_by_level(5)==ncases) then
    write(*,'(A)') 'PUB_P2E18D1_DIAGNOSTIC_OUTCOME=SCALING_HYPOTHESIS_SUPPORTED_FOR_REQUIRED_FINE_LEVELS'
  else
    write(*,'(A)') 'PUB_P2E18D1_DIAGNOSTIC_OUTCOME=SCALING_HYPOTHESIS_NOT_SUFFICIENT_FOR_REQUIRED_FINE_LEVELS'
  end if
  write(*,'(A)') 'PUB_P2E18D1_GATE=PASS'

contains

  subroutine run_level(case_id,nsub,material_id,se,forcing_id,top_factor,bottom_factor,valid,stage)
    ...
    valid=.false.; stage='INITIALIZATION'
    ...
    do isub=1,nsub
      ...
      write(*,'(*(g0))') 'PUB_P2E18D1_STEP|CASE=',case_id,'|M=',trim(material_id),'|SE=',se, &
           ...
           '|MASS_CM=',merge(result%integrated_mass_balance_residual_cm,0.0_real64, &
                             result%integrated_mass_balance_residual_available), &
           '|VALID=',step_valid
  ...
  subroutine report_level(case_id,nsub,material_id,se,forcing_id,valid,stage,completed_substeps,rate_tol)
    ...
    write(*,'(*(g0))') 'PUB_P2E18D1_LEVEL|CASE=',case_id,'|M=',trim(material_id),'|SE=',se, &
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E18D1_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_pub_p2e18d1_reference_balance_scaling_diagnostic
