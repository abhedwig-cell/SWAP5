program test_pub_p2e18d2_reference_retry_mechanism
  ...
  do il=1,nlevels
    write(*,'(*(g0))') 'PUB_P2E18D2_POLICY|NSUB=',nsub_levels(il), &
  ...
  do il=1,nlevels
    ...
    write(*,'(*(g0))') 'PUB_P2E18D2_LEVEL_SUMMARY|NSUB=',nsub_levels(il), &
         '|VALID=',valid_by_level(il),'|INVALID=',invalid_by_level(il)
  end do

  write(*,'(A,I0)') 'PUB_P2E18D2_CASE_COUNT=',ncases
  write(*,'(A,I0)') 'PUB_P2E18D2_LEVEL_RECORD_COUNT=',ncases*nlevels
  write(*,'(A,I0)') 'PUB_P2E18D2_RETRY_BALANCE_ONLY_COUNT=',retry_balance_only
  write(*,'(A,I0)') 'PUB_P2E18D2_RETRY_HEAD_ONLY_COUNT=',retry_head_only
  write(*,'(A,I0)') 'PUB_P2E18D2_RETRY_BOTH_COUNT=',retry_both
  write(*,'(A,I0)') 'PUB_P2E18D2_RETRY_OTHER_COUNT=',retry_other
  write(*,'(A)') 'PUB_P2E18D2_POLICY=INVARIANT_INTEGRATED_BALANCE_SCALE_UNCHANGED_FROM_D1'
  write(*,'(A)') 'PUB_P2E18D2_ROSSFAST_SOLVER_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E18D2_TIMING_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E18D2_REF_HIGH_STABILITY_QUALIFIED=FALSE'
  write(*,'(A)') 'PUB_P2E18D2_DIAGNOSTIC_OUTCOME=RETRY_MECHANISM_CLASSIFIED'
  write(*,'(A)') 'PUB_P2E18D2_GATE=PASS'
...
contains
  ...
  subroutine run_level(case_id,nsub,material_id,se,forcing_id,top_factor,bottom_factor,valid,stage,retry_class)
    ...
    do isub=1,nsub
      ...
      write(*,'(*(g0))') 'PUB_P2E18D2_STEP|CASE=',case_id,'|M=',trim(material_id),'|SE=',se, &
  ...
  subroutine report_level(case_id,nsub,material_id,se,forcing_id,valid,stage,completed_substeps,rate_tol)
    ...
    write(*,'(*(g0))') 'PUB_P2E18D2_LEVEL|CASE=',case_id,'|M=',trim(material_id),'|SE=',se, &
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E18D2_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_pub_p2e18d2_reference_retry_mechanism
