program test_pub_p2e21a_equal_error_control_selection
  ...
  integer, parameter :: n_all_cases=nmat*nse*nforcing,n_perf_cases=33
  ...
  do imat=1,nmat
    do ise=1,nse
      do iforce=1,nforcing
        case_id=case_id+1
        if (.not.performance_case(case_id)) then
          write(*,'(*(g0))') 'PUB_P2E21A_EXCLUDED_CASE|P2E20_CASE=',case_id,'|M=',trim(material_ids(imat)), &
  ...
  call require(perf_case_id==n_perf_cases,'exact 33 REF-HIGH-qualified cases retained')
  call require(ref_high_count==n_perf_cases,'REF-HIGH reconstructed for all 33 cases')
  call require(config_count==n_perf_cases*nsolver*nlevels,'exact 396 production configurations attempted')

  write(*,'(A,I0)') 'PUB_P2E21A_PERFORMANCE_CASE_COUNT=',perf_case_id
  write(*,'(A,I0)') 'PUB_P2E21A_PRODUCTION_CONFIG_COUNT=',config_count
  write(*,'(A,I0)') 'PUB_P2E21A_REF_HIGH_RECONSTRUCTION_COUNT=',ref_high_count
  do imat=1,nsolver
    write(*,'(*(g0))') 'PUB_P2E21A_SOLVER_SUMMARY|SOLVER=',trim(solver_label(imat)), &
         '|VALID_CONFIGS=',production_valid_count(imat),'|INVALID_CONFIGS=',production_invalid_count(imat)
    do iforce=1,ntarget
      write(*,'(*(g0))') 'PUB_P2E21A_TARGET_SUMMARY|SOLVER=',trim(solver_label(imat)), &
           '|TARGET=',trim(target_label(iforce)),'|SELECTED=',solver_selected_count(imat,iforce), &
           '|UNRESOLVED=',solver_unresolved_count(imat,iforce)
    end do
  end do
  do iforce=1,ntarget
    write(*,'(*(g0))') 'PUB_P2E21A_PAIRED_SUMMARY|TARGET=',trim(target_label(iforce)), &
         '|PAIRED=',paired_count(iforce),'|UNPAIRED=',n_perf_cases-paired_count(iforce)
  end do
  write(*,'(A)') 'PUB_P2E21A_TIMING_EXECUTED=FALSE'
  write(*,'(A)') 'PUB_P2E21A_STORAGE_TARGET_SCALED=FALSE'
  write(*,'(A)') 'PUB_P2E21A_ACCURACY_TARGETS_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E21A_CASE_POPULATION_CHANGED=FALSE'
  write(*,'(A)') 'PUB_P2E21A_SCIENTIFIC_RESULT_IS_CI_FAILURE=FALSE'
  write(*,'(A)') 'PUB_P2E21A_EQUAL_ERROR_CONTROL_SELECTION_GATE=PASS'

contains

  logical function performance_case(id) result(ok)
    integer,intent(in) :: id
    ok=(id>=3 .and. id<=35)
  end function performance_case
  ...
  subroutine run_performance_case(perf_id,p2e20_id,ise,material_id,se,forcing_id,top_factor,bottom_factor)
    ...
    write(*,'(*(g0))') 'PUB_P2E21A_REF_HIGH|PERF_CASE=',perf_id,'|P2E20_CASE=',p2e20_id, &
    ...
    do isolver=1,nsolver
      do ilevel=1,nlevels
        ...
        write(*,'(*(g0))') 'PUB_P2E21A_CONFIG|PERF_CASE=',perf_id,'|P2E20_CASE=',p2e20_id, &
    ...
    do itarget=1,ntarget
      do isolver=1,nsolver
        if (selected_nseg(isolver,itarget)>0) then
          ...
          write(*,'(*(g0))') 'PUB_P2E21A_SELECTION|PERF_CASE=',perf_id,'|P2E20_CASE=',p2e20_id, &
  ...
  subroutine run_ref_high(parameters,hydraulic_parameters,constitutive,source_sink,top_boundary,material, &
    ...
    real(real64) :: dt,rate_tol,current_h(n),current_theta(n),ponding
    integer :: i

    dt=horizon_day/32.0_real64
    rate_tol=ref_high_integrated_allowance_cm/dt
  ...
  subroutine run_production_configuration(isolver,nseg,parameters,hydraulic_parameters,constitutive,source_sink, &
    ...
    do i=1,nseg
      call bind_b110_default_mvg_provider(constitutive,hydraulic_parameters,dt)
      call initialize_request(request,parameters,constitutive,source_sink,top_boundary,current_theta,current_h,ponding, &
           top_flux,bottom_flux,dt,prod_balance_rate_tol)
      if (isolver==1) then
  ...
  subroutine endpoint_error(ise,parameters,h,theta,storage,ref_h,ref_theta,ref_storage, &
    ...
    if (parameters%active_nodes/=n) error stop 'P2E21A parameter cardinality'
  ...
  subroutine initialize_request(req,parameter_set,hydraulic_provider,source_provider,top_provider,theta,heads,ponding, &
    ...
    real(real64),intent(in) :: theta(n),heads(n),ponding,qtop,qbot,dt,balance_rate_tol
    ...
    req%numerical%compartment_balance_tolerance=balance_rate_tol
    req%numerical%total_balance_tolerance=balance_rate_tol
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'PUB_P2E21A_HARNESS_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_pub_p2e21a_equal_error_control_selection
