...
BASE=1ef995e682cbc0fbbce8b68281646b0de1bbbcd3
COMPOSITION=49bb76f21e8f6a5ab6cdb9c241bf437a12520446
COMPOSITION_TREE=e318b46129fb6676ae5f05beafa986de40b40cf0
OWNER_QUALIFIED=43c9f09110dea83928ef7d58990c37575d8ca223
OWNER_REVIEW=251adbb2aeece53cf9b745e488a44b49d67aa3eb
...
# Exact-base composition contract.
git cat-file -e "$BASE^{commit}"
git cat-file -e "$COMPOSITION^{commit}"
test "$(git rev-parse "$COMPOSITION^")" = "$BASE"
test "$(git rev-parse "$COMPOSITION^{tree}")" = "$COMPOSITION_TREE"
git merge-base --is-ancestor "$COMPOSITION" HEAD
echo 'FCI73_EXACT_BASE_COMPOSITION=PASS'

allowed=(
  ".github/workflows/fgc25-g05-multiswap-groundwater-composition.yml"
  ".github/workflows/fvq87-fgc25-multiswap-independent.yml"
  ".github/workflows/fci73-fgc25-current-canonical-admission.yml"
  "integration/f-gc/F-GC25_G05_CAPABILITY_QUALIFICATION.json"
  ...
  "tests/qualification/fci73/run_fci73_fgc25_current_canonical_admission_gate.sh"
)
...
for path in "${changed[@]}"; do
  ...
  if [[ "$ok" -ne 1 ]]; then
    echo "FCI73_SCOPE_FAIL unexpected path: $path" >&2
    exit 20
  fi
done
echo 'FCI73_SCOPE_ALLOWLIST=PASS'

# Production and owner tests must be the exact independently qualified source postimage.
owner_paths=(
  src/runtime/mod_groundwater_accuracy_binding.f90
  src/runtime/mod_groundwater_multiswap_coupler.f90
  src/runtime/mod_groundwater_multiswap_publication.f90
...
for path in "${owner_paths[@]}"; do
  ...
  test "$(git rev-parse "$OWNER_REVIEW:$path")" = "$(git rev-parse "$OWNER_QUALIFIED:$path")"
done
test "$(git rev-parse HEAD:.github/workflows/fgc25-g05-multiswap-groundwater-composition.yml)" = "$(git rev-parse "$OWNER_REVIEW:.github/workflows/fgc25-g05-multiswap-groundwater-composition.yml")"
test "$(git rev-parse HEAD:integration/f-gc/F-GC25_G05_CAPABILITY_QUALIFICATION.json)" = "$(git rev-parse "$OWNER_REVIEW:integration/f-gc/F-GC25_G05_CAPABILITY_QUALIFICATION.json")"
echo 'FCI73_OWNER_POSTIMAGE_EXACT=PASS'

# Independent evidence is inherited exactly from the successful F-VQ87 closeout.
fvq_paths=(
  .github/workflows/fvq87-fgc25-multiswap-independent.yml
  qualification/F-VQ87_STATUS.json
  tests/qualification/fvq87/run_fvq87_fgc25_multiswap_independent_gate.sh
...
echo 'FCI73_FVQ87_EVIDENCE_EXACT=PASS'

# Lock the inherited GC20/GC22 executable dependency boundary.
...
if grep -Eq 'use[[:space:]]+mod_reference_richards_temporal_indicator|use[[:space:]]+mod_groundwater_accuracy_binding' src/runtime/mod_groundwater_multiswap_*.f90; then
  echo 'FCI73_GC22_DEPENDENCY_BOUNDARY=FAIL' >&2
  exit 21
fi
echo 'FCI73_FGC20_FGC22_INHERITANCE=PASS'

# Re-run the owner gate on the composed current-canonical postimage.
owner_log="$(mktemp)"
work="$(mktemp -d)"
cleanup() {
  rm -f "$owner_log"
...
echo 'FCI73_OWNER_GATE_CURRENT_CANONICAL=PASS'

# Preserve the independent publication-boundary audit against the composed image.
python3 - <<'PY'
...
PY
echo 'FCI73_PUBLICATION_BOUNDARY_AUDIT=PASS'

# Re-run the independent adversarial oracle directly on the admission postimage.
...
for marker in \
  FVQ87_PREPARE_REJECTION_ATOMICITY \
  FVQ87_DUPLICATE_TILE_PRETRIAL_REJECTION \
  FVQ87_STALE_ORIGIN_PRETRIAL_REJECTION \
  FVQ87_NONCONVERGED_CELL_ROLLBACK \
  FVQ87_WEIGHTED_MASS_ACTION_REACTION \
  FVQ87_INDEPENDENT_ORACLE; do
...
cat "$work/o0.txt"
echo "FCI73_INDEPENDENT_ORACLE_SHA256=$oracle_sha"
echo 'FCI73_INDEPENDENT_ORACLE_O0_O2_IDENTITY=PASS'
echo 'F_CI73_FINAL_GATE=PASS'
