...
def main()->int:
    ...
    recon=[x for x in lines if x.startswith("RZM06E06_RECON|")]
    forcing=[x for x in lines if x.startswith("RZM06E06_FORCING|")]
    probe=[x for x in lines if x.startswith("RZM06E06_PROBE|")]
    ...
    result={
      "schema":"swap5.gc_rootzone_memory.rzm06e06.rootmatched_interface_memory_probe.v1",
      "preregistration_commit":"c25258bd0df363c29be693cf5c01b512f4630a27",
      ...
      "order_independence":any(x=="GC_RZM06E06_ORDER_INDEPENDENCE=PASS" for x in lines),
    ...
    if admitted_a and admitted_b:
        ...
        result["decision"]=(
          "SUPPORTED_INTERFACE_MEMORY_AT_1E6_STORAGE_RESOLUTION"
          if support else "NO_SUPPORT_AT_1E6_SELECTED_PAIR"
        )
    else:
        result["probe"].update({
    ...
    result["interpretation_boundary"]=[
      "Support means H_c plus W_profile and W_root30 do not determine a unique next interface exchange at the frozen 1e-6 cm aggregate-storage resolution for this selected C01 pair/window.",
    ...
    print("RZM06E06_RESULT_JSON",json.dumps(result,sort_keys=True,separators=(",",":")))
    print("GC_RZM06E06_ANALYSIS=PASS")
    return 0
