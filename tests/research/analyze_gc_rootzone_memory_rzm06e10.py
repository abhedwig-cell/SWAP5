...
def main()->int:
    ...
    recon=[x for x in lines if x.startswith("RZM06E10_RECON|")]
    forcing=[x for x in lines if x.startswith("RZM06E10_FORCING|")]
    probe=[x for x in lines if x.startswith("RZM06E10_PROBE|")]
    ...
    result={
      "schema":"swap5.gc_rootzone_memory.rzm06e10.rootmatched_interface_memory_probe.v1",
      "preregistration_commit":"2bc1b16f7e4d51e9cbcaa1a6cf16105a57403b9a",
      ...
      "order_independence":any(x=="GC_RZM06E10_ORDER_INDEPENDENCE=PASS" for x in lines),
    ...
    if admitted_a and admitted_b:
        ...
        result["decision"]=(
          "SUPPORTED_INTERFACE_MEMORY_WITH_EXACT_AGGREGATE_MATCH"
          if support else "NO_SUPPORT_AT_EXACT_AGGREGATE_PAIR"
        )
    else:
        result["probe"].update({
    ...
    print("RZM06E10_RESULT_JSON",json.dumps(result,sort_keys=True,separators=(",",":")))
    print("GC_RZM06E10_ANALYSIS=PASS")
    return 0
