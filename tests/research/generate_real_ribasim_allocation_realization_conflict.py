...
def build_model(*, root_priority: int, external_priority: int) -> Model:
    ...
    source = model.flow_boundary.add(
        Node(1, Point(-1.0, 0.0), subnetwork_id=2, name="fixed_60_m3_day"),
        [flow_boundary.Static(flow_rate=[SUPPLY])],
    )
    store = model.basin.add(
        Node(2, Point(0.0, 0.0), subnetwork_id=2, name="shared_surface_store"),
        [
    ...
    hold = model.level_demand.add(
        Node(7, Point(0.0, 1.0), subnetwork_id=2, name="hold_surface_level"),
        [
            level_demand.Static(
                min_level=[1.0],
                max_level=[1.0],
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit(
            "usage: generate_real_ribasim_allocation_realization_conflict.py <output-root>"
        )

    output_root = Path(sys.argv[1]).resolve()
    output_root.mkdir(parents=True, exist_ok=True)

    # demand_priority is ascending in pinned Ribasim; priority 1 belongs to
    # the LevelDemand forecast hold.
    write_case(output_root, "swap5_reduction_root_first", 2, 3)
    write_case(output_root, "swap5_reduction_external_first", 3, 2)
