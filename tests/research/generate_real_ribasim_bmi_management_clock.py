...
def build_model(*, root_priority: int, external_priority: int) -> Model:
    ...
    store = model.basin.add(
        ...
        [basin.Profile(level=[0.0, 2.0], area=AREA), basin.State(level=[1.0])],
    )
    root = model.user_demand.add(
        Node(3, Point(1.0, 0.5), subnetwork_id=2, name="root_proxy"),
        [user_demand.Static(demand=[ROOT_DEMAND], return_factor=0.0, min_level=MIN_LEVEL, demand_priority=root_priority)],
    )
    external = model.user_demand.add(
        Node(4, Point(1.0, -0.5), subnetwork_id=2, name="external_demand"),
        [user_demand.Static(demand=[EXTERNAL_DEMAND], return_factor=0.0, min_level=MIN_LEVEL, demand_priority=external_priority)],
    )
    root_sink = model.terminal.add(Node(5, Point(2.0, 0.5), subnetwork_id=2, name="root_sink"))
    external_sink = model.terminal.add(Node(6, Point(2.0, -0.5), subnetwork_id=2, name="external_sink"))
...
def write_case(output_root: Path, name: str, root_priority: int, external_priority: int) -> None:
    ...
    build_model(root_priority=root_priority, external_priority=external_priority).write(path / "ribasim.toml")
    print(f"Generated {name}")


def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: generate_real_ribasim_bmi_management_clock.py <output-root>")
    output_root = Path(sys.argv[1]).resolve()
    output_root.mkdir(parents=True, exist_ok=True)
    write_case(output_root, "swap5_bmi_clock_root_first", 2, 3)
    write_case(output_root, "swap5_bmi_clock_external_first", 3, 2)
