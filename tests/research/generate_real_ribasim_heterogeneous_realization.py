...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: generate_real_ribasim_heterogeneous_realization.py <output-root>")
    output_root = Path(sys.argv[1]).resolve()
    output_root.mkdir(parents=True, exist_ok=True)

    write_case(output_root, "swap5_heterogeneous_root_first", 2, 3)
    write_case(output_root, "swap5_heterogeneous_external_first", 3, 2)
