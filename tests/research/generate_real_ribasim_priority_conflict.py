...
def build_model(*, root_priority: int, external_priority: int) -> Model:
    ...
    store = model.basin.add(
        ...
        [
            basin.Profile(level=[0.0, 2.0], area=1000.0),
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: generate_real_ribasim_priority_conflict.py <output-root>")
    output_root = Path(sys.argv[1]).resolve()
    output_root.mkdir(parents=True, exist_ok=True)

    # demand_priority is ascending in pinned Ribasim; 1 is reserved for the
    # storage-hold LevelDemand.
    write_case(output_root, "swap5_priority_root_first", 2, 3)
    write_case(output_root, "swap5_priority_external_first", 3, 2)
