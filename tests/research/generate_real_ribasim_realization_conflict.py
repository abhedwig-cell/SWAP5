...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: generate_real_ribasim_realization_conflict.py <output-root>")
    output_root = Path(sys.argv[1]).resolve()
    output_root.mkdir(parents=True, exist_ok=True)

    # Priority 1 is reserved for the allocation-stage level-hold objective.
    write_case(output_root, "swap5_realization_root_first", 2, 3)
    write_case(output_root, "swap5_realization_external_first", 3, 2)
