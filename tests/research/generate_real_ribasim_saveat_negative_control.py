...
AREA = 1_000_000.0
MIN_LEVEL = 0.99
SAVEATS = (3600, 21600, 43200, 86400)


def build_model(*, saveat_seconds: int) -> Model:
    model = Model(
        starttime=datetime(2020, 1, 1),
        endtime=datetime(2020, 1, 2),
        crs="EPSG:28992",
        allocation=Allocation(dt=DAY),
        solver=Solver(saveat=float(saveat_seconds)),
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: generate_real_ribasim_saveat_negative_control.py <output-root>")

    output_root = Path(sys.argv[1]).resolve()
    output_root.mkdir(parents=True, exist_ok=True)

    for saveat in SAVEATS:
        name = f"swap5_saveat_negative_{saveat}"
        path = output_root / name
        if path.exists():
            shutil.rmtree(path)
        path.mkdir(parents=True)
        build_model(saveat_seconds=saveat).write(path / "ribasim.toml")
