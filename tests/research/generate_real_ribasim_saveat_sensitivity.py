...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: generate_real_ribasim_saveat_sensitivity.py <output-root>")
    ...
    for saveat in SAVEATS:
        write_case(
            output_root,
            f"swap5_saveat_{saveat}_root_first",
            saveat_seconds=saveat,
            root_priority=2,
            external_priority=3,
        )
        write_case(
            output_root,
            f"swap5_saveat_{saveat}_external_first",
            saveat_seconds=saveat,
            root_priority=3,
            external_priority=2,
        )
