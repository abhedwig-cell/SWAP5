...
const ROOT_MIN_LEVEL = 0.99
const EXTERNAL_MIN_LEVEL = 0.995
const LEVEL_THRESHOLD = 0.02

const EXPECTED_FINAL_LEVEL = 1.0000367990161128
const EXPECTED_ROOT_SUPPLIED = 20.055236373645673
const EXPECTED_EXTERNAL_SUPPLIED = 3.145747513661483
const EXPECTED_TOTAL_SUPPLIED = 23.200983887307157
const EXPECTED_STORAGE_GAIN = 36.799016112754046
const EXPECTED_FINAL_ROOT_FACTOR = 0.5027599137504485
const EXPECTED_FINAL_EXTERNAL_FACTOR = 0.1583250103267847
...
function check_case(
    ...
    require(
        all(normalized_root .> normalized_ext),
        "$model_name heterogeneous min_level values did not produce distinct ordered physical reduction factors",
    )
    ...
    require(
        ...
        "$model_name total physical supply differs from preregistered continuous reference",
    )
    ...
    final_root_factor = reduction_factor_reference(final_level - ROOT_MIN_LEVEL)
    final_ext_factor = reduction_factor_reference(final_level - EXTERNAL_MIN_LEVEL)
    require(
        isapprox(final_root_factor, EXPECTED_FINAL_ROOT_FACTOR; atol = 5.0e-6, rtol = 0.0),
        "$model_name final root reduction factor differs from preregistration",
    )
    require(
        isapprox(final_ext_factor, EXPECTED_FINAL_EXTERNAL_FACTOR; atol = 5.0e-6, rtol = 0.0),
        "$model_name final external reduction factor differs from preregistration",
    )

    println(
        "RIBASIM_REAL_19D_OBS model=$model_name " *
        "root_supplied_m3=$root_volume external_supplied_m3=$ext_volume " *
        "total_supplied_m3=$total_volume final_level=$final_level " *
        "storage_gain_m3=$storage_gain final_root_factor=$final_root_factor " *
        "final_external_factor=$final_ext_factor",
    )
    println(
        "RIBASIM_REAL_19D_CASE=PASS model=$model_name " *
...
function main()
    length(ARGS) == 1 || error("usage: real_ribasim_heterogeneous_realization_bridge.jl <ribasim-root>")
    root = abspath(ARGS[1])

    root_first = check_case(
        root,
        "swap5_heterogeneous_root_first";
        root_priority = 2,
        external_priority = 3,
    )
    external_first = check_case(
        root,
        "swap5_heterogeneous_external_first";
        root_priority = 3,
        external_priority = 2,
    )
    ...
    println("RIBASIM_REAL_19D_HETEROGENEOUS_REALIZATION=PASS")
end
