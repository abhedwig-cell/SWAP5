...
python3 tests/research/test_gc_dummy_swap_dsw25_interface_phreatic_darcy.py | tee "$BUILD/dsw25.txt"
grep -Fq 'GC_DSW25_ANALYTIC_GATE=PASS' "$BUILD/dsw25.txt" || fail "DSW-25 analytic gate"
...
if test "$HLINK02_STATUS" -eq 0; then
  grep -Fq 'GC_HLINK02_LIVE_GATE=PASS' "$BUILD/hlink02-live.txt" || fail "HLINK02 closed without marker"
  echo 'GC_HLINK02_ORIGINAL_LIVE_GATE=CLOSED'
else
  echo 'GC_HLINK02_ORIGINAL_LIVE_GATE=FAILED_PRESERVED_FOR_HLINK02D_DIAGNOSIS'
fi
echo 'GC_HLINK02D_FIRST_SOLVE_DIAGNOSTIC_EXECUTION=PASS'
