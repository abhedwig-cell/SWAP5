...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-gc-low01a-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/obj"
...
MODULE_SRC=(
  ...
  tests/research/test_gc_hlink_low01a_below_profile.f90
)
...
gfortran -fopenmp -O2 "${objects[@]}" -o "$BUILD/gc_low01a" || fail "link LOW01-A"
"$BUILD/gc_low01a" | tee "$BUILD/low01a.txt"
...
for marker in \
  ...
  grep -Fq "$marker" "$BUILD/low01a.txt" || fail "missing marker $marker"
done

git diff --check -- \
  tests/research/support/mod_gc_hlink_low01_groundwater_level_adapter.f90 \
  tests/research/test_gc_hlink_low01a_below_profile.f90 \
  tests/research/run_gc_hlink_low01a.sh \
  integration/research/GC_HLINK_LOW01A_PREREGISTRATION.json

echo 'GC HLINK LOW01-A RESEARCH GATE PASS'
