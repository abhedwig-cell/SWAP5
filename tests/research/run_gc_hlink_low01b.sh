...
MODULE_SRC=(
  ...
  tests/research/test_gc_hlink_low01b_inside_profile.f90
)
...
for marker in \
  'GC_LOW01B_ACTIVE_DOMAIN_TRUNCATION=PASS' \
  'GC_LOW01B_SATURATED_CONTINUATION=PASS' \
  'GC_LOW01B_QBOT_OUTPUT_AND_MASS=PASS' \
  'GC_LOW01B_REPLAY_DETERMINISM=PASS' \
...
git diff --check -- \
  ...
  integration/research/GC_HLINK_LOW01B_PREREGISTRATION.json
