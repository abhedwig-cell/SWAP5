...
# CI rerun after technical HeadCalc instrumentation-anchor repair.
# CI rerun after binding the observer to the existing cumulative backtracking diagnostic. A temporary HeadCalc
...
# Restore the FGC44 shared library to the ordinary committed HeadCalc,
# serialized backend and standard bridge before G23. G21L-M-N use temporary
# read-only instrumentation, which must not be part of endpoint qualification.
G23_HEADCALC_OBJ="$BUILD/bridge/g23_headcalc.o"
G23_BACKEND_OBJ="$BUILD/bridge/g23_mod_fmr_serialized_reference_backend.o"
G23_BRIDGE_OBJ="$BUILD/bridge/g23_mod_fgc44_real_swap_c_bridge.o"
gfortran "${COMMON[@]}" -O2 -J "$BUILD/bridge" -I "$BUILD/bridge" \
  -c src/legacy/b1_10_port/headcalc.f90 -o "$G23_HEADCALC_OBJ"
gfortran "${COMMON[@]}" -O2 -J "$BUILD/bridge" -I "$BUILD/bridge" \
  -c src/runtime/mod_fmr_serialized_reference_backend.f90 -o "$G23_BACKEND_OBJ"
gfortran "${COMMON[@]}" -O2 -J "$BUILD/bridge" -I "$BUILD/bridge" \
  -c tests/fgc/support/mod_fgc44_real_swap_c_bridge.f90 -o "$G23_BRIDGE_OBJ"

G23_OBJECTS=()
for obj in "${objects[@]}"; do
  case "$(basename "$obj")" in
    .g21m_headcalc.o|.g21l_mod_fmr_serialized_reference_backend.o|.g21l_mod_fgc44_real_swap_c_bridge.o|mod_gc_g21m_residual_observer.o)
      ;;
    *)
      G23_OBJECTS+=("$obj")
      ;;
...
G23_OBJECTS+=("$G23_HEADCALC_OBJ" "$G23_BACKEND_OBJ" "$G23_BRIDGE_OBJ")
gfortran -shared -fopenmp -O2 "${G23_OBJECTS[@]}" -o "$BUILD/bridge/libfgc44_swap.so"
nm -D "$BUILD/bridge/libfgc44_swap.so" | grep -q 'fgc44_swap_initialize_c' || {
  echo "GC_G23_FAIL missing standard FGC44 ABI after source restore" >&2
  exit 1
}
if nm -D "$BUILD/bridge/libfgc44_swap.so" | grep -q 'gc_g21m_snapshot_c'; then
  echo "GC_G23_FAIL G21M research observer leaked into restored G23 library" >&2
  exit 1
fi
echo 'GC_FIXED_INTERFACE_G23_STANDARD_SOURCE_RESTORE=PASS'
