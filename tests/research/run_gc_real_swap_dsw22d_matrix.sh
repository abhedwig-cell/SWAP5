...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-gc-dsw22d-${GITHUB_RUN_ID:-local}-$"
mkdir -p "$BUILD/bridge"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "GC_DSW22D_FAIL $*" >&2; exit 1; }
...
FGC44_SWAP_LIB="$BUILD/bridge/libfgc44_swap.so" \
  python3 tests/research/test_gc_real_swap_dsw22d_matrix.py | tee "$BUILD/dsw22d.txt"

for marker in \
  'GC_DSW22D_ORIGINAL_FALSIFICATION_PRESERVED=PASS' \
  'GC_DSW22D_FGC44_DEFAULT_POSITIVE_CONTROL=PASS' \
  'GC_DSW22D_SUCCESSFUL_CASE_ALGEBRA_AND_MASS=PASS' \
  'GC_DSW22D_COMPLETE_FIXED_MATRIX=PASS' \
  'GC_DSW22D_DIAGNOSTIC_GATE=PASS'; do
  grep -Fq "$marker" "$BUILD/dsw22d.txt" || fail "missing marker $marker"
done

git diff --check -- \
  integration/research/GC_DUMMY_SWAP_DSW22D_DIAGNOSTIC_DESIGN.json \
  tests/research/test_gc_real_swap_dsw22d_matrix.py \
  tests/research/run_gc_real_swap_dsw22d_matrix.sh

echo 'GC DSW22D FULL MATRIX DIAGNOSTIC GATE PASS'
