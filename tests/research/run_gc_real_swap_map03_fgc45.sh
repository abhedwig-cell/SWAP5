...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-gc-map03-${GITHUB_RUN_ID:-local}-$$"
...
LIBMF6="$BUILD/modflow-bin/libmf6.so" FGC45_MULTISWAP_LIB="$BUILD/bridge/libfgc45_multiswap.so" \
  python3 tests/research/test_gc_real_swap_map03_storage_decomposition.py | tee "$BUILD/map03.txt"

for marker in \
  'GC_MAP03_ALL_DIRECT_TRIALS_ACCEPTED_MASS_COMPLETE=PASS' \
  'GC_MAP03_TRANSACTION_MASS_IDENTITY=PASS' \
  'GC_MAP03_STORAGE_DERIVATIVE_EQUALS_U=PASS' \
  'GC_MAP03_BOTTOM_DERIVATIVE_EQUALS_MINUS_U=PASS' \
  'GC_MAP03_STORAGE_PLUS_BOTTOM_DERIVATIVE_CANCELS=PASS' \
  'GC_MAP03_LIVE_GATE=PASS'; do
  grep -Fq "$marker" "$BUILD/map03.txt" || fail "missing marker $marker"
done

git diff --check -- tests/research/test_gc_real_swap_map03_storage_decomposition.py \
  tests/research/run_gc_real_swap_map03_fgc45.sh \
  integration/research/GC_REAL_SWAP_MAP03_PREREGISTRATION.json

echo 'GC REAL SWAP MAP03 FGC45 GATE PASS'
