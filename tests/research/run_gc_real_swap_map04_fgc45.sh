...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-gc-map04-${GITHUB_RUN_ID:-local}-$$"
...
LIBMF6="$BUILD/modflow-bin/libmf6.so" FGC45_MULTISWAP_LIB="$BUILD/bridge/libfgc45_multiswap.so" \
  python3 tests/research/test_gc_real_swap_map04_slope_policy.py | tee "$BUILD/map04.txt"

for marker in \
  'GC_MAP04_ALL_POLICIES_CONVERGED=PASS' \
  'GC_MAP04_ACCEPTED_HEAD_INVARIANCE=PASS' \
  'GC_MAP04_ACCEPTED_CORRECTOR_FLUX_INVARIANCE=PASS' \
  'GC_MAP04_ACCEPTED_LEDGER_INVARIANCE=PASS' \
  'GC_MAP04_EXACTLY_ONCE_PUBLICATION=PASS' \
  'GC_MAP04_LIVE_GATE=PASS'; do
  grep -Fq "$marker" "$BUILD/map04.txt" || fail "missing marker $marker"
done

git diff --check -- tests/research/test_gc_real_swap_map04_slope_policy.py \
  tests/research/run_gc_real_swap_map04_fgc45.sh \
  integration/research/GC_REAL_SWAP_MAP04_PREREGISTRATION.json

echo 'GC REAL SWAP MAP04 FGC45 GATE PASS'
