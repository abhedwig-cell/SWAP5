...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-gc-map05-${GITHUB_RUN_ID:-local}-$$"
...
LIBMF6="$BUILD/modflow-bin/libmf6.so" \
  ...
  E3_WINDOW_DAY="1e-3" \
  E3_QBOT_CM_PER_DAY="1e-4" \
  E3_K_M_PER_DAY="0.1" \
  E3_CHD_OFFSET_M="0.0" \
  python3 tests/publication/test_pub_gc_e3_case.py | tee "$BUILD/e3r-b3-k01.txt"

grep -Fq '"status":"CONVERGED"' "$BUILD/e3r-b3-k01.txt" || fail "existing E3-R B3 K=0.1 baseline"
grep -Fq '"coupling_outer_iterations":3' "$BUILD/e3r-b3-k01.txt" || fail "E3-R B3 K=0.1 outer-count preservation"

LIBMF6="$BUILD/modflow-bin/libmf6.so" \
  FGC44_SWAP_LIB="$BUILD/bridge/libfgc44_swap.so" \
  python3 tests/research/test_gc_real_swap_map05_b3_slope_policy.py | tee "$BUILD/map05.txt"

for marker in \
  'GC_MAP05_CURRENT_POLICY_E3R_PRESERVATION=PASS' \
  'GC_MAP05_ALL_POLICIES_CONVERGED=PASS' \
  'GC_MAP05_ACCEPTED_HEAD_INVARIANCE=PASS' \
  'GC_MAP05_ACCEPTED_CORRECTOR_FLUX_INVARIANCE=PASS' \
  'GC_MAP05_ACCEPTED_LEDGER_INVARIANCE=PASS' \
  'GC_MAP05_LIVE_GATE=PASS'; do
  grep -Fq "$marker" "$BUILD/map05.txt" || fail "missing marker $marker"
done

git diff --check -- tests/research/test_gc_real_swap_map05_b3_slope_policy.py \
  tests/research/run_gc_real_swap_map05_b3.sh \
  integration/research/GC_REAL_SWAP_MAP05_PREREGISTRATION.json

echo 'GC REAL SWAP MAP05 B3 GATE PASS'
