...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-gc-map05a-${GITHUB_RUN_ID:-local}-$$"
...
LIBMF6="$BUILD/modflow-bin/libmf6.so" \
  FGC44_SWAP_LIB="$BUILD/bridge/libfgc44_swap.so" \
  python3 tests/research/test_gc_real_swap_map05a_oracle_policy.py | tee "$BUILD/map05a.txt"

for marker in \
  'GC_MAP05A_ORACLE_POLICY_CONVERGED=PASS' \
  'GC_MAP05A_ACCEPTED_HEAD_CONTROL_EQUIVALENCE=PASS' \
  'GC_MAP05A_ACCEPTED_FLUX_CONTROL_EQUIVALENCE=PASS' \
  'GC_MAP05A_LEDGER_CONTROL_EQUIVALENCE=PASS' \
  'GC_MAP05A_LIVE_GATE=PASS'; do
  grep -Fq "$marker" "$BUILD/map05a.txt" || fail "missing marker $marker"
done

git diff --check -- tests/research/test_gc_real_swap_map05a_oracle_policy.py \
  tests/research/run_gc_real_swap_map05a_b3.sh \
  integration/research/GC_REAL_SWAP_MAP05A_PREREGISTRATION.json

echo 'GC REAL SWAP MAP05A B3 GATE PASS'
