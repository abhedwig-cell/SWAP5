...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-gc-map06-${GITHUB_RUN_ID:-local}-$$"
...
FGC44_SWAP_LIB="$BUILD/bridge/libfgc44_swap.so" \
  python3 tests/research/test_gc_real_swap_map06_b3_corrector_domain.py | tee "$BUILD/map06.txt"

for marker in \
  'GC_MAP06_CENTER_ADMISSIBLE=PASS' \
  'GC_MAP06_MAP05_FAILED_HEAD_REPRODUCED=PASS' \
  'GC_MAP06_MIXED_VALIDITY_GRID=PASS' \
  'GC_MAP06_REPEAT_STATUS_DETERMINISM=PASS' \
  'GC_MAP06_LIVE_GATE=PASS'; do
  grep -Fq "$marker" "$BUILD/map06.txt" || fail "missing marker $marker"
done

git diff --check -- tests/research/test_gc_real_swap_map06_b3_corrector_domain.py \
  tests/research/run_gc_real_swap_map06_b3.sh \
  integration/research/GC_REAL_SWAP_MAP06_PREREGISTRATION.json

echo 'GC REAL SWAP MAP06 B3 CORRECTOR DOMAIN GATE PASS'
