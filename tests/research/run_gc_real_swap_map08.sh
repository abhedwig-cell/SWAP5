...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-gc-map08-${GITHUB_RUN_ID:-local}-$$"
...
nm -D "$BUILD/bridge/libfgc44_swap.so" | grep -q 'fgc44_raw_corrector_diagnostics_c' || fail "missing MAP08 raw diagnostic C ABI"

FGC44_SWAP_LIB="$BUILD/bridge/libfgc44_swap.so" \
  python3 tests/research/test_gc_real_swap_map08_raw_corrector_failure.py | tee "$BUILD/map08.txt"

for marker in \
  'GC_MAP08_MAP06_PARTICIPANT_STATUS_REPRODUCED=PASS' \
  'GC_MAP08_VALID_RAW_ACCEPTANCE=PASS' \
  'GC_MAP08_INVALID_RAW_FAILURE_IDENTIFIED=PASS' \
  'GC_MAP08_ZERO_COMMITTED_STATE_MUTATION=PASS' \
  'GC_MAP08_LIVE_GATE=PASS'; do
  grep -Fq "$marker" "$BUILD/map08.txt" || fail "missing marker $marker"
done
