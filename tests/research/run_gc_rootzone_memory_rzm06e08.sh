...
python3 tests/research/test_gc_rootzone_memory_rzm06e08_extract.py   --result integration/research/GC_ROOTZONE_MEMORY_RZM06E07_RESULT.json   --output-a "$BUILD/e08-A.txt" --output-b "$BUILD/e08-B.txt"   | tee "$EVIDENCE/origin-extract.txt"
...
for opt in 0 2; do
  ...
  "$out/rom0_test" "$BUILD/e08-A.txt" "$BUILD/e08-B.txt" > "$EVIDENCE/e08-o$opt.txt" 2>&1 || {
    cat "$EVIDENCE/e08-o$opt.txt" >&2
    fail "E08 executable O$opt"
  }
  cat "$EVIDENCE/e08-o$opt.txt"
  grep -Fq 'GC_RZM06E08_ORDER_INDEPENDENCE=PASS' "$EVIDENCE/e08-o$opt.txt" || fail "order gate O$opt"
  grep -Fq 'GC_RZM06E08_FIXED_HC_PROBE_EXECUTION=PASS' "$EVIDENCE/e08-o$opt.txt" || fail "execution marker O$opt"
done

cmp "$EVIDENCE/e08-o0.txt" "$EVIDENCE/e08-o2.txt" || fail "O0/O2 output drift"
python3 tests/research/analyze_gc_rootzone_memory_rzm06e08.py   --input "$EVIDENCE/e08-o2.txt" --output "$EVIDENCE/result.json" | tee "$EVIDENCE/analysis.txt"
grep -Fq 'GC_RZM06E08_ANALYSIS=PASS' "$EVIDENCE/analysis.txt" || fail "analysis marker"

sha256sum "$EVIDENCE/origin-extract.txt" "$EVIDENCE/e08-o0.txt" "$EVIDENCE/e08-o2.txt"   "$EVIDENCE/analysis.txt" "$EVIDENCE/result.json" > "$EVIDENCE/sha256.txt"
