...
for opt in 0 2; do
  ...
  "$out/rom0_test" > "$EVIDENCE/e09-o$opt.txt" 2>&1 || {
    cat "$EVIDENCE/e09-o$opt.txt" >&2
    fail "E09 executable O$opt"
  }
  cat "$EVIDENCE/e09-o$opt.txt"
  grep -Fq 'GC_RZM06E09_EQUILIBRIUM_CONTROL=PASS' "$EVIDENCE/e09-o$opt.txt" || fail "control O$opt"
  grep -Fq 'GC_RZM06E09_DEEP_INTERNAL_REDISTRIBUTION=PASS' "$EVIDENCE/e09-o$opt.txt" || fail "generation O$opt"
done

cmp "$EVIDENCE/e09-o0.txt" "$EVIDENCE/e09-o2.txt" || fail "O0/O2 output drift"
echo 'GC_RZM06E09_O0_O2_IDENTITY=PASS'

python3 tests/research/test_gc_rootzone_memory_rzm06e09_select.py   --o0 "$EVIDENCE/e09-o0.txt" --o2 "$EVIDENCE/e09-o2.txt"   --output "$EVIDENCE/result.json" | tee "$EVIDENCE/selection.txt"
grep -Fq 'GC_RZM06E09_RESPONSE_BLIND_SELECTION=PASS' "$EVIDENCE/selection.txt" || fail "selector marker"

sha256sum "$EVIDENCE/materialize.txt" "$EVIDENCE/e09-o0.txt" "$EVIDENCE/e09-o2.txt"   "$EVIDENCE/selection.txt" "$EVIDENCE/result.json" > "$EVIDENCE/sha256.txt"
