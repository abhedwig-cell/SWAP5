...
python3 tests/research/test_gc_rootzone_memory_rzm06e10_extract.py   --result integration/research/GC_ROOTZONE_MEMORY_RZM06E09_RESULT.json   --output-a "$BUILD/e08-A.txt" --output-b "$BUILD/e08-B.txt"   | tee "$EVIDENCE/origin-extract.txt"
...
for opt in 0 2; do
  ...
  grep -Fq 'GC_RZM06E10_ORDER_INDEPENDENCE=PASS' "$EVIDENCE/e08-o$opt.txt" || fail "order gate O$opt"
  grep -Fq 'GC_RZM06E10_FIXED_HC_PROBE_EXECUTION=PASS' "$EVIDENCE/e08-o$opt.txt" || fail "execution marker O$opt"
done

cmp "$EVIDENCE/e08-o0.txt" "$EVIDENCE/e08-o2.txt" || fail "O0/O2 output drift"
python3 tests/research/analyze_gc_rootzone_memory_rzm06e10.py   --input "$EVIDENCE/e08-o2.txt" --output "$EVIDENCE/result.json" | tee "$EVIDENCE/analysis.txt"
