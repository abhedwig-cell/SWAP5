...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-hydro-memory-acc02-f2-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/modflow-bin" "$BUILD/downloads" "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "HYDRO_MEMORY_ACC02_F2_FAIL $*" >&2; exit 72; }
...
grep -Fq '"decision": "ACC02_F1_PASS_LIVE_ROOT_ACTIVE_SINGLE_WINDOW"' integration/research/HYDRO_MEMORY_ACC02_F1_RESULT.json || fail "ACC02-F1 authority missing"
...
echo 'HYDRO_MEMORY_ACC02_F2_AUTHORITY_LOCK=PASS'

BRIDGE=tests/research/support/mod_hydro_memory_acc02_f2_bridge.f90
...
echo 'HYDRO_MEMORY_ACC02_F2_FROZEN_FIXTURE=PASS'
...
MODULE_SRC=(
  ...
  tests/research/support/mod_hydro_memory_acc02_f2_bridge.f90
)
...
build_bridge(){
  ...
  gfortran -shared -fopenmp -O"$opt" "${objects[@]}" -o "$out/libhydro_memory_acc02_f2.so" || fail "link O$opt bridge"
  nm -D "$out/libhydro_memory_acc02_f2.so" | grep -q 'fgc44_swap_initialize_c' || fail "missing SWAP ABI O$opt"
  nm -D "$out/libhydro_memory_acc02_f2.so" | grep -q 'hydro_memory_acc02_head_policy_c' || fail "missing governed policy ABI O$opt"
  nm -D "$out/libhydro_memory_acc02_f2.so" | grep -q 'hydro_memory_acc02_f2_begin_next_window_c' || fail "missing next-window ABI O$opt"
}
build_bridge 0
build_bridge 2

for opt in 0 2; do
  LIBMF6="$BUILD/modflow-bin/libmf6.so"   FGC44_SWAP_LIB="$BUILD/o$opt/libhydro_memory_acc02_f2.so"   python3 tests/research/test_hydro_memory_acc02_f2_live_modflow.py > "$BUILD/o$opt/live.txt" 2>&1 || {
    cat "$BUILD/o$opt/live.txt" >&2
    fail "live runtime O$opt"
  }
  for marker in \
    HYDRO_MEMORY_ACC02_F2_PERSISTENT_SWAPP_STATE=PASS \
    HYDRO_MEMORY_ACC02_F2_DYNAMIC_PREDICTOR_ORIGIN=PASS \
    HYDRO_MEMORY_ACC02_F2_REVISION_TIME_LEDGER_SEQUENCE=PASS \
    HYDRO_MEMORY_ACC02_F2_GOVERNED_ACCURACY=PASS \
    HYDRO_MEMORY_ACC02_F2_LIVE_MODFLOW680=PASS \
    HYDRO_MEMORY_ACC02_F2_EXACT_ONCE_PUBLICATION=PASS \
    HYDRO_MEMORY_ACC02_F2_GATE=PASS; do
...
paths=sys.argv[1:]
keys=[
"HYDRO_MEMORY_ACC02_F2_H_APP_CM",
"HYDRO_MEMORY_ACC02_F2_TEMPORAL_BUDGET_CM",
"HYDRO_MEMORY_ACC02_F2_INTERFACE_TOLERANCE_M",
"HYDRO_MEMORY_ACC02_F2_ROOT_TOTAL_CM_PER_DAY",
"HYDRO_MEMORY_ACC02_F2_MAX_TEMPORAL_INDICATOR",
"HYDRO_MEMORY_ACC02_F2_MAX_HEAD_RESIDUAL_M",
"HYDRO_MEMORY_ACC02_F2_MAX_FLUX_RESIDUAL",
"HYDRO_MEMORY_ACC02_F2_FINAL_REVISION",
"HYDRO_MEMORY_ACC02_F2_FINAL_TIME_DAY",
"HYDRO_MEMORY_ACC02_F2_FINAL_LEDGER_EXCHANGE_M",
]
def parse(path):
    out={}
...
a,b=map(parse,paths)
for k in keys:
    if k=="HYDRO_MEMORY_ACC02_F2_FINAL_REVISION":
...
print("HYDRO_MEMORY_ACC02_F2_O0_O2_BEHAVIOR=PASS")
PY
cat "$BUILD/o2/live.txt"
echo 'HYDRO_MEMORY_ACC02_F2_MODFLOW_ASSET_HASH=PASS'
echo 'HYDRO_MEMORY_ACC02_F2_QUALIFICATION=PASS'
