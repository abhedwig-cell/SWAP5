...
$Base = "07e9f1ba6762e03a5b43b3e4e310fff153539d77"
$ImodPin = "8907fb13f8301ba1e0f32dd90a64ea475d4896d6"
$ImodRoot = if ($env:RIBASIM_DUMMY_20E3_IMOD_COUPLER_ROOT) {
    (Resolve-Path $env:RIBASIM_DUMMY_20E3_IMOD_COUPLER_ROOT).Path
} else {
    (Resolve-Path (Join-Path $Root ".imod-coupler-pin")).Path
}

function Fail([string]$Message) {
    Write-Error "RIBASIM_DUMMY_20E3_FAIL $Message"
    exit 73
}

git merge-base --is-ancestor $Base HEAD
if ($LASTEXITCODE -ne 0) { Fail "branch is not descended from DUMMY-20E3 preregistration base" }
...
$allowed = @(
    ...
    "integration/research/RIBASIM_DUMMY_20E3_PREREGISTRATION.json",
    "integration/research/RIBASIM_DUMMY_20E3_CONCEPT.md",
    "integration/research/RIBASIM_DUMMY_20E3_STATUS.json",
    "integration/research/RIBASIM_DUMMY_20E3_RESULT.json",
    "integration/research/RIBASIM_DUMMY_PROGRAM_STATUS.json",
    "tests/research/test_ribasim_dummy_20e3_two_cell_groundwater.py",
    "tests/research/run_ribasim_dummy_20e3.ps1"
)
$delta = @(git diff --name-only "$Base..HEAD")
foreach ($path in $delta) {
    if (($path -ne "") -and ($allowed -notcontains $path)) {
        Fail "unexpected DUMMY-20E3 branch delta: $path"
    }
}
Write-Output "RIBASIM_DUMMY_20E3_SOURCE_SCOPE=PASS"
...
Write-Output "RIBASIM_DUMMY_20E3_IMOD_COUPLER_PIN=PASS sha=$ActualPin"
...
$WorkRoot = Join-Path $ImodRoot "generated_testmodels/swap5_dummy_20e3"
if (Test-Path $WorkRoot) { Remove-Item -Recurse -Force $WorkRoot }

pixi run -e dev python "$Root/tests/research/test_ribasim_dummy_20e3_two_cell_groundwater.py" --work-root "$WorkRoot" --modflow-dll "$ModflowDll" --ribasim-dll "$RibasimDll" --ribasim-dep "$RibasimDep"
if ($LASTEXITCODE -ne 0) { Fail "two-cell groundwater product test failed" }

Write-Output "RIBASIM_DUMMY_20E3_REAL_PRODUCT_RUNTIME_TESTS=PASS"
Write-Output "RIBASIM_DUMMY_20E3_GATE=PASS"
