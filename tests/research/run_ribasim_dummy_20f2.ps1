...
$Base = "d8a0bb90c4053dee73c2c9b18c6148088b6bc871"
$ImodPin = "8907fb13f8301ba1e0f32dd90a64ea475d4896d6"
$ImodRoot = if ($env:RIBASIM_DUMMY_20F2_IMOD_COUPLER_ROOT) {
    (Resolve-Path $env:RIBASIM_DUMMY_20F2_IMOD_COUPLER_ROOT).Path
} else {
    (Resolve-Path (Join-Path $Root ".imod-coupler-pin")).Path
}

function Fail([string]$Message) {
    Write-Error "RIBASIM_DUMMY_20F2_FAIL $Message"
    exit 73
}

git merge-base --is-ancestor $Base HEAD
if ($LASTEXITCODE -ne 0) { Fail "branch is not descended from DUMMY-20F2 post-20E3 base" }
...
$allowed = @(
    ...
    "integration/research/RIBASIM_DUMMY_20F2_PREREGISTRATION.json",
    "integration/research/RIBASIM_DUMMY_20F2_CONCEPT.md",
    "integration/research/RIBASIM_DUMMY_20F2_STATUS.json",
    "integration/research/RIBASIM_DUMMY_20F2_RESULT.json",
    "integration/research/RIBASIM_DUMMY_PROGRAM_STATUS.json",
    "tests/research/test_ribasim_dummy_20f2_two_day_memory.py",
    "tests/research/run_ribasim_dummy_20f2.ps1"
)
$delta = @(git diff --name-only "$Base..HEAD")
foreach ($path in $delta) {
    if (($path -ne "") -and ($allowed -notcontains $path)) {
        Fail "unexpected DUMMY-20F2 branch delta: $path"
    }
}
Write-Output "RIBASIM_DUMMY_20F2_SOURCE_SCOPE=PASS"
...
Write-Output "RIBASIM_DUMMY_20F2_IMOD_COUPLER_PIN=PASS sha=$ActualPin"
...
$WorkRoot = Join-Path $ImodRoot "generated_testmodels/swap5_dummy_20f2"
if (Test-Path $WorkRoot) { Remove-Item -Recurse -Force $WorkRoot }

pixi run -e dev python "$Root/tests/research/test_ribasim_dummy_20f2_two_day_memory.py" --work-root "$WorkRoot" --modflow-dll "$ModflowDll" --ribasim-dll "$RibasimDll" --ribasim-dep "$RibasimDep"
if ($LASTEXITCODE -ne 0) { Fail "two-day next-boundary memory product test failed" }

Write-Output "RIBASIM_DUMMY_20F2_REAL_PRODUCT_RUNTIME_TESTS=PASS"
Write-Output "RIBASIM_DUMMY_20F2_GATE=PASS"
