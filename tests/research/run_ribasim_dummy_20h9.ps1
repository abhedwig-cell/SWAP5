...
$Base = "f9973c9a3732c0063854233d701e5c0ac8d1972f"
$ImodPin = "8907fb13f8301ba1e0f32dd90a64ea475d4896d6"
$ImodRoot = if ($env:RIBASIM_DUMMY_20H9_IMOD_COUPLER_ROOT) {
    (Resolve-Path $env:RIBASIM_DUMMY_20H9_IMOD_COUPLER_ROOT).Path
} else {
    (Resolve-Path (Join-Path $Root ".imod-coupler-pin")).Path
}

function Fail([string]$Message) {
    Write-Error "RIBASIM_DUMMY_20H9_FAIL $Message"
    exit 73
}

git merge-base --is-ancestor $Base HEAD
if ($LASTEXITCODE -ne 0) { Fail "branch is not descended from post-H8 authority" }
...
$allowed = @(
    ...
    "integration/research/RIBASIM_DUMMY_20H9_PREREGISTRATION.json",
    "integration/research/RIBASIM_DUMMY_20H9_CONCEPT.md",
    "integration/research/RIBASIM_DUMMY_20H9_STATUS.json",
    "integration/research/RIBASIM_DUMMY_20H9_RESULT.json",
    "integration/research/RIBASIM_DUMMY_PROGRAM_STATUS.json",
    "tests/research/test_ribasim_dummy_20h9_positive_groundwater_next_boundary.py",
    "tests/research/run_ribasim_dummy_20h9.ps1"
)
$delta = @(git diff --name-only "$Base..HEAD")
foreach ($path in $delta) {
    if (($path -ne "") -and ($allowed -notcontains $path)) {
        Fail "unexpected DUMMY-20H9 branch delta: $path"
    }
}
Write-Output "RIBASIM_DUMMY_20H9_SOURCE_SCOPE=PASS"
...
Write-Output "RIBASIM_DUMMY_20H9_IMOD_COUPLER_PIN=PASS sha=$ActualPin"
...
$WorkRoot = Join-Path $ImodRoot "generated_testmodels/swap5_dummy_20h9"
if (Test-Path $WorkRoot) { Remove-Item -Recurse -Force $WorkRoot }

pixi run -e dev python "$Root/tests/research/test_ribasim_dummy_20h9_positive_groundwater_next_boundary.py" --work-root "$WorkRoot" --modflow-dll "$ModflowDll" --ribasim-dll "$RibasimDll" --ribasim-dep "$RibasimDep"
if ($LASTEXITCODE -ne 0) { Fail "actual positive-groundwater next-boundary product bridge failed" }

Write-Output "RIBASIM_DUMMY_20H9_REAL_PRODUCT_RUNTIME_TESTS=PASS"
Write-Output "RIBASIM_DUMMY_20H9_GATE=PASS"
