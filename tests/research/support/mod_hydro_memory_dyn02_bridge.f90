module mod_hydro_memory_dyn02_bridge
  use, intrinsic :: iso_c_binding, only: c_double, c_int
  use, intrinsic :: iso_fortran_env, only: int64, real64
  use, intrinsic :: ieee_arithmetic, only: ieee_is_finite
  use MOD_grid, only: numnod, z, dz, disnod
  use mod_transaction_reference, only: transaction_state_t, TX_TEMPORAL_MODEL_CERTIFICATE
  use mod_canonical_contracts, only: canonical_numerical_config_t, canonical_interval_t
  ...
  use mod_b110_dynamic_top_boundary_provider, only: b110_dynamic_top_boundary_request_t, &
       b110_dynamic_top_boundary_result_t, evaluate_b110_dynamic_top_boundary, B110_DYN_TOP_AVAILABLE, B110_DYN_TOP_REGIME_FLUX
  use mod_ppa_wu03_common_forcing_adapter, only: ppa_wu03_common_forcing_config_t, ppa_wu03_common_forcing_input_t, &
       ppa_wu03_common_forcing_result_t, ppa_wu03_common_forcing_diagnostics_t, materialize_ppa_wu03_common_forcing, &
       bind_ppa_wu03_flux_result_to_effective_forcing, PPA_WU03_OK, PPA_WU03_ET_REFERENCE, &
       PPA_WU03_INTERCEPTION_NONE, PPA_WU03_IRRIGATION_NONE
  use mod_reference_et_demand_process, only: reference_et_demand_result_t, reference_et_demand_diagnostics_t, REF_ET_DEMAND_OK
  use mod_fmr_reference_et_demand_binding, only: fmr_reference_et_binding_diagnostics_t, fmr_evaluate_reference_et_demand, &
  ...
  real(real64), parameter :: DEFAULT_DURATION_DAY=1.0e-1_real64
  real(real64), parameter :: TOL=1.0e-12_real64
  real(real64), parameter :: DEFAULT_PREDICTOR_QBOT=1.0e-6_real64
  real(real64), parameter :: H_APP_CM=4.0e-1_real64
  real(real64), parameter :: A_TEMPORAL=2.5e-1_real64
  real(real64), parameter :: A_INTERFACE=2.5e-1_real64
  real(real64), parameter :: ROOT_TOTAL=2.0e-2_real64
  real(real64), parameter :: DYN02_DROUGHT_ET_MM=4.0_real64
  real(real64), parameter :: DYN02_RECOVERY_ET_MM=3.0_real64
  real(real64), parameter :: DYN02_DROUGHT_P_CM=0.0_real64
  real(real64), parameter :: DYN02_RECOVERY_P_CM=5.0e-1_real64
  integer(int64), parameter :: ACCURACY_CONTRACT_ID=590200_int64
  integer(int64), parameter :: APPLICATION_PROVENANCE_ID=590201_int64
  integer(int64), parameter :: TEMPORAL_PROVENANCE_ID=590202_int64
  integer(int64), parameter :: INTERFACE_PROVENANCE_ID=590203_int64
  integer(int64), parameter :: HEAD_POLICY_ID=590210_int64
  integer, parameter :: HEAD_POLICY_VERSION=1
  ...
  type(coupling_application_accuracy_contract_t), save :: accuracy_contract
  type(groundwater_head_convergence_policy_t), save :: interface_policy
  type(groundwater_accuracy_binding_receipt_t), save :: accuracy_receipt
  ...
  logical, save :: predictor_root_coverage=.false.
  logical, save :: predictor_endpoint_authoritative=.false.
  logical, save :: accepted_interface_ready=.false.
  type(groundwater_interface_state_t), save :: accepted_interface_state
  integer, save :: dyn02_window_index=0
  integer(int64), save :: dyn02_forcing_revision=-1_int64
  real(real64), save :: dyn02_potential_transpiration=0.0_real64
  real(real64), save :: dyn02_actual_uptake=0.0_real64
  real(real64), save :: dyn02_top_flux=0.0_real64
  real(real64), save :: dyn02_root_sink_sum=0.0_real64
  ...
  public :: hydro_memory_acc02_accuracy_c, hydro_memory_acc02_head_policy_c
  public :: hydro_memory_dyn02_begin_next_window_c
  public :: hydro_memory_dyn02_window_diagnostics_c
...
contains
  ...
  subroutine fgc44_initialize_impl(duration_day,predictor_qbot,hcof,rhs,reference_head,c_status)
    ...
    predictor_root_coverage=.false.; predictor_endpoint_authoritative=.false.
    accepted_interface_ready=.false.; accepted_interface_state=groundwater_interface_state_t()
    dyn02_window_index=0; dyn02_forcing_revision=-1_int64
    dyn02_potential_transpiration=0.0_real64; dyn02_actual_uptake=0.0_real64
    dyn02_top_flux=0.0_real64; dyn02_root_sink_sum=0.0_real64
    ...
    call initialize_configs(predictor_config,corrector_config,ok)
    if(.not.ok)return
    c_status=102_c_int
    ...
    call materialize_dyn02_window_forcing(1,status)
    if(status/=0)return
    ...
    call build_modflow6_swap_predictor_tangent_endpoint(predictor_state,solver_parameters,constitutive, &
         result%accepted_trajectory_direction,qeq,datum,.false.,.true.,.false.,.false.,endpoint,status)
    predictor_root_coverage=result%accepted_trajectory_direction%root_sink_direction_coverage_complete
    predictor_endpoint_authoritative=endpoint%authoritative
    if(status/=MODFLOW6_TANGENT_ENDPOINT_OK .or. .not.endpoint%authoritative .or. .not.predictor_root_coverage)return
    c_status=108_c_int
    ...
    accepted_interface_state=accepted_interface
    accepted_interface_ready=.true.
    c_status=111_c_int
  ...
  integer(c_int) function hydro_memory_dyn02_window_diagnostics_c(window_index,forcing_revision,ptra,actual_uptake,top_flux,root_sum) &
       bind(C,name="hydro_memory_dyn02_window_diagnostics_c")
    integer(c_int), intent(out) :: window_index, forcing_revision
    real(c_double), intent(out) :: ptra,actual_uptake,top_flux,root_sum
    hydro_memory_dyn02_window_diagnostics_c=1_c_int
    window_index=int(dyn02_window_index,c_int)
    forcing_revision=int(dyn02_forcing_revision,c_int)
    ptra=real(dyn02_potential_transpiration,c_double)
    actual_uptake=real(dyn02_actual_uptake,c_double)
    top_flux=real(dyn02_top_flux,c_double)
    root_sum=real(dyn02_root_sink_sum,c_double)
    if(.not.initialized)return
    if(dyn02_window_index<1 .or. dyn02_forcing_revision<0_int64)return
    if(.not.ieee_is_finite(dyn02_potential_transpiration) .or. .not.ieee_is_finite(dyn02_actual_uptake) .or. &
       .not.ieee_is_finite(dyn02_top_flux) .or. .not.ieee_is_finite(dyn02_root_sink_sum))return
    hydro_memory_dyn02_window_diagnostics_c=0_c_int
  end function hydro_memory_dyn02_window_diagnostics_c

  subroutine materialize_dyn02_window_forcing(window_index,status_out)
    integer, intent(in) :: window_index
    integer, intent(out) :: status_out
    type(ppa_wu03_common_forcing_config_t) :: forcing_config
    type(ppa_wu03_common_forcing_input_t) :: normal_input
    type(ppa_wu03_common_forcing_result_t) :: forcing_result
    type(ppa_wu03_common_forcing_diagnostics_t) :: forcing_diag
    ...
    type(fmr_crop_root_uptake_adapter_diagnostics_t) :: root_adapter_diag
    type(fmr_root_uptake_binding_diagnostics_t) :: root_binding_diag
    type(fmr_reference_et_root_uptake_diagnostics_t) :: root_composition_diag
    type(fmr_b110_physical_forcing_t) :: bound_forcing
    type(soil_water_parameter_set_t) :: geometry
    type(b110_default_mvg_parameters_t), target :: hp
    class(transaction_state_t), allocatable :: snapshot
    real(real64) :: precipitation,et_mm
    logical :: available
    integer :: status

    status_out=1
    if(window_index<1 .or. window_index>4)return
    if(abs((window%t1-window%t0)-active_duration_day)>128.0_real64*epsilon(1.0_real64)*max(1.0_real64,active_duration_day))return
    ...
    class is(fmr_b110_physical_state_t)
      ...
      if(window_index<=2)then
        precipitation=DYN02_DROUGHT_P_CM
        et_mm=DYN02_DROUGHT_ET_MM
      else
        precipitation=DYN02_RECOVERY_P_CM
        et_mm=DYN02_RECOVERY_ET_MM
      end if
      ...
      normal_input%interval%t0=window%t0
      normal_input%interval%t1=window%t1
      normal_input%forcing_t0=window%t0
      normal_input%forcing_t1=window%t1
      normal_input%precipitation_rate_cm_per_day=precipitation
      normal_input%surface_irrigation_rate_cm_per_day=0.0_real64
      normal_input%reference_et%t0=window%t0
      normal_input%reference_et%t1=window%t1
      normal_input%reference_et%reference_et_mm_per_day=et_mm
      normal_input%canopy%crop_emerged=.true.
      normal_input%canopy%vegetation_cover_fraction=1.0_real64
      ...
      base_top%conductivity_mean_method=predictor_parameters%swkmean
      base_top%pressure_head_top_cm=physical%pressure_head(1)
      base_top%water_content_top=physical%water_content(1)
      base_top%candidate_ponding_depth_cm=0.0_real64
      base_top%previous_ponding_depth_cm=0.0_real64
      base_top%step_duration_day=active_duration_day
      base_top%ponding_max_cm=2.0_real64
      base_top%runoff_resistance_day=1.0_real64
      base_top%runoff_exponent=1.0_real64

      call materialize_ppa_wu03_common_forcing(forcing_config,normal_input,base_top,forcing_result,forcing_diag)
      if(forcing_diag%status/=PPA_WU03_OK .or. .not.forcing_result%valid)return

      geometry%parameter_set_id=predictor_parameters%parameter_set_id
      geometry%active_nodes=numnod
      allocate(geometry%z(numnod),geometry%dz(numnod),geometry%node_distance(numnod))
      geometry%z=predictor_parameters%z
      geometry%dz=predictor_parameters%dz
      geometry%node_distance=predictor_parameters%node_distance
      call initialize_b110_default_mvg_parameters(hp,predictor_parameters%cofgen)
      call evaluate_b110_dynamic_top_boundary(geometry,hp,forcing_result%top_request,top_result)
      if(top_result%status/=B110_DYN_TOP_AVAILABLE .or. top_result%regime/=B110_DYN_TOP_REGIME_FLUX)return
      if(top_result%runoff_potential .or. top_result%candidate_ponding_depth_cm/=0.0_real64 .or. top_result%runoff_depth_cm/=0.0_real64)return
      call bind_ppa_wu03_flux_result_to_effective_forcing(base_forcing,top_result,bound_forcing,status)
      if(status/=PPA_WU03_OK)return

      call fmr_evaluate_reference_et_demand(normal_input%interval,normal_input%reference_et, &
           forcing_config%reference_et_parameters,normal_input%canopy,et_result,et_process_diag,et_diag)
      if(et_diag%status/=FMR_REFERENCE_ET_BINDING_OK .or. et_process_diag%status/=REF_ET_DEMAND_OK .or. .not.et_diag%result_produced)return
      if(abs(et_result%potential_transpiration_cm_per_day-forcing_result%reference_et_demand%potential_transpiration_cm_per_day) &
           >128.0_real64*epsilon(1.0_real64)*max(1.0_real64,abs(et_result%potential_transpiration_cm_per_day)))return
      ...
      root_input%crop_emerged=.true.
      root_input%potential_transpiration=999.0_real64
      ...
      call fmr_evaluate_reference_et_root_uptake(committed,root_parameters,root_input,et_result,et_diag, &
           root_flux,root_process_diag,ptra_diag,root_adapter_diag,root_binding_diag,root_composition_diag)
      if(root_composition_diag%status/=FMR_REFERENCE_ET_ROOT_UPTAKE_OK .or. .not.root_composition_diag%result_produced)return
      if(.not.allocated(root_flux%root_extraction_sink) .or. size(root_flux%root_extraction_sink)/=numnod)return
      if(any(.not.ieee_is_finite(root_flux%root_extraction_sink)) .or. any(root_flux%root_extraction_sink<0.0_real64))return
      if(.not.ieee_is_finite(root_flux%actual_uptake_total) .or. root_flux%actual_uptake_total<0.0_real64)return
      if(root_flux%actual_uptake_total>et_result%potential_transpiration_cm_per_day+ &
           128.0_real64*epsilon(1.0_real64)*max(1.0_real64,et_result%potential_transpiration_cm_per_day))return

      bound_forcing%root_extraction_sink=root_flux%root_extraction_sink
      base_forcing=bound_forcing
      dyn02_window_index=window_index
      dyn02_forcing_revision=committed%current_revision()
      dyn02_potential_transpiration=et_result%potential_transpiration_cm_per_day
      dyn02_actual_uptake=root_flux%actual_uptake_total
      dyn02_top_flux=top_result%actual_top_flux_cm_per_day
      dyn02_root_sink_sum=sum(root_flux%root_extraction_sink)
      if(abs(dyn02_root_sink_sum-dyn02_actual_uptake)>128.0_real64*epsilon(1.0_real64)*max(1.0_real64,abs(dyn02_actual_uptake)))return
      status_out=0
    class default
      return
    end select
  end subroutine materialize_dyn02_window_forcing

  integer(c_int) function hydro_memory_dyn02_begin_next_window_c(accepted_groundwater_head_m,hcof,rhs,reference_head) &
       bind(C,name="hydro_memory_dyn02_begin_next_window_c")
    real(c_double), value, intent(in) :: accepted_groundwater_head_m
    real(c_double), intent(out) :: hcof,rhs,reference_head
    real(real64) :: committed_time,q_groundwater
    logical :: available
    integer :: status,flux_status

    hydro_memory_dyn02_begin_next_window_c=1_c_int
    hcof=0.0_c_double; rhs=0.0_c_double; reference_head=0.0_c_double
    if(.not.initialized .or. .not.accepted_interface_ready)return
    if(.not.ieee_is_finite(real(accepted_groundwater_head_m,real64)))return
    if(participant%has_live_candidate())return
    if(ledger%has_active_trial() .or. ledger%has_prepared_trial())return
    if(.not.last_trial%valid)return
    ...
    accepted_interface_state%h_swap_m=last_trial%prescribed_head_m
    accepted_interface_state%h_groundwater_m=real(accepted_groundwater_head_m,real64)
    accepted_interface_state%q_swap_m_per_s=last_trial%q_swap_m_per_s
    call pair_groundwater_flux_from_swap(last_trial%q_swap_m_per_s,q_groundwater,flux_status)
    if(flux_status/=GW_INTERFACE_OK)return
    accepted_interface_state%q_groundwater_m_per_s=q_groundwater
    if(.not.accepted_interface_state%finite())return

    window%t0=committed_time
    window%t1=committed_time+active_duration_day
    call materialize_dyn02_window_forcing(int(committed%current_revision())+1,status)
    if(status/=0)return
    call materializer%initialize(base_forcing)
    call build_next_window_predictor(accepted_interface_state,hcof,rhs,reference_head,status)
    if(status/=0)return
    accepted_interface_ready=.true.
    hydro_memory_dyn02_begin_next_window_c=0_c_int
  end function hydro_memory_dyn02_begin_next_window_c

  subroutine build_next_window_predictor(accepted_interface,hcof,rhs,reference_head,status_out)
    ...
    call materialize_solver_view(candidate,predictor_state,solver_parameters,ok); if(.not.ok)return
    call build_modflow6_swap_predictor_tangent_endpoint(predictor_state,solver_parameters,constitutive, &
         result%accepted_trajectory_direction,active_predictor_qbot,datum,.false.,.true.,.false.,.false.,endpoint,status)
    predictor_root_coverage=result%accepted_trajectory_direction%root_sink_direction_coverage_complete
    predictor_endpoint_authoritative=endpoint%authoritative
    if(status/=MODFLOW6_TANGENT_ENDPOINT_OK .or. .not.endpoint%authoritative .or. .not.predictor_root_coverage)return
    ...
    predictor_lineage%coupling_id=COUPLING_ID
    predictor_lineage%swap_lineage_id=COLUMN_ID
    predictor_lineage%swap_origin_revision=origin_revision
    predictor_lineage%groundwater_service_id=GW_SERVICE_ID
    predictor_lineage%groundwater_lineage_id=GW_LINEAGE_ID
    predictor_lineage%groundwater_origin_revision=origin_revision
    ...
    hcof=term%hcof_m2_per_day
    rhs=term%rhs_m3_per_day
    reference_head=term%reference_head_m

    call predictor_backend%discard_trial_candidate(candidate,diagnostics)
    call participant%capture_origin(committed,status)
    if(status/=GW_SWAP_PARTICIPANT_OK)return
    status_out=0
  end subroutine build_next_window_predictor
  ...
  integer(c_int) function fgc44_ledger_prepare_c() bind(C,name="fgc44_ledger_prepare_c")
    ...
    lineage%coupling_id=COUPLING_ID; lineage%swap_lineage_id=COLUMN_ID
    lineage%swap_origin_revision=participant%captured_revision()
    lineage%groundwater_lineage_id=GW_LINEAGE_ID
    lineage%groundwater_origin_revision=participant%captured_revision()
    lineage%candidate_revision=participant%captured_revision()+1_int64
  ...
  integer(c_int) function hydro_memory_acc02_accuracy_c(h_app_cm,temporal_budget_cm,interface_tolerance_m, &
       root_total_cm_per_day,root_coverage,endpoint_authoritative) bind(C,name="hydro_memory_acc02_accuracy_c")
    real(c_double),intent(out) :: h_app_cm,temporal_budget_cm,interface_tolerance_m,root_total_cm_per_day
    integer(c_int),intent(out) :: root_coverage,endpoint_authoritative
    real(real64) :: h_app_m,temporal_budget_m,interface_fraction,temporal_fraction
    logical :: available
    hydro_memory_acc02_accuracy_c=1_c_int
    h_app_cm=0.0_c_double; temporal_budget_cm=0.0_c_double; interface_tolerance_m=0.0_c_double
    root_total_cm_per_day=0.0_c_double; root_coverage=0_c_int; endpoint_authoritative=0_c_int
    if(.not.initialized)return
    call accuracy_receipt%budgets(h_app_m,temporal_budget_m,interface_tolerance_m,temporal_fraction,interface_fraction,available)
    if(.not.available)return
    h_app_cm=h_app_m*100.0_real64
    temporal_budget_cm=temporal_budget_m*100.0_real64
    root_total_cm_per_day=ROOT_TOTAL
    if(predictor_root_coverage)root_coverage=1_c_int
    if(predictor_endpoint_authoritative)endpoint_authoritative=1_c_int
    hydro_memory_acc02_accuracy_c=0_c_int
  end function hydro_memory_acc02_accuracy_c

  integer(c_int) function hydro_memory_acc02_head_policy_c(head_residual_m,converged) &
       bind(C,name="hydro_memory_acc02_head_policy_c")
    real(c_double),value,intent(in) :: head_residual_m
    integer(c_int),intent(out) :: converged
    logical :: accepted
    integer :: policy_status
    hydro_memory_acc02_head_policy_c=1_c_int; converged=0_c_int
    if(.not.initialized)return
    call interface_policy%evaluate(real(head_residual_m,real64),accepted,policy_status)
    if(policy_status/=GW_HEAD_POLICY_OK)return
    if(accepted)converged=1_c_int
    hydro_memory_acc02_head_policy_c=0_c_int
  end function hydro_memory_acc02_head_policy_c
  ...
  subroutine initialize_parameters(p,bottom_mode)
    ...
    p%head_rel_tolerance=TOL; p%ponding_tolerance=TOL; p%root_extraction_active=.true.
  ...
  subroutine initialize_configs(pred,corr,ok)
    ...
    ok=.false.
    accuracy_contract=coupling_application_accuracy_contract_t()
    accuracy_contract%contract_id=ACCURACY_CONTRACT_ID
    accuracy_contract%contract_version=1
    accuracy_contract%qoi_kind=COUPLING_QOI_GROUNDWATER_HEAD
    accuracy_contract%h_app_available=.true.
    accuracy_contract%h_app_cm=H_APP_CM
    accuracy_contract%h_app_externally_qualified=.true.
    accuracy_contract%application_provenance_id=APPLICATION_PROVENANCE_ID
    accuracy_contract%a_temporal_available=.true.
    accuracy_contract%a_temporal=A_TEMPORAL
    accuracy_contract%a_temporal_externally_qualified=.true.
    accuracy_contract%temporal_allocation_provenance_id=TEMPORAL_PROVENANCE_ID
    ...
    pred%model_temporal_indicator_budget_available=.false.; pred%model_temporal_indicator_budget=0.0_real64
    call accuracy_contract%materialize_model_temporal_budget(pred,materialized)
    ...
    call bind_groundwater_head_accuracy(accuracy_contract,HEAD_POLICY_ID,HEAD_POLICY_VERSION, &
         INTERFACE_PROVENANCE_ID,A_INTERFACE,interface_policy,accuracy_receipt,bind_status)
    if(bind_status/=GW_ACCURACY_BIND_OK .or. .not.accuracy_receipt%ready() .or. .not.interface_policy%valid())return
    ok=.true.
...
end module mod_hydro_memory_dyn02_bridge
