program test_gc_rootzone_memory_rzm06d02_reseed
  ...
  write(*,'(*(g0))') 'RZM06D02_RESEED|TA=',t_a,'|TB=',t_b,'|DT=',dt, &
...
contains
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)')'GC_RZM06D02_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_gc_rootzone_memory_rzm06d02_reseed
