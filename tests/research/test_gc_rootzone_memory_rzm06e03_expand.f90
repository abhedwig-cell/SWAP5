program test_gc_rootzone_memory_rzm06e03_expand
  ...
  integer, parameter :: perturb_steps=10
  integer, parameter :: recovery_steps=24
  integer(int64), parameter :: lineage_base=960703_int64

  type(fmr_b110_physical_parameters_t) :: parameters
  type(fmr_b110_physical_state_t) :: initial_state
  type(fmr_logical_column_t) :: column
  type(fmr_template_t) :: template
  real(real64) :: h0,k0,qeq,qclosed,qover

  call require(numnod==16,'E03 geometry node count')
  call require(all(abs(dz-10.0_real64)<=1.0e-14_real64),'E03 geometry dz')
  call require(same_bits(dt,3435974.0_real64/4294967296.0_real64),'E03 dyadic dt')
  ...
  write(*,'(*(g0))') 'RZM06E03_CONFIG|N=',numnod,'|DZ_CM=',dz(1),'|DT_DAY=',dt,'|K0=',k0, &
       '|QEQ=',qeq,'|QCLOSED=',qclosed,'|QOVER=',qover,'|PERTURB_STEPS=',perturb_steps, &
       '|RECOVERY_STEPS=',recovery_steps

  call run_single('BASE_EQ','CONTROL',qeq,parameters,initial_state,column,template,lineage_base)
  call run_two_stage('DRY_RECOVER',qclosed,qover,parameters,initial_state,column,template,lineage_base+1_int64)
  call run_two_stage('WET_RECOVER',qover,qclosed,parameters,initial_state,column,template,lineage_base+2_int64)

  write(*,'(A)') 'GC_RZM06E03_STATE_EXPANSION=PASS'

contains

  subroutine run_single(label,phase,q,p,origin,col,tmpl,lineage)
    ...
    call run_family(label,phase,1,q,0,p,origin,col,tmpl,lineage,.true.,completed)
    call require(completed,trim(label)//' control completed')
  end subroutine run_single

  subroutine run_two_stage(label,q1,q2,p,origin,col,tmpl,lineage)
    character(len=*),intent(in) :: label
    real(real64),intent(in) :: q1,q2
    type(fmr_b110_physical_parameters_t),intent(in) :: p
    type(fmr_b110_physical_state_t),intent(in) :: origin
    type(fmr_logical_column_t),intent(in) :: col
    type(fmr_template_t),intent(in) :: tmpl
    ...
    call run_segment(label,'PERTURB',perturb_steps,q1,0,p,committed,backend,col,tmpl,completed)
    if(.not.completed) return
    call run_segment(label,'RECOVER',recovery_steps,q2,perturb_steps,p,committed,backend,col,tmpl,completed)
  end subroutine run_two_stage

  subroutine run_family(label,phase,nsteps,q,step_offset,p,origin,col,tmpl,lineage,required,completed)
    character(len=*),intent(in) :: label,phase
    integer,intent(in) :: nsteps,step_offset
    real(real64),intent(in) :: q
    type(fmr_b110_physical_parameters_t),intent(in) :: p
    type(fmr_b110_physical_state_t),intent(in) :: origin
    type(fmr_logical_column_t),intent(in) :: col
    ...
    call require(ok.and.committed%ready(),trim(label)//' committed seed')
    call backend%initialize(top)
    call run_segment(label,phase,nsteps,q,step_offset,p,committed,backend,col,tmpl,completed)
    if(required) call require(completed,trim(label)//' required segment completed')
  end subroutine run_family

  subroutine run_segment(label,phase,nsteps,q,step_offset,p,committed,backend,col,tmpl,completed)
    character(len=*),intent(in) :: label,phase
    integer,intent(in) :: nsteps,step_offset
    real(real64),intent(in) :: q
    type(fmr_b110_physical_parameters_t),intent(in) :: p
    ...
    integer :: phase_step,global_step,commit_status
    ...
    do phase_step=1,nsteps
      global_step=step_offset+phase_step
      ...
      if(result%status/=KERNEL_REFERENCE_FLOOR_STATUS_OK .or. .not.result%sample_valid .or. .not.candidate%ready() .or. &
        ...
        call require(committed%current_revision()==int(global_step-1,int64),trim(label)//' failed revision immutable')
        write(*,'(*(g0))') 'RZM06E03_STOP|FAMILY=',trim(label),'|PHASE=',trim(phase),'|STEP=',global_step, &
             '|PHASE_STEP=',phase_step,'|STATUS=',result%status,'|SAMPLE_VALID=',result%sample_valid, &
             '|CANDIDATE_READY=',candidate%ready(),'|MASS_COMPLETE=',result%mass%complete, &
             '|ATTEMPTS=',diag%attempts,'|RETRIES=',diag%retries,'|SOLVER_REJECTIONS=',diag%solver_rejections, &
      ...
      call require(committed%current_revision()==int(global_step,int64),trim(label)//' revision progression')
      call committed%current_time(tcur,available)
      call require(available.and.same_bits(tcur,t1),trim(label)//' time progression')
      call emit_committed_state(label,phase,global_step,phase_step,tcur,committed)
    end do

    completed=.true.
  end subroutine run_segment

  subroutine emit_committed_state(label,phase,step,phase_step,t,committed)
    character(len=*),intent(in) :: label,phase
    integer,intent(in) :: step,phase_step
    ...
    class is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'RZM06E03_STATE|FAMILY=',trim(label),'|PHASE=',trim(phase),'|STEP=',step, &
           '|PHASE_STEP=',phase_step,'|T=',t
      do i=1,numnod
        write(*,'(*(g0))') 'RZM06E03_NODE|FAMILY=',trim(label),'|PHASE=',trim(phase),'|STEP=',step, &
             '|NODE=',i,'|H=',s%pressure_head(i),'|THETA=',s%water_content(i)
      end do
    class default
  ...
  subroutine initialize_parameters(p)
    ...
    p%parameter_set_id=960703_int64;p%active_nodes=numnod
  ...
  subroutine initialize_state(p,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'E03 finite B01 seed')
  ...
  subroutine initialize_identity(col,tmpl)
    ...
    tmpl%template_id=960703_int64;tmpl%physics_topology_id=960704_int64
    tmpl%vertical_layout_id=960705_int64;tmpl%state_layout_id=960706_int64
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)')'GC_RZM06E03_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_gc_rootzone_memory_rzm06e03_expand
