program test_gc_rootzone_memory_rzm06e03b_expand
  ...
  integer, parameter :: ngrid(5)=[8,9,10,11,12]
  integer(int64), parameter :: lineage_base=960704_int64
  ...
  call require(numnod==16,'E03B geometry node count')
  call require(all(abs(dz-10.0_real64)<=1.0e-14_real64),'E03B geometry dz')
  call require(same_bits(dt,3435974.0_real64/4294967296.0_real64),'E03B dyadic dt')
  call initialize_parameters(parameters)
  call initialize_state(parameters,h0,k0,initial_state)
  qeq=-k0
  call initialize_identity(column,template)

  write(*,'(*(g0))') 'RZM06E03B_CONFIG|N=',numnod,'|DZ_CM=',dz(1),'|DT_DAY=',dt, &
       '|K0=',k0,'|QEQ=',qeq,'|CLOSED_STEPS=',closed_steps,'|NGRID=8,9,10,11,12'

  call run_baseline(parameters,initial_state,column,template,lineage_base,qeq)

  do j=1,size(ngrid)
    write(label,'("TB_N",I2.2)') ngrid(j)
    call run_split(trim(label),ngrid(j),.true.,parameters,initial_state,column,template, &
         lineage_base+int(2*j-1,int64),qeq)
    write(label,'("BT_N",I2.2)') ngrid(j)
    call run_split(trim(label),ngrid(j),.false.,parameters,initial_state,column,template, &
         lineage_base+int(2*j,int64),qeq)
  end do

  write(*,'(A)') 'GC_RZM06E03B_SPLIT_BOUNDARY_GENERATION=PASS'
...
contains
  ...
  subroutine run_baseline(p,origin,col,tmpl,lineage,q)
    ...
    committed_steps=0
    call run_segment('BASE_EQ','CONTROL',1,q,q,p,committed,backend,col,tmpl,committed_steps,completed)
    call require(completed,'BASE_EQ strict control completion')
    call emit_candidate('BASE_EQ','CONTROL',1,1,committed)
  end subroutine run_baseline

  subroutine run_split(label,n,top_first,p,origin,col,tmpl,lineage,q)
    character(len=*),intent(in) :: label
    integer,intent(in) :: n
    logical,intent(in) :: top_first
    type(fmr_b110_physical_parameters_t),intent(in) :: p
    type(fmr_b110_physical_state_t),intent(in) :: origin
    type(fmr_logical_column_t),intent(in) :: col
    type(fmr_template_t),intent(in) :: tmpl
    ...
    call run_segment(label,'CLOSED',closed_steps,0.0_real64,0.0_real64,p,committed,backend,col,tmpl, &
         committed_steps,completed)
    if(.not.completed) return

    if(top_first)then
      call run_segment(label,'TOP',n,q,0.0_real64,p,committed,backend,col,tmpl,committed_steps,completed)
      if(.not.completed) return
      call run_segment(label,'BOTTOM',n,0.0_real64,q,p,committed,backend,col,tmpl,committed_steps,completed)
      if(.not.completed) return
      call emit_candidate(label,'TOP_THEN_BOTTOM',n,committed_steps,committed)
    else
      call run_segment(label,'BOTTOM',n,0.0_real64,q,p,committed,backend,col,tmpl,committed_steps,completed)
      if(.not.completed) return
      call run_segment(label,'TOP',n,q,0.0_real64,p,committed,backend,col,tmpl,committed_steps,completed)
      if(.not.completed) return
      call emit_candidate(label,'BOTTOM_THEN_TOP',n,committed_steps,committed)
    end if
  end subroutine run_split

  subroutine run_segment(label,phase,nsteps,qtop,qbottom,p,committed,backend,col,tmpl,committed_steps,completed)
    ...
    do phase_step=1,nsteps
      ...
      if(result%status/=KERNEL_REFERENCE_FLOOR_STATUS_OK .or. .not.result%sample_valid .or. .not.candidate%ready() .or. &
        ...
        write(*,'(*(g0))') 'RZM06E03B_STOP|FAMILY=',trim(label),'|PHASE=',trim(phase), &
             '|PHASE_STEP=',phase_step,'|COMMITTED_STEPS=',committed_steps,'|STATUS=',result%status, &
  ...
  subroutine emit_candidate(label,order,n,steps,committed)
    character(len=*),intent(in) :: label,order
    ...
    class is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'RZM06E03B_CANDIDATE|FAMILY=',trim(label),'|ORDER=',trim(order), &
           '|N=',n,'|STEPS=',steps,'|T=',tcur
      do i=1,numnod
        write(*,'(*(g0))') 'RZM06E03B_NODE|FAMILY=',trim(label),'|NODE=',i, &
  ...
  subroutine initialize_state(p,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'E03B finite B01 seed')
  ...
  subroutine initialize_identity(col,tmpl)
    ...
    tmpl%template_id=960704_int64;tmpl%physics_topology_id=960705_int64
    tmpl%vertical_layout_id=960706_int64;tmpl%state_layout_id=960707_int64
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)')'GC_RZM06E03B_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_gc_rootzone_memory_rzm06e03b_expand
