program test_gc_rootzone_memory_rzm06e09_expand
  ...
  integer, parameter :: step_grid(2)=[1,2]
  integer, parameter :: upper_deep_node=15
  integer, parameter :: lower_deep_node=16
  integer(int64), parameter :: lineage_base=960709_int64
  ...
  call require(numnod==16,'E09 geometry node count')
  call require(all(abs(dz-10.0_real64)<=1.0e-14_real64),'E09 geometry dz')
  call require(same_bits(dt,3435974.0_real64/4294967296.0_real64),'E09 dyadic dt')
  call initialize_parameters(parameters)
  call initialize_state(parameters,h0,k0,initial_state)
  qeq=-k0
  call initialize_identity(column,template)

  write(*,'(*(g0))') 'RZM06E09_CONFIG|N=',numnod,'|DZ_CM=',dz(1),'|DT_DAY=',dt, &
  ...
  do fi=1,size(fractions)
    do si=1,size(step_grid)
      call run_family('LOCAL_UP',fi,fractions(fi),step_grid(si),upper_deep_node,lower_deep_node, &
           parameters,initial_state,column,template,qeq,lineage_base+int((fi-1)*8+(si-1)*2+1,int64))
      call run_family('LOCAL_DOWN',fi,fractions(fi),step_grid(si),lower_deep_node,upper_deep_node, &
           parameters,initial_state,column,template,qeq,lineage_base+int((fi-1)*8+(si-1)*2+2,int64))
    end do
  end do

  write(*,'(A)') 'GC_RZM06E09_DEEP_INTERNAL_REDISTRIBUTION=PASS'
...
contains
  ...
  subroutine run_control(p,origin,col,tmpl,qeq_in)
    ...
    call require(ok.and.committed%ready(),'E09 control seed')
    ...
    call require(completed.and.accepted==1,'E09 equilibrium control accepted')
    write(*,'(A)') 'GC_RZM06E09_EQUILIBRIUM_CONTROL=PASS'
  ...
  subroutine run_segment(direction,rate_index,fraction,nsteps,forcing,p,committed,backend,col,tmpl,accepted,completed)
    ...
    do step=1,nsteps
      ...
      if(result%status/=KERNEL_REFERENCE_FLOOR_STATUS_OK .or. .not.result%sample_valid .or. .not.candidate%ready() .or. &
        ...
        write(*,'(*(g0))') 'RZM06E09_STOP|DIRECTION=',trim(direction),'|RATE_INDEX=',rate_index, &
  ...
  subroutine emit_candidate(direction,rate_index,fraction,nsteps,transfer_rate,committed)
    ...
    class is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'RZM06E09_CANDIDATE|DIRECTION=',trim(direction),'|RATE_INDEX=',rate_index, &
           '|FRACTION=',fraction,'|STEPS=',nsteps,'|TRANSFER_RATE=',transfer_rate,'|T=',tcur
      do i=1,numnod
        write(*,'(*(g0))') 'RZM06E09_NODE|DIRECTION=',trim(direction),'|RATE_INDEX=',rate_index, &
  ...
  subroutine initialize_parameters(p)
    ...
    p%parameter_set_id=960709_int64;p%active_nodes=numnod
  ...
  subroutine initialize_state(p,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'E09 finite B01 seed')
  ...
  subroutine initialize_identity(col,tmpl)
    ...
    tmpl%template_id=960709_int64;tmpl%physics_topology_id=960710_int64
    tmpl%vertical_layout_id=960711_int64;tmpl%state_layout_id=960712_int64
    tmpl%solver_interface_id=960713_int64
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)')'GC_RZM06E09_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_gc_rootzone_memory_rzm06e09_expand
