...
NODES=16
DZ_CM=10.0
LADDER=[1e-9,1e-10,1e-11,1e-12,1e-13,1e-14]
H16_MIN=1e-2
...
def parse(path:Path):
    ...
    for line in raw.decode("utf-8").splitlines():
        if line.startswith("RZM06E09_CANDIDATE|"):
        ...
        elif line.startswith("RZM06E09_NODE|"):
        ...
        elif line.startswith("RZM06E09_STOP|"):
            stops.append(line)
    records={}
...
def main():
    ...
    assert raw0==raw2,"E09 O0/O2 output drift"
    ...
    for rate_index in range(1,7):
        for steps in (1,2,4,8):
            ku=("LOCAL_UP",rate_index,steps);kd=("LOCAL_DOWN",rate_index,steps)
    ...
    assert twins,"E09 no complete direction twins"
    ...
    if exact:
        decision="SELECTED_EXACT_LOCAL_REDISTRIBUTION_PAIR";mode="EXACT";sel=exact[0]
    elif tightest is not None:
        decision="SELECTED_TIGHT_LOCAL_REDISTRIBUTION_PAIR";mode=tightest[0];sel=tightest[1]
    else:
        decision="NO_MATCH";mode=None;sel=None

    evidence={
      "schema":"swap5.gc_rootzone_memory.rzm06e09.near_interface_local_redistribution_selection.v1",
      "preregistration_commit":"0c13b1fc1e003f5e494d9f7ef7ec9f91b980ed4b",
      ...
      "decision":decision,
      "nonclaims":["E09 performs no fixed-Hc response probe",
    ...
    print("RZM06E09_SELECTION_JSON",json.dumps(evidence,sort_keys=True,separators=(",",":")))
    print("GC_RZM06E09_RESPONSE_BLIND_SELECTION=PASS")
    return 0
