program test_gc_rootzone_memory_rzm06e10_probe
  ...
  real(real64), parameter :: w_tol_cm=1.0e-12_real64
  real(real64), parameter :: root30_tol_cm=1.0e-12_real64
  real(real64), parameter :: h16_min_cm=1.0e-2_real64
  real(real64), parameter :: response_threshold_cm=1.0e-18_real64
  integer(int64), parameter :: lineage_id=960710_int64
  ...
  call require(numnod==16,'E10 geometry node count')
  call require(all(abs(dz-10.0_real64)<=1.0e-14_real64),'E10 geometry dz')
  call require(same_bits(probe_dt,3435974.0_real64/4294967296.0_real64),'E10 dyadic dt')

  call get_command_argument(1,path_a)
  call get_command_argument(2,path_b)
  call require(len_trim(path_a)>0.and.len_trim(path_b)>0,'E10 origin paths')
  ...
  call require(abs(wb-wa)<=w_tol_cm,'E10 frozen profile-water gate')
  call require(abs(ub-ua)<=root30_tol_cm,'E10 frozen root30-water gate')
  call require(abs(h16b-h16a)>=h16_min_cm,'E10 frozen H16 gate')
  write(*,'(*(g0))') 'RZM06E10_RECON|WA=',wa,'|WB=',wb,'|ABS_DW=',abs(wb-wa), &
  ...
  datum%datum_id=960710_int64
  datum%bottom_boundary_elevation_m=bottom_z_m
  call materializer%initialize(base_forcing)
  call require(materializer%profile_admitted(parameters),'E10 mode5 profile admitted')
  call materializer%materialize(hc_m,datum,forcing_poly,forcing_status)
  call require(forcing_status==GW_SWAP_FORCING_OK.and.allocated(forcing_poly),'E10 fixed Hc materialized')
  select type(f=>forcing_poly)
  type is(fmr_b110_physical_forcing_t)
    forcing=f
  class default
    call require(.false.,'E10 materialized forcing type')
  end select
  call require(same_bits(forcing%top_flux,0.0_real64),'E10 zero top flux')
  call require(abs(forcing%bottom_head-expected_bottom_head_cm)<=1.0e-12_real64,'E10 Hc pressure mapping')
  write(*,'(*(g0))') 'RZM06E10_FORCING|HC_M=',hc_m,'|BOTTOM_Z_M=',bottom_z_m, &
  ...
  call require(probe_same(a1,a2),'E10 A order independence')
  call require(probe_same(b1,b2),'E10 B order independence')
  write(*,'(A)') 'GC_RZM06E10_ORDER_INDEPENDENCE=PASS'
  ...
  if(a1%admitted.and.b1%admitted)then
    ...
    write(*,'(*(g0))') 'RZM06E10_PROBE|ADMITTED_A=T|ADMITTED_B=T|A_STATUS=',a1%result_status, &
    ...
    write(*,'(*(g0))') 'RZM06E10_PROBE|ADMITTED_A=',a1%admitted,'|ADMITTED_B=',b1%admitted, &
  ...
  write(*,'(A)') 'GC_RZM06E10_FIXED_HC_PROBE_EXECUTION=PASS'
...
contains
  ...
  subroutine run_probe(origin,p,f,col,tmpl,obs)
    ...
    call require(ok.and.committed%ready(),'E10 fresh committed origin')
    call committed%snapshot(before,available)
    call require(available.and.snapshot_matches_state(before,origin),'E10 reconstructed committed state bit identity')
    ...
    call require(available.and.state_bits_equal(before,after),'E10 read-only origin after probe')
    call require(committed%current_revision()==0_int64,'E10 probe does not commit')
    if(candidate%ready())then
      call backend%discard_reference_floor_candidate(candidate,diag)
      call require(.not.candidate%ready(),'E10 discard candidate')
    end if
  ...
  subroutine initialize_parameters(p)
    ...
    p%parameter_set_id=960710_int64;p%active_nodes=numnod
  ...
  subroutine initialize_identity(col,tmpl)
    ...
    tmpl%template_id=960710_int64;tmpl%physics_topology_id=960703_int64
    tmpl%vertical_layout_id=960710_int64;tmpl%state_layout_id=960705_int64
    tmpl%solver_interface_id=960710_int64
  ...
  subroutine read_origin(path,state)
    ...
    call require(ios==0,'E10 open origin')
    do i=1,numnod
      read(unit,*,iostat=ios) node,hh,tt
      call require(ios==0.and.node==i,'E10 read origin node')
      call require(ieee_is_finite(hh).and.ieee_is_finite(tt),'E10 finite origin')
  ...
  subroutine state_observables(state,w,upper,h16)
    ...
    call require(w>0.0_real64,'E10 positive storage')
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'GC_RZM06E10_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_gc_rootzone_memory_rzm06e10_probe
