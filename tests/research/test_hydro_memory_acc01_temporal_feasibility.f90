program test_hydro_memory_acc01_temporal_feasibility
  ...
  real(real64), parameter :: DURATION_DAY = 1.0e-1_real64
  ...
  real(real64) :: qref, max_head_change, max_head_diff, max_theta_diff
  ...
  call require(root_result%status == CANONICAL_STATUS_COMPLETED .and. root_result%completed, &
       'governed ROOT transaction did not complete')
  call require(root_candidate%ready(), 'governed ROOT candidate not ready')
  call require(root_result%mass%complete, 'governed ROOT mass incomplete')
  call require(abs(root_result%mass%residual) <= MASS_TOL, 'governed ROOT hard mass gate')

  call require(generic_result%status == CANONICAL_STATUS_COMPLETED .and. generic_result%completed, &
       'governed GENERIC control did not complete')
  call require(generic_candidate%ready(), 'governed GENERIC candidate not ready')
  call require(generic_result%mass%complete, 'governed GENERIC mass incomplete')
  call require(abs(generic_result%mass%residual) <= MASS_TOL, 'governed GENERIC hard mass gate')

  call inspect_root_state(root_candidate, max_head_change)
  call require(max_head_change > 0.0_real64 .and. ieee_is_finite(max_head_change), &
       'governed ROOT trajectory did not change hydraulic state')

  call compare_candidate_states(root_candidate, generic_candidate, max_head_diff, max_theta_diff)
  call require(same_scaled(max_head_diff, 0.0_real64), 'ROOT/GENERIC final head divergence')
  call require(same_scaled(max_theta_diff, 0.0_real64), 'ROOT/GENERIC final theta divergence')

  call require(root_diag%retries == generic_diag%retries, 'ROOT/GENERIC retry-count divergence')
  call require(root_diag%solver_rejections == generic_diag%solver_rejections, 'ROOT/GENERIC solver-rejection divergence')
  call require(root_diag%temporal_rejections == generic_diag%temporal_rejections, 'ROOT/GENERIC temporal-rejection divergence')
  call require(root_diag%mass_rejections == generic_diag%mass_rejections, 'ROOT/GENERIC mass-rejection divergence')
  ...
  call require(same_scaled(root_obs%temporal_head_budget, H_APP_CM*A_TEMPORAL), &
       'ROOT runtime temporal budget drift')
  call require(same_scaled(generic_obs%temporal_head_budget, H_APP_CM*A_TEMPORAL), &
       'GENERIC runtime temporal budget drift')

  write(*,'(a,es24.16)') 'HYDRO_MEMORY_F1_QREF=', qref
  write(*,'(a,es24.16)') 'HYDRO_MEMORY_F1_H_TEMPORAL_CM=', H_APP_CM*A_TEMPORAL
  write(*,'(a,es24.16)') 'HYDRO_MEMORY_F1_ROOT_MAX_HEAD_CHANGE_CM=', max_head_change
  write(*,'(a,i0)') 'HYDRO_MEMORY_F1_RETRIES=', root_diag%retries
  write(*,'(a,i0)') 'HYDRO_MEMORY_F1_SOLVER_REJECTIONS=', root_diag%solver_rejections
  write(*,'(a,i0)') 'HYDRO_MEMORY_F1_TEMPORAL_REJECTIONS=', root_diag%temporal_rejections
  write(*,'(a,es24.16)') 'HYDRO_MEMORY_F1_ROOT_GENERIC_MAX_HEAD_DIFF_CM=', max_head_diff
  write(*,'(a,es24.16)') 'HYDRO_MEMORY_F1_ROOT_GENERIC_MAX_THETA_DIFF=', max_theta_diff
  write(*,'(a)') 'HYDRO_MEMORY_ACC01_F1_GOVERNED_TEMPORAL_FEASIBILITY=PASS'

contains

  subroutine run_route(root_route, result, candidate, diagnostics, observation)
    ...
    call initialize_temporal_committed_state(committed, previous_derivative)
    call fmr_capture_checkpoint(committed, checkpoint, ok)
    call require(ok, 'F1 checkpoint capture')

    config = numerical_config()
    accuracy = governed_accuracy()
    call accuracy%materialize_model_temporal_budget(config, materialized)
    call require(materialized, 'F1 governed temporal budget materialization')
    call require(config%model_temporal_indicator_budget_available, 'F1 governed temporal budget availability')
    call require(same_scaled(config%model_temporal_indicator_budget, H_APP_CM*A_TEMPORAL), &
         'F1 governed temporal budget value')

    call backend%initialize(top)
    call backend%run_trial(column, template, parameters, committed, forcing, config, &
         0.0_real64, DURATION_DAY, checkpoint, result, candidate, diagnostics)
    observation = backend%observation()
  end subroutine run_route

  function governed_accuracy() result(accuracy)
    type(coupling_application_accuracy_contract_t) :: accuracy
    accuracy%contract_id = 590200_int64
    accuracy%contract_version = 1
    accuracy%qoi_kind = COUPLING_QOI_GROUNDWATER_HEAD
    accuracy%h_app_available = .true.
    accuracy%h_app_cm = H_APP_CM
    accuracy%h_app_externally_qualified = .true.
    accuracy%application_provenance_id = 590201_int64
    accuracy%a_temporal_available = .true.
    accuracy%a_temporal = A_TEMPORAL
    accuracy%a_temporal_externally_qualified = .true.
    accuracy%temporal_allocation_provenance_id = 590202_int64
  end function governed_accuracy
  ...
  subroutine derive_equilibrium_flux(provider, flux)
    ...
    flux = -conductivity(1)
    call require(ieee_is_finite(flux) .and. flux < 0.0_real64, 'F1 equilibrium flux')
  ...
  subroutine initialize_temporal_committed_state(state, previous_derivative)
    ...
    call require(ok, 'F1 temporal committed state initialization')
  end subroutine initialize_temporal_committed_state

  subroutine inspect_root_state(candidate, max_head_change)
    type(kernel_candidate_state_t), intent(in) :: candidate
    real(real64), intent(out) :: max_head_change
    class(transaction_state_t), allocatable :: snapshot
    logical :: ok

    call candidate%snapshot(snapshot, ok)
    call require(ok .and. allocated(snapshot), 'F1 ROOT snapshot')
    select type (typed => snapshot)
    class is (fmr_b110_physical_state_t)
      max_head_change = maxval(abs(typed%pressure_head-H0_CM))
    class default
      call require(.false., 'F1 ROOT snapshot type')
    end select
  end subroutine inspect_root_state
  ...
  subroutine compare_candidate_states(a, b, max_head_diff, max_theta_diff)
    ...
    call require(oka .and. okb .and. allocated(sa) .and. allocated(sb), 'F1 candidate snapshots')
    select type (ta => sa)
    class is (fmr_b110_physical_state_t)
      select type (tb => sb)
      class is (fmr_b110_physical_state_t)
      ...
      class default
        call require(.false., 'F1 GENERIC snapshot type')
      end select
    class default
  ...
  subroutine require(condition, label)
    ...
    if (.not. condition) then
      write(*,'(a,1x,a)') 'HYDRO_MEMORY_F1_FAIL', trim(label)
      error stop 1
    end if
  end subroutine require

end program test_hydro_memory_acc01_temporal_feasibility
