...
def bind_hydro_memory_api(swap:Fgc44RealSwap):
    ...
    next_window=swap.lib.hydro_memory_acc02_f2_begin_next_window_c
    next_window.restype=ctypes.c_int
    next_window.argtypes=[
        ctypes.c_double,
        ctypes.POINTER(ctypes.c_double),ctypes.POINTER(ctypes.c_double),ctypes.POINTER(ctypes.c_double),
    ]
    return accuracy,policy,next_window
...
def build_model(workdir:Path, reference_head:float)->None:
    sim=flopy.mf6.MFSimulation(sim_name="HYDRO_MEMORY_ACC02_F2",version="mf6",sim_ws=str(workdir))
...
def main()->None:
    ...
    accuracy_fn,policy_fn,next_window_fn=bind_hydro_memory_api(swap)
    ...
    with tempfile.TemporaryDirectory(prefix="hydro-memory-acc02-f2-") as tmp:
        workdir=Path(tmp)
        ...
        try:
            ...
            for window_index in range(1,WINDOW_COUNT+1):
                ...
                for outer in range(1,MAX_COUPLING_ITERATIONS+1):
                    ...
                    print(
                        f"HYDRO_MEMORY_ACC02_F2_ITER window={window_index} iter={outer} "
                ...
                print(
                    f"HYDRO_MEMORY_ACC02_F2_WINDOW={window_index} "
                ...
                if window_index<WINDOW_COUNT:
                    ...
                    require(abs(root_total2-ROOT_TOTAL_CM_PER_DAY)<=1e-15 and root_coverage2 and endpoint_authoritative2,
            ...
            print(f"HYDRO_MEMORY_ACC02_F2_H_APP_CM={h_app_cm:.17g}")
            print(f"HYDRO_MEMORY_ACC02_F2_TEMPORAL_BUDGET_CM={temporal_budget_cm:.17g}")
            print(f"HYDRO_MEMORY_ACC02_F2_INTERFACE_TOLERANCE_M={interface_tol_m:.17g}")
            print(f"HYDRO_MEMORY_ACC02_F2_ROOT_TOTAL_CM_PER_DAY={root_total:.17g}")
            print(f"HYDRO_MEMORY_ACC02_F2_MAX_TEMPORAL_INDICATOR={max(m['max_temporal_indicator'] for m in window_metrics):.17g}")
            print(f"HYDRO_MEMORY_ACC02_F2_MAX_HEAD_RESIDUAL_M={max(abs(m['head_residual']) for m in window_metrics):.17g}")
            print(f"HYDRO_MEMORY_ACC02_F2_MAX_FLUX_RESIDUAL={max(abs(m['flux_residual']) for m in window_metrics):.17g}")
            print(f"HYDRO_MEMORY_ACC02_F2_FINAL_REVISION={window_metrics[-1]['window']}")
            print(f"HYDRO_MEMORY_ACC02_F2_FINAL_TIME_DAY={window_metrics[-1]['mf_time']:.17g}")
            print(f"HYDRO_MEMORY_ACC02_F2_FINAL_LEDGER_EXCHANGE_M={window_metrics[-1]['ledger']:.17g}")
            print("HYDRO_MEMORY_ACC02_F2_PERSISTENT_SWAPP_STATE=PASS")
            print("HYDRO_MEMORY_ACC02_F2_DYNAMIC_PREDICTOR_ORIGIN=PASS")
            print("HYDRO_MEMORY_ACC02_F2_REVISION_TIME_LEDGER_SEQUENCE=PASS")
            print("HYDRO_MEMORY_ACC02_F2_GOVERNED_ACCURACY=PASS")
            print("HYDRO_MEMORY_ACC02_F2_LIVE_MODFLOW680=PASS")
            print("HYDRO_MEMORY_ACC02_F2_EXACT_ONCE_PUBLICATION=PASS")
            print("HYDRO_MEMORY_ACC02_F2_GATE=PASS")
        finally:
            if initialized:
                try:
                    raw.finalize()
