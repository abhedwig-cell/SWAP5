...
def build_model(request_depth_cm: float) -> Model:
    ...
    source = model.flow_boundary.add(
        Node(1, Point(-1.0, 0.0), subnetwork_id=2, name="rm08_supply"),
        [flow_boundary.Static(flow_rate=[SUPPLY_M3_PER_DAY / DAY])],
    )
    store = model.basin.add(
        Node(2, Point(0.0, 0.0), subnetwork_id=2, name="rm08_store"),
        [
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: generate_rm08_real_ribasim.py <output-dir> <request-depth-cm>"
        )
    output = Path(sys.argv[1]).resolve()
    request_depth_cm = float(sys.argv[2])
    if not (request_depth_cm > 0.0):
        raise SystemExit("RM08 requires a positive SWAP management request")
    if output.exists():
    ...
    print(
        f"RM08_MODEL_GENERATED path={model_path} "
