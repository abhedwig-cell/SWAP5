...
def build_model() -> Model:
    model = Model(
        ...
        crs="EPSG:28992",
        allocation=Allocation(dt=18000.0),
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: generate_rm12_real_ribasim.py <output-dir>")
    out = Path(sys.argv[1]).resolve()
    if out.exists():
        shutil.rmtree(out)
    out.mkdir(parents=True)
    model_path = out / "ribasim.toml"
    build_model().write(model_path)
    print(f"RM12_MODEL_GENERATED path={model_path}")
