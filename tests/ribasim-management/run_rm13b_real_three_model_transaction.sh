...
cd "$ROOT"
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-rm13b-triangle-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/modflow-bin" "$BUILD/downloads" "$BUILD/bridge"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "RM13B_FAIL $*" >&2; exit 1; }
...
MODEL_DIR="$RIBASIM_ROOT/generated_testmodels/swap5_rm13b"
(
  cd "$RIBASIM_ROOT"
  pixi run python "$ROOT/tests/ribasim-management/generate_rm13b_real_ribasim.py" "$MODEL_DIR"
)
...
echo 'RM13B_REAL_THREE_MODEL_TRANSACTION_GATE=PASS'
