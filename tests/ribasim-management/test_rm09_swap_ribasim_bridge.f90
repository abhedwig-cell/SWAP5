program test_rm09_swap_ribasim_bridge
  ...
  integer(int64), parameter :: lineage = 9009_int64
  integer(int64), parameter :: crop_revision = 102_int64
  ...
  if (len_trim(mode)==0) error stop 'usage: test_rm09_swap_ribasim_bridge request|consume [allocated_cm supplied_cm]'
  ...
  case('request')
    write(*,'(A,ES26.17E3)') 'RM09_SWAP_REQUEST_DEPTH_CM=',receipt%requested_depth_cm()
    write(*,'(A,I0)') 'RM09_SWAP_ORIGIN_LINEAGE=',receipt%origin_lineage_id()
    write(*,'(A,I0)') 'RM09_SWAP_ORIGIN_REVISION=',receipt%origin_revision()
    write(*,'(A)') 'RM09_SWAP_READ_ONLY_REQUEST=PASS'
  ...
  case('consume')
    ...
    write(*,'(A,ES26.17E3)') 'RM09_CONSUMED_REQUEST_DEPTH_CM=',ob%requested_depth_cm
    write(*,'(A,ES26.17E3)') 'RM09_CONSUMED_ALLOCATED_DEPTH_CM=',ob%allocated_depth_cm
    write(*,'(A,ES26.17E3)') 'RM09_CONSUMED_SUPPLIED_DEPTH_CM=',ob%supplied_depth_cm
    write(*,'(A,ES26.17E3)') 'RM09_CONSUMED_NET_IRRIGATION_CM=',ob%net_surface_irrigation_amount_cm
    write(*,'(A)') 'RM09_SWAP_CANDIDATE_REPLAY=PASS'
    write(*,'(A)') 'RM09_ALLOCATION_SUPPLY_BOUND_TO_SAME_ORIGIN=PASS'
    write(*,'(A)') 'RM09_SWAP_EXACTLY_ONCE_MANAGEMENT_COMMIT=PASS'
    write(*,'(A)') 'RM09 SWAP SIDE REAL RIBASIM RECEIPT CONSUMPTION PASS'

  case default
    error stop 'unknown RM09 mode'
  end select
...
contains
  ...
  subroutine require(condition,message)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)')'RM09_FAIL',trim(message)
      error stop 1
    end if
  end subroutine require

end program test_rm09_swap_ribasim_bridge
