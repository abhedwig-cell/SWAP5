program test_sw_rib_pa01_external_owner_admission
  ...
  call verify_missing_external_head_rejected()
  call verify_fixed_weir_owner_conflict_rejected()
  call verify_invalid_process_rolls_back()
  write(*,'(A)') 'SW_RIB_PA01_EXTERNAL_OWNER_ADMISSION=PASS'
...
contains
  ...
  subroutine verify_signed_commit(control_head, balancing_qssdi, positive)
    ...
    if (positive) then
      ...
      write(*,'(A)') 'SW_RIB_PA01_POSITIVE_DRAINAGE_SINGLE_BOOKING=PASS'
    else
      ...
      write(*,'(A)') 'SW_RIB_PA01_NEGATIVE_INFILTRATION_SINGLE_BOOKING=PASS'
    end if
    ...
    write(*,'(A)') 'SW_RIB_PA01_SIGNED_HARD_MASS_CLOSURE=PASS'
  end subroutine verify_signed_commit

  subroutine verify_missing_external_head_rejected()
    type(fmr_serialized_reference_backend_t) :: backend
    type(kernel_executor_t) :: tx_control
    type(kernel_committed_state_t) :: committed
    type(fmr_logical_column_t) :: column
    ...
    forcing%drainage_response_controls(1)%resolved_surface_water_head_supplied = .false.
    ...
    call require(.not. output%solver_executed, 'missing external head reached physical solve')
    call require(.not. output%committed, 'missing external head unexpectedly committed')
    call require(output%admission_assessed .and. .not. output%admitted, 'missing external head admission state')
    call require(trim(output%admission_status) == 'EXT_SURFACE_OWNER_REJECTED', 'missing external head admission status')
    call require(trim(diagnostic%failure_classification) == 'EXT_SURFACE_OWNER_REJECTED', &
         'missing external head diagnostic classification')
    call require(committed%current_revision() == 0_int64, 'missing external head mutated revision')
    call snapshot_groundwater_level(committed, gwl_after)
    call require(same_bits(gwl_after, initial_gwl), 'missing external head mutated committed state')
    call require(active_calls == 0, 'missing external head physical call counter')
    write(*,'(A)') 'SW_RIB_PA01_MISSING_EXTERNAL_HEAD_FAIL_CLOSED=PASS'
  end subroutine verify_missing_external_head_rejected

  subroutine verify_fixed_weir_owner_conflict_rejected()
    type(fmr_serialized_reference_backend_t) :: backend
    type(kernel_executor_t) :: tx_control
    type(kernel_committed_state_t) :: committed
    type(fmr_logical_column_t) :: column
    ...
    call require(.not. output%solver_executed, 'owner conflict reached physical solve')
    call require(.not. output%committed, 'owner conflict unexpectedly committed')
    call require(output%admission_assessed .and. .not. output%admitted, 'owner conflict admission state')
    call require(trim(output%admission_status) == 'EXT_SURFACE_OWNER_REJECTED', 'owner conflict admission status')
    call require(trim(diagnostic%failure_classification) == 'EXT_SURFACE_OWNER_REJECTED', &
         'owner conflict diagnostic classification')
    call require(committed%current_revision() == 0_int64, 'owner conflict mutated revision')
    call snapshot_groundwater_level(committed, gwl_after)
    call require(same_bits(gwl_after, initial_gwl), 'owner conflict mutated committed state')
    call require(active_calls == 0, 'owner conflict physical call counter')
    write(*,'(A)') 'SW_RIB_PA01_FIXED_WEIR_OWNER_CONFLICT_FAIL_CLOSED=PASS'
  end subroutine verify_fixed_weir_owner_conflict_rejected
  ...
  subroutine verify_invalid_process_rolls_back()
    ...
    write(*,'(A)') 'SW_RIB_PA01_INVALID_PROCESS_ROLLBACK=PASS'
  ...
  subroutine initialize_case(committed, column, template, parameters, forcing, config, control_head, balancing_qssdi)
    ...
    forcing%drainage_response_controls(1)%resolved_surface_water_head_supplied = .true.
    forcing%drainage_response_controls(1)%resolved_surface_water_head_cm = control_head
  ...
  subroutine require(condition,label)
    ...
    if (.not. condition) then
      write(*,'(A,1X,A)') 'SW_RIB_PA01_TEST_FAIL', trim(label)
      error stop 1
    end if
  end subroutine require

end program test_sw_rib_pa01_external_owner_admission
