...
def main():
    ...
    for line in text.splitlines():
        if "F_ROM0R_R3D2_CLASS|" in line:
            r=fields(line.split("F_ROM0R_R3D2_CLASS|",1)[1]); classes[r["CASE"]]=r
        elif "F_ROM0R_R3D2_BOUND|" in line:
            r=fields(line.split("F_ROM0R_R3D2_BOUND|",1)[1]); bounds[r["CASE"]]=r
    ...
    if not complete:
        decision="B01_REPRESENTATION_BOUND_DIAGNOSTIC_BLOCKED"
    elif all(positive):
        decision="B01_TOTAL_ONLY_WITHIN_PRIOR_REPRESENTATION_BOUND"
    elif any(positive):
        decision="B01_REPRESENTATION_BOUND_DIAGNOSTIC_NONUNIFORM"
    else:
        decision="B01_TOTAL_ONLY_EXCEEDS_PRIOR_REPRESENTATION_BOUND"
    result={
      "schema":"swap5.f-rom0r.r3d2-result.v1",
      "work_unit":"ROM-0R-R3D2",
      "decision":decision,
    ...
    if decision=="B01_REPRESENTATION_BOUND_DIAGNOSTIC_BLOCKED" or not repeat_identity:
        return 2
    return 0
