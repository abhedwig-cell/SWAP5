...
def parse(path):
    ...
    for line in pathlib.Path(path).read_text().splitlines():
        if line.startswith("F_ROMV2_D3_GEOMETRY|"):
            geometry=fields(line.split("|",1)[1])
        elif "F_ROMV2_D3_STATE|" in line:
            r=fields(line.split("F_ROMV2_D3_STATE|",1)[1]); states[(r["HISTORY"],int(r["STEP"]))]=r
        elif "F_ROMV2_D3_NODE|" in line:
            r=fields(line.split("F_ROMV2_D3_NODE|",1)[1]); nodes[(r["HISTORY"],int(r["STEP"]))][int(r["NODE"])]=r
...
def main():
    ...
    for g in ("R8","R4","R2"):
        ...
        results[g]={
          ...
          "computational_proxies":{"node_count":run["n"],"total_nonlinear_iterations":nl,
                                   ...
                                   "total_backtracking_attempts":back,"representation_aware_reattempt_count":fb}
        }
    ...
    strongly_coarse=[g for g in ("R4","R2") if g in results and integrity[g]["pass"]]
    decision="STRONGLY_COARSE_RICHARDS_HYDROLOGICALLY_MEASURABLE_UNDER_PURPOSE_APPROPRIATE_NUMERICAL_AUTHORITY" if strongly_coarse else "STRONGLY_COARSE_RICHARDS_REMAINS_FAIL_CLOSED_AFTER_REPRESENTATION_AWARE_AUTHORITY"
    out={
      "schema":"swap5.f-romv2-d3.result.v1","workstream":"F-ROM","work_unit":"F-ROMV2-D3",
      "decision":decision,"blind_confirmation":False,"application_acceptance":False,
      "strongly_coarse_complete":strongly_coarse,
      "reference":{"id":"R16","nodes":16,"integrity":integrity["R16"],
                   "computational_proxies":{"total_nonlinear_iterations":ref_nl,
                                            ...
                                            "representation_aware_reattempt_count":ref_fb}},
      "candidates":results,
      ...
      "interpretation_rule":"Representation-aware numerical admissibility plus multi-objective development frontier only; no weighted score and no application threshold is inferred from D3.",
