...
def parse(path):
    ...
    for line in pathlib.Path(path).read_text().splitlines():
        if line.startswith("F_ROMV2_D4_GEOMETRY|"):
            geometry=fields(line.split("|",1)[1])
        elif "F_ROMV2_D4_STATE|" in line:
            r=fields(line.split("F_ROMV2_D4_STATE|",1)[1]); states[(r["HISTORY"],int(r["STEP"]))]=r
        elif "F_ROMV2_D4_NODE|" in line:
            r=fields(line.split("F_ROMV2_D4_NODE|",1)[1]); nodes[(r["HISTORY"],int(r["STEP"]))][int(r["NODE"])]=r
...
def main():
    ...
    for g in ("R8","R4","R2"):
        ...
        results[g]={
          ...
          "computational_proxies":{"node_count":run["n"],"total_nonlinear_iterations":nl,
                                   ...
                                   "total_backtracking_attempts":back,"integrated_policy_reattempt_count":fb}
        }
    ...
    decision="STRONGLY_COARSE_RICHARDS_HYDROLOGICALLY_MEASURABLE_UNDER_INTEGRATED_MASS_POLICY" if strongly_coarse else "STRONGLY_COARSE_RICHARDS_FAILS_INTEGRATED_MASS_POLICY"
    out={
      "schema":"swap5.f-romv2-d4.result.v1","workstream":"F-ROM","work_unit":"F-ROMV2-D4",
      ...
      "reference":{"id":"R16","nodes":16,"integrity":integrity["R16"],
                   "computational_proxies":{"total_nonlinear_iterations":ref_nl,
                                            ...
                                            "integrated_policy_reattempt_count":ref_fb}},
      "candidates":results,
