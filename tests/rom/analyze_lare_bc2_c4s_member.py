...
def main():
    ...
    a=ap.parse_args()
    dz=[float(x) for x in a.thickness.split(",")]
    if len(dz)!=a.dimension or abs(sum(dz)-160.0)>1e-12:
        raise SystemExit("C4S dimension/thickness drift")
    manifest=json.loads((a.evidence_dir/"execution_manifest.json").read_text())
    if manifest["member"]!=a.member: raise SystemExit("C4S member drift")
    ...
    for h in HISTS:
        ...
        if status=="QUALIFIED":
            r0=o0.read_text(errors="replace"); r2=o2.read_text(errors="replace")
            ...
            valid=(sci_identity and state_count==64 and initial_count==1 and complete and history_count==1 and math.isfinite(mass) and mass<=1e-12 and fallbacks>=0)
            if not valid:
            ...
            rows[h]={
                "status":status,
                "O0_O2_identity":ident,
                "state_count":0,
                "initial_count":None,
                "history_count":None,
                "max_abs_mass_cm":None,
                "fallback_count":None,
                "failure_marker":f0
            }
        else:
    ...
    result={
      "schema":"swap5.lare.bc2.c4s.member-result.v1",
      "workstream":"F-ROM-LARE","work_unit":"LARE-BC2-C4S",
      "member":a.member,"dimension":a.dimension,"thickness_cm":dz,
    ...
    print(json.dumps({
      "member":a.member,"dimension":a.dimension,
      "qualified_histories":qualified,"fail_closed_histories":failed,
      "technical_failure_histories":technical,
      "maximum_qualified_mass_residual_cm":maxmass
    },sort_keys=True))
    return 0
