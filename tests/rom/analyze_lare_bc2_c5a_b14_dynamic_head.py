...
bc=load_module("bc1_c5a_base",HERE/"run_lare_bc1_stage_b.py")
...
def c5a_boundary_psi(history,sym,psi0):
...
bc.symbol=c5a_symbol
bc.boundary_psi=c5a_boundary_psi
# Matched constitutive transfer only. Histories and boundary forcing remain C4Z-identical.
bc.THETA_R=0.01
bc.THETA_S=0.416774
bc.ALPHA=0.00541
bc.N_VG=1.301528
...
def main():
    ...
    assert p["phase"]=="PREREGISTERED_BEFORE_C5A_B14_DYNAMIC_RESPONSE"
    ...
    assert p["representations"]["LARE"]["closure"]=="BC1 CURRENT_LAYER_FACE with B14 constitutive parameters only"
    assert p["matched_transfer"]["source_dynamic_head_authority"]=="integration/f-rom/LARE_BC2_C4Z_CLOSEOUT.json"
    ...
    if not integrity:
        decision="C5A_REFERENCE_OR_EXECUTION_BLOCKED"
    elif profile:
        decision="C5A_DYNAMIC_HEAD_GW_PROFILE_FRONTIERS_PRESENT"
    elif gw:
        decision="C5A_DYNAMIC_HEAD_GW_FRONTIER_ONLY"
    else:
        decision="C5A_DYNAMIC_HEAD_REDUCED_FRONTIER_NOT_PRESERVED"

    out={
      "schema":"swap5.lare.bc2.c5a.result.v1",
      "workstream":"F-ROM-LARE","work_unit":"LARE-BC2-C5A",
      "response_blind":True,
      "material_previously_exposed":True,
      "matched_cross_axis_transfer":True,
      "material":"B14",
      "decision":decision,
      "reference_workload":{
        "histories":HISTS,
      ...
      "hypotheses":{
        "H_R3_R2_RELATIVE_GW_TRANSFER":None if min_gw is None else min_gw==3,
        "H_LOWER_ZONE_PLACEMENT_R4":placement("R4","U4"),
        "H_LOWER_ZONE_PLACEMENT_R8":placement("R8","U8"),
        "H_R3_R2_RELATIVE_PROFILE_TRANSFER":None if min_profile is None else min_profile==3,
      },
      "interpretation_boundaries":[
        "Comparator-relative crossing against R2 is not application acceptance.",
        "C5A is a matched B14 transfer of the fixed-domain prescribed-head C4Z workload; it is not moving-water-table geometry and not groundwater-coupling authority.",
        "No FMC comparison is made because no pre-existing B14 dynamic prescribed-head FMC boundary authority exists.",
