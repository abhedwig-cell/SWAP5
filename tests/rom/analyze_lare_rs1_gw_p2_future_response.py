...
def pareto_noninferior(candidate: dict[str, float | int], control: dict[str, float | int]) -> dict[str, object]:
    numeric = (
        ...
        "terminal_flux_sign_mismatch_count",
...
def main() -> int:
    ...
    for representation in selection["representations"]:
        ...
        metrics = {
            ...
            "gw_r_mean_signed_pair_exchange_difference_cm": (
            ...
            "terminal_flux_sign_mismatch_count": sign_mismatches,
    ...
    for cid, row in representation_results.items():
        if not cid.startswith("D"):
            continue
        relative[cid] = {}
        for control_id in uniform_controls:
            if int(representation_results[control_id]["dimension"]) < int(row["dimension"]):
                continue
            relative[cid][control_id] = pareto_noninferior(
                row["metrics"], representation_results[control_id]["metrics"]
            )
    ...
    result = {
        ...
        "relative_to_uniform_controls": relative,
