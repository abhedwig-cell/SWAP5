...
refmod=load_module("rom_purpose_p1_reference_analysis",HERE/"analyze_rom_purpose_p1_reference.py")
...
def reversals(values):
    return refmod.reversals(values)
...
def gw_metrics(candidate:dict,reference:dict)->dict[str,float|int]:
    ...
    for h in refmod.GW_H:
        ...
        if len(ca)!=len(ra):
            seq+=1; timing=max(timing,refmod.NOBS)
        else:
...
def componentwise_relation(a:dict,b:dict):
    keys=tuple(a)
    if set(keys)!=set(b): raise ValueError("metric key drift")
...
def crosses(candidate:dict,comparator:dict):
    rel=componentwise_relation(candidate,comparator)
...
def main():
    ...
    rr=json.loads(a.reference_result.read_text())
    ...
    refs={}
    comparator={}
    for purpose,kind in (("SURF_P","surface"),("GW_LB","gw")):
        refs[purpose]={}
        comparator[purpose]={}
        for material in ("B01","B14"):
            target=(refmod.parse_surface if kind=="surface" else refmod.parse_gw)(
              a.reference_root/f"{kind}_{material}_R2048_T32_o0.txt","R2048_T32")
            r512=(refmod.parse_surface if kind=="surface" else refmod.parse_gw)(
              a.reference_root/f"{kind}_{material}_R512_T32_o0.txt","R512_T32")
            refs[purpose][material]=target
            comparator[purpose][material]=(
              reference_surface_metrics(r512,target) if purpose=="SURF_P"
              else reference_gw_metrics(r512,target)
            )
        ...
        for material in ("B01","B14"):
            ...
            for member in MEMBERS:
                ...
                if c["status"]=="QUALIFIED" and c["histories"] is not None:
                    ...
                    cross,cross_relation=crosses(metrics,comparator[purpose][material])
    ...
    for purpose in ("SURF_P","GW_LB"):
        ...
        for material in ("B01","B14"):
            ...
            if am["metrics"] is None or u["metrics"] is None:
                ...
                rel=componentwise_relation(am["metrics"],u["metrics"])
            ...
            if am["metrics"] is not None and op["metrics"] is not None:
                crossdiag=componentwise_relation(am["metrics"],op["metrics"])
            need_closure=(am["status"]=="QUALIFIED" and not am["crosses_R512_T32_numerical_comparator"])
            if need_closure:
                closure_followup.append({"purpose":purpose,"material":material,"partition":aligned[purpose]})
            placement[purpose][material]={
              "aligned_partition":aligned[purpose],"uniform_partition":"U4",
              "opposite_partition":opposite[purpose],
              "primary":primary,"cross_purpose_diagnostic":crossdiag,
    ...
    out={
      "schema":"swap5.rom-purpose.p1.placement-result.v1",
      "workstream":"ROM-PURPOSE","work_unit":"ROM-PURPOSE-P1-PLACEMENT",
      "reference_authority":rr["status"],
      "metric_contract":"integration/f-rom/ROM_PURPOSE_P1_METRIC_CONTRACT.json",
      "numerical_comparator":{"route":"R512_T32","target":"R2048_T32","metrics":comparator},
      "cases":cases,
      "placement":placement,
      "hypotheses":{
        "H1_groundwater_aligned_placement_supported":all(
          placement["GW_LB"][m]["primary"]["decision"]=="PLACEMENT_SUPPORTED" for m in ("B01","B14")),
        "H2_surface_aligned_placement_supported":all(
          placement["SURF_P"][m]["primary"]["decision"]=="PLACEMENT_SUPPORTED" for m in ("B01","B14")),
      ...
      "application_authority_status":"NOT_BOUND",
      "scientific_firewall":{
        "weighted_score_used":False,"application_acceptance_adjudicated":False,
    ...
    print(json.dumps({
      ...
      "placement":{p:{m:placement[p][m]["primary"]["decision"] for m in placement[p]} for p in placement}
    },sort_keys=True))
