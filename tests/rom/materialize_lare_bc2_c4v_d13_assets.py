...
"""
    ...
    if out.count("integer, parameter :: NHIST=4, NSTEPS=64") != 1:
        raise SystemExit("unexpected D13 Fortran NSTEPS structure")
    out=out.replace("integer, parameter :: NHIST=4, NSTEPS=64",
                    "integer, parameter :: NHIST=4, NSTEPS=1024",1)
...
def materialize_analyzer(src: pathlib.Path, dst: pathlib.Path):
    ...
    if out.count("NSTEPS=64") != 1:
        raise SystemExit("unexpected D13 analyzer NSTEPS structure")
    out=out.replace("NSTEPS=64","NSTEPS=1024",1)
    dst.write_text(out,encoding="utf-8")
...
def main():
    ...
    manifest={
      "schema":"swap5.lare.bc2.c4v.materialization.v1",
      ...
      "allowed_changes":{
        "fortran":["history labels G* -> V01..V04","lambda values -> 0.375,0.625,0.875,1.125","split label -> BLIND","NSTEPS 64 -> 1024"],
        "analyzer":["HISTS mapping G* -> V01..V04","NSTEPS 64 -> 1024"]
      }
    }
