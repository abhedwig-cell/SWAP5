...
def history_block(factor:int)->str:
    a=256*factor
    b=576*factor
    c=320*factor
    ...
    call require(symbol==SYM_HOLD.or.symbol==SYM_RISE.or.symbol==SYM_FALL,'LAREGW1 C5F valid boundary-only symbol')
...
def main()->int:
    ...
    ap.add_argument("--temporal-factor",required=True,type=int,choices=(2,4))
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        base_manifest=td/"c5e_manifest.json"
        subprocess.run([
    ...
    nsteps=1024*f
    dt=0.0008/f
    dt_literal={2:"0.0004_real64",4:"0.0002_real64"}[f]
    text=one(text,
    ...
    text=one(text,
      ...
      f"real(real64), parameter :: step_dt={dt_literal}",
      "main transaction interval")
    text=block(
      text,
      r"^  integer function history_symbol\(ih,step\) result\(symbol\).*?^  end function history_symbol\n",
      history_block(f),
      "physical-time matched history symbols")
    text=one(text,
      "write(*,'(A)') 'LAREGW1_C5E_B14_REFERENCE_EXTENSION_GENERATED=TRUE'",
      "write(*,'(A)') 'LAREGW1_C5F_B14_SPATIOTEMPORAL_GENERATED=TRUE'",
      "completion marker")

    a.output.write_text(text,encoding="utf-8")
    out={
      "schema":"swap5.lare.bc2.c5f.temporal-materialization.v1",
      "source_harness_sha256":sha256(a.source),
      "c5e_materializer_sha256":sha256(a.c5e_materializer),
      "temporal_factor":f,
      "main_step_dt_day":dt,
      "main_steps_per_history":nsteps,
