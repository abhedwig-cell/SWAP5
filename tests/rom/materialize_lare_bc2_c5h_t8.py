...
def history_block()->str:
    ...
    call require(symbol==SYM_HOLD.or.symbol==SYM_RISE.or.symbol==SYM_FALL,'LAREGW1 C5H valid boundary-only symbol')
...
def main()->int:
    ...
    text=one(text,"integer, parameter :: NHIST=4, NSTEPS=1024",
             "integer, parameter :: NHIST=4, NSTEPS=8192","temporal state count")
    text=one(text,"real(real64), parameter :: step_dt=0.0008_real64",
             "real(real64), parameter :: step_dt=0.0001_real64","main transaction interval")
    ...
    text=one(text,
      ...
      "write(*,'(A)') 'LAREGW1_C5H_B14_T8_GENERATED=TRUE'",
      "completion marker")

    a.output.write_text(text,encoding="utf-8")
    out={
      "schema":"swap5.lare.bc2.c5h.t8-materialization.v1",
      "temporal_factor":8,
      "main_step_dt_day":0.0001,
      "main_steps_per_history":8192,
