...
def main()->int:
    ...
    ap.add_argument("--c5l-materializer",required=True,type=pathlib.Path)
    ...
    with tempfile.TemporaryDirectory() as td:
        td=pathlib.Path(td)
        base=td/"c5l_full.f90"
        bm=td/"c5l_manifest.json"
        subprocess.run([
          sys.executable,str(a.c5l_materializer),
        ...
        if m.get("response_based") is not False:
            raise SystemExit("C5L full materializer is not response-independent")
    ...
    text=one(
      ...
      "call require(total_states==NSTEPS,'LAREGW1 exact C5L sliced history state count')",
      "state-count guard"
    )
    text=one(
      text,
    ...
    out={
      "schema":"swap5.lare.bc2.c5l.history-slice-materialization.v1",
      "history_index":idx,
      "history_label":f"Z{idx:02d}",
      "full_c5l_materializer_sha256":sha256(a.c5l_materializer),
