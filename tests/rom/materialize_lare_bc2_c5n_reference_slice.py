...
def main()->int:
    ...
    with tempfile.TemporaryDirectory() as td:
        td=pathlib.Path(td)
        base=td/"c5n_full.f90"
        bm=td/"c5n_manifest.json"
        subprocess.run([
        ...
        if m.get("response_based") is not False:
            raise SystemExit("C5N full materializer is not response-independent")
    ...
    text=one(
      ...
      "call require(total_states==NSTEPS,'LAREGW1 exact C5N sliced history state count')",
      "state count"
    )
    text=one(
      text,
    ...
    out={
      "schema":"swap5.lare.bc2.c5n.reference-history-slice.v1",
      "history_index":idx,
      "history_label":f"N{idx:02d}",
