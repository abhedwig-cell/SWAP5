...
def main()->int:
    ...
    with tempfile.TemporaryDirectory() as td:
        td=pathlib.Path(td)
        base=td/"c5p_full.f90"; bm=td/"c5p_manifest.json"
        subprocess.run([
        ...
        if m.get("response_based") is not False:
            raise SystemExit("C5P full materializer is not response-independent")
    ...
    text=one(
      ...
      "call require(total_states==NSTEPS,'LAREGW1 exact C5P sliced history state count')",
      "state count"
    )
    text=one(
      text,
    ...
    out={
      "schema":"swap5.lare.bc2.c5p.reference-history-slice.v1",
      "history_index":idx,
      "history_label":f"P{idx:02d}",
