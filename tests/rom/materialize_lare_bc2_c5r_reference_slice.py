...
def main()->int:
    ...
    with tempfile.TemporaryDirectory() as td:
        td=pathlib.Path(td)
        base=td/"c5r_full.f90"; bm=td/"c5r_manifest.json"
        subprocess.run([
        ...
        if m.get("response_based") is not False:
            raise SystemExit("C5R full materializer is not response-independent")
    ...
    text=one(
      ...
      "call require(total_states==NSTEPS,'LAREGW1 exact C5R sliced history state count')",
      "state count"
    )
    text=one(
      text,
    ...
    out={
      "schema":"swap5.lare.bc2.c5r.reference-history-slice.v1",
      "history_index":idx,
