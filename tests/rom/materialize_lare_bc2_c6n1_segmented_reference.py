...
def run_history_block(start:int,end:int)->str:
    ...
    if(SEGMENT_START==1)then
      ...
      call require(ok.and.state%ready(),'LAREDYN0R segmented initial committed state')
      call seed_steady(column,template,p,state,forcing,qeq,ok)
      call require(ok,'LAREDYN0R segmented gravity-consistent seed')
      t0=real(seed_intervals,real64)*seed_dt
      revision_base=seed_intervals
    else
      call read_segment_checkpoint(initial_state,t0,ok)
      call require(ok,'LAREDYN0R segmented checkpoint read')
      call fmr_new_b110_committed_state(state,column_id,initial_state,t0,ok)
      call require(ok.and.state%ready(),'LAREDYN0R segmented restart committed state')
      revision_base=0
    end if
    ...
    do step=SEGMENT_START,SEGMENT_END
      ...
      do substep=1,substeps
        ...
        call require(abs(mass)<=hard_mass_gate,'LAREDYN0R segmented hard mass gate')
        call require(state%current_revision()==int(revision_base+(step-SEGMENT_START)*substeps+substep,int64), &
             'LAREDYN0R segmented exact local revision progression')
    ...
    call require(ok,'LAREDYN0R segmented checkpoint write')
...
def main()->int:
    ...
    if not (1<=a.segment_start<=a.segment_end<=32768):
        raise SystemExit("segment bounds outside T32 route")
  ...
  call get_environment_variable('C6N1_CHECKPOINT_IN',segment_checkpoint_in,status=env_status)
  if(SEGMENT_START>1)call require(env_status==0.and.len_trim(segment_checkpoint_in)>0,'LAREDYN0R segmented checkpoint input path')
  call get_environment_variable('C6N1_CHECKPOINT_OUT',segment_checkpoint_out,status=env_status)
  call require(env_status==0.and.len_trim(segment_checkpoint_out)>0,'LAREDYN0R segmented checkpoint output path')
...
"""
    ...
    text=one(
      text,
      "  write(*,'(A)') 'LAREDYN0R_C6N1_SURFACE_FIXED_FLUX_REFERENCE_GENERATED=TRUE'",
      "  write(*,'(A)') 'LAREDYN0R_C6N1_SEGMENTED_SURFACE_FIXED_FLUX_REFERENCE_GENERATED=TRUE'\n"
      "  write(*,'(A)') 'LAREDYN0R_C6N1_SURFACE_FIXED_FLUX_REFERENCE_GENERATED=TRUE'",
      "segmented marker"
    )
    ...
    out={
      "schema":"swap5.lare.bc2.c6n1.segmented-reference-materialization.v1",
