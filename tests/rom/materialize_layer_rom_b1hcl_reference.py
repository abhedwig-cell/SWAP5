...
def main()->int:
    ...
    if not any(abs(a.dt_day-x)<=1e-15 for x in ALLOWED_DT):
        raise SystemExit(f"dt not in frozen B1HCL holdout: {a.dt_day}")
    factor=int(round(OBS_DT/a.dt_day))
    if factor!=8 or abs(factor*a.dt_day-OBS_DT)>1e-15:
        raise SystemExit("B1HCL dt does not define eight substeps per observation")
    ...
    text=replace_once(text,"real(real64), parameter :: step_dt=0.001_real64",
                      f"real(real64), parameter :: step_dt={format(a.dt_day,'.17g')}_real64","step_dt")
    a.output.write_text(text,encoding="utf-8")

    manifest={
        "schema":"swap5.layer-rom.b1hcl.reference-materialization.v1",
        "workstream":"F-ROM-LAYER","work_unit":"LAYER-ROM-B1HCL",
        "material":a.material,"dt_day":a.dt_day,
        "substeps_per_observation":factor,"nsteps":nsteps,
    ...
    print(json.dumps({"material":a.material,"dt_day":a.dt_day,
                      "substeps_per_observation":factor,"nsteps":nsteps,
                      "output_fortran_sha256":manifest["output_fortran_sha256"]},sort_keys=True))
    return 0
