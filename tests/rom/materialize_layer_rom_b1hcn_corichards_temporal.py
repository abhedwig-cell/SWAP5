...
def main()->int:
    ...
    a=ap.parse_args()
    dt=next((x for x in ALLOWED if abs(a.dt_day-x)<=1e-15),None)
    if dt is None: raise SystemExit(f"dt outside frozen B1HCN ladder: {a.dt_day}")
    factor=ALLOWED[dt]; nsteps=BASE_STEPS*factor
    ...
    text=replace_once(text,"real(real64), parameter :: step_dt=0.001_real64",
                      f"real(real64), parameter :: step_dt={format(dt,'.17g')}_real64","step_dt")
    a.output.write_text(text,encoding="utf-8")
    out={
      "schema":"swap5.layer-rom.b1hcn.corichards-temporal-materialization.v1",
      "workstream":"F-ROM-LAYER","work_unit":"LAYER-ROM-B1HCN",
      "dt_day":dt,"substeps_per_observation":factor,"nsteps":nsteps,
      "common_observation_interval_day":OBS_DT,"common_observation_count":BASE_STEPS,
