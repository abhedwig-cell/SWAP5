...
def run_history_block(start:int,end:int)->str:
    ...
    if(SEGMENT_START==1)then
      ...
      call require(ok.and.state%ready(),'LAREDYN0R ROMPURP_P1 segmented initial committed state')
      call seed_steady(column,template,p,state,forcing,qeq,ok)
      call require(ok,'LAREDYN0R ROMPURP_P1 segmented gravity-consistent seed')
      t0=real(seed_intervals,real64)*seed_dt
      revision_base=seed_intervals
    else
      call read_segment_checkpoint(initial_state,t0,ok)
      call require(ok,'LAREDYN0R ROMPURP_P1 segmented checkpoint read')
      call fmr_new_b110_committed_state(state,column_id,initial_state,t0,ok)
      call require(ok.and.state%ready(),'LAREDYN0R ROMPURP_P1 segmented restart committed state')
      revision_base=0
    end if
    ...
    do step=SEGMENT_START,SEGMENT_END
      ...
      do substep=1,substeps
        ...
        call require(abs(mass)<=hard_mass_gate,'LAREDYN0R ROMPURP_P1 segmented hard mass gate')
        call require(state%current_revision()==int(revision_base+(step-SEGMENT_START)*substeps+substep,int64), &
             'LAREDYN0R ROMPURP_P1 segmented exact local revision progression')
    ...
    call require(ok,'LAREDYN0R ROMPURP_P1 segmented checkpoint write')
...
def main()->int:
    ...
    if not (1<=a.segment_start<=a.segment_end<=total_steps):
        raise SystemExit("segment bounds outside P1 route")
  ...
  if(SEGMENT_START>1)call require(env_status==0.and.len_trim(segment_checkpoint_in)>0,'LAREDYN0R ROMPURP_P1 segmented checkpoint input path')
  call get_environment_variable('ROMPURP_P1_CHECKPOINT_OUT',segment_checkpoint_out,status=env_status)
  call require(env_status==0.and.len_trim(segment_checkpoint_out)>0,'LAREDYN0R ROMPURP_P1 segmented checkpoint output path')
...
"""
    ...
    text=one(
      ...
      "  write(*,'(A)') 'LAREDYN0R_ROMPURP_P1_SEGMENTED_SURFACE_REFERENCE_GENERATED=TRUE'\n"
      "  write(*,'(A)') 'LAREDYN0R_ROMPURP_P1_SURFACE_REFERENCE_GENERATED=TRUE'",
      "segmented marker"
    )
    ...
    out={
      "schema":"swap5.rom-purpose.p1.segmented-surface-reference-materialization.v1",
