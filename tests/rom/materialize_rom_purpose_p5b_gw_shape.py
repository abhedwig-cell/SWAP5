...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        td=pathlib.Path(td); base=td/"p4gw.f90"; bm=td/"p4gw.json"
        subprocess.run([
          sys.executable,"tests/rom/materialize_rom_purpose_p4_gw_reference.py",
...
"""
    text=one(text,old_decl,new_decl,"P5B declarations")

    old="""      call require(mod(numnod,16)==0,'LAREGW1 ROMPURP_P4_GW profile geometry divisible by 16')
    ...
    new="""      call require(mod(numnod,16)==0,'LAREGW1 ROMPURP_P4_GW profile geometry divisible by 16')
      call require(mod(numnod,128)==0,'LAREGW1 ROMPURP_P5B diagnostic geometry divisible by 128')
      if(mod(step,OUTPUT_FACTOR)==0)then
        nodes_per_bin=numnod/16
        do bin=1,16
          lo_node=(bin-1)*nodes_per_bin+1
        ...
        call require(moment_den>0.0_real64,'LAREGW1 ROMPURP_P5B positive head moment denominator')
        h_slope=moment_num/moment_den
        write(*,'(*(g0))') 'ROMPURP_P5B_GW_SHAPE|HISTORY=',trim(history_label(ih)), &
             ...
             '|H_SLOPE_BOTTOM=',h_slope,'|BOTTOM_FLUX=',bflux,'|BOTTOM_HEAD=',forcing%bottom_head
      end if"""
    text=one(text,old,new,"P5B shape diagnostic block")
    text=one(text,
      "write(*,'(A)') 'LAREGW1_ROMPURP_P4_GW_REFERENCE_GENERATED=TRUE'",
      "write(*,'(A)') 'LAREGW1_ROMPURP_P5B_GW_SHAPE_GENERATED=TRUE'"+chr(10)+
      "          write(*,'(A)') 'LAREGW1_ROMPURP_P4_GW_REFERENCE_GENERATED=TRUE'",
      "P5B marker")
    ...
    out={
      "schema":"swap5.rom-purpose.p5b.gw-shape-materialization.v1",
      "material":a.material,"temporal_factor":a.temporal_factor,
      "base_materialization":m,
      "diagnostic_route":"R512_T8",
      "diagnostic_fields":["THETA_PREV","THETA_BOTTOM","THETA_HALF1","THETA_HALF2","H_SLOPE_BOTTOM","BOTTOM_FLUX","BOTTOM_HEAD"],
