...
SE={1:0.75,2:0.83,3:0.89,4:0.95}
...
def initial_block()->str:
    ...
    select case(ih)
    case(1); value=0.75_real64
    case(2); value=0.83_real64
    case(3); value=0.89_real64
...
"""+"\n".join(rows)+"""
    ...
    call require(symbol==SYM_HOLD.or.symbol==SYM_RISE.or.symbol==SYM_FALL,'LAREGW1 ROMPURP_P8B_GW valid fresh boundary-only symbol')
...
def profile_block()->str:
    return """      call require(mod(numnod,16)==0,'LAREGW1 ROMPURP_P8B_GW profile geometry divisible by 16')
...
def main()->int:
    ...
    text=one(
      ...
      "ROMPURP_P8B_GW time-grid parameters"
    )
    text=one(
      text,
      "real(real64), parameter :: step_dt=0.0008_real64",
      f"real(real64), parameter :: step_dt={0.0008/factor:.12g}_real64",
      "ROMPURP_P8B_GW transaction interval"
    )
    text=one(
      text,
      "call require(any(numnod==[2,16]),'LAREGW1 C4Z geometry is R2 or R16')",
      "call require(any(numnod==[512,1024,2048]),'LAREGW1 ROMPURP_P8B_GW geometry is R512/R1024/R2048')",
      "ROMPURP_P8B_GW geometry guard"
    )
    ...
    if a.material=="B01":
        ...
        text=one(
          ...
          "write(*,'(A)') 'LAREGW1_ROMPURP_P8B_GW_REFERENCE_GENERATED=TRUE'"+chr(10)+"          write(*,'(A)') 'LAREGW1_ROMPURP_P1_GW_REFERENCE_GENERATED=TRUE'",
          "completion marker"
        )
    else:
    ...
    out={
      "schema":"swap5.rom-purpose.p8b.gw-reference-materialization.v1",
      ...
      "histories":{
        "G25":{"Se":0.75,"phase1":"RISE","phase1_steps":288,"phase2":"FALL","phase2_steps":384,"hold_steps":352},
        "G26":{"Se":0.83,"phase1":"FALL","phase1_steps":352,"phase2":"RISE","phase2_steps":384,"hold_steps":288},
        "G27":{"Se":0.89,"phase1":"RISE","phase1_steps":416,"phase2":"FALL","phase2_steps":384,"hold_steps":224},
        "G28":{"Se":0.95,"phase1":"FALL","phase1_steps":480,"phase2":"RISE","phase2_steps":384,"hold_steps":160}
      },
