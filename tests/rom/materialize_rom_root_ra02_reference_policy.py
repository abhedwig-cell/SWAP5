#!/usr/bin/env python3
"""Materialize ROM-ROOT-RA02 research-only Reference policy.

Input is the already frozen C6R root-active harness. RA02 does not reopen C6R.
...
MARKER="ROM_ROOT_RA02_P2E20_REPRESENTATION_FLOOR"
...
def main()->int:
    ...
    if MARKER in before:
        raise SystemExit("RA02 policy already materialized")
...
"""
    ...
    if(p%root_extraction_active)then
      call rom_root_ra02_total_bound(p,rep_bound,bound_ok)
      call require(bound_ok.and.rep_bound>0.0_real64,'ROM-ROOT RA02 external representation bound')
      p%total_balance_tolerance=max(original_total_tol,rep_bound/(t1-t0))
    end if

    call sample_fresh(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
"""
    text=one(before,anchor,replacement,"prospective RA02 policy insertion")

    helper="""  subroutine rom_root_ra02_total_bound(p,bound,ok)
  ...
  end subroutine rom_root_ra02_total_bound
...
"""
    ...
    if base_marker in text:
        text=one(
            text,
            base_marker,
            base_marker+"  write(*,'(A)') 'ROM_ROOT_RA02_P2E20_REPRESENTATION_FLOOR=TRUE'\n",
            "RA02 base completion marker"
        )
    elif segmented_marker in text:
        text=one(
            text,
            segmented_marker,
            segmented_marker+"  write(*,'(A)') 'ROM_ROOT_RA02_P2E20_REPRESENTATION_FLOOR=TRUE'\n",
            "RA02 segmented completion marker"
        )
    else:
        raise SystemExit("RA02 completion marker source not found")

    args.output.parent.mkdir(parents=True,exist_ok=True)
    args.output.write_text(text)

    manifest={
      "schema":"swap5.rom_root.ra02.materialization.v1",
