...
TEST=tests/rom/test_f_rom0r_r3d4_failclosed_fallback.f90
ANALYZER=tests/rom/analyze_f_rom0r_r3d4_failclosed_fallback.py
...
assert p["frozen_controls"]["dt_day"]==0.0008
...
assert d["candidate_policy_qualified"] is False
assert d["next_permitted_action"]=="PREREGISTER_FAIL_CLOSED_TOTAL_ONLY_FALLBACK_POLICY_IF_INDEPENDENT_AUTHORITY_SUPPORTS_IT"
...
for opt in 0 2; do
  ...
  [[ "$(grep -c 'F_ROM0R_R3D4_CONTROL|' "$EVIDENCE/o$opt.txt")" -eq 4 ]] || fail "four control rows O$opt"
  [[ "$(grep -c 'F_ROM0R_R3D4_POLICY_CASE_PASS|' "$EVIDENCE/o$opt.txt")" -eq 4 ]] || fail "four policy pass rows O$opt"
done
...
assert r["decision"] in {"R3_FAIL_CLOSED_TOTAL_ONLY_FALLBACK_QUALIFIED","R3_FAIL_CLOSED_TOTAL_ONLY_FALLBACK_NO_GO"}
