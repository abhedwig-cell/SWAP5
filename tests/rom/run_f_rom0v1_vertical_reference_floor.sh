...
Q2_STATUS=integration/f-rom/F-ROM0R_R3Q2_STATUS.json
...
git merge-base --is-ancestor "$PREREG_COMMIT" HEAD || fail "ROM0V1 corrected preregistration not ancestor"
...
python3 - "$PREREG" "$Q2_STATUS" "$D4_STATUS" <<'PY'
import json,sys
p=json.load(open(sys.argv[1])); q=json.load(open(sys.argv[2])); d4=json.load(open(sys.argv[3]))
assert p["schema"]=="swap5.f-rom0v1-preregistration.v2"
assert p["phase"]=="PREREGISTERED_BEFORE_EXECUTION"
assert p["pre_execution_correction"]["execution_evidence_seen_before_correction"] is False
...
assert q["decision"]=="FKT_PRESCRIBED_HEAD_REPRESENTATION_TRAJECTORY_QUALIFIED"
assert q["vertical_reference_floor_authorized"] is True
...
python3 "$MATERIALIZER" --source tests/fsi/fsi04_real_headcalc_stubs.f90 \
  --output "$BUILD/stubs_n32.f90" --nodes 32 --dz-cm 5
...
for geom in 16 32; do
  ...
  "$BUILD/g${geom}_o2/rom0_test" > "$EVIDENCE/g${geom}_o2_repeat.txt" 2>&1 || fail "repeat geometry ${geom}"
  cmp "$EVIDENCE/g${geom}_o2.txt" "$EVIDENCE/g${geom}_o2_repeat.txt" || fail "repeat drift geometry ${geom}"
  echo "F_ROM0V1_O0_O2_IDENTITY_G${geom}=PASS"
  echo "F_ROM0V1_REPEAT_IDENTITY_G${geom}=PASS"
done

python3 "$ANALYZER" --g16 "$EVIDENCE/g16_o2.txt" --g32 "$EVIDENCE/g32_o2.txt" \
  --output "$EVIDENCE/F-ROM0V1_RESULT.json" | tee "$EVIDENCE/analyzer.txt"
...
assert r["dt_day"]==0.0016
assert r["common_endpoints_per_case"]==32
print("F_ROM0V1_SCIENTIFIC_DECISION="+r["decision"])
PY

sha256sum "$EVIDENCE/g16_o0.txt" "$EVIDENCE/g16_o2.txt" "$EVIDENCE/g16_o2_repeat.txt" \
  "$EVIDENCE/g32_o0.txt" "$EVIDENCE/g32_o2.txt" "$EVIDENCE/g32_o2_repeat.txt" \
  "$EVIDENCE/F-ROM0V1_RESULT.json" "$EVIDENCE/analyzer.txt" > "$EVIDENCE/sha256.txt"
