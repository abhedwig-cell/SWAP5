...
ADMISSION=5db312c3845ccb0a00372b3ccf3e6a45f0be9a2b
QUALIFIED_KERNEL=c28cb8246aaf087da92538e58b1aa2da5d1b5b11
QUALIFIED_BACKEND=5d63f91b37443952b4a96292925f645aae0b22d1
POST_ADMISSION_REPLAY="${F_ROM1A_I0_POST_ADMISSION_REPLAY:-0}"
...
if [[ "$POST_ADMISSION_REPLAY" == "1" ]]; then
  # Moving-current preservation mode. The original detached preregistration
  # branch is deliberately not required to be in canonical history after
  # admission. Instead bind to the admitted postimage and the exact two
  # source blobs that passed the frozen qualification.
  git merge-base --is-ancestor "$ADMISSION" HEAD || fail "canonical admission is not ancestor"
  test "$(git rev-parse HEAD:src/kernel/mod_kernel_transactions.f90)" = "$QUALIFIED_KERNEL" ||     fail "qualified kernel postimage drift"
  test "$(git rev-parse HEAD:src/runtime/mod_fmr_serialized_reference_backend.f90)" = "$QUALIFIED_BACKEND" ||     fail "qualified backend postimage drift"
  echo "F_ROM1A_I0_POST_ADMISSION_AUTHORITY=PASS"
