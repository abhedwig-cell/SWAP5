...
for spec in "R16 16 10" "R8 8 20" "R4 4 40" "R2 2 80"; do
  ...
  for opt in 0 2; do
    ...
    rc=$?
    set -e
    if grep -Fq 'F_ROMV2_D2_EXECUTION_COMPLETE=PASS' "$EVIDENCE/${id}_o${opt}.txt"; then complete=1; else complete=0; fi
  ...
  if [[ "$rc0" -eq 0 && "$rc2" -eq 0 && "$complete0" -eq 1 && "$complete2" -eq 1 ]]; then
    ...
    scientific_status=FROZEN_REFERENCE_POLICY_NO_GO
    echo "F_ROMV2_D2_${id}_FROZEN_POLICY_NO_GO rc0=${rc0} rc2=${rc2}"
    grep 'F_ROMV2_D2_POLICY_REJECT|' "$EVIDENCE/${id}_o0.txt" | tail -1 > "$EVIDENCE/${id}_policy_reject.txt" || true
    grep 'F_ROMV2_D2_POLICY_REJECT|' "$EVIDENCE/${id}_o2.txt" | tail -1 >> "$EVIDENCE/${id}_policy_reject.txt" || true
  fi
...
import json,sys
out={"schema":"swap5.f-romv2-d2.geometry-execution-status.v1","geometries":{}}
...
args=(--r16 "$EVIDENCE/R16_o2.txt" --status "$EVIDENCE/geometry-status.json" --prereg "$PREREG" --output "$EVIDENCE/F-ROMV2_D2_RESULT.json")
...
sha256sum "$EVIDENCE"/R*_o*.txt "$EVIDENCE/geometry-status.json" "$EVIDENCE/F-ROMV2_D2_RESULT.json" "$EVIDENCE/analyzer.txt" > "$EVIDENCE/sha256.txt"
