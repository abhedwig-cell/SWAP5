...
def corrected_fluxes(y:np.ndarray,H:float,Hdot:float)->tuple[np.ndarray,float]:
    ...
    q=q_state-q_eq_discrete
    qH=qH_state-qH_eq_discrete
    return q,qH
...
def fixed_equilibrium_rhs_crosscheck(H_values:list[float])->float:
    maximum=0.0
    for H in H_values:
        eq=equilibrium_state(H)
        y=np.concatenate([eq,[0.0]])
        got,_=rhs(y,H,0.0)
        maximum=max(maximum,float(np.max(np.abs(got))))
    return maximum
...
def main()->int:
    ...
    assert pre["phase"]=="PREREGISTERED_BEFORE_RESIDUAL_ONLY_EQUILIBRIUM_SUBTRACTION"
    assert pre["predecessors"]["required_B0_decision"]==b0["decision"]
    assert pre["predecessors"]["required_B1_decision"]==b1["decision"]
    assert b0["decision"]=="BC2_CONSERVATIVE_MOVING_GEOMETRY_QUALIFIED_WITH_RESPONSE_CONFUNDING"
    assert b1["decision"]=="BC2_B1_EQSUB_CONTROL_MIXED_RESPONSE"
    ...
    rhs_error=fixed_equilibrium_rhs_crosscheck(H_values[::max(1,len(H_values)//256)]+[min(H_values),max(H_values)])
    ...
    if not hard_ok:
        decision="BC2_B2_RESIDUAL_ONLY_NUMERICALLY_BLOCKED"
    elif all_noninferior and strict_any:
        decision="BC2_B2_RESIDUAL_ONLY_RELATIVE_SUPPORT"
    elif any(
        any(v for v in row.get("component_noninferiority",{}).values())
        for row in comparisons.values()
    ):
        decision="BC2_B2_RESIDUAL_ONLY_MIXED_RESPONSE"
    else:
        decision="BC2_B2_RESIDUAL_ONLY_NO_RELATIVE_SUPPORT"

    hold=compact(cases["WT_HOLD"])
    b0hold=b0["conservative"]["histories"]["WT_HOLD"]
    result={
        "schema":"swap5.lare.bc2.b2.result.v1",
        "workstream":"F-ROM-LARE",
        "work_unit":"LARE-BC2-B2",
        "decision":decision,
        "hard_gates":{
            ...
            "fixed_equilibrium_rhs_max_residual_cm_per_day":rhs_error,
            "fixed_equilibrium_rhs_gate_cm_per_day":RHS_GATE,
        ...
        "secondary_B1_path_control":{
            "decision":b1["decision"],
            "WT_HOLD_total_storage_error_cm":b1["histories"]["WT_HOLD"]["max_abs_total_unsaturated_storage_error_cm"],
            "WT_RISE_total_storage_error_cm":b1["histories"]["WT_RISE"]["max_abs_total_unsaturated_storage_error_cm"],
            "WT_FALL_total_storage_error_cm":b1["histories"]["WT_FALL"]["max_abs_total_unsaturated_storage_error_cm"],
            "WT_CYCLE_total_storage_error_cm":b1["histories"]["WT_CYCLE"]["max_abs_total_unsaturated_storage_error_cm"],
        },
        "stationary_WT_HOLD_attribution":{
            "B0_total_storage_error_cm":b0hold["max_total_unsat_error_cm"],
            "B2_total_storage_error_cm":hold["max_abs_total_unsaturated_storage_error_cm"],
            "storage_error_ratio_B2_over_B0":hold["max_abs_total_unsaturated_storage_error_cm"]/b0hold["max_total_unsat_error_cm"],
            "B0_cumulative_qH_error_cm":b0hold["max_cum_qH_error_cm"],
            "B2_cumulative_qH_error_cm":hold["max_abs_cumulative_qH_error_cm"],
            "cumulative_error_ratio_B2_over_B0":hold["max_abs_cumulative_qH_error_cm"]/b0hold["max_cum_qH_error_cm"],
            "B0_sign_mismatch_count":b0hold["sign_mismatch_count"],
            "B2_sign_mismatch_count":hold["qH_sign_mismatch_count"],
        },
        "interpretation":[
            "BC2-B2 is a diagnostic residual-only equilibrium-subtraction control, not selected production LARE physics.",
            "The correction has no fitted coefficient: it subtracts only the standard-LARE discrete hydrostatic residual at the current geometry and adds no quasi-static equilibrium-path flux.",
            "WT_HOLD isolates fixed-geometry hydrostatic drift. Moving-history response is driven by the conservative geometry term and non-equilibrium state departure under the prescribed Reference H(t).",
