...
TEST=tests/rom/test_lare_dyn0a_mech3b_reference.f90
COMPILER=tests/rom/compile_f_rom0_fortran_closure.py
UNIFORM=tests/rom/materialize_f_rom0_headcalc_stubs.py

BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-lare-dyn0a-mech3b-${GITHUB_RUN_ID:-local}-$$"
EVIDENCE="${LARE_DYN0A_MECH3B_EVIDENCE_DIR:-$ROOT/LARE_DYN0A_MECH3B_EVIDENCE}"
mkdir -p "$BUILD" "$EVIDENCE"
trap 'rm -rf "$BUILD"' EXIT

fail(){ echo "LARE_DYN0A_MECH3B_FAIL $*" >&2; exit 1; }

git merge-base --is-ancestor "$CANONICAL_START" HEAD || fail "canonical start not ancestor"
git diff --quiet "$CANONICAL_START"...HEAD -- src reference || fail "MECH3B changed src/reference"
echo 'LARE_DYN0A_MECH3B_SOURCE_FREEZE=PASS'

python3 "$UNIFORM" --source tests/fsi/fsi04_real_headcalc_stubs.f90 --output "$BUILD/fine.f90" --nodes 16 --dz-cm 10

materials=(SAND LOAM CLAY)
cases=()
for s in 1 2 3; do
  for f in 1 2 3 4; do
    cases+=("S${s}_B1_F${f}")
...
for opt in 0 2; do
  ...
  for material in "${materials[@]}"; do
    for case_id in "${cases[@]}"; do
      ...
      set +e
      LARE_DYN0A_MATERIAL="$material" LARE_DYN0A_BOTTOM_FILTER=1 LARE_DYN0A_CASE_FILTER="$case_id" \
...
if rc==0:
    ...
    if known_domain:
        ...
        raise SystemExit('unexpected technical MECH3B Reference failure\n'+tail)
PY
...
echo 'LARE_DYN0A_MECH3B_CASEWISE_O0_O2_IDENTITY=PASS'

python3 - "$EVIDENCE" <<'PY'
import json,pathlib,re,sys
root=pathlib.Path(sys.argv[1])
rows=[line.split('\t') for line in (root/'status.tsv').read_text().splitlines()]
materials=sorted({r[0] for r in rows})
out={}
for material in materials:
    ...
    out[material]=cases
summary={
  'schema':'swap5.lare.dyn0a.mech3b.reference-status.v1',
  'decision':'LARE_DYN0A_MECH3B_BLIND_REFERENCE_PANEL_CHARACTERIZED',
...
(root/'LARE_DYN0A_MECH3B_REFERENCE_STATUS.json').write_text(json.dumps(summary,indent=2,sort_keys=True)+'\n')
...
echo 'LARE_DYN0A_MECH3B_REFERENCE_GATE=PASS'
