...
SE={"01":0.68,"02":0.82,"03":0.73,"04":0.90}
...
def histories(purpose:str)->dict[str,float]:
    prefix="S" if purpose=="SURF_P" else "G"
    return {f"{prefix}{k}":v for k,v in SE.items()}


def symbol_surface(history:str,step:int)->str:
    if history=="S01":
        if step<=192:return "WET"
        if step<=512:return "DRY"
        return "HOLD"
    if history=="S02":
        if step<=192:return "DRY"
        if step<=512:return "WET"
        return "HOLD"
    if history=="S03":
        if step<=320:return "WET"
        if step<=512:return "DRY"
        return "HOLD"
    if history=="S04":
        if step<=320:return "DRY"
        if step<=512:return "WET"
        return "HOLD"
...
def main():
    ...
    ref=json.loads(a.reference_result.read_text())
    ...
    assert ref["status"]=="P1_REFERENCE_QUALIFIED_CANDIDATES_AUTHORIZED"
    assert ref["candidate_response_authorized"] is True
    expected=pre["representations"][a.member]
    assert [float(x) for x in expected]==PARTITIONS[a.member]
    assert pre["representations"]["dimension"]==4
    assert pre["representations"]["closure"]=="CURRENT_LAYER_FACE"
    ...
    out={
      ...
      "work_unit":"ROM-PURPOSE-P1-CANDIDATE",
      "candidate":candidate,
      "scientific_firewall":{
        "reference_qualified_before_response":True,
        "representation_changed_after_response":False,
