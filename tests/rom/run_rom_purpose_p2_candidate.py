...
SURF_SE={"05":0.64,"06":0.76,"07":0.86,"08":0.93}
GW_SE={"01":0.68,"02":0.82,"03":0.73,"04":0.90}
...
def qtop_surface(sym:str,k0:float)->float:
    if sym=="WET":return 1.20*k0
    if sym=="DRY":return 0.80*k0
...
def symbol_gw(history:str,step:int)->str:
    if history=="G01":
        ...
        if step<=512:return "BOTTOM_HEAD_FALL"
        return "HOLD"
    if history=="G02":
        if step<=192:return "BOTTOM_HEAD_FALL"
        if step<=512:return "BOTTOM_HEAD_RISE"
        return "HOLD"
    if history=="G03":
...
def main():
    ...
    p2ref=json.loads(a.p2_reference_result.read_text())
    ...
    assert p2ref["status"]=="P2_REFERENCE_QUALIFIED_CANDIDATES_AUTHORIZED"
    assert p2ref["candidate_response_authorized"] is True
    ...
    expected=pre["inherited_frozen_candidate_design"]["representations"][a.member]
    assert [float(x) for x in expected]==PARTITIONS[a.member]
    reps=pre["inherited_frozen_candidate_design"]["representations"]
    assert reps["dimension"]==4
    assert reps["closure"]=="CURRENT_LAYER_FACE"

    candidate=run_candidate(a.purpose,a.material,a.member)
    out={
      "schema":"swap5.rom-purpose.p2.layer-candidate.v1",
      "workstream":"ROM-PURPOSE",
      "work_unit":"ROM-PURPOSE-P2-CANDIDATE",
      "candidate":candidate,
      "authority":{
        "surface_reference":"P2 fresh S05-S08",
        "groundwater_reference":"P1 inherited G01-G04",
        "P1_failure_overridden":False
      },
      "scientific_firewall":{
        "P2_reference_qualified_before_response":True,
