...
PARTITIONS={
    ...
    "S6":[0.0,10.0,20.0,40.0,60.0,80.0,160.0],
    "S8":[0.0,10.0,20.0,30.0,40.0,50.0,60.0,80.0,160.0],
    "G4":[0.0,80.0,120.0,140.0,160.0],
    "G6":[0.0,80.0,100.0,120.0,140.0,150.0,160.0],
    "G8":[0.0,80.0,100.0,120.0,130.0,140.0,150.0,155.0,160.0],
    "U4":[0.0,40.0,80.0,120.0,160.0],
    "U6":[0.0,26.6666666666667,53.3333333333333,80.0,106.666666666667,133.333333333333,160.0],
    "U8":[0.0,20.0,40.0,60.0,80.0,100.0,120.0,140.0,160.0],
}
ALLOWED={
    "SURF_P":("S4","S6","S8","U4","U6","U8"),
    "GW_LB":("G4","G6","G8","U4","U6","U8"),
}
SURF_SE={"09":0.70,"10":0.79,"11":0.88,"12":0.91}
GW_SE={"06":0.70,"07":0.78,"08":0.86,"09":0.92}
...
def main():
    ...
    if a.member.startswith("S"):
        ...
        expected=reps["uniform"][a.member]
    assert [float(x) for x in expected]==PARTITIONS[a.member]
    assert pre["candidate_execution"]["closure"]=="CURRENT_LAYER_FACE"
    assert a.member in pre["candidate_execution"]["candidate_matrix"][a.purpose]

    candidate=run_candidate(a.purpose,a.material,a.member)
    out={
      "schema":"swap5.rom-purpose.p3.layer-candidate.v1",
      "workstream":"ROM-PURPOSE",
      "work_unit":"ROM-PURPOSE-P3-CANDIDATE",
      "candidate":candidate,
      "authority":{
