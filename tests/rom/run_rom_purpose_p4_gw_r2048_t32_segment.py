...
def main()->int:
    ...
    target=a.canonical_root/"tests/rom/test_p4gw_t32_segment.f90"
    ...
    stub=f"p4gwt32_{a.material}_o{a.opt}_h{a.history}_s{a.segment}_stubs.f90"
    run([
    ...
    build=pathlib.Path(f"/tmp/p4gwt32_{a.material}_o{a.opt}_h{a.history}_s{a.segment}")
    run([
      sys.executable,str(a.canonical_root/"tests/rom/compile_f_rom0_fortran_closure.py"),
      "--root",str(a.canonical_root),"--stub",stub,
      "--target","tests/rom/test_p4gw_t32_segment.f90",
    ...
    if "LAREGW1_ROMPURP_P4_GW_REFERENCE_GENERATED=TRUE" not in text:
        raise SystemExit("P4 GW marker missing")
    ...
    out={
      "schema":"swap5.rom-purpose.p4.gw-r2048-t32-segment-execution.v2",
