...
def main():
    ...
    generated=a.canonical_root/"tests/rom/test_p5c_gw_interface.f90"
    manifest=a.output_dir/"materialization.json"
    run([
      sys.executable,"tests/rom/materialize_rom_purpose_p5c_gw_interface.py",
    ...
    logs={}
    for opt in (0,2):
        stub=f"p5c_{a.material}_o{opt}_stubs.f90"
        run([
        ...
        build=pathlib.Path(f"/tmp/p5c_{a.material}_o{opt}")
        run([
          sys.executable,str(a.canonical_root/"tests/rom/compile_f_rom0_fortran_closure.py"),
          "--root",str(a.canonical_root),"--stub",stub,"--target","tests/rom/test_p5c_gw_interface.f90",
          "--external-source","src/legacy/b1_10_port/headcalc.f90","--build",str(build),"--opt",str(opt)
        ])
        log=a.output_dir/f"p5c_{a.material}_o{opt}.txt"
        with log.open("w") as fh:
            cp=subprocess.run([str(build/"rom0_test")],stdout=fh,stderr=subprocess.STDOUT)
        if cp.returncode!=0:
            raise SystemExit(cp.returncode)
        text=log.read_text(errors="strict")
        if "LAREGW1_ROMPURP_P5C_GW_INTERFACE_GENERATED=TRUE" not in text:
            raise SystemExit("P5C interface marker missing")
        if text.count("LAREGW1_STATE|")!=4*1024*8:
            raise SystemExit("P5C state coverage mismatch")
        if text.count("ROMPURP_P5C_GW_INTERFACE|")!=4*1024:
            raise SystemExit("P5C interface coverage mismatch")
        logs[opt]=log
    if logs[0].read_bytes()!=logs[2].read_bytes():
        raise SystemExit("P5C O0/O2 trace identity mismatch")
    out={
      "schema":"swap5.rom-purpose.p5c.gw-interface-execution.v1",
      "material":a.material,"route":"R512_T8","histories":["G21","G22","G23","G24"],
      ...
      "forcing_changed":False,"scientific_change":False
    }
    (a.output_dir/f"p5c_{a.material}_execution.json").write_text(json.dumps(out,indent=2,sort_keys=True)+"\n")
