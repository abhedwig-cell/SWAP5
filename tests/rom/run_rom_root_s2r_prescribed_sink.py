...
ARITH_GUARD = 64.0 * EPS
N_TRANSACTIONS = int(round(HORIZON / DT))
...
def case(material: str, history: str, member: str) -> dict:
    ...
    fraction_fsum = math.fsum(float(v) for v in fractions)
    sink_fsum = math.fsum(float(v) for v in sink)
    sink_route_sum = float(np.sum(sink))
    sink_l1 = math.fsum(abs(float(v)) for v in sink)
    layer_sum_bound = gamma(max(0, len(sink) - 1)) * sink_l1

    final_root = float(root["cumulative_root_withdrawal_cm"][-1])
    analytical_root = tp * HORIZON
    route_increment = float(0.5 * DT * (sink_route_sum + sink_route_sum))
    cumulative_reference = math.fsum([route_increment] * N_TRANSACTIONS)
    cumulative_roundoff_bound = (
        gamma(N_TRANSACTIONS) * N_TRANSACTIONS * abs(route_increment)
        + 2.0 * EPS * max(1.0, abs(cumulative_reference))
    )
    scale_rate = max(1.0, abs(tp))

    checks = {
        "fractions_nonnegative": bool(np.all(fractions >= 0.0)),
        "fraction_sum_within_arithmetic_guard": abs(fraction_fsum - 1.0) <= ARITH_GUARD,
        "sink_rate_within_arithmetic_guard": abs(sink_fsum - tp) <= ARITH_GUARD * scale_rate,
        "route_sum_within_layer_summation_bound": abs(sink_route_sum - sink_fsum) <= layer_sum_bound,
        "cumulative_root_within_operation_count_bound": abs(final_root - cumulative_reference) <= cumulative_roundoff_bound,
    ...
    return {
        ...
        "root_fraction_sum_fsum": fraction_fsum,
        "prescribed_total_rate_fsum_cm_per_day": sink_fsum,
        "prescribed_total_rate_route_sum_cm_per_day": sink_route_sum,
        "layer_summation_bound_cm_per_day": layer_sum_bound,
        "analytical_tp_times_horizon_cm_diagnostic_only": analytical_root,
        "binary64_route_increment_cm": route_increment,
        "transaction_count": N_TRANSACTIONS,
        "cumulative_reference_fsum_cm": cumulative_reference,
        "actual_cumulative_root_cm": final_root,
        "cumulative_roundoff_bound_cm": cumulative_roundoff_bound,
        "cumulative_difference_cm": final_root - cumulative_reference,
...
def main() -> int:
    ...
    assert pre["frozen_execution"]["ledger_gate_cm"] == LEDGER_GATE
    assert pre["arithmetic_derivation"]["transactions"] == N_TRANSACTIONS

    cases = {member: case(args.material, args.history, member) for member in PARTITIONS}
    out = {
        "schema": "swap5.rom_root.s2r.execution.v1",
        "workstream": "ROM-ROOT",
