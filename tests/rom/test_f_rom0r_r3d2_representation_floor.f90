program test_f_rom0r_r3d2_retry_cause
  ...
  call require(numnod==16,'R3D2 geometry frozen')
  do icase=1,size(cases)
    call diagnose_case(trim(cases(icase)))
  end do
  write(*,'(A)') 'F_ROM0R_R3D2_DIAGNOSTIC_GATE=PASS'
...
contains
  ...
  subroutine diagnose_case(case_id)
    ...
    case('BOTTOM_HEAD_FALL')
      ...
      pre_steps=9
    case default
      call require(.false.,'known R3D2 case')
      hbot=h0; pre_steps=0
    end select
    ...
    write(*,'(*(g0))') 'F_ROM0R_R3D2_CASE_COMPLETE|CASE=',trim(case_id),'|PRE_STEPS=',pre_steps, &
  ...
  subroutine direct_diagnostic(case_id,parameters,forcing,committed,t0,t1,backend,column,template)
    ...
    write(*,'(*(g0))') 'F_ROM0R_R3D2_CLASS|CASE=',trim(case_id),'|CLASS=',trim(class_name), &
    ...
    write(*,'(*(g0))') 'F_ROM0R_R3D2_BOUND|CASE=',trim(case_id), &
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'F_ROM0R_R3D2_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_f_rom0r_r3d2_retry_cause
