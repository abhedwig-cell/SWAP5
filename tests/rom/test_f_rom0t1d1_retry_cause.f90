program test_f_rom0t1d1_retry_cause
  ...
  integer(int64), parameter :: column_id=967101_int64
  character(len=24), parameter :: cases(3)=[character(len=24) :: &
       'B01_E1_NOMINAL_FLUX','B01_E2_DRYING_FLUX','B14_E2_DRYING_FLUX']
  character(len=8), parameter :: materials(3)=[character(len=8) :: 'B01','B01','B14']
  real(real64), parameter :: top_factor(3)=[0.01_real64,-0.005_real64,-0.005_real64]
  real(real64), parameter :: bottom_factor(3)=[-0.004_real64,-0.019_real64,-0.019_real64]
  integer, parameter :: predecessor_steps(3)=[17,3,3]
  integer :: icase

  call require(numnod==16,'T1D1 geometry frozen')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'T1D1 depth frozen')
  do icase=1,size(cases)
    call diagnose_case(trim(cases(icase)),trim(materials(icase)),top_factor(icase),bottom_factor(icase),predecessor_steps(icase))
  end do
  write(*,'(A)') 'F_ROM0T1D1_DIAGNOSTIC_GATE=PASS'

contains

  subroutine diagnose_case(case_id,material_id,qtop_factor,qbot_factor,pre_steps)
    character(len=*),intent(in) :: case_id,material_id
    real(real64),intent(in) :: qtop_factor,qbot_factor
    integer,intent(in) :: pre_steps
    ...
    real(real64) :: h0,k0,qtop,qbot,t0,t1,mass,bex,bflux
    ...
    do i=1,pre_steps
      ...
      call require(ok,'T1D1 accepted predecessor sample')
      t0=t1
    end do
    t1=t0+step_dt
    ...
    write(*,'(*(g0))') 'F_ROM0T1D1_CASE_COMPLETE|CASE=',trim(case_id),'|PRE_STEPS=',pre_steps, &
  ...
  subroutine direct_diagnostic(case_id,parameters,forcing,committed,t0,t1,backend,column,template)
    ...
    real(real64) :: rep_integrated_bound,rep_rate_bound,integrated_total_abs,rep_ratio
    ...
    type is(fmr_b110_physical_state_t)
      ...
      rep_integrated_bound=0.5_real64*sum((spacing(parameters%cofgen(2,1:numnod)) + &
    ...
    call require(ieee_is_finite(rep_integrated_bound).and.rep_integrated_bound>0.0_real64, &
         'positive finite prospective representation bound')
    integrated_total_abs=abs(rsum)*(t1-t0)
    rep_rate_bound=rep_integrated_bound/(t1-t0)
    rep_ratio=integrated_total_abs/rep_integrated_bound
    ...
    write(*,'(*(g0))') 'F_ROM0T1D1_CLASS|CASE=',trim(case_id),'|CLASS=',trim(class_name), &
    ...
    write(*,'(*(g0))') 'F_ROM0T1D1_BOUND|CASE=',trim(case_id), &
         '|REP_BOUND_CM=',rep_integrated_bound,'|REP_RATE_BOUND=',rep_rate_bound, &
         '|ABS_TOTAL_RESIDUAL_CM=',integrated_total_abs,'|RESIDUAL_OVER_BOUND=',rep_ratio, &
         '|WITHIN_BOUND=',integrated_total_abs<=rep_integrated_bound
  ...
  subroutine initialize_identity(col,tmpl)
    ...
    tmpl%template_id=967101_int64;tmpl%physics_topology_id=967102_int64
    tmpl%vertical_layout_id=967103_int64;tmpl%state_layout_id=967104_int64
    tmpl%solver_interface_id=967105_int64
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'F_ROM0T1D1_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_f_rom0t1d1_retry_cause
