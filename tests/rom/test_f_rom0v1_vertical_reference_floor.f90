program test_f_rom0v1_vertical_reference_floor
  ...
  real(real64), parameter :: qtop_factor(2)=[0.01_real64,-0.005_real64]
  real(real64), parameter :: qbot_factor(2)=[-0.004_real64,-0.019_real64]
  integer :: icase

  call require(numnod==16.or.numnod==32,'geometry node count is 16 or 32')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'geometry depth 160 cm')
  if(numnod==16) call require(maxval(abs(dz(1:numnod)-10.0_real64))<=1.0e-12_real64,'G16 dz 10 cm')
  if(numnod==32) call require(maxval(abs(dz(1:numnod)-5.0_real64))<=1.0e-12_real64,'G32 dz 5 cm')

  do icase=1,2
    call run_case(trim(case_ids(icase)),trim(material_ids(icase)),qtop_factor(icase),qbot_factor(icase))
  end do

  write(*,'(A,I0)') 'F_ROM0V1_GEOMETRY_NODES=',numnod
  write(*,'(A)') 'F_ROM0V1_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_case(case_id,material_id,top_factor,bottom_factor)
    ...
    write(*,'(*(g0))') 'F_ROM0V1_CASE_START|GEOM_N=',numnod,'|CASE=',trim(case_id),'|MATERIAL=',trim(material_id), &
         '|H0=',h0,'|K0=',k0,'|QTOP=',qtop,'|QBOT=',qbot,'|DT=',dt_ref,'|STEPS=',nsteps
  ...
  subroutine emit_nodes(state,case_id,step,time)
    type(kernel_committed_state_t),intent(in) :: state
    ...
    type is(fmr_b110_physical_state_t)
      do i=1,numnod
        write(*,'(*(g0))') 'F_ROM0V1_NODE|GEOM_N=',numnod,'|CASE=',trim(case_id),'|STEP=',step,'|T=',time, &
  ...
  subroutine initialize_state(p,h0,k0,state)
    ...
    call bind_b110_default_mvg_provider(provider,hp,dt_ref)
  ...
  subroutine initialize_identity(col,tmpl)
    ...
    tmpl%template_id=965101_int64+int(numnod,int64);tmpl%physics_topology_id=965201_int64
    tmpl%vertical_layout_id=965300_int64+int(numnod,int64);tmpl%state_layout_id=965401_int64
    tmpl%solver_interface_id=965501_int64
    ...
    col%column_id=column_id+int(numnod,int64);col%template_id=tmpl%template_id;col%parameter_ref=1_int64
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'F_ROM0V1_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_f_rom0v1_vertical_reference_floor
