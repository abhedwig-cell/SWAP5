program test_f_rom1a_i0_reference_observation_seam
  ...
  use mod_kernel_transactions, only: kernel_committed_state_t, kernel_reference_floor_result_t, &
       kernel_reference_floor_candidate_t, kernel_diagnostics_t, KERNEL_REFERENCE_FLOOR_STATUS_OK, KERNEL_REFERENCE_FLOOR_STATUS_SOLVER_FAILED, &
  ...
  call require(numnod==16,'I0 geometry frozen at 16 nodes')
  do imat=1,size(materials)
    call qualify_material(trim(materials(imat)))
  end do
  call qualify_fail_closed_retry()
  write(*,'(A)') 'F_ROM1A_I0_REFERENCE_OBSERVATION_SEAM_GATE=PASS'
...
contains
  ...
  subroutine execute_one(material_id,label,parameters,forcing,initial_state,column,template)
    ...
    write(*,'(*(g0))') 'F_ROM1A_I0_ROW|MATERIAL=',trim(material_id),'|MODE=',trim(label), &
  ...
  subroutine qualify_fail_closed_retry()
    type(fmr_b110_physical_parameters_t) :: parameters
    type(fmr_b110_physical_forcing_t) :: forcing
    type(fmr_b110_physical_state_t) :: initial_state
    type(fmr_serialized_reference_backend_t) :: backend
    ...
    real(real64) :: h0,k0,qeq,t_before,t_after
    integer(int64) :: rev_before,lineage_before
    logical :: ok,before_ok,after_ok,time_ok

    call initialize_parameters('B01',parameters)
    call initialize_state(parameters,h0,k0,initial_state)
    qeq=-k0
    parameters%bottom_mode=5
    parameters%max_iterations=1
    call initialize_forcing(forcing,qeq,qeq,0.75_real64*h0)
    call initialize_identity(column,template)
    call fmr_new_b110_committed_state(committed,column_id,initial_state,0.0_real64,ok)
    call require(ok.and.committed%ready(),'I0 failure committed seed')
    call backend%initialize(top_boundary)

    rev_before=committed%current_revision()
    lineage_before=committed%current_lineage_id()
    call committed%current_time(t_before,time_ok)
    call require(time_ok,'I0 failure initial time bound')
    call committed%snapshot(before_state,before_ok)
    call require(before_ok,'I0 failure before snapshot')

    call backend%run_reference_floor_sample(column,template,parameters,committed,forcing,0.0_real64,dt,hard_mass_gate, &
         result,candidate,diagnostics)

    call require(result%status==KERNEL_REFERENCE_FLOOR_STATUS_SOLVER_FAILED,'I0 controlled solver failure')
    call require(.not.result%sample_valid,'I0 failed sample invalid')
    call require(.not.candidate%ready(),'I0 failed sample candidate unavailable')
    call require(result%physical_advances==1,'I0 failure one attempted physical advance')
    call require(committed%current_revision()==rev_before,'I0 failed sample revision immutable')
    call require(committed%current_lineage_id()==lineage_before,'I0 failed sample lineage immutable')
    call committed%current_time(t_after,time_ok)
    call require(time_ok.and.same_bits(t_before,t_after),'I0 failed sample time immutable')
    call committed%snapshot(after_state,after_ok)
    call require(after_ok,'I0 failure after snapshot')
    call require(state_bits_equal(before_state,after_state),'I0 failed sample physical state immutable')

    write(*,'(*(g0))') 'F_ROM1A_I0_FAILCLOSED|STATUS=PASS|SOLVER_STATUS=',result%status, &
         '|PHYSICAL_ADVANCES=',result%physical_advances,'|NL=',result%nonlinear_iterations, &
         '|BACKTRACK=',result%backtracking_attempts,'|REV=',committed%current_revision()
  end subroutine qualify_fail_closed_retry
  ...
  subroutine initialize_parameters(material_id,p)
    ...
    case default
      call require(.false.,'I0 known material')
  ...
  subroutine initialize_state(p,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'I0 finite seed')
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition) then
      write(*,'(A,1X,A)') 'F_ROM1A_I0_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_f_rom1a_i0_reference_observation_seam
