program test_f_romv2_d16_surface_comparators
  ...
  integer, parameter :: SYM_HIATUS=0, SYM_RAIN=1
  integer, parameter :: fmc_bins=200, fmc_i=100, fmc_j0=101, fmc_j1=199
  real(real64), parameter :: step_dt=90.0_real64/86400.0_real64
  ...
  integer(int64), parameter :: column_id=976001_int64

  integer :: ih,total_states,total_fallbacks
  real(real64) :: max_abs_mass

  call require(any(numnod==[2,16]),'F_ROMV2_D16_REF geometry is R2 or R16')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'F_ROMV2_D16_REF depth frozen')
  write(*,'(*(g0))') 'F_ROMV2_D16_REF_GEOMETRY|N=',numnod,'|DZ_CM=',dz(1),'|DEPTH_CM=',sum(dz(1:numnod))
  ...
  call require(total_states==NHIST*NSTEPS,'F_ROMV2_D16_REF exact library state count')
  write(*,'(A,I0)') 'F_ROMV2_D16_REF_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'F_ROMV2_D16_REF_DEVELOPMENT_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'F_ROMV2_D16_REF_STATE_COUNT=',total_states
  write(*,'(A,I0)') 'F_ROMV2_D16_REF_DEVELOPMENT_STATE_COUNT=',NHIST*NSTEPS
  write(*,'(A,I0)') 'F_ROMV2_D16_REF_TOTAL_FALLBACK_COUNT=',total_fallbacks
  write(*,'(*(g0))') 'F_ROMV2_D16_REF_MAX_ABS_MASS=',max_abs_mass
  write(*,'(A)') 'F_ROMV2_D16_REF_B14_MATERIAL_TRANSFER_GENERATED=FALSE'
  write(*,'(A)') 'F_ROMV2_D16_REF_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    call require(ok.and.state%ready(),'F-ROMV2 D16 REF initial committed state')
    ...
    do step=1,NSTEPS
      ...
      call require(ok,'F-ROMV2 D16 REF accepted history step')
      call require(abs(mass)<=hard_mass_gate,'F-ROMV2 D16 REF hard mass gate')
      call require(state%current_revision()==int(step,int64),'F-ROMV2 D16 REF exact revision progression')
    ...
    write(*,'(*(g0))') 'F_ROMV2_D16_REF_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
         '|RAIN_FACTOR=',history_rain_factor(ih),'|PULSE_STEPS=',history_pulse_steps(ih), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(time_ok.and.same_bits(time_before,t0),'F-ROMV2 D16 REF pre-attempt committed time')
    call state%snapshot(before,got)
    call require(got,'F-ROMV2 D16 REF pre-attempt snapshot')
    ...
    call require(bound_ok,'F_ROMV2_D16_REF integrated water-depth gates')
    ...
    call require(time_ok.and.same_bits(time_after,time_before),'F-ROMV2 D16 REF failed first attempt time immutable')
    call require(state%current_revision()==rev_before,'F-ROMV2 D16 REF failed first attempt revision immutable')
    call require(state%current_lineage_id()==lineage_before,'F-ROMV2 D16 REF failed first attempt lineage immutable')
    call state%snapshot(after,got)
    call require(got,'F-ROMV2 D16 REF failed first attempt snapshot')
    unchanged=state_bits_equal(before,after)
    call require(unchanged,'F-ROMV2 D16 REF failed first attempt physical state immutable')
    ...
    if (.not.ieee_is_finite(abs_integrated) .or. abs_integrated>total_integrated_gate .or. &
      ...
      write(*,'(*(g0))') 'F_ROMV2_D16_REF_POLICY_REJECT|CLASS=',trim(failure_class),'|T0=',t0,'|T1=',t1, &
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=total_integrated_gate, &
         'F_ROMV2_D16_REF total residual within integrated water-depth budget')
    call require(ieee_is_finite(local_integrated).and.local_integrated<=local_integrated_gate, &
         'F_ROMV2_D16_REF local residual within per-compartment integrated water-depth budget')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'F_ROMV2_D16_REF total-only flag contract')
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'F_ROMV2_D16_REF local-only flag contract')
    case default
      call require(.false.,'F_ROMV2_D16_REF failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'F-ROMV2 D16 REF qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'F_ROMV2_D16_REF_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    call require(got,'F-ROMV2 D16 REF state snapshot for output')
    ...
    type is(fmr_b110_physical_state_t)
      ...
      call require(all(ieee_is_finite(physical%pressure_head)).and.all(ieee_is_finite(physical%water_content)), &
           'F-ROMV2 D16 REF finite full profiles')
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'F_ROMV2_D16_REF_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'F-ROMV2 D16 REF expected B110 physical state')
    end select
  ...
  subroutine metrics_from_physical(physical,total,upper,lower)
    ...
    call require(ieee_is_finite(total).and.ieee_is_finite(upper).and.ieee_is_finite(lower),'F-ROMV2 D16 REF finite primary outputs')
  ...
  subroutine require_current_time(state,expected)
    ...
    call require(ok.and.same_bits(actual,expected),'F-ROMV2 D16 REF exact committed time progression')
  end subroutine require_current_time

  integer function history_symbol(ih,step) result(symbol)
    integer,intent(in) :: ih,step
    call require(ih>=1.and.ih<=NHIST.and.step>=1.and.step<=NSTEPS,'F-ROMV2 D16 REF history index')
    if(step<=history_pulse_steps(ih))then
      symbol=SYM_RAIN
    else
      symbol=SYM_HIATUS
    end if
  ...
  subroutine configure_step(ih,step,p,forcing)
    ...
    if(step<=history_pulse_steps(ih))then
      forcing%top_flux=-history_rain_factor(ih)*p%cofgen(3,1)
    else
  ...
  function history_label(ih) result(label)
    ...
    select case(ih)
    case(1);label='S10_P16_H48 '
    case(2);label='M25_P16_H48 '
    case(3);label='D50_P16_H48 '
    case(4);label='M50_P32_H32 '
    case default;label='BAD         '
    end select
  end function history_label

  pure real(real64) function history_rain_factor(ih) result(value)
    integer,intent(in) :: ih
    select case(ih)
    case(1);value=0.10_real64
    case(2);value=0.25_real64
    case(3);value=0.50_real64
    case(4);value=0.50_real64
    case default;value=-1.0_real64
    end select
  end function history_rain_factor

  pure integer function history_pulse_steps(ih) result(value)
    integer,intent(in) :: ih
    select case(ih)
    case(1:3);value=16
    case(4);value=32
  ...
  end function history_pulse_steps

  pure real(real64) function history_front_z101(ih) result(value)
    integer,intent(in) :: ih
    select case(ih)
    case(1);value=20.0_real64
    case(2);value=60.0_real64
    case(3);value=120.0_real64
    case(4);value=60.0_real64
    case default;value=-1.0_real64
    end select
  end function history_front_z101

  pure real(real64) function history_front_z199(ih) result(value)
    integer,intent(in) :: ih
    select case(ih)
    case(1);value=2.0_real64
    case(2);value=5.0_real64
    case(3);value=10.0_real64
    case(4);value=5.0_real64
    case default;value=-1.0_real64
    end select
  end function history_front_z199
  ...
  pure function symbol_label(symbol) result(label)
    ...
    select case(symbol)
    case(SYM_RAIN);label='RAIN_PULSE'
    case(SYM_HIATUS);label='HIATUS'
  ...
  subroutine diagnose_failed_interval(p,forcing,state,t0,t1,fmr_status,fmr_nl,fmr_back,class_name,bal,head,rmax,rsum,imax)
    ...
    call require(fmr_status==SW_SOLVE_RETRY_ADVISED,'F-ROMV2 D16 REF FMR failed status retry-advised')
    ...
    call state%snapshot(snap,got);call require(got,'F-ROMV2 D16 REF diagnostic snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
    ...
    class default
      call require(.false.,'F-ROMV2 D16 REF diagnostic B110 state')
    end select
    call bind_b110_serialized_legacy_context(request,context_ok);call require(context_ok,'F-ROMV2 D16 REF diagnostic context')
    call solver%solve(request,workspace,result)
    call require(result%status==SW_SOLVE_RETRY_ADVISED,'F-ROMV2 D16 REF direct retry advised')
    call require(trim(result%diagnostics%route)=='legacy-reference-retry','F-ROMV2 D16 REF direct retry route')
    call require(result%diagnostics%nonlinear_iterations==fmr_nl,'F-ROMV2 D16 REF direct/FMR nonlinear identity')
    call require(result%diagnostics%backtracking_attempts==fmr_back,'F-ROMV2 D16 REF direct/FMR backtracking identity')
  ...
  subroutine initialize_state(p,ih,h0,k0,state)
    ...
    real(real64) :: dtheta,theta_i,front,ztop,zbot,overlap,target_storage,se,mvg_m,z101,z199
    ...
    z101=history_front_z101(ih);z199=history_front_z199(ih)
    call require(z101>0.0_real64.and.z101<=160.0_real64.and.z199>0.0_real64.and.z199<=z101, &
         'F-ROMV2 D16 REF initial front family bounds')
    mvg_m=1.0_real64-1.0_real64/p%cofgen(6,1)
    target_storage=theta_i*160.0_real64
    do j=fmc_j0,fmc_j1
      front=z101+(z199-z101)*real(j-fmc_j0,real64)/real(fmc_j1-fmc_j0,real64)
      target_storage=target_storage+dtheta*front
    end do
    ...
    do node=1,numnod
      ...
      call require(ieee_is_finite(heads(node)).and.heads(node)<0.0_real64, &
           'F-ROMV2 D16 REF initial inverse retention')
      ztop=zbot
    end do

    call require(abs(sum(water*dz(1:numnod))-target_storage)<=1.0e-12_real64, &
         'F-ROMV2 D16 REF exact initial FMC storage mapping')
    ...
    write(*,'(*(g0))') 'F_ROMV2_D16_REF_INITIAL|HISTORY=',trim(history_label(ih)), &
         '|RAIN_FACTOR=',history_rain_factor(ih),'|PULSE_STEPS=',history_pulse_steps(ih), &
         '|Z101=',z101,'|Z199=',z199,'|TOTAL_STORAGE=',sum(water*dz(1:numnod)), &
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'F_ROMV2_D16_REF_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_f_romv2_d16_surface_comparators
