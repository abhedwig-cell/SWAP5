program test_f_romv2_d16_surface_comparators
  ...
  write(*,'(A,I0)') 'F_ROMV2_D16_REF_ZERO_BOTTOM_FLUX_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'F_ROMV2_D16_REF_STATE_COUNT=',total_states
  write(*,'(A,I0)') 'F_ROMV2_D16_REF_DEVELOPMENT_STATE_COUNT=',NHIST*NSTEPS
  write(*,'(A,I0)') 'F_ROMV2_D16_REF_ZERO_BOTTOM_FLUX_STATE_COUNT=',NHIST*NSTEPS
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    do step=1,NSTEPS
      ...
      call require(abs(bex)<=hard_mass_gate,'F-ROMV2 D16 REF zero bottom exchange')
      call require(abs(bflux)<=hard_mass_gate,'F-ROMV2 D16 REF zero terminal bottom flux')
    ...
    write(*,'(*(g0))') 'F_ROMV2_D16_REF_HISTORY_PASS|SPLIT=DEVELOPMENT|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine emit_initial(ih,state)
    ...
    call require(got,'F-ROMV2 D16 REF initial snapshot')
    ...
    type is(fmr_b110_physical_state_t)
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'F_ROMV2_D16_REF_INITIAL_NODE|HISTORY=',trim(history_label(ih)), &
             '|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'F-ROMV2 D16 REF expected initial B110 state')
    end select
  ...
  subroutine emit_state(ih,step,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    type is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'F_ROMV2_D16_REF_STATE|SPLIT=DEVELOPMENT|HISTORY=',trim(history_label(ih)), &
           '|STEP=',step,'|REL_T=',real(step,real64)*step_dt,'|T=',t1,'|REV=',state%current_revision(), &
           '|PHASE=',trim(phase_label(step)),'|TOP_FACTOR=',history_factor(ih),'|BOTTOM_MODE=',2, &
           '|TOTAL_STORAGE=',total,'|UPPER_STORAGE=',upper,'|LOWER_STORAGE=',lower, &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'F_ROMV2_D16_REF_NODE|SPLIT=DEVELOPMENT|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine seed_steady(column,template,p,state,forcing,qeq,ok)
    ...
    do i=1,seed_intervals
      ...
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      if(.not.ok)return
      call require(abs(mass)<=hard_mass_gate,'F-ROMV2 D16 REF seed hard mass gate')
      t0=t1
    end do
  ...
  subroutine initialize_state(p,state)
    ...
    do j=fmc_j0,fmc_j1
      ...
      call require(zfront>0.0_real64.and.zfront<160.0_real64,'F-ROMV2 D16 REF initial front bounds')
