program test_f_romv2_d17_combined_comparators
  ...
  integer, parameter :: NHIST=4, NSTEPS=16
  ...
  call require(any(numnod==[2,16]),'F_ROMV2_D17_REF geometry is R2 or R16')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'F_ROMV2_D17_REF depth frozen')
  write(*,'(*(g0))') 'F_ROMV2_D17_REF_GEOMETRY|N=',numnod,'|DZ_CM=',dz(1),'|DEPTH_CM=',sum(dz(1:numnod))
  ...
  call require(total_states==NHIST*NSTEPS,'F_ROMV2_D17_REF exact library state count')
  write(*,'(A,I0)') 'F_ROMV2_D17_REF_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'F_ROMV2_D17_REF_DEVELOPMENT_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'F_ROMV2_D17_REF_FIXED_WATER_TABLE_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'F_ROMV2_D17_REF_STATE_COUNT=',total_states
  write(*,'(A,I0)') 'F_ROMV2_D17_REF_DEVELOPMENT_STATE_COUNT=',NHIST*NSTEPS
  write(*,'(A,I0)') 'F_ROMV2_D17_REF_FIXED_WATER_TABLE_STATE_COUNT=',NHIST*NSTEPS
  write(*,'(A,I0)') 'F_ROMV2_D17_REF_TOTAL_FALLBACK_COUNT=',total_fallbacks
  write(*,'(*(g0))') 'F_ROMV2_D17_REF_MAX_ABS_MASS=',max_abs_mass
  write(*,'(A)') 'F_ROMV2_D17_REF_B14_MATERIAL_TRANSFER_GENERATED=FALSE'
  write(*,'(A)') 'F_ROMV2_D17_REF_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    call require(ok.and.state%ready(),'F-ROMV2 D17 REF initial committed state')
    ...
    do step=1,NSTEPS
      ...
      call require(ok,'F-ROMV2 D17 REF accepted history step')
      call require(abs(mass)<=hard_mass_gate,'F-ROMV2 D17 REF hard mass gate')
      call require(state%current_revision()==int(step,int64),'F-ROMV2 D17 REF exact revision progression')
    ...
    write(*,'(*(g0))') 'F_ROMV2_D17_REF_HISTORY_PASS|SPLIT=DEVELOPMENT|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(time_ok.and.same_bits(time_before,t0),'F-ROMV2 D17 REF pre-attempt committed time')
    call state%snapshot(before,got)
    call require(got,'F-ROMV2 D17 REF pre-attempt snapshot')
    ...
    call require(bound_ok,'F_ROMV2_D17_REF integrated water-depth gates')
    ...
    call require(time_ok.and.same_bits(time_after,time_before),'F-ROMV2 D17 REF failed first attempt time immutable')
    call require(state%current_revision()==rev_before,'F-ROMV2 D17 REF failed first attempt revision immutable')
    call require(state%current_lineage_id()==lineage_before,'F-ROMV2 D17 REF failed first attempt lineage immutable')
    call state%snapshot(after,got)
    call require(got,'F-ROMV2 D17 REF failed first attempt snapshot')
    unchanged=state_bits_equal(before,after)
    call require(unchanged,'F-ROMV2 D17 REF failed first attempt physical state immutable')
    ...
    if (.not.ieee_is_finite(abs_integrated) .or. abs_integrated>total_integrated_gate .or. &
      ...
      write(*,'(*(g0))') 'F_ROMV2_D17_REF_POLICY_REJECT|CLASS=',trim(failure_class),'|T0=',t0,'|T1=',t1, &
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=total_integrated_gate, &
         'F_ROMV2_D17_REF total residual within integrated water-depth budget')
    call require(ieee_is_finite(local_integrated).and.local_integrated<=local_integrated_gate, &
         'F_ROMV2_D17_REF local residual within per-compartment integrated water-depth budget')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'F_ROMV2_D17_REF total-only flag contract')
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'F_ROMV2_D17_REF local-only flag contract')
    case default
      call require(.false.,'F_ROMV2_D17_REF failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'F-ROMV2 D17 REF qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'F_ROMV2_D17_REF_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_initial(ih,state)
    ...
    call require(got,'F-ROMV2 D17 REF initial snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      call metrics_from_physical(physical,total,upper,lower)
      write(*,'(*(g0))') 'F_ROMV2_D17_REF_INITIAL|HISTORY=',trim(history_label(ih)), &
           '|TOTAL_STORAGE=',total,'|UPPER_STORAGE=',upper,'|LOWER_STORAGE=',lower
      do node=1,numnod
        write(*,'(*(g0))') 'F_ROMV2_D17_REF_INITIAL_NODE|HISTORY=',trim(history_label(ih)), &
             '|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'F-ROMV2 D17 REF expected initial B110 state')
    end select
  ...
  subroutine emit_state(ih,step,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    integer :: node
    call state%snapshot(snap,got)
    call require(got,'F-ROMV2 D17 REF state snapshot for output')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      call metrics_from_physical(physical,total,upper,lower)
      call require(all(ieee_is_finite(physical%pressure_head)).and.all(ieee_is_finite(physical%water_content)), &
           'F-ROMV2 D17 REF finite full profiles')
      call require(all(physical%water_content>=theta_r_b01).and.all(physical%water_content<=theta_s_b01), &
           'F-ROMV2 D17 REF physical water-content bounds')
      write(*,'(*(g0))') 'F_ROMV2_D17_REF_STATE|SPLIT=DEVELOPMENT|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'F_ROMV2_D17_REF_NODE|SPLIT=DEVELOPMENT|HISTORY=',trim(history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'F-ROMV2 D17 REF expected B110 physical state')
    end select
    if(allocated(snap))deallocate(snap)
  ...
  subroutine metrics_from_physical(physical,total,upper,lower)
    ...
    call require(ieee_is_finite(total).and.ieee_is_finite(upper).and.ieee_is_finite(lower),'F-ROMV2 D17 REF finite primary outputs')
  ...
  subroutine require_current_time(state,expected)
    ...
    call require(ok.and.same_bits(actual,expected),'F-ROMV2 D17 REF exact committed time progression')
  ...
  subroutine configure_step(ih,step,p,forcing)
    ...
    call require(ih>=1.and.ih<=NHIST.and.step>=1.and.step<=NSTEPS,'F-ROMV2 D17 REF history index')
    p%total_balance_tolerance=original_total_tol
    p%bottom_mode=5
    forcing%top_head=0.0_real64
    forcing%top_flux=-history_factor(ih)*p%cofgen(3,1)
  ...
  function history_label(ih) result(label)
    ...
    select case(ih)
    case(1);label='C10_G25'
    case(2);label='C25_G25'
    case(3);label='C50_G25'
    case(4);label='C75_G25'
  ...
  subroutine seed_steady(column,template,p,state,forcing,qeq,ok)
    ...
    do i=1,seed_intervals
      ...
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      if(.not.ok)return
      call require(abs(mass)<=hard_mass_gate,'F-ROMV2 D17 REF seed hard mass gate')
      t0=t1
    end do
  ...
  subroutine diagnose_failed_interval(p,forcing,state,t0,t1,fmr_status,fmr_nl,fmr_back,class_name,bal,head,rmax,rsum,imax)
    ...
    call require(fmr_status==SW_SOLVE_RETRY_ADVISED,'F-ROMV2 D17 REF FMR failed status retry-advised')
    ...
    call state%snapshot(snap,got);call require(got,'F-ROMV2 D17 REF diagnostic snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
    ...
    class default
      call require(.false.,'F-ROMV2 D17 REF diagnostic B110 state')
    end select
    call bind_b110_serialized_legacy_context(request,context_ok);call require(context_ok,'F-ROMV2 D17 REF diagnostic context')
    call solver%solve(request,workspace,result)
    call require(result%status==SW_SOLVE_RETRY_ADVISED,'F-ROMV2 D17 REF direct retry advised')
    call require(trim(result%diagnostics%route)=='legacy-reference-retry','F-ROMV2 D17 REF direct retry route')
    call require(result%diagnostics%nonlinear_iterations==fmr_nl,'F-ROMV2 D17 REF direct/FMR nonlinear identity')
    call require(result%diagnostics%backtracking_attempts==fmr_back,'F-ROMV2 D17 REF direct/FMR backtracking identity')
  ...
  subroutine initialize_state(p,state)
    ...
    do j=fmc_j0,fmc_j1
      ...
      call require(zfront>0.0_real64.and.zfront<160.0_real64,'F-ROMV2 D17 REF initial surface-front bounds')
      call require(hj>0.0_real64.and.hj<160.0_real64,'F-ROMV2 D17 REF initial groundwater-front bounds')
      call require(zfront+hj<160.0_real64,'F-ROMV2 D17 REF initial surface-groundwater separation')
    ...
    do node=1,numnod
      ...
      do j=fmc_j0,fmc_j1
        ...
        call require(surf_overlap+gw_overlap<=p%dz(node)+1.0e-12_real64, &
             'F-ROMV2 D17 REF no double-counted cell occupancy')
      ...
      call require(water(node)>theta_r_b01.and.water(node)<theta_s_b01, &
           'F-ROMV2 D17 REF initial cell-average theta open bounds')
      ...
      call require(ieee_is_finite(heads(node)).and.heads(node)<0.0_real64, &
           'F-ROMV2 D17 REF initial inverse retention')
      ztop=zbot
    end do
    call require(abs(sum(water*dz(1:numnod))-target_storage)<=1.0e-12_real64, &
         'F-ROMV2 D17 REF exact composite FMC storage mapping')
    write(*,'(*(g0))') 'F_ROMV2_D17_REF_INITIAL_COMPOSITE|TARGET_STORAGE=',target_storage, &
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'F_ROMV2_D17_REF_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_f_romv2_d17_combined_comparators
