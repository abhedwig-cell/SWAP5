program test_f_romv2_d5_r16_reference
  ...
  call require(any(numnod==[2,4,8,16]),'F_ROMV2_D5_REF geometry is one of R2/R4/R8/R16')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'F_ROMV2_D5_REF depth frozen')
  write(*,'(*(g0))') 'F_ROMV2_D5_REF_GEOMETRY|N=',numnod,'|DZ_CM=',dz(1),'|DEPTH_CM=',sum(dz(1:numnod))
  ...
  call require(total_states==NHIST*NSTEPS,'F_ROMV2_D5_REF exact library state count')
  write(*,'(A,I0)') 'F_ROMV2_D5_REF_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'F_ROMV2_D5_REF_TRAINING_HISTORY_COUNT=',8
  write(*,'(A,I0)') 'F_ROMV2_D5_REF_VALIDATION_HISTORY_COUNT=',4
  write(*,'(A,I0)') 'F_ROMV2_D5_REF_STATE_COUNT=',total_states
  write(*,'(A,I0)') 'F_ROMV2_D5_REF_TRAINING_STATE_COUNT=',8*NSTEPS
  write(*,'(A,I0)') 'F_ROMV2_D5_REF_VALIDATION_STATE_COUNT=',4*NSTEPS
  write(*,'(A,I0)') 'F_ROMV2_D5_REF_TOTAL_FALLBACK_COUNT=',total_fallbacks
  write(*,'(*(g0))') 'F_ROMV2_D5_REF_MAX_ABS_MASS=',max_abs_mass
  write(*,'(A)') 'F_ROMV2_D5_REF_B14_MATERIAL_TRANSFER_GENERATED=FALSE'
  write(*,'(A)') 'F_ROMV2_D5_REF_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    call require(ok.and.state%ready(),'F-ROMV2 D5 REF initial committed state')

    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'F-ROMV2 D5 REF gravity-consistent seed')
    ...
    do step=1,NSTEPS
      ...
      call require(ok,'F-ROMV2 D5 REF accepted history step')
      call require(abs(mass)<=hard_mass_gate,'F-ROMV2 D5 REF hard mass gate')
      call require(state%current_revision()==int(seed_intervals+step,int64),'F-ROMV2 D5 REF exact revision progression')
    ...
    write(*,'(*(g0))') 'F_ROMV2_D5_REF_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(time_ok.and.same_bits(time_before,t0),'F-ROMV2 D5 REF pre-attempt committed time')
    call state%snapshot(before,got)
    call require(got,'F-ROMV2 D5 REF pre-attempt snapshot')
    ...
    call require(bound_ok,'F_ROMV2_D5_REF integrated water-depth gates')
    ...
    call require(time_ok.and.same_bits(time_after,time_before),'F-ROMV2 D5 REF failed first attempt time immutable')
    call require(state%current_revision()==rev_before,'F-ROMV2 D5 REF failed first attempt revision immutable')
    call require(state%current_lineage_id()==lineage_before,'F-ROMV2 D5 REF failed first attempt lineage immutable')
    call state%snapshot(after,got)
    call require(got,'F-ROMV2 D5 REF failed first attempt snapshot')
    unchanged=state_bits_equal(before,after)
    call require(unchanged,'F-ROMV2 D5 REF failed first attempt physical state immutable')
    ...
    if (.not.ieee_is_finite(abs_integrated) .or. abs_integrated>total_integrated_gate .or. &
      ...
      write(*,'(*(g0))') 'F_ROMV2_D5_REF_POLICY_REJECT|CLASS=',trim(failure_class),'|T0=',t0,'|T1=',t1, &
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=total_integrated_gate, &
         'F_ROMV2_D5_REF total residual within integrated water-depth budget')
    call require(ieee_is_finite(local_integrated).and.local_integrated<=local_integrated_gate, &
         'F_ROMV2_D5_REF local residual within per-compartment integrated water-depth budget')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'F_ROMV2_D5_REF total-only flag contract')
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'F_ROMV2_D5_REF local-only flag contract')
    case default
      call require(.false.,'F_ROMV2_D5_REF failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'F-ROMV2 D5 REF qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'F_ROMV2_D5_REF_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    call require(got,'F-ROMV2 D5 REF state snapshot for output')
    ...
    type is(fmr_b110_physical_state_t)
      ...
      call require(all(ieee_is_finite(physical%pressure_head)).and.all(ieee_is_finite(physical%water_content)), &
           'F-ROMV2 D5 REF finite full profiles')
      call require(all(physical%water_content>=theta_r_b01).and.all(physical%water_content<=theta_s_b01), &
           'F-ROMV2 D5 REF physical water-content bounds')
      write(*,'(*(g0))') 'F_ROMV2_D5_REF_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'F_ROMV2_D5_REF_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'F-ROMV2 D5 REF expected B110 physical state')
    end select
  ...
  subroutine metrics_from_physical(physical,total,upper,lower)
    ...
    call require(ieee_is_finite(total).and.ieee_is_finite(upper).and.ieee_is_finite(lower),'F-ROMV2 D5 REF finite primary outputs')
  ...
  subroutine require_current_time(state,expected)
    ...
    call require(ok.and.same_bits(actual,expected),'F-ROMV2 D5 REF exact committed time progression')
  ...
  integer function history_symbol(ih,step) result(symbol)
    ...
    call require(symbol>=SYM_HOLD.and.symbol<=SYM_COMBINED_FALL_MINUS,'F-ROMV2 D5 REF valid history symbol')
  ...
  subroutine configure_symbol(symbol,h0,k0,qeq,p,forcing)
    ...
    case default
      call require(.false.,'F-ROMV2 D5 REF configure known symbol')
    end select
  ...
  subroutine diagnose_failed_interval(p,forcing,state,t0,t1,fmr_status,fmr_nl,fmr_back,class_name,bal,head,rmax,rsum,imax)
    ...
    call require(fmr_status==SW_SOLVE_RETRY_ADVISED,'F-ROMV2 D5 REF FMR failed status retry-advised')
    ...
    call state%snapshot(snap,got);call require(got,'F-ROMV2 D5 REF diagnostic snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
    ...
    class default
      call require(.false.,'F-ROMV2 D5 REF diagnostic B110 state')
    end select
    call bind_b110_serialized_legacy_context(request,context_ok);call require(context_ok,'F-ROMV2 D5 REF diagnostic context')
    call solver%solve(request,workspace,result)
    call require(result%status==SW_SOLVE_RETRY_ADVISED,'F-ROMV2 D5 REF direct retry advised')
    call require(trim(result%diagnostics%route)=='legacy-reference-retry','F-ROMV2 D5 REF direct retry route')
    call require(result%diagnostics%nonlinear_iterations==fmr_nl,'F-ROMV2 D5 REF direct/FMR nonlinear identity')
    call require(result%diagnostics%backtracking_attempts==fmr_back,'F-ROMV2 D5 REF direct/FMR backtracking identity')
  ...
  subroutine initialize_state(p,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'F-ROMV2 D5 REF finite seed')
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'F_ROMV2_D5_REF_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_f_romv2_d5_r16_reference
