program test_lare_bc2_a2_reference_geometry
  ...
  integer, parameter :: NHIST=4, NSTEPS=768
  ...
  call require(numnod==16,'LAREBC2A2 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'LAREBC2A2 depth frozen')
  total_states=0;total_fallbacks=0;max_abs_mass=0.0_real64

  do ih=1,NHIST
    call run_history(ih,total_states,max_abs_mass)
  end do

  call require(total_states==2048,'LAREBC2A2 exact library state count')
  write(*,'(A,I0)') 'LAREBC2A2_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'LAREBC2A2_STATE_COUNT=',total_states
  write(*,'(*(g0))') 'LAREBC2A2_MAX_ABS_MASS=',max_abs_mass
  write(*,'(A)') 'LAREBC2A2_EXECUTION_COMPLETE=PASS'

contains

  subroutine run_history(ih,total_states,max_abs_mass)
    integer,intent(in) :: ih
    integer,intent(inout) :: total_states
    ...
    integer :: step,status,nl,ir,back,nsteps
    ...
    call initialize_hydrostatic_state(p,120.0_real64,initial_state)
    call initialize_identity(column,template)
    call initialize_forcing(forcing,0.0_real64,0.0_real64,40.0_real64)
    call fmr_new_b110_committed_state(state,column_id,initial_state,0.0_real64,ok)
    call require(ok.and.state%ready(),'LAREBC2A2 initial committed state')
    call emit_initial(ih,state)
    nsteps=history_steps(ih)
    t0=0.0_real64

    do step=1,nsteps
      call configure_bc2_history(ih,step,p,forcing)
      ...
      call require(ok,'LAREBC2A2 accepted history step')
      call require(abs(mass)<=hard_mass_gate,'LAREBC2A2 hard mass gate')
      call require_current_time(state,t1)
      max_abs_mass=max(max_abs_mass,abs(mass))
      total_states=total_states+1
      call emit_bc2_state(ih,step,state,forcing,t0,t1,mass,bex,bflux,nl,back)
      t0=t1
    end do
    write(*,'(*(g0))') 'LAREBC2A2_HISTORY_PASS|HISTORY=',trim(bc2_history_label(ih)), &
         '|STATES=',nsteps,'|FINAL_REV=',state%current_revision(),'|FINAL_T=',t0
  end subroutine run_history

  pure integer function history_steps(ih) result(n)
    integer,intent(in) :: ih
    select case(ih)
    case(1); n=256
    case(2); n=512
  ...
  end function history_steps

  pure function bc2_history_label(ih) result(label)
    integer,intent(in) :: ih
    character(len=8) :: label
    select case(ih)
    case(1); label='WT_HOLD '
    case(2); label='WT_RISE '
    case(3); label='WT_FALL '
    case(4); label='WT_CYCLE'
    case default; label='INVALID '
    end select
  end function bc2_history_label

  subroutine configure_bc2_history(ih,step,p,forcing)
    ...
    select case(ih)
    case(1)
      forcing%bottom_head=40.0_real64
    case(2)
      forcing%bottom_head=60.0_real64
    case(3)
      forcing%bottom_head=20.0_real64
    case(4)
      if(step<=256)then
        forcing%bottom_head=60.0_real64
      else if(step<=512)then
        forcing%bottom_head=20.0_real64
      else
        forcing%bottom_head=40.0_real64
      end if
    case default
      call require(.false.,'LAREBC2A2 known history')
    end select
  end subroutine configure_bc2_history
  ...
  subroutine emit_initial(ih,state)
    ...
    integer :: node
    real(real64) :: total
    call state%snapshot(snap,got);call require(got,'LAREBC2A2 initial snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      total=sum(physical%water_content(1:numnod)*dz(1:numnod))
      write(*,'(*(g0))') 'LAREBC2A2_INITIAL|HISTORY=',trim(bc2_history_label(ih)), &
           '|TOTAL_STORAGE=',total
      do node=1,numnod
        write(*,'(*(g0))') 'LAREBC2A2_INITIAL_NODE|HISTORY=',trim(bc2_history_label(ih)), &
             '|NODE=',node,'|Z=',z(node),'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'LAREBC2A2 expected initial physical state')
    end select
    if(allocated(snap))deallocate(snap)
  end subroutine emit_initial

  subroutine emit_bc2_state(ih,step,state,forcing,t0,t1,mass,bex,bflux,nl,back)
    ...
    integer :: node
    real(real64) :: total
    call state%snapshot(snap,got);call require(got,'LAREBC2A2 state snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      total=sum(physical%water_content(1:numnod)*dz(1:numnod))
      write(*,'(*(g0))') 'LAREBC2A2_STATE|HISTORY=',trim(bc2_history_label(ih)), &
           '|STEP=',step,'|T0=',t0,'|T1=',t1,'|BOTTOM_HEAD=',forcing%bottom_head, &
           '|TOTAL_STORAGE=',total,'|BOTTOM_OUTWARD_EXCHANGE=',bex,'|BOTTOM_FLUX=',bflux, &
           '|MASS=',mass,'|NL=',nl,'|BACKTRACK=',back
      do node=1,numnod
        write(*,'(*(g0))') 'LAREBC2A2_NODE|HISTORY=',trim(bc2_history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|Z=',z(node),'|H=',physical%pressure_head(node), &
             '|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'LAREBC2A2 expected physical state')
    end select
    if(allocated(snap))deallocate(snap)
  end subroutine emit_bc2_state
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(time_ok.and.same_bits(time_before,t0),'LAREBC2A2 pre-attempt committed time')
    call state%snapshot(before,got)
    call require(got,'LAREBC2A2 pre-attempt snapshot')
    ...
    call require(time_ok.and.same_bits(time_after,time_before),'LAREBC2A2 failed first attempt time immutable')
    call require(state%current_revision()==rev_before,'LAREBC2A2 failed first attempt revision immutable')
    call require(state%current_lineage_id()==lineage_before,'LAREBC2A2 failed first attempt lineage immutable')
    call state%snapshot(after,got)
    call require(got,'LAREBC2A2 failed first attempt snapshot')
    unchanged=state_bits_equal(before,after)
    call require(unchanged,'LAREBC2A2 failed first attempt physical state immutable')
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=rep_bound, &
         'LAREBC2A2 total residual within pre-solve bound')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'LAREBC2A2 total-only flag contract')
      call require(rmax<=p%compartment_balance_tolerance,'LAREBC2A2 total-only local criterion unchanged')
      fallback_p%compartment_balance_tolerance=p%compartment_balance_tolerance
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'LAREBC2A2 local-only flag contract')
      call require(ieee_is_finite(local_integrated).and.local_integrated<=1.6e-15_real64, &
           'LAREBC2A2 local residual within prior integrated allowance')
    ...
    case default
      call require(.false.,'LAREBC2A2 failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'LAREBC2A2 qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'LAREBC2A2_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    call require(got,'LAREBC2A2 state snapshot for output')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      ...
      call require(all(ieee_is_finite(physical%pressure_head)).and.all(ieee_is_finite(physical%water_content)), &
           'LAREBC2A2 finite full profiles')
      write(*,'(*(g0))') 'LAREBC2A2_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'LAREBC2A2_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'LAREBC2A2 expected B110 physical state')
    end select
    if(allocated(snap))deallocate(snap)
  ...
  subroutine metrics_from_physical(physical,total,upper,lower)
    ...
    call require(ieee_is_finite(total).and.ieee_is_finite(upper).and.ieee_is_finite(lower),'LAREBC2A2 finite primary outputs')
  ...
  subroutine require_current_time(state,expected)
    ...
    call require(ok.and.same_bits(actual,expected),'LAREBC2A2 exact committed time progression')
  ...
  integer function history_symbol(ih,step) result(symbol)
    ...
    call require(symbol>=SYM_HOLD.and.symbol<=SYM_COMBINED_FALL_MINUS,'LAREBC2A2 valid history symbol')
  ...
  subroutine configure_symbol(symbol,h0,k0,qeq,p,forcing)
    ...
    case default
      call require(.false.,'LAREBC2A2 configure known symbol')
    end select
  ...
  subroutine diagnose_failed_interval(p,forcing,state,t0,t1,fmr_status,fmr_nl,fmr_back,class_name,bal,head,rmax,rsum,imax)
    ...
    call require(fmr_status==SW_SOLVE_RETRY_ADVISED,'LAREBC2A2 FMR failed status retry-advised')
    ...
    call state%snapshot(snap,got);call require(got,'LAREBC2A2 diagnostic snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
    ...
    class default
      call require(.false.,'LAREBC2A2 diagnostic B110 state')
    end select
    call bind_b110_serialized_legacy_context(request,context_ok);call require(context_ok,'LAREBC2A2 diagnostic context')
    call solver%solve(request,workspace,result)
    call require(result%status==SW_SOLVE_RETRY_ADVISED,'LAREBC2A2 direct retry advised')
    call require(trim(result%diagnostics%route)=='legacy-reference-retry','LAREBC2A2 direct retry route')
    call require(result%diagnostics%nonlinear_iterations==fmr_nl,'LAREBC2A2 direct/FMR nonlinear identity')
    call require(result%diagnostics%backtracking_attempts==fmr_back,'LAREBC2A2 direct/FMR backtracking identity')
  ...
  subroutine initialize_hydrostatic_state(p,H0,state)
    type(fmr_b110_physical_parameters_t),intent(in) :: p
    ...
    integer :: node
    do node=1,numnod
      heads(node)=-H0-z(node)
    end do
    call initialize_b110_default_mvg_parameters(hp,p%cofgen)
    call bind_b110_default_mvg_provider(provider,hp,step_dt)
    call provider%evaluate(heads,water,conductivity,capacity,dkdh)
    call require(all(ieee_is_finite(water)).and.all(ieee_is_finite(heads)),'LAREBC2A2 finite hydrostatic seed')
    call require(any(heads<0.0_real64).and.any(heads>0.0_real64),'LAREBC2A2 interior initial water table')
    state%active_nodes=numnod
    allocate(state%pressure_head(numnod),state%water_content(numnod))
    state%pressure_head=heads;state%water_content=water
    state%ponding_depth=0.0_real64
    state%groundwater_level=-H0
  end subroutine initialize_hydrostatic_state
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'LAREBC2A2_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_lare_bc2_a2_reference_geometry