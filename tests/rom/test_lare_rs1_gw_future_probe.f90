program test_lare_rs1_gw_future_probe
  ...
  integer, parameter :: horizon_steps(NHORIZ)=[4,64,256,1024]
  integer :: start_unit,ios,ih,target_step,start_count
  character(len=512) :: starts_file

  call require(numnod==16,'LAREGW1P geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'LAREGW1P depth frozen')
  call get_environment_variable('LARE_RS1_GW_STARTS_FILE',starts_file)
  call require(len_trim(starts_file)>0,'LAREGW1P start request file supplied')
  open(newunit=start_unit,file=trim(starts_file),status='old',action='read',iostat=ios)
  call require(ios==0,'LAREGW1P start request file opened')
  start_count=0
  do
    read(start_unit,*,iostat=ios) ih,target_step
    if(ios<0)exit
    call require(ios==0,'LAREGW1P start request row readable')
    call require(ih>=1.and.ih<=NHIST,'LAREGW1P history index')
    call require(target_step>=1.and.target_step<=NSTEPS,'LAREGW1P step index')
    call run_start_request(ih,target_step)
    start_count=start_count+1
  end do
  close(start_unit)
  call require(start_count>0,'LAREGW1P nonempty frozen start set')
  write(*,'(A,I0)') 'LAREGW1P_UNIQUE_START_COUNT=',start_count
  write(*,'(A)') 'LAREGW1P_HELDOUT_USED=FALSE'
  write(*,'(A)') 'LAREGW1P_B14_USED=FALSE'
  write(*,'(A)') 'LAREGW1P_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    call require(ok.and.state%ready(),'LAREGW1P initial committed state')

    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'LAREGW1P gravity-consistent seed')
    ...
    do step=1,NSTEPS
      ...
      call require(ok,'LAREGW1P accepted history step')
      call require(abs(mass)<=hard_mass_gate,'LAREGW1P hard mass gate')
      call require(state%current_revision()==int(seed_intervals+step,int64),'LAREGW1P exact revision progression')
    ...
    write(*,'(*(g0))') 'LAREGW1P_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(time_ok.and.same_bits(time_before,t0),'LAREGW1P pre-attempt committed time')
    call state%snapshot(before,got)
    call require(got,'LAREGW1P pre-attempt snapshot')
    ...
    call require(time_ok.and.same_bits(time_after,time_before),'LAREGW1P failed first attempt time immutable')
    call require(state%current_revision()==rev_before,'LAREGW1P failed first attempt revision immutable')
    call require(state%current_lineage_id()==lineage_before,'LAREGW1P failed first attempt lineage immutable')
    call state%snapshot(after,got)
    call require(got,'LAREGW1P failed first attempt snapshot')
    unchanged=state_bits_equal(before,after)
    call require(unchanged,'LAREGW1P failed first attempt physical state immutable')
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=rep_bound, &
         'LAREGW1P total residual within pre-solve bound')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'LAREGW1P total-only flag contract')
      call require(rmax<=p%compartment_balance_tolerance,'LAREGW1P total-only local criterion unchanged')
      fallback_p%compartment_balance_tolerance=p%compartment_balance_tolerance
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'LAREGW1P local-only flag contract')
      call require(ieee_is_finite(local_integrated).and.local_integrated<=1.6e-15_real64, &
           'LAREGW1P local residual within prior integrated allowance')
    ...
    case default
      call require(.false.,'LAREGW1P failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'LAREGW1P qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'LAREGW1P_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    call require(got,'LAREGW1P state snapshot for output')
    ...
    type is(fmr_b110_physical_state_t)
      ...
      call require(all(ieee_is_finite(physical%pressure_head)).and.all(ieee_is_finite(physical%water_content)), &
           'LAREGW1P finite full profiles')
      write(*,'(*(g0))') 'LAREGW1P_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'LAREGW1P_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'LAREGW1P expected B110 physical state')
    end select
  ...
  subroutine metrics_from_physical(physical,total,upper,lower)
    ...
    call require(ieee_is_finite(total).and.ieee_is_finite(upper).and.ieee_is_finite(lower),'LAREGW1P finite primary outputs')
  ...
  subroutine require_current_time(state,expected)
    ...
    call require(ok.and.same_bits(actual,expected),'LAREGW1P exact committed time progression')
  ...
  integer function history_symbol(ih,step) result(symbol)
    ...
    call require(symbol>=SYM_HOLD.and.symbol<=SYM_COMBINED_FALL_MINUS,'LAREGW1P valid history symbol')
  ...
  subroutine configure_symbol(symbol,h0,k0,qeq,p,forcing)
    ...
    case default
      call require(.false.,'LAREGW1P configure known symbol')
    end select
  ...
  subroutine diagnose_failed_interval(p,forcing,state,t0,t1,fmr_status,fmr_nl,fmr_back,class_name,bal,head,rmax,rsum,imax)
    ...
    call require(fmr_status==SW_SOLVE_RETRY_ADVISED,'LAREGW1P FMR failed status retry-advised')
    ...
    call state%snapshot(snap,got);call require(got,'LAREGW1P diagnostic snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
    ...
    class default
      call require(.false.,'LAREGW1P diagnostic B110 state')
    end select
    call bind_b110_serialized_legacy_context(request,context_ok);call require(context_ok,'LAREGW1P diagnostic context')
    call solver%solve(request,workspace,result)
    call require(result%status==SW_SOLVE_RETRY_ADVISED,'LAREGW1P direct retry advised')
    call require(trim(result%diagnostics%route)=='legacy-reference-retry','LAREGW1P direct retry route')
    call require(result%diagnostics%nonlinear_iterations==fmr_nl,'LAREGW1P direct/FMR nonlinear identity')
    call require(result%diagnostics%backtracking_attempts==fmr_back,'LAREGW1P direct/FMR backtracking identity')
  ...
  subroutine initialize_state(p,se_anchor,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'LAREGW1P finite seed')
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'LAREGW1P_FAIL',trim(label)
      error stop 1
    end if
  ...
  subroutine run_start_request(ih,target_step)
    ...
    write(*,'(*(g0))') 'LAREGW1P_START|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
    ...
    do node=1,numnod
      write(*,'(*(g0))') 'LAREGW1P_START_NODE|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
  ...
  subroutine reconstruct_start_state(ih,target_step,physical_out,h0,k0,qeq)
    ...
    call initialize_forcing(forcing,qeq,qeq,h0)
    call fmr_new_b110_committed_state(state,column_id,initial_state,0.0_real64,ok)
    call require(ok.and.state%ready(),'LAREGW1P start initial committed state')
    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'LAREGW1P start steady seed')
    t0=real(seed_intervals,real64)*seed_dt
    do step=1,target_step
      ...
      t1=real(seed_intervals,real64)*seed_dt+real(step,real64)*step_dt
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'LAREGW1P start reconstruction accepted')
      call require(abs(mass)<=hard_mass_gate,'LAREGW1P start reconstruction mass')
      t0=t1
    end do
    call state%snapshot(snap,got)
    call require(got,'LAREGW1P start snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      physical_out=physical
    class default
      call require(.false.,'LAREGW1P expected B110 start state')
    end select
    if(allocated(snap))deallocate(snap)
  ...
  subroutine run_probe(ih,target_step,ip,start_physical,h0,k0,qeq,start_total,start_upper,start_lower)
    ...
    call require(ok.and.state%ready(),'LAREGW1P probe committed start')
    ...
    do step=1,NSTEPS
      ...
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'LAREGW1P frozen future probe accepted')
      call require(abs(mass)<=hard_mass_gate,'LAREGW1P probe hard mass gate')
      ...
      do ihor=1,NHORIZ
        if(step==horizon_steps(ihor))then
          call state%snapshot(snap,got)
          call require(got,'LAREGW1P probe endpoint snapshot')
          select type(physical=>snap)
          type is(fmr_b110_physical_state_t)
            call metrics_from_physical(physical,total,upper,lower)
          class default
            call require(.false.,'LAREGW1P expected B110 probe state')
          end select
          if(allocated(snap))deallocate(snap)
          write(*,'(*(g0))') 'LAREGW1P_PROBE|HISTORY=',trim(history_label(ih)),'|START_STEP=',target_step, &
  ...
  integer function probe_symbol(ip,step) result(symbol)
    ...
    case(6)
      if(step<=512)then;symbol=SYM_RISE
      else;symbol=SYM_FALL;end if
    case(7)
      if(step<=512)then;symbol=SYM_FALL
      else;symbol=SYM_RISE;end if
    case default
      call require(.false.,'LAREGW1P known probe id')
      symbol=0
    end select
...
end program test_lare_rs1_gw_future_probe