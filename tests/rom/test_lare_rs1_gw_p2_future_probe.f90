program test_lare_rs1_gw_p2_future_probe
  ...
  integer, parameter :: NHIST=6, NSTEPS=1024, FUTURE_STEPS=1250
  ...
  integer, parameter :: NPROBES=7
  integer, parameter :: NHORIZ=4
  ...
  call require(numnod==16,'LAREGW2 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'LAREGW2 depth frozen')
  call get_environment_variable('LARE_RS1_GW_P2_STARTS_FILE',starts_file)
  call require(len_trim(starts_file)>0,'LAREGW2 start request file supplied')
  open(newunit=start_unit,file=trim(starts_file),status='old',action='read',iostat=ios)
  call require(ios==0,'LAREGW2 start request file opened')
  start_count=0
  do
    read(start_unit,*,iostat=ios) ih,target_step
    if(ios<0)exit
    call require(ios==0,'LAREGW2 start request row readable')
    call require(ih>=1.and.ih<=NHIST,'LAREGW2 history index')
    call require(target_step>=1.and.target_step<=NSTEPS,'LAREGW2 step index')
    call run_start_request(ih,target_step)
    start_count=start_count+1
  end do
  close(start_unit)
  call require(start_count>0,'LAREGW2 nonempty frozen start set')
  write(*,'(A,I0)') 'LAREGW2_UNIQUE_START_COUNT=',start_count
  write(*,'(A)') 'LAREGW2_HELDOUT_USED=FALSE'
  write(*,'(A)') 'LAREGW2_B14_USED=FALSE'
  write(*,'(A)') 'LAREGW2_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    call require(ok.and.state%ready(),'LAREGW2 initial committed state')

    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'LAREGW2 gravity-consistent seed')
    ...
    do step=1,NSTEPS
      ...
      call require(ok,'LAREGW2 accepted history step')
      call require(abs(mass)<=hard_mass_gate,'LAREGW2 hard mass gate')
      call require(state%current_revision()==int(seed_intervals+step,int64),'LAREGW2 exact revision progression')
    ...
    write(*,'(*(g0))') 'LAREGW2_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(time_ok.and.same_bits(time_before,t0),'LAREGW2 pre-attempt committed time')
    call state%snapshot(before,got)
    call require(got,'LAREGW2 pre-attempt snapshot')
    ...
    call require(time_ok.and.same_bits(time_after,time_before),'LAREGW2 failed first attempt time immutable')
    call require(state%current_revision()==rev_before,'LAREGW2 failed first attempt revision immutable')
    call require(state%current_lineage_id()==lineage_before,'LAREGW2 failed first attempt lineage immutable')
    call state%snapshot(after,got)
    call require(got,'LAREGW2 failed first attempt snapshot')
    unchanged=state_bits_equal(before,after)
    call require(unchanged,'LAREGW2 failed first attempt physical state immutable')
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=rep_bound, &
         'LAREGW2 total residual within pre-solve bound')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'LAREGW2 total-only flag contract')
      call require(rmax<=p%compartment_balance_tolerance,'LAREGW2 total-only local criterion unchanged')
      fallback_p%compartment_balance_tolerance=p%compartment_balance_tolerance
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'LAREGW2 local-only flag contract')
      call require(ieee_is_finite(local_integrated).and.local_integrated<=1.6e-15_real64, &
           'LAREGW2 local residual within prior integrated allowance')
    ...
    case default
      call require(.false.,'LAREGW2 failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'LAREGW2 qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'LAREGW2_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    call require(got,'LAREGW2 state snapshot for output')
    ...
    type is(fmr_b110_physical_state_t)
      ...
      call require(all(ieee_is_finite(physical%pressure_head)).and.all(ieee_is_finite(physical%water_content)), &
           'LAREGW2 finite full profiles')
      write(*,'(*(g0))') 'LAREGW2_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'LAREGW2_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'LAREGW2 expected B110 physical state')
    end select
  ...
  subroutine metrics_from_physical(physical,total,upper,lower)
    ...
    call require(ieee_is_finite(total).and.ieee_is_finite(upper).and.ieee_is_finite(lower),'LAREGW2 finite primary outputs')
  ...
  subroutine require_current_time(state,expected)
    ...
    call require(ok.and.same_bits(actual,expected),'LAREGW2 exact committed time progression')
  ...
  integer function history_symbol(ih,step) result(symbol)
    ...
    call require(symbol>=SYM_HOLD.and.symbol<=SYM_COMBINED_FALL_MINUS,'LAREGW2 valid history symbol')
  ...
  subroutine configure_symbol(symbol,h0,k0,qeq,p,forcing)
    ...
    case default
      call require(.false.,'LAREGW2 configure known symbol')
    end select
  ...
  subroutine diagnose_failed_interval(p,forcing,state,t0,t1,fmr_status,fmr_nl,fmr_back,class_name,bal,head,rmax,rsum,imax)
    ...
    call require(fmr_status==SW_SOLVE_RETRY_ADVISED,'LAREGW2 FMR failed status retry-advised')
    ...
    call state%snapshot(snap,got);call require(got,'LAREGW2 diagnostic snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
    ...
    class default
      call require(.false.,'LAREGW2 diagnostic B110 state')
    end select
    call bind_b110_serialized_legacy_context(request,context_ok);call require(context_ok,'LAREGW2 diagnostic context')
    call solver%solve(request,workspace,result)
    call require(result%status==SW_SOLVE_RETRY_ADVISED,'LAREGW2 direct retry advised')
    call require(trim(result%diagnostics%route)=='legacy-reference-retry','LAREGW2 direct retry route')
    call require(result%diagnostics%nonlinear_iterations==fmr_nl,'LAREGW2 direct/FMR nonlinear identity')
    call require(result%diagnostics%backtracking_attempts==fmr_back,'LAREGW2 direct/FMR backtracking identity')
  ...
  subroutine initialize_state(p,se_anchor,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'LAREGW2 finite seed')
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'LAREGW2_FAIL',trim(label)
      error stop 1
    end if
  ...
  subroutine run_start_request(ih,target_step)
    ...
    write(*,'(*(g0))') 'LAREGW2_START|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
    ...
    do node=1,numnod
      write(*,'(*(g0))') 'LAREGW2_START_NODE|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
  ...
  subroutine reconstruct_start_state(ih,target_step,physical_out,h0,k0,qeq)
    ...
    call initialize_forcing(forcing,qeq,qeq,h0)
    call fmr_new_b110_committed_state(state,column_id,initial_state,0.0_real64,ok)
    call require(ok.and.state%ready(),'LAREGW2 start initial committed state')
    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'LAREGW2 start steady seed')
    t0=real(seed_intervals,real64)*seed_dt
    do step=1,target_step
      ...
      t1=real(seed_intervals,real64)*seed_dt+real(step,real64)*step_dt
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'LAREGW2 start reconstruction accepted')
      call require(abs(mass)<=hard_mass_gate,'LAREGW2 start reconstruction mass')
      t0=t1
    end do
    call state%snapshot(snap,got)
    call require(got,'LAREGW2 start snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      physical_out=physical
    class default
      call require(.false.,'LAREGW2 expected B110 start state')
    end select
    if(allocated(snap))deallocate(snap)
  ...
  subroutine run_probe(ih,target_step,ip,start_physical,h0,k0,qeq,start_total,start_upper,start_lower)
    ...
    integer :: step,symbol,status,nl,ir,back,ihor,fallback_count
    character(len=96) :: route
    logical :: ok,fallback_used,got
    ...
    call require(ok.and.state%ready(),'LAREGW2 probe committed start')
    cum_top=0.0_real64;cum_bottom=0.0_real64;max_mass=0.0_real64
    fallback_count=0;t0=0.0_real64
    do step=1,FUTURE_STEPS
      symbol=probe_symbol(ip,step)
      call configure_symbol(symbol,h0,k0,qeq,p,forcing)
      t1=real(step,real64)*step_dt
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'LAREGW2 frozen future probe accepted')
      call require(abs(mass)<=hard_mass_gate,'LAREGW2 probe hard mass gate')
      ...
      write(*,'(*(g0))') 'LAREGW2_STEP|HISTORY=',trim(history_label(ih)),'|START_STEP=',target_step, &
      ...
      do ihor=1,NHORIZ
        if(step==horizon_steps(ihor))then
          call state%snapshot(snap,got)
          call require(got,'LAREGW2 probe endpoint snapshot')
          select type(physical=>snap)
          type is(fmr_b110_physical_state_t)
            call metrics_from_physical(physical,total,upper,lower)
          class default
            call require(.false.,'LAREGW2 expected B110 probe state')
          end select
          if(allocated(snap))deallocate(snap)
          write(*,'(*(g0))') 'LAREGW2_PROBE|HISTORY=',trim(history_label(ih)),'|START_STEP=',target_step, &
  ...
  integer function probe_symbol(ip,step) result(symbol)
    integer,intent(in) :: ip,step
    select case(ip)
    case(1);symbol=SYM_HOLD
    case(2);symbol=SYM_TOP_PLUS
    case(3);symbol=SYM_TOP_MINUS
    case(4);symbol=SYM_RISE
    case(5);symbol=SYM_FALL
    case(6)
      if(step<=625)then;symbol=SYM_RISE
      else;symbol=SYM_FALL;end if
    case(7)
      if(step<=625)then;symbol=SYM_FALL
      else;symbol=SYM_RISE;end if
    case default
      call require(.false.,'LAREGW2 known probe id')
      symbol=0
    end select
  end function probe_symbol
  ...
  pure function probe_label(ip) result(label)
    ...
    case(6);label='BOTTOM_HEAD_RISE_THEN_FALL'
    case(7);label='BOTTOM_HEAD_FALL_THEN_RISE'
    case default;label='UNKNOWN'
    end select
  end function probe_label

end program test_lare_rs1_gw_p2_future_probe