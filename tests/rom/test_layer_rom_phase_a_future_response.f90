! Selective research-only derivative of work/f-rom-lare-rs1 future-response Reference harness.\n! Source branch inspected at 96f902d336a2ed8eb6e4c3a091fe7d290af006a6.\n! Reused surface: deterministic Reference start reconstruction and bounded retry mechanics only.\n! Base histories and future probes are independently preregistered Layer-ROM Phase A definitions.\n! No LARE propagation closure or production semantics are executed here.\nprogram test_layer_rom_phase_a_future_response
  ...
  integer, parameter :: NHIST=6, NSTEPS=1024, FUTURE_STEPS=256
  ...
  call require(numnod==16,'LAYERR2 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'LAYERR2 depth frozen')
  call get_environment_variable('LAYER_ROM_PHASE_A_STARTS_FILE',starts_file)
  call require(len_trim(starts_file)>0,'LAYERR2 start request file supplied')
  open(newunit=start_unit,file=trim(starts_file),status='old',action='read',iostat=ios)
  call require(ios==0,'LAYERR2 start request file opened')
  start_count=0
  do
    read(start_unit,*,iostat=ios) ih,target_step
    if(ios<0)exit
    call require(ios==0,'LAYERR2 start request row readable')
    call require(ih>=1.and.ih<=NHIST,'LAYERR2 history index')
    call require(target_step>=1.and.target_step<=NSTEPS,'LAYERR2 step index')
    call run_start_request(ih,target_step)
    start_count=start_count+1
  end do
  close(start_unit)
  call require(start_count>0,'LAYERR2 nonempty frozen start set')
  write(*,'(A,I0)') 'LAYERR2_UNIQUE_START_COUNT=',start_count
  write(*,'(A)') 'LAYERR2_HELDOUT_USED=FALSE'
  write(*,'(A)') 'LAYERR2_B14_USED=FALSE'
  write(*,'(A)') 'LAYERR2_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    call require(ok.and.state%ready(),'LAYERR2 initial committed state')

    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'LAYERR2 gravity-consistent seed')
    ...
    do step=1,NSTEPS
      ...
      call configure_symbol(ih,symbol,h0,k0,qeq,p,forcing)
      ...
      call require(ok,'LAYERR2 accepted history step')
      call require(abs(mass)<=hard_mass_gate,'LAYERR2 hard mass gate')
      call require(state%current_revision()==int(seed_intervals+step,int64),'LAYERR2 exact revision progression')
    ...
    write(*,'(*(g0))') 'LAYERR2_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(time_ok.and.same_bits(time_before,t0),'LAYERR2 pre-attempt committed time')
    call state%snapshot(before,got)
    call require(got,'LAYERR2 pre-attempt snapshot')
    call prospective_bound(state,p,rep_bound,bound_ok)
    call require(bound_ok.and.rep_bound>0.0_real64,'LAYERPA_RESP prospective representation bound')
    ...
    call require(time_ok.and.same_bits(time_after,time_before),'LAYERR2 failed first attempt time immutable')
    call require(state%current_revision()==rev_before,'LAYERR2 failed first attempt revision immutable')
    call require(state%current_lineage_id()==lineage_before,'LAYERR2 failed first attempt lineage immutable')
    call state%snapshot(after,got)
    call require(got,'LAYERR2 failed first attempt snapshot')
    unchanged=state_bits_equal(before,after)
    call require(unchanged,'LAYERR2 failed first attempt physical state immutable')
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=rep_bound, &
         'LAYERR2 total residual within pre-solve bound')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'LAYERR2 total-only flag contract')
      call require(rmax<=p%compartment_balance_tolerance,'LAYERR2 total-only local criterion unchanged')
      fallback_p%compartment_balance_tolerance=p%compartment_balance_tolerance
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'LAYERR2 local-only flag contract')
      call require(ieee_is_finite(local_integrated).and.local_integrated<=1.6e-15_real64, &
           'LAYERR2 local residual within prior integrated allowance')
    ...
    case default
      call require(.false.,'LAYERR2 failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'LAYERR2 qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'LAYERR2_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    call require(got,'LAYERR2 state snapshot for output')
    ...
    type is(fmr_b110_physical_state_t)
      ...
      call require(all(ieee_is_finite(physical%pressure_head)).and.all(ieee_is_finite(physical%water_content)), &
           'LAYERR2 finite full profiles')
      write(*,'(*(g0))') 'LAYERR2_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
           '|STEP=',step,'|REL_T=',real(step,real64)*step_dt,'|T=',t1,'|REV=',state%current_revision(), &
           '|SYMBOL=',trim(symbol_label(symbol)),'|BOTTOM_MODE=',merge(5,2,is_head_history(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'LAYERR2_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'LAYERR2 expected B110 physical state')
    end select
  ...
  subroutine metrics_from_physical(physical,total,upper,lower)
    ...
    call require(ieee_is_finite(total).and.ieee_is_finite(upper).and.ieee_is_finite(lower),'LAYERR2 finite primary outputs')
  ...
  subroutine require_current_time(state,expected)
    ...
    call require(ok.and.same_bits(actual,expected),'LAYERR2 exact committed time progression')
  end subroutine require_current_time

  pure real(real64) function initial_se(ih) result(value)
    integer,intent(in) :: ih
    select case(ih)
    case(1,4); value=0.60_real64
    case(2,5); value=0.75_real64
    case(3,6); value=0.90_real64
  ...
  integer function history_symbol(ih,step) result(symbol)
    ...
    select case(ih)
    case(1)
      if(step<=192)then; symbol=SYM_TOP_PLUS
      else if(step<=512)then; symbol=SYM_HOLD
      else if(step<=704)then; symbol=SYM_TOP_MINUS
      else; symbol=SYM_HOLD; end if
    case(2)
      if(step<=128)then; symbol=SYM_TOP_MINUS
      else if(step<=256)then; symbol=SYM_HOLD
      else if(step<=640)then; symbol=SYM_TOP_PLUS
      else; symbol=SYM_HOLD; end if
    case(3)
      if(step<=128)then; symbol=SYM_TOP_PLUS
      else if(step<=384)then; symbol=SYM_TOP_MINUS
      else if(step<=512)then; symbol=SYM_HOLD
      else if(step<=768)then; symbol=SYM_TOP_PLUS
      else; symbol=SYM_HOLD; end if
    case(4)
    ...
    case(5)
      if(step<=256)then; symbol=SYM_TOP_MINUS
      else if(step<=384)then; symbol=SYM_HOLD
      else if(step<=640)then; symbol=SYM_TOP_PLUS
      else; symbol=SYM_HOLD; end if
    case(6)
      if(step<=128)then; symbol=SYM_TOP_PLUS
      else if(step<=256)then; symbol=SYM_HOLD
      else if(step<=640)then; symbol=SYM_TOP_MINUS
      else; symbol=SYM_HOLD; end if
    case default
      symbol=0
    end select
    call require(symbol>=SYM_HOLD.and.symbol<=SYM_TOP_MINUS,'Layer-ROM valid history symbol')
  end function history_symbol

  subroutine configure_symbol(ih,symbol,h0,k0,qeq,p,forcing)
    integer,intent(in) :: ih,symbol
    ...
    if(ih<=3)then
      ...
      p%bottom_mode=5
      select case(ih)
      case(4); hfac=0.75_real64
      case(5); hfac=1.00_real64
      case(6); hfac=1.25_real64
      case default
        call require(.false.,'Layer-ROM known head-lab history')
      end select
      forcing%bottom_head=hfac*h0
    end if
    ...
    case(SYM_TOP_PLUS)
      forcing%top_flux=qeq+0.0125_real64*k0
    case(SYM_TOP_MINUS)
      forcing%top_flux=qeq-0.0125_real64*k0
    case default
      call require(.false.,'Layer-ROM known forcing symbol')
    end select
  end subroutine configure_symbol

  logical function is_head_history(ih)
    integer,intent(in) :: ih
    is_head_history=ih>=4.and.ih<=6
  end function is_head_history
  ...
  subroutine diagnose_failed_interval(p,forcing,state,t0,t1,fmr_status,fmr_nl,fmr_back,class_name,bal,head,rmax,rsum,imax)
    ...
    call require(fmr_status==SW_SOLVE_RETRY_ADVISED,'LAYERR2 FMR failed status retry-advised')
    ...
    call state%snapshot(snap,got);call require(got,'LAYERR2 diagnostic snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
    ...
    class default
      call require(.false.,'LAYERR2 diagnostic B110 state')
    end select
    call bind_b110_serialized_legacy_context(request,context_ok);call require(context_ok,'LAYERR2 diagnostic context')
    call solver%solve(request,workspace,result)
    call require(result%status==SW_SOLVE_RETRY_ADVISED,'LAYERR2 direct retry advised')
    call require(trim(result%diagnostics%route)=='legacy-reference-retry','LAYERR2 direct retry route')
    call require(result%diagnostics%nonlinear_iterations==fmr_nl,'LAYERR2 direct/FMR nonlinear identity')
    call require(result%diagnostics%backtracking_attempts==fmr_back,'LAYERR2 direct/FMR backtracking identity')
  ...
  subroutine initialize_state(p,se_anchor,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'LAYERR2 finite seed')
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'LAYERR2_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require

  subroutine run_start_request(ih,target_step)
    integer,intent(in) :: ih,target_step
    type(fmr_b110_physical_state_t) :: start_physical
    real(real64) :: h0,k0,qeq,start_total,start_upper,start_lower
    integer :: ip,node

    call reconstruct_start_state(ih,target_step,start_physical,h0,k0,qeq)
    call metrics_from_physical(start_physical,start_total,start_upper,start_lower)
    write(*,'(*(g0))') 'LAYERR2_START|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
         '|TOTAL_STORAGE=',start_total,'|UPPER_STORAGE=',start_upper,'|LOWER_STORAGE=',start_lower, &
         '|POND=',start_physical%ponding_depth,'|GWL=',start_physical%groundwater_level
    do node=1,numnod
      write(*,'(*(g0))') 'LAYERR2_START_NODE|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
           '|NODE=',node,'|H=',start_physical%pressure_head(node),'|THETA=',start_physical%water_content(node)
    end do

    do ip=1,NPROBES
      call run_probe(ih,target_step,ip,start_physical,h0,k0,qeq,start_total,start_upper,start_lower)
    end do
  end subroutine run_start_request

  subroutine reconstruct_start_state(ih,target_step,physical_out,h0,k0,qeq)
    integer,intent(in) :: ih,target_step
    type(fmr_b110_physical_state_t),intent(out) :: physical_out
    real(real64),intent(out) :: h0,k0,qeq
    type(fmr_b110_physical_parameters_t) :: p
    type(fmr_b110_physical_forcing_t) :: forcing
    type(fmr_b110_physical_state_t) :: initial_state
    type(kernel_committed_state_t) :: state
    ...
    call initialize_forcing(forcing,qeq,qeq,h0)
    call fmr_new_b110_committed_state(state,column_id,initial_state,0.0_real64,ok)
    call require(ok.and.state%ready(),'LAYERR2 start initial committed state')
    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'LAYERR2 start steady seed')
    t0=real(seed_intervals,real64)*seed_dt
    do step=1,target_step
      symbol=history_symbol(ih,step)
      call configure_symbol(ih,symbol,h0,k0,qeq,p,forcing)
      t1=real(seed_intervals,real64)*seed_dt+real(step,real64)*step_dt
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'LAYERR2 start reconstruction accepted')
      call require(abs(mass)<=hard_mass_gate,'LAYERR2 start reconstruction mass')
      t0=t1
    end do
    call state%snapshot(snap,got)
    call require(got,'LAYERR2 start snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      physical_out=physical
    class default
      call require(.false.,'LAYERR2 expected B110 start state')
    end select
    if(allocated(snap))deallocate(snap)
  end subroutine reconstruct_start_state

  subroutine run_probe(ih,target_step,ip,start_physical,h0,k0,qeq,start_total,start_upper,start_lower)
    integer,intent(in) :: ih,target_step,ip
    type(fmr_b110_physical_state_t),intent(in) :: start_physical
    real(real64),intent(in) :: h0,k0,qeq,start_total,start_upper,start_lower
    type(fmr_b110_physical_parameters_t) :: p
    type(fmr_b110_physical_forcing_t) :: forcing
    type(kernel_committed_state_t) :: state
    type(fmr_logical_column_t) :: column
    type(fmr_template_t) :: template
    class(transaction_state_t),allocatable :: snap
    real(real64) :: t0,t1,mass,bex,bflux,total,upper,lower,cum_top,cum_bottom,max_mass
    integer :: step,status,nl,ir,back,ihor,fallback_count,node
    character(len=96) :: route
    logical :: ok,fallback_used,got
    ...
    call initialize_identity(column,template)
    call initialize_forcing(forcing,qeq,qeq,h0)
    call fmr_new_b110_committed_state(state,column_id,start_physical,0.0_real64,ok)
    call require(ok.and.state%ready(),'LAYERR2 probe committed start')
    cum_top=0.0_real64;cum_bottom=0.0_real64;max_mass=0.0_real64
    fallback_count=0;t0=0.0_real64
    do step=1,FUTURE_STEPS
      call configure_probe(ip,step,p,forcing)
      t1=real(step,real64)*step_dt
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'LAYERR2 frozen future probe accepted')
      call require(abs(mass)<=hard_mass_gate,'LAYERR2 probe hard mass gate')
      ...
      write(*,'(*(g0))') 'LAYERR2_STEP|HISTORY=',trim(history_label(ih)),'|START_STEP=',target_step, &
      ...
      do ihor=1,NHORIZ
        if(step==horizon_steps(ihor))then
          call state%snapshot(snap,got)
          call require(got,'LAYERR2 probe endpoint snapshot')
          select type(physical=>snap)
          type is(fmr_b110_physical_state_t)
            call metrics_from_physical(physical,total,upper,lower)
          class default
            call require(.false.,'LAYERR2 expected B110 probe state')
          end select
          write(*,'(*(g0))') 'LAYERR2_PROBE|HISTORY=',trim(history_label(ih)),'|START_STEP=',target_step, &
               '|PROBE=',trim(probe_label(ip)),'|HORIZON_STEP=',step,'|HORIZON_DAY=',t1, &
               '|D_TOTAL_STORAGE=',total-start_total,'|D_UPPER_STORAGE=',upper-start_upper, &
               '|D_LOWER_STORAGE=',lower-start_lower,'|CUM_TOP_EXCHANGE=',cum_top, &
               '|CUM_BOTTOM_OUTWARD_EXCHANGE=',cum_bottom,'|TERMINAL_BOTTOM_FLUX=',bflux, &
               '|FALLBACK_COUNT=',fallback_count,'|MAX_ABS_MASS=',max_mass,'|FINAL_NL=',nl,'|FINAL_BACKTRACK=',back
          select type(physical=>snap)
          type is(fmr_b110_physical_state_t)
            do node=1,numnod
              write(*,'(*(g0))') 'LAYERR2_PROBE_NODE|HISTORY=',trim(history_label(ih)),'|START_STEP=',target_step, &
                   '|PROBE=',trim(probe_label(ip)),'|HORIZON_STEP=',step,'|NODE=',node, &
                   '|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
            end do
          class default
            call require(.false.,'LAYERR2 expected B110 endpoint profile')
          end select
          if(allocated(snap))deallocate(snap)
        end if
      end do
  ...
  subroutine configure_probe(ip,step,p,forcing)
    integer,intent(in) :: ip,step
    type(fmr_b110_physical_parameters_t),intent(inout) :: p
    type(fmr_b110_physical_forcing_t),intent(inout) :: forcing
    p%total_balance_tolerance=original_total_tol
    forcing%top_head=0.0_real64
    ...
    case(2) ! P_FX_OPPOSED
      p%bottom_mode=2
      forcing%top_flux=0.02_real64
      forcing%bottom_flux=-0.02_real64
      forcing%bottom_head=0.0_real64
    case(3) ! P_HD_WETTER
      p%bottom_mode=5
      forcing%top_flux=0.0_real64
      forcing%bottom_flux=0.0_real64
      forcing%bottom_head=-25.0_real64
    case(4) ! P_HD_DRIER
      p%bottom_mode=5
      forcing%top_flux=0.0_real64
      forcing%bottom_flux=0.0_real64
      forcing%bottom_head=-65.0_real64
    case(5) ! P_HD_SWITCH
      p%bottom_mode=5
      forcing%top_flux=0.0_real64
      forcing%bottom_flux=0.0_real64
    ...
    case default
      call require(.false.,'LAYERR2 known future probe id')
    end select
  end subroutine configure_probe

  pure function probe_label(ip) result(label)
    integer,intent(in) :: ip
    character(len=32) :: label
    select case(ip)
    case(1);label='P_FX_ZERO'
    case(2);label='P_FX_OPPOSED'
    case(3);label='P_HD_WETTER'
    case(4);label='P_HD_DRIER'
    case(5);label='P_HD_SWITCH'
    case(6);label='P_TOP_SWITCH'
    case default;label='UNKNOWN'
    end select
  end function probe_label

end program test_layer_rom_phase_a_future_response