! Selective research-only derivative of work/f-rom-lare-rs1 tests/rom/test_lare_rs1_gw_library.f90
! Source branch inspected at 96f902d336a2ed8eb6e4c3a091fe7d290af006a6.
! Reused surface: qualified serialized Reference observation harness and bounded retry policy only.
! No LARE propagation closure, production source, or coupling semantics are imported.
program test_layer_rom_phase_a_reference
  ...
  call require(numnod==16,'LAYERR1 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'LAYERR1 depth frozen')
  ...
  call require(total_states==NHIST*NSTEPS,'LAYERR1 exact library state count')
  write(*,'(A,I0)') 'LAYERR1_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'LAYERR1_STEPS_PER_HISTORY=',NSTEPS
  write(*,'(A,I0)') 'LAYERR1_STATE_COUNT=',total_states
  write(*,'(A,I0)') 'LAYERR1_TOTAL_FALLBACK_COUNT=',total_fallbacks
  write(*,'(*(g0))') 'LAYERR1_MAX_ABS_MASS=',max_abs_mass
  write(*,'(A)') 'LAYERR1_B14_MATERIAL_TRANSFER_GENERATED=FALSE'
  write(*,'(A)') 'LAYERR1_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    call require(ok.and.state%ready(),'LAYERPA_REF initial committed state')

    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'LAYERPA_REF gravity-consistent seed')
    ...
    do step=1,NSTEPS
      ...
      call require(ok,'LAYERPA_REF accepted history step')
      call require(abs(mass)<=hard_mass_gate,'LAYERPA_REF hard mass gate')
      call require(state%current_revision()==int(seed_intervals+step,int64),'LAYERPA_REF exact revision progression')
    ...
    write(*,'(*(g0))') 'LAYERR1_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(time_ok.and.same_bits(time_before,t0),'LAYERPA_REF pre-attempt committed time')
    call state%snapshot(before,got)
    call require(got,'LAYERPA_REF pre-attempt snapshot')
    call prospective_bound(state,p,rep_bound,bound_ok)
    call require(bound_ok.and.rep_bound>0.0_real64,'LAYERPA_REF prospective representation bound')
    ...
    call require(time_ok.and.same_bits(time_after,time_before),'LAYERPA_REF failed first attempt time immutable')
    call require(state%current_revision()==rev_before,'LAYERPA_REF failed first attempt revision immutable')
    call require(state%current_lineage_id()==lineage_before,'LAYERPA_REF failed first attempt lineage immutable')
    call state%snapshot(after,got)
    call require(got,'LAYERPA_REF failed first attempt snapshot')
    unchanged=state_bits_equal(before,after)
    call require(unchanged,'LAYERPA_REF failed first attempt physical state immutable')
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=rep_bound, &
         'LAYERPA_REF total residual within pre-solve bound')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'LAYERPA_REF total-only flag contract')
      call require(rmax<=p%compartment_balance_tolerance,'LAYERPA_REF total-only local criterion unchanged')
      fallback_p%compartment_balance_tolerance=p%compartment_balance_tolerance
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'LAYERPA_REF local-only flag contract')
      call require(ieee_is_finite(local_integrated).and.local_integrated<=1.6e-15_real64, &
           'LAYERPA_REF local residual within prior integrated allowance')
    ...
    case default
      call require(.false.,'LAYERPA_REF failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'LAYERPA_REF qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'LAYERR1_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    call require(got,'LAYERPA_REF state snapshot for output')
    ...
    type is(fmr_b110_physical_state_t)
      ...
      call require(all(ieee_is_finite(physical%pressure_head)).and.all(ieee_is_finite(physical%water_content)), &
           'LAYERPA_REF finite full profiles')
      write(*,'(*(g0))') 'LAYERR1_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'LAYERR1_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'LAYERPA_REF expected B110 physical state')
    end select
  ...
  subroutine metrics_from_physical(physical,total,upper,lower)
    ...
    call require(ieee_is_finite(total).and.ieee_is_finite(upper).and.ieee_is_finite(lower),'LAYERPA_REF finite primary outputs')
  ...
  subroutine require_current_time(state,expected)
    ...
    call require(ok.and.same_bits(actual,expected),'LAYERPA_REF exact committed time progression')
  ...
  subroutine diagnose_failed_interval(p,forcing,state,t0,t1,fmr_status,fmr_nl,fmr_back,class_name,bal,head,rmax,rsum,imax)
    ...
    call require(fmr_status==SW_SOLVE_RETRY_ADVISED,'LAYERPA_REF FMR failed status retry-advised')
    ...
    call state%snapshot(snap,got);call require(got,'LAYERPA_REF diagnostic snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
    ...
    class default
      call require(.false.,'LAYERPA_REF diagnostic B110 state')
    end select
    call bind_b110_serialized_legacy_context(request,context_ok);call require(context_ok,'LAYERPA_REF diagnostic context')
    call solver%solve(request,workspace,result)
    call require(result%status==SW_SOLVE_RETRY_ADVISED,'LAYERPA_REF direct retry advised')
    call require(trim(result%diagnostics%route)=='legacy-reference-retry','LAYERPA_REF direct retry route')
    call require(result%diagnostics%nonlinear_iterations==fmr_nl,'LAYERPA_REF direct/FMR nonlinear identity')
    call require(result%diagnostics%backtracking_attempts==fmr_back,'LAYERPA_REF direct/FMR backtracking identity')
  ...
  subroutine initialize_state(p,se_anchor,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'LAYERPA_REF finite seed')
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'LAYERR1_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_layer_rom_phase_a_reference