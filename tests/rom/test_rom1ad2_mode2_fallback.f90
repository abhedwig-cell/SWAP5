program test_rom1ad2_mode2_fallback
  ...
  integer :: total_steps,total_fallbacks
  real(real64) :: max_abs_mass

  call require(numnod==16,'ROM1AD2 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'ROM1AD2 depth frozen')
  total_steps=0
  total_fallbacks=0
  max_abs_mass=0.0_real64

  call run_d2_trajectory('TOP_PLUS', 0.01_real64, total_steps,total_fallbacks,max_abs_mass)
  call run_d2_trajectory('TOP_MINUS',-0.01_real64, total_steps,total_fallbacks,max_abs_mass)

  call require(total_steps==128,'ROM1AD2 exact committed step count')
  call require(total_fallbacks>0,'ROM1AD2 at least one total-only fallback')
  write(*,'(A,I0)') 'ROM1AD2_TRAJECTORY_COUNT=',2
  write(*,'(A,I0)') 'ROM1AD2_COMMITTED_STEP_COUNT=',total_steps
  write(*,'(A,I0)') 'ROM1AD2_FALLBACK_COUNT=',total_fallbacks
  write(*,'(*(g0))') 'ROM1AD2_MAX_ABS_MASS=',max_abs_mass
  write(*,'(A)') 'ROM1AD2_HELDOUT_EXECUTED=FALSE'
  write(*,'(A)') 'ROM1AD2_B14_EXECUTED=FALSE'
  write(*,'(A)') 'ROM1AD2_EXECUTION_COMPLETE=PASS'

contains

  subroutine run_d2_trajectory(case_id,top_delta,total_steps,total_fallbacks,max_abs_mass)
    character(len=*),intent(in) :: case_id
    real(real64),intent(in) :: top_delta
    integer,intent(inout) :: total_steps,total_fallbacks
    ...
    integer :: step,status,nl,ir,back,case_fallbacks
    ...
    call require(ok.and.state%ready(),'ROM1AD2 initial committed state')

    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'ROM1AD2 gravity-consistent seed')

    forcing%top_flux=qeq+top_delta*k0
    ...
    do step=1,64
      ...
      call require(ok,'ROM1AD2 accepted mode2 step')
      call require(abs(mass)<=hard_mass_gate,'ROM1AD2 hard mass gate')
      call require(state%current_revision()==int(seed_intervals+step,int64),'ROM1AD2 exact revision progression')
      ...
      if(fallback_used)then
        ...
        case_fallbacks=case_fallbacks+1
      end if
      write(*,'(*(g0))') 'ROM1AD2_STEP|CASE=',trim(case_id),'|STEP=',step,'|T=',t1, &
           '|MASS=',mass,'|BOTTOM_EXCHANGE=',bex,'|BOTTOM_FLUX=',bflux,'|NL=',nl, &
           '|BACKTRACK=',back,'|FALLBACK=',fallback_used
      t0=t1
    end do

    write(*,'(*(g0))') 'ROM1AD2_CASE_PASS|CASE=',trim(case_id),'|STEPS=64|FALLBACKS=',case_fallbacks, &
         '|FINAL_REV=',state%current_revision(),'|FINAL_T=',t0
  end subroutine run_d2_trajectory
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    write(*,'(*(g0))') 'ROM1AD2_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(bound_ok.and.rep_bound>0.0_real64,'ROM1AD2 prospective representation bound')
    ...
    ! D2 qualifies only the fixed-flux mode-2 sibling policy.
    ...
    write(*,'(*(g0))') 'ROM1AD2_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|CLASS=',trim(failure_class), &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    type is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'ROM1AD2_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'ROM1AD2_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'ROM1AD2_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_rom1ad2_mode2_fallback
