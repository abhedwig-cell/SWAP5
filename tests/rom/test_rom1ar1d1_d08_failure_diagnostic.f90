program test_rom1ar1d1_d08_failure_diagnostic
  ...
  real(real64) :: h0,k0,qeq,t0,t1,mass,bex,bflux,rep_bound,rsum,rmax,abs_integrated
  real(real64) :: time_before,time_after
  integer :: step,symbol,status,nl,ir,back,bal_flags,head_flags,imax,prefix_fallbacks
  integer(int64) :: rev_before,lineage_before
  character(len=96) :: route
  character(len=24) :: failure_class
  logical :: ok,fallback_used,bound_ok,got,time_ok

  call require(numnod==16,'ROM1AR1D1 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'ROM1AR1D1 depth frozen')
  ...
  call require(ok.and.state%ready(),'ROM1AR1D1 initial committed state')

  p%bottom_mode=2
  p%total_balance_tolerance=original_total_tol
  call seed_steady(column,template,p,state,forcing,qeq,ok)
  call require(ok,'ROM1AR1D1 gravity-consistent seed')
  t0=real(seed_intervals,real64)*seed_dt
  prefix_fallbacks=0

  do step=1,33
    symbol=history_symbol(8,step)
    ...
    call require(ok,'ROM1AR1D1 accepted D08 predecessor step')
    call require(abs(mass)<=hard_mass_gate,'ROM1AR1D1 predecessor hard mass gate')
    call require(state%current_revision()==int(seed_intervals+step,int64),'ROM1AR1D1 predecessor revision progression')
    call require_current_time(state,t1)
    if(fallback_used)prefix_fallbacks=prefix_fallbacks+1
    t0=t1
  end do

  call require(state%current_revision()==35_int64,'ROM1AR1D1 exact predecessor revision')
  call state%current_time(time_before,time_ok)
  call require(time_ok.and.same_bits(time_before,t0),'ROM1AR1D1 exact predecessor time')
  call prospective_bound(state,p,rep_bound,bound_ok)
  call require(bound_ok.and.rep_bound>0.0_real64,'ROM1AR1D1 pre-solve representation bound')
  write(*,'(*(g0))') 'ROM1AR1D1_PREFIX|HISTORY=D08|PASS_STEPS=33|FALLBACKS=',prefix_fallbacks, &
       '|REV=',state%current_revision(),'|T=',time_before

  symbol=history_symbol(8,34)
  call require(symbol==SYM_RISE,'ROM1AR1D1 step34 symbol')
  call configure_symbol(symbol,h0,k0,qeq,p,forcing)
  p%total_balance_tolerance=original_total_tol
  ...
  call require(got,'ROM1AR1D1 predecessor snapshot')

  t1=real(seed_intervals,real64)*seed_dt+34.0_real64*step_dt
  call sample_fresh(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
  call require(.not.ok,'ROM1AR1D1 strict step34 reproduces failure')
  call require(status==SW_SOLVE_RETRY_ADVISED,'ROM1AR1D1 strict step34 retry advised')
  call require(trim(route)=='legacy-reference-retry','ROM1AR1D1 strict step34 route')

  call state%current_time(time_after,time_ok)
  call require(time_ok.and.same_bits(time_after,time_before),'ROM1AR1D1 failed step time immutable')
  call require(state%current_revision()==rev_before,'ROM1AR1D1 failed step revision immutable')
  call require(state%current_lineage_id()==lineage_before,'ROM1AR1D1 failed step lineage immutable')
  call state%snapshot(after,got)
  call require(got.and.state_bits_equal(before,after),'ROM1AR1D1 failed step state bit immutable')
  ...
  write(*,'(*(g0))') 'ROM1AR1D1_CLASS|CLASS=',trim(failure_class),'|BAL_FLAGS=',bal_flags, &
  ...
  write(*,'(*(g0))') 'ROM1AR1D1_BOUND|REP_BOUND_CM=',rep_bound, &
       '|ABS_TOTAL_RESIDUAL_CM=',abs_integrated,'|RESIDUAL_OVER_BOUND=',abs_integrated/rep_bound, &
       '|WITHIN_BOUND=',abs_integrated<=rep_bound
  write(*,'(*(g0))') 'ROM1AR1D1_FAILURE|HISTORY=D08|STEP=34|SYMBOL=BOTTOM_HEAD_RISE|BOTTOM_MODE=',p%bottom_mode, &
       '|T0=',t0,'|T1=',t1,'|REV=',rev_before
  write(*,'(A)') 'ROM1AR1D1_DIAGNOSTIC_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    write(*,'(*(g0))') 'ROM1AR1_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(trim(failure_class)=='RETRY_TOTAL_ONLY','ROM1A fallback exact total-only')
    abs_integrated=abs(rsum)*(t1-t0)
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=rep_bound,'ROM1A failed residual within bound')
    ...
    call require(ok,'ROM1A qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'ROM1AR1_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
         '|CLASS=',trim(failure_class), &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    type is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'ROM1AR1_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'ROM1AR1_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'ROM1AR1_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_rom1ar1d1_d08_failure_diagnostic
