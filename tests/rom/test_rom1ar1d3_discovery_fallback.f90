program test_rom1ar1d3_discovery_fallback
  ...
  integer :: ih,total_states,total_fallbacks,total_local_fallbacks,total_total_fallbacks
  real(real64) :: max_abs_mass

  call require(numnod==16,'ROM1AR1D3 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'ROM1AR1D3 depth frozen')
  total_states=0;total_fallbacks=0;max_abs_mass=0.0_real64
  total_local_fallbacks=0;total_total_fallbacks=0

  do ih=1,8
    call run_history(ih,total_states,total_fallbacks,max_abs_mass)
  end do

  call require(total_states==8*NSTEPS,'ROM1AR1D3 exact discovery state count')
  write(*,'(A,I0)') 'ROM1AR1D3_HISTORY_COUNT=',8
  write(*,'(A,I0)') 'ROM1AR1D3_STATE_COUNT=',total_states
  write(*,'(A,I0)') 'ROM1AR1D3_TOTAL_FALLBACK_COUNT=',total_fallbacks
  write(*,'(*(g0))') 'ROM1AR1D3_MAX_ABS_MASS=',max_abs_mass
  write(*,'(A)') 'ROM1AR1D3_HELDOUT_EXECUTED=FALSE'
  write(*,'(A)') 'ROM1AR1D3_B14_EXECUTED=FALSE'
  write(*,'(A)') 'ROM1AR1D3_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    write(*,'(*(g0))') 'ROM1AR1D3_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    write(*,'(*(g0))') 'ROM1AR1D3_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    type is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'ROM1AR1D3_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'ROM1AR1D3_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'ROM1AR1D3_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_rom1ar1d3_discovery_fallback
