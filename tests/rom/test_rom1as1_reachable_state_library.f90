program test_rom1as1_reachable_state_library
  ...
  call require(numnod==16,'ROM1AS1 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'ROM1AS1 depth frozen')
  ...
  call require(total_states==NHIST*NSTEPS,'ROM1AS1 exact accepted state count')
  write(*,'(A,I0)') 'ROM1AS1_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'ROM1AS1_DISCOVERY_HISTORY_COUNT=',8
  write(*,'(A,I0)') 'ROM1AS1_HELDOUT_HISTORY_COUNT=',4
  write(*,'(A,I0)') 'ROM1AS1_STATE_COUNT=',total_states
  write(*,'(A,I0)') 'ROM1AS1_DISCOVERY_STATE_COUNT=',8*NSTEPS
  write(*,'(A,I0)') 'ROM1AS1_HELDOUT_STATE_COUNT=',4*NSTEPS
  write(*,'(A,I0)') 'ROM1AS1_TOTAL_FALLBACK_COUNT=',total_fallbacks
  write(*,'(*(g0))') 'ROM1AS1_MAX_ABS_MASS=',max_abs_mass
  write(*,'(A)') 'ROM1AS1_B14_MATERIAL_TRANSFER_GENERATED=FALSE'
  write(*,'(A)') 'ROM1AS1_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    call require(ok.and.state%ready(),'ROM1AS1 initial committed state')

    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'ROM1AS1 gravity-consistent seed')
    ...
    do step=1,NSTEPS
      ...
      call strict_first_sample(column,template,p,state,forcing,symbol,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'ROM1AS1 accepted history step')
      call require(abs(mass)<=hard_mass_gate,'ROM1AS1 hard mass gate')
      call require(state%current_revision()==int(seed_intervals+step,int64),'ROM1AS1 exact revision progression')
    ...
    write(*,'(*(g0))') 'ROM1AS1_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,symbol,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    real(real64) :: rep_bound,fallback_tol,rsum,rmax,abs_integrated
    ...
    logical :: got,time_ok,bound_ok,unchanged,fallback_authorized
    ...
    call require(time_ok.and.same_bits(time_before,t0),'ROM1AS1 pre-attempt committed time')
    call state%snapshot(before,got)
    call require(got,'ROM1AS1 pre-attempt snapshot')
    ...
    ! S1 authority dispatch is frozen before library execution.
    ! Mode 2 uses ROM1A-D2. Pure prescribed-head rise/fall uses ROM-0R-R3D4.
    ! Combined mode-5 held-out symbols remain strict-only and fail closed here.
    fallback_authorized = p%bottom_mode==2 .or. &
         (p%bottom_mode==5 .and. (symbol==SYM_RISE .or. symbol==SYM_FALL))
    if(.not.fallback_authorized)then
      write(*,'(*(g0))') 'ROM1AS1_UNAUTHORIZED_FALLBACK_BLOCK|BOTTOM_MODE=',p%bottom_mode, &
           '|SYMBOL=',trim(symbol_label(symbol)),'|T0=',t0,'|T1=',t1
      if(allocated(before))deallocate(before)
      return
    end if

    call state%current_time(time_after,time_ok)
    call require(time_ok.and.same_bits(time_after,time_before),'ROM1AS1 failed first attempt time immutable')
    call require(state%current_revision()==rev_before,'ROM1AS1 failed first attempt revision immutable')
    call require(state%current_lineage_id()==lineage_before,'ROM1AS1 failed first attempt lineage immutable')
    call state%snapshot(after,got)
    call require(got,'ROM1AS1 failed first attempt snapshot')
    unchanged=state_bits_equal(before,after)
    call require(unchanged,'ROM1AS1 failed first attempt physical state immutable')

    call prospective_bound(state,p,rep_bound,bound_ok)
    call require(bound_ok.and.rep_bound>0.0_real64,'ROM1AS1 prospective representation bound')
    ...
    call require(trim(failure_class)=='RETRY_TOTAL_ONLY','ROM1AS1 fallback exact total-only')
    abs_integrated=abs(rsum)*(t1-t0)
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=rep_bound,'ROM1AS1 failed residual within bound')
    ...
    call require(ok,'ROM1AS1 qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'ROM1AS1_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
         '|SYMBOL=',trim(symbol_label(symbol)),'|CLASS=',trim(failure_class), &
         '|BAL_FLAGS=',bal_flags,'|HEAD_FLAGS=',head_flags,'|RMAX=',rmax,'|RSUM=',rsum,'|IMAX=',imax, &
         '|REP_BOUND_CM=',rep_bound,'|ABS_TOTAL_RESIDUAL_CM=',abs_integrated,'|FALLBACK_TOTAL_TOL=',fallback_tol, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    call require(got,'ROM1AS1 state snapshot for output')
    ...
    type is(fmr_b110_physical_state_t)
      ...
      call require(all(ieee_is_finite(physical%pressure_head)).and.all(ieee_is_finite(physical%water_content)), &
           'ROM1AS1 finite full profiles')
      write(*,'(*(g0))') 'ROM1AS1_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
           ...
           '|REFERENCE_POLICY=',trim(reference_policy_label(symbol,fallback_used))
      do node=1,numnod
        write(*,'(*(g0))') 'ROM1AS1_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
             '|STEP=',step,'|NODE=',node,'|H=',physical%pressure_head(node),'|THETA=',physical%water_content(node)
      end do
    class default
      call require(.false.,'ROM1AS1 expected B110 physical state')
    end select
  ...
  subroutine metrics_from_physical(physical,total,upper,lower)
    ...
    call require(ieee_is_finite(total).and.ieee_is_finite(upper).and.ieee_is_finite(lower),'ROM1AS1 finite primary outputs')
  ...
  subroutine require_current_time(state,expected)
    ...
    call require(ok.and.same_bits(actual,expected),'ROM1AS1 exact committed time progression')
  ...
  integer function history_symbol(ih,step) result(symbol)
    ...
    call require(symbol>=SYM_HOLD.and.symbol<=SYM_COMBINED_FALL_MINUS,'ROM1AS1 valid history symbol')
  ...
  subroutine configure_symbol(symbol,h0,k0,qeq,p,forcing)
    ...
    case default
      call require(.false.,'ROM1AS1 configure known symbol')
    end select
  ...
  pure function reference_policy_label(symbol,fallback_used) result(label)
    integer,intent(in) :: symbol
    logical,intent(in) :: fallback_used
    character(len=24) :: label
    if(.not.fallback_used)then
      label='STRICT_ORIGINAL'
    else
      select case(symbol)
      case(SYM_HOLD,SYM_TOP_PLUS,SYM_TOP_MINUS)
        label='MODE2_D2_FALLBACK'
      case(SYM_RISE,SYM_FALL)
        label='MODE5_D4_FALLBACK'
      case default
        label='UNAUTHORIZED_FALLBACK'
      end select
    end if
  end function reference_policy_label
  ...
  subroutine diagnose_failed_interval(p,forcing,state,t0,t1,fmr_status,fmr_nl,fmr_back,class_name,bal,head,rmax,rsum,imax)
    ...
    call require(fmr_status==SW_SOLVE_RETRY_ADVISED,'ROM1AS1 FMR failed status retry-advised')
    ...
    call state%snapshot(snap,got);call require(got,'ROM1AS1 diagnostic snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
    ...
    class default
      call require(.false.,'ROM1AS1 diagnostic B110 state')
    end select
    call bind_b110_serialized_legacy_context(request,context_ok);call require(context_ok,'ROM1AS1 diagnostic context')
    call solver%solve(request,workspace,result)
    call require(result%status==SW_SOLVE_RETRY_ADVISED,'ROM1AS1 direct retry advised')
    call require(trim(result%diagnostics%route)=='legacy-reference-retry','ROM1AS1 direct retry route')
    call require(result%diagnostics%nonlinear_iterations==fmr_nl,'ROM1AS1 direct/FMR nonlinear identity')
    call require(result%diagnostics%backtracking_attempts==fmr_back,'ROM1AS1 direct/FMR backtracking identity')
  ...
  subroutine initialize_state(p,h0,k0,state)
    ...
    k0=conductivity(1)
    call require(k0>0.0_real64.and.all(ieee_is_finite(water)),'ROM1AS1 finite seed')
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'ROM1AS1_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_rom1as1_reachable_state_library
