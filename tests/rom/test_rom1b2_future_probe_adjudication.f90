program test_rom1b2_future_probe_adjudication
  ...
  call require(numnod==16,'ROM1B2 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'ROM1B2 depth frozen')
  call get_environment_variable('ROM1B2_STARTS_FILE',starts_file)
  call require(len_trim(starts_file)>0,'ROM1B2 start request file supplied')
  open(newunit=start_unit,file=trim(starts_file),status='old',action='read',iostat=ios)
  call require(ios==0,'ROM1B2 start request file opened')
  start_count=0
  do
    read(start_unit,*,iostat=ios) ih,target_step
    if(ios<0)exit
    call require(ios==0,'ROM1B2 start request row readable')
    call require(ih>=1.and.ih<=8,'ROM1B2 discovery history index')
    call require(target_step>=1.and.target_step<=NSTEPS,'ROM1B2 discovery step index')
    call run_start_request(ih,target_step)
    start_count=start_count+1
  end do
  close(start_unit)
  call require(start_count>0,'ROM1B2 nonempty frozen start set')
  write(*,'(A,I0)') 'ROM1B2_UNIQUE_START_COUNT=',start_count
  write(*,'(A)') 'ROM1B2_HELDOUT_USED=FALSE'
  write(*,'(A)') 'ROM1B2_B14_USED=FALSE'
  write(*,'(A)') 'ROM1B2_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    write(*,'(*(g0))') 'ROM1B2_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=rep_bound, &
         'ROM1B2 total residual within pre-solve bound')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'ROM1B2 total-only flag contract')
      call require(rmax<=p%compartment_balance_tolerance,'ROM1B2 total-only local criterion unchanged')
      fallback_p%compartment_balance_tolerance=p%compartment_balance_tolerance
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'ROM1B2 local-only flag contract')
      call require(ieee_is_finite(local_integrated).and.local_integrated<=1.6e-15_real64, &
           'ROM1B2 local residual within prior integrated allowance')
    ...
    case default
      call require(.false.,'ROM1B2 failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'ROM1B2 qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'ROM1B2_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    type is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'ROM1B2_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'ROM1B2_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'ROM1B2_FAIL',trim(label)
      error stop 1
    end if
  ...
  subroutine run_start_request(ih,target_step)
    ...
    write(*,'(*(g0))') 'ROM1B2_START|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
    ...
    do node=1,numnod
      write(*,'(*(g0))') 'ROM1B2_START_NODE|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
  ...
  subroutine reconstruct_start_state(ih,target_step,physical_out,h0,k0,qeq)
    ...
    call initialize_forcing(forcing,qeq,qeq,h0)
    call fmr_new_b110_committed_state(state,column_id,initial_state,0.0_real64,ok)
    call require(ok.and.state%ready(),'ROM1B2 start initial committed state')
    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'ROM1B2 start steady seed')
    t0=real(seed_intervals,real64)*seed_dt
    do step=1,target_step
      ...
      t1=real(seed_intervals,real64)*seed_dt+real(step,real64)*step_dt
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'ROM1B2 start reconstruction accepted')
      call require(abs(mass)<=hard_mass_gate,'ROM1B2 start reconstruction mass')
      t0=t1
    end do
    call state%snapshot(snap,got)
    call require(got,'ROM1B2 start snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      physical_out=physical
    class default
      call require(.false.,'ROM1B2 expected B110 start state')
    end select
    if(allocated(snap))deallocate(snap)
  ...
  subroutine run_probe(ih,target_step,ip,start_physical,h0,k0,qeq,start_total,start_upper,start_lower)
    ...
    call require(ok.and.state%ready(),'ROM1B2 probe committed start')
    ...
    do step=1,NSTEPS
      ...
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'ROM1B2 frozen future probe accepted')
      call require(abs(mass)<=hard_mass_gate,'ROM1B2 probe hard mass gate')
      ...
      do ihor=1,NHORIZ
        if(step==horizon_steps(ihor))then
          call state%snapshot(snap,got)
          call require(got,'ROM1B2 probe endpoint snapshot')
          select type(physical=>snap)
          type is(fmr_b110_physical_state_t)
            call metrics_from_physical(physical,total,upper,lower)
          class default
            call require(.false.,'ROM1B2 expected B110 probe state')
          end select
          if(allocated(snap))deallocate(snap)
          write(*,'(*(g0))') 'ROM1B2_PROBE|HISTORY=',trim(history_label(ih)),'|START_STEP=',target_step, &
  ...
  integer function probe_symbol(ip) result(symbol)
    ...
    case(4);symbol=SYM_FALL
    case default
      call require(.false.,'ROM1B2 known probe id')
      symbol=0
    end select
...
end program test_rom1b2_future_probe_adjudication
