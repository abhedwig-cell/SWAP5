program test_rom1x1_b14_material_transfer
  ...
  integer(int64), parameter :: column_id=981001_int64

  integer :: ih,total_states,total_fallbacks
  real(real64) :: max_abs_mass

  call require(numnod==16,'ROM1X1 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'ROM1X1 depth frozen')
  ...
  call require(total_states==NHIST*NSTEPS,'ROM1X1 exact library state count')
  write(*,'(A,I0)') 'ROM1X1_HISTORY_COUNT=',NHIST
  write(*,'(A,I0)') 'ROM1X1_DISCOVERY_HISTORY_COUNT=',8
  write(*,'(A,I0)') 'ROM1X1_HELDOUT_HISTORY_COUNT=',4
  write(*,'(A,I0)') 'ROM1X1_STATE_COUNT=',total_states
  write(*,'(A,I0)') 'ROM1X1_DISCOVERY_STATE_COUNT=',8*NSTEPS
  write(*,'(A,I0)') 'ROM1X1_HELDOUT_STATE_COUNT=',4*NSTEPS
  write(*,'(A,I0)') 'ROM1X1_TOTAL_FALLBACK_COUNT=',total_fallbacks
  write(*,'(*(g0))') 'ROM1X1_MAX_ABS_MASS=',max_abs_mass
  write(*,'(A)') 'ROM1X1_B14_MATERIAL_TRANSFER_GENERATED=TRUE'
  write(*,'(A)') 'ROM1X1_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    write(*,'(*(g0))') 'ROM1X1_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    write(*,'(*(g0))') 'ROM1X1_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    type is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'ROM1X1_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'ROM1X1_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine initialize_parameters(p)
    ...
    p%parameter_set_id=981001_int64;p%active_nodes=numnod
  ...
  subroutine initialize_identity(col,tmpl)
    ...
    tmpl%template_id=981001_int64;tmpl%physics_topology_id=971002_int64
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'ROM1X1_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_rom1x1_b14_material_transfer
