program test_rom1x2_b14_predictive_ambiguity
  ...
  integer(int64), parameter :: column_id=982001_int64

  integer, parameter :: NPROBES=4
  integer, parameter :: NHORIZ=3
  integer, parameter :: horizon_steps(NHORIZ)=[4,16,64]
  integer :: start_unit,ios,ih,target_step,start_count
  character(len=512) :: starts_file

  call require(numnod==16,'ROM1X2 geometry frozen at 16 nodes')
  call require(abs(sum(dz(1:numnod))-160.0_real64)<=1.0e-12_real64,'ROM1X2 depth frozen')
  call get_environment_variable('ROM1X2_STARTS_FILE',starts_file)
  call require(len_trim(starts_file)>0,'ROM1X2 start request file supplied')
  open(newunit=start_unit,file=trim(starts_file),status='old',action='read',iostat=ios)
  call require(ios==0,'ROM1X2 start request file opened')
  start_count=0
  do
    read(start_unit,*,iostat=ios) ih,target_step
    if(ios<0)exit
    call require(ios==0,'ROM1X2 start request row readable')
    call require(ih>=1.and.ih<=8,'ROM1X2 discovery history index')
    call require(target_step>=1.and.target_step<=NSTEPS,'ROM1X2 discovery step index')
    call run_start_request(ih,target_step)
    start_count=start_count+1
  end do
  close(start_unit)
  call require(start_count>0,'ROM1X2 nonempty frozen start set')
  write(*,'(A,I0)') 'ROM1X2_UNIQUE_START_COUNT=',start_count
  write(*,'(A)') 'ROM1X2_HELDOUT_USED=FALSE'
  write(*,'(A)') 'ROM1X2_B14_USED=TRUE'
  write(*,'(A)') 'ROM1X2_EXECUTION_COMPLETE=PASS'
...
contains
  ...
  subroutine run_history(ih,total_states,total_fallbacks,max_abs_mass)
    ...
    write(*,'(*(g0))') 'ROM1X2_HISTORY_PASS|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  subroutine strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
    ...
    call require(ieee_is_finite(abs_integrated).and.abs_integrated<=rep_bound, &
         'ROM1X2 total residual within pre-solve bound')
    ...
    case('RETRY_TOTAL_ONLY')
      call require(bal_flags==0.and.head_flags==0,'ROM1X2 total-only flag contract')
      call require(rmax<=p%compartment_balance_tolerance,'ROM1X2 total-only local criterion unchanged')
      fallback_p%compartment_balance_tolerance=p%compartment_balance_tolerance
    case('RETRY_LOCAL_BALANCE')
      call require(bal_flags>0.and.head_flags==0,'ROM1X2 local-only flag contract')
      call require(ieee_is_finite(local_integrated).and.local_integrated<=1.6e-15_real64, &
           'ROM1X2 local residual within prior integrated allowance')
    ...
    case default
      call require(.false.,'ROM1X2 failure class not authorized')
    end select

    call sample_fresh(column,template,fallback_p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back)
    call require(ok,'ROM1X2 qualified fallback reattempt')
    fallback_used=.true.

    write(*,'(*(g0))') 'ROM1X2_FALLBACK|HISTORY_STEP_T0=',t0,'|T1=',t1,'|BOTTOM_MODE=',p%bottom_mode, &
  ...
  subroutine emit_state(ih,step,symbol,state,forcing,t0,t1,mass,bex,bflux,nl,back,fallback_used)
    ...
    type is(fmr_b110_physical_state_t)
      ...
      write(*,'(*(g0))') 'ROM1X2_STATE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
      ...
      do node=1,numnod
        write(*,'(*(g0))') 'ROM1X2_NODE|SPLIT=',trim(split_label(ih)),'|HISTORY=',trim(history_label(ih)), &
  ...
  integer function history_symbol(ih,step) result(symbol)
    ...
    case(9)
      if(step<=8)then;symbol=SYM_TOP_MINUS
      else if(step<=32)then;symbol=SYM_TOP_PLUS
      else;symbol=SYM_TOP_MINUS;end if
    case(10); symbol=merge(SYM_FALL,SYM_RISE,step<=24)
    case(11)
      if(step<=8)then;symbol=SYM_HOLD
      else if(step<=24)then;symbol=SYM_RISE
      else if(step<=40)then;symbol=SYM_TOP_MINUS
      else if(step<=56)then;symbol=SYM_FALL
      else;symbol=SYM_TOP_PLUS;end if
    case(12); symbol=merge(SYM_COMBINED_RISE_PLUS,SYM_COMBINED_FALL_MINUS,step<=32)
    case default
      symbol=0
    end select
    call require(symbol>=SYM_HOLD.and.symbol<=SYM_COMBINED_FALL_MINUS,'ROM1A valid history symbol')
  ...
  subroutine initialize_parameters(p)
    ...
    p%parameter_set_id=982001_int64;p%active_nodes=numnod
  ...
  subroutine initialize_identity(col,tmpl)
    ...
    tmpl%template_id=982001_int64;tmpl%physics_topology_id=971002_int64
  ...
  subroutine require(condition,label)
    ...
    if(.not.condition)then
      write(*,'(A,1X,A)') 'ROM1X2_FAIL',trim(label)
      error stop 1
    end if
  ...
  subroutine run_start_request(ih,target_step)
    ...
    write(*,'(*(g0))') 'ROM1X2_START|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
    ...
    do node=1,numnod
      write(*,'(*(g0))') 'ROM1X2_START_NODE|HISTORY=',trim(history_label(ih)),'|STEP=',target_step, &
  ...
  subroutine reconstruct_start_state(ih,target_step,physical_out,h0,k0,qeq)
    ...
    call initialize_forcing(forcing,qeq,qeq,h0)
    call fmr_new_b110_committed_state(state,column_id,initial_state,0.0_real64,ok)
    call require(ok.and.state%ready(),'ROM1X2 start initial committed state')
    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'ROM1X2 start steady seed')
    t0=real(seed_intervals,real64)*seed_dt
    do step=1,target_step
      ...
      t1=real(seed_intervals,real64)*seed_dt+real(step,real64)*step_dt
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'ROM1X2 start reconstruction accepted')
      call require(abs(mass)<=hard_mass_gate,'ROM1X2 start reconstruction mass')
      t0=t1
    end do
    call state%snapshot(snap,got)
    call require(got,'ROM1X2 start snapshot')
    select type(physical=>snap)
    type is(fmr_b110_physical_state_t)
      physical_out=physical
    class default
      call require(.false.,'ROM1X2 expected B110 start state')
    end select
    if(allocated(snap))deallocate(snap)
  ...
  subroutine run_probe(ih,target_step,ip,start_physical,h0,k0,qeq,start_total,start_upper,start_lower)
    ...
    call require(ok.and.state%ready(),'ROM1X2 probe committed start')
    cum_top=0.0_real64;cum_bottom=0.0_real64;max_mass=0.0_real64
    fallback_count=0;t0=0.0_real64
    symbol=probe_symbol(ip)

    do step=1,NSTEPS
      call configure_symbol(symbol,h0,k0,qeq,p,forcing)
      t1=real(step,real64)*step_dt
      call strict_first_sample(column,template,p,state,forcing,t0,t1,ok,mass,bex,bflux,status,route,nl,ir,back,fallback_used)
      call require(ok,'ROM1X2 frozen future probe accepted')
      call require(abs(mass)<=hard_mass_gate,'ROM1X2 probe hard mass gate')
      ...
      do ihor=1,NHORIZ
        if(step==horizon_steps(ihor))then
          call state%snapshot(snap,got)
          call require(got,'ROM1X2 probe endpoint snapshot')
          select type(physical=>snap)
          type is(fmr_b110_physical_state_t)
            call metrics_from_physical(physical,total,upper,lower)
          class default
            call require(.false.,'ROM1X2 expected B110 probe state')
          end select
          if(allocated(snap))deallocate(snap)
          write(*,'(*(g0))') 'ROM1X2_PROBE|HISTORY=',trim(history_label(ih)),'|START_STEP=',target_step, &
  ...
  integer function probe_symbol(ip) result(symbol)
    ...
    case(4);symbol=SYM_FALL
    case default
      call require(.false.,'ROM1X2 known probe id')
      symbol=0
    end select
  ...
  pure function probe_label(ip) result(label)
    ...
    case(3);label='BOTTOM_HEAD_RISE'
    case(4);label='BOTTOM_HEAD_FALL'
    case default;label='UNKNOWN'
    end select
  end function probe_label

end program test_rom1x2_b14_predictive_ambiguity
