...
def main():
    ...
    if not fingerprint_match:
        raise SystemExit(
            f"F_ROSS25_FAIL table fingerprint {a.material} generated={generated_fingerprint} expected={expected_fingerprint}"
        )
    ...
    if len(fixture_rows) != EXPECTED_KERNEL_CASES:
        raise SystemExit(f"F_ROSS25_FAIL expected {EXPECTED_KERNEL_CASES} kernel cases got {len(fixture_rows)}")
