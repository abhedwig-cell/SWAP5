...
def run_science_case(gate_f, material: str, se: float, row: dict, table):
    ...
    return {
        ...
        "sigma": gate_f.SIGMA,
        "initial_head_cm": h0,
        "initial_heads": list(initial_heads),
        "initial_theta": list(theta0),
        "reference_fine_heads": list(ref_fine["heads"]),
        "reference_fine_theta": list(ref_fine["theta"]),
...
def evaluate_science(gate_f, material: str, row: dict, table):
    ...
    nonroute_pass = all(nontransition_tests.values())
    ...
    return {
        "pass": nonroute_pass and route_policy_pass,
        "nonroute_pass": nonroute_pass,
...
def main():
    ...
    if contract["work_unit"] != "F-ROSS26":
    ...
    if a.material not in materials:
        raise SystemExit(f"material outside F-ROSS26 domain: {a.material}")
    ...
    if not fingerprint_match:
        raise SystemExit(
            f"F_ROSS26_FAIL table fingerprint {a.material} generated={generated_fingerprint} expected={expected_fingerprint}"
        )

    science = evaluate_science(gate_f, a.material, row, table)
    science_pass = bool(science["nonroute_pass"])

    fixture_rows = []
    for case in science["cases"]:
        se = float(case["effective_saturation"])
        heads0 = tuple(float(v) for v in case["initial_heads"])
        theta0 = tuple(float(v) for v in case["initial_theta"])
        ext = target_external(gate_f, heads0)
        fixture_rows.append({
            "se": se,
            "q_top": float(ext["q_top"]),
            "q_bottom_up": -float(ext["q_bottom"]),
            "reference_self": float(case["reference_self_theta_span_normalized"]),
            "initial_heads": heads0,
            "initial_theta": theta0,
            "reference_heads": tuple(float(v) for v in case["reference_fine_heads"]),
            "reference_theta": tuple(float(v) for v in case["reference_fine_theta"]),
        })

    if len(fixture_rows) != EXPECTED_KERNEL_CASES:
        raise SystemExit(f"F_ROSS26_FAIL expected {EXPECTED_KERNEL_CASES} kernel cases got {len(fixture_rows)}")
    ...
    with a.fixture_out.open("w", encoding="utf-8") as f:
        ...
        for case in fixture_rows:
            f.write(
                f"{case['se']:.17e} {case['q_top']:.17e} {case['q_bottom_up']:.17e} "
                f"{case['reference_self']:.17e}\n"
            )
            write_vector(f, case["initial_heads"])
            write_vector(f, case["initial_theta"])
            write_vector(f, case["reference_heads"])
            write_vector(f, case["reference_theta"])
    ...
    metadata = {
        ...
        "historical_route_policy_pass": bool(science["route_policy_pass"]),
    ...
    raise SystemExit(0 if science_pass and fingerprint_match else 1)
