...
contains
  ...
  subroutine rossfast_d3r_table_kernel_solve(self, request, result)
    ...
    ! Research-only F-ROSS19 rule: K2 is authoritative only when its own
    ! model certificate is acceptable. Otherwise recompute independently from
    ! the original base state with K4. No K2 trajectory/state is reused.
  ...
  logical function request_contract_is_admitted(request)
    ...
    ! F-ROSS19 research ABI advertises the fast K2 attempt. K4 is an internal
