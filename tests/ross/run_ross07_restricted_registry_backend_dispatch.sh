...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ross07-${GITHUB_RUN_ID:-local}-$$"
...
TEST=tests/ross/test_ross07_restricted_registry_backend_dispatch.f90
ASSET_ROOT=assets/rossfast/d3r
ROSS06_STATUS=integration/f-ross/F-ROSS06_STATUS.json

# Immutable qualified dependencies inherited from the green F-ROSS06 stack.
...
test "$(git rev-parse HEAD:$ROSS06_STATUS)" = 4a7c855bb026e2a5be1c64288c53f4b1829e751c

# This workunit is registry/application dispatch only. It must not route through
# or alter the B1.10 serialized reference backend/multiswap implementation.
...
grep -Fq 'FMR_BACKEND_SERIALIZED_INJECTED_KERNEL' "$DISPATCH"
grep -Fq 'call fmr_build_execution_order' "$DISPATCH"
grep -Fq 'Preflight every selected immutable table before any physical transaction' "$DISPATCH"
grep -Fq 'rossfast_d3r_select_transaction_window' "$DISPATCH"
...
for opt in o0 o2; do
  ...
  gfortran -fopenmp "$moddir/tx.o" "$moddir/contracts.o" "$moddir/runtime.o" \
    ...
    "$moddir/serial_backend.o" "$moddir/dispatch.o" "$moddir/test.o" -o "$moddir/test"

  "$moddir/test" "$ASSET_ROOT" > "$moddir/output.txt"
  grep -Fq 'ROSS07_RESTRICTED_REGISTRY_BACKEND_DISPATCH PASS' "$moddir/output.txt"
  for material in B01 B12 O01 O05 O14 O18; do
    grep -Fq "ROSS07_MATERIAL $material" "$moddir/output.txt"
  done
done

cmp "$BUILD/o0/output.txt" "$BUILD/o2/output.txt"
cat "$BUILD/o0/output.txt"
echo "ROSS07_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'ROSS07_RESTRICTED_REGISTRY_BACKEND_DISPATCH=PASS'
