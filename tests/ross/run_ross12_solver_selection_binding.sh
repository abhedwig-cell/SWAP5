...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ross12-selection-${GITHUB_RUN_ID:-local}-$$"
...
SELECTION_BINDING=src/runtime/mod_fmr_rossfast_solver_selection_binding.f90
TEST=tests/ross/test_ross12_solver_selection_binding.f90
...
current_model="$(git rev-parse HEAD:$MODEL_BINDING)"
...
test "$(git rev-parse HEAD:$MODEL_BINDING)" = "$EXPECTED_MODEL"
...
for opt in o0 o2; do
  ...
  gfortran "${WARN[@]}" "$flag" -std=f2008 -J "$m" -I "$m" -c "$MODEL_BINDING" -o "$m/model_binding.o"
  ...
  gfortran "${WARN[@]}" "$flag" -std=f2008 -J "$m" -I "$m" -c "$APP_HOST" -o "$m/app_host.o"
  gfortran "${WARN[@]}" "$flag" -std=f2008 -J "$m" -I "$m" -c "$SELECTION_BINDING" -o "$m/selection_binding.o"
  ...
  gfortran -fopenmp "$m/tx.o" "$m/stepdir.o" "$m/trajsens.o" "$m/trajpub.o" "$m/contracts.o" "$m/sw.o" \
    "$m/refbind.o" "$m/policy.o" "$m/model_binding.o" "$m/kernel.o" "$m/provider.o" "$m/adapter.o" \
    "$m/app_host.o" "$m/selection_binding.o" "$m/test.o" -o "$m/test"
  if ! SWAP5_ROSSFAST_EXPECT_TIERED_WORK="$ROSSFAST_TIERED_MODE" "$m/test" "$ASSET_ROOT" > "$m/output.txt"; then
    cat "$m/output.txt"
    exit 1
  fi
  grep -Fq 'ROSS12_SOLVER_SELECTION_BINDING PASS' "$m/output.txt"
done
cmp "$BUILD/o0/output.txt" "$BUILD/o2/output.txt"
cat "$BUILD/o0/output.txt"
echo "F_ROSS12_SOLVER_SELECTION_BINDING_O0_O2_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo "F_ROSS12_SOLVER_SELECTION_BINDING=PASS"
