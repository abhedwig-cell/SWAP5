...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-f-ross23-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
...
git diff --quiet "$CURRENT_BASE" HEAD -- src reference || fail 'F-ROSS23 mutated src or reference'
echo "F_ROSS23_RECONCILED_BASE=$CURRENT_BASE"
...
gfortran -fopenmp -O2 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/f_ross23_benchmark"

python3 "$RUNNER" --executable "$BUILD/f_ross23_benchmark" --output "$BUILD/F-ROSS23_SCREENING_RESULT.json" | tee "$BUILD/output.txt"

for marker in \
  'F_ROSS23_SCREENING_OUTCOME=' \
  'F_ROSS23_MEAN_RELATIVE_DELTA=' \
  'F_ROSS23_MDE=' \
  'F_ROSS23_MEAN_SPEEDUP=' \
  'F_ROSS23_FORMAL_SPEEDUP_CLAIM=FALSE' \
  'F_ROSS23_SCREENING_GATE=PASS'; do
  grep -Fq "$marker" "$BUILD/output.txt" || fail "missing marker $marker"
done

cp "$BUILD/F-ROSS23_SCREENING_RESULT.json" "${GITHUB_WORKSPACE:-$ROOT}/F-ROSS23_SCREENING_RESULT.json"
echo 'F_ROSS23_PERFORMANCE_SCREEN=PASS'
