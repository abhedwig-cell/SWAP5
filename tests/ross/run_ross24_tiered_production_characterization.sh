...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-f-ross24-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"
fail() { echo "F_ROSS24_GATE_FAIL $*" >&2; exit 1; }

PREREG=integration/f-ross/F-ROSS24_TIERED_CHARACTERIZATION_PREREGISTRATION.json
TEST=tests/ross/test_ross24_tiered_production_characterization.f90
RUNNER=tools/performance/f_ross24_paired_screen.py
...
for path in "$PREREG" "$TEST" "$RUNNER"; do [[ -f "$path" ]] || fail "missing $path"; done
grep -Fq '"phase": "DESIGN"' "$PREREG" || fail 'preregistration phase drift'
grep -Fq '"case_count": 216' "$PREREG" || fail '216-case contract drift'
grep -Fq '"measured_pairs": 12' "$PREREG" || fail 'pair-count drift'
grep -Fq '"inner_repetitions_per_paired_valid_case": 200' "$PREREG" || fail 'repetition-count drift'
grep -Fq '"NO_PRODUCTION_OPTIMIZATION"' "$PREREG" || fail 'optimization firewall drift'
grep -Fq '"NO_CERTIFICATE_THRESHOLD_CHANGE"' "$PREREG" || fail 'certificate firewall drift'

git merge-base --is-ancestor "$FROSS22_ADMISSION" HEAD || fail 'F-ROSS22 canonical admission not ancestor'
BASE="$(git merge-base HEAD origin/integration/f-ci-canonical)"
[[ -n "$BASE" ]] || fail 'unable to resolve canonical merge-base'
git diff --quiet "$BASE" HEAD -- src reference || fail 'F-ROSS24 mutated src or reference'
...
gfortran -fopenmp -O2 "${objects[@]}" "$BUILD/test.o" -o "$BUILD/f_ross24_benchmark"

python3 "$RUNNER" --executable "$BUILD/f_ross24_benchmark" --output "$BUILD/F-ROSS24_SCREENING_RESULT.json" | tee "$BUILD/output.txt"
for marker in   'F_ROSS24_SCREENING_OUTCOME='   'F_ROSS24_MEAN_RELATIVE_DELTA='   'F_ROSS24_MDE='   'F_ROSS24_MEAN_SPEEDUP='   'F_ROSS24_FORMAL_SPEEDUP_CLAIM=FALSE'   'F_ROSS24_SCREENING_GATE=PASS'; do
  grep -Fq "$marker" "$BUILD/output.txt" || fail "missing marker $marker"
done
cp "$BUILD/F-ROSS24_SCREENING_RESULT.json" "${GITHUB_WORKSPACE:-$ROOT}/F-ROSS24_SCREENING_RESULT.json"
echo 'F_ROSS24_TIERED_CHARACTERIZATION_SCREEN=PASS'
