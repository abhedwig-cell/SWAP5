...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-ross26-${MATERIAL}-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD/o0" "$BUILD/o2"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"
fail() { echo "F_ROSS26_GATE_FAIL $*" >&2; exit 1; }

CONTRACT=integration/f-ross/F-ROSS26_TIERED_WETTING_SEAM_QUALIFICATION_CONTRACT.json
TEST=tests/ross/test_ross26_tiered_wetting_kernel.f90
...
git diff --quiet "$BASE" HEAD -- src reference || fail 'F-ROSS26 mutated src or reference'
...
for opt in 0 2; do
  ...
  grep -Fq 'F_ROSS26_CASE_COUNT=3' "$OUT/output.txt" || fail "missing exact case count O$opt"
  grep -Fq 'F_ROSS26_TIERED_WETTING_KERNEL=PASS' "$OUT/output.txt" || fail "missing qualification pass O$opt"
  cat "$OUT/output.txt"
  echo "F_ROSS26_${MATERIAL}_O${opt}=PASS"
done
...
cp "$BUILD/o0/output.txt" "${GITHUB_WORKSPACE:-$ROOT}/F-ROSS26-${MATERIAL}-output.txt"
echo "F_ROSS26_${MATERIAL}_O0_O2_SHA256=$HASH"
echo "F_ROSS26_${MATERIAL}_MATERIAL_QUALIFICATION=PASS"
