program test_ross15_reference_vs_rossfast_performance
  ...
  write(*,'(A,A)') 'F_ROSS15_ROUTE=',trim(selected_route)
  write(*,'(A,I0)') 'F_ROSS15_CASE_COUNT=',case_id
  write(*,'(A,I0)') 'F_ROSS15_INNER_REPETITIONS=',inner_repetitions
  write(*,'(A,I0)') 'F_ROSS15_TIMED_SOLVE_COUNT=',case_id*inner_repetitions
  write(*,'(A,ES24.16E3)') 'F_ROSS15_SOLVER_CPU_SECONDS=',total_solver_cpu_seconds
  write(*,'(A,ES24.16E3)') 'F_ROSS15_CHECKSUM=',checksum
  write(*,'(A)') 'F_ROSS15_LAYER=A_FIXED_INTERVAL_SOLVER_SEAM'
  write(*,'(A)') 'F_ROSS15_STEP_DURATION_DAY=0.0016'
  write(*,'(A)') 'F_ROSS15_TIMED_REGION=SOLVE_CALLS_ONLY'
  write(*,'(A)') 'F_ROSS15_GATE=PASS'
...
contains
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'F_ROSS15_FAIL',trim(label)
      error stop 1
    end if
