program test_ross16_profile
  ...
  integer, parameter :: inner_repetitions=5000
  ...
  checksum=0.0_real64
  selected_route=''
  call get_environment_variable('SWAP5_ROSS16_PROFILE_ROUTE',selected_route,status=env_status)
  call require(env_status==0,'SWAP5_ROSS16_PROFILE_ROUTE must be present')
  ...
  call require(case_id==expected_cases,'exact 36-case performance domain attempted')
  call require(count_admissible==expected_cases,'all 36 preregistered cases remain paired-valid admissible')
  ...
  write(*,'(A,A)') 'F_ROSS16_PROFILE_ROUTE=',trim(selected_route)
  write(*,'(A,I0)') 'F_ROSS16_PROFILE_CASE_COUNT=',case_id
  write(*,'(A,I0)') 'F_ROSS16_PROFILE_INNER_REPETITIONS=',inner_repetitions
  write(*,'(A,I0)') 'F_ROSS16_PROFILE_TIMED_SOLVE_COUNT=',case_id*inner_repetitions
  write(*,'(A,ES24.16E3)') 'F_ROSS16_PROFILE_SOLVER_CPU_SECONDS=',total_solver_cpu_seconds
  write(*,'(A,ES24.16E3)') 'F_ROSS16_PROFILE_CHECKSUM=',checksum
  write(*,'(A)') 'F_ROSS16_PROFILE_LAYER=A_FIXED_INTERVAL_SOLVER_SEAM'
  write(*,'(A)') 'F_ROSS16_PROFILE_STEP_DURATION_DAY=0.0016'
  write(*,'(A)') 'F_ROSS16_PROFILE_TIMED_REGION=SOLVE_CALLS_ONLY'
  write(*,'(A)') 'F_ROSS16_PROFILE_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,ise,material_id,se,forcing_id,top_factor,bottom_factor,classification,metrics_available, &
    ...
    call require(trim(classification)=='PAIRED_VALID_ADMISSIBLE','performance case must retain P2E10 admissibility')
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'F_ROSS16_PROFILE_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_ross16_profile
