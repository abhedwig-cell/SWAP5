program test_ross17_state_dump
  ...
  call require(case_id==expected_cases,'exact 36-case diagnostic domain attempted')
  call require(count_admissible==expected_cases,'all 36 cases remain paired-valid admissible')
  ...
  write(*,'(A,I0)') 'F_ROSS17_CASE_COUNT=',case_id
  write(*,'(A)') 'F_ROSS17_STATE_DUMP_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,ise,material_id,se,forcing_id,top_factor,bottom_factor,classification,metrics_available, &
    ...
    call require(alternative_result%diagnostics%linear_solves==24,'exact 24 RossFast linear solves')
    ...
    write(*,'(*(g0))') 'F_ROSS17_CASE|CASE=',case_id,'|ISE=',ise,'|M=',trim(material_id),'|F=',trim(forcing_id), &
    ...
    write(*,'(A,I0,A)',advance='no') 'F_ROSS17_H|CASE=',case_id,'|'
    do i=1,n
      write(*,'(ES24.16E3,1X)',advance='no') alternative_result%candidate_state%pressure_head(i)
    end do
    write(*,*)
    write(*,'(A,I0,A)',advance='no') 'F_ROSS17_T|CASE=',case_id,'|'
    do i=1,n
      write(*,'(ES24.16E3,1X)',advance='no') alternative_result%candidate_state%water_content(i)
    end do
    write(*,*)
    call require(trim(classification)=='PAIRED_VALID_ADMISSIBLE','diagnostic case must retain P2E10 admissibility')
    write(*,'(*(g0))') 'F_ROSS16_CASE|CASE=',case_id,'|M=',trim(material_id),'|SE=',se,'|F=',trim(forcing_id), &
         '|REF_NL=',reference_result%diagnostics%nonlinear_iterations, &
         '|REF_JAC=',reference_result%diagnostics%jacobian_builds, &
         '|REF_LIN=',reference_result%diagnostics%linear_solves, &
         '|REF_BT=',reference_result%diagnostics%backtracking_attempts, &
         '|REF_RETRY=',reference_result%diagnostics%internal_retries, &
         '|REF_ALT=',reference_result%diagnostics%alternative_solver_calls, &
         '|ROSS_LIN=',alternative_result%diagnostics%linear_solves, &
         '|ROSS_RETRY=',alternative_result%diagnostics%internal_retries, &
         '|ROSS_ALT=',alternative_result%diagnostics%alternative_solver_calls
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'F_ROSS17_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_ross17_state_dump
