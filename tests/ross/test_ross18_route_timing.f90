program test_ross18_route_timing
  ...
  integer, parameter :: nmat=6, nse=3, nforcing=2, expected_cases=nmat*nse*nforcing
  ...
  integer :: env_status, expected_linear_solves
  character(len=16) :: expected_linear_solves_text
    ...
    do ise=1,nse
      do iforce=1,nforcing
        ...
        call run_case(case_id,ise,material_ids(imat),se_levels(ise),forcing_ids(iforce), &
             ...
             selected_route,expected_linear_solves,total_solver_cpu_seconds,checksum)
  ...
  call require(case_id==expected_cases,'exact 36-case research performance domain attempted')
  call require(count_reference_invalid==0 .and. count_rossfast_invalid==0 .and. count_both_invalid==0, &
       'both routes must remain executable for all 36 timing cases')
  ...
  write(*,'(A,A)') 'F_ROSS18_ROUTE=',trim(selected_route)
  write(*,'(A,I0)') 'F_ROSS18_CASE_COUNT=',case_id
  write(*,'(A,I0)') 'F_ROSS18_INNER_REPETITIONS=',inner_repetitions
  write(*,'(A,I0)') 'F_ROSS18_TIMED_SOLVE_COUNT=',case_id*inner_repetitions
  write(*,'(A,ES24.16E3)') 'F_ROSS18_SOLVER_CPU_SECONDS=',total_solver_cpu_seconds
  write(*,'(A,ES24.16E3)') 'F_ROSS18_CHECKSUM=',checksum
  write(*,'(A)') 'F_ROSS18_LAYER=A_FIXED_INTERVAL_SOLVER_SEAM'
  write(*,'(A)') 'F_ROSS18_STEP_DURATION_DAY=0.0016'
  write(*,'(A)') 'F_ROSS18_TIMED_REGION=SOLVE_CALLS_ONLY'
  write(*,'(A,I0)') 'F_ROSS18_EXPECTED_LINEAR_SOLVES=',expected_linear_solves
  write(*,'(A,I0)') 'F_ROSS18_PAIRED_ADMISSIBLE_COUNT=',count_admissible
  write(*,'(A,I0)') 'F_ROSS18_DISCREPANCY_FAIL_COUNT=',count_discrepancy_fail
  write(*,'(A)') 'F_ROSS18_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,ise,material_id,se,forcing_id,top_factor,bottom_factor,classification,metrics_available, &
       ...
       selected_route,expected_linear_solves,solver_cpu_accum,checksum_accum)
    ...
    integer,intent(in) :: expected_linear_solves
    ...
    case('ROSSFAST')
      ...
      call require(alternative_result%diagnostics%linear_solves==expected_linear_solves, &
           'timed RossFast work count matches research variant')
      call require(alternative_result%diagnostics%internal_retries==0 .and. &
           alternative_result%diagnostics%alternative_solver_calls==0,'timed RossFast retry path remains unused')
    case default
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'F_ROSS18_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_ross18_route_timing
