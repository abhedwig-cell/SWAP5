program test_ross18_substep_matrix
  ...
  integer :: expected_linear_solves, env_status
  ...
  call require(case_id==expected_cases,'exact 36-case research domain attempted')
  ...
  write(*,'(*(g0))') 'F_ROSS18_COUNTS|ADMISSIBLE=',count_admissible,'|DISCFAIL=',count_discrepancy_fail, &
  ...
  write(*,'(A)') 'F_ROSS18_MATRIX_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,ise,material_id,se,forcing_id,top_factor,bottom_factor,classification,metrics_available, &
    ...
    if (rossfast_valid) then
      ...
      call require(alternative_result%diagnostics%linear_solves==expected_linear_solves,'expected RossFast linear solves')
      ...
      write(*,'(*(g0))') 'F_ROSS18_CASE|CASE=',case_id,'|ISE=',ise,'|M=',trim(material_id),'|F=',trim(forcing_id), &
      ...
      write(*,'(A,I0,A)',advance='no') 'F_ROSS18_H|CASE=',case_id,'|'
      do i=1,n
        write(*,'(ES24.16E3,1X)',advance='no') alternative_result%candidate_state%pressure_head(i)
      end do
      write(*,*)
      write(*,'(A,I0,A)',advance='no') 'F_ROSS18_T|CASE=',case_id,'|'
      do i=1,n
      ...
      write(*,'(*(g0))') 'F_ROSS18_CASE|CASE=',case_id,'|ISE=',ise,'|M=',trim(material_id),'|F=',trim(forcing_id), &
           '|CLASS=',trim(classification),'|LIN=-1|RETRY=-1|ALT=-1|MASS=NaN|TEMP=NaN', &
           '|HINF=',dh_inf,'|HRMS=',dh_rms,'|TINF=',dtheta_inf,'|TRMS=',dtheta_rms,'|STORAGE=',dstorage
    end if
...
end program test_ross18_substep_matrix
