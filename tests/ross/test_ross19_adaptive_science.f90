program test_ross19_adaptive_science
  ...
  write(*,'(*(g0))') 'F_ROSS19_COUNTS|ADMISSIBLE=',count_admissible,'|DISCFAIL=',count_discrepancy_fail, &
       '|ROSSINVALID=',count_rossfast_invalid,'|REFINVALID=',count_reference_invalid,'|BOTHINVALID=',count_both_invalid
  write(*,'(A)') 'F_ROSS19_MATRIX_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,ise,material_id,se,forcing_id,top_factor,bottom_factor,classification,metrics_available, &
    ...
    if (rossfast_valid) then
      ...
      write(*,'(*(g0))') 'F_ROSS19_CASE|CASE=',case_id,'|ISE=',ise,'|M=',trim(material_id),'|F=',trim(forcing_id), &
      ...
      write(*,'(A,I0,A)',advance='no') 'F_ROSS19_H|CASE=',case_id,'|'
      do i=1,n
        write(*,'(ES24.16E3,1X)',advance='no') alternative_result%candidate_state%pressure_head(i)
      end do
      write(*,*)
      write(*,'(A,I0,A)',advance='no') 'F_ROSS19_T|CASE=',case_id,'|'
      do i=1,n
...
end program test_ross19_adaptive_science
