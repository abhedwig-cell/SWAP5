program test_ross19_adaptive_timing
  ...
  checksum=0.0_real64
  selected_route=''
  call get_environment_variable('SWAP5_ROSS19_ROUTE',selected_route,status=env_status)
  call require(env_status==0,'SWAP5_ROSS19_ROUTE must be present')
  ...
  write(*,'(A,A)') 'F_ROSS19_ROUTE=',trim(selected_route)
  write(*,'(A,I0)') 'F_ROSS19_CASE_COUNT=',case_id
  write(*,'(A,I0)') 'F_ROSS19_INNER_REPETITIONS=',inner_repetitions
  write(*,'(A,I0)') 'F_ROSS19_TIMED_SOLVE_COUNT=',case_id*inner_repetitions
  write(*,'(A,ES24.16E3)') 'F_ROSS19_SOLVER_CPU_SECONDS=',total_solver_cpu_seconds
  write(*,'(A,ES24.16E3)') 'F_ROSS19_CHECKSUM=',checksum
  write(*,'(A)') 'F_ROSS19_LAYER=A_FIXED_INTERVAL_SOLVER_SEAM'
  write(*,'(A)') 'F_ROSS19_STEP_DURATION_DAY=0.0016'
  write(*,'(A)') 'F_ROSS19_TIMED_REGION=SOLVE_CALLS_ONLY'
  write(*,'(A,I0)') 'F_ROSS19_PAIRED_ADMISSIBLE_COUNT=',count_admissible
  write(*,'(A,I0)') 'F_ROSS19_DISCREPANCY_FAIL_COUNT=',count_discrepancy_fail
  write(*,'(A)') 'F_ROSS19_GATE=PASS'
...
contains
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'F_ROSS19_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_ross19_adaptive_timing
