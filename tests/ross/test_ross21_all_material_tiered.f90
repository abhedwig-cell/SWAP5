program test_ross21_all_material_tiered
  ...
  write(*,'(A,I0)') 'F_ROSS21_CASE_COUNT=',case_id
  write(*,'(*(g0))') 'F_ROSS21_COUNTS|ADMISSIBLE=',count_admissible,'|DISCFAIL=',count_discrepancy_fail, &
       '|ROSSINVALID=',count_rossfast_invalid,'|REFINVALID=',count_reference_invalid,'|BOTHINVALID=',count_both_invalid
  write(*,'(A)') 'F_ROSS21_MATRIX_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,ise,material_id,se,forcing_id,top_factor,bottom_factor,classification,metrics_available, &
    ...
    if (rossfast_valid) then
      ...
      call require(alternative_result%diagnostics%linear_solves==6 .or. &
           alternative_result%diagnostics%linear_solves==18 .or. &
           alternative_result%diagnostics%linear_solves==42,'tiered RossFast work count must be 6, 18 or 42')
      ...
      write(*,'(*(g0))') 'F_ROSS21_CASE|CASE=',case_id,'|ISE=',ise,'|M=',trim(material_id),'|F=',trim(forcing_id), &
           ...
           '|LEVEL=',merge(2,merge(1,0,alternative_result%diagnostics%linear_solves==18), &
                alternative_result%diagnostics%linear_solves==42), &
      ...
      write(*,'(A,I0,A)',advance='no') 'F_ROSS21_H|CASE=',case_id,'|'
      do i=1,n
        write(*,'(ES24.16E3,1X)',advance='no') alternative_result%candidate_state%pressure_head(i)
      end do
      write(*,*)
      write(*,'(A,I0,A)',advance='no') 'F_ROSS21_T|CASE=',case_id,'|'
      do i=1,n
      ...
      write(*,'(*(g0))') 'F_ROSS21_CASE|CASE=',case_id,'|ISE=',ise,'|M=',trim(material_id),'|F=',trim(forcing_id), &
           '|CLASS=',trim(classification),'|LIN=-1|RETRY=-1|ALT=-1|LEVEL=-1|MASS=NaN|TEMP=NaN', &
           '|HINF=',dh_inf,'|HRMS=',dh_rms,'|TINF=',dtheta_inf,'|TRMS=',dtheta_rms,'|STORAGE=',dstorage
    end if
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'F_ROSS21_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_ross21_all_material_tiered
