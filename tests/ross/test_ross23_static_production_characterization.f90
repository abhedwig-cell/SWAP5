program test_ross15_reference_vs_rossfast_performance
  ...
  checksum=0.0_real64
  selected_route=''
  call get_environment_variable('SWAP5_ROSS23_ROUTE',selected_route,status=env_status)
  call require(env_status==0,'SWAP5_ROSS23_ROUTE must be present')
  ...
  call require(count_admissible==expected_cases,'all 216 preregistered cases remain paired-valid admissible')
  ...
  write(*,'(A)') 'F_ROSS23_STEP_DURATION_DAY=0.0016'
  write(*,'(A)') 'F_ROSS23_TIMED_REGION=SOLVE_CALLS_ONLY'
  write(*,'(A)') 'F_ROSS23_GATE=PASS'
...
contains
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'F_ROSS23_FAIL',trim(label)
      error stop 1
    end if
