program test_ross15_reference_vs_rossfast_performance
  ...
  integer, parameter :: inner_repetitions=200
  ...
  character(len=16) :: selected_route
  integer :: env_status
  real(real64) :: total_solver_cpu_seconds, checksum
  ...
  checksum=0.0_real64
  selected_route=''
  call get_environment_variable('SWAP5_ROSS24_ROUTE',selected_route,status=env_status)
  call require(env_status==0,'SWAP5_ROSS24_ROUTE must be present')
    ...
    do ise=1,nse
      do iforce=1,nforcing
        ...
        call run_case(case_id,ise,material_ids(imat),se_levels(ise),forcing_ids(iforce), &
             ...
             selected_route,total_solver_cpu_seconds,checksum)
        ...
        write(*,'(*(g0))') 'F_ROSS24_CASE|ID=',case_id,'|MATERIAL=',trim(material_ids(imat)), &
  ...
  write(*,'(A,I0)') 'F_ROSS24_PAIRED_VALID=',count_paired_valid
  write(*,'(A,I0)') 'F_ROSS24_PAIRED_VALID_ADMISSIBLE=',count_admissible
  write(*,'(A,I0)') 'F_ROSS24_PAIRED_VALID_DISCREPANCY_FAIL=',count_discrepancy_fail
  write(*,'(A,I0)') 'F_ROSS24_REFERENCE_ROUTE_INVALID=',count_reference_invalid
  write(*,'(A,I0)') 'F_ROSS24_ROSSFAST_ROUTE_INVALID=',count_rossfast_invalid
  write(*,'(A,I0)') 'F_ROSS24_BOTH_ROUTES_INVALID=',count_both_invalid
  write(*,'(A,I0)') 'F_ROSS24_FAIL_H_INF=',fail_h_inf
  write(*,'(A,I0)') 'F_ROSS24_FAIL_H_RMS=',fail_h_rms
  write(*,'(A,I0)') 'F_ROSS24_FAIL_THETA_INF=',fail_theta_inf
  write(*,'(A,I0)') 'F_ROSS24_FAIL_THETA_RMS=',fail_theta_rms
  write(*,'(A,I0)') 'F_ROSS24_FAIL_STORAGE=',fail_storage
  ...
  write(*,'(A,A)') 'F_ROSS24_ROUTE=',trim(selected_route)
  write(*,'(A,I0)') 'F_ROSS24_CASE_COUNT=',case_id
  write(*,'(A,I0)') 'F_ROSS24_INNER_REPETITIONS=',inner_repetitions
  write(*,'(A,I0)') 'F_ROSS24_TIMED_SOLVE_COUNT=',count_paired_valid*inner_repetitions
  write(*,'(A,ES24.16E3)') 'F_ROSS24_SOLVER_CPU_SECONDS=',total_solver_cpu_seconds
  write(*,'(A,ES24.16E3)') 'F_ROSS24_CHECKSUM=',checksum
  write(*,'(A)') 'F_ROSS24_LAYER=A_FIXED_INTERVAL_SOLVER_SEAM'
  write(*,'(A)') 'F_ROSS24_STEP_DURATION_DAY=0.0016'
  write(*,'(A)') 'F_ROSS24_TIMED_REGION=SOLVE_CALLS_ONLY'
  write(*,'(A)') 'F_ROSS24_GATE=PASS'
...
contains
  ...
  subroutine run_case(case_id,ise,material_id,se,forcing_id,top_factor,bottom_factor,classification,metrics_available, &
       ...
       selected_route,solver_cpu_accum,checksum_accum)
    ...
    character(len=*),intent(in) :: selected_route
    real(real64),intent(inout) :: solver_cpu_accum,checksum_accum
    ...
    write(*,'(*(g0))') 'F_ROSS24_ROUTE_REASON|ID=',case_id,'|REF_REASON=',trim(reference_reason), &
         '|ROSS_REASON=',trim(rossfast_reason)

    if (rossfast_valid) then
      call require(certificate_available,'admitted tiered temporal certificate available')
      call require(ieee_is_finite(temporal_indicator) .and. temporal_indicator>=0.0_real64 .and. temporal_indicator<=1.0_real64, &
           'admitted tiered temporal certificate accepted')
      tier_work=alternative_result%diagnostics%linear_solves
      ...
      case(42)
        tier_name='K8'
      case default
        call require(.false.,'admitted tiered work count must be 6, 18 or 42')
      end select
      call require(alternative_result%diagnostics%internal_retries==0,'zero tiered internal retries')
    ...
    if (reference_valid .and. rossfast_valid) then
      ...
      write(*,'(*(g0))') 'F_ROSS24_TIMING_CASE|ID=',case_id,'|MATERIAL=',trim(material_id), &
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'F_ROSS24_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require
end program test_ross15_reference_vs_rossfast_performance
