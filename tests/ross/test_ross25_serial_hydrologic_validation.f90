...
contains
  ...
  logical function forcing_admitted(k,qtop,qbot) result(ok)
    ...
    ! soil_water_solve_request_t uses the public SWAP/Reference sign convention:
    ! positive top flux leaves the column. The RossFast model binding converts
    ! that value with forcing%top_flux_cm_per_day = -request%boundary%top_flux
    ! before applying its state-local D2/D3R envelope around +0.01*K.
    ! Therefore the equivalent envelope centre in this public request space is
    ! -0.01*K, not +0.01*K. Bottom flux is not sign-inverted by the adapter.
