program test_ross26_tiered_wetting_kernel
  ...
  if (len_trim(material_arg)==0 .or. len_trim(fixture_path)==0) then
    error stop 'usage: test_ross26 MATERIAL FIXTURE.txt'
  end if
  ...
  do i=1,ncase
    ...
    write(*,'(*(g0))') 'F_ROSS26_CASE|MATERIAL=',trim(material_id),'|SE=',se,'|TIER=',trim(tier_name), &
  ...
  write(*,'(A,1X,A)') 'F_ROSS26_MATERIAL',trim(material_id)
  write(*,'(A,I0)') 'F_ROSS26_CASE_COUNT=',ncase
  write(*,'(A,ES26.17E3)') 'F_ROSS26_MAX_THETA_NORM=',max_theta_norm
  write(*,'(A,ES26.17E3)') 'F_ROSS26_MAX_STATE_CHANGE_REL=',max_state_change_rel
  write(*,'(A,ES26.17E3)') 'F_ROSS26_MAX_HEAD_INF_CM=',max_head_inf
  write(*,'(A,ES26.17E3)') 'F_ROSS26_MAX_ABS_MASS_RESIDUAL_CM=',max_mass_residual
  write(*,'(A)') 'F_ROSS26_TIERED_WETTING_KERNEL=PASS'
...
contains
  ...
  subroutine require(condition,label)
    ...
    if (.not.condition) then
      write(*,'(A,1X,A)') 'F_ROSS26_FAIL',trim(label)
      error stop 1
    end if
  end subroutine require

end program test_ross26_tiered_wetting_kernel
