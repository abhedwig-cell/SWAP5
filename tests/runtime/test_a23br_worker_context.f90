program test_a23br_worker_context
  use, intrinsic :: iso_fortran_env, only: real64
  use omp_lib
  use mod_a23br_worker_execution_context
  implicit none
  integer, parameter :: nworkers=8, nchecks=1000
  type(a23br_worker_context_t) :: workers(nworkers)
  integer :: i, j, failures

  failures = 0
  do i=1,nworkers
    call a23br_initialize_worker(workers(i), 34, i)
  end do

!$omp parallel do default(none) shared(workers) reduction(+:failures) private(i,j)
  do i=1,nworkers
    do j=1,nchecks
      call a23br_reset_attempt_diagnostics(workers(i))
      call a23br_reset_all_numerical_control(workers(i))
      workers(i)%control%at_min_dt = mod(i+j,2) == 0
      workers(i)%control%last_numbit = i + j
      call a23br_request_dt_reduction(workers(i))
      ...
      call a23br_record_internal_retry(workers(i))
      ...
      call a23br_reset_attempt_control(workers(i))
...
!$omp end parallel do
  ...
  if (failures /= 0) then
    print '(A,I0)', 'A23BR_WORKER_CONTEXT_GATE FAIL failures=',failures
    error stop 1
  end if
  print '(A)', 'A23BR_WORKER_CONTEXT_GATE PASS'
  print '(A,I0)', 'workers=',nworkers
  print '(A,I0)', 'checks=',nworkers*nchecks
  print '(A,I0)', 'parallel_failures=',failures
  print '(A,I0)', 'scratch_payload_bytes_per_worker=',a23br_scratch_payload_bytes(workers(1))

  do i=1,nworkers
    call a23br_release_worker(workers(i))
  end do
end program test_a23br_worker_context
