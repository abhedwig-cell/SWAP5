program test_a23bu_worker_context
  use, intrinsic :: iso_fortran_env, only: real64
  use omp_lib
  use mod_a23bu_worker_execution_context
  implicit none
  integer, parameter :: nworkers=8, nchecks=1000
  type(a23bu_worker_context_t) :: workers(nworkers)
  integer :: i, j, failures

  failures = 0
  do i=1,nworkers
    call a23bu_initialize_worker(workers(i), 34, i)
  end do

!$omp parallel do default(none) shared(workers) reduction(+:failures) private(i,j)
  do i=1,nworkers
    do j=1,nchecks
      call a23bu_reset_attempt_diagnostics(workers(i))
      call a23bu_reset_all_numerical_control(workers(i))
      ...
      call a23bu_record_internal_retry(workers(i))
...
!$omp end parallel do
  ...
  if (failures /= 0) then
    print '(A,I0)', 'A23BU_WORKER_CONTEXT_GATE FAIL failures=',failures
    error stop 1
  end if
  print '(A)', 'A23BU_WORKER_CONTEXT_GATE PASS'
  print '(A,I0)', 'workers=',nworkers
  print '(A,I0)', 'checks=',nworkers*nchecks
  print '(A,I0)', 'parallel_failures=',failures
  print '(A,I0)', 'scratch_payload_bytes_per_worker=',a23bu_scratch_payload_bytes(workers(1))
  print '(A,I0)', 'reporting_shim_payload_bytes_per_worker=',a23bu_reporting_shim_payload_bytes()
  if (a23bu_reporting_shim_payload_bytes() /= 56) error stop 'A23BU reporting shim payload changed'

  do i=1,nworkers
    call a23bu_release_worker(workers(i))
  end do
end program test_a23bu_worker_context
