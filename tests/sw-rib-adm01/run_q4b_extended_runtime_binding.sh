...
BUILD="${TMPDIR:-/tmp}/swap5-sw-rib-swm01-q4b-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail(){ echo "SW_RIB_SWM01_Q4B_FAIL $*" >&2; exit 87; }
...
root = Path(".")
test = Path("tests/sw-rib-adm01/test_q4b_extended_runtime_binding.f90")
...
print(f"SW_RIB_SWM01_Q4B_COMPILE_CLOSURE_FILES={len(order)}")
PY
...
echo 'SW_RIB_SWM01_Q4B_COMPILE_CLOSURE=PASS'
...
for opt in 0 2; do
  ...
  for marker in \
    ...
    'SW_RIB_SWM01_Q4B_TRANSACTIONAL_RUNTIME=PASS' \
    'SW_RIB_ADM01_G3_EXTERNAL_HEAD_BINDING=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || {
      cat "$OUT/output.txt" >&2
      fail "missing O$opt marker: $marker"
    }
  done
  echo "SW_RIB_SWM01_Q4B_O${opt}=PASS"
done
...
echo "SW_RIB_SWM01_Q4B_OUTPUT_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'SW_RIB_SWM01_Q4B_O0_O2_EXACT_IDENTITY=PASS'
echo 'SW_RIB_SWM01_Q4B_GATE=PASS'
