...
BUILD="${TMPDIR:-/tmp}/swap5-sw-rib-pa01-signed-exchange-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"

fail(){ echo "SW_RIB_PA01_SIGNED_EXCHANGE_FAIL $*" >&2; exit 87; }
...
root = Path(".")
test = Path("tests/sw-rib-pa01/test_signed_exchange_runtime.f90")
...
print(f"SW_RIB_PA01_SIGNED_EXCHANGE_COMPILE_CLOSURE_FILES={len(order)}")
PY
...
echo 'SW_RIB_PA01_SIGNED_EXCHANGE_COMPILE_CLOSURE=PASS'
...
for opt in 0 2; do
  ...
  for marker in \
    'SW_RIB_PA01_SIGNED_EXCHANGE_POSITIVE_DRAINAGE_SINGLE_BOOKING=PASS' \
    'SW_RIB_PA01_SIGNED_EXCHANGE_NEGATIVE_INFILTRATION_SINGLE_BOOKING=PASS' \
    'SW_RIB_PA01_SIGNED_EXCHANGE_SIGNED_HARD_MASS_CLOSURE=PASS' \
    'SW_RIB_PA01_SIGNED_EXCHANGE_INVALID_PROCESS_ROLLBACK=PASS' \
    'SW_RIB_PA01_SIGNED_EXCHANGE_TRANSACTIONAL_RUNTIME=PASS'; do
    grep -Fq "$marker" "$OUT/output.txt" || {
      cat "$OUT/output.txt" >&2
      fail "missing O$opt marker: $marker"
    }
  done
  echo "SW_RIB_PA01_SIGNED_EXCHANGE_O${opt}=PASS"
done
...
echo "SW_RIB_PA01_SIGNED_EXCHANGE_OUTPUT_SHA256=$(sha256sum "$BUILD/o0/output.txt" | awk '{print $1}')"
echo 'SW_RIB_PA01_SIGNED_EXCHANGE_O0_O2_EXACT_IDENTITY=PASS'
echo 'SW_RIB_PA01_SIGNED_EXCHANGE_GATE=PASS'
