program test_sw_rib_pa01_signed_exchange
  ...
  write(*,'(A)') 'SW_RIB_PA01_SIGNED_EXCHANGE_TRANSACTIONAL_RUNTIME=PASS'
...
contains
  ...
  subroutine verify_signed_commit(control_head, balancing_qssdi, positive)
    ...
    if (positive) then
      ...
      write(*,'(A)') 'SW_RIB_PA01_SIGNED_EXCHANGE_POSITIVE_DRAINAGE_SINGLE_BOOKING=PASS'
    else
      ...
      write(*,'(A)') 'SW_RIB_PA01_SIGNED_EXCHANGE_NEGATIVE_INFILTRATION_SINGLE_BOOKING=PASS'
    end if
    ...
    write(*,'(A)') 'SW_RIB_PA01_SIGNED_EXCHANGE_SIGNED_HARD_MASS_CLOSURE=PASS'
  ...
  subroutine verify_invalid_process_rolls_back()
    ...
    write(*,'(A)') 'SW_RIB_PA01_SIGNED_EXCHANGE_INVALID_PROCESS_ROLLBACK=PASS'
  ...
  subroutine require(condition,label)
    ...
    if (.not. condition) then
      write(*,'(A,1X,A)') 'SW_RIB_PA01_SIGNED_EXCHANGE_TEST_FAIL', trim(label)
      error stop 1
    end if
  end subroutine require

end program test_sw_rib_pa01_signed_exchange
