...
from ribasim.nodes import basin, continuous_control, flow_boundary, pump
from shapely.geometry import Point

DURATION_SECONDS = 8.64
DAY = 86400.0
AREA_M2 = 1.0
SLOPE_M3_PER_DAY_PER_M = 0.5
...
def build_model(case) -> Model:
    ...
    discharge = model.pump.add(
        Node(3, Point(1.0, 0.0), name="controlled_one_way_discharge"),
        [pump.Static(flow_rate=[0.0], min_flow_rate=[0.0])],
    )
    sink = model.terminal.add(Node(4, Point(2.0, 0.0), name="terminal"))
    control = model.continuous_control.add(
        Node(5, Point(1.0, 1.0), name="linear_fixed_weir_law"),
        [
            ...
            continuous_control.Function(
                input=[0.0, 1.0],
                output=[0.0, SLOPE_M3_PER_DAY_PER_M / DAY],
    ...
    model.link.add(control, discharge)
    return model
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: generate_q1a_r3_real_ribasim.py <oracle.csv> <output-root>"
        )
    ...
    for case in rows:
        ...
        print(f"SW_RIB_SWM01_Q1A_R3_MODEL_GENERATED case={case['id']} path={model_path}")

    print("SW_RIB_SWM01_Q1A_R3_MODEL_GENERATION=PASS")
