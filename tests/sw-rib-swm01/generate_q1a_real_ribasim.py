...
def build_model(case) -> Model:
    ...
    weir = model.tabulated_rating_curve.add(
        Node(3, Point(1.0, 0.0), name="fixed_weir"),
        [
            tabulated_rating_curve.Static(
                level=[-1.0, 0.0, 1.0],
                flow_rate=[0.0, 0.0, RATING_TOP_M3_PER_DAY / DAY],
            )
        ],
    )
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: generate_q1a_real_ribasim.py <oracle.csv> <output-root>"
        )
    ...
    for case in rows:
        ...
        print(f"SW_RIB_SWM01_Q1A_MODEL_GENERATED case={case['id']} path={model_path}")

    print("SW_RIB_SWM01_Q1A_MODEL_GENERATION=PASS")
