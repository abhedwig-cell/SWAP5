...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-sw-rib-swm01-q1a-r3-${GITHUB_RUN_ID:-local}-$$"
RIBASIM_ROOT="${SW_RIB_Q1A_R3_RIBASIM_ROOT:-$ROOT/.ribasim-product-release}"
MODEL_ROOT="$RIBASIM_ROOT/generated_testmodels/swap5_sw_rib_swm01_q1a_r3"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
fail(){ echo "SW_RIB_SWM01_Q1A_R3_FAIL $*" >&2; exit 1; }
...
echo "SW_RIB_SWM01_Q1A_R3_RIBASIM_PIN=PASS sha=$ACTUAL_PIN"
...
(
  ...
  fail "real Ribasim Q1A-R3 execution"
}
...
echo 'SW_RIB_SWM01_Q1A_R3_GATE=PASS'
