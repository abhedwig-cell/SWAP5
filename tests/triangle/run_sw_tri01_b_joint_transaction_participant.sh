...
BUILD="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/swap5-tri01b-${GITHUB_RUN_ID:-local}-$$"
mkdir -p "$BUILD"
trap 'rm -rf "$BUILD"' EXIT
cd "$ROOT"
fail(){ echo "SW_TRI01_B_GATE_FAIL $*" >&2; exit 102; }

python3 - "$BUILD/compile-order.txt" <<'PY'
from pathlib import Path
import re,sys
test=Path("tests/triangle/test_sw_tri01_b_joint_transaction_participant.f90")
...
print(f"SW_TRI01_B_COMPILE_CLOSURE_FILES={len(order)}")
PY

for required in   src/runtime/mod_fmr_groundwater_surface_water_swap_participant.f90   src/runtime/mod_fmr_surface_water_head_forcing_adapter.f90   src/runtime/mod_fmr_groundwater_head_forcing_adapter.f90   src/runtime/mod_fmr_serialized_reference_backend.f90; do
  grep -Fqx "$required" "$BUILD/compile-order.txt" || fail "missing compile closure: $required"
done
echo 'SW_TRI01_B_COMPILE_CLOSURE=PASS'
...
for opt in 0 2; do
  ...
  for marker in     SW_TRI01_B_CASE_PASS=B1_POSITIVE_MATCHED_COMMIT     SW_TRI01_B_CASE_PASS=B2_NEGATIVE_MATCHED_COMMIT     SW_TRI01_B_CASE_PASS=B3_MISMATCH_DISCARD_REPLAY     SW_TRI01_B_CASE_PASS=B4_STALE_ORIGIN_FAIL_CLOSED     SW_TRI01_B_JOINT_TRANSACTION_PARTICIPANT=PASS; do
    grep -Fq "$marker" "$OUT/output.txt" || fail "missing O$opt marker $marker"
  done
  grep '^SW_TRI01_B_' "$OUT/output.txt" > "$OUT/stable.txt"
  echo "SW_TRI01_B_O${opt}=PASS"
done
diff -u "$BUILD/o0/stable.txt" "$BUILD/o2/stable.txt"
cat "$BUILD/o0/stable.txt"
echo 'SW_TRI01_B_O0_O2_STABLE_IDENTITY=PASS'
echo 'SW-TRI01-B JOINT TRANSACTION PARTICIPANT GATE PASS'
