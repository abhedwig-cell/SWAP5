...
F-DOC03 may document exact frozen RB1 contracts and explicit gaps. It may not
change production/reference content, expand the RB1 denominator, fabricate
scientific provenance, infer full verification, or claim Status A/AA.
"""
...
CONTRACTS_PATH = "src/runtime/mod_canonical_contracts.f90"
INTERVAL_PATH = "src/runtime/mod_canonical_interval_runtime.f90"
TRANSACTION_PATH = "src/transaction/mod_transaction_reference.f90"
EXPECTED_BLOBS = {
    CONTRACTS_PATH: "c06aa869a0bd479df4c7d6e1d0b4f5c07a207144",
    INTERVAL_PATH: "55f3d271aa6200a994fd0144d6fce0701c918a74",
    TRANSACTION_PATH: "2fd932b74dbd0ffc0ec089f49e632b7ac8852df4",
}
CORE_IDS = [
    "RB1-CORE-INTERVAL",
    "RB1-CORE-DATA",
...
FORBIDDEN_POSITIVE_TEST = "tests/runtime/test_canonical_runtime.f90"
...
# Exact F-DOC02 ancestry and documentation-only delta.
...
require(sh("git", "rev-parse", "HEAD:src") == sh("git", "rev-parse", f"{SOURCE}:src"), "SOURCE_TREE_STILL_FROZEN")
require(sh("git", "rev-parse", "HEAD:reference") == sh("git", "rev-parse", f"{SOURCE}:reference"), "REFERENCE_TREE_STILL_FROZEN")

# Exact frozen source authority and code blobs, verified from Git rather than self-report.
require(sh("git", "rev-parse", f"{SOURCE}^{{tree}}") == SOURCE_TREE, "SOURCE_AUTHORITY_TREE")
for path, expected in EXPECTED_BLOBS.items():
    require(blob(SOURCE, path) == expected,
...
# Registry structural validity and exact bounded capability set.
require(registry["schema_version"] == 1, "REGISTRY_SCHEMA_V1")
...
cap_ids = [x["capability_id"] for x in capabilities]
require(cap_ids == CORE_IDS, "EXACT_CORE_CAPABILITY_SET_5")

nodes = registry["nodes"]
node_ids = [x["id"] for x in nodes]
require(len(node_ids) == len(set(node_ids)), "NODE_IDS_UNIQUE")
require(all(NODE_RE.match(x) for x in node_ids), "NODE_IDS_VALID")
node_map = {x["id"]: x for x in nodes}
for edge in registry["edges"]:
    require(edge["from"] in node_map and edge["to"] in node_map, "EDGE_ENDPOINTS_RESOLVE")

# Production T9/T10 mappings must point to exact frozen code and cannot float.
prod_nodes = [x for x in nodes if x["tier"] in ("T9", "T10")]
require(len(prod_nodes) == 10, "T9_T10_NODE_COUNT_10")
for node in prod_nodes:
    ...
    require(node.get("production") is True, node["id"] + "_PRODUCTION_TRUE")
    require(node.get("repository_path") in EXPECTED_BLOBS, node["id"] + "_KNOWN_SOURCE_PATH")
    require(bool(node.get("symbol")), node["id"] + "_SYMBOL_PRESENT")
print("FDOC03_T9_T10_MAPPING=PASS")

# No positive evidence locator may use the nonexistent preliminary test assumption.
positive_locators = []
for node in nodes:
    positive_locators.extend(node.get("sources", []))
    if "repository_path" in node:
        positive_locators.append(node["repository_path"])
require(FORBIDDEN_POSITIVE_TEST not in positive_locators, "NONEXISTENT_TEST_NOT_USED_AS_EVIDENCE")
require(FORBIDDEN_POSITIVE_TEST in doc_text, "NONEXISTENT_TEST_CORRECTION_DOCUMENTED")
require(any(FORBIDDEN_POSITIVE_TEST in x for x in registry.get("nonclaims", [])), "NONEXISTENT_TEST_NONCLAIM_DOCUMENTED")

# Capability-specific fail-closed tier semantics.
for cap in capabilities:
    tiers = cap["tiers"]
    require(set(tiers) == {f"T{i}" for i in range(15)}, cap["capability_id"] + "_ALL_TIERS_CLASSIFIED")
    ...
    for tier, obj in tiers.items():
        if obj["status"] == "NOT_APPLICABLE":
            require(bool(obj.get("rationale", "").strip()), cap["capability_id"] + "_" + tier + "_NA_RATIONALE")
print("FDOC03_T11_SCOPED_NOT_COMPLETE=PASS")
print("FDOC03_NO_FULLY_TRACED_CLAIM=PASS")

by_cap = {x["capability_id"]: x for x in capabilities}
...
# Architecture-only CORE-DATA may not fabricate a physical theory chain.
...
# Upstream F-RB01 authority remains exactly the same and all five are required PASS.
...
# Status and nonclaim boundaries.
...
holds = status["scope_holds"]
for key in ("production_delta", "reference_delta", "physics_delta", "solver_delta", "acceptance_threshold_delta", "performance_delta"):
    require(holds[key] == "NONE", "STATUS_" + key.upper() + "_NONE")
require(holds["rb1_denominator_changed"] is False, "STATUS_DENOMINATOR_UNCHANGED")
require(holds["scientific_authority_reopened"] is False, "STATUS_SCIENCE_NOT_REOPENED")
require(holds["release_authority_reopened"] is False, "STATUS_RELEASE_NOT_REOPENED")
...
# Explicit 30-invariant audit, with mass conservation hard and unchanged.
items = audit["items"]
...
require(audit["summary"]["total"] == 30 and audit["summary"]["pass"] == 30 and audit["summary"]["fail"] == 0, "INVARIANT_SUMMARY")
require(audit["summary"]["overall"] == "30_OF_30_NO_ADVERSE_DELTA", "INVARIANT_OVERALL")
require(audit["mass_conservation"] == "HARD_UNCHANGED", "HARD_MASS_UNCHANGED")

# Evidence is generated from the exact checked-out head for artifact retention.
evidence = {
    "work_unit": "F-DOC03",
...
(ROOT / "_fdoc03_rb1_core_traceability_evidence.json").write_text(
