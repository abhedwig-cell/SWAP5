...
def parse_output(text: str, expected_route: str) -> dict:
    ...
    if int(values["cases"]) != 36 or int(values["repetitions"]) != 200 or int(values["solve_count"]) != 7200:
...
def main() -> int:
    ...
    result = {
        "schema": "swap5.f-ross15.performance-screening-result.v1",
        "workunit": "F-ROSS15",
        ...
        "measurement": {
            ...
            "paired_valid_case_count": 36,
            "timed_solves_per_sample": 7200,
    ...
    print(f"F_ROSS15_SCREENING_OUTCOME={screening}")
    print(f"F_ROSS15_MEAN_RELATIVE_DELTA={mean_delta:.12g}")
    print(f"F_ROSS15_MDE={mde:.12g}")
    print(f"F_ROSS15_MEAN_SPEEDUP={statistics.fmean(solver_speedups):.12g}")
    print("F_ROSS15_FORMAL_SPEEDUP_CLAIM=FALSE")
