...
CASE_RE = re.compile(
    r"^F_ROSS20_CASE\|CASE=(?P<case>\d+)\|ISE=(?P<ise>\d+)\|M=(?P<material>[^|]+)\|F=(?P<forcing>[^|]+)"
...
def main() -> int:
    ...
    work_ok=[r for r in route_valid if r["linear_solves"] in (6,18) and
             ((r["linear_solves"]==18) == (r["fallback"]==1))]
    retry_ok=[r for r in route_valid if r["internal_retries"]==0 and r["alternative_solver_calls"]==0]
    fallback=[r for r in route_valid if r["fallback"]==1]
    fast=[r for r in route_valid if r["fallback"]==0]

    passed=(len(route_valid)==216 and len(temporal_ok)==216 and len(mass_ok)==216 and
            len(work_ok)==216 and len(retry_ok)==216)

    by_material=Counter(r["material"] for r in fallback)
    by_ise=Counter(str(r["ise"]) for r in fallback)
    by_forcing=Counter(r["forcing"] for r in fallback)
    result={
        "schema":"swap5.f-ross20.all-material-production-matrix.v1",
        "case_count":216,
        ...
        "fallback_by_material":dict(sorted(by_material.items())),
        "fallback_by_effective_saturation_index":dict(sorted(by_ise.items())),
        "fallback_by_forcing":dict(sorted(by_forcing.items())),
    ...
    print(f"F_ROSS20_ALL_MATERIAL_ROUTE_VALID={len(route_valid)}")
    print(f"F_ROSS20_ALL_MATERIAL_TEMPORAL_ACCEPTED={len(temporal_ok)}")
    print(f"F_ROSS20_ALL_MATERIAL_FALLBACK_COUNT={len(fallback)}")
    print(f"F_ROSS20_ALL_MATERIAL_MEAN_LINEAR_SOLVES={result['mean_linear_solves']:.12g}")
    print(f"F_ROSS20_ALL_MATERIAL_GATE={result['gate']}")
