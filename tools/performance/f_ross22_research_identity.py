...
H_RE=re.compile(r"^F_ROSS21_H\|CASE=(?P<case>\d+)\|(?P<values>.*)$")
T_RE=re.compile(r"^F_ROSS21_T\|CASE=(?P<case>\d+)\|(?P<values>.*)$")
...
def parse(path:Path)->dict[int,dict]:
    ...
    for raw in path.read_text(encoding="utf-8",errors="replace").splitlines():
        ...
        if m:
            ...
            rows[c]={
                ...
                "level":int(d["level"]),"mass_residual_cm":fnum(d["mass"]),
    ...
    if sorted(rows)!=list(range(1,217)):
        raise RuntimeError(f"{path}: expected cases 1..216")
...
def main()->int:
    ...
    for c in range(1,217):
        a=p[c]; b=r[c]
        identity_fields=("ise","material","forcing","classification","linear_solves","internal_retries","alternative_solver_calls","level")
        ...
        records.append({
            "case":c,"pass":case_pass,"field_drift":field_drift,"numeric_field_drift":numeric_drift,
    ...
    out={
        "schema":"swap5.f-ross22.production-research-identity.v1",
        "case_count":216,
        "all_cases_exact":passed,
        "max_head_inf":max_h,
        "max_theta_inf":max_t,
        "max_temporal_indicator_abs_delta":max_temp,
        "max_mass_residual_abs_delta_cm":max_mass,
        "failed_cases":[x["case"] for x in records if not x["pass"]],
        "records":records,
        "gate":"PASS" if passed else "FAIL"
    }
    args.output.write_text(json.dumps(out,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    print(json.dumps(out,indent=2,sort_keys=True))
    print(f"F_ROSS22_PRODUCTION_RESEARCH_IDENTITY={out['gate']}")
