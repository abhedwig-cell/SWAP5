...
CASE_PATTERN = re.compile(
    r"^F_ROSS24_CASE\|ID=(?P<id>\d+)\|MATERIAL=(?P<material>[^|]+)\|SE=(?P<se>[^|]+)\|FORCING=(?P<forcing>[^|]+)\|CLASS=(?P<classification>[^|]+)\|METRICS=(?P<metrics>[^|]+)\|D_H_INF=(?P<dh_inf>[^|]+)\|D_H_RMS=(?P<dh_rms>[^|]+)\|D_THETA_INF=(?P<dtheta_inf>[^|]+)\|D_THETA_RMS=(?P<dtheta_rms>[^|]+)\|D_STORAGE=(?P<dstorage>[^|]+)$",
    re.MULTILINE,
)

REASON_PATTERN = re.compile(
    r"^F_ROSS24_ROUTE_REASON\|ID=(?P<id>\d+)\|REF_REASON=(?P<ref_reason>[^|]+)\|ROSS_REASON=(?P<ross_reason>[^|]+)$",
    re.MULTILINE,
)

TIMING_PATTERN = re.compile(
    r"^F_ROSS24_TIMING_CASE\|ID=(?P<id>\d+)\|MATERIAL=(?P<material>[^|]+)\|SE=(?P<se>[^|]+)\|FORCING=(?P<forcing>[^|]+)\|ROUTE=(?P<route>REFERENCE|ROSSFAST)\|CPU=(?P<cpu>[0-9.Ee+\-]+)$",
    re.MULTILINE,
)

TIER_PATTERN = re.compile(
    r"^F_ROSS24_TIER\|ID=(?P<id>\d+)\|LIN=(?P<lin>-?\d+)\|TIER=(?P<tier>K2|K4|K8|NA)\|TEMP=(?P<temp>[0-9.Ee+\-]+)\|MASS=(?P<mass>[0-9.Ee+\-]+)$",
    re.MULTILINE,
)


MARKERS = {
    "route": re.compile(r"^F_ROSS24_ROUTE=(REFERENCE|ROSSFAST)$", re.MULTILINE),
    "cases": re.compile(r"^F_ROSS24_CASE_COUNT=(\d+)$", re.MULTILINE),
    "repetitions": re.compile(r"^F_ROSS24_INNER_REPETITIONS=(\d+)$", re.MULTILINE),
    "solve_count": re.compile(r"^F_ROSS24_TIMED_SOLVE_COUNT=(\d+)$", re.MULTILINE),
    "solver_cpu": re.compile(r"^F_ROSS24_SOLVER_CPU_SECONDS=\s*([0-9.Ee+\-]+)$", re.MULTILINE),
    "checksum": re.compile(r"^F_ROSS24_CHECKSUM=\s*([0-9.Ee+\-]+)$", re.MULTILINE),
    "gate": re.compile(r"^F_ROSS24_GATE=PASS$", re.MULTILINE),
    "paired_valid": re.compile(r"^F_ROSS24_PAIRED_VALID=(\d+)$", re.MULTILINE),
    "admissible": re.compile(r"^F_ROSS24_PAIRED_VALID_ADMISSIBLE=(\d+)$", re.MULTILINE),
    "discrepancy_fail": re.compile(r"^F_ROSS24_PAIRED_VALID_DISCREPANCY_FAIL=(\d+)$", re.MULTILINE),
    "reference_invalid": re.compile(r"^F_ROSS24_REFERENCE_ROUTE_INVALID=(\d+)$", re.MULTILINE),
    "rossfast_invalid": re.compile(r"^F_ROSS24_ROSSFAST_ROUTE_INVALID=(\d+)$", re.MULTILINE),
    "both_invalid": re.compile(r"^F_ROSS24_BOTH_ROUTES_INVALID=(\d+)$", re.MULTILINE),
}
...
def run_one(executable: Path, route: str, target_cpu: int, cycle: int, measured: bool) -> dict:
    ...
    case_lines = [line for line in completed.stdout.splitlines() if line.startswith("F_ROSS24_CASE|")]
    ...
    tier_rows = {}
    for match in TIER_PATTERN.finditer(completed.stdout):
        tier_rows[str(int(match.group("id")))] = {
            "linear_solves": int(match.group("lin")),
            "tier": match.group("tier"),
            "temporal_indicator": float(match.group("temp")),
            "mass_residual_cm": float(match.group("mass")),
        }
    if len(tier_rows) != 216:
        raise RuntimeError(f"expected 216 tier rows, got {len(tier_rows)}")
    parsed["case_matrix"] = case_lines
    parsed["route_reasons"] = route_reasons
    parsed["tier_rows"] = tier_rows
...
def screen(values: list[float]) -> dict:
    dist = distribution(values)
    mde = RESOLUTION_MULTIPLIER * float(dist["stdev"]) / math.sqrt(len(values))
    mean = float(dist["mean"])
    if mean < -mde:
        outcome = "ROSSFAST_FASTER"
    elif mean > mde:
        outcome = "REFERENCE_FASTER"
    else:
...
def build_case_and_regime_analysis(by_cycle: dict[int, dict[str, dict]], tier_rows: dict[str, dict]) -> dict:
    case_deltas: dict[int, list[float]] = {}
    meta: dict[int, dict] = {}
    group_members: dict[str, dict[str, set[int]]] = {
        "tier": {}, "material": {}, "family": {}, "effective_saturation": {}, "forcing": {}
    }

    timing_maps = {}
    for cycle, pair in by_cycle.items():
        timing_maps[cycle] = {}
        for route in ROUTES:
            timing_maps[cycle][route] = {int(x["id"]): x for x in pair[route]["case_timings"]}
        common = set(timing_maps[cycle]["REFERENCE"]) & set(timing_maps[cycle]["ROSSFAST"])
        for case_id in common:
            ref = timing_maps[cycle]["REFERENCE"][case_id]
            ross = timing_maps[cycle]["ROSSFAST"][case_id]
            case_deltas.setdefault(case_id, []).append(ross["cpu_seconds"] / ref["cpu_seconds"] - 1.0)
            if case_id not in meta:
                tier = tier_rows[str(case_id)]["tier"]
                meta[case_id] = {
                    "material": ross["material"],
                    "family": ross["material"][0],
                    "effective_saturation": ross["se"],
                    "forcing": ross["forcing"],
                    "tier": tier,
                    "linear_solves": tier_rows[str(case_id)]["linear_solves"],
                    "temporal_indicator": tier_rows[str(case_id)]["temporal_indicator"],
                    "mass_residual_cm": tier_rows[str(case_id)]["mass_residual_cm"],
                }

    case_results = {}
    counts = {"ROSSFAST_FASTER": 0, "REFERENCE_FASTER": 0, "NOT_RESOLVED": 0}
    for case_id, vals in sorted(case_deltas.items()):
        s = screen(vals)
        counts[s["outcome"]] += 1
        case_results[str(case_id)] = {**meta[case_id], "relative_delta": s}
        labels = {
            "tier": meta[case_id]["tier"],
            "material": meta[case_id]["material"],
            "family": meta[case_id]["family"],
            "effective_saturation": str(meta[case_id]["effective_saturation"]),
            "forcing": meta[case_id]["forcing"],
        }
        for kind, label in labels.items():
            group_members[kind].setdefault(label, set()).add(case_id)

    group_results = {}
    for kind, groups in group_members.items():
        group_results[kind] = {}
        for label, members in sorted(groups.items()):
            deltas = []
            for cycle in sorted(by_cycle):
                refs = timing_maps[cycle]["REFERENCE"]
                rosses = timing_maps[cycle]["ROSSFAST"]
                ids = members & set(refs) & set(rosses)
                if not ids:
                    continue
                ref_cpu = sum(refs[i]["cpu_seconds"] for i in ids)
                ross_cpu = sum(rosses[i]["cpu_seconds"] for i in ids)
                deltas.append(ross_cpu / ref_cpu - 1.0)
            group_results[kind][label] = {
                "case_count": len(members),
                "relative_delta": screen(deltas),
                "rossfast_over_reference_ratio_mean": statistics.fmean([1.0 + x for x in deltas]),
                "reference_over_rossfast_speedup_mean": statistics.fmean([1.0 / (1.0 + x) for x in deltas]),
            }
    ...
    return {
        ...
        "individual_case_outcomes": counts,
        "cases": case_results,
        "regimes": group_results,
    }
...
def main() -> int:
    ...
    canonical_tier_rows = samples[0]["tier_rows"]
    for row in samples[1:]:
        if row["tier_rows"] != canonical_tier_rows:
            raise RuntimeError("tier diagnostics changed across repeated runs")
    characterization = build_case_and_regime_analysis(by_cycle, canonical_tier_rows)

    result = {
        "schema": "swap5.f-ross24.tiered-production-characterization-result.v1",
        "workunit": "F-ROSS24",
        "phase": "MEASURED_TIERED_PRODUCTION_CHARACTERIZATION",
        "measurement": {
            ...
            "timing_scope": "PAIRED_VALID_SUBSET_ONLY; route-invalid cases remain scientific robustness evidence",
        ...
        "tier_rows": canonical_tier_rows,
        "characterization": characterization,
        "samples": [
            {k:v for k,v in row.items() if k not in ("case_matrix", "route_reasons", "tier_rows")}
            for row in samples
        ],
    }
    args.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps(result, indent=2, sort_keys=True))
    print(f"F_ROSS24_SCREENING_OUTCOME={screening}")
    print(f"F_ROSS24_MEAN_RELATIVE_DELTA={mean_delta:.12g}")
    print(f"F_ROSS24_MDE={mde:.12g}")
    print(f"F_ROSS24_MEAN_SPEEDUP={statistics.fmean(solver_speedups):.12g}")
    print("F_ROSS24_FORMAL_SPEEDUP_CLAIM=FALSE")
    print("F_ROSS24_SCREENING_GATE=PASS")
    return 0
