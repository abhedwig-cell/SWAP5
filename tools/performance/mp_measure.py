...
TIMING_CLOCK_KIND = "monotonic_elapsed"

TIMING_CATEGORIES = (
    "constitutive", "residual", "jacobian", "linear_solve", "newton_control",
    "soil_water_other", "surface_atmosphere", "crop_et_root", "drainage_irrigation",
    "macropore", "thermal", "solute", "crop_growth_nutrients", "transaction",
    "runtime_batching", "diagnostics", "other_kernel",
)
COUNTER_NAMES = (
    "newton_iterations", "residual_evaluations", "jacobian_builds", "linear_solves",
    "backtracking_attempts", "retries", "timestep_reductions", "alternative_linear_solves",
)
MEMORY_NAMES = (
    "persistent_column_state", "optional_state", "column_bookkeeping", "parameter_references",
    "checkpoint", "warm_start", "worker_scratch_reserved", "worker_scratch_peak",
    "jacobian_factorisation", "newton_residual_vectors", "constitutive_intermediates",
    "process_peak_rss",
)
...
class IntervalContext:
    ...
    def validate(self) -> None:
        for name in ("run_id", "code_revision", "case_id", "template_id", "physics_signature",
                     "numerical_policy_id", "execution_class", "t0", "t1"):
...
class _NullIntervalRecorder:
    ...
    def finish(self, *, accepted: bool, mass_balance_residual: float, mass_balance_passed: bool,
               mass_balance_tolerance: float | None = None, fallback_provenance: str | None = None,
               environment: Mapping[str, Any] | None = None) -> None:
        return None


class IntervalRecorder:
    def __init__(self, owner: "MeasurementCollector", context: IntervalContext,
    ...
    def span(self, category: str) -> Iterator[None]:
        ...
        if self._active_span is not None:
            raise RuntimeError(f"timing spans are exclusive; {self._active_span!r} is already active")
    ...
    def finish(self, *, accepted: bool, mass_balance_residual: float, mass_balance_passed: bool,
        ...
        timing_seconds = {
            ...
            **{key: value / 1_000_000_000.0 for key, value in self._timing_ns.items() if value},
        }
        record: dict[str, Any] = {
            ...
            **asdict(self._context),
            ...
            "mass_balance": {
                ...
                "tolerance": None if mass_balance_tolerance is None else float(mass_balance_tolerance),
                "passed": bool(mass_balance_passed),
            },
            "counters": dict(self._counters),
            "timing_clock_kind": TIMING_CLOCK_KIND,
...
class MeasurementCollector:
    """Measurement-only interval collector using a monotonic elapsed clock.

    The disabled path is clock-free. The collector never decides physical acceptance or
    mass balance. The default clock is time.perf_counter_ns and therefore must not be
    interpreted as process or worker CPU time.
    """
    ...
    def begin_interval(self, context: IntervalContext) -> IntervalRecorder | _NullIntervalRecorder:
        if not self.enabled:
            return self._null
...
def _distribution(values: Sequence[float]) -> dict[str, float]:
    ...
    return {
        ...
        "p50": _percentile(ordered, .50), "p90": _percentile(ordered, .90),
...
def aggregate_records(records: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    phase_totals = {name: 0.0 for name in TIMING_CATEGORIES}
    ...
    for row in rows:
        ...
        by_template.setdefault(str(row.get("template_id", "<missing>")), []).append(total)
        by_execution_class.setdefault(str(row.get("execution_class", "<missing>")), []).append(total)
        if row.get("batch_id") is not None:
            by_batch.setdefault(str(row["batch_id"]), []).append(total)
    ...
    return {
        ...
        "timing_clock_kind": TIMING_CLOCK_KIND,
        "sum_interval_elapsed_seconds": sum(total_times),
        "interval_elapsed_distribution_seconds": _distribution(total_times),
        "top_1pct_interval_elapsed_share": _top_fraction_share(total_times, .01),
        "phase_elapsed_totals_seconds": {name: value for name, value in phase_totals.items() if value},
        "counter_totals": counter_totals,
        "template_elapsed_distributions_seconds": {key: _distribution(values) for key, values in sorted(by_template.items())},
        "execution_class_elapsed_distributions_seconds": {key: _distribution(values) for key, values in sorted(by_execution_class.items())},
        "batch_divergence_max_over_median": _distribution(batch_divergence),
        "worker_attributed_elapsed_seconds": dict(sorted(worker_totals.items())),
        "worker_attributed_elapsed_max_over_mean": worker_imbalance,
    }
