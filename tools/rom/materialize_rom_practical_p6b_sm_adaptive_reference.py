...
def main():
    ...
    extra=(f"  real(real64), parameter :: RNP02_MAX_STEP={a.max_step_day:.17e}_real64\n"
           "  real(real64), parameter :: RNP02_RETRY_SCALE=0.5_real64\n"
           "  integer, parameter :: RNP02_MAX_RETRIES=8\n")
    ...
    call require(ok.and.state%ready(),'RNP02 initial committed state')

    p%bottom_mode=2
    p%total_balance_tolerance=original_total_tol
    call seed_steady(column,template,p,state,forcing,qeq,ok)
    call require(ok,'RNP02 gravity-consistent seed')
    ...
    do step=1,NSTEPS
      ...
      do while(cursor < target-1.0e-14_real64)
        attempt_dt=min(RNP02_MAX_STEP,target-cursor)
        retry=0
        do
          ...
          call require(retry<=RNP02_MAX_RETRIES,'RNP02 retry budget')
          attempt_dt=attempt_dt*RNP02_RETRY_SCALE
          call require(attempt_dt>=p%min_step_duration,'RNP02 minimum step')
        end do
        call require(abs(mass)<=hard_mass_gate,'RNP02 hard mass gate')
        cursor=attempt_t1
      ...
      call emit_state(ih,step,state,forcing,t1-min(RNP02_MAX_STEP,step_dt),t1,day_mass,day_bex,day_bflux,nl,back,.false.)
      write(*,'(*(g0))') 'RNP02_DAY|CASE=',trim(case_label(ih)),'|DAY=',step,'|ACCEPTED_SUBSTEPS=',accepted_substeps, &
...
'''
    ...
    t=t.replace("call require(total_states==active_histories*NSTEPS,'LAREDYN0R exact library state count')",
                "call require(total_states==active_histories*NSTEPS,'RNP02 exact daily state count')",1)
    t=t.replace("  write(*,'(A)') 'LAREDYN0R_EXECUTION_COMPLETE=PASS'",
                "  write(*,'(A)') 'RNP02_ADAPTIVE_REFERENCE=PASS'\n  write(*,'(A)') 'LAREDYN0R_EXECUTION_COMPLETE=PASS'",1)
    a.output.write_text(t)
    print(f"ROM_PRACTICAL_P6B_RNP02_MAX_STEP={a.max_step_day}")
