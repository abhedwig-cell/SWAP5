...
def build_gfortran(root: Path, build: Path) -> tuple[Path, list[str]]:
    ...
    for first in ("ttutilprefs.f90", "ttutil.f90"):
        completed = run(
            common_tt + ["-ffree-line-length-none", "-I", str(tt_src), "-c", str(tt_src / first)], objtt
        )
    ...
    for source in sorted(tt_src.glob("*.f90")) + sorted(tt_src.glob("*.for")):
        ...
        completed = run(
            common_tt + [length, "-I", str(tt_src), "-I", str(objtt), "-c", str(source)], objtt
        )
        logs.append(completed.stdout)
        if completed.returncode:
    ...
    completed = run(
        [gfortran, "-o", str(executable), *map(str, sorted(objsw.glob("*.o"))), str(objtt / "libttutil.a")], objsw
    )
    logs.append(completed.stdout)
    if completed.returncode:
